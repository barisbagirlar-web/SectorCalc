...
import type { QuoteInput, QuoteResult } from './engine-api/types.js';
...
const resultEl = document.querySelector('sc-quote-result') as El<{
...
async function renderFromInputs(inputs: QuoteInput) {
  setLoading(true);
  try {
    const response = await requests.run('SC-012', inputs);
    if (!response) return;
    const r: QuoteResult = response.result;
    const w = r.warnings;
    ...
    const marginRatio = totalCost > 0 ? (Number(r.sellPrice) / totalCost).toFixed(2) : '0.00';
    if (reportEl) {
      reportEl.report = buildToolReport({
        metricName: 'Sell/Cost ratio',
        metricValue: marginRatio,
        gaugeMax: 2,
        direction: 'high',
        warn: '1.1',
        crit: '1.0',
        insights: [
          'A ratio below 1.10 leaves almost no room for error.',
          'Scrap, setup and financing are included — verify each.'
        ],
...
form.addEventListener('sc-submit', (e) => {
  const inputs = (e as CustomEvent).detail as QuoteInput;
  (form as unknown as { _last?: QuoteInput })._last = inputs;
...
sensEl.addEventListener('sc-quote-sensitivity', (e) => {
  ...
  const base = (form as unknown as { _last?: QuoteInput })._last ?? DEFAULT_BASE;
