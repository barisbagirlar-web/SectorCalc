...
const buttonPrimaryClass =
  "inline-flex min-h-[44px] w-full items-center justify-center rounded-lg bg-professional-blue px-4 text-sm font-semibold text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50";
...
export function AdminLoginForm() {
  ...
  if (authLoading) {
    return (
      <p className="text-center text-sm text-slate" role="status">
  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-5 rounded-xl border border-slate/20 bg-white p-6 shadow-card sm:p-8">
      ...
      {error ? (
        <p className="text-sm font-medium text-soft-red" role="alert">
          {error}
        </p>
      ) : null}
      ...
      <div className="relative py-1">
        ...
        <p className="relative mx-auto w-fit bg-white px-3 text-xs font-medium uppercase tracking-wide text-slate">
          veya e-posta ile
        </p>
      </div>
...
export function AdminAuthBar() {
  ...
  if (loading) {
    return (
      <div
        className="rounded-xl border border-slate/20 bg-white px-5 py-4 text-sm text-slate shadow-card"
        role="status"
      >
        Oturum kontrol ediliyor…
      </div>
  ...
  if (!user) {
    return (
      <div
        className="flex flex-col gap-3 rounded-xl border border-amber/35 bg-amber/10 px-5 py-4 text-sm text-deep-navy sm:flex-row sm:items-center sm:justify-between"
        role="alert"
      >
        <p>
        ...
        <Link
          href="/login"
          className="inline-flex min-h-[44px] shrink-0 items-center justify-center rounded-lg bg-professional-blue px-4 text-sm font-semibold text-white transition-colors hover:bg-blue-700"
        >
          Giriş yap
        </Link>
      </div>
  ...
  if (!isAdmin) {
    return (
      <div
        className="flex flex-col gap-3 rounded-xl border border-soft-red/25 bg-soft-red/5 px-5 py-4 text-sm text-deep-navy sm:flex-row sm:items-center sm:justify-between"
        role="alert"
      >
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-slate/20 bg-white px-5 py-4 text-sm text-deep-navy shadow-card sm:flex-row sm:items-center sm:justify-between">
      <p>
        <span className="text-slate">Giriş yapıldı:</span>{" "}
