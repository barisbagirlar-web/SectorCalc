...
export function StrategicPremiumRoadmapPanel({ items }: StrategicPremiumRoadmapPanelProps) {
  const t = useTranslations("catalogExplorer.premiumRoadmap");
  ...
  return (
    <div className="min-w-0 space-y-6">
      <div className="space-y-2">
        ...
        <h2 className="sc-craft-headline text-xl sm:text-2xl">{t("title")}</h2>
        <p className="max-w-3xl text-sm leading-relaxed text-body-charcoal">{t("subtitle")}</p>
      </div>
            ...
            <li key={item.id} className="min-w-0">
              <article className="flex h-full flex-col border border-technical-gray bg-white p-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {item.status === "live" ? t("statusLive") : t("statusPlanned")}
                  </span>
                </div>
