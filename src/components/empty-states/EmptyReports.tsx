...
export function EmptyReports({ hasPurchaseCredits = false }: EmptyReportsProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate/30 bg-bg-subtle/50 p-12 text-center">
      ...
      <p className="mb-6 max-w-md text-sm text-slate">
        {hasPurchaseCredits ? t("bodyWithCredits") : t("body")}
      </p>
      <Link
        href={hasPurchaseCredits ? "/premium-tools" : "/free-tools"}
        className="inline-flex min-h-[44px] items-center rounded-xl bg-accent-teal px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-accent-teal/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-teal focus-visible:ring-offset-2"
      >
