...
import "@/styles/jqb-tool.css";

const TOOL_KEY = "job-quote-builder-pro-pack";
...
const GROUPS: Record<number,{n:string;t:string;d:string}> = {
  1: { n:"01", t:"Process time & rates", d:"Cycle and setup time, and the machine/labor/overhead rates that value them." },
  2: { n:"02", t:"Material & batch", d:"Material cost, batch quantity, and annual volume for this job." },
  3: { n:"03", t:"Margin & risk", d:"Target margin, defect allowance, and confidence/uncertainty factors." },
};
...
export default function JobQuoteBuilderProPackToolPage() {
  ...
  const renderField=(id:string)=>{
    const f=FIELDS[id]; const st=fieldStates[id]; const p="jqb";
  ...
  return (
    <div className="jqb-shell">
      <div className="jqb-mast">
        <div className="jqb-kicker">SectorCalc PRO · Manufacturing · Job quoting & pricing</div>
        <h1>Job Quote Builder Pro Pack</h1>
        <p className="jqb-lede">What should this job be quoted at? Computes full job cost, target-margin markup, and a risk-adjusted price — with a sealed proof report.</p>
        <div className="jqb-meta"><span>Engine <b>v5.3.2-domain</b></span><span>Report <b>sealed · SHA-256</b></span></div>
        <div className="jqb-curbar">
          <label htmlFor="jqb-curSel">Report currency</label>
          <select id="jqb-curSel" value={curSym} onChange={e=>setCurSym(e.target.value)} style={{minHeight:"48px"}}>
            {CURRENCIES.map(c=><option key={c.code} value={c.sym}>{c.code} · {c.sym}</option>)}
          </select>
          <span className="jqb-curnote">Symbol only — no exchange-rate conversion.</span>
        </div>
      </div>
      <div className="jqb-bench">
        <div className="jqb-form-col">
          {[1,2,3].map(g=>(
            <div className="jqb-grp" key={g}>
              <div className="jqb-grp-h"><span className="jqb-grp-n">{GROUPS[g].n}</span><span className="jqb-grp-t">{GROUPS[g].t}</span></div>
              <div className="jqb-grp-d">{GROUPS[g].d}</div>
              {Object.keys(FIELDS).filter(id=>FIELDS[id].grp===g).map(renderField)}
            </div>
          ))}
        </div>
        <div className="jqb-rail">
          <div className="jqb-rail-in">
            <div className="jqb-verdict">
              <div className="jqb-verdict-band jqb-neutral">ready</div>
              <div className="jqb-verdict-body">
                <div className="jqb-big">{canGenerate?"✓":"—"}</div>
                <div className="jqb-big-cap">{errorCount?`${errorCount} input(s) need attention`:"all inputs valid — generate report"}</div>
              </div>
            </div>
            {!usageSessionId?(
              <button className="jqb-cta" disabled={sessionLoading||!canGenerate} onClick={requestSession}>
                {sessionLoading?"Checking credits…":"Unlock sealed report · 1 credit"}
              </button>
            ):(
              <button className="jqb-cta" disabled={!canGenerate||isExecuting} onClick={handleGenerate}>
                {isExecuting?"Generating…":"Generate sealed report"}
              </button>
            )}
            {remainingRuns!=null&&usageSessionId!==BYPASS_SESSION_ID&&<div className="jqb-conf">{remainingRuns} run(s) remaining.</div>}
            {sessionError&&<div className="jqb-conf" style={{color:"var(--jqb-neg)"}}>{sessionError}</div>}
            {executeError&&<div className="jqb-conf" style={{color:"var(--jqb-neg)"}}>{executeError}</div>}
          </div>
        </div>
      </div>
      {serverResult&&(
        <div className="jqb-report" ref={reportRef}>
          <div className="jqb-rep-mast">
            <h2>Job Quote Builder Pro Pack — proof report</h2>
            <div className="jqb-rid">SC-PRO · {new Date().toISOString().slice(0,10)}<br/>engine v5.3.2-domain · {serverResult.currency}</div>
          </div>
          <div className="jqb-rep-body">
            <h3 className="jqb-sec-h">Sealed outputs</h3>
            {Object.entries(serverResult.outputs).filter(([k])=>!k.includes("Payload")&&!k.toLowerCase().includes("fmea")&&!k.toLowerCase().includes("trigger")&&!k.toLowerCase().includes("crossing")).slice(0,12).map(([k,v])=>(
              <div className="jqb-stat-row" key={k}><span>{k.replace(/^out_/,"").replace(/_/g," ")}</span><b>{typeof v==="number"?(!isFinite(v)||Math.abs(v)>1e9?"—":v.toFixed(2)):"—"}</b></div>
            ))}
            <div className="jqb-seal">SEAL · {serverResult.seal.hash_algorithm} {serverResult.seal.output_hash}<br/>Sealed at {serverResult.seal.executed_at??"—"}.</div>
            <button type="button" className="jqb-print-btn" onClick={()=>window.print()}>Download PDF</button>
            <div className="jqb-disc">Technical simulation only; not financial, legal, or engineering advice.</div>
