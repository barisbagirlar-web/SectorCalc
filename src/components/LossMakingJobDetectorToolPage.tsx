...
import {
  ...
  type LossMakingJobInputs,
  type LossMakingJobOutputs,
} from "@/sectorcalc/formulas/pro-v531/loss-making-job-detector.formula";
...
import "@/styles/loss-making-job-detector-tool.css";
...
const FIELDS: Record<string, FieldDef> = {
  quoted_job_price:        { ev: "n_quoted_job_price",        dom: "cur", label: "Quoted / planned job price (per unit)", def: 45,   hard: [0, 5e7],  ref: [5, 2000],       refUnit: "", hint: "The price this job is quoted or planned at, per unit.", src: "quote sheet or ERP order line", grp: 1 },
...
function svgOpen(w: number, h: number) { return `<svg class="lmj-chart" viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">`; }

function chartCostBreakdown(o: LossMakingJobOutputs, curSym: string): string {
...
function chartPriceVsCost(x: LossMakingJobInputs, o: LossMakingJobOutputs, curSym: string): string {
  const w = 680, h = 150, padL = 130, padR = 90, padT = 14, rowH = 30, gap = 12;
  const data = [
    { n: "Quoted price", v: x.quotedJobPrice, col: o.out_verdict === 2 ? CCOL.neg : CCOL.pos },
...
function chartTornado(x: LossMakingJobInputs): string {
  const drivers: Array<{ key: keyof LossMakingJobInputs; label: string }> = [
    { key: "quotedJobPrice", label: "Quoted price ±15%" },
  ...
  const bars = drivers.map((d) => {
    const up = { ...x, [d.key]: (x[d.key] as number) * 1.15 } as LossMakingJobInputs;
    const dn = { ...x, [d.key]: (x[d.key] as number) * 0.85 } as LossMakingJobInputs;
...
// loss-making-job-detector.formula.ts's executeFormula — verified identical
// output before shipping.
function calcPure(inp: LossMakingJobInputs): LossMakingJobOutputs {
  ...
  return {
    ...
    out_minimum_acceptable_price: minAcceptablePrice, out_price_gap_to_target: priceGapToTarget,
...
function buildInsights(x: LossMakingJobInputs, o: LossMakingJobOutputs, curSym: string): Insight[] {
  const out: Insight[] = [];
  ...
  out.push({ sev: "info", t: "price gap",
    msg: o.out_price_gap_to_target > 0
      ? `To hit the ${(x.targetMargin * 100).toFixed(0)}% target margin at current cost, the price needs to be at least <strong>${fmtMoney(o.out_minimum_acceptable_price)}</strong> — a gap of ${fmtMoney(o.out_price_gap_to_target)} versus the current quote.`
      : `The current quote already clears the minimum acceptable price of ${fmtMoney(o.out_minimum_acceptable_price)} by ${fmtMoney(-o.out_price_gap_to_target)}.` });
...
interface ServerResultState { outputs: Record<string, number>; seal: ServerSeal; inputs: LossMakingJobInputs; currency: string }

export default function LossMakingJobDetectorToolPage() {
  ...
  const collectedInputs = useMemo((): LossMakingJobInputs | null => {
    ...
    return {
      quotedJobPrice: o.quoted_job_price, machineRate: o.machine_rate, cycleTime: o.cycle_time,
  ...
  const engineResult = useMemo((): LossMakingJobOutputs | null => {
  ...
  const renderField = (id: string) => {
    ...
    return (
      <div className="lmj-f" key={id}>
        <div className="lmj-f-top">
          <label htmlFor={`in_${id}`}>{f.label}</label>
          <span className="lmj-unitline">{canonTxt}</span>
        </div>
        <div className={`lmj-control ${cls}`}>
          <input
            id={`in_${id}`}
            type="number"
            step="any"
          ...
          />
          {f.dom === "cur" && <span className="lmj-prefix">{curSym}{f.refUnit ? <em className="lmj-prefix-unit">{f.refUnit}</em> : null}</span>}
          {f.dom === "pct" && <span className="lmj-prefix" style={{ borderLeft: "1px solid var(--lmj-line)", borderRight: "none" }}>%</span>}
          {f.units && (
            <select
              aria-label={`Unit for ${f.label}`}
              value={st.unit}
              onChange={(e) => updateFieldUnit(id, e.target.value)}
              style={{ borderLeft: "1px solid var(--lmj-line)", background: "var(--lmj-panel)", fontFamily: "var(--lmj-mono)", fontSize: "14px", padding: "8px", minHeight: "48px", color: "var(--lmj-ink)" }}
            >
              {f.units.map((u) => <option key={u.k} value={u.k}>{u.l}</option>)}
            </select>
          )}
        </div>
        <div className="lmj-f-foot">
          <span className="lmj-hint">{f.hint} <em style={{ fontStyle: "normal", color: "var(--lmj-faint)" }}>· {f.src}</em></span>
          <span className="lmj-bench-ref">{fmtRef(f)}</span>
        </div>
        {msg && <div id={msgId} className={`lmj-msg ${msgCls}`} role={st.error ? "alert" : "status"}>{msg}</div>}
      </div>
    );
  };
  ...
  const renderReport = () => {
    ...
    const sealed: LossMakingJobOutputs = {
      ...
      out_price_gap_to_target: s.out_price_gap_to_target,
    ...
    return (
      <div className="lmj-report" ref={reportRef}>
        <div className="lmj-rep-mast">
          <h2>Loss-Making Job Detector — proof report</h2>
          <div className="lmj-rid">
            SC-PRO-LMJ · {new Date().toISOString().slice(0, 10)}<br />
            engine v5.3.2-domain · cost-threshold comparison<br />
            currency {repCur}
          </div>
        </div>
        <div className="lmj-rep-body">
          <div className="lmj-sec">
            <div className={`lmj-verdict-box lmj-verdict-${sealed.out_verdict}`}>
              <div className="lmj-head">Verdict: {verdictLabel[sealed.out_verdict]}.</div>
              <p>Quoted at <strong>{fmtMoney(rx.quotedJobPrice)}</strong> against a true cost of <strong>{fmtMoney(sealed.out_total_cost_per_unit)}</strong> per unit — a gross margin of <strong>{fmtMoney(sealed.out_gross_margin_per_unit)}</strong> ({(sealed.out_contribution_margin_pct * 100).toFixed(1)}% contribution margin).</p>
              {sealed.out_verdict === 2 && <p>Annualized exposure at the expected volume: <strong>{fmtMoney(sealed.out_money_at_risk)}</strong>.</p>}
            </div>
          </div>

          <div className="lmj-sec">
            <div className="lmj-sec-h"><span className="lmj-sec-n">1</span><span className="lmj-sec-t">Unit cost breakdown</span></div>
            <div dangerouslySetInnerHTML={{ __html: chartCostBreakdown(sealed, repCur) }} />
            <div className="lmj-note">Every component of the true cost per unit, summing to {fmtMoney(sealed.out_total_cost_per_unit)}.</div>
          </div>

          <div className="lmj-sec">
            <div className="lmj-sec-h"><span className="lmj-sec-n">2</span><span className="lmj-sec-t">Price vs. cost</span></div>
            <div dangerouslySetInnerHTML={{ __html: chartPriceVsCost(rx, sealed, repCur) }} />
            <div className="lmj-note">Quoted price against true cost and the price needed to hit target margin.</div>
          </div>

          <div className="lmj-sec">
            <div className="lmj-sec-h"><span className="lmj-sec-n">3</span><span className="lmj-sec-t">Sensitivity — what moves the margin</span></div>
            <div dangerouslySetInnerHTML={{ __html: chartTornado(rx) }} />
            <div className="lmj-note">Gross-margin-per-unit swing when each input is stressed ±15%.</div>
          </div>

          <div className="lmj-sec">
            <div className="lmj-sec-h"><span className="lmj-sec-n">4</span><span className="lmj-sec-t">Engineering insights</span></div>
            {ins.map((i, k) => (
              <div className={`lmj-ins lmj-${i.sev}`} key={k}>
                <span className="lmj-t">{i.t}</span>
                <span dangerouslySetInnerHTML={{ __html: i.msg }} />
              </div>
            ))}
          </div>

          <div className="lmj-seal">
            SEAL · {serverResult.seal.hash_algorithm} {seal}<br />
            Sealed server-side at {serverResult.seal.executed_at ?? "—"}.
          </div>
          <button
            type="button"
            className="lmj-print-btn"
            onClick={() => window.print()}
            aria-label="Download this report as a PDF"
          >
            Download PDF
          </button>
          <div className="lmj-disc">
          ...
          <PremiumReportFeedback
            ...
            inputSnapshot={{
              quoted_job_price: rx.quotedJobPrice, machine_rate: rx.machineRate, cycle_time: rx.cycleTime,
  ...
  return (
    <div className="lmj-shell">
      <div className="lmj-mast">
        <div className="lmj-kicker">SectorCalc PRO · Job Costing · Go / no-go decision</div>
        <h1>Loss-Making Job Detector</h1>
        <p className="lmj-lede">Compares a quoted price against the true fully-loaded cost per unit — machine, labor, overhead, material, defect, and amortized setup — and flags whether the job actually makes money.</p>
        <div className="lmj-meta">
          <span>Engine <b>v5.3.2-domain</b></span>
          <span>Method <b>cost-threshold comparison</b></span>
          <span>Report <b>sealed · SHA-256</b></span>
        </div>
        <div className="lmj-curbar">
          <label htmlFor="lmj-curSel">Report currency</label>
          <select id="lmj-curSel" value={curSym} onChange={(e) => setCurSym(e.target.value)}>
            {CURRENCIES.map((c) => <option key={c.code} value={c.sym}>{c.code} · {c.sym}</option>)}
          </select>
          <span className="lmj-curnote">Symbol only — no exchange-rate conversion. Enter every figure in the same currency.</span>
        </div>
      </div>

      <div className="lmj-bench">
        <div className="lmj-form-col">
          {[1, 2, 3, 4].map((g) => (
            <div className="lmj-grp" key={g}>
              <div className="lmj-grp-h"><span className="lmj-grp-n">{GROUPS[g].n}</span><span className="lmj-grp-t">{GROUPS[g].t}</span></div>
              <div className="lmj-grp-d">{GROUPS[g].d}</div>
              {Object.keys(FIELDS).filter((id) => FIELDS[id].grp === g).map(renderField)}
            </div>
          ))}
        </div>
        <div className="lmj-rail">
          <div className="lmj-rail-in">
            <div className="lmj-verdict">
              <div className={`lmj-verdict-band ${r ? ["lmj-pos", "lmj-warn2", "lmj-neg"][r.out_verdict] : "lmj-warn2"}`}>
                {r ? verdictLabel[r.out_verdict] : "incomplete"}
              </div>
              <div className="lmj-verdict-body">
                <div className="lmj-big">{r ? <>{curSym}{fmt(r.out_gross_margin_per_unit)} <small>margin/unit</small></> : "—"}</div>
                <div className="lmj-big-cap">
                  {r
            ...
            <div className="lmj-stat"><span>Total cost per unit</span><b>{r ? curSym + fmt(r.out_total_cost_per_unit) : "—"}</b></div>
            <div className="lmj-stat"><span>Minimum acceptable price</span><b>{r ? curSym + fmt(r.out_minimum_acceptable_price) : "—"}</b></div>
            <div className="lmj-stat"><span>Money at risk (annualized)</span><b>{r ? curSym + fmt(r.out_money_at_risk) : "—"}</b></div>

            {!usageSessionId ? (
              <button className="lmj-cta" disabled={sessionLoading || !canGenerate} onClick={requestSession}>
                {sessionLoading ? "Checking credits…" : "Unlock sealed report · 1 credit"}
              </button>
            ) : (
              <button className="lmj-cta" disabled={!canGenerate || isExecuting} onClick={handleGenerate}>
                {isExecuting ? "Generating…" : "Generate sealed report"}
              </button>
            )}
            {remainingRuns != null && usageSessionId !== BYPASS_SESSION_ID && (
              <div className="lmj-conf">{remainingRuns} run(s) remaining this session.</div>
            )}
            {sessionError && <div className="lmj-conf" style={{ color: "var(--lmj-neg)" }}>{sessionError}</div>}
            {executeError && <div className="lmj-conf" style={{ color: "var(--lmj-neg)" }}>{executeError}</div>}
            <div className="lmj-conf">
              <span className="lmj-d" style={{ background: r ? ["var(--lmj-pos)", "var(--lmj-warn2)", "var(--lmj-neg)"][r.out_verdict] : "var(--lmj-warn2)" }} />
