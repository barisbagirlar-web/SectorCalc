...
function resolveSearchParam(value: string | null): string | undefined {
  return value && value.length > 0 ? value : undefined;
}

export function CatalogHubToolsClientPanel({
  locale,
  tools,
  variant,
}: CatalogHubToolsClientPanelProps) {
    ...
    <div className="min-w-0 flex-1" id="tools-list">
      <header className="mb-4">
        <h2 className="text-2xl font-semibold text-gray-900">
          {activeSector
            ? t(`labels.${variant}.categoryToolsTitle`, { category: activeSectorLabel })
            : allLabel}
        </h2>
