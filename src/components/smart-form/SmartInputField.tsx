...
export function SmartInputField({
  ...
  return (
    <div className="sc-industrial-field min-w-0" data-field-key={input.key}>
      ...
      <div className="relative min-w-0">
        ...
        <input
          ...
          className={`sc-ledger-input-underline min-h-[44px] w-full min-w-0${isCurrency ? " pl-5" : ""}${error ? " sc-ledger-input--error" : ""}`}
        />
      </div>
      {help ? (
        <p id={helperId} className="sc-industrial-field__helper line-clamp-2">
          {help}
        </p>
      ) : null}
      {error ? (
