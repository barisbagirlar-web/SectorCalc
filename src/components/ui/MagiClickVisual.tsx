...
export function MagiClickVisual({ variant, className = "" }: MagiClickVisualProps) {
  const base =
    `relative overflow-hidden rounded-lg border border-border-subtle bg-gradient-to-br from-slate-100 via-white to-blue-50 ${className}`;
  ...
  if (variant === "platform-dashboard") {
    return (
      <div className={`${base} min-h-[320px] w-full max-w-[900px] shadow-md ${className}`} aria-hidden>
        <div className="absolute inset-x-0 top-0 flex h-10 items-center gap-2 border-b border-border-subtle bg-white/90 px-4">
        ...
        <div className="absolute inset-0 top-10 grid grid-cols-12 gap-3 p-4">
          <div className="col-span-3 space-y-2 rounded-md bg-bg-primary/90 p-3">
            {[1, 2, 3, 4, 5].map((i) => (
          ...
          <div className="col-span-9 space-y-3">
            ...
            <div className="h-24 rounded-md border border-border-subtle bg-white p-3">
          ...
          <div className="mt-4 space-y-2">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="flex gap-2">
                <span className="flex h-6 w-6 items-center justify-center rounded bg-bg-primary text-[10px] font-bold text-white">
                  {i}
                </span>
  ...
  return (
    <div className={`${base} aspect-[5/4] w-full`} aria-hidden>
      ...
      <div className="absolute left-1/2 top-1/2 h-12 w-12 -translate-x-1/2 -translate-y-1/2 rounded-lg bg-bg-primary" />
    </div>
  );
}
