...
const T: Record<string, any> = {
  ...
  es: {
    ...
    res_docs:'Referencia de fórmulas', res_docs_d:'Normes & fuentes',
...
const INDUSTRY_GROUPS = [
  { groupEn:'Production', groupTr:'Üretim', items:[
    { slug:'manufacturing', icon:'🏭', en:'Manufacturing', count: 48 },
  ...
  { groupEn:'Engineering', groupTr:'Mühendislik', items:[
    { slug:'mechanical-hvac',  icon:'⚙️', en:'Mechanical & HVAC', count: 42 },
    { slug:'electrical-power', icon:'⚡', en:'Electrical & Power', count: 18 },
    { slug:'construction',     icon:'🏗️', en:'Construction',       count: 26 },
  ]},
  { groupEn:'Operations', groupTr:'Operasyon', items:[
    { slug:'supply-chain',     icon:'🚚', en:'Supply Chain',   count: 21 },
    { slug:'energy-esg',       icon:'🌱', en:'Energy & ESG',    count: 16 },
    { slug:'technology-cloud', icon:'☁️', en:'Technology & AI', count: 19 },
  ]},
];
  ...
  return (
    <>
      <style>{`
        ...
        .sc-lang{position:relative;display:flex;align-items:center;gap:8px;}
        ...
        .sc-draw-lang{display:flex;flex-wrap:wrap;gap:8px;padding:16px 24px;border-top:1px solid var(--border);align-items:center;}
      `}</style>
      ...
      <header className="sc-h" dir={dir}>
        <div className="sc-inner">
              ...
              <div className="sc-megawrap" onMouseEnter={() => openWithIntent(openMenu)} onMouseLeave={closeWithIntent}>
                {openMenu === 'products' && (
                  <div className="sc-mega sc-mega-products">
                    ...
                    <Link href="/premium-tools" className="sc-mega-panel">
                ...
                {openMenu === 'industries' && (
                  <div className="sc-mega sc-mega-industries">
                    <div className="sc-mega-grid">
                      {INDUSTRY_GROUPS.map((g) => (
                        <div className="sc-mega-col" key={g.groupEn}>
                          ...
                          {g.items.map((it) => (
                            <Link key={it.slug} href={`/free-tools?category=${it.slug}`} className="sc-mega-item">
                    ...
                    <div className="sc-mega-foot">
                      <Link href="/free-tools">{t.view_all_industries} →</Link>
                      <span className="promo">18 sectors · {freeToolsCount + proToolsCount} {t.tools}</span>
                    </div>
                  </div>
                )}
                {openMenu === 'resources' && (
          ...
          <div className="sc-right">
            {/* The existing layout components for Locale and Region exactly as requested */}
            <div className="sc-lang" aria-label="Region and Language">
              <div className="hidden xl:block" style={{ transform: 'scale(0.9)', transformOrigin: 'right' }}>
            ...
            <Link href="/auth/signup" className="sc-getstarted">{t.getStarted}</Link>
        ...
        <div className={`sc-drawer${mobileOpen ? ' open' : ''}`} dir={dir}>
          <div className="sc-draw-sec">
            ...
            {mobileSection === 'products' && (
              <div className="sc-draw-body">
                ...
                <Link href="/premium-tools" onClick={() => setMobileOpen(false)}>⚡ {t.col_pro} <span className="c">{proToolsCount}</span></Link>
              </div>
            )}
          </div>
          <div className="sc-draw-sec">
            ...
            {mobileSection === 'industries' && (
              <div className="sc-draw-body">
                {INDUSTRY_GROUPS.flatMap((g) => g.items).map((it) => (
                  <Link key={it.slug} href={`/free-tools?category=${it.slug}`} onClick={() => setMobileOpen(false)}>
                    {it.icon} {it.en} <span className="c">{it.count}</span>
                  </Link>
                ))}
                <Link href="/free-tools" onClick={() => setMobileOpen(false)} style={{ color: 'var(--accent)', fontWeight: 600 }}>{t.view_all_industries} →</Link>
              </div>
            )}
          </div>
          ...
          <div className="sc-draw-cta">
            <Link href="/auth/signup" className="sc-getstarted" onClick={() => setMobileOpen(false)}>{t.getStarted}</Link>
