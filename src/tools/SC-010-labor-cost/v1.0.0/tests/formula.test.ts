...
describe('SC-010 formula', () => {
  it('true cost > net', () => {
    expect(D(calculate(US).trueMonthlyCost).gt(3500)).toBe(true);
  });
  ...
  it('monotonic: higher net -> higher cost', () => {
    expect(
      D(calculate({ ...US, netSalary: 2000 }).trueMonthlyCost).gt(
        calculate({ ...US, netSalary: 1000 }).trueMonthlyCost
      )
    ).toBe(true);
  });
  it('weekly normalization', () => {
    expect(
      D(calculate({ country: 'US', netSalary: 800, payFrequency: 'weekly' }).grossMonthly).gt(800)
    ).toBe(true);
  });
  it('biweekly normalization', () => {
    expect(
      D(calculate({ country: 'US', netSalary: 1600, payFrequency: 'biweekly' }).grossMonthly).gt(
        1600
      )
    ).toBe(true);
  });
  it('hourly normalization', () => {
    expect(
      D(
        calculate({ country: 'US', netSalary: 20, payFrequency: 'hourly', hoursPerWeek: 40 })
          .grossMonthly
      ).gt(0)
    ).toBe(true);
  });
  it('employerSSRate override raises cost', () => {
    expect(
      D(calculate({ ...US, employerSSRate: 0.3 }).trueMonthlyCost).gt(
        calculate({ ...US, employerSSRate: 0.1 }).trueMonthlyCost
      )
    ).toBe(true);
  });
  it('employeeRate override raises gross', () => {
    expect(
      D(calculate({ ...US, employeeRate: 0.4 }).grossMonthly).gt(
        calculate({ ...US, employeeRate: 0.1 }).grossMonthly
      )
    ).toBe(true);
  });
  it('throws hoursPerWeek <= 0', () => {
    expect(() => calculate({ ...US, hoursPerWeek: 0 })).toThrow();
  });
  it('throws tenureYears <= 0', () => {
    expect(() => calculate({ ...US, tenureYears: 0, recruitmentCost: 1000 })).toThrow();
  });
  ...
  it('throws unknown country', () => {
    expect(() => calculate({ country: 'XX', netSalary: 1000, payFrequency: 'monthly' })).toThrow();
  });
