...
describe('SC-001 formula', () => {
  it('monotonic: higher load -> larger required throat', () => {
    expect(
      D(calculate({ ...base, designLoadN: 100000 }).requiredThroatMm).gt(
        calculate(base).requiredThroatMm
      )
    ).toBe(true);
  });
  ...
  it('throws weldLength 0', () => {
    expect(() => calculate({ ...base, weldLengthMm: 0 })).toThrow();
  });
  it('throws weldStrength 0', () => {
    expect(() => calculate({ ...base, weldStrengthMpa: 0 })).toThrow();
  });
  ...
  it('throws on negative designLoadN', () => {
    expect(() => calculate({ ...base, designLoadN: -1 })).toThrow();
  });
});
