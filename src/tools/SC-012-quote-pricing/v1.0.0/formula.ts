...
export function calculate(inputs: QuoteInputs): QuoteResult {
  ...
  steps.push({
    step: ++n,
    description: 'Material cost with scrap',
    formula: `material / (1 - ${scrap.toString()})`,
    result: money(effMaterial)
  });

  // 2. labor / machine / setup
  const labor = requireNonNegative(inputs.laborHours, 'laborHours').times(
    requireNonNegative(inputs.laborHourlyCost, 'laborHourlyCost')
  );
  const machine = requireNonNegative(inputs.machineHours, 'machineHours').times(
    requireNonNegative(inputs.machineHourlyCost, 'machineHourlyCost')
  );
  ...
  steps.push({
    step: ++n,
    description: 'Labor + machine + setup',
    formula: 'laborH*rate + machineH*rate + (setupMin/60)*setupRate',
    result: money(labor.plus(machine).plus(setup))
  });
  ...
  steps.push({
    ...
    formula: 'effMaterial+labor+machine+setup+energy+consumables+shipping',
    result: money(direct)
  });
  ...
  steps.push({
    ...
    formula: `direct * ${overheadRate.toString()}`,
    result: money(overhead)
  });
  ...
  steps.push({
    step: ++n,
    description: 'Finance charge on payment terms',
    formula: `costBeforeFinance * ${monthRate.toString()} * (days/30)`,
    result: money(finance)
  });
  ...
  steps.push({
    step: ++n,
    description: 'TOTAL COST (conservation-locked)',
    formula: 'sum of 9 components',
  ...
  steps.push({
    step: ++n,
    description: 'Sell price with target margin',
    formula: `totalCost / (1 - ${margin.toString()})`,
    result: money(sellPrice)
  });
