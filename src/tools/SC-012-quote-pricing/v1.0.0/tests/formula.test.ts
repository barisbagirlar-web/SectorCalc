...
describe('SC-012 formula', () => {
  ...
  it('monotonic: higher margin -> higher sellPrice', () => {
    expect(
      D(calculate({ ...base, targetMargin: 0.4 }).sellPrice).gt(
        calculate({ ...base, targetMargin: 0.1 }).sellPrice
      )
    ).toBe(true);
  });
  it('monotonic: higher scrap -> higher sellPrice', () => {
    expect(
      D(calculate({ ...base, scrapRate: 0.3 }).sellPrice).gt(
        calculate({ ...base, scrapRate: 0.05 }).sellPrice
      )
    ).toBe(true);
  });
  ...
  it('zero payment days -> zero finance charge', () => {
    expect(calculate({ ...base, paymentDays: 0, monthlyInterestRate: 0.02 }).financeCharge).toBe(
      '0.00'
    );
  });
