...
register({
  ...
  sections: [
    section("Primary Results", 10, [
      entry("out_contribution_margin_pct", "Job Margin", "percentage"),
      entry("out_total_cost_per_unit", "Total Cost per Unit", "currency", "USD"),
      entry("out_money_at_risk", "Annualized Loss Exposure", "currency", "USD"),
    ]),
    section("Risk Assessment", 20, [
      entry("out_minimum_acceptable_price", "Minimum Acceptable Price", "currency", "USD"),
      entry("out_price_gap_to_target", "Price Gap to Target", "currency", "USD"),
      entry("out_gross_margin_per_unit", "Gross Margin per Unit", "currency", "USD"),
    ]),
    section("Quality & Decision", 30, [
      entry("out_evidence_completeness", "Input Confidence Score", "ratio"),
      entry("out_verdict", "Go / Review / Block Decision", "number"),
    ]),
  ],
  paretoBreakdown: {
    ...
    segments: [
      ...
      { outputId: "out_setup_cost_component", label: "Setup (amortized)" },
    ],
  },
  sensitivityTargetOutput: "out_contribution_margin_pct",
  sensitivityDrivers: [
...
register({
  ...
  sections: [
    section("Primary Results", 10, [
      entry("out_carrying_cost", "AR Carrying Cost", "currency", "USD"),
      entry("out_financing_cost_pct_of_revenue", "Financing Cost (% of Revenue)", "percentage"),
      entry("out_collection_days", "Collection Days (DSO)", "number", "days"),
    ]),
    section("Risk Assessment", 20, [
      entry("out_implied_ar_from_dso", "AR Implied by DSO", "currency", "USD"),
      entry("out_ar_consistency_gap_pct", "AR Consistency Gap", "percentage"),
      entry("out_savings_per_day_dso_reduction", "Savings per Day of DSO Reduction", "currency", "USD"),
    ]),
    section("Quality & Decision", 30, [
      entry("out_evidence_completeness", "Input Confidence Score", "ratio"),
      entry("out_verdict", "Go / Review / Block Decision", "number"),
  ...
  sensitivityTargetOutput: "out_financing_cost_pct_of_revenue",
  sensitivityDrivers: [
  ...
  insights: [
    {
      ...
      severity: "critical",
      when: (o) => o.out_verdict === 2,
      message: (o) => `The financing cost of these payment terms exceeds the review threshold (${(o.out_carrying_cost ?? 0).toLocaleString("en-US", {maximumFractionDigits: 0})}/yr). Either shorten the terms or price the credit cost into the quote.`,
    },
    {
...
register({
  ...
  sections: [
    section("Primary Results", 10, [
      entry("out_annual_savings", "Annual Savings", "currency", "USD"),
      entry("out_payback_months", "Payback Period", "number", "months"),
      entry("out_roi_pct", "Return on Investment", "percentage"),
    ]),
    section("Risk Assessment", 20, [
      entry("out_annual_hours_reclaimed", "Annual Hours Reclaimed", "number", "hrs"),
      entry("out_money_at_risk", "SMED Investment Committed", "currency", "USD"),
    ]),
    section("Quality & Decision", 30, [
      entry("out_evidence_completeness", "Input Confidence Score", "ratio"),
      entry("out_verdict", "Go / Review / Block Decision", "number"),
  ...
  paretoBreakdown: {
    title: "Reclaimed Capacity Value Breakdown",
    segments: [
      { outputId: "out_machine_share_component", label: "Machine" },
      { outputId: "out_labor_share_component", label: "Labor" },
      { outputId: "out_overhead_share_component", label: "Overhead" },
    ],
  },
  sensitivityTargetOutput: "out_annual_savings",
  sensitivityDrivers: [
    { inputId: "n_machine_rate", label: "Machine Hourly Rate" },
    { inputId: "n_setup_time", label: "Batch Setup Time" },
    { inputId: "n_batch_quantity", label: "Batch Quantity" },
...
register({
  ...
  sections: [
    section("Primary Results", 10, [
      entry("out_contribution_margin_pct", "Contribution Margin", "percentage"),
      entry("out_total_cost_per_unit", "Total Cost per Unit", "currency", "USD"),
      entry("out_annual_profit_contribution", "Annual Profit Contribution", "currency", "USD"),
    ]),
    section("Risk Assessment", 20, [
      entry("out_minimum_acceptable_price", "Minimum Acceptable Price", "currency", "USD"),
      entry("out_gross_margin_per_unit", "Gross Margin per Unit", "currency", "USD"),
  ...
  sensitivityTargetOutput: "out_annual_profit_contribution",
  sensitivityDrivers: [
    { inputId: "n_machine_rate", label: "Machine Hourly Rate" },
    { inputId: "n_cycle_time", label: "Cycle Time per Unit" },
    { inputId: "n_setup_time", label: "Batch Setup Time" },
  ...
  insights: [
    {
      ...
      severity: "critical",
      when: (o) => o.out_verdict === 2,
      message: (o) => `This SKU's contribution margin is below target at current volume -- annualized profit contribution is ${(o.out_annual_profit_contribution ?? 0).toLocaleString("en-US", {maximumFractionDigits: 0})}. Reprice or delist before scaling volume further.`,
    },
    {
...
register({
  ...
  sections: [
    section("Primary Results", 10, [
      entry("out_npv", "Net Present Value", "currency", "USD"),
      entry("out_irr", "Internal Rate of Return", "percentage"),
      entry("out_profitability_index", "Profitability Index", "ratio"),
    ]),
    section("Risk Assessment", 20, [
      entry("out_payback_years", "Simple Payback Period", "number", "yrs"),
      entry("out_discounted_payback_years", "Discounted Payback Period", "number", "yrs"),
      entry("out_irr_minus_hurdle", "IRR Margin over Hurdle Rate", "percentage"),
    ]),
    section("Quality & Decision", 30, [
      entry("out_evidence_completeness", "Input Confidence Score", "ratio"),
      entry("out_decision", "Accept / Reject Decision", "number"),
    ]),
  ],
  sensitivityTargetOutput: "out_npv",
  sensitivityDrivers: [
    { inputId: "n_capex", label: "CAPEX -- Initial Investment" },
    { inputId: "n_discount_rate", label: "Discount Rate / Hurdle Rate" },
    { inputId: "n_study_years", label: "Analysis Period" },
    { inputId: "n_annual_cash_flow", label: "Annual Net Cash Flow (Year 1)" },
    { inputId: "n_cash_flow_growth_rate", label: "Cash Flow Growth Rate" },
    { inputId: "n_residual_value", label: "Residual / Salvage Value" },
    { inputId: "n_working_capital", label: "Incremental Working Capital" },
  ],
  insights: [
    {
      id: "capital-equipment-investment-appraisal-npv-irr-critical",
      severity: "critical",
      when: (o) => o.out_decision === 0,
      message: (o) => `Net present value (${(o.out_npv ?? 0).toLocaleString("en-US", {maximumFractionDigits: 0})}) does not clear the hurdle -- this investment destroys value at the current discount rate and cash flow assumptions.`,
    },
    {
      id: "capital-equipment-investment-appraisal-npv-irr-favorable",
      severity: "opportunity",
      when: (o) => o.out_decision === 1,
      message: () => `NPV is positive and IRR clears the hurdle rate -- this investment creates value at the assumptions entered.`,
    },
  ],
});
