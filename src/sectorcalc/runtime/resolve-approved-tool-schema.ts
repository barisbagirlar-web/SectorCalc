...
import { getBarisProSchema } from "@/sectorcalc/runtime/baris-schema-loader";
...
export function resolveApprovedToolSchema(toolKey: string): ApprovedSchemaResult {
  ...
  function buildAndCache(buildFn: () => SuperV4Schema, source: ResolvedSource): ApprovedSchemaResult {
    ...
    try {
      const built = buildFn();
      superV4 = source === "free_v531" ? applyCertifiedFreeCalculationContract(built) : built;
    } catch (err: unknown) { return { ok: false, reason: "CAUGHT_EXCEPTION", errors: ["Build threw: " + (err instanceof Error ? err.message : String(err))] }; }
