...
function chartTheme() {
  ...
  return {
    ...
    amber: P.amber,
    red: P.red,
    greenFill: P.greenFill,
    amberFill: P.amberFill,
    redFill: P.redFill,
    blueFill: P.blueFill
  };
}
...
const presets = {
  standard: {
    ...
    seed: 12345,
    dims: [
      { name: 'Spacer Width', nominal: 25, tolerance: 0.05, dist: 'normal' },
      { name: 'Bearing OD', nominal: 30, tolerance: 0.025, dist: 'normal' },
      { name: 'Housing Bore', nominal: -55, tolerance: 0.04, dist: 'normal' }
    ]
  },
  tight: {
    specUpper: 0.05,
    ...
    seed: 54321,
    dims: [
      { name: 'Ti Spacer', nominal: 15, tolerance: 0.01, dist: 'truncated_normal' },
      { name: 'Ball Bearing', nominal: 20, tolerance: 0.005, dist: 'truncated_normal' },
      { name: 'Al Housing', nominal: -35, tolerance: 0.015, dist: 'normal' }
    ]
  },
  loose: {
    specUpper: 0.3,
    ...
    seed: 99999,
    dims: [
      { name: 'Steel Plate', nominal: 50, tolerance: 0.2, dist: 'uniform' },
      { name: 'Bushing', nominal: 25, tolerance: 0.1, dist: 'uniform' },
      { name: 'Cast Housing', nominal: -75, tolerance: 0.25, dist: 'triangular' }
    ]
  },
  vtight: {
    specUpper: 0.025,
    ...
    seed: 77777,
    dims: [
      { name: 'Implant Spacer', nominal: 10, tolerance: 0.005, dist: 'truncated_normal' },
      { name: 'Ceramic Bearing', nominal: 8, tolerance: 0.003, dist: 'truncated_normal' },
      { name: 'Ti Housing', nominal: -18, tolerance: 0.008, dist: 'truncated_normal' }
    ]
  }
};
let currentUnit = 'mm';
...
function validate() {
  ...
  if (isNaN(cpkT) || cpkT < 0.5 || cpkT > 3) {
    $('fld-cpkTarget').classList.add('has-error');
    $('val-cpkTarget').className = 'sc-val error';
    $('val-cpkTarget').textContent = 'X target 0.5-3.0';
    ok = false;
  } else {
    $('fld-cpkTarget').classList.remove('has-error');
    $('val-cpkTarget').className = 'sc-val ok';
    $('val-cpkTarget').textContent = cpkT >= 1.67 ? 'OK tight target' : 'OK';
  }
  dimensions.forEach((d) => {
    if (!(d.tolerance > 0)) ok = false;
  });
  return ok;
}
...
function compute() {
  ...
  $('liveSub').innerHTML =
    '<span>' +
    b.dims.length +
    ' dims</span><span>pred. Cpk ' +
    cpk.toFixed(2) +
    '</span><span>' +
    (rssInSpec ? 'predicted in spec' : 'predicted OUT') +
    '</span>';
  syncReportIfOpen();
}
...
function loadPreset(key) {
  const p = presets[key];
  $('specUpper').value = p.specUpper;
  $('specLower').value = p.specLower;
  $('cpkTarget').value = p.cpkTarget;
  $('mcSeed').value = p.seed;
  $('unitSpec').value = 'mm';
  $('unitSpec2').value = 'mm';
  currentUnit = 'mm';
...
function radarAxes(d) {
  ...
  const mcRssAgree = Math.max(
    0,
    Math.min(1, 1 - Math.abs(d.mcSpread - d.rssTol) / Math.max(d.rssTol, 1e-9))
  );
...
function generateReport(_opts) {
  ...
  const histBars = d.hist
    .map((h, i) => {
      const bw = 500 / d.hist.length;
      const bh = (h.c / maxC) * 140;
      return `<rect x="${50 + i * bw}" y="${150 - bh}" width="${bw - 1}" height="${bh}" fill="${TH.blue}" opacity="0.8"/>`;
    })
    .join('');
  ...
  $('reportArea').innerHTML = `
    ...
    <div class="sc-sec"><div class="sc-sec-hd">Method Stack Comparison</div><div class="sc-chart"><div class="sc-stack">
      ${[
        ['Worst', d.wcTol, TH.red, 1],
        ['RSS', d.rssTol, TH.amber, 1],
        ['Monte Carlo', d.mcSpread, TH.blue, 1],
        ['Spec dev', d.specHalf, TH.green, 0.25]
      ]
        .map(
          ([l, v, c, o]) =>
            `<div class="sc-stack-col"><div class="sc-stack-wrap"><div class="sc-stack-bar" style="height:${Math.min(100, (v / d.specHalf) * 100)}%;background:${c};opacity:${o}"><span class="sc-stack-bar-lbl">+/-${(v * u).toFixed(3)}</span></div></div><div class="sc-stack-lbl">${l}</div></div>`
        )
        .join('')}
    </div></div></div>
        ...
        .map((f) => {
          const pts = axes
            .map((_, i) => {
              ...
              return `${150 + f * 110 * Math.cos(a)},${150 + f * 110 * Math.sin(a)}`;
            })
            .join(' ');
          return `<polygon points="${pts}" fill="none" stroke="${TH.grid}"/>`;
        })
        .join('')}
      ${axes
        .map((_, i) => {
          const a = ((i * 72 - 90) * Math.PI) / 180;
          return `<line x1="150" y1="150" x2="${150 + 110 * Math.cos(a)}" y2="${150 + 110 * Math.sin(a)}" stroke="${TH.grid}"/>`;
        })
        .join('')}
      <polygon points="${axes
        .map((c, i) => {
        ...
        .join(
          ' '
        )}" fill="${overall === 'PASS' ? TH.greenFill : overall === 'WARNING' ? TH.amberFill : TH.redFill}" stroke="${gCol}" stroke-width="2"/>
      ${axes
        .map((c, i) => {
          const a = ((i * 72 - 90) * Math.PI) / 180;
          const x = 150 + 132 * Math.cos(a),
            y = 150 + 132 * Math.sin(a);
          return `<text x="${x}" y="${y}" text-anchor="middle" fill="${TH.text}" font-size="9" font-weight="600">${c.name}<title>${c.name}: ${c.note} = ${(c.val * 100).toFixed(0)}%</title></text>`;
        })
        .join('')}
    ...
    <div class="sc-sec"><div class="sc-sec-hd">What-If (predicted Cpk after 10% tightening, recomputed)</div><div class="sc-card"><div style="font-size:11px;color:var(--text-muted);margin-bottom:14px;font-family:var(--font-mono)">Cost impact NOT estimated — needs your process cost model.</div>
      <div class="sc-whatif">${whatIfs
        .map((w) => {
          const dc = w.cpk - d.cpk;
          return `<div class="sc-whatif-card"><div class="sc-whatif-lbl">${w.name}</div><div class="sc-whatif-val">Cpk ${w.cpk.toFixed(2)}</div><div class="sc-whatif-chg ${dc > 0 ? 'pos' : dc < 0 ? 'neg' : 'neu'}">${dc >= 0 ? '+' : ''}${dc.toFixed(2)} | ${w.ppm.toFixed(0)} PPM</div></div>`;
        })
        .join('')}</div>
    </div></div>
  ...
  on('shareBtn', () => {
    navigator.clipboard
      .writeText(makeShareURL(location.origin, toProjectState()))
      .then(() => alert('Share URL copied (tamper-evident integrity hash attached)'));
  });
  const snap = (slot) => {
    window['__snap' + slot] = {
      cpk: d.cpk,
      ...
      rss: d.rssTol,
      worst: d.wcTol,
      label: 'Snap ' + slot + ' (' + d.calcId + ')'
    };
    alert('Saved ' + window['__snap' + slot].label);
    if (window.__snapA && window.__snapB) showCompare();
  };
  on('snapABtn', () => snap('A'));
  on('snapBBtn', () => snap('B'));
  function showCompare() {
    ...
    const tbl = rows
      .map(
        (r) =>
          `<tr><td class="td-name">${r.metric}</td><td>${r.a.toFixed(3)}</td><td>${r.b.toFixed(3)}</td><td class="${r.better ? 'td-ok' : 'td-high'}">${r.delta >= 0 ? '+' : ''}${r.delta.toFixed(3)}</td></tr>`
      )
      .join('');
...
function exportPDF(graphic) {
  ...
  const line = (t, s, c, b) => {
    ...
    doc.setTextColor(c || '#222');
    doc.setFont('helvetica', b ? 'bold' : 'normal');
    const ls = doc.splitTextToSize(t, 500);
    doc.text(ls, 48, y);
    y += ls.length * ((s || 10) + 4) + 2;
    if (y > 780) {
      doc.addPage();
      y = 48;
    }
  ...
  line(
    'Engineering preview — predicted from drawing tolerances + chosen distributions, not measured data. Not for production approval.',
    9,
    '#b8860b'
  );
  line(
  ...
  line(
    'VERDICT (predicted): ' +
      ...
      ')',
    11,
    overall === 'PASS' ? '#1a7f37' : overall === 'WARNING' ? '#b8860b' : '#c0392b',
    true
  );
  y += 8;
  ...
  d.b.dims.forEach((dim, i) => {
    ...
    line(
      i +
        ...
        dim.name +
        '  ' +
        (dim.nominal * u).toFixed(3) +
        ' +/-' +
        (dim.tolerance * u).toFixed(3) +
        '  [' +
        dim.dist +
        ']  (' +
        (c ? c.pct : '0') +
  ...
  y += 8;
  line(
    'LIMITATIONS: 1D linear; no correlation; per-part sigma from tol; predicted not observed capability.',
    8,
    '#666'
  );
...
$('unitSpec2').addEventListener('change', () => {
  $('unitSpec').value = $('unitSpec2').value;
  convertUnits();
});
...
$('dimList').addEventListener('click', (e) => {
  const t = e.target;
  if (t.dataset.del !== undefined && dimensions.length > 1) {
    dimensions.splice(+t.dataset.del, 1);
    renderDims();
    compute();
  }
});
