...
function validateAndCalc() {
  ...
  const checks = [
    ['designLoadN', (v) => v <= 0],
    ['weldLengthMm', (v) => v <= 0],
    ['weldStrengthMpa', (v) => v <= 0],
    ['safetyFactor', (v) => v < 1],
    ['materialThicknessMm', (v) => v <= 0]
  ];
  checks.forEach(([f, bad]) => {
    const v = parseFloat($(f).value);
    if (isNaN(v) || bad(v)) {
      setFieldState(f, false, 'X must be positive');
      hasError = true;
    } else {
...
function gaugeColor(u) {
  return u >= 1 ? '#9B2423' : u >= 0.9 ? '#D05D29' : '#237F52';
}
function overallStatus(u) {
  return u >= 1 ? 'CRITICAL' : u >= 0.9 ? 'WARNING' : 'PASS';
}
...
function generateReport(opts = {}) {
  ...
  const calcId =
    opts.sync && window.calcId
      ? window.calcId
      : 'SC-001-' + Math.random().toString(36).substr(2, 9).toUpperCase();
  ...
  const stepsRows = d.steps.length
    ? d.steps
        .map(
          (s) =>
            `<tr><td>${s.step !== undefined ? s.step : ''}</td><td class="td-name">${s.description || ''}</td><td class="td-val">${s.result !== undefined ? s.result : s.formula || ''}</td></tr>`
        )
        .join('')
    : `<tr><td>-</td><td class="td-name">No step trace from engine</td><td class="td-val">see inputs</td></tr>`;
  ...
  const reportHTML = `
    ...
    <div class="sc-sec"><div class="sc-sec-hd">Utilization Gauge</div><div class="sc-card"><div style="display:flex;justify-content:center"><svg width="300" height="170" viewBox="0 0 300 170">
      ...
      <text x="150" y="135" text-anchor="middle" fill="${P.ink}" font-size="24" font-weight="700" font-family="IBM Plex Mono">${(d.util * 100).toFixed(0)}%</text>
      <text x="150" y="155" text-anchor="middle" fill="${P.muted}" font-size="10">utilization</text>
      <text x="30" y="168" fill="${P.muted}" font-size="9">0</text><text x="262" y="168" fill="${P.muted}" font-size="9">150%</text>
    </svg></div></div></div>
...
function exportPDF() {
  ...
  (d.steps.length ? d.steps : [{ step: '-', description: 'no step trace', result: '' }]).forEach(
    (s) =>
      line(
        (s.step !== undefined ? s.step + '. ' : '') +
          (s.description || '') +
          ' = ' +
          (s.result !== undefined ? s.result : s.formula || ''),
        9
      )
  );
  y += 8;
