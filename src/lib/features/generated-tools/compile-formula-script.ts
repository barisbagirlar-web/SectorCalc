import { toSafeVarName } from "@/lib/features/generated-tools/export-names";
import { isSafeCompiledFormulaExpression } from "@/lib/features/generated-tools/compile-formula-safety";
import { FormulaFailureAccumulator } from "@/lib/features/generated-tools/formula-failure-catalog";
  ...
  if (!expression || !isSafeCompiledFormulaExpression(expression)) {
    if (options.failureAccumulator) {
      ...
      options.failureAccumulator.add(
        "unknown",
        options.selfKey ?? "unknown",
        firstIssue ? (firstIssue.category as import("./formula-failure-catalog").FormulaFailureCategory | "UNKNOWN") : "PARSE_FAILURE",
        rawExpression,
        firstIssue
