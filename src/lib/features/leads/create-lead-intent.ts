...
import { appendStoredLeadIntent } from "@/lib/features/leads/storage";
import type {
...
/** Email-only validation for free-tool verdict unlock gate. */
export function validateVerdictUnlockEmail(email: string): string | undefined {
...
/** Build a minimal lead record from the verdict unlock email gate. */
export function buildVerdictUnlockLeadInput(
 email: string,
 context: {
 ...
 const localPart = trimmedEmail.split("@")[0]?.replace(/[^a-zA-Z0-9._-]/g, " ") ?? "";
 const name = localPart.length >= 2 ? localPart : "Guest User";

 return {
 name,
 email: trimmedEmail,
 company: "Not provided",
 industry: context.industry?.trim() || "Other",
 toolRequested: context.toolRequested?.trim() || "Premium verdict unlock",
 intendedUse: "Unlock full verdict report",
 source: "premium_unlock",
 pagePath: context.pagePath,
 plan: "single_report",
 message: "Verdict unlock email gate",
 };
}
