...
export const SUPPLIER_CURRENCY_RISK_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "currentExchangeRate", label: "Guncel Kur", label_i18n: {"en":"Current exchange rate"}, type: "number", unit: "TRY/USD", required: true, smartDefault: 30, validation: { min: 0.01 }, helper: "", expertMeaning: "Current exchange rate", expertMeaning_i18n: {"en":"Current exchange rate"} },
  ...
  thresholds: [{ fieldId: "fxNetRiskCost", warning: 5000, critical: 15000, direction: "higher_is_bad", warningMessage: "Net risk maliyeti > $5K — hedge degerlendirilmeli.", warningMessage_i18n: {"en":"Net risk cost > $5K — hedge should be evaluated."}, criticalMessage: "Net risk maliyeti > $15K — fx korunmasi zorunlu.", criticalMessage_i18n: {"en":"Net risk cost > $15K — currency hedging is mandatory."} }],
  formulaPipeline: [
