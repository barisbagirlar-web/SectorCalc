...
export const SUPPLIER_CURRENCY_RISK_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Tedarikçi Döviz Riski Analizi", name_i18n: {"en":"Supplier Currency Risk Analysis"}, sectorSlug: "financial-planning", category: "cost",
  painStatement: "Yabancı para tedarikçilerinde kur dalgalanması maliyeti hesaplanmazsa beklenmedik zararlar oluşur.", painStatement_i18n: {"en":"If the cost of exchange rate fluctuations is not calculated for foreign currency suppliers, unexpected losses occur."},
  inputs: [
    { id: "invoiceTotal", label: "Toplam Fatura Tutarı (Döviz)", label_i18n: {"en":"Total invoice in foreign currency"}, type: "number", unit: "USD", required: true, smartDefault: 100000, validation: { min: 1 }, helper: "", expertMeaning: "Total invoice in foreign currency", expertMeaning_i18n: {"en":"Total invoice in foreign currency"} },
    ...
    { id: "expectedRateShift", label: "Beklenen Kur Değişimi", label_i18n: {"en":"Expected exchange rate shift"}, type: "number", unit: "%", required: true, smartDefault: 10, validation: { min: -50, max: 50 }, helper: "", expertMeaning: "Expected exchange rate shift", expertMeaning_i18n: {"en":"Expected exchange rate shift"} },
    ...
    { id: "contractClauseSavings", label: "Sözleşme Klausül Tasarrufu", label_i18n: {"en":"Savings from FX clauses"}, type: "number", unit: "USD", required: false, smartDefault: 2000, validation: { min: 0 }, helper: "", expertMeaning: "Savings from FX clauses", expertMeaning_i18n: {"en":"Savings from FX clauses"} },
  ],
  outputs: [
    { id: "fxExposureSupplier", label: "Döviz Pozisyonu", label_i18n: {"en":"Doviz Pozisyonu"}, unit: "USD", format: "currency" },
    { id: "fxExpectedLoss", label: "Beklenen Kur Zararı", label_i18n: {"en":"Beklenen Kur Zarar"}, unit: "USD", format: "currency" },
  ...
  thresholds: [{ fieldId: "fxNetRiskCost", warning: 5000, critical: 15000, direction: "higher_is_bad", warningMessage: "Net risk maliyeti > $5K — hedge değerlendirilmeli.", warningMessage_i18n: {"en":"Net risk maliyeti > $5K — hedge değerlendirilmeli."}, criticalMessage: "Net risk maliyeti > $15K — döviz korunması zorunlu.", criticalMessage_i18n: {"en":"Net risk maliyeti > $15K — döviz korunması zorunlu."} }],
  formulaPipeline: [
