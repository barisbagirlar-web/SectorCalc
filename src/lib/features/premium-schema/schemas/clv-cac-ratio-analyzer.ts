...
export const CLV_CAC_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Musteri kazanma maliyeti (CAC) ile yasam boyu value (CLV) arasindaki dengesizlik, pazarlama butcesinin verimsiz kullanimina ve kârlilik sorunlarina yol acar.", painStatement_i18n: {"en":"The imbalance between customer Win Cost (CAC) and lifetime Value (CLV) leads to inefficient marketing budget use and profitability issues."},
  inputs: [
    ...
    { id: "purchaseFreq", label: "Purchase frequency per year", label_i18n: {"en":"Purchase frequency per year"}, type: "number", unit: "adet/yil", required: true, smartDefault: 6, validation: { min: 0.1 }, helper: "", expertMeaning: "Purchase frequency per year", expertMeaning_i18n: {"en":"Purchase frequency per year"} },
    { id: "lifespan", label: "Average customer lifespan (years)", label_i18n: {"en":"Average customer lifespan (years)"}, type: "number", unit: "yil", required: true, smartDefault: 3, validation: { min: 0.5 }, helper: "", expertMeaning: "Average customer lifespan (years)", expertMeaning_i18n: {"en":"Average customer lifespan (years)"} },
    { id: "grossMarginPct", label: "Brut Marj", label_i18n: {"en":"Gross margin percentage"}, type: "number", unit: "%", required: true, smartDefault: 40, validation: { min: 1, max: 100 }, helper: "", expertMeaning: "Gross margin percentage", expertMeaning_i18n: {"en":"Gross margin percentage"} },
    ...
    { id: "newCustomers", label: "New customers acquired", label_i18n: {"en":"New customers acquired"}, type: "number", unit: "kisi", required: true, smartDefault: 200, validation: { min: 1 }, helper: "", expertMeaning: "New customers acquired", expertMeaning_i18n: {"en":"New customers acquired"} },
  ...
  outputs: [
    { id: "clv", label: "CLV (Brut)", label_i18n: {"en":"CLV (Gross)"}, unit: "USD", format: "currency" },
    { id: "gmClv", label: "Brut Marj CLV", label_i18n: {"en":"Gross Margin CLV"}, unit: "USD", format: "currency" },
    ...
    { id: "payback", label: "Geri Odeme Suresi", label_i18n: {"en":"Payback Period"}, unit: "ay", format: "number" },
    { id: "ltvCac", label: "LTV/CAC Oran", label_i18n: {"en":"LTV/CAC Rate"}, unit: "", format: "number", isBigNumber: true },
  ],
  thresholds: [
    { fieldId: "ltvCac", warning: 3, critical: 1, direction: "lower_is_bad", warningMessage: "LTV/CAC < 3 — pazarlama verimliligi iyilestirilmeli.", warningMessage_i18n: {"en":"LTV/CAC < 3 — marketing efficiency should be improved."}, criticalMessage: "LTV/CAC < 1 — her musteri zarar yaziyor, acil strateji degisikligi.", criticalMessage_i18n: {"en":"LTV/CAC < 1 — every customer generates Loss, urgent strategy change needed."} },
  ],
  formulaPipeline: [
