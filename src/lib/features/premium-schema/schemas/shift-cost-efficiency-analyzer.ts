...
export const SHIFT_COST_EFFICIENCY_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "shiftCount", label: "Number of Shifts", label_i18n: {"en":"Number of Shifts"}, type: "number", unit: "vardiya/gün", required: true, smartDefault: 3, validation: { min: 1, max: 3 }, helper: "", expertMeaning: "Number of daily shifts", expertMeaning_i18n: {"en":"Number of daily shifts"} },
    { id: "workersPerShift", label: "Workers per Shift", label_i18n: {"en":"Workers per Shift"}, type: "number", unit: "kişi", required: true, smartDefault: 10, validation: { min: 1 }, helper: "", expertMeaning: "Workers per shift", expertMeaning_i18n: {"en":"Workers per shift"} },
  ...
  outputs: [
    ...
    { id: "annualShiftCost", label: "Annual Shift Cost", label_i18n: {"en":"Annual Shift Cost"}, unit: "USD/yıl", format: "currency", isBigNumber: true },
  ],
  thresholds: [
