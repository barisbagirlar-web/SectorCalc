...
export const SHIFT_COST_EFFICIENCY_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "workersPerShift", label: "Workers per Shift", label_i18n: {"en":"Workers per Shift"}, type: "number", unit: "kisi", required: true, smartDefault: 10, validation: { min: 1 }, helper: "", expertMeaning: "Workers per shift", expertMeaning_i18n: {"en":"Workers per shift"} },
    { id: "hourlyWage", label: "Saatlik Ucret", label_i18n: {"en":"Hourly Wage"}, type: "number", unit: "USD/saat", required: true, smartDefault: 20, validation: { min: 1 }, helper: "", expertMeaning: "Average hourly wage", expertMeaning_i18n: {"en":"Average hourly wage"} },
    { id: "shiftHours", label: "Vardiya Suresi", label_i18n: {"en":"Shift Duration"}, type: "number", unit: "saat", required: true, smartDefault: 8, validation: { min: 1 }, helper: "", expertMeaning: "Shift duration in hours", expertMeaning_i18n: {"en":"Shift duration in hours"} },
    ...
    { id: "dailyOutput", label: "Daily Production Quantity", label_i18n: {"en":"Daily Production Quantity"}, type: "number", unit: "adet", required: true, smartDefault: 1000, validation: { min: 1 }, helper: "", expertMeaning: "Total daily output across all shifts", expertMeaning_i18n: {"en":"Total daily output across all shifts"} },
  ...
  outputs: [
    { id: "shiftTotalCost", label: "Toplam Vardiya Maliyeti", label_i18n: {"en":"Total Shift Cost"}, unit: "USD/gun", format: "currency" },
    { id: "shiftEfficiency", label: "Shift Efficiency", label_i18n: {"en":"Shift Efficiency"}, unit: "%", format: "percentage" },
    { id: "shiftCostPerUnit", label: "Shift Cost per Unit", label_i18n: {"en":"Shift Cost per Unit"}, unit: "USD/adet", format: "currency" },
    { id: "annualShiftCost", label: "Annual Shift Cost", label_i18n: {"en":"Annual Shift Cost"}, unit: "USD/yil", format: "currency", isBigNumber: true },
  ],
  thresholds: [
