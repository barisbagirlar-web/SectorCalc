...
export const CONTRACT_INCENTIVE_ANALYZER: PremiumCalculatorSchema = {
  ...
  name: "Sözleşme Teşvik Analizi", name_i18n: {"en":"Contract Incentive Analysis"}, sectorSlug: "construction", category: "cost",
  painStatement: "İnşaat sözleşmelerinde teşvik mekanizmaları doğru yapılandırılmazsa yüklenici ve işveren arasında maliyet sapmaları ve anlaşmazlıklar kaçınılmaz olur.", painStatement_i18n: {"en":"If incentive mechanisms in construction contracts are not configured correctly, cost variance and disputes between contractor and employer become inevitable."},
  inputs: [
    ...
    { id: "actualCost", label: "Gerçekleşen Maliyet", label_i18n: {"en":"Actual Cost"}, type: "number", unit: "USD", required: true, smartDefault: 4800000, validation: { min: 1 }, helper: "", expertMeaning: "Actual cost incurred", expertMeaning_i18n: {"en":"Actual cost incurred"} },
    ...
    { id: "shareRatio", label: "Pay Oranı (Yüklenici)", label_i18n: {"en":"Share Ratio (Contractor)"}, type: "number", unit: "%", required: true, smartDefault: 50, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Contractor share of savings/overrun", expertMeaning_i18n: {"en":"Contractor share of savings/overrun"} },
  ...
  outputs: [
    { id: "incentiveTargetFee", label: "Hedef Teşvik Ücreti", label_i18n: {"en":"Hedef Tesvik Ucreti"}, unit: "USD", format: "currency" },
    { id: "incentiveActualFee", label: "Gerçekleşen Teşvik Ücreti", label_i18n: {"en":"Gerceklesen Tesvik Ucreti"}, unit: "USD", format: "currency", isBigNumber: true },
  ],
  thresholds: [{ fieldId: "incentiveActualFee", warning: 600000, critical: 300000, direction: "lower_is_bad", warningMessage: "Teşvik ücreti <$600K — yüklenici motivasyonu düşebilir.", warningMessage_i18n: {"en":"Incentive fee <$600K — contractor motivation may decrease."}, criticalMessage: "Teşvik ücreti <$300K — sözleşme yapısı gözden geçirilmeli.", criticalMessage_i18n: {"en":"Incentive fee <$300K — review contract structure."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Sözleşme Teşvik Analiz Raporu", title_i18n: {"en":"Contract Incentive Analysis Report"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
