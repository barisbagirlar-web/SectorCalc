...
export const SIX_SIGMA_PRIORITIZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "productionVolume", label: "Toplam Uretim Hacmi", label_i18n: {"en":"Units produced"}, type: "number", unit: "adet", required: true, smartDefault: 100000, validation: { min: 1 }, helper: "", expertMeaning: "Units produced", expertMeaning_i18n: {"en":"Units produced"} },
    ...
    { id: "preventionCost", label: "Onleme Maliyeti", label_i18n: {"en":"Training, FMEA, SPC"}, type: "number", unit: "USD", required: false, smartDefault: 10000, validation: { min: 0 }, helper: "", expertMeaning: "Training, FMEA, SPC", expertMeaning_i18n: {"en":"Training, FMEA, SPC"} },
    ...
    { id: "safetyImpact", label: "Guvenlik Etkisi (1-10)", label_i18n: {"en":"Safety score"}, type: "number", unit: "", required: false, smartDefault: 5, validation: { min: 1, max: 10 }, helper: "", expertMeaning: "Safety score", expertMeaning_i18n: {"en":"Safety score"} },
  ...
  thresholds: [
    { fieldId: "sigmaLevel", warning: 4, critical: 3, direction: "lower_is_bad", warningMessage: "Sigma level < 4 — iyilestirme firsati mevcut.", warningMessage_i18n: {"en":"Sigma level < 4 — improvement opportunity exists."}, criticalMessage: "Sigma level < 3 — acil aksiyon gerekiyor.", criticalMessage_i18n: {"en":"Sigma level < 3 — urgent aksiyon gerekiyor."} },
  ],
  formulaPipeline: [
