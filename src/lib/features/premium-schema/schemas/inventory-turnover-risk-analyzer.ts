...
export const INVENTORY_TURNOVER_RISK_ANALYZER: PremiumCalculatorSchema = {
  ...
  painStatement: "Stok devir hizi izlenmezse fazla stokta bekleyen sermaye atil kalir, modasi gecen products ise hurda veya iskonto ile satilmak zorunda kalir.", painStatement_i18n: {"en":"Inventory rotation rate if not tracked leads to idle capital in overtime stock, and obsolete products must be sold as scrap or at a discount."},
  inputs: [
    ...
    { id: "daysInPeriod", label: "Days in accounting period", label_i18n: {"en":"Days in accounting period"}, type: "number", unit: "gun", required: false, smartDefault: 365, validation: { min: 1 }, helper: "", expertMeaning: "Days in accounting period", expertMeaning_i18n: {"en":"Days in accounting period"} },
  ...
  outputs: [
    { id: "inventoryTurnoverRatio", label: "Inventory Turnover Rate", label_i18n: {"en":"Inventory Turnover Rate"}, unit: "tur/yil", format: "number" },
    { id: "dsiDays", label: "Stokta Kalma Suresi (DSI)", label_i18n: {"en":"Inventory Holding Period (DSI)"}, unit: "gun", format: "number" },
    { id: "obsolescenceRiskCost", label: "Obsolescence Risk Cost", label_i18n: {"en":"Obsolescence Risk Cost"}, unit: "USD/yil", format: "currency", isBigNumber: true },
    { id: "liquidationLoss", label: "Liquidation Loss", label_i18n: {"en":"Liquidation Loss"}, unit: "USD", format: "currency" },
  ],
  thresholds: [{ fieldId: "inventoryTurnoverRatio", warning: 4, critical: 2, direction: "lower_is_bad", warningMessage: "Stok devir hizi <4 — sermaye atil kaliyor.", warningMessage_i18n: {"en":"Inventory rotation rate < 4 — capital is idle."}, criticalMessage: "Stok devir hizi <2 — acil stok eritme aksiyonu gerekli.", criticalMessage_i18n: {"en":"Inventory rotation rate < 2 — urgent inventory reduction action required."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Stok Devir Hizi ve Risk Reportu", title_i18n: {"en":"Inventory Rotation Rate and Risk Report"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["Stok devir = SMM / ortalama stok. DSI = gun / devir.", "Eskime risk maliyeti = stok × eskime orani.", "Tasfiye zarari = adet × fiyat × iskonto orani.", "OEM stoklari ortalama 4-6 tur/yil hedefler."],assumptionNotes_i18n:[{"en":"Inventory turnover = COGS / average inventory. DSI = days / turnover."},{"en":"Obsolescence risk cost = inventory × obsolescence rate."},{"en":"Liquidation loss = quantity × price × discount rate."},{"en":"OEM inventories target an average of 4-6 turns/year."}] },
};
