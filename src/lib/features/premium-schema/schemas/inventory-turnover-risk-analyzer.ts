...
export const INVENTORY_TURNOVER_RISK_ANALYZER: PremiumCalculatorSchema = {
  ...
  painStatement: "Inventory rotation rate if not tracked leads to idle capital in overtime stock, and obsolete products must be sold as scrap or at a discount.", painStatement_i18n: {"en":"Inventory rotation rate if not tracked leads to idle capital in overtime stock, and obsolete products must be sold as scrap or at a discount."},
  inputs: [
    ...
    { id: "daysInPeriod", label: "Days in accounting period", label_i18n: {"en":"Days in accounting period"}, type: "number", unit: "days", required: false, smartDefault: 365, validation: { min: 1 }, helper: "", expertMeaning: "Days in accounting period", expertMeaning_i18n: {"en":"Days in accounting period"} },
    ...
    { id: "inventoryUnitCount", label: "Inventory in units", label_i18n: {"en":"Inventory in units"}, type: "number", unit: "units", required: false, smartDefault: 3000, validation: { min: 1 }, helper: "", expertMeaning: "Inventory in units", expertMeaning_i18n: {"en":"Inventory in units"} },
  ...
  outputs: [
    ...
    { id: "dsiDays", label: "Inventory Holding Period (DSI)", label_i18n: {"en":"Inventory Holding Period (DSI)"}, unit: "days", format: "number" },
  ...
  reportTemplate: { title: "Inventory Rotation Rate and Risk Report", title_i18n: {"en":"Inventory Rotation Rate and Risk Report"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: [],assumptionNotes_i18n:[{"en":"Inventory turnover = COGS / average inventory. DSI = days / turnover."},{"en":"Obsolescence risk cost = inventory × obsolescence rate."},{"en":"Liquidation loss = quantity × price × discount rate."},{"en":"OEM inventories target an average of 4-6 turns/year."}] },
};
