...
export const MACHINE_ECONOMIC_LIFE_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Makinenin gercek ekonomik omru bilinmezse ya erken yenileme ya da bakim maliyeti yuksek eski makineyle uretim loss olusur.", painStatement_i18n: {"en":"If the actual economic life of the machine is unknown, either early replacement occurs or production loss occurs with the old machine due to high maintenance cost."},
  inputs: [
    ...
    { id: "lifeYears", label: "Physical life in years", label_i18n: {"en":"Physical life in years"}, type: "number", unit: "yil", required: true, smartDefault: 10, validation: { min: 1, max: 50 }, helper: "", expertMeaning: "Physical life in years", expertMeaning_i18n: {"en":"Physical life in years"} },
    { id: "annualOperatingCost", label: "Annual operating cost", label_i18n: {"en":"Annual operating cost"}, type: "number", unit: "USD/yil", required: true, smartDefault: 25000, validation: { min: 0 }, helper: "", expertMeaning: "Annual operating cost", expertMeaning_i18n: {"en":"Annual operating cost"} },
    ...
    { id: "annualEnergy", label: "Annual energy cost", label_i18n: {"en":"Annual energy cost"}, type: "number", unit: "USD/yil", required: false, smartDefault: 8000, validation: { min: 0 }, helper: "", expertMeaning: "Annual energy cost", expertMeaning_i18n: {"en":"Annual energy cost"} },
  ],
  outputs: [
    { id: "euacCapital", label: "EUAC (Sermaye)", label_i18n: {"en":"EUAC (Sermaye)"}, unit: "USD/yil", format: "currency" },
    { id: "euacOperating", label: "EUAC (Isletme)", label_i18n: {"en":"EUAC (operation)"}, unit: "USD/yil", format: "currency" },
    { id: "totalEuac", label: "Toplam EUAC", label_i18n: {"en":"Total EUAC"}, unit: "USD/yil", format: "currency" },
    { id: "economicLife", label: "Ekonomik Omur", label_i18n: {"en":"economical life"}, unit: "yil", format: "number" },
  ],
  thresholds: [{ fieldId: "totalEuac", warning: 40000, critical: 75000, direction: "higher_is_bad", warningMessage: "EUAC > $40K — alternatif makineler degerlendirilmeli.", warningMessage_i18n: {"en":"EUAC > $40K — alternative machines should be evaluated."}, criticalMessage: "EUAC > $75K — makine yenileme planina alinmali.", criticalMessage_i18n: {"en":"EUAC > $75K — machine should be scheduled for replacement."} }],
  formulaPipeline: [
