...
export const AI_TOKEN_COST_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "completionTokens", label: "Avg Completion Tokens", label_i18n: {"en":"Avg Completion Tokens"}, type: "number", unit: "token", required: true, smartDefault: 500, validation: { min: 1, max: 1_000_000 }, helper: "Average output token count per request.", helper_i18n: {"en":"Average output token count per request."}, expertMeaning: "Completion tokens per request", expertMeaning_i18n: {"en":"Completion tokens per request"} },
    ...
    { id: "confidenceBuffer", label: "Safety Buffer", label_i18n: {"en":"Safety Buffer"}, type: "number", unit: "%", required: false, smartDefault: 15, validation: { min: 0, max: 100 }, helper: "Safety margin on projection.", helper_i18n: {"en":"Safety margin on projection."}, expertMeaning: "Contingency buffer", expertMeaning_i18n: {"en":"Contingency buffer"} },
    { id: "infraOverhead", label: "Infrastructure Overhead", label_i18n: {"en":"Infrastructure Overhead"}, type: "number", unit: "USD/month", required: false, smartDefault: 500, validation: { min: 0, max: 1_000_000 }, helper: "GPU/server/orchestration costs.", helper_i18n: {"en":"GPU/server/orchestration costs."}, expertMeaning: "Non-API monthly infra cost", expertMeaning_i18n: {"en":"Non-API monthly infra cost"} },
    { id: "fallbackCost", label: "Fallback Model Cost", label_i18n: {"en":"Fallback Model Cost"}, type: "number", unit: "USD/month", required: false, smartDefault: 300, validation: { min: 0, max: 1_000_000 }, helper: "Backup model API spend.", helper_i18n: {"en":"Backup model API spend."}, expertMeaning: "Secondary model cost", expertMeaning_i18n: {"en":"Secondary model cost"} },
  ],
  outputs: [
    { id: "dailyBasePromptCost", label: "Daily Prompt Cost", label_i18n: {"en":"Daily Prompt Cost"}, unit: "USD", format: "currency" },
    { id: "dailyBaseCompletionCost", label: "Daily Completion Cost", label_i18n: {"en":"Daily Completion Cost"}, unit: "USD", format: "currency" },
    { id: "dailyCacheNetCost", label: "Daily Cache Net Cost", label_i18n: {"en":"Daily Cache Net Cost"}, unit: "USD", format: "currency" },
    { id: "dailyBaseCost", label: "Daily Total Base Cost", label_i18n: {"en":"Daily Total Base Cost"}, unit: "USD", format: "currency" },
