...
export const AI_TOKEN_COST_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "AI Token Cost Analyzer", name_i18n: {"en":"AI Token Cost Analyzer","tr":"AI Token Cost Analyzer"},
  sectorSlug: "it-cloud",
  category: "cost",
  painStatement:
    "LLM API costs grow silently with usage. Without a structured TCO model, teams miss cache savings and seasonal spikes — leading to budget overruns.", painStatement_i18n: {"en":"LLM API costs grow silently with usage. Without a structured TCO model, teams miss cache savings and seasonal spikes — leading to budget overruns.","tr":"LLM API costs grow silently with usage. Without a structured TCO model, teams miss cache savings and seasonal spikes — leading to budget overruns."},
  inputs: [
    { id: "dailyRequests", label: "Günlük API Request", label_i18n: {"en":"Daily API Requests","tr":"Günlük API Request"}, type: "number", unit: "adet", required: true, smartDefault: 10000, validation: { min: 1, max: 100_000_000 }, helper: "Total LLM API calls per day.", helper_i18n: {"en":"Total LLM API calls per day.","tr":"Total LLM API calls per day."}, expertMeaning: "Daily inference request count", expertMeaning_i18n: {"en":"Daily inference request count","tr":"Günlük çıkarım istek sayısı"} },
    { id: "promptTokens", label: "Ortalama Prompt Token", label_i18n: {"en":"Avg Prompt Tokens","tr":"Ortalama Prompt Token"}, type: "number", unit: "token", required: true, smartDefault: 2000, validation: { min: 1, max: 1_000_000 }, helper: "Average input token count per request.", helper_i18n: {"en":"Average input token count per request.","tr":"Average input token count per request."}, expertMeaning: "Prompt tokens per request", expertMeaning_i18n: {"en":"Prompt tokens per request","tr":"İstek başına prompt token"} },
    { id: "completionTokens", label: "Ortalama Completion Token", label_i18n: {"en":"Avg Completion Tokens","tr":"Ortalama Completion Token"}, type: "number", unit: "token", required: true, smartDefault: 500, validation: { min: 1, max: 1_000_000 }, helper: "Average output token count per request.", helper_i18n: {"en":"Average output token count per request.","tr":"Average output token count per request."}, expertMeaning: "Completion tokens per request", expertMeaning_i18n: {"en":"Completion tokens per request","tr":"İstek başına completion token"} },
    { id: "cacheHitRatio", label: "Cache Hit Ratio", label_i18n: {"en":"Cache Hit Ratio","tr":"Cache Hit Ratio"}, type: "number", unit: "%", required: true, smartDefault: 25, validation: { min: 0, max: 100 }, helper: "% of tokens served from prompt cache.", helper_i18n: {"en":"% of tokens served from prompt cache.","tr":"% of tokens served from prompt cache."}, expertMeaning: "Reduces effective cost", expertMeaning_i18n: {"en":"Reduces effective cost","tr":"Etkin maliyeti düşürür"} },
    { id: "promptPrice", label: "Model Prompt Price", label_i18n: {"en":"Model Prompt Price","tr":"Model Prompt Price"}, type: "number", unit: "USD/1M tok", required: true, smartDefault: 15, validation: { min: 0.1, max: 500 }, helper: "Price per million prompt tokens.", helper_i18n: {"en":"Price per million prompt tokens.","tr":"Price per million prompt tokens."}, expertMeaning: "Per-model input token price", expertMeaning_i18n: {"en":"Per-model input token price","tr":"Model başına giriş token fiyatı"} },
    { id: "completionPrice", label: "Model Completion Price", label_i18n: {"en":"Model Completion Price","tr":"Model Completion Price"}, type: "number", unit: "USD/1M tok", required: true, smartDefault: 60, validation: { min: 0.1, max: 500 }, helper: "Price per million completion tokens.", helper_i18n: {"en":"Price per million completion tokens.","tr":"Price per million completion tokens."}, expertMeaning: "Per-model output token price", expertMeaning_i18n: {"en":"Per-model output token price","tr":"Model başına çıkış token fiyatı"} },
    { id: "cacheReadPrice", label: "Cache Read Price", label_i18n: {"en":"Cache Read Price","tr":"Cache Read Price"}, type: "number", unit: "USD/1M tok", required: false, smartDefault: 7.5, validation: { min: 0, max: 500 }, helper: "Price per million cached read tokens.", helper_i18n: {"en":"Price per million cached read tokens.","tr":"Price per million cached read tokens."}, expertMeaning: "Typically 50% of prompt price", expertMeaning_i18n: {"en":"Typically 50% of prompt price","tr":"Genelde prompt fiyatının %50'si"} },
    { id: "monthlyGrowthRate", label: "Aylık Büyüme Oranı", label_i18n: {"en":"Monthly Growth Rate","tr":"Aylık Büyüme Oranı"}, type: "number", unit: "%", required: true, smartDefault: 10, validation: { min: 0, max: 500 }, helper: "Month-over-month volume growth.", helper_i18n: {"en":"Month-over-month volume growth.","tr":"Month-over-month volume growth."}, expertMeaning: "MoM growth rate", expertMeaning_i18n: {"en":"MoM growth rate","tr":"Aylık büyüme oranı"} },
    { id: "confidenceBuffer", label: "Güven Tamponu", label_i18n: {"en":"Safety Buffer","tr":"Güven Tamponu"}, type: "number", unit: "%", required: false, smartDefault: 15, validation: { min: 0, max: 100 }, helper: "Safety margin on projection.", helper_i18n: {"en":"Safety margin on projection.","tr":"Safety margin on projection."}, expertMeaning: "Contingency buffer", expertMeaning_i18n: {"en":"Contingency buffer","tr":"Güven tamponu"} },
    { id: "infraOverhead", label: "Altyapı Overhead", label_i18n: {"en":"Infrastructure Overhead","tr":"Altyapı Overhead"}, type: "number", unit: "USD/ay", required: false, smartDefault: 500, validation: { min: 0, max: 1_000_000 }, helper: "GPU/server/orchestration costs.", helper_i18n: {"en":"GPU/server/orchestration costs.","tr":"GPU/server/orchestration costs."}, expertMeaning: "Non-API monthly infra cost", expertMeaning_i18n: {"en":"Non-API monthly infra cost","tr":"API dışı aylık altyapı maliyeti"} },
  ...
  reportTemplate: { title: "AI Token Cost Report", title_i18n: {"en":"AI Token Cost Report","tr":"AI Token Cost Report"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.15, volatilityPercent: 15, targetMarginPercent: 20, assumptionNotes: ["Token prices based on public API pricing as of mid-2026.", "Cached tokens charged at cache read price (typically 50% of prompt price).", "Growth rate is month-over-month, compounding not modeled.", "TCO excludes: data egress, fine-tuning, human-in-the-loop."],assumptionNotes_i18n:[{"en":"Token prices based on public API pricing as of mid-2026.","tr":"Token prices based on public API pricing as of mid-2026."},{"en":"Cached tokens charged at cache read price (typically 50% of prompt price).","tr":"Cached tokens charged at cache read price (typically 50% of prompt price)."},{"en":"Growth rate is month-over-month, compounding not modeled.","tr":"Growth rate is month-over-month, compounding not modeled."},{"en":"TCO excludes: data egress, fine-tuning, human-in-the-loop.","tr":"TCO excludes: data egress, fine-tuning, human-in-the-loop."}]},
};
