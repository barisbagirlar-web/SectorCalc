...
export const AI_TOKEN_COST_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "dailyRequests", label: "Günlük API Request", label_i18n: {"en":"Daily API Requests"}, type: "number", unit: "adet", required: true, smartDefault: 10000, validation: { min: 1, max: 100_000_000 }, helper: "Total LLM API calls per day.", helper_i18n: {"en":"Total LLM API calls per day."}, expertMeaning: "Daily inference request count", expertMeaning_i18n: {"en":"Daily inference request count"} },
    ...
    { id: "monthlyGrowthRate", label: "Aylık Büyüme Oranı", label_i18n: {"en":"Monthly Growth Rate"}, type: "number", unit: "%", required: true, smartDefault: 10, validation: { min: 0, max: 500 }, helper: "Month-over-month volume growth.", helper_i18n: {"en":"Month-over-month volume growth."}, expertMeaning: "MoM growth rate", expertMeaning_i18n: {"en":"MoM growth rate"} },
    { id: "confidenceBuffer", label: "Güven Tamponu", label_i18n: {"en":"Safety Buffer"}, type: "number", unit: "%", required: false, smartDefault: 15, validation: { min: 0, max: 100 }, helper: "Safety margin on projection.", helper_i18n: {"en":"Safety margin on projection."}, expertMeaning: "Contingency buffer", expertMeaning_i18n: {"en":"Contingency buffer"} },
    { id: "infraOverhead", label: "Altyapı Overhead", label_i18n: {"en":"Infrastructure Overhead"}, type: "number", unit: "USD/ay", required: false, smartDefault: 500, validation: { min: 0, max: 1_000_000 }, helper: "GPU/server/orchestration costs.", helper_i18n: {"en":"GPU/server/orchestration costs."}, expertMeaning: "Non-API monthly infra cost", expertMeaning_i18n: {"en":"Non-API monthly infra cost"} },
  ...
  outputs: [
    { id: "dailyBasePromptCost", label: "Günlük Prompt Maliyeti", label_i18n: {"en":"Gunluk Prompt Maliyeti"}, unit: "USD", format: "currency" },
    { id: "dailyBaseCompletionCost", label: "Günlük Completion Maliyeti", label_i18n: {"en":"Gunluk Completion Maliyeti"}, unit: "USD", format: "currency" },
    { id: "dailyCacheNetCost", label: "Günlük Cache Net Maliyeti", label_i18n: {"en":"Gunluk Cache Net Maliyeti"}, unit: "USD", format: "currency" },
    { id: "dailyBaseCost", label: "Günlük Toplam Baz Maliyet", label_i18n: {"en":"Gunluk Toplam Baz Maliyet"}, unit: "USD", format: "currency" },
    { id: "monthlyProjection", label: "Aylık Projeksiyon", label_i18n: {"en":"Aylk Projeksiyon"}, unit: "USD", format: "currency" },
    { id: "tco", label: "TCO (Aylık)", label_i18n: {"en":"TCO (Aylk)"}, unit: "USD", format: "currency", isBigNumber: true },
  ],
  thresholds: [
    { fieldId: "tco", warning: 5000, critical: 25000, direction: "higher_is_bad", warningMessage: "TCO > $5K/ay — caching veya model optimizasyonu değerlendir.", warningMessage_i18n: {"en":"TCO > $5K/ay — caching veya model optimizasyonu değerlendir."}, criticalMessage: "TCO > $25K/ay — acil maliyet optimizasyonu gerekiyor.", criticalMessage_i18n: {"en":"TCO > $25K/ay — acil maliyet optimizasyonu gerekiyor."} },
  ],
  formulaPipeline: [
