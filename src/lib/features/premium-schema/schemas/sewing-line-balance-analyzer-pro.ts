...
export const SEWING_LINE_BALANCE_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Dikiş Hattı Dengeleyici (Pro)", name_i18n: {"en":"Sewing Line Balancer (Pro)"}, sectorSlug: "textile", category: "measurement",
  painStatement: "Dikiş hattında SMV dağılımı dengelenmezse hat verimliliği düşer, WIP birikir ve teslimat gecikir.", painStatement_i18n: {"en":"If SMV distribution is not balanced on the sewing line, line efficiency drops, WIP accumulates, and delivery is delayed."},
  inputs: [
    { id: "smvTimes", label: "SMV Süreleri (virgülle ayır)", label_i18n: {"en":"Array of SMV times per operation"}, type: "number", unit: "dak", array: true, required: true, validation: { min: 0 }, helper: "", expertMeaning: "Array of SMV times per operation", expertMeaning_i18n: {"en":"Array of SMV times per operation"} },
    ...
    { id: "downtime", label: "Planlı Duruş", label_i18n: {"en":"Planned breaks & meetings"}, type: "number", unit: "dak", required: false, smartDefault: 30, validation: { min: 0 }, helper: "", expertMeaning: "Planned breaks & meetings", expertMeaning_i18n: {"en":"Planned breaks & meetings"} },
    ...
    { id: "operatorCount", label: "Operatör Sayısı", label_i18n: {"en":"Number of Operators"}, type: "number", unit: "", required: false, smartDefault: 5, validation: { min: 1 }, helper: "", expertMeaning: "Number of operators", expertMeaning_i18n: {"en":"Number of operators"} },
    ...
    { id: "defectRate", label: "Hata Oranı", label_i18n: {"en":"Defect Rate"}, type: "number", unit: "%", required: false, smartDefault: 3, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Defect rate", expertMeaning_i18n: {"en":"Defect rate"} },
  ...
  outputs: [
    ...
    { id: "actualOperators", label: "Gerçek Operatör", label_i18n: {"en":"Gercek Operator"}, unit: "kişi", format: "number" },
    { id: "lineEfficiency", label: "Hat Verimliliği", label_i18n: {"en":"Hat Verimliligi"}, unit: "%", format: "percentage" },
    { id: "balanceDelay", label: "Denge Kaybı", label_i18n: {"en":"Balance Loss"}, unit: "%", format: "percentage", isBigNumber: true },
  ],
  thresholds: [{ fieldId: "lineEfficiency", warning: 80, critical: 70, direction: "lower_is_bad", warningMessage: "Verim < %80 — SMV dağılımı iyileştirilmeli.", warningMessage_i18n: {"en":"Verim < %80 — SMV dağılımı iyileştirilmeli."}, criticalMessage: "Verim < %70 — hat yeniden dengelenmeli.", criticalMessage_i18n: {"en":"Verim < %70 — hat yeniden dengelenmeli."} }],
  formulaPipeline: [
