...
export const VSM_FINANCIAL_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "totalLeadTime", label: "Toplam Teslim Süresi", label_i18n: {"en":"Total Lead Time"}, type: "number", unit: "dk", required: true, smartDefault: 480, validation: { min: 1 }, helper: "", expertMeaning: "Total lead time from order to delivery", expertMeaning_i18n: {"en":"Total lead time from order to delivery"} },
    ...
    { id: "dailyProduction", label: "Daily Production Quantity", label_i18n: {"en":"Daily Production Quantity"}, type: "number", unit: "adet/gün", required: true, smartDefault: 500, validation: { min: 1 }, helper: "", expertMeaning: "Daily production volume", expertMeaning_i18n: {"en":"Daily production volume"} },
    ...
    { id: "operatingDays", label: "Operating Days / Year", label_i18n: {"en":"Operating Days / Year"}, type: "number", unit: "gün", required: false, smartDefault: 240, validation: { min: 1 }, helper: "", expertMeaning: "Annual operating days", expertMeaning_i18n: {"en":"Annual operating days"} },
  ...
  outputs: [
    { id: "vsmLeadtimeCost", label: "Teslim Süresi Maliyeti", label_i18n: {"en":"Lead Time Cost"}, unit: "USD/gün", format: "currency" },
    { id: "vsmValueAddedRatio", label: "Value-Added Ratio", label_i18n: {"en":"Value-Added Ratio"}, unit: "%", format: "percentage" },
    { id: "vsmNonValueAddedCost", label: "Non-Value-Added Cost", label_i18n: {"en":"Non-Value-Added Cost"}, unit: "USD/yıl", format: "currency", isBigNumber: true },
    { id: "vsmTotalFinancialImpact", label: "Toplam Finansal Etki", label_i18n: {"en":"Total Financial Impact"}, unit: "USD/yıl", format: "currency", isBigNumber: true },
  ],
  thresholds: [
  ...
  assumptions: { hiddenLossMultiplier: 1.2, volatilityPercent: 10, targetMarginPercent: 10, assumptionNotes: ["Value-added ratio = VA time / total lead time.", "KD olmayan maliyet = (toplam - KD süre) × birim maliyet × üretim.", "Scrap and rework costs are included in non-value-added cost."],assumptionNotes_i18n:[{"en":"Value-added ratio = VA time / total lead time."},{"en":"Non-value-added cost = (total - VA time) × unit cost × production."},{"en":"Scrap and rework costs are included in non-value-added cost."}] },
};
