/**
 * Tool #34 — VSM Financial Converter
 */
...
export const VSM_FINANCIAL_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "totalLeadTime", label: "Total Lead Time", label_i18n: {"en":"Total Lead Time"}, type: "number", unit: "min", required: true, smartDefault: 480, validation: { min: 1 }, helper: "", expertMeaning: "Total lead time from order to delivery", expertMeaning_i18n: {"en":"Total lead time from order to delivery"} },
    { id: "valueAddedTime", label: "Value-Added Time", label_i18n: {"en":"Value-Added Time"}, type: "number", unit: "min", required: true, smartDefault: 120, validation: { min: 1 }, helper: "", expertMeaning: "Value-added processing time", expertMeaning_i18n: {"en":"Value-added processing time"} },
    { id: "dailyProduction", label: "Daily Production Quantity", label_i18n: {"en":"Daily Production Quantity"}, type: "number", unit: "pcs/day", required: true, smartDefault: 500, validation: { min: 1 }, helper: "", expertMeaning: "Daily production volume", expertMeaning_i18n: {"en":"Daily production volume"} },
    { id: "costPerMinute", label: "Cost Per Minute", label_i18n: {"en":"Cost Per Minute"}, type: "number", unit: "USD/min", required: true, smartDefault: 1.5, validation: { min: 0.01 }, helper: "", expertMeaning: "Operating cost per minute", expertMeaning_i18n: {"en":"Operating cost per minute"} },
    { id: "operatingDays", label: "Operating Days / Year", label_i18n: {"en":"Operating Days / Year"}, type: "number", unit: "days", required: false, smartDefault: 240, validation: { min: 1 }, helper: "", expertMeaning: "Annual operating days", expertMeaning_i18n: {"en":"Annual operating days"} },
  ...
  outputs: [
    { id: "vsmLeadtimeCost", label: "Lead Time Cost", label_i18n: {"en":"Lead Time Cost"}, unit: "USD/day", format: "currency" },
    { id: "vsmValueAddedRatio", label: "Value-Added Ratio", label_i18n: {"en":"Value-Added Ratio"}, unit: "%", format: "percentage" },
    { id: "vsmNonValueAddedCost", label: "Non-Value-Added Cost", label_i18n: {"en":"Non-Value-Added Cost"}, unit: "USD/yr", format: "currency", isBigNumber: true },
    { id: "vsmTotalFinancialImpact", label: "Total Financial Impact", label_i18n: {"en":"Total Financial Impact"}, unit: "USD/yr", format: "currency", isBigNumber: true },
  ],
  thresholds: [
  ...
  assumptions: { hiddenLossMultiplier: 1.2, volatilityPercent: 10, targetMarginPercent: 10, assumptionNotes: ["Value-added ratio = VA time / total lead time.", "Non-value-added cost = (total - VA time) x unit cost x production.", "Scrap and rework costs are included in non-value-added cost."],assumptionNotes_i18n:[{"en":"Value-added ratio = VA time / total lead time."},{"en":"Non-value-added cost = (total - VA time) x unit cost x production."},{"en":"Scrap and rework costs are included in non-value-added cost."}] },
};
