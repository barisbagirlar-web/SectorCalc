...
export const AQL_SAMPLING_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Muayene planı seçiminde AQL ve LTPD arasındaki risk dengesini bilmeden yapılan örnekleme, ya gereksiz maliyet ya da kaçan hata riski doğurur.", painStatement_i18n: {"en":"Muayene planı seçiminde AQL ve LTPD arasındaki risk dengesini bilmeden yapılan örnekleme, ya gereksiz maliyet ya da kaçan hata riski doğurur."},
  inputs: [
    { id: "lotSize", label: "Parti Büyüklüğü (N)", label_i18n: {"en":"Lot Size (N)"}, type: "number", unit: "adet", required: true, smartDefault: 1000, validation: { min: 1 }, helper: "Toplam parti büyüklüğü.", helper_i18n: {"en":"Toplam parti büyüklüğü."}, expertMeaning: "Lot size", expertMeaning_i18n: {"en":"Lot size"} },
    ...
    { id: "costPerDefectEscaped", label: "Kaçan Hata Maliyeti", label_i18n: {"en":"Escaped Defect Cost"}, type: "number", unit: "USD", required: true, smartDefault: 100, validation: { min: 0.01 }, helper: "", expertMeaning: "Cost per defect that escapes", expertMeaning_i18n: {"en":"Cost per defect that escapes"} },
    { id: "actualDefectRate", label: "Ortalama Kusur Oranı (p)", label_i18n: {"en":"Avg Defect Rate (p)"}, type: "number", unit: "%", required: true, smartDefault: 1.5, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Actual process defect rate", expertMeaning_i18n: {"en":"Actual process defect rate"} },
    { id: "detectionRate", label: "Tespit Oranı", label_i18n: {"en":"Detection Rate"}, type: "number", unit: "%", required: false, smartDefault: 80, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Inspection detection effectiveness", expertMeaning_i18n: {"en":"Inspection detection effectiveness"} },
  ],
  outputs: [
    ...
    { id: "ati", label: "ATI (Ort. Toplam Muayene)", label_i18n: {"en":"ATI (Ort. Toplam Muayene)"}, unit: "", format: "number" },
    { id: "totalRiskCost", label: "Toplam Risk Maliyeti", label_i18n: {"en":"Toplam Risk Maliyeti"}, unit: "USD", format: "currency", isBigNumber: true },
  ],
  thresholds: [
