...
export const AQL_SAMPLING_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Muayene planı seçiminde AQL ve LTPD arasındaki risk dengesini bilmeden yapılan örnekleme, ya gereksiz maliyet ya da kaçan hata riski doğurur.", painStatement_i18n: {"en":"Performing sampling without understanding the risk balance between AQL and LTPD in inspection plan selection creates either unnecessary Cost or risk of missed Error."},
  inputs: [
    { id: "lotSize", label: "Lot Size (N)", label_i18n: {"en":"Lot Size (N)"}, type: "number", unit: "adet", required: true, smartDefault: 1000, validation: { min: 1 }, helper: "Toplam parti büyüklüğü.", helper_i18n: {"en":"Total lot size."}, expertMeaning: "Lot size", expertMeaning_i18n: {"en":"Lot size"} },
    ...
    { id: "ltpdPercent", label: "LTPD (%)", label_i18n: {"en":"LTPD (%)"}, type: "number", unit: "%", required: true, smartDefault: 5, validation: { min: 0.01, max: 20 }, helper: "Parti tolerans kusur yüzdesi.", helper_i18n: {"en":"Lot tolerance defect percentage."}, expertMeaning: "Lot Tolerance Percent Defective", expertMeaning_i18n: {"en":"Lot Tolerance Percent Defective"} },
    { id: "sampleSize", label: "Sample Size (n)", label_i18n: {"en":"Sample Size (n)"}, type: "number", unit: "adet", required: true, smartDefault: 80, validation: { min: 1 }, helper: "Muayene edilecek örnek sayısı.", helper_i18n: {"en":"Number of samples to inspect."}, expertMeaning: "Sample size per ANSI/ASQ Z1.4", expertMeaning_i18n: {"en":"Sample size per ANSI/ASQ Z1.4"} },
    { id: "acceptanceNumber", label: "Accept Number (Ac)", label_i18n: {"en":"Accept Number (Ac)"}, type: "number", unit: "adet", required: true, smartDefault: 2, validation: { min: 0 }, helper: "Maksimum kabul edilebilir hatalı sayısı.", helper_i18n: {"en":"Maximum acceptable defect count."}, expertMeaning: "Accept number per code letter", expertMeaning_i18n: {"en":"Accept number per code letter"} },
  ...
  thresholds: [
    { fieldId: "alphaRisk", warning: 5, critical: 10, direction: "higher_is_bad", warningMessage: "Üretici riski > %5 — iyi parti reddedilebilir.", warningMessage_i18n: {"en":"Producer risk > 5% — good lot may be rejected."}, criticalMessage: "Üretici riski > %10 — kabul planı çok sıkı.", criticalMessage_i18n: {"en":"Producer risk > 10% — acceptance plan is too strict."} },
    { fieldId: "betaRisk", warning: 10, critical: 20, direction: "higher_is_bad", warningMessage: "Tüketici riski > %10 — kötü parti kaçabilir.", warningMessage_i18n: {"en":"Consumer risk > 10% — bad lot may be accepted."}, criticalMessage: "Tüketici riski > %20 — kabul planı yetersiz.", criticalMessage_i18n: {"en":"Consumer risk > 20% — acceptance plan is inadequate."} },
  ],
  formulaPipeline: [
