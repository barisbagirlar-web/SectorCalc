...
export const CUT_FILL_BALANCE_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Kazi-dolgu dengesi hesaplanmazsa, fazla hafriyat veya odunc malzeme maliyeti kontrol edilemez.", painStatement_i18n: {"en":"If cut-fill balance is not calculated, overtime excavation or borrow material cost cannot be controlled."},
  inputs: [
    ...
    { id: "shrinkageFactor", label: "Compaction shrinkage", label_i18n: {"en":"Compaction shrinkage"}, type: "number", unit: "", required: false, smartDefault: 0.85, validation: { min: 0, max: 1 }, helper: "", expertMeaning: "Compaction shrinkage", expertMeaning_i18n: {"en":"Compaction shrinkage"} },
    { id: "swellFactor", label: "Kabarma Faktoru", label_i18n: {"en":"Material swell factor"}, type: "number", unit: "", required: false, smartDefault: 1.25, validation: { min: 1 }, helper: "", expertMeaning: "Material swell factor", expertMeaning_i18n: {"en":"Material swell factor"} },
  ...
  outputs: [
    { id: "netBalance", label: "Net Denge", label_i18n: {"en":"Net balance"}, unit: "m³", format: "number" },
    { id: "borrowRequired", label: "Odunc Malzeme", label_i18n: {"en":"borrow material"}, unit: "m³", format: "number" },
  ...
  thresholds: [{ fieldId: "borrowRequired", warning: 500, critical: 2000, direction: "higher_is_bad", warningMessage: "Odunc > 500m³ — maliyet artisi beklenir.", warningMessage_i18n: {"en":"borrow > 500m³ — cost increase expected."}, criticalMessage: "Odunc > 2000m³ — proje ekonomisi riskli.", criticalMessage_i18n: {"en":"borrow > 2000m³ — proje ekonomisi riskli."} }],
  formulaPipeline: [
