...
export const SEED_RATE_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "fieldArea", label: "Tarla Alanı", label_i18n: {"en":"Field Area","tr":"Tarla Alanı"}, type: "number", unit: "dekar", required: true, smartDefault: 100, validation: { min: 1 }, helper: "", expertMeaning: "Field area in decares", expertMeaning_i18n: {"en":"Field area in decares","tr":"Dekar cinsinden tarla alanı"} },
    { id: "seedPerDecare", label: "Dekar Başına Tohum", label_i18n: {"en":"Seed per Decare","tr":"Dekar Başına Tohum"}, type: "number", unit: "kg/dekar", required: true, smartDefault: 20, validation: { min: 0.1 }, helper: "", expertMeaning: "Seed rate per decare", expertMeaning_i18n: {"en":"Seed rate per decare","tr":"Dekar başına tohum oranı"} },
    { id: "seedUnitCost", label: "Tohum Birim Maliyeti", label_i18n: {"en":"Seed Unit Cost","tr":"Tohum Birim Maliyeti"}, type: "number", unit: "USD/kg", required: true, smartDefault: 3, validation: { min: 0.01 }, helper: "", expertMeaning: "Seed cost per kg", expertMeaning_i18n: {"en":"Seed cost per kg","tr":"Kg başına tohum maliyeti"} },
    { id: "germinationRate", label: "Çimlenme Oranı", label_i18n: {"en":"Germination Rate","tr":"Çimlenme Oranı"}, type: "number", unit: "%", required: true, smartDefault: 90, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Expected germination rate", expertMeaning_i18n: {"en":"Expected germination rate","tr":"Beklenen çimlenme oranı"} },
    { id: "wasteRateSeeds", label: "Tohum Fire Oranı", label_i18n: {"en":"Seed Waste Rate","tr":"Tohum Fire Oranı"}, type: "number", unit: "%", required: false, smartDefault: 5, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Seed waste percentage", expertMeaning_i18n: {"en":"Seed waste percentage","tr":"Tohum fire yüzdesi"} },
    { id: "desiredStandCount", label: "Hedef Bitki Sayısı", label_i18n: {"en":"Target Plant Count","tr":"Hedef Bitki Sayısı"}, type: "number", unit: "adet/dekar", required: false, smartDefault: 5000, validation: { min: 0 }, helper: "", expertMeaning: "Target plant stand per decare", expertMeaning_i18n: {"en":"Target plant stand per decare","tr":"Dekar başına hedef bitki sayısı"} },
  ],
  outputs: [
    ...
    { id: "seedCostTotal", label: "Toplam Tohum Maliyeti", label_i18n: {"en":"Total Seed Cost","tr":"Toplam Tohum Maliyeti"}, unit: "USD", format: "currency" },
