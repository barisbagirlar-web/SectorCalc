...
export const POKA_YOKE_ROI_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Hata onleme (Poka-Yoke) yatiriminin geri donusu hesaplanmazsa kalite iyilestirme butcesi verimsiz kullanilabilir.", painStatement_i18n: {"en":"If the return on error prevention (Poka-Yoke) investment is not calculated, the quality improvement budget may be used inefficiently."},
  inputs: [
    ...
    { id: "productionVolume", label: "Uretim Hacmi", label_i18n: {"en":"Annual production volume"}, type: "number", unit: "adet/yil", required: true, smartDefault: 100000, validation: { min: 1 }, helper: "", expertMeaning: "Annual production volume", expertMeaning_i18n: {"en":"Annual production volume"} },
    ...
    { id: "usefulLife", label: "Expected useful life of solution", label_i18n: {"en":"Expected useful life of solution"}, type: "number", unit: "yil", required: false, smartDefault: 5, validation: { min: 1 }, helper: "", expertMeaning: "Expected useful life of solution", expertMeaning_i18n: {"en":"Expected useful life of solution"} },
  ],
  outputs: [
    { id: "currentDefectRate", label: "Mevcut Hata Oran", label_i18n: {"en":"Current Error Rate"}, unit: "%", format: "number" },
    { id: "defectCostAnnual", label: "Yllk Hata Maliyeti", label_i18n: {"en":"Annual Error Cost"}, unit: "USD/yil", format: "currency", isBigNumber: true },
    ...
    { id: "pokaYokeSavings", label: "Yllk Tasarruf", label_i18n: {"en":"Annual Tasarruf"}, unit: "USD/yil", format: "currency", isBigNumber: true },
    { id: "pokaYokeRoi", label: "Yatrm Getirisi (ROI)", label_i18n: {"en":"Yatrm Return (ROI)"}, unit: "%", format: "number" },
    { id: "pokaYokePayback", label: "Geri Odeme Suresi", label_i18n: {"en":"Payback Period"}, unit: "ay", format: "number" },
  ],
  thresholds: [{ fieldId: "pokaYokeRoi", warning: 50, critical: 20, direction: "lower_is_bad", warningMessage: "ROI < %50 — yatirim fizibilitesi sorgulanmali.", warningMessage_i18n: {"en":"ROI < %50 — investment feasibility should be questioned."}, criticalMessage: "ROI < %20 — Poka-Yoke yatirimi onerilmez.", criticalMessage_i18n: {"en":"ROI < %20 — Poka-Yoke investment is not recommended."} }],
  formulaPipeline: [
