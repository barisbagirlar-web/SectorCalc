...
export const BOLT_TORQUE_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Cıvata Tork & Öngerilme Analizi", name_i18n: {"en":"Bolt Torque & Preload Analysis"}, sectorSlug: "cnc-manufacturing", category: "measurement",
  painStatement: "Cıvata bağlantılarında yanlış tork değeri ya yetersiz sıkma (gevşeme) ya da aşırı sıkma (kopma) riski doğurur. Bu araç ISO 898 standartlarına göre tork, öngerilme ve akma kontrolü yapar.", painStatement_i18n: {"en":"Cıvata bağlantılarında yanlış tork değeri ya yetersiz sıkma (gevşeme) ya da aşırı sıkma (kopma) riski doğurur. Bu araç ISO 898 standartlarına göre tork, öngerilme ve akma kontrolü yapar."},
  inputs: [
    { id: "nominalDiameter", label: "Nominal Çap (d)", label_i18n: {"en":"Nominal Diameter (d)"}, type: "number", unit: "mm", required: true, smartDefault: 12, validation: { min: 1 }, helper: "", expertMeaning: "Bolt nominal diameter", expertMeaning_i18n: {"en":"Bolt nominal diameter"} },
    ...
    { id: "torqueCoefficient", label: "Sürtünme Katsayısı (K)", label_i18n: {"en":"Friction Coefficient (K)"}, type: "number", unit: "", required: true, smartDefault: 0.2, validation: { min: 0.05, max: 0.5 }, helper: "", expertMeaning: "Nut factor (typically 0.15-0.25)", expertMeaning_i18n: {"en":"Nut factor (typically 0.15-0.25)"} },
    { id: "grade", label: "Malzeme Sınıfı", label_i18n: {"en":"Material Grade"}, type: "select", unit: "", required: true, smartDefault: "8.8", options: [...GRADE_OPTIONS], helper: "", expertMeaning: "Bolt grade per ISO 898", expertMeaning_i18n: {"en":"Bolt grade per ISO 898"} },
    { id: "yieldStrength", label: "Akma Dayanımı", label_i18n: {"en":"Yield Strength"}, type: "number", unit: "MPa", required: true, smartDefault: 660, validation: { min: 100 }, helper: "", expertMeaning: "Yield strength of bolt material", expertMeaning_i18n: {"en":"Yield strength of bolt material"} },
    ...
    { id: "calibrationError", label: "Tork Anahtarı Kalibrasyon Hatası", label_i18n: {"en":"Torque Wrench Calibration Error"}, type: "number", unit: "%", required: false, smartDefault: 5, validation: { min: 0, max: 50 }, helper: "", expertMeaning: "Wrench calibration uncertainty", expertMeaning_i18n: {"en":"Wrench calibration uncertainty"} },
  ],
  outputs: [
    { id: "pitchDiameter", label: "Kavrama Çapı (d₂)", label_i18n: {"en":"Kavrama Cap (d2)"}, unit: "mm", format: "number" },
    { id: "rootDiameter", label: "Kök Çapı (d₃)", label_i18n: {"en":"Kok Cap (d3)"}, unit: "mm", format: "number" },
    { id: "tensileArea", label: "Çekme Gerilme Alanı (A_t)", label_i18n: {"en":"Cekme Gerilme Alan (A_t)"}, unit: "mm²", format: "number" },
    { id: "preloadForce", label: "Öngerilme Kuvveti (F)", label_i18n: {"en":"Ongerilme Kuvveti (F)"}, unit: "kN", format: "number" },
    { id: "torque", label: "Sıkma Torku (T)", label_i18n: {"en":"Skma Torku (T)"}, unit: "Nm", format: "number" },
