...
export const CROP_YIELD_LOSS_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "interventionCost", label: "Intervention cost", label_i18n: {"en":"Intervention cost"}, type: "number", unit: "USD", required: false, smartDefault: 500, validation: { min: 0 }, helper: "", expertMeaning: "Intervention cost", expertMeaning_i18n: {"en":"Intervention cost"} },
  ],
  outputs: [
    { id: "potentialYield", label: "Potansiyel Efficiency", label_i18n: {"en":"Potansiyel Efficiency"}, unit: "kg", format: "number" },
  ...
  reportTemplate: { title: "Mahsul Efficiency Raporu", title_i18n: {"en":"Mahsul Efficiency Raporu"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.15, volatilityPercent: 15, targetMarginPercent: 20, assumptionNotes: [],assumptionNotes_i18n:[{"en":"Potential = Genetic × Environment."},{"en":"Yield difference = Potential - Actual."},{"en":"Intervention ROI = (Recovered - Cost) / Cost."}] },
};
