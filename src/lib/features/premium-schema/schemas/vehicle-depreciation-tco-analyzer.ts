...
export const VEHICLE_DEPRECIATION_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Araç Amortismanı & TCO Analizi", name_i18n: {"en":"Vehicle Depreciation & TCO Analyzer"}, sectorSlug: "logistics-transport", category: "cost",
  painStatement: "Filo yatırımlarında amortisman yöntemi seçimi ve TCO hesabı yapılmazsa vergi avantajı kaçırılır, nakit akışı yanlış modellenir.", painStatement_i18n: {"en":"Filo yatırımlarında amortisman yöntemi seçimi ve TCO hesabı yapılmazsa vergi avantajı kaçırılır, nakit akışı yanlış modellenir."},
  inputs: [
    ...
    { id: "salvageValue", label: "Kalıntı Değer", label_i18n: {"en":"Residual value at end of life"}, type: "number", unit: "USD", required: true, smartDefault: 10000, validation: { min: 0 }, helper: "Aracın ekonomik ömrü sonundaki tahmini değeri.", helper_i18n: {"en":"Aracın ekonomik ömrü sonundaki tahmini değeri."}, expertMeaning: "Residual value at end of life", expertMeaning_i18n: {"en":"Residual value at end of life"} },
    { id: "usefulLife", label: "Faydalı Ömür", label_i18n: {"en":"Depreciation period"}, type: "number", unit: "yıl", required: true, smartDefault: 5, validation: { min: 1, max: 30 }, helper: "", expertMeaning: "Depreciation period", expertMeaning_i18n: {"en":"Depreciation period"} },
    { id: "annualKm", label: "Yıllık Km Beklentisi", label_i18n: {"en":"For UoP method"}, type: "number", unit: "km/yıl", required: false, smartDefault: 30000, validation: { min: 0 }, helper: "", expertMeaning: "For UoP method", expertMeaning_i18n: {"en":"For UoP method"} },
    { id: "totalExpectedKm", label: "Toplam Beklenen Km", label_i18n: {"en":"Toplam Beklenen Km"}, type: "number", unit: "km", required: false, smartDefault: 150000, validation: { min: 0 }, helper: "", expertMeaning: "Total useful life in km (UoP)", expertMeaning_i18n: {"en":"Total useful life in km (UoP)"} },
    ...
    { id: "wacc", label: "İskonto Oranı (WACC)", label_i18n: {"en":"Discount rate for PV"}, type: "number", unit: "%", required: true, smartDefault: 10, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Discount rate for PV", expertMeaning_i18n: {"en":"Discount rate for PV"} },
    { id: "annualOpCost", label: "Yıllık İşletme Maliyeti", label_i18n: {"en":"Insurance, registration, tires"}, type: "number", unit: "USD", required: false, smartDefault: 5000, validation: { min: 0 }, helper: "", expertMeaning: "Insurance, registration, tires", expertMeaning_i18n: {"en":"Insurance, registration, tires"} },
    { id: "annualMaintCost", label: "Yıllık Bakım Maliyeti", label_i18n: {"en":"Repairs, maintenance"}, type: "number", unit: "USD", required: false, smartDefault: 3000, validation: { min: 0 }, helper: "", expertMeaning: "Repairs, maintenance", expertMeaning_i18n: {"en":"Repairs, maintenance"} },
  ],
  outputs: [
    { id: "slAnnual", label: "SL Yıllık Amortisman", label_i18n: {"en":"SL Yllk Amortisman"}, unit: "USD", format: "currency" },
    { id: "slMonthly", label: "SL Aylık Amortisman", label_i18n: {"en":"SL Aylk Amortisman"}, unit: "USD", format: "currency" },
    { id: "dbRate", label: "DB Yıllık Oran", label_i18n: {"en":"DB Yllk Oran"}, unit: "%", format: "percentage" },
    { id: "taxShieldAnnual", label: "Yıllık Vergi Kalkanı", label_i18n: {"en":"Yllk Vergi Kalkan"}, unit: "USD", format: "currency" },
    { id: "tco", label: "TCO (Toplam Sahip Olma Maliyeti)", label_i18n: {"en":"TCO (Toplam Sahip Olma Maliyeti)"}, unit: "USD", format: "currency", isBigNumber: true },
  ],
  thresholds: [
