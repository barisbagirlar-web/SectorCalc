...
export const VEHICLE_DEPRECIATION_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Filo yatirimlarinda amortisman yontemi secimi ve TCO calc yapilmazsa vergi avantaji kacirilir, nakit akisi yanlis modellenir.", painStatement_i18n: {"en":"If depreciation method selection and TCO calculation are not performed in fleet investments, tax advantage is missed and cash flow is incorrectly modeled."},
  inputs: [
    ...
    { id: "salvageValue", label: "Residual value at end of life", label_i18n: {"en":"Residual value at end of life"}, type: "number", unit: "USD", required: true, smartDefault: 10000, validation: { min: 0 }, helper: "Aracin ekonomik omru sonundaki tahmini degeri.", helper_i18n: {"en":"Estimated value of the asset at the end of its economic life."}, expertMeaning: "Residual value at end of life", expertMeaning_i18n: {"en":"Residual value at end of life"} },
    { id: "usefulLife", label: "Depreciation period", label_i18n: {"en":"Depreciation period"}, type: "number", unit: "yil", required: true, smartDefault: 5, validation: { min: 1, max: 30 }, helper: "", expertMeaning: "Depreciation period", expertMeaning_i18n: {"en":"Depreciation period"} },
    ...
    { id: "depreciationMethod", label: "Amortisman Yontemi", label_i18n: {"en":"SL, DB, MACRS or UoP"}, type: "select", unit: "", required: true, smartDefault: "SL", options: [...DEPRECIATION_METHOD_OPTIONS], helper: "", expertMeaning: "SL, DB, MACRS or UoP", expertMeaning_i18n: {"en":"SL, DB, MACRS or UoP"} },
  ...
  thresholds: [
    { fieldId: "tco", warning: 75000, critical: 100000, direction: "higher_is_bad", warningMessage: "TCO > $75K — maliyet optimizasyonu degerlendir.", warningMessage_i18n: {"en":"TCO > $75K — Evaluate cost optimization."}, criticalMessage: "TCO > $100K — alternatif arac/finansman modelleri incelenmeli.", criticalMessage_i18n: {"en":"TCO > $100K — Alternative vehicle/financing models must be reviewed."} },
  ],
  formulaPipeline: [
