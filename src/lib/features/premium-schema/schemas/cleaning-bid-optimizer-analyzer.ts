...
export const CLEANING_BID_OPTIMIZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "laborRate", label: "Hourly labor rate", label_i18n: {"en":"Hourly labor rate"}, type: "number", unit: "USD/saat", required: true, smartDefault: 20, validation: { min: 1 }, helper: "", expertMeaning: "Hourly labor rate", expertMeaning_i18n: {"en":"Hourly labor rate"} },
    { id: "hoursPerVisit", label: "Hours per cleaning visit", label_i18n: {"en":"Hours per cleaning visit"}, type: "number", unit: "saat", required: true, smartDefault: 4, validation: { min: 0.5 }, helper: "", expertMeaning: "Hours per cleaning visit", expertMeaning_i18n: {"en":"Hours per cleaning visit"} },
  ...
  thresholds: [{ fieldId: "cleaningBidPrice", warning: 0, critical: 0, direction: "higher_is_bad", warningMessage: "Teklif > rakip — rekabetcilik dusebilir.", warningMessage_i18n: {"en":"quote > competitor — competitiveness may decline."}, criticalMessage: "Teklif maliyetin altinda — zarar risk var.", criticalMessage_i18n: {"en":"quote below cost — Loss risk present."} }],
  formulaPipeline: [
