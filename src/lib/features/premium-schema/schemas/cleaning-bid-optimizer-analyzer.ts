...
export const CLEANING_BID_OPTIMIZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "laborRate", label: "İşçilik Saatlik Ücreti", label_i18n: {"en":"Hourly labor rate","tr":"İşçilik Saatlik Ücreti"}, type: "number", unit: "USD/saat", required: true, smartDefault: 20, validation: { min: 1 }, helper: "", expertMeaning: "Hourly labor rate", expertMeaning_i18n: {"en":"Hourly labor rate","tr":"i̇şçilik saatlik ücreti"} },
    { id: "hoursPerVisit", label: "Ziyaret Başına Saat", label_i18n: {"en":"Hours per cleaning visit","tr":"Ziyaret Başına Saat"}, type: "number", unit: "saat", required: true, smartDefault: 4, validation: { min: 0.5 }, helper: "", expertMeaning: "Hours per cleaning visit", expertMeaning_i18n: {"en":"Hours per cleaning visit","tr":"ziyaret başına saat"} },
    { id: "materialCostPerVisit", label: "Ziyaret Başına Malzeme", label_i18n: {"en":"Material cost per cleaning visit","tr":"Ziyaret Başına Malzeme"}, type: "number", unit: "USD", required: true, smartDefault: 30, validation: { min: 0 }, helper: "", expertMeaning: "Material cost per cleaning visit", expertMeaning_i18n: {"en":"Material cost per cleaning visit","tr":"ziyaret başına malzeme"} },
  ...
  outputs: [
    { id: "cleaningLaborCost", label: "İşçilik Maliyeti", label_i18n: {"en":"Iscilik Maliyeti","tr":"İşçilik Maliyeti"}, unit: "USD/ay", format: "currency" },
    { id: "cleaningBidPrice", label: "Önerilen Teklif Fiyatı", label_i18n: {"en":"Onerilen Teklif Fiyat","tr":"Önerilen Teklif Fiyatı"}, unit: "USD/ay", format: "currency" },
  ],
