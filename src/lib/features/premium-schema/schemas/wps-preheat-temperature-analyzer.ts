...
export const WPS_PREHEAT_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Kaynak öncesi ön ısıtma sıcaklığı doğru hesaplanmazsa çatlama riski artar ve enerji maliyeti yükselir. Karbon eşdeğeri ve malzeme kalınlığına göre optimum sıcaklık belirlenmelidir.", painStatement_i18n: {"en":"resource öncesi pre ısıtma sıcaklığı accurate if not calculated çatlama riski artar ve energy Cost yükselir. Carbon eşdeğeri ve material kalınlığına per optimum Temperature belirlenmelidir."},
  inputs: [
  ...
  thresholds: [
    { fieldId: "carbonEquivalent", warning: 0.4, critical: 0.55, direction: "higher_is_bad", warningMessage: "CEV > 0.4 — ön ısıtma gereklidir, hidrojen kontrolü yapılmalı.", warningMessage_i18n: {"en":"CEV > 0.4 — pre ısıtma gereklidir, hidrojen kontrolü yapılmalı."}, criticalMessage: "CEV > 0.55 — yüksek çatlama riski, kaynak prosedürü gözden geçirilmeli.", criticalMessage_i18n: {"en":"CEV > 0.55 — high çatlama riski, resource prosedürü gözden geçirilmeli."} },
    { fieldId: "preheatEnergyCost", warning: 500, critical: 1500, direction: "higher_is_bad", warningMessage: "Ön ısıtma maliyeti > $500 — enerji verimliliği değerlendirilmeli.", warningMessage_i18n: {"en":"pre ısıtma Cost > $500 — energy verimliliği değerlendirilmeli."}, criticalMessage: "Ön ısıtma maliyeti > $1,500 — alternatif kaynak yöntemleri araştırılmalı.", criticalMessage_i18n: {"en":"pre ısıtma Cost > $1,500 — alternatif resource yöntemleri araştırılmalı."} },
  ],
  formulaPipeline: [
  ...
  reportTemplate: { title: "WPS Ön Isıtma Sıcaklık Analiz Raporu", title_i18n: {"en":"WPS pre Isıtma Temperature Analiz Raporu"}, sections: ["executive_summary", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
