/**
 * Tool #29 — Toplam Calisan Maliyeti
 */
...
export const TOTAL_EMPLOYEE_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Calisan basina brut maas, yan haklar ve isveren payi toplami hesaplanmazsa gercek isgucu maliyeti gizli kalir.", painStatement_i18n: {"en":"If the total of employee gross salary, benefits, and employer contribution is not calculated, the actual labor cost remains hidden."},
  inputs: [
    { id: "numEmployees", label: "Number of employees", label_i18n: {"en":"Number of employees"}, type: "number", unit: "kisi", required: true, smartDefault: 50, validation: { min: 1 }, helper: "", expertMeaning: "Number of employees", expertMeaning_i18n: {"en":"Number of employees"} },
    ...
    { id: "trainingCostPerEmployee", label: "Annual training cost per employee", label_i18n: {"en":"Annual training cost per employee"}, type: "number", unit: "USD/yil", required: false, smartDefault: 500, validation: { min: 0 }, helper: "", expertMeaning: "Annual training cost per employee", expertMeaning_i18n: {"en":"Annual training cost per employee"} },
  ...
  thresholds: [{ fieldId: "employeeCostPerHour", warning: 30, critical: 50, direction: "higher_is_bad", warningMessage: "Saatlik maliyet > $30 — verimlilik analizi onerilir.", warningMessage_i18n: {"en":"Hourly cost > $30 — productivity analysis is recommended."}, criticalMessage: "Saatlik maliyet > $50 — maliyet yapisi optimize edilmeli.", criticalMessage_i18n: {"en":"Hourly cost > $50 — cost structure should be optimized."} }],
  formulaPipeline: [
