...
export const DAIRY_PROFIT_DETECTOR_ANALYZER: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "dailyFeedCostPerCow", label: "Daily feed cost per cow", label_i18n: {"en":"Daily feed cost per cow"}, type: "number", unit: "USD/gun", required: true, smartDefault: 6.5, validation: { min: 0.01 }, helper: "", expertMeaning: "Daily feed cost per cow", expertMeaning_i18n: {"en":"Daily feed cost per cow"} },
    ...
    { id: "otherCostsPerCow", label: "Other daily costs per cow", label_i18n: {"en":"Other daily costs per cow"}, type: "number", unit: "USD/gun", required: false, smartDefault: 1.5, validation: { min: 0 }, helper: "", expertMeaning: "Other daily costs per cow", expertMeaning_i18n: {"en":"Other daily costs per cow"} },
  ],
  outputs: [
    { id: "fcmMilk", label: "Corrected Milk Yield (FCM)", label_i18n: {"en":"Corrected Milk Yield (FCM)"}, unit: "kg/gun", format: "number" },
    { id: "dairyIncomeOverFeed", label: "Yem Uzeri Gelir (IOFC)", label_i18n: {"en":"Income Over Feed Cost (IOFC)"}, unit: "USD/gun", format: "currency", isBigNumber: true },
  ],
  thresholds: [{ fieldId: "dairyIncomeOverFeed", warning: 3, critical: 1.5, direction: "lower_is_bad", warningMessage: "Yem uzeri gelir <$3/inek/gun — kârlilik daraliyor.", warningMessage_i18n: {"en":"Feed Over Income <$3/cow/day — profitability is contracting."}, criticalMessage: "Yem uzeri gelir <$1.5/inek/gun — isletme zarar risk yuksek.", criticalMessage_i18n: {"en":"Feed Over Income <$1.5/cow/day — operation loss risk is high."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Sut Kâr Dedektoru Reportu", title_i18n: {"en":"Milk Profit Detector Report"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["FCM = verim × (0.4324 + 0.1625 × yag% + 0.0345 × protein%).", "IOFC = (FCM × sut fiyati) − (yem maliyeti + diger maliyetler).", "SCC >400K hucre/mL kalite primi kaybina yol acar.", "NRC 2001 yem standartlari baz alinmistir."],assumptionNotes_i18n:[{"en":"FCM = yield × (0.4324 + 0.1625 × fat% + 0.0345 × protein%)."},{"en":"IOFC = (FCM × milk price) − (feed cost + other costs)."},{"en":"SCC >400K cells/mL leads to quality premium loss."},{"en":"NRC 2001 feed standards are used as the basis."}] },
};
