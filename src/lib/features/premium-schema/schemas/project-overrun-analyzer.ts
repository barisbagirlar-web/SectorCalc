...
export const PROJECT_OVERRUN_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Proje Aşım Riski ve Kazanılmış Değer", name_i18n: {"en":"Project Overrun Risk & Earned Value"}, sectorSlug: "construction", category: "cost",
  painStatement: "Proje aşımları erken tespit edilmezse bütçe şişer, teslimat gecikir ve taahhüt cezaları devreye girer.", painStatement_i18n: {"en":"If project overruns are not detected early, budget swells, delivery is delayed, and commitment penalties kick in."},
  inputs: [
    { id: "plannedValue", label: "Planlanan Değer (PV)", label_i18n: {"en":"Planned Value (PV)"}, type: "number", unit: "USD", required: true, smartDefault: 1000000, validation: { min: 1 }, helper: "", expertMeaning: "Planned Value (budget)", expertMeaning_i18n: {"en":"Planned Value (budget)"} },
    { id: "earnedValue", label: "Kazanılan Değer (EV)", label_i18n: {"en":"Earned Value (EV)"}, type: "number", unit: "USD", required: true, smartDefault: 750000, validation: { min: 0 }, helper: "", expertMeaning: "Earned Value (work completed)", expertMeaning_i18n: {"en":"Earned Value (work completed)"} },
    { id: "actualCost", label: "Gerçekleşen Maliyet (AC)", label_i18n: {"en":"Actual Cost (AC)"}, type: "number", unit: "USD", required: true, smartDefault: 800000, validation: { min: 0 }, helper: "", expertMeaning: "Actual Cost incurred", expertMeaning_i18n: {"en":"Actual Cost incurred"} },
    ...
    { id: "actualDays", label: "Gerçekleşen Süre", label_i18n: {"en":"Actual Duration"}, type: "number", unit: "gün", required: false, smartDefault: 160, validation: { min: 0 }, helper: "", expertMeaning: "Actual elapsed days", expertMeaning_i18n: {"en":"Actual elapsed days"} },
    { id: "riskProbability", label: "Risk Olasılığı", label_i18n: {"en":"Risk Probability"}, type: "number", unit: "%", required: false, smartDefault: 25, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Probability of risk events", expertMeaning_i18n: {"en":"Probability of risk events"} },
  ...
  outputs: [
    ...
    { id: "cpi", label: "Maliyet Performans Endeksi (CPI)", label_i18n: {"en":"Maliyet Performans Endeksi (CPI)"}, unit: "", format: "number" },
    { id: "eac", label: "Tahmini Tamamlanma Maliyeti (EAC)", label_i18n: {"en":"Tahmini Tamamlanma Maliyeti (EAC)"}, unit: "USD", format: "currency", isBigNumber: true },
    { id: "expectedOverrun", label: "Beklenen Aşım", label_i18n: {"en":"Beklenen Asm"}, unit: "USD", format: "currency", isBigNumber: true },
  ...
  thresholds: [{ fieldId: "cpi", warning: 0.95, critical: 0.85, direction: "lower_is_bad", warningMessage: "CPI < 0.95 — maliyet aşımı sinyali.", warningMessage_i18n: {"en":"CPI < 0.95 — cost overrun signal."}, criticalMessage: "CPI < 0.85 — acil maliyet düzeltici aksiyon.", criticalMessage_i18n: {"en":"CPI < 0.85 — urgent cost corrective action."} }],
  formulaPipeline: [
  ...
  assumptions: { hiddenLossMultiplier: 1.0, volatilityPercent: 15, targetMarginPercent: 15, assumptionNotes: ["SPI = EV/PV, CPI = EV/AC.", "EAC = BAC/CPI (base varsayımı).", "Beklenen aşım = EAC - BAC.", "Net risk = risk maruziyeti - azaltma maliyeti."],assumptionNotes_i18n:[{"en":"SPI = EV/PV, CPI = EV/AC."},{"en":"EAC = BAC/CPI (base assumption)."},{"en":"Expected overrun = EAC - BAC."},{"en":"Net risk = risk exposure - mitigation cost."}]},
};
