/**
 * Tool #37 — Proje Asim Riski Analizi
 */
...
export const PROJECT_OVERRUN_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "scheduleDays", label: "Planlanan Sure", label_i18n: {"en":"Planned Duration"}, type: "number", unit: "gun", required: true, smartDefault: 200, validation: { min: 1 }, helper: "", expertMeaning: "Planned schedule duration", expertMeaning_i18n: {"en":"Planned schedule duration"} },
    { id: "actualDays", label: "Actual Duration", label_i18n: {"en":"Actual Duration"}, type: "number", unit: "gun", required: false, smartDefault: 160, validation: { min: 0 }, helper: "", expertMeaning: "Actual elapsed days", expertMeaning_i18n: {"en":"Actual elapsed days"} },
  ...
  outputs: [
    ...
    { id: "scheduleDelay", label: "Program Gecikmesi", label_i18n: {"en":"Program Gecikmesi"}, unit: "gun", format: "number" },
  ...
  thresholds: [{ fieldId: "cpi", warning: 0.95, critical: 0.85, direction: "lower_is_bad", warningMessage: "CPI < 0.95 — cost overrun signal.", warningMessage_i18n: {"en":"CPI < 0.95 — cost overrun signal."}, criticalMessage: "CPI < 0.85 — acil maliyet duzeltici aksiyon.", criticalMessage_i18n: {"en":"CPI < 0.85 — urgent cost corrective action."} }],
  formulaPipeline: [
