...
export const PROJECT_OVERRUN_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "scheduleDays", label: "Planned Duration", label_i18n: {"en":"Planned Duration"}, type: "number", unit: "days", required: true, smartDefault: 200, validation: { min: 1 }, helper: "", expertMeaning: "Planned schedule duration", expertMeaning_i18n: {"en":"Planned schedule duration"} },
    { id: "actualDays", label: "Actual Duration", label_i18n: {"en":"Actual Duration"}, type: "number", unit: "days", required: false, smartDefault: 160, validation: { min: 0 }, helper: "", expertMeaning: "Actual elapsed days", expertMeaning_i18n: {"en":"Actual elapsed days"} },
    ...
    { id: "riskImpact", label: "Risk Impact", label_i18n: {"en":"Risk Impact"}, type: "number", unit: "USD", required: false, smartDefault: 50000, validation: { min: 0 }, helper: "", expertMeaning: "Financial impact of risks", expertMeaning_i18n: {"en":"Financial impact of risks"} },
  ...
  assumptions: { hiddenLossMultiplier: 1.0, volatilityPercent: 15, targetMarginPercent: 15, assumptionNotes: [],assumptionNotes_i18n:[{"en":"SPI = EV/PV, CPI = EV/AC."},{"en":"EAC = BAC/CPI (base assumption)."},{"en":"Expected overrun = EAC - BAC."},{"en":"Net risk = risk exposure - mitigation cost."}]},
};
