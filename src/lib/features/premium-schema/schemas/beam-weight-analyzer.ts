...
export const BEAM_WEIGHT_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "modulusE", label: "Elastisite Modulu E", label_i18n: {"en":"Young's Modulus E"}, type: "number", unit: "GPa", required: false, smartDefault: 210, validation: { min: 0 }, helper: "", expertMeaning: "Young's modulus", expertMeaning_i18n: {"en":"Young's modulus"} },
  ...
  thresholds: [{ fieldId: "materialCost", warning: 50000, critical: 150000, direction: "higher_is_bad", warningMessage: "Malzeme > $50K — alternatif profil degerlendirilmeli.", warningMessage_i18n: {"en":"Material > $50K — alternative profile should be evaluated."}, criticalMessage: "Malzeme > $150K — butce revizyonu gerekli.", criticalMessage_i18n: {"en":"material > $150K — budget revizyonu gerekli."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Kiris Agirlik Reportu", title_i18n: {"en":"Beam Weight Report"}, sections: ["executive_summary", "thresholds", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.05, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["Agirlik = Kesit × Uzunluk × Adet × Yogunluk.", "Kesit alani profil tipine gore lookup tablosundan."],assumptionNotes_i18n:[{"en":"Weight = Section × Length × Quantity × Density."},{"en":"Section area from lookup table by profile type."}]},
};
