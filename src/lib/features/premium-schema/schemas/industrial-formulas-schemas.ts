...
const BUILDING_TYPE_OPTIONS = [
  { value: "warehouse", label: "Warehouse / Storage (fire load 500-1000 MJ/m\u00b2)", label_i18n: {"en":"Warehouse / Storage (waste load 500-1000 MJ/m\u00b2)"} },
  { value: "industrial", label: "Industrial / Factory (fire load 400-800 MJ/m\u00b2)", label_i18n: {"en":"Industrial / Factory (waste load 400-800 MJ/m\u00b2)"} },
  { value: "commercial", label: "Commercial / Office (fire load 300-600 MJ/m\u00b2)", label_i18n: {"en":"Commercial / Office (waste load 300-600 MJ/m\u00b2)"} },
  { value: "retail", label: "Retail / Mall (fire load 400-700 MJ/m\u00b2)", label_i18n: {"en":"Retail / Mall (waste load 400-700 MJ/m\u00b2)"} },
  { value: "parking", label: "Parking garage (fire load 200-400 MJ/m\u00b2)", label_i18n: {"en":"Parking garage (waste load 200-400 MJ/m\u00b2)"} },
] as const;
...
const SMOKE_EXHAUST_SHEV_SCHEMA: PremiumCalculatorSchema = {
  ...
  sectorSlug: "general",
  category: "measurement",
  painStatement: "Smoke exhaust sizing per EN 12101-5 requires iterative calculation of fire perimeter, plume mass flow, and vent area \u2014 a 20% undersized SHEV can render a smoke control system ineffective during a fire event.", painStatement_i18n: {"en":"Smoke exhaust sizing per EN 12101-5 requires iterative calculation of waste perimeter, plume mass flow, and vent area \u2014 a 20% undersized SHEV can render a smoke control system ineffective during a waste event."},
  inputs: [
    { id: "buildingType", label: "Building type", label_i18n: {"en":"Building type"}, type: "select", unit: "", required: true, smartDefault: "warehouse", options: [...BUILDING_TYPE_OPTIONS], helper: "Building occupancy type determines design fire.", helper_i18n: {"en":"Building occupancy type determines design fire."}, expertMeaning: "Design fire HRR per EN 12101-5 Table A.1", expertMeaning_i18n: {"en":"Design waste HRR per EN 12101-5 Table A.1"} },
    ...
    { id: "zoneLength", label: "Zone length L", label_i18n: {"en":"Zone length L"}, type: "number", unit: "m", required: true, smartDefault: 50, validation: { min: 1 }, helper: "Smoke zone length.", helper_i18n: {"en":"Smoke zone length."}, expertMeaning: "Fire perimeter ~ 2\u00d7(L+W) for design fire", expertMeaning_i18n: {"en":"waste perimeter ~ 2\u00d7(L+W) for design waste"} },
    { id: "zoneWidth", label: "Zone width W", label_i18n: {"en":"Zone width W"}, type: "number", unit: "m", required: true, smartDefault: 40, validation: { min: 1 }, helper: "Smoke zone width.", helper_i18n: {"en":"Smoke zone width."}, expertMeaning: "Fire perimeter ~ 2\u00d7(L+W) for design fire", expertMeaning_i18n: {"en":"waste perimeter ~ 2\u00d7(L+W) for design waste"} },
    ...
    { id: "fireArea", label: "Fire area A_fire", label_i18n: {"en":"waste area A_fire"}, type: "number", unit: "m\u00b2", required: false, smartDefault: 36, validation: { min: 0.1 }, helper: "Design fire area (typically 6m \u00d7 6m = 36 m\u00b2 for warehouses).", helper_i18n: {"en":"Design fire area (typically 6m \u00d7 6m = 36 m\u00b2 for warehouses)."}, expertMeaning: "Fire perimeter P = 2\u00d7\u221a(\u03c0\u00d7A_fire) for circular fire", expertMeaning_i18n: {"en":"waste perimeter P = 2\u00d7\u221a(\u03c0\u00d7A_fire) for circular waste"} },
    ...
    { id: "windSpeed", label: "External wind speed", label_i18n: {"en":"External wind speed"}, type: "number", unit: "m/s", required: false, smartDefault: 5, validation: { min: 0, max: 50 }, helper: "Design wind speed at roof level.", helper_i18n: {"en":"Design wind speed at roof level."}, expertMeaning: "Wind effect on natural vent performance per EN 12101-2", expertMeaning_i18n: {"en":"Wind effect pre natural vent performance per EN 12101-2"} },
  ],
  outputs: [
    { id: "firePerimeter", label: "Fire perimeter P", label_i18n: {"en":"waste perimeter P"}, unit: "m", format: "number" },
...
const CPM_PERT_CALCULATOR_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "kritikAktiviteSayisi", label: "Activities on critical path", label_i18n: {"en":"Activities pre critical path"}, type: "number", unit: "", required: true, smartDefault: 5, validation: { min: 1 }, helper: "Number of activities on the longest path determining project duration.", helper_i18n: {"en":"Number of activities on the longest path determining project duration."}, expertMeaning: "k for critical path variance sum", expertMeaning_i18n: {"en":"k for critical path variance sum"} },
...
const FMEA_RPN_SCHEMA: PremiumCalculatorSchema = {
  ...
  formulaPipeline: [
    { formulaId: "industrial.fmea_0", inputMap: { ortalamaSiddet_S: "ortalamaSiddet_S", ortalamaOlusma_O: "ortalamaOlusma_O", ortalamaSaptama_D: "ortalamaSaptama_D" }, outputId: "RPN_ortalama" },
    { formulaId: "industrial.fmea_3", inputMap: {}, outputId: "RPN_max" },
    { formulaId: "industrial.fmea_1", inputMap: { RPN_ortalama: "RPN_ortalama" }, outputId: "riskDuzeyi" },
    { formulaId: "industrial.fmea_4", inputMap: { RPN_ortalama: "RPN_ortalama", ortalamaSiddet_S: "ortalamaSiddet_S", ortalamaOlusma_O: "ortalamaOlusma_O", ortalamaSaptama_D: "ortalamaSaptama_D" }, outputId: "oncelikSirasi" },
    { formulaId: "industrial.fmea_5", inputMap: { maliyet_failure: "maliyet_failure", prosesAdimiSayisi: "prosesAdimiSayisi", ortalamaOlusma_O: "ortalamaOlusma_O" }, outputId: "failureMaliyet_toplam" },
    { formulaId: "industrial.fmea_6", inputMap: { maliyet_onlem: "maliyet_onlem", prosesAdimiSayisi: "prosesAdimiSayisi" }, outputId: "onlemMaliyet_toplam" },
    ...
    { formulaId: "industrial.fmea_1", inputMap: { RPN_ortalama: "RPN_ortalama" }, outputId: "cozumOncelikleri" },
  ],
...
const KAIZEN_EVENT_SCHEMA: PremiumCalculatorSchema = {
  ...
  outputs: [
    ...
    { id: "ROI", label: "Return on Investment (ROI)", label_i18n: {"en":"Return pre Investment (ROI)"}, unit: "%", format: "percentage" },
