...
export const COMPRESSOR_TANK_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "airFlow", label: "Kompresor Debisi Q", label_i18n: {"en":"Free air delivery"}, type: "number", unit: "m³/dk", required: true, smartDefault: 2, validation: { min: 0.01 }, helper: "", expertMeaning: "Free air delivery", expertMeaning_i18n: {"en":"Free air delivery"} },
    ...
    { id: "fillTimeTarget", label: "Hedef Dolum Suresi", label_i18n: {"en":"Target fill time"}, type: "number", unit: "sn", required: true, smartDefault: 30, validation: { min: 1 }, helper: "", expertMeaning: "Target fill time", expertMeaning_i18n: {"en":"Target fill time"} },
  ...
  outputs: [
    ...
    { id: "cyclesPerHour", label: "Saatteki Cevrim", label_i18n: {"en":"Saatteki Cycle"}, unit: "cevrim/saat", format: "number" },
  ...
  thresholds: [{ fieldId: "motorStartCheck", warning: 0, critical: 0, direction: "higher_is_bad", warningMessage: "Motor start limit asildi — tank hacmi artirilmali.", warningMessage_i18n: {"en":"Motor start limit exceeded — tank Volume must be increased."}, criticalMessage: "" }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Kompresor Tanki Reportu", title_i18n: {"en":"Compressor Tank Report"}, sections: ["executive_summary", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1, volatilityPercent: 5, targetMarginPercent: 10, assumptionNotes: ["Tank = t×Q×Patm / (Pmax - Pmin).", "Cevrim = Tank×(Pmax-Pmin) / (Q×Patm).", "Motor start < maxStart/saat."],assumptionNotes_i18n:[{"en":"Tank = t×Q×Patm / (Pmax - Pmin)."},{"en":"Cycle = Tank×(Pmax-Pmin) / (Q×Patm)."},{"en":"Motor start < maxStart/saat."}] },
};
