...
export const COMPRESSOR_TANK_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "airFlow", label: "Free air delivery", label_i18n: {"en":"Free air delivery"}, type: "number", unit: "m³/dk", required: true, smartDefault: 2, validation: { min: 0.01 }, helper: "", expertMeaning: "Free air delivery", expertMeaning_i18n: {"en":"Free air delivery"} },
    ...
    { id: "fillTimeTarget", label: "Target fill time", label_i18n: {"en":"Target fill time"}, type: "number", unit: "sn", required: true, smartDefault: 30, validation: { min: 1 }, helper: "", expertMeaning: "Target fill time", expertMeaning_i18n: {"en":"Target fill time"} },
    { id: "maxStartsPerHour", label: "Max Start/Saat", label_i18n: {"en":"Max Start/Saat"}, type: "number", unit: "scalar", required: true, smartDefault: 10, validation: { min: 1 }, helper: "", expertMeaning: "Max Motor starts per hour", expertMeaning_i18n: {"en":"Max Motor starts per hour"} },
    ...
    { id: "currentTankVol", label: "Current Tank Volume", label_i18n: {"en":"Current Tank Volume"}, type: "number", unit: "L", required: false, smartDefault: 500, validation: { min: 0 }, helper: "", expertMeaning: "Existing tank volume", expertMeaning_i18n: {"en":"Existing tank volume"} },
  ],
  outputs: [
    { id: "requiredTankVol", label: "required Tank Volume", label_i18n: {"en":"required Tank Volume"}, unit: "L", format: "number", isBigNumber: true },
    { id: "cycleTime", label: "Cycle Time", label_i18n: {"en":"Cycle Time"}, unit: "dk", format: "duration" },
    { id: "cyclesPerHour", label: "Saatteki Cycle", label_i18n: {"en":"Saatteki Cycle"}, unit: "cycles/hour", format: "number" },
  ...
  reportTemplate: { title: "Compressor Tank Report", title_i18n: {"en":"Compressor Tank Report"}, sections: ["executive_summary", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1, volatilityPercent: 5, targetMarginPercent: 10, assumptionNotes: [],assumptionNotes_i18n:[{"en":"Tank = t×Q×Patm / (Pmax - Pmin)."},{"en":"Cycle = Tank×(Pmax-Pmin) / (Q×Patm)."},{"en":"Motor start < maxStart/saat."}] },
};
