...
export const RECURRING_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Tekrarlayan hatalar ve maliyetler kök neden analizi yapılmadan devam ederse toplam kayıp katlanarak büyür.", painStatement_i18n: {"en":"If recurring errors and costs continue without root cause analysis, total loss grows exponentially."},
  inputs: [
    { id: "monthlyRecurringCost", label: "Aylık Tekrarlayan Maliyet", label_i18n: {"en":"Monthly Recurring Cost"}, type: "number", unit: "USD/ay", required: true, smartDefault: 5000, validation: { min: 1 }, helper: "", expertMeaning: "Monthly recurring defect cost", expertMeaning_i18n: {"en":"Monthly recurring defect cost"} },
    ...
    { id: "discountRate", label: "İskonto Oranı", label_i18n: {"en":"Discount Rate"}, type: "number", unit: "%", required: false, smartDefault: 10, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Discount rate for NPV", expertMeaning_i18n: {"en":"Discount rate for NPV"} },
    { id: "annualGrowthRecurring", label: "Yıllık Maliyet Artış Oranı", label_i18n: {"en":"Annual Cost Growth Rate"}, type: "number", unit: "%", required: false, smartDefault: 3, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Annual cost growth rate", expertMeaning_i18n: {"en":"Annual cost growth rate"} },
  ],
  outputs: [
    { id: "recurringAnnualCost", label: "Yıllık Tekrarlayan Maliyet", label_i18n: {"en":"Annual Recurring Cost"}, unit: "USD/yıl", format: "currency" },
    { id: "presentValueRecurring", label: "Tekrarlayan Maliyetin Bugünkü Değeri", label_i18n: {"en":"PV of Recurring Cost"}, unit: "USD", format: "currency" },
  ...
  thresholds: [{ fieldId: "rootCausePayback", warning: 12, critical: 24, direction: "higher_is_bad", warningMessage: "Geri ödeme süresi > 12 ay — önceliklendirme yapılmalı.", warningMessage_i18n: {"en":"Payback period > 12 months — prioritize accordingly."}, criticalMessage: "Geri ödeme süresi > 24 ay — proje fizibilitesi sorgulanmalı.", criticalMessage_i18n: {"en":"Payback period > 24 months — question project feasibility."} }],
  formulaPipeline: [
