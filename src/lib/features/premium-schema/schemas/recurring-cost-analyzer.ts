...
export const RECURRING_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "monthlyRecurringCost", label: "Aylık Tekrarlayan Maliyet", label_i18n: {"en":"Monthly Recurring Cost","tr":"Aylık Tekrarlayan Maliyet"}, type: "number", unit: "USD/ay", required: true, smartDefault: 5000, validation: { min: 1 }, helper: "", expertMeaning: "Monthly recurring defect cost", expertMeaning_i18n: {"en":"Monthly recurring defect cost","tr":"Aylık tekrarlayan hata maliyeti"} },
    { id: "eliminationProjectCost", label: "Giderme Proje Maliyeti", label_i18n: {"en":"Elimination Project Cost","tr":"Giderme Proje Maliyeti"}, type: "number", unit: "USD", required: true, smartDefault: 40000, validation: { min: 1 }, helper: "", expertMeaning: "Root cause elimination project cost", expertMeaning_i18n: {"en":"Root cause elimination project cost","tr":"Kök neden giderme proje maliyeti"} },
    { id: "expectedLifeYears", label: "Beklenen Proje Ömrü", label_i18n: {"en":"Expected Project Life","tr":"Beklenen Proje Ömrü"}, type: "number", unit: "yıl", required: true, smartDefault: 5, validation: { min: 1 }, helper: "", expertMeaning: "Expected benefit life in years", expertMeaning_i18n: {"en":"Expected benefit life in years","tr":"Beklenen fayda ömrü (yıl)"} },
    { id: "discountRate", label: "İskonto Oranı", label_i18n: {"en":"Discount Rate","tr":"İskonto Oranı"}, type: "number", unit: "%", required: false, smartDefault: 10, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Discount rate for NPV", expertMeaning_i18n: {"en":"Discount rate for NPV","tr":"NPV için iskonto oranı"} },
    { id: "annualGrowthRecurring", label: "Yıllık Maliyet Artış Oranı", label_i18n: {"en":"Annual Cost Growth Rate","tr":"Yıllık Maliyet Artış Oranı"}, type: "number", unit: "%", required: false, smartDefault: 3, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Annual cost growth rate", expertMeaning_i18n: {"en":"Annual cost growth rate","tr":"Yıllık maliyet artış oranı"} },
  ],
  outputs: [
    ...
    { id: "npvElimination", label: "Giderme Projesi NPV", label_i18n: {"en":"Elimination Project NPV","tr":"Giderme Projesi NPV"}, unit: "USD", format: "currency" },
