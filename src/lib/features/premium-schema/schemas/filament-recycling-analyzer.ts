...
export const FILAMENT_RECYCLING_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "3D printing filamentinde geri donusum ekonomisi hesaplanmazsa hammadde maliyeti ve surdurulebilirlik hedefleri arasinda denge kurulamaz.", painStatement_i18n: {"en":"If the recycling economics of 3D printing filament is not calculated, a balance between raw material cost and sustainability targets cannot be established."},
  inputs: [
    ...
    { id: "virginTransport", label: "Saf Malzeme Navlun", label_i18n: {"en":"Saf material freight"}, type: "number", unit: "USD/kg", required: false, smartDefault: 2, validation: { min: 0 }, helper: "", expertMeaning: "Transport cost per kg", expertMeaning_i18n: {"en":"Transport cost per kg"} },
    { id: "collectCost", label: "Toplama Maliyeti", label_i18n: {"en":"Toplama Cost"}, type: "number", unit: "USD/kg", required: true, smartDefault: 3, validation: { min: 0 }, helper: "", expertMeaning: "Collection cost per kg", expertMeaning_i18n: {"en":"Collection cost per kg"} },
    ...
    { id: "pelletCost", label: "Pelet Maliyeti", label_i18n: {"en":"Pelet Cost"}, type: "number", unit: "USD/kg", required: false, smartDefault: 4, validation: { min: 0 }, helper: "", expertMeaning: "Pelletizing cost", expertMeaning_i18n: {"en":"Pelletizing cost"} },
  ...
  outputs: [
    { id: "virginCost", label: "Saf Filament Maliyeti", label_i18n: {"en":"Saf Filament Cost"}, unit: "USD/kg", format: "currency" },
  ...
  thresholds: [{ fieldId: "roi", warning: 30, critical: 10, direction: "lower_is_bad", warningMessage: "ROI < %30 — geri donusum yatirimi sorgulanmali.", warningMessage_i18n: {"en":"ROI < %30 — Return on conversion investment should be questioned."}, criticalMessage: "ROI < %10 — yatirim fizibil degil.", criticalMessage_i18n: {"en":"ROI < %10 — Investment is not feasible."} }],
  formulaPipeline: [
