...
export const TOOL_WEAR_COST_ANALYZER: PremiumCalculatorSchema = {
  ...
  name: "Takım Aşınma Maliyet Analizi", name_i18n: {"en":"Tool Wear Cost Analysis","tr":"Takım Aşınma Maliyet Analizi"}, sectorSlug: "cnc-manufacturing", category: "cost",
  painStatement: "Kesici takım aşınması takip edilmezse parça başı takım maliyeti artar, erken kırılmalar plansız duruşlara ve yüksek hurda oranına yol açar.", painStatement_i18n: {"en":"If tool wear is not tracked, per-part tooling cost increases and premature failures lead to unplanned downtime and high scrap rates.","tr":"Kesici takım aşınması takip edilmezse parça başı takım maliyeti artar, erken kırılmalar plansız duruşlara ve yüksek hurda oranına yol açar."},
  inputs: [
    { id: "toolCost", label: "Takım Birim Maliyeti", label_i18n: {"en":"Tool Unit Cost","tr":"Takım Birim Maliyeti"}, type: "number", unit: "USD/takım", required: true, smartDefault: 50, validation: { min: 0.01 }, helper: "", expertMeaning: "Cost per tool", expertMeaning_i18n: {"en":"Cost per tool","tr":"Takım başına maliyet"} },
    { id: "toolLifeParts", label: "Takım Ömrü", label_i18n: {"en":"Tool Life","tr":"Takım Ömrü"}, type: "number", unit: "parça", required: true, smartDefault: 500, validation: { min: 1 }, helper: "", expertMeaning: "Parts produced per tool life", expertMeaning_i18n: {"en":"Parts produced per tool life","tr":"Takım ömrü boyunca üretilen parça"} },
    { id: "annualPartVolume", label: "Yıllık Parça Hacmi", label_i18n: {"en":"Annual Part Volume","tr":"Yıllık Parça Hacmi"}, type: "number", unit: "adet", required: true, smartDefault: 50000, validation: { min: 1 }, helper: "", expertMeaning: "Annual part production volume", expertMeaning_i18n: {"en":"Annual part production volume","tr":"Yıllık parça üretim hacmi"} },
    { id: "toolChangeTime", label: "Takım Değişim Süresi", label_i18n: {"en":"Tool Change Time","tr":"Takım Değişim Süresi"}, type: "number", unit: "dakika", required: false, smartDefault: 5, validation: { min: 0 }, helper: "", expertMeaning: "Tool change time in minutes", expertMeaning_i18n: {"en":"Tool change time in minutes","tr":"Takım değişim süresi (dakika)"} },
    { id: "machineRate", label: "Makine Saatlik Maliyeti", label_i18n: {"en":"Machine Hourly Cost","tr":"Makine Saatlik Maliyeti"}, type: "number", unit: "USD/saat", required: false, smartDefault: 85, validation: { min: 1 }, helper: "", expertMeaning: "Machine hourly cost", expertMeaning_i18n: {"en":"Machine hourly cost","tr":"Makine saatlik maliyeti"} },
    { id: "prematureFailureRate", label: "Erken Kırılma Oranı", label_i18n: {"en":"Premature Failure Rate","tr":"Erken Kırılma Oranı"}, type: "number", unit: "%", required: false, smartDefault: 5, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Premature tool failure rate", expertMeaning_i18n: {"en":"Premature tool failure rate","tr":"Erken takım kırılma oranı"} },
    { id: "scrapCostPerFail", label: "Kırılma Başına Hurda Maliyeti", label_i18n: {"en":"Scrap Cost Per Failure","tr":"Kırılma Başına Hurda Maliyeti"}, type: "number", unit: "USD", required: false, smartDefault: 20, validation: { min: 0 }, helper: "", expertMeaning: "Scrap cost per tool failure", expertMeaning_i18n: {"en":"Scrap cost per tool failure","tr":"Takım kırılması başına hurda maliyeti"} },
