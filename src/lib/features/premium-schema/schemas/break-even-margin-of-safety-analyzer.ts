...
export const BREAK_EVEN_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "fixedCosts", label: "Sabit Maliyetler", label_i18n: {"en":"Fixed Costs"}, type: "number", unit: "USD", required: true, smartDefault: 50000, validation: { min: 1 }, helper: "Kira, maas, sigorta gibi sabit giderler toplami.", helper_i18n: {"en":"Total fixed expenses such as rent, salary, insurance."}, expertMeaning: "Total fixed costs", expertMeaning_i18n: {"en":"Total fixed costs"} },
    { id: "unitVariableCost", label: "Unit Variable Cost", label_i18n: {"en":"Unit Variable Cost"}, type: "number", unit: "USD", required: true, smartDefault: 30, validation: { min: 0.01 }, helper: "Birim basina degisken maliyet (hammadde, iscilik).", helper_i18n: {"en":"Variable cost per unit (raw material, labor)."}, expertMeaning: "Variable cost per unit", expertMeaning_i18n: {"en":"Variable cost per unit"} },
    { id: "unitPrice", label: "Unit Selling Price", label_i18n: {"en":"Unit Selling Price"}, type: "number", unit: "USD", required: true, smartDefault: 50, validation: { min: 0.01 }, helper: "Urun basina satis fiyati.", helper_i18n: {"en":"Selling price per product."}, expertMeaning: "Selling price per unit", expertMeaning_i18n: {"en":"Selling price per unit"} },
    { id: "actualSalesUnits", label: "Current Sales Volume", label_i18n: {"en":"Current Sales Volume"}, type: "number", unit: "adet", required: true, smartDefault: 3000, validation: { min: 0 }, helper: "Mevcut satis adedi.", helper_i18n: {"en":"Current sales quantity."}, expertMeaning: "Actual sales volume", expertMeaning_i18n: {"en":"Actual sales volume"} },
    { id: "actualRevenue", label: "Guncel Gelir", label_i18n: {"en":"Current Revenue"}, type: "number", unit: "USD", required: false, smartDefault: 150000, validation: { min: 0 }, helper: "Toplam ciro.", helper_i18n: {"en":"Total revenue."}, expertMeaning: "Actual revenue", expertMeaning_i18n: {"en":"Actual revenue"} },
    { id: "targetProfit", label: "Hedef Kâr", label_i18n: {"en":"Target Profit"}, type: "number", unit: "USD", required: false, smartDefault: 50000, validation: { min: 0 }, helper: "Ulasilmak istenen kâr hedefi.", helper_i18n: {"en":"Target profit to be achieved."}, expertMeaning: "Target profit amount", expertMeaning_i18n: {"en":"Target profit amount"} },
    { id: "contributionMarginTotal", label: "Total Contribution Margin", label_i18n: {"en":"Total Contribution Margin"}, type: "number", unit: "USD", required: false, smartDefault: 60000, validation: { min: 0 }, helper: "(Birim fiyat - BM) × satis adedi.", helper_i18n: {"en":"(Unit price - BM) × sales quantity."}, expertMeaning: "Total contribution margin", expertMeaning_i18n: {"en":"Total contribution margin"} },
    { id: "netOpIncome", label: "Net Operating Income", label_i18n: {"en":"Net Operating Income"}, type: "number", unit: "USD", required: false, smartDefault: 10000, validation: { min: 0 }, helper: "Katki marji - sabit giderler.", helper_i18n: {"en":"Contribution margin - fixed expenses."}, expertMeaning: "Net operating income", expertMeaning_i18n: {"en":"Net operating income"} },
    { id: "unitCm", label: "Unit Contribution Margin", label_i18n: {"en":"Unit Contribution Margin"}, type: "number", unit: "USD", required: false, smartDefault: 20, validation: { min: 0 }, helper: "Birim fiyat - birim degisken maliyet.", helper_i18n: {"en":"Unit price - unit variable cost."}, expertMeaning: "Unit contribution margin", expertMeaning_i18n: {"en":"Unit contribution margin"} },
  ],
  outputs: [
