...
export const MOQ_STOCK_BALANCE_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "MOQ ile gercek talep arasindaki fark hesaplanmazsa ya stok fazlasi cezasi ya da firsat maliyeti olusur.", painStatement_i18n: {"en":"The difference between MOQ and actual demand, if not calculated, results in either excess inventory penalty or opportunity cost."},
  inputs: [
    ...
    { id: "actualDemand", label: "Actual demand per order", label_i18n: {"en":"Actual demand per order"}, type: "number", unit: "adet/siparis", required: true, smartDefault: 300, validation: { min: 1 }, helper: "", expertMeaning: "Actual demand per order", expertMeaning_i18n: {"en":"Actual demand per order"} },
    ...
    { id: "annualOrders", label: "Orders per year", label_i18n: {"en":"Orders per year"}, type: "number", unit: "siparis/yil", required: true, smartDefault: 12, validation: { min: 1 }, helper: "", expertMeaning: "Orders per year", expertMeaning_i18n: {"en":"Orders per year"} },
  ],
  outputs: [
    { id: "optimalQty", label: "Optimum Siparis Miktar", label_i18n: {"en":"Optimum Siparis Miktar"}, unit: "adet", format: "number" },
    { id: "moqPenalty", label: "MOQ Cezas", label_i18n: {"en":"MOQ Cezas"}, unit: "USD/yil", format: "currency" },
    { id: "priceBreakSavings", label: "Fiyat Krlm Tasarrufu", label_i18n: {"en":"Fiyat Krlm Tasarrufu"}, unit: "USD/yil", format: "currency" },
    { id: "netBenefit", label: "Net Fayda", label_i18n: {"en":"Net Fayda"}, unit: "USD/yil", format: "currency" },
  ],
  thresholds: [{ fieldId: "moqPenalty", warning: 5000, critical: 15000, direction: "higher_is_bad", warningMessage: "MOQ cezasi > $5K — tedarikci ile MOQ yeniden muzakere edilmeli.", warningMessage_i18n: {"en":"MOQ penalty > $5K — MOQ should be renegotiated with the supplier."}, criticalMessage: "MOQ cezasi > $15K — alternatif tedarikci degerlendirilmeli.", criticalMessage_i18n: {"en":"MOQ penalty > $15K — alternative supplier should be evaluated."} }],
  formulaPipeline: [
  ...
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["MOQ cezasi = max(0, MOQ − talep) × birim maliyet × siparis sayisi.", "Optimum miktar = max(talep, MOQ).", "Fiyat kirilim avantaji skontolu fiyat bazinda hesaplanir."],assumptionNotes_i18n:[{"en":"MOQ penalty = max(0, MOQ − demand) × unit cost × order count."},{"en":"Optimum miktar = max(talep, MOQ)."},{"en":"Price break advantage is calculated based on the discounted price."}] },
};
