...
export const PALLET_RACK_OPTIMIZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "loadCapacity", label: "Load capacity per pallet position", label_i18n: {"en":"Load capacity per pallet position"}, type: "number", unit: "kg", required: true, smartDefault: 1000, validation: { min: 1 }, helper: "", expertMeaning: "Load capacity per pallet position", expertMeaning_i18n: {"en":"Load capacity per pallet position"} },
    ...
    { id: "costPerPosition", label: "Raf position Cost", label_i18n: {"en":"Raf position Cost"}, type: "number", unit: "USD", required: false, smartDefault: 150, validation: { min: 1 }, helper: "", expertMeaning: "Cost per rack position", expertMeaning_i18n: {"en":"Cost per rack position"} },
  ],
  outputs: [
  ...
  assumptions: { hiddenLossMultiplier: 1.0, volatilityPercent: 10, targetMarginPercent: 10, assumptionNotes: [],assumptionNotes_i18n:[{"en":"Capacity = tier × position × load capacity."},{"en":"Space utilization = rack area / warehouse area."},{"en":"Safety factor = load / strength ratio."},{"en":"Access time depends on rack height and depth."}] },
};
