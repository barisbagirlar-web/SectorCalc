...
export const PALLET_RACK_OPTIMIZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "loadCapacity", label: "Birim Yuk Kapasitesi", label_i18n: {"en":"Load capacity per pallet position"}, type: "number", unit: "kg", required: true, smartDefault: 1000, validation: { min: 1 }, helper: "", expertMeaning: "Load capacity per pallet position", expertMeaning_i18n: {"en":"Load capacity per pallet position"} },
  ...
  thresholds: [{ fieldId: "floorUtilization", warning: 65, critical: 50, direction: "lower_is_bad", warningMessage: "Alan kullanimi < %65 — yerlesim optimizasyonu onerilir.", warningMessage_i18n: {"en":"Space utilization < %65 — layout optimization is recommended."}, criticalMessage: "Alan kullanimi < %50 — depo duzeni yenilenmeli.", criticalMessage_i18n: {"en":"Space utilization < %50 — warehouse layout should be renewed."} }],
  formulaPipeline: [
  ...
  assumptions: { hiddenLossMultiplier: 1.0, volatilityPercent: 10, targetMarginPercent: 10, assumptionNotes: ["Kapasite = kat × pozisyon × yuk kapasitesi.", "Alan kullanimi = raf alani / depo alani.", "Guvenlik faktoru = yuk / dayanim orani.", "Erisim suresi raf yuksekligi ve derinligine baglidir."],assumptionNotes_i18n:[{"en":"Capacity = tier × position × load capacity."},{"en":"Space utilization = rack area / warehouse area."},{"en":"Safety factor = load / strength ratio."},{"en":"Access time depends on rack height and depth."}] },
};
