...
export const IRRIGATION_COST_CHECK_ANALYZER: PremiumCalculatorSchema = {
  ...
  painStatement: "Sulama maliyetleri su, enerji ve iscilik kalemlerinde dogru hesaplanmazsa ciftci veya isletme urun maliyetini gercekci goremez.", painStatement_i18n: {"en":"Irrigation costs in water, energy, and labor items if not accurately calculated prevent the farmer or operation from seeing the true product cost."},
  inputs: [
    ...
    { id: "pumpPower", label: "Pompa Gucu", label_i18n: {"en":"Pump power in kW"}, type: "number", unit: "kW", required: true, smartDefault: 15, validation: { min: 0.1 }, helper: "", expertMeaning: "Pump power in kW", expertMeaning_i18n: {"en":"Pump power in kW"} },
  ...
  thresholds: [{ fieldId: "irrigationTotalCost", warning: 20000, critical: 40000, direction: "higher_is_bad", warningMessage: "Sulama maliyeti >$20K — sistem verimliligi sorgulanmali.", warningMessage_i18n: {"en":"Irrigation cost > $20K — system efficiency should be questioned."}, criticalMessage: "Sulama maliyeti >$40K — alternatif sulama yontemleri degerlendirilmeli.", criticalMessage_i18n: {"en":"Irrigation cost > $40K — alternative irrigation methods should be evaluated."} }],
  formulaPipeline: [
  ...
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["Sulama suyu = (bitki ihtiyaci − yagis) × alan × 10 / verimlilik.", "Enerji maliyeti = guc × saat × elektrik birim fiyati.", "Toplam maliyet = su + enerji + iscilik.", "10 mm = 100 m³/ha donusumu kullanilir."],assumptionNotes_i18n:[{"en":"Irrigation water = (crop requirement − rainfall) × area × 10 / efficiency."},{"en":"Energy cost = power × hours × electricity unit price."},{"en":"Total cost = water + energy + labor."},{"en":"10 mm = 100 m³/ha conversion is used."}] },
};
