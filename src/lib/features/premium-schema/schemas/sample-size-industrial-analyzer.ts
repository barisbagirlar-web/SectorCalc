...
export const SAMPLE_SIZE_INDUSTRIAL_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Örneklem Büyüklüğü Hesaplama", name_i18n: {"en":"Sample Size Calculation"}, sectorSlug: "quality", category: "measurement",
  painStatement: "Yanlış örneklem büyüklüğü ya gereksiz maliyet yaratır ya da istatistiksel anlamlılığı kaybettirir.", painStatement_i18n: {"en":"Wrong sample size either creates unnecessary cost or loses statistical significance."},
  inputs: [
    { id: "populationSize", label: "Ana Kütle Büyüklüğü", label_i18n: {"en":"Population Size"}, type: "number", unit: "adet", required: true, smartDefault: 10000, validation: { min: 1 }, helper: "", expertMeaning: "Population size", expertMeaning_i18n: {"en":"Population size"} },
    ...
    { id: "marginError", label: "Hata Payı", label_i18n: {"en":"Margin of Error"}, type: "number", unit: "%", required: true, smartDefault: 5, validation: { min: 0.1, max: 20 }, helper: "", expertMeaning: "Margin of error", expertMeaning_i18n: {"en":"Margin of error"} },
    ...
    { id: "designEffect", label: "Tasarım Etkisi", label_i18n: {"en":"Design Effect"}, type: "number", unit: "", required: false, smartDefault: 1.0, validation: { min: 0.5, max: 5 }, helper: "", expertMeaning: "Design effect (1.0 = simple random)", expertMeaning_i18n: {"en":"Design effect (1.0 = simple random)"} },
  ...
  outputs: [
    ...
    { id: "sampleContinuous", label: "Sürekli Değişken Örneklemi", label_i18n: {"en":"Continuous Variable Sample"}, unit: "adet", format: "number" },
    { id: "samplePowerAdj", label: "Güç Ayarlı Örneklem", label_i18n: {"en":"Power Adjusted Sample"}, unit: "adet", format: "number" },
    { id: "sampleDesignEffect", label: "Tasarım Etkili Örneklem", label_i18n: {"en":"Design Effect Sample"}, unit: "adet", format: "number" },
    { id: "sampleFinalN", label: "Nihai Örneklem Büyüklüğü", label_i18n: {"en":"Final Sample Size"}, unit: "adet", format: "number", isBigNumber: true },
    { id: "samplingTotalCost", label: "Toplam Örneklem Maliyeti", label_i18n: {"en":"Total Sampling Cost"}, unit: "USD", format: "currency" },
  ],
  thresholds: [{ fieldId: "sampleFinalN", warning: 500, critical: 1000, direction: "higher_is_bad", warningMessage: "Örneklem > 500 — maliyet etkinliği değerlendirilmeli.", warningMessage_i18n: {"en":"Sample > 500 — evaluate cost effectiveness."}, criticalMessage: "Örneklem > 1000 — alternatif yöntem düşünülmeli.", criticalMessage_i18n: {"en":"Sample > 1000 — consider alternative method."} }],
  formulaPipeline: [
  ...
  assumptions: { hiddenLossMultiplier: 1.0, volatilityPercent: 5, targetMarginPercent: 10, assumptionNotes: ["Sonsuz = Z² × p(1-p) / e².", "Sonlu = n0 / (1 + (n0-1)/N).", "Güç ayarı = n × f(power).", "Tasarım etkisi = n × DEFF."],assumptionNotes_i18n:[{"en":"Infinite = Z² × p(1-p) / e²."},{"en":"Finite = n0 / (1 + (n0-1)/N)."},{"en":"Power adjustment = n × f(power)."},{"en":"Design effect = n × DEFF."}] },
};
