...
export const WELD_STRENGTH_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Kaynak mukavemeti ve guvenlik faktoru hesaplanmazsa, yapisal butunluk risk altindadir.", painStatement_i18n: {"en":"If resource strength and safety factor are not calculated, structural integrity is at risk."},
  inputs: [
    ...
    { id: "appliedLoad", label: "Uygulanan Yuk", label_i18n: {"en":"Applied load"}, type: "number", unit: "N", required: true, smartDefault: 50000, validation: { min: 0 }, helper: "", expertMeaning: "Applied load", expertMeaning_i18n: {"en":"Applied load"} },
  ...
  outputs: [
    ...
    { id: "maxShearLoad", label: "Maksimum Kesme Yuku", label_i18n: {"en":"Maksimum Kesme Load"}, unit: "N", format: "number" },
    { id: "safetyFactor", label: "Guvenlik Faktoru", label_i18n: {"en":"Guvenlik Faktoru"}, unit: "", format: "number" },
  ],
  thresholds: [{ fieldId: "safetyFactor", warning: 2, critical: 1.25, direction: "lower_is_bad", warningMessage: "SF < 2 — tasarim gozden gecirilmeli.", warningMessage_i18n: {"en":"SF < 2 — Design must be reviewed."}, criticalMessage: "SF < 1.25 — yapisal risk acil!", criticalMessage_i18n: {"en":"SF < 1.25 — Structural risk is urgent!"} }],
  formulaPipeline: [
