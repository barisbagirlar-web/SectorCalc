...
export const DIGITAL_TWIN_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Digital twin yatiriminin geleneksel yonteme gore maliyet avantajini ROI olarak calculationdan karar vermek risklidir.", painStatement_i18n: {"en":"Deciding without calculating the cost advantage of digital twin investment over the traditional method as ROI is risky."},
  inputs: [
    { id: "traditionalCost", label: "Geleneksel Yontem Maliyeti", label_i18n: {"en":"Prototyping+FieldTest+Downtime+Travel"}, type: "number", unit: "USD", required: true, smartDefault: 200000, validation: { min: 0 }, helper: "", expertMeaning: "Prototyping+FieldTest+Downtime+Travel", expertMeaning_i18n: {"en":"Prototyping+FieldTest+Downtime+Travel"} },
    ...
    { id: "physicalCycle", label: "Physical prototyping cycle (days)", label_i18n: {"en":"Physical prototyping cycle (days)"}, type: "number", unit: "gun", required: false, smartDefault: 30, validation: { min: 0 }, helper: "", expertMeaning: "Physical prototyping cycle (days)", expertMeaning_i18n: {"en":"Physical prototyping cycle (days)"} },
    { id: "digitalCycle", label: "Digital simulation cycle (days)", label_i18n: {"en":"Digital simulation cycle (days)"}, type: "number", unit: "gun", required: false, smartDefault: 10, validation: { min: 0 }, helper: "", expertMeaning: "Digital simulation cycle (days)", expertMeaning_i18n: {"en":"Digital simulation cycle (days)"} },
    { id: "iterations", label: "Number of design iterations", label_i18n: {"en":"Number of design iterations"}, type: "number", unit: "", required: false, smartDefault: 5, validation: { min: 0 }, helper: "", expertMeaning: "Number of design iterations", expertMeaning_i18n: {"en":"Number of design iterations"} },
    ...
    { id: "productionVolume", label: "Annual production volume", label_i18n: {"en":"Annual production volume"}, type: "number", unit: "adet", required: false, smartDefault: 10000, validation: { min: 0 }, helper: "", expertMeaning: "Annual production volume", expertMeaning_i18n: {"en":"Annual production volume"} },
  ],
  outputs: [
    { id: "totalSavings", label: "Toplam Tasarruf", label_i18n: {"en":"Total Tasarruf"}, unit: "USD", format: "currency" },
  ...
  thresholds: [{ fieldId: "roi", warning: 100, critical: 50, direction: "lower_is_bad", warningMessage: "ROI < %100 — fizibilite tekrar degerlendirilmeli.", warningMessage_i18n: {"en":"ROI < 100% — feasibility must be re-evaluated."}, criticalMessage: "ROI < %50 — yatirim onerilmez.", criticalMessage_i18n: {"en":"ROI < 50% — investment is not recommended."} }],
  formulaPipeline: [
