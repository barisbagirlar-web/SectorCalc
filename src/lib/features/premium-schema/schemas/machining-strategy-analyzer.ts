...
export const MACHINING_STRATEGY_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "İşleme Stratejisi & Takım Ömrü Optimizasyonu", name_i18n: {"en":"Machining Strategy & Tool Life Optimization"}, sectorSlug: "cnc-manufacturing", category: "measurement",
  painStatement: "Kesme parametreleri (Vc, f, ap) ve Taylor takım ömrü modeli kullanılmadan yapılan işleme stratejisi ne optimum ne de güvenlidir.", painStatement_i18n: {"en":"A machining strategy without cutting parameters (Vc, f, ap) and Taylor tool life model is neither optimal nor reliable."},
  inputs: [
    { id: "cuttingSpeed", label: "Kesme Hızı (Vc)", label_i18n: {"en":"Cutting Speed (Vc)"}, type: "number", unit: "m/dak", required: true, smartDefault: 180, validation: { min: 1 }, helper: "", expertMeaning: "Cutting speed", expertMeaning_i18n: {"en":"Cutting speed"} },
    { id: "feedPerRev", label: "İlerleme (f)", label_i18n: {"en":"Feed (f)"}, type: "number", unit: "mm/dev", required: true, smartDefault: 0.15, validation: { min: 0.001 }, helper: "", expertMeaning: "Feed per revolution", expertMeaning_i18n: {"en":"Feed per revolution"} },
    ...
    { id: "maxPower", label: "Maksimum Güç", label_i18n: {"en":"Maximum Power"}, type: "number", unit: "kW", required: false, smartDefault: 22, validation: { min: 0.1 }, helper: "", expertMeaning: "Maximum spindle power", expertMeaning_i18n: {"en":"Maximum spindle power"} },
    { id: "noseRadius", label: "Uç Radyüsü (rε)", label_i18n: {"en":"Nose Radius (rε)"}, type: "number", unit: "mm", required: false, smartDefault: 0.8, validation: { min: 0.05 }, helper: "", expertMeaning: "Tool nose radius", expertMeaning_i18n: {"en":"Tool nose radius"} },
    { id: "roughnessTol", label: "Pürüzlülük Toleransı (Ra)", label_i18n: {"en":"Roughness Tolerance (Ra)"}, type: "number", unit: "µm", required: false, smartDefault: 3.2, validation: { min: 0.1 }, helper: "", expertMeaning: "Surface roughness limit", expertMeaning_i18n: {"en":"Surface roughness limit"} },
  ],
  outputs: [
    { id: "mrr", label: "MRR (Talaş Kaldırma Oranı)", label_i18n: {"en":"MRR (Talas Kaldrma Oran)"}, unit: "cm³/dak", format: "number" },
    { id: "powerRequired", label: "Güç İhtiyacı", label_i18n: {"en":"Guc Ihtiyac"}, unit: "kW", format: "number" },
    { id: "toolLife", label: "Takım Ömrü", label_i18n: {"en":"Tool Life"}, unit: "dak", format: "number" },
    { id: "constraintPass", label: "Kısıt Kontrolü (1=Geçer)", label_i18n: {"en":"Kst Kontrolu (1=Gecer)"}, unit: "", format: "number", isBigNumber: true },
  ],
  thresholds: [{ fieldId: "constraintPass", warning: 1, critical: 0, direction: "lower_is_bad", warningMessage: "Kısıtlar sağlanmıyor (güç/pürüzlülük) — parametreler düşürülmeli.", warningMessage_i18n: {"en":"Constraints not met (power/roughness) — reduce parameters."}, criticalMessage: "Kısıt hatası — mevcut parametreler kullanılamaz.", criticalMessage_i18n: {"en":"Constraint error — current parameters unusable."} }],
  formulaPipeline: [
