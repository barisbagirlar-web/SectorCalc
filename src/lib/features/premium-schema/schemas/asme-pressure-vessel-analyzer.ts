...
const HEAD_TYPE_OPTIONS = [
  ...
  { value: "spherical", label: "Küresel (Sphere)", label_i18n: {"en":"Kuresel (Sphere)"} },
...
export const ASME_VESSEL_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Basınç Vessel Kalınlık Analizi (ASME BPVC)", name_i18n: {"en":"Pressure Vessel Thickness Analysis (ASME BPVC)"}, sectorSlug: "sheet-metal", category: "measurement",
  ...
  inputs: [
    { id: "internalPressure", label: "İç Basınç (P)", label_i18n: {"en":"Internal Pressure (P)"}, type: "number", unit: "MPa", required: true, smartDefault: 1.5, validation: { min: 0.001 }, helper: "", expertMeaning: "Internal design pressure", expertMeaning_i18n: {"en":"Internal design pressure"} },
    { id: "innerRadius", label: "İç Yarıçap (R)", label_i18n: {"en":"Inner Radius (R)"}, type: "number", unit: "mm", required: true, smartDefault: 500, validation: { min: 1 }, helper: "", expertMeaning: "Inside radius of vessel", expertMeaning_i18n: {"en":"Inside radius of vessel"} },
    { id: "innerDiameter", label: "İç Çap (D)", label_i18n: {"en":"Inner Diameter (D)"}, type: "number", unit: "mm", required: false, smartDefault: 1000, validation: { min: 1 }, helper: "", expertMeaning: "Inside diameter (for heads)", expertMeaning_i18n: {"en":"Inside diameter (for heads)"} },
    ...
    { id: "stressAllowable", label: "İzin Verilen Gerilme (S)", label_i18n: {"en":"Allowable Stress (S)"}, type: "number", unit: "MPa", required: true, smartDefault: 138, validation: { min: 1 }, helper: "", expertMeaning: "Allowable stress per ASME II-D", expertMeaning_i18n: {"en":"Allowable stress per ASME II-D"} },
    ...
    { id: "corrosionAllowance", label: "Korozyon Payı (C_A)", label_i18n: {"en":"Corrosion Allowance (C_A)"}, type: "number", unit: "mm", required: true, smartDefault: 2, validation: { min: 0 }, helper: "", expertMeaning: "Corrosion allowance", expertMeaning_i18n: {"en":"Corrosion allowance"} },
    { id: "vesselLength", label: "Kapak Uzunluğu", label_i18n: {"en":"Head Length"}, type: "number", unit: "mm", required: false, smartDefault: 2000, validation: { min: 0 }, helper: "", expertMeaning: "Vessel cylindrical length", expertMeaning_i18n: {"en":"Vessel cylindrical length"} },
    { id: "materialDensity", label: "Malzeme Yoğunluğu", label_i18n: {"en":"Material Density"}, type: "number", unit: "kg/m³", required: false, smartDefault: 7850, validation: { min: 0 }, helper: "", expertMeaning: "Material density", expertMeaning_i18n: {"en":"Material density"} },
    { id: "crownRadiusMm", label: "Taç Yarıçapı (L) — Torisferik", label_i18n: {"en":"Crown Radius (L) — Torispherical"}, type: "number", unit: "mm", required: false, smartDefault: 1000, validation: { min: 0 }, helper: "", expertMeaning: "Crown radius for torispherical head", expertMeaning_i18n: {"en":"Crown radius for torispherical head"} },
    { id: "knuckleRadiusMm", label: "Büküm Yarıçapı (r) — Torisferik", label_i18n: {"en":"Knuckle Radius (r) — Torispherical"}, type: "number", unit: "mm", required: false, smartDefault: 100, validation: { min: 0 }, helper: "", expertMeaning: "Knuckle radius for torispherical head", expertMeaning_i18n: {"en":"Knuckle radius for torispherical head"} },
  ],
  outputs: [
    { id: "requiredThickness", label: "Gerekli Cidar Kalınlığı", label_i18n: {"en":"Gerekli Cidar Kalnlg"}, unit: "mm", format: "number" },
    ...
    { id: "vesselWeight", label: "Kap Ağırlığı", label_i18n: {"en":"Kap Agrlg"}, unit: "kg", format: "number" },
    { id: "designVerdict", label: "Tasarım Kararı", label_i18n: {"en":"Tasarm Karar"}, unit: "", format: "score", isBigNumber: true },
  ],
  thresholds: [
    { fieldId: "requiredThickness", warning: 10, critical: 25, direction: "higher_is_bad", warningMessage: "Gerekli kalınlık > 10 mm — ağırlık ve maliyet artıyor.", warningMessage_i18n: {"en":"Gerekli kalınlık > 10 mm — ağırlık ve maliyet artıyor."}, criticalMessage: "Gerekli kalınlık > 25 mm — alternatif malzeme değerlendirilmeli.", criticalMessage_i18n: {"en":"Gerekli kalınlık > 25 mm — alternatif malzeme değerlendirilmeli."} },
  ],
  formulaPipeline: [
