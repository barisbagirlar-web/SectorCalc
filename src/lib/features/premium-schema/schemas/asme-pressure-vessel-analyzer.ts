...
export const ASME_VESSEL_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Wall thickness, MAWP, and Weight calculation without manufacturing per ASME BPVC Section VIII Div.1 for pressure vessel design is risky. This tool calculates required thickness and MAWP using ASME formulas.", painStatement_i18n: {"en":"Wall thickness, MAWP, and Weight calculation without manufacturing per ASME BPVC Section VIII Div.1 for pressure vessel design is risky. This tool calculates required thickness and MAWP using ASME formulas."},
  inputs: [
    ...
    { id: "headType", label: "Head Type", label_i18n: {"en":"Head Type"}, type: "select", unit: "", required: true, smartDefault: "cylindrical", options: [...HEAD_TYPE_OPTIONS], helper: "", expertMeaning: "ASME head type", expertMeaning_i18n: {"en":"ASME head type"} },
    { id: "material", label: "Material", label_i18n: {"en":"Material"}, type: "select", unit: "", required: true, smartDefault: "SA516_70", options: [...MATERIAL_OPTIONS], helper: "", expertMeaning: "Vessel material grade", expertMeaning_i18n: {"en":"Vessel material grade"} },
    ...
    { id: "jointEfficiency", label: "Joint Efficiency (E)", label_i18n: {"en":"Joint Efficiency (E)"}, type: "number", unit: "", required: true, smartDefault: 0.85, validation: { min: 0, max: 1 }, helper: "", expertMeaning: "Joint efficiency per UW-12", expertMeaning_i18n: {"en":"Joint efficiency per UW-12"} },
  ...
  outputs: [
    ...
    { id: "vesselWeight", label: "vessel Agrlg", label_i18n: {"en":"vessel Agrlg"}, unit: "kg", format: "number" },
    { id: "designVerdict", label: "Tasarm decision", label_i18n: {"en":"Tasarm decision"}, unit: "", format: "score", isBigNumber: true },
  ],
  thresholds: [
