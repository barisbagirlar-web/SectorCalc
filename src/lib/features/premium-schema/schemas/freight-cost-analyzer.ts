...
export const FREIGHT_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Navlun maliyetinde agirlik, bunker, terminal ve gumruk kalemleri ayri hesaplanmazsa gercek lojistik maliyeti gizli kalir.", painStatement_i18n: {"en":"If weight, bunker, terminal, and customs items in freight cost are not calculated separately, the actual logistics cost remains hidden."},
  inputs: [
    ...
    { id: "bunkerPct", label: "Bunker Ek Yuzdesi", label_i18n: {"en":"Bunker surcharge percentage"}, type: "number", unit: "%", required: true, smartDefault: 15, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Bunker surcharge percentage", expertMeaning_i18n: {"en":"Bunker surcharge percentage"} },
    ...
    { id: "customsFee", label: "Gumruk Ucreti", label_i18n: {"en":"Customs clearance fee"}, type: "number", unit: "USD", required: true, smartDefault: 150, validation: { min: 0 }, helper: "", expertMeaning: "Customs clearance fee", expertMeaning_i18n: {"en":"Customs clearance fee"} },
  ...
  outputs: [
    ...
    { id: "bunkerSurcharge", label: "Bunker Ek Ucreti", label_i18n: {"en":"Bunker Ek Rate"}, unit: "USD", format: "currency" },
    { id: "terminalCost", label: "Terminal Maliyeti", label_i18n: {"en":"Terminal Cost"}, unit: "USD", format: "currency" },
    { id: "customsCost", label: "Gumruk Maliyeti", label_i18n: {"en":"customs Cost"}, unit: "USD", format: "currency" },
  ...
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["Tasinabilir agirlik = max(brut agirlik, hacim × 167).", "Bunker = baz navlun × bunker yuzdesi.", "Toplam = baz + bunker + terminal + gumruk."],assumptionNotes_i18n:[{"en":"Chargeable weight = max(gross weight, volume × 167)."},{"en":"Bunker = base freight × bunker percentage."},{"en":"Total = base + bunker + terminal + customs."}] },
};
