...
export const FREIGHT_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "If weight, bunker, terminal, and customs items in freight cost are not calculated separately, the actual logistics cost remains hidden.", painStatement_i18n: {"en":"If weight, bunker, terminal, and customs items in freight cost are not calculated separately, the actual logistics cost remains hidden."},
  inputs: [
    ...
    { id: "baseRate", label: "freight Base Fiyat", label_i18n: {"en":"freight Base Fiyat"}, type: "number", unit: "USD/kg", required: true, smartDefault: 2.5, validation: { min: 0.01 }, helper: "", expertMeaning: "Base freight rate per kg", expertMeaning_i18n: {"en":"Base freight rate per kg"} },
    { id: "bunkerPct", label: "Bunker surcharge percentage", label_i18n: {"en":"Bunker surcharge percentage"}, type: "number", unit: "%", required: true, smartDefault: 15, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Bunker surcharge percentage", expertMeaning_i18n: {"en":"Bunker surcharge percentage"} },
    ...
    { id: "customsFee", label: "Customs clearance fee", label_i18n: {"en":"Customs clearance fee"}, type: "number", unit: "USD", required: true, smartDefault: 150, validation: { min: 0 }, helper: "", expertMeaning: "Customs clearance fee", expertMeaning_i18n: {"en":"Customs clearance fee"} },
  ...
  outputs: [
    { id: "chargeableWeight", label: "Tasnabilir Weight", label_i18n: {"en":"Tasnabilir Weight"}, unit: "kg", format: "number" },
    { id: "baseFreight", label: "Base freight Bedeli", label_i18n: {"en":"Base freight Bedeli"}, unit: "USD", format: "currency" },
    { id: "bunkerSurcharge", label: "Bunker Ek Rate", label_i18n: {"en":"Bunker Ek Rate"}, unit: "USD", format: "currency" },
    { id: "terminalCost", label: "Terminal Cost", label_i18n: {"en":"Terminal Cost"}, unit: "USD", format: "currency" },
    { id: "customsCost", label: "customs Cost", label_i18n: {"en":"customs Cost"}, unit: "USD", format: "currency" },
  ...
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: [],assumptionNotes_i18n:[{"en":"Chargeable weight = max(gross weight, volume × 167)."},{"en":"Bunker = base freight × bunker percentage."},{"en":"Total = base + bunker + terminal + customs."}] },
};
