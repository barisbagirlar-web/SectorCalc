...
export const COMPRESSED_AIR_LEAK_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Basincli hava kacaklari tespit edilmezse enerji maliyeti gizlice artar ve karbon emisyonu yukselir.", painStatement_i18n: {"en":"If compressed Air leaks are not detected, energy Cost silently increases and Carbon emissions rise."},
  inputs: [
    ...
    { id: "compressorEff", label: "Kompresor Verimi", label_i18n: {"en":"Compressor efficiency"}, type: "number", unit: "", required: false, smartDefault: 0.85, validation: { min: 0, max: 1 }, helper: "", expertMeaning: "Compressor efficiency", expertMeaning_i18n: {"en":"Compressor efficiency"} },
    ...
    { id: "gridEF", label: "Emisyon Faktoru", label_i18n: {"en":"Grid emission factor"}, type: "number", unit: "tCO2e/MWh", required: false, smartDefault: 0.45, validation: { min: 0 }, helper: "", expertMeaning: "Grid emission factor", expertMeaning_i18n: {"en":"Grid emission factor"} },
  ...
  outputs: [
    ...
    { id: "costPerLeak", label: "Kacak Bas Maliyet", label_i18n: {"en":"Leak Bas Cost"}, unit: "USD/yil", format: "currency" },
    { id: "totalLeakCost", label: "Total Leak Cost", label_i18n: {"en":"Total Leak Cost"}, unit: "USD/yil", format: "currency", isBigNumber: true },
    { id: "carbonEmissions", label: "Karbon Emisyonu", label_i18n: {"en":"Carbon Emisyonu"}, unit: "tCO2e", format: "number" },
    { id: "paybackMonths", label: "Tamir Geri Odeme", label_i18n: {"en":"Tamir Return Payment"}, unit: "ay", format: "duration" },
  ],
  thresholds: [{ fieldId: "totalLeakCost", warning: 5000, critical: 20000, direction: "higher_is_bad", warningMessage: "Kacak maliyeti > $5K/yil — onarim programi baslatilmali.", warningMessage_i18n: {"en":"Leak Cost > $5K/yr — repair program should be initiated."}, criticalMessage: "Kacak maliyeti > $20K/yil — acil mudahale gerekli.", criticalMessage_i18n: {"en":"Leak Cost > $20K/yr — urgent intervention required."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Kompresor Kacak Reportu", title_i18n: {"en":"Compressor Leak Report"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["Kacak debisi = 22.4×d²×P/√T.", "Guc kaybi = Q×P×144/(33000×Verim).", "Geri odeme = Tamir / Yillik maliyet."],assumptionNotes_i18n:[{"en":"Leak flow rate = 22.4×d²×P/√T."},{"en":"Power loss = Q×P×144/(33000×Efficiency)."},{"en":"Payback = Repair / Annual cost."}] },
};
