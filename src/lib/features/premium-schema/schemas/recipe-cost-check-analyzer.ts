...
export const RECIPE_COST_CHECK_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "yieldLossPct", label: "Waste Rate", label_i18n: {"en":"Waste Rate"}, type: "number", unit: "%", required: true, smartDefault: 8, validation: { min: 0, max: 50 }, helper: "", expertMeaning: "Yield loss percentage (trimming, spillage)", expertMeaning_i18n: {"en":"Yield loss percentage (trimming, spillage)"} },
    { id: "evaporationPct", label: "Evaporation Rate", label_i18n: {"en":"Evaporation Rate"}, type: "number", unit: "%", required: false, smartDefault: 5, validation: { min: 0, max: 50 }, helper: "", expertMeaning: "Evaporation loss percentage", expertMeaning_i18n: {"en":"Evaporation loss percentage"} },
  ],
  outputs: [
    { id: "recipeTheoretical", label: "Theoretical Recipe Cost", label_i18n: {"en":"Theoretical Recipe Cost"}, unit: "USD", format: "currency" },
    { id: "recipeActual", label: "Actual Recipe Cost", label_i18n: {"en":"Actual Recipe Cost"}, unit: "USD", format: "currency" },
    { id: "recipeVariance", label: "Recipe Variance", label_i18n: {"en":"Recipe Variance"}, unit: "USD", format: "currency" },
  ...
  assumptions: { hiddenLossMultiplier: 1.0, volatilityPercent: 10, targetMarginPercent: 20, assumptionNotes: ["Teorik = miktar × birim fiyat.", "Actual = usage × unit price.", "Yield loss = theoretical × (loss%/100).", "Efficiency = theoretical / actual × 100."],assumptionNotes_i18n:[{"en":"Theoretical = quantity × unit price."},{"en":"Actual = usage × unit price."},{"en":"Yield loss = theoretical × (loss%/100)."},{"en":"Efficiency = theoretical / actual × 100."}] },
};
