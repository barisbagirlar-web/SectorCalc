...
export const RENEWABLE_ENERGY_IRR_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "annualGeneration", label: "Yıllık Enerji Üretimi", label_i18n: {"en":"Annual energy generation","tr":"Yıllık Enerji Üretimi"}, type: "number", unit: "kWh/yıl", required: true, smartDefault: 800000, validation: { min: 1 }, helper: "", expertMeaning: "Annual energy generation", expertMeaning_i18n: {"en":"Annual energy generation","tr":"yıllık enerji üretimi"} },
    { id: "tariffRate", label: "Enerji Satış Fiyatı", label_i18n: {"en":"Feed-in tariff or PPA rate","tr":"Enerji Satış Fiyatı"}, type: "number", unit: "USD/kWh", required: true, smartDefault: 0.10, validation: { min: 0.01 }, helper: "", expertMeaning: "Feed-in tariff or PPA rate", expertMeaning_i18n: {"en":"Feed-in tariff or PPA rate","tr":"enerji satış fiyatı"} },
    { id: "operatingCost", label: "Yıllık İşletme Maliyeti", label_i18n: {"en":"Annual Operating Cost","tr":"Yıllık İşletme Maliyeti"}, type: "number", unit: "USD/yıl", required: true, smartDefault: 15000, validation: { min: 0 }, helper: "", expertMeaning: "Annual O&M cost", expertMeaning_i18n: {"en":"Annual O&M cost","tr":"Yıllık işletme ve bakım maliyeti"} },
    { id: "projectLife", label: "Proje Ömrü", label_i18n: {"en":"Project Life","tr":"Proje Ömrü"}, type: "number", unit: "yıl", required: true, smartDefault: 25, validation: { min: 1, max: 50 }, helper: "", expertMeaning: "Project economic life", expertMeaning_i18n: {"en":"Project economic life","tr":"Proje ekonomik ömrü"} },
    { id: "discountRate", label: "İskonto Oranı", label_i18n: {"en":"Discount Rate","tr":"İskonto Oranı"}, type: "number", unit: "%", required: false, smartDefault: 8, validation: { min: 1, max: 30 }, helper: "", expertMeaning: "Discount rate for NPV", expertMeaning_i18n: {"en":"Discount rate for NPV","tr":"NPV için iskonto oranı"} },
    { id: "degradationRate", label: "Yıllık Verim Düşüşü", label_i18n: {"en":"Annual degradation rate","tr":"Yıllık Verim Düşüşü"}, type: "number", unit: "%", required: false, smartDefault: 0.5, validation: { min: 0, max: 5 }, helper: "", expertMeaning: "Annual degradation rate", expertMeaning_i18n: {"en":"Annual degradation rate","tr":"yıllık verim düşüşü"} },
    { id: "incentiveAmount", label: "Teşvik / Sübvansiyon", label_i18n: {"en":"Government incentive amount","tr":"Teşvik / Sübvansiyon"}, type: "number", unit: "USD", required: false, smartDefault: 50000, validation: { min: 0 }, helper: "", expertMeaning: "Government incentive amount", expertMeaning_i18n: {"en":"Government incentive amount","tr":"teşvik / sübvansiyon"} },
  ],
  outputs: [
    { id: "renewableAnnualGen", label: "Net Yıllık Üretim (Degradasyon Sonrası)", label_i18n: {"en":"Net Yllk Uretim (Degradasyon Sonras)","tr":"Net Yıllık Üretim (Degradasyon Sonrası)"}, unit: "kWh/yıl", format: "number" },
    { id: "renewableNpv", label: "Net Bugünkü Değer (NPV)", label_i18n: {"en":"Net Present Value (NPV)","tr":"Net Bugünkü Değer (NPV)"}, unit: "USD", format: "currency", isBigNumber: true },
    { id: "renewableLcoe", label: "Seviyelendirilmiş Enerji Maliyeti (LCOE)", label_i18n: {"en":"Seviyelendirilmis Enerji Maliyeti (LCOE)","tr":"Seviyelendirilmiş Enerji Maliyeti (LCOE)"}, unit: "USD/kWh", format: "currency" },
    { id: "renewableIrr", label: "İç Verim Oranı (IRR)", label_i18n: {"en":"Internal Rate of Return (IRR)","tr":"İç Verim Oranı (IRR)"}, unit: "%", format: "percentage" },
    { id: "paybackPeriod", label: "Geri Ödeme Süresi", label_i18n: {"en":"Payback Period","tr":"Geri Ödeme Süresi"}, unit: "yıl", format: "number" },
  ],
  thresholds: [
    { fieldId: "renewableNpv", warning: 0, critical: -50000, direction: "lower_is_bad", warningMessage: "NPV pozitif değil — yatırım fizibilitesi zayıf.", warningMessage_i18n: {"en":"NPV pozitif değil — yatırım fizibilitesi zayıf.","tr":"NPV pozitif değil — yatırım fizibilitesi zayıf."}, criticalMessage: "NPV < -$50K — yatırım yapılmamalı, alternatifler değerlendirilmeli.", criticalMessage_i18n: {"en":"NPV < -$50K — yatırım yapılmamalı, alternatifler değerlendirilmeli.","tr":"NPV < -$50K — yatırım yapılmamalı, alternatifler değerlendirilmeli."} },
    { fieldId: "renewableLcoe", warning: 0.12, critical: 0.20, direction: "higher_is_bad", warningMessage: "LCOE > $0.12/kWh — şebeke paritesi riski var.", warningMessage_i18n: {"en":"LCOE > $0.12/kWh — şebeke paritesi riski var.","tr":"LCOE > $0.12/kWh — şebeke paritesi riski var."}, criticalMessage: "LCOE > $0.20/kWh — proje ekonomik olarak sürdürülemez.", criticalMessage_i18n: {"en":"LCOE > $0.20/kWh — proje ekonomik olarak sürdürülemez.","tr":"LCOE > $0.20/kWh — proje ekonomik olarak sürdürülemez."} },
