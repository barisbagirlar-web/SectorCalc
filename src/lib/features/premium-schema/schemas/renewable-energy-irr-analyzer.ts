...
export const RENEWABLE_ENERGY_IRR_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "annualGeneration", label: "Annual energy generation", label_i18n: {"en":"Annual energy generation"}, type: "number", unit: "kWh/yıl", required: true, smartDefault: 800000, validation: { min: 1 }, helper: "", expertMeaning: "Annual energy generation", expertMeaning_i18n: {"en":"Annual energy generation"} },
    ...
    { id: "operatingCost", label: "Annual Operating Cost", label_i18n: {"en":"Annual Operating Cost"}, type: "number", unit: "USD/yıl", required: true, smartDefault: 15000, validation: { min: 0 }, helper: "", expertMeaning: "Annual O&M cost", expertMeaning_i18n: {"en":"Annual O&M cost"} },
  ...
  outputs: [
    { id: "renewableAnnualGen", label: "Net Yllk Uretim (Degradasyon Sonras)", label_i18n: {"en":"Net Annual Production (Degradasyon Sonras)"}, unit: "kWh/yıl", format: "number" },
  ...
  thresholds: [
    { fieldId: "renewableNpv", warning: 0, critical: -50000, direction: "lower_is_bad", warningMessage: "NPV pozitif değil — yatırım fizibilitesi zayıf.", warningMessage_i18n: {"en":"NPV is not positive — investment feasibility is weak."}, criticalMessage: "NPV < -$50K — yatırım yapılmamalı, alternatifler değerlendirilmeli.", criticalMessage_i18n: {"en":"NPV < -$50K — investment should not be made; alternatives should be evaluated."} },
    { fieldId: "renewableLcoe", warning: 0.12, critical: 0.20, direction: "higher_is_bad", warningMessage: "LCOE > $0.12/kWh — şebeke paritesi riski var.", warningMessage_i18n: {"en":"LCOE > $0.12/kWh — grid parity risk exists."}, criticalMessage: "LCOE > $0.20/kWh — proje ekonomik olarak sürdürülemez.", criticalMessage_i18n: {"en":"LCOE > $0.20/kWh — project is economically unsustainable."} },
    { fieldId: "renewableIrr", warning: 8, critical: 5, direction: "lower_is_bad", warningMessage: "IRR < %8 — iskonto oranının altında getiri.", warningMessage_i18n: {"en":"IRR < 8% — return below discount rate."}, criticalMessage: "IRR < %5 — alternatif yatırımlar daha cazip.", criticalMessage_i18n: {"en":"IRR < 5% — alternative investments are more attractive."} },
  ],
  formulaPipeline: [
