...
export const RENEWABLE_ENERGY_IRR_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "operatingCost", label: "Annual Operating Cost", label_i18n: {"en":"Annual Operating Cost"}, type: "number", unit: "USD/yil", required: true, smartDefault: 15000, validation: { min: 0 }, helper: "", expertMeaning: "Annual O&M cost", expertMeaning_i18n: {"en":"Annual O&M cost"} },
    { id: "projectLife", label: "Proje Omru", label_i18n: {"en":"Project Life"}, type: "number", unit: "yil", required: true, smartDefault: 25, validation: { min: 1, max: 50 }, helper: "", expertMeaning: "Project economic life", expertMeaning_i18n: {"en":"Project economic life"} },
  ...
  outputs: [
    { id: "renewableAnnualGen", label: "Net Yllk Uretim (Degradasyon Sonras)", label_i18n: {"en":"Net Annual Production (Degradasyon Sonras)"}, unit: "kWh/yil", format: "number" },
    ...
    { id: "renewableLcoe", label: "Seviyelendirilmis Enerji Maliyeti (LCOE)", label_i18n: {"en":"Seviyelendirilmis energy Cost (LCOE)"}, unit: "USD/kWh", format: "currency" },
    { id: "renewableIrr", label: "Internal Rate of Return (IRR)", label_i18n: {"en":"Internal Rate of Return (IRR)"}, unit: "%", format: "percentage" },
    { id: "paybackPeriod", label: "Geri Odeme Suresi", label_i18n: {"en":"Payback Period"}, unit: "yil", format: "number" },
  ],
  thresholds: [
    { fieldId: "renewableNpv", warning: 0, critical: -50000, direction: "lower_is_bad", warningMessage: "NPV pozitif degil — yatirim fizibilitesi zayif.", warningMessage_i18n: {"en":"NPV is not positive — investment feasibility is weak."}, criticalMessage: "NPV < -$50K — yatirim yapilmamali, alternatifler degerlendirilmeli.", criticalMessage_i18n: {"en":"NPV < -$50K — investment should not be made; alternatives should be evaluated."} },
    { fieldId: "renewableLcoe", warning: 0.12, critical: 0.20, direction: "higher_is_bad", warningMessage: "LCOE > $0.12/kWh — sebeke paritesi risk var.", warningMessage_i18n: {"en":"LCOE > $0.12/kWh — grid parity risk exists."}, criticalMessage: "LCOE > $0.20/kWh — proje ekonomik olarak surdurulemez.", criticalMessage_i18n: {"en":"LCOE > $0.20/kWh — project is economically unsustainable."} },
    { fieldId: "renewableIrr", warning: 8, critical: 5, direction: "lower_is_bad", warningMessage: "IRR < %8 — iskonto oraninin altinda getiri.", warningMessage_i18n: {"en":"IRR < 8% — return below discount rate."}, criticalMessage: "IRR < %5 — alternatif yatirimlar daha cazip.", criticalMessage_i18n: {"en":"IRR < 5% — alternative investments are more attractive."} },
  ],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Yenilenebilir Enerji IRR Analiz Raporu", title_i18n: {"en":"Yenilenebilir energy IRR Analiz Raporu"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "sensitivity", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
