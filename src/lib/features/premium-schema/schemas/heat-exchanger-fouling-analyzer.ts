...
export const HEAT_EXCHANGER_FOULING_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Isi degistiricide kirlenme (fouling) nedeniyle isi transfer verimi duser ve enerji maliyeti artar. Temizlik ROI'si hesaplanmalidir.", painStatement_i18n: {"en":"Fouling in the heat exchanger causes heat transfer efficiency to drop and energy cost to rise. Cleaning ROI should be calculated."},
  inputs: [
    ...
    { id: "boilerEff", label: "Kazan Verimi", label_i18n: {"en":"Kazan Efficiency"}, type: "number", unit: "%", required: false, smartDefault: 85, validation: { min: 1, max: 100 }, helper: "", expertMeaning: "Boiler/furnace efficiency", expertMeaning_i18n: {"en":"Boiler/furnace efficiency"} },
    ...
    { id: "cleanCost", label: "Temizlik Maliyeti", label_i18n: {"en":"Temizlik Cost"}, type: "number", unit: "USD", required: false, smartDefault: 5000, validation: { min: 0 }, helper: "", expertMeaning: "Cost of cleaning", expertMeaning_i18n: {"en":"Cost of cleaning"} },
  ...
  outputs: [
    ...
    { id: "heatLoss", label: "Is Transfer Kayb", label_i18n: {"en":"Is Transfer Loss"}, unit: "kW", format: "number" },
    { id: "cost", label: "Annual Energy Loss", label_i18n: {"en":"Annual Energy Loss"}, unit: "USD/yil", format: "currency" },
    { id: "roi", label: "Temizlik ROI", label_i18n: {"en":"Temizlik ROI"}, unit: "%", format: "percentage", isBigNumber: true },
  ],
  thresholds: [{ fieldId: "cost", warning: 10000, critical: 30000, direction: "higher_is_bad", warningMessage: "Enerji kaybi > $10K/yil — temizlik planlanmali.", warningMessage_i18n: {"en":"Energy loss > $10K/year — cleaning should be scheduled."}, criticalMessage: "Kayip > $30K/yil — acil temizlik gerekli.", criticalMessage_i18n: {"en":"Loss > $30K/year — urgent cleaning is required."} }],
  formulaPipeline: [
