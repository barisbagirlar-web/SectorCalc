...
export const AUTO_REPAIR_COMEBACK_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Auto Repair Comeback Cost Analysis", name_i18n: {"en":"Auto Repair Comeback Cost Analysis"}, sectorSlug: "auto-repair", category: "cost",
  painStatement: "Return repairs (comeback) cause not only Direct labor but also bay occupancy Loss and loss of customer trust. Actual Cost is higher than visible.", painStatement_i18n: {"en":"Return repairs (comeback) cause not only Direct labor but also bay occupancy Loss and loss of customer trust. Actual Cost is higher than visible."},
  inputs: [
    { id: "totalCompletedOrders", label: "Completed Repair Orders", label_i18n: {"en":"Completed Repair Orders"}, type: "number", unit: "units", required: true, smartDefault: 200, validation: { min: 1 }, helper: "", expertMeaning: "Total repair orders completed", expertMeaning_i18n: {"en":"Total repair orders completed"} },
    { id: "comebackOrders", label: "Comeback Repair Orders", label_i18n: {"en":"Comeback Repair Orders"}, type: "number", unit: "units", required: true, smartDefault: 10, validation: { min: 0 }, helper: "", expertMeaning: "Return repair orders", expertMeaning_i18n: {"en":"Return repair orders"} },
    { id: "avgDiagnosisMinutes", label: "Avg Diagnosis Time", label_i18n: {"en":"Avg Diagnosis Time"}, type: "number", unit: "minutes", required: true, smartDefault: 45, validation: { min: 0 }, helper: "", expertMeaning: "Average diagnosis time per comeback", expertMeaning_i18n: {"en":"Average diagnosis time per comeback"} },
    { id: "avgRepairMinutes", label: "Avg Repair Time", label_i18n: {"en":"Avg Repair Time"}, type: "number", unit: "minutes", required: false, smartDefault: 60, validation: { min: 0 }, helper: "", expertMeaning: "Average repair time per comeback", expertMeaning_i18n: {"en":"Average repair time per comeback"} },
    { id: "laborRate", label: "Labor Rate", label_i18n: {"en":"Labor Rate"}, type: "number", unit: "USD/hour", required: true, smartDefault: 85, validation: { min: 0 }, helper: "", expertMeaning: "Shop labor rate per hour", expertMeaning_i18n: {"en":"Shop labor rate per hour"} },
    ...
    { id: "bayOccupancyHours", label: "Bay Occupancy Time", label_i18n: {"en":"Bay Occupancy Time"}, type: "number", unit: "hours", required: false, smartDefault: 2.5, validation: { min: 0 }, helper: "", expertMeaning: "Bay hours per comeback", expertMeaning_i18n: {"en":"Bay hours per comeback"} },
    { id: "revenuePerBayHour", label: "Revenue Per Bay Hour", label_i18n: {"en":"Revenue Per Bay Hour"}, type: "number", unit: "USD/hour", required: false, smartDefault: 125, validation: { min: 0 }, helper: "", expertMeaning: "Average revenue per bay hour", expertMeaning_i18n: {"en":"Average revenue per bay hour"} },
  ],
  outputs: [
