...
export const CONTAINER_LOAD_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "itemVolumes", label: "Urun Hacimleri", label_i18n: {"en":"Item volumes array"}, type: "number", unit: "m³", array: true, required: true, validation: { min: 0 }, helper: "", expertMeaning: "Item volumes array", expertMeaning_i18n: {"en":"Item volumes array"} },
  ...
  outputs: [
    ...
    { id: "weightUtil", label: "Agrlk Doluluk", label_i18n: {"en":"Weight Doluluk"}, unit: "%", format: "percentage" },
    { id: "loadEfficiency", label: "Yukleme Verimi", label_i18n: {"en":"Loading Efficiency"}, unit: "%", format: "percentage" },
    { id: "wastedSpaceCost", label: "Void Space Cost", label_i18n: {"en":"Void Space Cost"}, unit: "USD", format: "currency" },
  ],
  thresholds: [{ fieldId: "loadEfficiency", warning: 75, critical: 50, direction: "lower_is_bad", warningMessage: "Verim < %75 — konteyner optimizasyonu onerilir.", warningMessage_i18n: {"en":"Efficiency < 75% — container optimization recommended."}, criticalMessage: "Verim < %50 — yukleme plani yenilenmeli.", criticalMessage_i18n: {"en":"Efficiency < 50% — Loading plan should be revised."} }],
  formulaPipeline: [
  ...
  assumptions: { hiddenLossMultiplier: 1.05, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["Doluluk = min(Hacim%, Agirlik%).", "Bos alan maliyeti = (1 - Verim) × Navlun."],assumptionNotes_i18n:[{"en":"Fill rate = min(Volume%, Weight%)."},{"en":"Empty space cost = (1 - Efficiency) × Freight."}] },
};
