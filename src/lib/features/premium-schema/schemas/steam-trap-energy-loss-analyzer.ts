...
export const STEAM_TRAP_ENERGY_LOSS_ANALYZER: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "operatingHoursPerYear", label: "Annual Operating Hours", label_i18n: {"en":"Annual Operating Hours"}, type: "number", unit: "saat/yıl", required: true, smartDefault: 8000, validation: { min: 1 }, helper: "", expertMeaning: "Operating hours per year", expertMeaning_i18n: {"en":"Operating hours per year"} },
    ...
    { id: "faultyTraps", label: "Number of Faulty Traps", label_i18n: {"en":"Number of Faulty Traps"}, type: "number", unit: "adet", required: false, smartDefault: 10, validation: { min: 1 }, helper: "", expertMeaning: "Number of failed traps", expertMeaning_i18n: {"en":"Number of failed traps"} },
    { id: "replacementCost", label: "Replacement Cost", label_i18n: {"en":"Replacement Cost"}, type: "number", unit: "USD/adet", required: false, smartDefault: 350, validation: { min: 1 }, helper: "", expertMeaning: "Replacement cost per trap", expertMeaning_i18n: {"en":"Replacement cost per trap"} },
  ],
  outputs: [
    { id: "steamLossRate", label: "Steam Leak Rate", label_i18n: {"en":"Steam Leak Rate"}, unit: "ton/saat", format: "number" },
    { id: "annualLoss", label: "Annual Energy Loss", label_i18n: {"en":"Annual Energy Loss"}, unit: "USD/yıl", format: "currency", isBigNumber: true },
