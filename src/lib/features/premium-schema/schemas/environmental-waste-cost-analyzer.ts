...
export const ENVIRONMENTAL_WASTE_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Atik bertaraf, tehlikeli atik ve emisyon maliyetleri ayristirilmazsa cevre cezalari ve surdurulebilirlik reportlamasi hatali olur.", painStatement_i18n: {"en":"If waste disposal, hazardous waste, and emission costs are not separated, environmental penalties and sustainability reporting will be incorrect."},
  inputs: [
    ...
    { id: "disposalFee", label: "Bertaraf Ucreti", label_i18n: {"en":"Disposal fee per ton"}, type: "number", unit: "USD/ton", required: true, smartDefault: 100, validation: { min: 0 }, helper: "", expertMeaning: "Disposal fee per ton", expertMeaning_i18n: {"en":"Disposal fee per ton"} },
    ...
    { id: "hazSurcharge", label: "Tehlikeli Ek Ucret", label_i18n: {"en":"Hazardous surcharge"}, type: "number", unit: "USD/ton", required: false, smartDefault: 100, validation: { min: 0 }, helper: "", expertMeaning: "Hazardous surcharge", expertMeaning_i18n: {"en":"Hazardous surcharge"} },
  ...
  thresholds: [{ fieldId: "totalEnvCost", warning: 50000, critical: 150000, direction: "higher_is_bad", warningMessage: "Cevre maliyeti > $50K — azaltim programi baslatilmali.", warningMessage_i18n: {"en":"Environmental Cost > $50K — mitigation program must be initiated."}, criticalMessage: "Maliyet > $150K — acil cevre yonetimi aksiyonu.", criticalMessage_i18n: {"en":"Cost > $150K — urgent environmental management action."} }],
  formulaPipeline: [
