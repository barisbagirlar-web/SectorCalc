...
export const CURRENCY_RISK_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Kur riski hedge edilmezse, döviz açık pozisyonu beklenmedik zararlara yol açar.", painStatement_i18n: {"en":"Kur riski hedge edilmezse, döviz açık pozisyonu beklenmedik zararlara yol açar."},
  inputs: [
    ...
    { id: "fxPair", label: "Döviz Çifti", label_i18n: {"en":"Currency pair"}, type: "select", unit: "", enumValues: ["USD/TRY", "EUR/USD", "EUR/TRY", "GBP/USD", "USD/JPY"], required: true, smartDefault: "USD/TRY", helper: "", expertMeaning: "Currency pair", expertMeaning_i18n: {"en":"Currency pair"} },
    ...
    { id: "timeHorizon", label: "Zaman Ufku", label_i18n: {"en":"Zaman Ufku"}, type: "number", unit: "gün", required: true, smartDefault: 90, validation: { min: 1 }, helper: "", expertMeaning: "Risk horizon", expertMeaning_i18n: {"en":"Risk horizon"} },
    ...
    { id: "hedgeRatio", label: "Hedge Oranı", label_i18n: {"en":"Hedge ratio"}, type: "number", unit: "", required: false, smartDefault: 0.5, validation: { min: 0, max: 1 }, helper: "", expertMeaning: "Hedge ratio", expertMeaning_i18n: {"en":"Hedge ratio"} },
    { id: "forwardPoints", label: "Forward Puanı", label_i18n: {"en":"Forward premium/discount"}, type: "number", unit: "", required: false, smartDefault: 0.02, validation: { min: 0 }, helper: "", expertMeaning: "Forward premium/discount", expertMeaning_i18n: {"en":"Forward premium/discount"} },
    { id: "spotRate", label: "Spot Kur", label_i18n: {"en":"Spot Kur"}, type: "number", unit: "", required: true, smartDefault: 30, validation: { min: 0.01 }, helper: "", expertMeaning: "Current spot rate", expertMeaning_i18n: {"en":"Current spot rate"} },
    { id: "forwardRate", label: "Forward Kur", label_i18n: {"en":"Forward Kur"}, type: "number", unit: "", required: false, smartDefault: 32, validation: { min: 0.01 }, helper: "", expertMeaning: "Forward rate", expertMeaning_i18n: {"en":"Forward rate"} },
  ],
  outputs: [
    ...
    { id: "unhedgedVaR", label: "Hedge Edilmemiş VaR", label_i18n: {"en":"Hedge Edilmemis VaR"}, unit: "USD", format: "currency" },
    { id: "costOfHedge", label: "Hedge Maliyeti", label_i18n: {"en":"Hedge Maliyeti"}, unit: "USD", format: "currency" },
    { id: "netImpact", label: "Net Kur Etkisi", label_i18n: {"en":"Net Kur Etkisi"}, unit: "USD", format: "currency" },
  ],
  thresholds: [{ fieldId: "unhedgedVaR", warning: 50000, critical: 200000, direction: "higher_is_bad", warningMessage: "Açık VaR > $50K — hedge oranı artırılmalı.", warningMessage_i18n: {"en":"Açık VaR > $50K — hedge oranı artırılmalı."}, criticalMessage: "Açık VaR > $200K — acil hedge işlemi gerekli.", criticalMessage_i18n: {"en":"Açık VaR > $200K — acil hedge işlemi gerekli."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Kur Riski Raporu", title_i18n: {"en":"Kur Riski Raporu"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
