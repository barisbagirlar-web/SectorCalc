...
export const VACUUM_LEAK_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Vakum sistemlerindeki kaçaklar fark edilmeden yüksek enerji tüketimine ve üretim kaybına yol açar. Kaçak debisi ölçülmezse gereksiz kompresör kapasitesi kullanılır.", painStatement_i18n: {"en":"Vakum sistemlerindeki kaçaklar difference edilmeden high energy tüketimine ve Production kaybına yol açar. Leak debisi if not measured gereksiz kompresör Capacity kullanılır."},
  inputs: [
  ...
  thresholds: [
    { fieldId: "vacuumLeakCost", warning: 5000, critical: 15000, direction: "higher_is_bad", warningMessage: "Kaçak maliyeti > $5K — vakum hattı bakımı planlanmalı.", warningMessage_i18n: {"en":"Leak Cost > $5K — vakum hattı bakımı planlanmalı."}, criticalMessage: "Kaçak maliyeti > $15K — acil kaçak onarım programı başlatılmalı.", criticalMessage_i18n: {"en":"Leak Cost > $15K — urgent Leak repair program başlatılmalı."} },
    { fieldId: "vacuumCapacityWaste", warning: 10, critical: 25, direction: "higher_is_bad", warningMessage: "Kapasite kaybı > %10 — kompresör yükü optimize edilmeli.", warningMessage_i18n: {"en":"Capacity Loss > %10 — kompresör Load optimize edilmeli."}, criticalMessage: "Kapasite kaybı > %25 — sistem yeniden tasarlanmalı.", criticalMessage_i18n: {"en":"Capacity Loss > %25 — System re tasarlanmalı."} },
  ],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Vakum Kaçağı Enerji Maliyeti Raporu", title_i18n: {"en":"Vakum Kaçağı energy Cost Raporu"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
