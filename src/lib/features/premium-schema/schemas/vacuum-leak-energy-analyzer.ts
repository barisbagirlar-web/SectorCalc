...
export const VACUUM_LEAK_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Vakum sistemlerindeki kacaklar fark edilmeden yuksek enerji tuketimine ve uretim lossna yol acar. Kacak debisi olculmezse gereksiz kompresor kapasitesi kullanilir.", painStatement_i18n: {"en":"Leaks in vacuum systems cause high energy consumption and production loss if not detected. If leak flow rate is not measured, unnecessary compressor capacity is used."},
  inputs: [
    ...
    { id: "motorPower", label: "Vakum Pompa Gucu", label_i18n: {"en":"Vacuum pump Motor power"}, type: "number", unit: "kW", required: false, smartDefault: 7.5, validation: { min: 0.1 }, helper: "", expertMeaning: "Vacuum pump motor power", expertMeaning_i18n: {"en":"Vacuum pump Motor power"} },
  ...
  outputs: [
    ...
    { id: "vacuumLeakCost", label: "Leak Energy Cost", label_i18n: {"en":"Leak Energy Cost"}, unit: "USD/yil", format: "currency" },
    { id: "vacuumCapacityWaste", label: "Capacity Waste", label_i18n: {"en":"Capacity Waste"}, unit: "%", format: "percentage" },
    { id: "potentialSavings", label: "Potansiyel Tasarruf", label_i18n: {"en":"Potansiyel Tasarruf"}, unit: "USD/yil", format: "currency" },
  ],
  thresholds: [
    { fieldId: "vacuumLeakCost", warning: 5000, critical: 15000, direction: "higher_is_bad", warningMessage: "Kacak maliyeti > $5K — vakum hatti bakimi planlanmali.", warningMessage_i18n: {"en":"Leak cost > $5K — vacuum line maintenance should be scheduled."}, criticalMessage: "Kacak maliyeti > $15K — acil kacak onarim programi baslatilmali.", criticalMessage_i18n: {"en":"Leak cost > $15K — urgent leak repair program should be initiated."} },
    { fieldId: "vacuumCapacityWaste", warning: 10, critical: 25, direction: "higher_is_bad", warningMessage: "Kapasite kaybi > %10 — kompresor yuku optimize edilmeli.", warningMessage_i18n: {"en":"Capacity Loss > 10% — compressor Load must be optimized."}, criticalMessage: "Kapasite kaybi > %25 — sistem yeniden tasarlanmali.", criticalMessage_i18n: {"en":"Capacity Loss > 25% — System must be redesigned."} },
  ],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Vakum Kacagi Enerji Maliyeti Reportu", title_i18n: {"en":"Vacuum Leak Energy Cost Report"}, sections: ["executive_summary", "loss_breakdown", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 5, targetMarginPercent: 10, assumptionNotes: ["Kacak debisi ortalama deger uzerinden hesaplanir.", "Enerji maliyeti sabit $/kWh uzerinden hesaplanir.", "Kapasite kaybi yuzdesel olarak tahmin edilmistir."],assumptionNotes_i18n:[{"en":"Leak flow rate is calculated based on the average value."},{"en":"Energy cost is calculated based on a fixed $/kWh rate."},{"en":"Capacity loss is estimated as a percentage."}] },
};
