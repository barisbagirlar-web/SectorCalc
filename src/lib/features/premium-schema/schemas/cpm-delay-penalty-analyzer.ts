...
export const CPM_DELAY_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "CPM Gecikme Cezası & EOT Talep Analizi", name_i18n: {"en":"CPM Delay Penalty & EOT Claim Analyzer"}, sectorSlug: "construction", category: "cost",
  ...
  inputs: [
    ...
    { id: "actualDuration", label: "Gerçek Süre", label_i18n: {"en":"Actual project duration"}, type: "number", unit: "gün", required: true, smartDefault: 240, validation: { min: 1 }, helper: "", expertMeaning: "Actual project duration", expertMeaning_i18n: {"en":"Actual project duration"} },
    { id: "totalFloat", label: "Toplam Float", label_i18n: {"en":"Toplam Float"}, type: "number", unit: "gün", required: true, smartDefault: 10, validation: { min: 0 }, helper: "", expertMeaning: "Total float on critical path", expertMeaning_i18n: {"en":"Total float on critical path"} },
    ...
    { id: "ownerCaused", label: "İşveren Kaynaklı Gecikme", label_i18n: {"en":"Employer-caused delay"}, type: "number", unit: "gün", required: false, smartDefault: 10, validation: { min: 0 }, helper: "", expertMeaning: "Employer-caused delay", expertMeaning_i18n: {"en":"Employer-caused delay"} },
    { id: "dailyPenalty", label: "Günlük Gecikme Cezası", label_i18n: {"en":"Daily liquidated damages"}, type: "number", unit: "USD/gün", required: true, smartDefault: 5000, validation: { min: 0 }, helper: "", expertMeaning: "Daily liquidated damages", expertMeaning_i18n: {"en":"Daily liquidated damages"} },
    { id: "accelerationCost", label: "Hızlandırma Maliyeti", label_i18n: {"en":"Crashing/acceleration cost"}, type: "number", unit: "USD", required: false, smartDefault: 15000, validation: { min: 0 }, helper: "", expertMeaning: "Crashing/acceleration cost", expertMeaning_i18n: {"en":"Crashing/acceleration cost"} },
  ...
  outputs: [
    ...
    { id: "liquidatedDamages", label: "Sıvılaştırılmış Tazminat", label_i18n: {"en":"Svlastrlms Tazminat"}, unit: "USD", format: "currency" },
    { id: "netPenalty", label: "Net Ceza", label_i18n: {"en":"Net Ceza"}, unit: "USD", format: "currency" },
    { id: "eotClaim", label: "EOT Talep Hakkı", label_i18n: {"en":"EOT Talep Hakk"}, unit: "gün", format: "number", isBigNumber: true },
  ],
  thresholds: [
    { fieldId: "liquidatedDamages", warning: 50000, critical: 150000, direction: "higher_is_bad", warningMessage: "Ceza > $50K — hızlandırma veya EOT değerlendirilmeli.", warningMessage_i18n: {"en":"Ceza > $50K — hızlandırma veya EOT değerlendirilmeli."}, criticalMessage: "Ceza > $150K — hukuki süreç riski yüksek.", criticalMessage_i18n: {"en":"Ceza > $150K — hukuki süreç riski yüksek."} },
  ],
  formulaPipeline: [
