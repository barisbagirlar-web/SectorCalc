...
export const CONCRETE_VOLUME_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Şantiyelerde beton hacmi elle hesaplanırken fire, pompa ve tesviye maliyetleri sıklıkla gözden kaçar. Bu araç geometrik hacim + waste + maliyeti tek adımda hesaplar.", painStatement_i18n: {"en":"Şantiyelerde Concrete Volume elle hesaplanırken waste, Pump ve leveling maliyetleri sıklıkla gözden kaçar. Bu araç geometrik hacim + waste + Cost tek adımda hesaplar."},
  inputs: [
    ...
    { id: "footingCount", label: "Number of footings", label_i18n: {"en":"Number of footings"}, type: "number", unit: "adet", required: false, smartDefault: 6, validation: { min: 0 }, helper: "", expertMeaning: "Number of footings", expertMeaning_i18n: {"en":"Number of footings"} },
    ...
    { id: "columnCount", label: "Number of columns", label_i18n: {"en":"Number of columns"}, type: "number", unit: "adet", required: false, smartDefault: 10, validation: { min: 0 }, helper: "", expertMeaning: "Number of columns", expertMeaning_i18n: {"en":"Number of columns"} },
    ...
    { id: "concreteGrade", label: "Concrete strength class", label_i18n: {"en":"Concrete strength class"}, type: "select", unit: "", required: false, smartDefault: "C25", options: [...CONCRETE_GRADE_OPTIONS], helper: "", expertMeaning: "Concrete strength class", expertMeaning_i18n: {"en":"Concrete strength class"} },
    ...
    { id: "unitPrice", label: "Beton Birim Fiyat", label_i18n: {"en":"Concrete Unit Fiyat"}, type: "number", unit: "USD/m³", required: false, smartDefault: 120, validation: { min: 0 }, helper: "", expertMeaning: "Concrete price per m³", expertMeaning_i18n: {"en":"Concrete price per m³"} },
  ...
  outputs: [
    ...
    { id: "truckLoads", label: "Kamyon Says", label_i18n: {"en":"Kamyon Says"}, unit: "adet", format: "number" },
    { id: "totalCost", label: "Total Cost", label_i18n: {"en":"Total Cost"}, unit: "USD", format: "currency", isBigNumber: true },
  ],
  thresholds: [
    { fieldId: "totalCost", warning: 10000, critical: 50000, direction: "higher_is_bad", warningMessage: "Beton maliyeti > $10K — alternatif tedarikçi değerlendir.", warningMessage_i18n: {"en":"Concrete Cost > $10K — alternatif supplier değerlendir."}, criticalMessage: "Beton maliyeti > $50K — ihale süreci başlatılmalı.", criticalMessage_i18n: {"en":"Concrete Cost > $50K — ihale process başlatılmalı."} },
  ],
  formulaPipeline: [
