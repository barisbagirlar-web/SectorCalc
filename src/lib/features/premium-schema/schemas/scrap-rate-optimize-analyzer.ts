...
export const SCRAP_OPTIMIZE_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "scrapQty", label: "Scrap Quantity", label_i18n: {"en":"Scrap Quantity"}, type: "number", unit: "adet/yil", required: true, smartDefault: 5000, validation: { min: 0 }, helper: "", expertMeaning: "Annual scrap quantity", expertMeaning_i18n: {"en":"Annual scrap quantity"} },
    { id: "totalInput", label: "Toplam Uretim Girdisi", label_i18n: {"en":"Total Production Input"}, type: "number", unit: "adet/yil", required: true, smartDefault: 100000, validation: { min: 1 }, helper: "", expertMeaning: "Total input quantity", expertMeaning_i18n: {"en":"Total input quantity"} },
    ...
    { id: "labRate", label: "Labor Hourly Rate", label_i18n: {"en":"Labor Hourly Rate"}, type: "number", unit: "USD/saat", required: false, smartDefault: 25, validation: { min: 0 }, helper: "", expertMeaning: "Labor hourly rate", expertMeaning_i18n: {"en":"Labor hourly rate"} },
    { id: "machRate", label: "Makine Saat Ucreti", label_i18n: {"en":"Machine Hourly Rate"}, type: "number", unit: "USD/saat", required: false, smartDefault: 75, validation: { min: 0 }, helper: "", expertMeaning: "Machine hourly rate", expertMeaning_i18n: {"en":"Machine hourly rate"} },
  ...
  outputs: [
    ...
    { id: "totalScrapCost", label: "Total Scrap Cost", label_i18n: {"en":"Total Scrap Cost"}, unit: "USD/yil", format: "currency", isBigNumber: true },
  ],
