...
export const AUTO_SHOP_MARGIN_LEAK_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Auto Shop Marj Kaçak Analizi", name_i18n: {"en":"Auto Shop Margin Leak Analysis"}, sectorSlug: "auto-repair", category: "cost",
  ...
  inputs: [
    { id: "monthlyPartsRevenue", label: "Aylık Parça Geliri", label_i18n: {"en":"Monthly Parts Revenue"}, type: "number", unit: "USD", required: true, smartDefault: 40000, validation: { min: 0 }, helper: "", expertMeaning: "Parts revenue per month", expertMeaning_i18n: {"en":"Parts revenue per month"} },
    { id: "monthlyLaborRevenue", label: "Aylık İşçilik Geliri", label_i18n: {"en":"Monthly Labor Revenue"}, type: "number", unit: "USD", required: true, smartDefault: 60000, validation: { min: 0 }, helper: "", expertMeaning: "Labor revenue per month", expertMeaning_i18n: {"en":"Labor revenue per month"} },
    { id: "partsCogs", label: "Parça COGS", label_i18n: {"en":"Parts COGS"}, type: "number", unit: "USD", required: true, smartDefault: 28000, validation: { min: 0 }, helper: "", expertMeaning: "Parts cost of goods sold", expertMeaning_i18n: {"en":"Parts cost of goods sold"} },
    { id: "laborCost", label: "İşçilik Maliyeti", label_i18n: {"en":"Labor Cost"}, type: "number", unit: "USD", required: true, smartDefault: 36000, validation: { min: 0 }, helper: "", expertMeaning: "Direct labor cost", expertMeaning_i18n: {"en":"Direct labor cost"} },
    ...
    { id: "industryBenchmarkMargin", label: "Sektör Benchmark Marjı", label_i18n: {"en":"Industry Benchmark Margin"}, type: "number", unit: "%", required: false, smartDefault: 15, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Industry average net margin", expertMeaning_i18n: {"en":"Industry average net margin"} },
    { id: "monthlyDiscounts", label: "Aylık İskontolar", label_i18n: {"en":"Monthly Discounts"}, type: "number", unit: "USD", required: false, smartDefault: 3000, validation: { min: 0 }, helper: "", expertMeaning: "Total discounts given", expertMeaning_i18n: {"en":"Total discounts given"} },
  ...
  outputs: [
    { id: "grossMarginParts", label: "Parça Brüt Marjı", label_i18n: {"en":"Parca Brut Marj"}, unit: "%", format: "percentage" },
    { id: "effectiveLaborRate", label: "Efektif İşçilik Ücreti", label_i18n: {"en":"Efektif Iscilik Ucreti"}, unit: "USD/saat", format: "currency" },
    { id: "netMargin", label: "Net Marj", label_i18n: {"en":"Net Marj"}, unit: "%", format: "percentage" },
    { id: "annualLeakage", label: "Yıllık Marj Kaçağı", label_i18n: {"en":"Yllk Marj Kacag"}, unit: "USD", format: "currency", isBigNumber: true },
  ],
  thresholds: [
    { fieldId: "annualLeakage", warning: 30000, critical: 100000, direction: "higher_is_bad", warningMessage: "Yıllık kaçak > $30K — fiyatlama gözden geçirilmeli.", warningMessage_i18n: {"en":"Yıllık kaçak > $30K — fiyatlama gözden geçirilmeli."}, criticalMessage: "Yıllık kaçak > $100K — acil marj iyileştirme programı başlatılmalı.", criticalMessage_i18n: {"en":"Yıllık kaçak > $100K — acil marj iyileştirme programı başlatılmalı."} },
  ],
  formulaPipeline: [
