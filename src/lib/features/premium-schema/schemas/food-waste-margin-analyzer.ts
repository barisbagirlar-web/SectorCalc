...
export const FOOD_WASTE_MARGIN_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Gida uretiminde fire, bozulma ve asiri uretim maliyetleri ayristirilmazsa marj lossnin kaynagi tespit edilemez.", painStatement_i18n: {"en":"If waste, spoilage, and excessive production costs in food manufacturing are not disaggregated, the source of margin loss cannot be detected."},
  inputs: [
    ...
    { id: "prodCost", label: "Uretim Maliyeti", label_i18n: {"en":"Production cost per kg"}, type: "number", unit: "USD/kg", required: false, smartDefault: 5, validation: { min: 0 }, helper: "", expertMeaning: "Production cost per kg", expertMeaning_i18n: {"en":"Production cost per kg"} },
  ...
  thresholds: [{ fieldId: "yield", warning: 80, critical: 70, direction: "lower_is_bad", warningMessage: "Verim < %80 — fire azaltma programi baslatilmali.", warningMessage_i18n: {"en":"Efficiency < %80 — a waste reduction program should be initiated."}, criticalMessage: "Verim < %70 — proses iyilestirme acil.", criticalMessage_i18n: {"en":"Efficiency < %70 — proses improvement acil."} }],
  formulaPipeline: [
