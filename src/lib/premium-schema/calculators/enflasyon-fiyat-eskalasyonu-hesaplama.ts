...
import {
  validateEnflasyonFiyatEskalasyonuHesaplamaInputs,
  type EnflasyonFiyatEskalasyonuHesaplamaInputs,
} from "@/lib/premium-schema/calculators/enflasyon-fiyat-eskalasyonu-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: EnflasyonFiyatEskalasyonuHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof EnflasyonFiyatEskalasyonuHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: EnflasyonFiyatEskalasyonuHesaplamaInputs): Record<string, number> {
...
export function calculateEnflasyonFiyatEskalasyonuHesaplama(inputs: EnflasyonFiyatEskalasyonuHesaplamaInputs): {
