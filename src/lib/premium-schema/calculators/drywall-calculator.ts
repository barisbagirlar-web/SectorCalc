...
import {
  validateDrywallCalculatorInputs,
  type DrywallCalculatorInputs,
} from "@/lib/premium-schema/calculators/drywall-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DrywallCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DrywallCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: DrywallCalculatorInputs): Record<string, number> {
...
export function calculateDrywallCalculator(inputs: DrywallCalculatorInputs): {
