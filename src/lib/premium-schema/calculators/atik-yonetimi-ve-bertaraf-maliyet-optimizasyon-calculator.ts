...
import {
  validateAtikYonetimiVeBertarafMaliyetOptimizasyonCalculatorInputs,
  type AtikYonetimiVeBertarafMaliyetOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/atik-yonetimi-ve-bertaraf-maliyet-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AtikYonetimiVeBertarafMaliyetOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AtikYonetimiVeBertarafMaliyetOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: AtikYonetimiVeBertarafMaliyetOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateAtikYonetimiVeBertarafMaliyetOptimizasyonCalculator(inputs: AtikYonetimiVeBertarafMaliyetOptimizasyonCalculatorInputs): {
