...
import {
  validateKapasitePlanlamaVeDarbogazYatirimOnceliklendirmeCalculatorInputs,
  type KapasitePlanlamaVeDarbogazYatirimOnceliklendirmeCalculatorInputs,
} from "@/lib/premium-schema/calculators/kapasite-planlama-ve-darbogaz-yatirim-onceliklendirme-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KapasitePlanlamaVeDarbogazYatirimOnceliklendirmeCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KapasitePlanlamaVeDarbogazYatirimOnceliklendirmeCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KapasitePlanlamaVeDarbogazYatirimOnceliklendirmeCalculatorInputs): Record<string, number> {
...
export function calculateKapasitePlanlamaVeDarbogazYatirimOnceliklendirmeCalculator(inputs: KapasitePlanlamaVeDarbogazYatirimOnceliklendirmeCalculatorInputs): {
