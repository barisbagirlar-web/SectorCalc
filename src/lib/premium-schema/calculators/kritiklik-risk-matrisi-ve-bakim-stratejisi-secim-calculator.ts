...
import {
  validateKritiklikRiskMatrisiVeBakimStratejisiSecimCalculatorInputs,
  type KritiklikRiskMatrisiVeBakimStratejisiSecimCalculatorInputs,
} from "@/lib/premium-schema/calculators/kritiklik-risk-matrisi-ve-bakim-stratejisi-secim-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KritiklikRiskMatrisiVeBakimStratejisiSecimCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KritiklikRiskMatrisiVeBakimStratejisiSecimCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KritiklikRiskMatrisiVeBakimStratejisiSecimCalculatorInputs): Record<string, number> {
...
export function calculateKritiklikRiskMatrisiVeBakimStratejisiSecimCalculator(inputs: KritiklikRiskMatrisiVeBakimStratejisiSecimCalculatorInputs): {
