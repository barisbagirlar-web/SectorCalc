...
import {
  validateUnitCostCalculatorInputs,
  type UnitCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/unit-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: UnitCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof UnitCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: UnitCostCalculatorInputs): Record<string, number> {
...
export function calculateUnitCostCalculator(inputs: UnitCostCalculatorInputs): {
