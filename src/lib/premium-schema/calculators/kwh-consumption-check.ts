...
import {
  validateKwhConsumptionCheckInputs,
  type KwhConsumptionCheckInputs,
} from "@/lib/premium-schema/calculators/kwh-consumption-check-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KwhConsumptionCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KwhConsumptionCheckInputs] as number;
}

function runFormulaPipeline(inputs: KwhConsumptionCheckInputs): Record<string, number> {
...
export function calculateKwhConsumptionCheck(inputs: KwhConsumptionCheckInputs): {
