...
import {
  validateAmbalajMalzemesiDegisimiVeMaliyetEtkiCalculatorInputs,
  type AmbalajMalzemesiDegisimiVeMaliyetEtkiCalculatorInputs,
} from "@/lib/premium-schema/calculators/ambalaj-malzemesi-degisimi-ve-maliyet-etki-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AmbalajMalzemesiDegisimiVeMaliyetEtkiCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AmbalajMalzemesiDegisimiVeMaliyetEtkiCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: AmbalajMalzemesiDegisimiVeMaliyetEtkiCalculatorInputs): Record<string, number> {
...
export function calculateAmbalajMalzemesiDegisimiVeMaliyetEtkiCalculator(inputs: AmbalajMalzemesiDegisimiVeMaliyetEtkiCalculatorInputs): {
