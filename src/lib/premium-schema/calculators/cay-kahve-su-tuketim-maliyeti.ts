...
import {
  validateCayKahveSuTuketimMaliyetiInputs,
  type CayKahveSuTuketimMaliyetiInputs,
} from "@/lib/premium-schema/calculators/cay-kahve-su-tuketim-maliyeti-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CayKahveSuTuketimMaliyetiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CayKahveSuTuketimMaliyetiInputs] as number;
}

function runFormulaPipeline(inputs: CayKahveSuTuketimMaliyetiInputs): Record<string, number> {
...
export function calculateCayKahveSuTuketimMaliyeti(inputs: CayKahveSuTuketimMaliyetiInputs): {
