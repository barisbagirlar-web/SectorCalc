...
import {
  validatePaintCoverageCalculatorInputs,
  type PaintCoverageCalculatorInputs,
} from "@/lib/premium-schema/calculators/paint-coverage-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PaintCoverageCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PaintCoverageCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: PaintCoverageCalculatorInputs): Record<string, number> {
...
export function calculatePaintCoverageCalculator(inputs: PaintCoverageCalculatorInputs): {
