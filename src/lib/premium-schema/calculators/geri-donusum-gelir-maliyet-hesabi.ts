...
import {
  validateGeriDonusumGelirMaliyetHesabiInputs,
  type GeriDonusumGelirMaliyetHesabiInputs,
} from "@/lib/premium-schema/calculators/geri-donusum-gelir-maliyet-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: GeriDonusumGelirMaliyetHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof GeriDonusumGelirMaliyetHesabiInputs] as number;
}

function runFormulaPipeline(inputs: GeriDonusumGelirMaliyetHesabiInputs): Record<string, number> {
...
export function calculateGeriDonusumGelirMaliyetHesabi(inputs: GeriDonusumGelirMaliyetHesabiInputs): {
