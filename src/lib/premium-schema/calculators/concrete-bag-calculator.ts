...
import {
  validateConcreteBagCalculatorInputs,
  type ConcreteBagCalculatorInputs,
} from "@/lib/premium-schema/calculators/concrete-bag-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ConcreteBagCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ConcreteBagCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ConcreteBagCalculatorInputs): Record<string, number> {
...
export function calculateConcreteBagCalculator(inputs: ConcreteBagCalculatorInputs): {
