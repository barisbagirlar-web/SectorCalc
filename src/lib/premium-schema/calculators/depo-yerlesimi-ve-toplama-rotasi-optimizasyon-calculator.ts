...
import {
  validateDepoYerlesimiVeToplamaRotasiOptimizasyonCalculatorInputs,
  type DepoYerlesimiVeToplamaRotasiOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/depo-yerlesimi-ve-toplama-rotasi-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DepoYerlesimiVeToplamaRotasiOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DepoYerlesimiVeToplamaRotasiOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: DepoYerlesimiVeToplamaRotasiOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateDepoYerlesimiVeToplamaRotasiOptimizasyonCalculator(inputs: DepoYerlesimiVeToplamaRotasiOptimizasyonCalculatorInputs): {
