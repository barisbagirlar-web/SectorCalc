...
import {
  validateTesisYerlesimiVeMalzemeAkisMesafeOptimizasyonCalculatorInputs,
  type TesisYerlesimiVeMalzemeAkisMesafeOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/tesis-yerlesimi-ve-malzeme-akis-mesafe-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TesisYerlesimiVeMalzemeAkisMesafeOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TesisYerlesimiVeMalzemeAkisMesafeOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: TesisYerlesimiVeMalzemeAkisMesafeOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateTesisYerlesimiVeMalzemeAkisMesafeOptimizasyonCalculator(inputs: TesisYerlesimiVeMalzemeAkisMesafeOptimizasyonCalculatorInputs): {
