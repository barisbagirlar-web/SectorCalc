...
import {
  validateBoruHattiSurtunmeVePompaEnerjiKayipCalculatorInputs,
  type BoruHattiSurtunmeVePompaEnerjiKayipCalculatorInputs,
} from "@/lib/premium-schema/calculators/boru-hatti-surtunme-ve-pompa-enerji-kayip-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BoruHattiSurtunmeVePompaEnerjiKayipCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BoruHattiSurtunmeVePompaEnerjiKayipCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: BoruHattiSurtunmeVePompaEnerjiKayipCalculatorInputs): Record<string, number> {
...
export function calculateBoruHattiSurtunmeVePompaEnerjiKayipCalculator(inputs: BoruHattiSurtunmeVePompaEnerjiKayipCalculatorInputs): {
