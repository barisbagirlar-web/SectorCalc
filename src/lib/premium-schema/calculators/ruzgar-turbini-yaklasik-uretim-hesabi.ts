...
import {
  validateRuzgarTurbiniYaklasikUretimHesabiInputs,
  type RuzgarTurbiniYaklasikUretimHesabiInputs,
} from "@/lib/premium-schema/calculators/ruzgar-turbini-yaklasik-uretim-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: RuzgarTurbiniYaklasikUretimHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof RuzgarTurbiniYaklasikUretimHesabiInputs] as number;
}

function runFormulaPipeline(inputs: RuzgarTurbiniYaklasikUretimHesabiInputs): Record<string, number> {
...
export function calculateRuzgarTurbiniYaklasikUretimHesabi(inputs: RuzgarTurbiniYaklasikUretimHesabiInputs): {
