...
import {
  validateUpsKesintisizGucKaynagiSecimiInputs,
  type UpsKesintisizGucKaynagiSecimiInputs,
} from "@/lib/premium-schema/calculators/ups-kesintisiz-guc-kaynagi-secimi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: UpsKesintisizGucKaynagiSecimiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof UpsKesintisizGucKaynagiSecimiInputs] as number;
}

function runFormulaPipeline(inputs: UpsKesintisizGucKaynagiSecimiInputs): Record<string, number> {
...
export function calculateUpsKesintisizGucKaynagiSecimi(inputs: UpsKesintisizGucKaynagiSecimiInputs): {
