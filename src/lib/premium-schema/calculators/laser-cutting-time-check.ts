...
import {
  validateLaserCuttingTimeCheckInputs,
  type LaserCuttingTimeCheckInputs,
} from "@/lib/premium-schema/calculators/laser-cutting-time-check-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: LaserCuttingTimeCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof LaserCuttingTimeCheckInputs] as number;
}

function runFormulaPipeline(inputs: LaserCuttingTimeCheckInputs): Record<string, number> {
...
export function calculateLaserCuttingTimeCheck(inputs: LaserCuttingTimeCheckInputs): {
