...
import {
  validateEnergyConsumptionCheckInputs,
  type EnergyConsumptionCheckInputs,
} from "@/lib/premium-schema/calculators/energy-consumption-check-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: EnergyConsumptionCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof EnergyConsumptionCheckInputs] as number;
}

function runFormulaPipeline(inputs: EnergyConsumptionCheckInputs): Record<string, number> {
...
export function calculateEnergyConsumptionCheck(inputs: EnergyConsumptionCheckInputs): {
