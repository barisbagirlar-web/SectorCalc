...
import {
  validateElectricalPanelReworkCostInputs,
  type ElectricalPanelReworkCostInputs,
} from "@/lib/premium-schema/calculators/electrical-panel-rework-cost-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ElectricalPanelReworkCostInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ElectricalPanelReworkCostInputs] as number;
}

function runFormulaPipeline(inputs: ElectricalPanelReworkCostInputs): Record<string, number> {
...
export function calculateElectricalPanelReworkCost(inputs: ElectricalPanelReworkCostInputs): {
  totalExposure: number;
  wiringOverrunCost: number;
  ...
  return {
    ...
    wiringOverrunCost: computed.wiringOverrunCost,
