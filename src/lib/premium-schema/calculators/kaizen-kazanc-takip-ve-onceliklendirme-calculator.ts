...
import {
  validateKaizenKazancTakipVeOnceliklendirmeCalculatorInputs,
  type KaizenKazancTakipVeOnceliklendirmeCalculatorInputs,
} from "@/lib/premium-schema/calculators/kaizen-kazanc-takip-ve-onceliklendirme-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KaizenKazancTakipVeOnceliklendirmeCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KaizenKazancTakipVeOnceliklendirmeCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KaizenKazancTakipVeOnceliklendirmeCalculatorInputs): Record<string, number> {
...
export function calculateKaizenKazancTakipVeOnceliklendirmeCalculator(inputs: KaizenKazancTakipVeOnceliklendirmeCalculatorInputs): {
