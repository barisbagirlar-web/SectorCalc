...
import {
  validateVolumetricWeightCalculatorInputs,
  type VolumetricWeightCalculatorInputs,
} from "@/lib/premium-schema/calculators/volumetric-weight-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: VolumetricWeightCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof VolumetricWeightCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: VolumetricWeightCalculatorInputs): Record<string, number> {
...
export function calculateVolumetricWeightCalculator(inputs: VolumetricWeightCalculatorInputs): {
