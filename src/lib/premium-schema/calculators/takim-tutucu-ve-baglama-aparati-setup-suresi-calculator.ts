...
import {
  validateTakimTutucuVeBaglamaAparatiSetupSuresiCalculatorInputs,
  type TakimTutucuVeBaglamaAparatiSetupSuresiCalculatorInputs,
} from "@/lib/premium-schema/calculators/takim-tutucu-ve-baglama-aparati-setup-suresi-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TakimTutucuVeBaglamaAparatiSetupSuresiCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TakimTutucuVeBaglamaAparatiSetupSuresiCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: TakimTutucuVeBaglamaAparatiSetupSuresiCalculatorInputs): Record<string, number> {
...
export function calculateTakimTutucuVeBaglamaAparatiSetupSuresiCalculator(inputs: TakimTutucuVeBaglamaAparatiSetupSuresiCalculatorInputs): {
