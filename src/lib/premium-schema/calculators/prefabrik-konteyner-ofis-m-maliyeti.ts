...
import {
  validatePrefabrikKonteynerOfisMMaliyetiInputs,
  type PrefabrikKonteynerOfisMMaliyetiInputs,
} from "@/lib/premium-schema/calculators/prefabrik-konteyner-ofis-m-maliyeti-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PrefabrikKonteynerOfisMMaliyetiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PrefabrikKonteynerOfisMMaliyetiInputs] as number;
}

function runFormulaPipeline(inputs: PrefabrikKonteynerOfisMMaliyetiInputs): Record<string, number> {
...
export function calculatePrefabrikKonteynerOfisMMaliyeti(inputs: PrefabrikKonteynerOfisMMaliyetiInputs): {
