...
import {
  validateBoilerEfficiencyCalculatorInputs,
  type BoilerEfficiencyCalculatorInputs,
} from "@/lib/premium-schema/calculators/boiler-efficiency-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BoilerEfficiencyCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BoilerEfficiencyCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: BoilerEfficiencyCalculatorInputs): Record<string, number> {
...
export function calculateBoilerEfficiencyCalculator(inputs: BoilerEfficiencyCalculatorInputs): {
