...
import {
  validateRoofingWeatherDelayRiskInputs,
  type RoofingWeatherDelayRiskInputs,
} from "@/lib/premium-schema/calculators/roofing-weather-delay-risk-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: RoofingWeatherDelayRiskInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof RoofingWeatherDelayRiskInputs] as number;
}

function runFormulaPipeline(inputs: RoofingWeatherDelayRiskInputs): Record<string, number> {
...
export function calculateRoofingWeatherDelayRisk(inputs: RoofingWeatherDelayRiskInputs): {
