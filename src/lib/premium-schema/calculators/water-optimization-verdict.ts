...
import {
  validateWaterOptimizationVerdictInputs,
  type WaterOptimizationVerdictInputs,
} from "@/lib/premium-schema/calculators/water-optimization-verdict-validation";
...
const SUMMARY_CRITICAL_THRESHOLD = 1;
const summaryDirection: "lower_is_bad" | "higher_is_bad" = "lower_is_bad";

function resolveMappedValue(
  sourceKey: string,
  userInputs: WaterOptimizationVerdictInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof WaterOptimizationVerdictInputs] as number;
}

function runFormulaPipeline(inputs: WaterOptimizationVerdictInputs): Record<string, number> {
...
export function calculateWaterOptimizationVerdict(inputs: WaterOptimizationVerdictInputs): {
