...
import {
  validateBoyaVeApreRecetesiMaliyetOptimizasyonCalculatorInputs,
  type BoyaVeApreRecetesiMaliyetOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/boya-ve-apre-recetesi-maliyet-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BoyaVeApreRecetesiMaliyetOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BoyaVeApreRecetesiMaliyetOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: BoyaVeApreRecetesiMaliyetOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateBoyaVeApreRecetesiMaliyetOptimizasyonCalculator(inputs: BoyaVeApreRecetesiMaliyetOptimizasyonCalculatorInputs): {
