...
import {
  validateKlimaBtuSecimHesaplamaInputs,
  type KlimaBtuSecimHesaplamaInputs,
} from "@/lib/premium-schema/calculators/klima-btu-secim-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KlimaBtuSecimHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KlimaBtuSecimHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: KlimaBtuSecimHesaplamaInputs): Record<string, number> {
...
export function calculateKlimaBtuSecimHesaplama(inputs: KlimaBtuSecimHesaplamaInputs): {
