...
import {
  validateFreightCostCalculatorInputs,
  type FreightCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/freight-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FreightCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FreightCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FreightCostCalculatorInputs): Record<string, number> {
...
export function calculateFreightCostCalculator(inputs: FreightCostCalculatorInputs): {
