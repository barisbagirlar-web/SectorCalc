...
import {
  validateWaterUsageCalculatorInputs,
  type WaterUsageCalculatorInputs,
} from "@/lib/premium-schema/calculators/water-usage-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: WaterUsageCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof WaterUsageCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: WaterUsageCalculatorInputs): Record<string, number> {
...
export function calculateWaterUsageCalculator(inputs: WaterUsageCalculatorInputs): {
