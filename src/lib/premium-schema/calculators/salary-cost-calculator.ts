...
import {
  validateSalaryCostCalculatorInputs,
  type SalaryCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/salary-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SalaryCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SalaryCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: SalaryCostCalculatorInputs): Record<string, number> {
...
export function calculateSalaryCostCalculator(inputs: SalaryCostCalculatorInputs): {
