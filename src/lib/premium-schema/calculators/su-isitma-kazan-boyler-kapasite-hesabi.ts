...
import {
  validateSuIsitmaKazanBoylerKapasiteHesabiInputs,
  type SuIsitmaKazanBoylerKapasiteHesabiInputs,
} from "@/lib/premium-schema/calculators/su-isitma-kazan-boyler-kapasite-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SuIsitmaKazanBoylerKapasiteHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SuIsitmaKazanBoylerKapasiteHesabiInputs] as number;
}

function runFormulaPipeline(inputs: SuIsitmaKazanBoylerKapasiteHesabiInputs): Record<string, number> {
...
export function calculateSuIsitmaKazanBoylerKapasiteHesabi(inputs: SuIsitmaKazanBoylerKapasiteHesabiInputs): {
