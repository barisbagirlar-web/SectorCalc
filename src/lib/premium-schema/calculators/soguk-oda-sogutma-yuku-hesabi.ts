...
import {
  validateSogukOdaSogutmaYukuHesabiInputs,
  type SogukOdaSogutmaYukuHesabiInputs,
} from "@/lib/premium-schema/calculators/soguk-oda-sogutma-yuku-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SogukOdaSogutmaYukuHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SogukOdaSogutmaYukuHesabiInputs] as number;
}

function runFormulaPipeline(inputs: SogukOdaSogutmaYukuHesabiInputs): Record<string, number> {
...
export function calculateSogukOdaSogutmaYukuHesabi(inputs: SogukOdaSogutmaYukuHesabiInputs): {
