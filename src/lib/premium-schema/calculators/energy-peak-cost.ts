...
import {
  validateEnergyPeakCostInputs,
  type EnergyPeakCostInputs,
} from "@/lib/premium-schema/calculators/energy-peak-cost-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: EnergyPeakCostInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof EnergyPeakCostInputs] as number;
}

function runFormulaPipeline(inputs: EnergyPeakCostInputs): Record<string, number> {
...
export function calculateEnergyPeakCost(inputs: EnergyPeakCostInputs): {
