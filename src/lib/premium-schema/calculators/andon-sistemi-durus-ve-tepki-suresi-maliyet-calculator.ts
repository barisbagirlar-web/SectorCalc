...
import {
  validateAndonSistemiDurusVeTepkiSuresiMaliyetCalculatorInputs,
  type AndonSistemiDurusVeTepkiSuresiMaliyetCalculatorInputs,
} from "@/lib/premium-schema/calculators/andon-sistemi-durus-ve-tepki-suresi-maliyet-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AndonSistemiDurusVeTepkiSuresiMaliyetCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AndonSistemiDurusVeTepkiSuresiMaliyetCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: AndonSistemiDurusVeTepkiSuresiMaliyetCalculatorInputs): Record<string, number> {
...
export function calculateAndonSistemiDurusVeTepkiSuresiMaliyetCalculator(inputs: AndonSistemiDurusVeTepkiSuresiMaliyetCalculatorInputs): {
