...
import {
  validateBrickCalculatorInputs,
  type BrickCalculatorInputs,
} from "@/lib/premium-schema/calculators/brick-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BrickCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BrickCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: BrickCalculatorInputs): Record<string, number> {
...
export function calculateBrickCalculator(inputs: BrickCalculatorInputs): {
