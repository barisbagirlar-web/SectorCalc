...
import {
  validateFotokopiYaziciTonerSayfaMaliyetiInputs,
  type FotokopiYaziciTonerSayfaMaliyetiInputs,
} from "@/lib/premium-schema/calculators/fotokopi-yazici-toner-sayfa-maliyeti-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FotokopiYaziciTonerSayfaMaliyetiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FotokopiYaziciTonerSayfaMaliyetiInputs] as number;
}

function runFormulaPipeline(inputs: FotokopiYaziciTonerSayfaMaliyetiInputs): Record<string, number> {
...
export function calculateFotokopiYaziciTonerSayfaMaliyeti(inputs: FotokopiYaziciTonerSayfaMaliyetiInputs): {
