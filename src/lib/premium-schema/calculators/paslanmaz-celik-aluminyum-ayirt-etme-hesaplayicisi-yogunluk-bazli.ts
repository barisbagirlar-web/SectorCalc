...
import {
  validatePaslanmazCelikAluminyumAyirtEtmeHesaplayicisiYogunlukBazliInputs,
  type PaslanmazCelikAluminyumAyirtEtmeHesaplayicisiYogunlukBazliInputs,
} from "@/lib/premium-schema/calculators/paslanmaz-celik-aluminyum-ayirt-etme-hesaplayicisi-yogunluk-bazli-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PaslanmazCelikAluminyumAyirtEtmeHesaplayicisiYogunlukBazliInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PaslanmazCelikAluminyumAyirtEtmeHesaplayicisiYogunlukBazliInputs] as number;
}

function runFormulaPipeline(inputs: PaslanmazCelikAluminyumAyirtEtmeHesaplayicisiYogunlukBazliInputs): Record<string, number> {
...
export function calculatePaslanmazCelikAluminyumAyirtEtmeHesaplayicisiYogunlukBazli(inputs: PaslanmazCelikAluminyumAyirtEtmeHesaplayicisiYogunlukBazliInputs): {
