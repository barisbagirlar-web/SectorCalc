...
import {
  validateEnjeksiyonDokumCekmePayiHesabiInputs,
  type EnjeksiyonDokumCekmePayiHesabiInputs,
} from "@/lib/premium-schema/calculators/enjeksiyon-dokum-cekme-payi-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: EnjeksiyonDokumCekmePayiHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof EnjeksiyonDokumCekmePayiHesabiInputs] as number;
}

function runFormulaPipeline(inputs: EnjeksiyonDokumCekmePayiHesabiInputs): Record<string, number> {
...
export function calculateEnjeksiyonDokumCekmePayiHesabi(inputs: EnjeksiyonDokumCekmePayiHesabiInputs): {
