...
import {
  validateHourlyRateCalculatorInputs,
  type HourlyRateCalculatorInputs,
} from "@/lib/premium-schema/calculators/hourly-rate-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: HourlyRateCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof HourlyRateCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: HourlyRateCalculatorInputs): Record<string, number> {
...
export function calculateHourlyRateCalculator(inputs: HourlyRateCalculatorInputs): {
