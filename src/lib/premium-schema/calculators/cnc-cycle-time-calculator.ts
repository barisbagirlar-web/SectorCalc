...
import {
  validateCncCycleTimeCalculatorInputs,
  type CncCycleTimeCalculatorInputs,
} from "@/lib/premium-schema/calculators/cnc-cycle-time-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CncCycleTimeCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CncCycleTimeCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: CncCycleTimeCalculatorInputs): Record<string, number> {
...
export function calculateCncCycleTimeCalculator(inputs: CncCycleTimeCalculatorInputs): {
