...
import {
  validatePanelShopMarginVerdictInputs,
  type PanelShopMarginVerdictInputs,
} from "@/lib/premium-schema/calculators/panel-shop-margin-verdict-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PanelShopMarginVerdictInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PanelShopMarginVerdictInputs] as number;
}

function runFormulaPipeline(inputs: PanelShopMarginVerdictInputs): Record<string, number> {
...
export function calculatePanelShopMarginVerdict(inputs: PanelShopMarginVerdictInputs): {
