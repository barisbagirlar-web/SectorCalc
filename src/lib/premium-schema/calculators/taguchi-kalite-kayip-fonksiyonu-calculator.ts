...
import {
  validateTaguchiKaliteKayipFonksiyonuCalculatorInputs,
  type TaguchiKaliteKayipFonksiyonuCalculatorInputs,
} from "@/lib/premium-schema/calculators/taguchi-kalite-kayip-fonksiyonu-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TaguchiKaliteKayipFonksiyonuCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TaguchiKaliteKayipFonksiyonuCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: TaguchiKaliteKayipFonksiyonuCalculatorInputs): Record<string, number> {
...
export function calculateTaguchiKaliteKayipFonksiyonuCalculator(inputs: TaguchiKaliteKayipFonksiyonuCalculatorInputs): {
