...
import {
  validateMaterialWasteCalculatorInputs,
  type MaterialWasteCalculatorInputs,
} from "@/lib/premium-schema/calculators/material-waste-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: MaterialWasteCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof MaterialWasteCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: MaterialWasteCalculatorInputs): Record<string, number> {
...
export function calculateMaterialWasteCalculator(inputs: MaterialWasteCalculatorInputs): {
