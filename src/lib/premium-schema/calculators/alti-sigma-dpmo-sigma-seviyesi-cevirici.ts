...
import {
  validateAltiSigmaDpmoSigmaSeviyesiCeviriciInputs,
  type AltiSigmaDpmoSigmaSeviyesiCeviriciInputs,
} from "@/lib/premium-schema/calculators/alti-sigma-dpmo-sigma-seviyesi-cevirici-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AltiSigmaDpmoSigmaSeviyesiCeviriciInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AltiSigmaDpmoSigmaSeviyesiCeviriciInputs] as number;
}

function runFormulaPipeline(inputs: AltiSigmaDpmoSigmaSeviyesiCeviriciInputs): Record<string, number> {
...
export function calculateAltiSigmaDpmoSigmaSeviyesiCevirici(inputs: AltiSigmaDpmoSigmaSeviyesiCeviriciInputs): {
