...
import {
  validateFoodCostCalculatorInputs,
  type FoodCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/food-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FoodCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FoodCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FoodCostCalculatorInputs): Record<string, number> {
...
export function calculateFoodCostCalculator(inputs: FoodCostCalculatorInputs): {
