...
import {
  validateIsiYukuVeIsitmaSogutmaKapasiteOptimizasyonCalculatorInputs,
  type IsiYukuVeIsitmaSogutmaKapasiteOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/isi-yuku-ve-isitma-sogutma-kapasite-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IsiYukuVeIsitmaSogutmaKapasiteOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IsiYukuVeIsitmaSogutmaKapasiteOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: IsiYukuVeIsitmaSogutmaKapasiteOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateIsiYukuVeIsitmaSogutmaKapasiteOptimizasyonCalculator(inputs: IsiYukuVeIsitmaSogutmaKapasiteOptimizasyonCalculatorInputs): {
