...
import {
  validateCobotVsManuelIscilikKarsilastirmaCalculatorInputs,
  type CobotVsManuelIscilikKarsilastirmaCalculatorInputs,
} from "@/lib/premium-schema/calculators/cobot-vs-manuel-iscilik-karsilastirma-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CobotVsManuelIscilikKarsilastirmaCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CobotVsManuelIscilikKarsilastirmaCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: CobotVsManuelIscilikKarsilastirmaCalculatorInputs): Record<string, number> {
...
export function calculateCobotVsManuelIscilikKarsilastirmaCalculator(inputs: CobotVsManuelIscilikKarsilastirmaCalculatorInputs): {
