...
import {
  validateMotorGucuKwHpCeviriciInputs,
  type MotorGucuKwHpCeviriciInputs,
} from "@/lib/premium-schema/calculators/motor-gucu-kw-hp-cevirici-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: MotorGucuKwHpCeviriciInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof MotorGucuKwHpCeviriciInputs] as number;
}

function runFormulaPipeline(inputs: MotorGucuKwHpCeviriciInputs): Record<string, number> {
...
export function calculateMotorGucuKwHpCevirici(inputs: MotorGucuKwHpCeviriciInputs): {
