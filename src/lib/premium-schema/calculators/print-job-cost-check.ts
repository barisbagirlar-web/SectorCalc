...
import {
  validatePrintJobCostCheckInputs,
  type PrintJobCostCheckInputs,
} from "@/lib/premium-schema/calculators/print-job-cost-check-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PrintJobCostCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PrintJobCostCheckInputs] as number;
}

function runFormulaPipeline(inputs: PrintJobCostCheckInputs): Record<string, number> {
...
export function calculatePrintJobCostCheck(inputs: PrintJobCostCheckInputs): {
