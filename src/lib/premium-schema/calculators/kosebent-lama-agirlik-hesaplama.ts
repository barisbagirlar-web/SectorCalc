...
import {
  validateKosebentLamaAgirlikHesaplamaInputs,
  type KosebentLamaAgirlikHesaplamaInputs,
} from "@/lib/premium-schema/calculators/kosebent-lama-agirlik-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KosebentLamaAgirlikHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KosebentLamaAgirlikHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: KosebentLamaAgirlikHesaplamaInputs): Record<string, number> {
...
export function calculateKosebentLamaAgirlikHesaplama(inputs: KosebentLamaAgirlikHesaplamaInputs): {
