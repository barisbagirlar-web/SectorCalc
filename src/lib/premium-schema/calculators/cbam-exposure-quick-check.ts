...
import {
  validateCbamExposureQuickCheckInputs,
  type CbamExposureQuickCheckInputs,
} from "@/lib/premium-schema/calculators/cbam-exposure-quick-check-validation";
...
const SUMMARY_WARNING_THRESHOLD = 250000;
const SUMMARY_CRITICAL_THRESHOLD = 500000;
const summaryDirection: "lower_is_bad" | "higher_is_bad" = "higher_is_bad";

function resolveMappedValue(
  sourceKey: string,
  userInputs: CbamExposureQuickCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CbamExposureQuickCheckInputs] as number;
}

function runFormulaPipeline(inputs: CbamExposureQuickCheckInputs): Record<string, number> {
...
export function calculateCbamExposureQuickCheck(inputs: CbamExposureQuickCheckInputs): {
  ...
  return {
    ...
    cbamCost: computed.cbamCost,
    unitCbamCost: computed.unitCbamCost,
    summaryLevel,
    primaryDriver: "totalExposure",
    decisionVerdict: {
      summaryLevel,
