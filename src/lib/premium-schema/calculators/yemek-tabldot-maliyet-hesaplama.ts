...
import {
  validateYemekTabldotMaliyetHesaplamaInputs,
  type YemekTabldotMaliyetHesaplamaInputs,
} from "@/lib/premium-schema/calculators/yemek-tabldot-maliyet-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: YemekTabldotMaliyetHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof YemekTabldotMaliyetHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: YemekTabldotMaliyetHesaplamaInputs): Record<string, number> {
...
export function calculateYemekTabldotMaliyetHesaplama(inputs: YemekTabldotMaliyetHesaplamaInputs): {
