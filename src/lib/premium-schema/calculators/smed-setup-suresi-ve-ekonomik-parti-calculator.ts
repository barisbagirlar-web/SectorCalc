...
import {
  validateSmedSetupSuresiVeEkonomikPartiCalculatorInputs,
  type SmedSetupSuresiVeEkonomikPartiCalculatorInputs,
} from "@/lib/premium-schema/calculators/smed-setup-suresi-ve-ekonomik-parti-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SmedSetupSuresiVeEkonomikPartiCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SmedSetupSuresiVeEkonomikPartiCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: SmedSetupSuresiVeEkonomikPartiCalculatorInputs): Record<string, number> {
...
export function calculateSmedSetupSuresiVeEkonomikPartiCalculator(inputs: SmedSetupSuresiVeEkonomikPartiCalculatorInputs): {
