...
import {
  validateDonguselEkonomiVeUrunOmruUzatmaRoiCalculatorInputs,
  type DonguselEkonomiVeUrunOmruUzatmaRoiCalculatorInputs,
} from "@/lib/premium-schema/calculators/dongusel-ekonomi-ve-urun-omru-uzatma-roi-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DonguselEkonomiVeUrunOmruUzatmaRoiCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DonguselEkonomiVeUrunOmruUzatmaRoiCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: DonguselEkonomiVeUrunOmruUzatmaRoiCalculatorInputs): Record<string, number> {
...
export function calculateDonguselEkonomiVeUrunOmruUzatmaRoiCalculator(inputs: DonguselEkonomiVeUrunOmruUzatmaRoiCalculatorInputs): {
