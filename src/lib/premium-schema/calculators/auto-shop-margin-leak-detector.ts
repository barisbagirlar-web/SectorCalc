...
import {
  validateAutoShopMarginLeakDetectorInputs,
  type AutoShopMarginLeakDetectorInputs,
} from "@/lib/premium-schema/calculators/auto-shop-margin-leak-detector-validation";
...
const FORMULA_PIPELINE = [
  {
    ...
    "inputMap": {
      "amount": "monthlyRepairRevenue",
      "percent": "comebackRatePercent"
    },
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AutoShopMarginLeakDetectorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AutoShopMarginLeakDetectorInputs] as number;
}

function runFormulaPipeline(inputs: AutoShopMarginLeakDetectorInputs): Record<string, number> {
...
export function calculateAutoShopMarginLeakDetector(inputs: AutoShopMarginLeakDetectorInputs): {
  ...
  return {
    ...
    comebackCost: computed.comebackCost,
    diagnosticLeak: computed.diagnosticLeak,
    summaryLevel,
    primaryDriver: "totalExposure",
    decisionVerdict: {
      summaryLevel,
