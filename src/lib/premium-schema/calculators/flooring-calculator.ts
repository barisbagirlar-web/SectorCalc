...
import {
  validateFlooringCalculatorInputs,
  type FlooringCalculatorInputs,
} from "@/lib/premium-schema/calculators/flooring-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FlooringCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FlooringCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FlooringCalculatorInputs): Record<string, number> {
...
export function calculateFlooringCalculator(inputs: FlooringCalculatorInputs): {
