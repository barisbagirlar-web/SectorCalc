...
import {
  validateKritikYolCpmGecikmeCezasiVeHizlandirmaOptimizasyonCalculatorInputs,
  type KritikYolCpmGecikmeCezasiVeHizlandirmaOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/kritik-yol-cpm-gecikme-cezasi-ve-hizlandirma-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KritikYolCpmGecikmeCezasiVeHizlandirmaOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KritikYolCpmGecikmeCezasiVeHizlandirmaOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KritikYolCpmGecikmeCezasiVeHizlandirmaOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateKritikYolCpmGecikmeCezasiVeHizlandirmaOptimizasyonCalculator(inputs: KritikYolCpmGecikmeCezasiVeHizlandirmaOptimizasyonCalculatorInputs): {
