...
import {
  validateFazlaMesaiUcretiHesaplamaInputs,
  type FazlaMesaiUcretiHesaplamaInputs,
} from "@/lib/premium-schema/calculators/fazla-mesai-ucreti-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FazlaMesaiUcretiHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FazlaMesaiUcretiHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: FazlaMesaiUcretiHesaplamaInputs): Record<string, number> {
...
export function calculateFazlaMesaiUcretiHesaplama(inputs: FazlaMesaiUcretiHesaplamaInputs): {
