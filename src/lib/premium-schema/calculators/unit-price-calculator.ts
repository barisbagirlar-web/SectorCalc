...
import {
  validateUnitPriceCalculatorInputs,
  type UnitPriceCalculatorInputs,
} from "@/lib/premium-schema/calculators/unit-price-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: UnitPriceCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof UnitPriceCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: UnitPriceCalculatorInputs): Record<string, number> {
...
export function calculateUnitPriceCalculator(inputs: UnitPriceCalculatorInputs): {
