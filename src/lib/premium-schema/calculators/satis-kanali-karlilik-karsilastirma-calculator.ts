...
import {
  validateSatisKanaliKarlilikKarsilastirmaCalculatorInputs,
  type SatisKanaliKarlilikKarsilastirmaCalculatorInputs,
} from "@/lib/premium-schema/calculators/satis-kanali-karlilik-karsilastirma-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SatisKanaliKarlilikKarsilastirmaCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SatisKanaliKarlilikKarsilastirmaCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: SatisKanaliKarlilikKarsilastirmaCalculatorInputs): Record<string, number> {
...
export function calculateSatisKanaliKarlilikKarsilastirmaCalculator(inputs: SatisKanaliKarlilikKarsilastirmaCalculatorInputs): {
