...
import {
  validateKwhCostCalculatorInputs,
  type KwhCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/kwh-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KwhCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KwhCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KwhCostCalculatorInputs): Record<string, number> {
...
export function calculateKwhCostCalculator(inputs: KwhCostCalculatorInputs): {
