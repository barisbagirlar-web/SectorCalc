...
import {
  validateToolLifeCalculatorInputs,
  type ToolLifeCalculatorInputs,
} from "@/lib/premium-schema/calculators/tool-life-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ToolLifeCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ToolLifeCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ToolLifeCalculatorInputs): Record<string, number> {
...
export function calculateToolLifeCalculator(inputs: ToolLifeCalculatorInputs): {
