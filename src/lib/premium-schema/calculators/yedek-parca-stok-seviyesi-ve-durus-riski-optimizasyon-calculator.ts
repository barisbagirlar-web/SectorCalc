...
import {
  validateYedekParcaStokSeviyesiVeDurusRiskiOptimizasyonCalculatorInputs,
  type YedekParcaStokSeviyesiVeDurusRiskiOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/yedek-parca-stok-seviyesi-ve-durus-riski-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: YedekParcaStokSeviyesiVeDurusRiskiOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof YedekParcaStokSeviyesiVeDurusRiskiOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: YedekParcaStokSeviyesiVeDurusRiskiOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateYedekParcaStokSeviyesiVeDurusRiskiOptimizasyonCalculator(inputs: YedekParcaStokSeviyesiVeDurusRiskiOptimizasyonCalculatorInputs): {
