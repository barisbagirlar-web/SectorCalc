...
import {
  validateTitresimFrekansPeriyotHesaplamaInputs,
  type TitresimFrekansPeriyotHesaplamaInputs,
} from "@/lib/premium-schema/calculators/titresim-frekans-periyot-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TitresimFrekansPeriyotHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TitresimFrekansPeriyotHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: TitresimFrekansPeriyotHesaplamaInputs): Record<string, number> {
...
export function calculateTitresimFrekansPeriyotHesaplama(inputs: TitresimFrekansPeriyotHesaplamaInputs): {
