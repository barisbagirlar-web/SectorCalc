...
import {
  validateFertilizerDosageCalculatorInputs,
  type FertilizerDosageCalculatorInputs,
} from "@/lib/premium-schema/calculators/fertilizer-dosage-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FertilizerDosageCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FertilizerDosageCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FertilizerDosageCalculatorInputs): Record<string, number> {
...
export function calculateFertilizerDosageCalculator(inputs: FertilizerDosageCalculatorInputs): {
