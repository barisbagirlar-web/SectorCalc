...
import {
  validateSheetMetalWeightCalculatorInputs,
  type SheetMetalWeightCalculatorInputs,
} from "@/lib/premium-schema/calculators/sheet-metal-weight-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SheetMetalWeightCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SheetMetalWeightCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: SheetMetalWeightCalculatorInputs): Record<string, number> {
...
export function calculateSheetMetalWeightCalculator(inputs: SheetMetalWeightCalculatorInputs): {
