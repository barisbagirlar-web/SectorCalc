...
import {
  validateSogutmaSivisiKarisimOraniAntifrizBorYagiHesaplamaInputs,
  type SogutmaSivisiKarisimOraniAntifrizBorYagiHesaplamaInputs,
} from "@/lib/premium-schema/calculators/sogutma-sivisi-karisim-orani-antifriz-bor-yagi-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SogutmaSivisiKarisimOraniAntifrizBorYagiHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SogutmaSivisiKarisimOraniAntifrizBorYagiHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: SogutmaSivisiKarisimOraniAntifrizBorYagiHesaplamaInputs): Record<string, number> {
...
export function calculateSogutmaSivisiKarisimOraniAntifrizBorYagiHesaplama(inputs: SogutmaSivisiKarisimOraniAntifrizBorYagiHesaplamaInputs): {
