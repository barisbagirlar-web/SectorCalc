...
import {
  validateEnjeksiyonSogutmaSuresiVeCevrimOptimizasyonCalculatorInputs,
  type EnjeksiyonSogutmaSuresiVeCevrimOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/enjeksiyon-sogutma-suresi-ve-cevrim-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: EnjeksiyonSogutmaSuresiVeCevrimOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof EnjeksiyonSogutmaSuresiVeCevrimOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: EnjeksiyonSogutmaSuresiVeCevrimOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateEnjeksiyonSogutmaSuresiVeCevrimOptimizasyonCalculator(inputs: EnjeksiyonSogutmaSuresiVeCevrimOptimizasyonCalculatorInputs): {
