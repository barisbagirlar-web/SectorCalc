...
import {
  validateRecipeCostCheckInputs,
  type RecipeCostCheckInputs,
} from "@/lib/premium-schema/calculators/recipe-cost-check-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: RecipeCostCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof RecipeCostCheckInputs] as number;
}

function runFormulaPipeline(inputs: RecipeCostCheckInputs): Record<string, number> {
...
export function calculateRecipeCostCheck(inputs: RecipeCostCheckInputs): {
