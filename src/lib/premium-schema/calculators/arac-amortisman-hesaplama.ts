...
import {
  validateAracAmortismanHesaplamaInputs,
  type AracAmortismanHesaplamaInputs,
} from "@/lib/premium-schema/calculators/arac-amortisman-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AracAmortismanHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AracAmortismanHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: AracAmortismanHesaplamaInputs): Record<string, number> {
...
export function calculateAracAmortismanHesaplama(inputs: AracAmortismanHesaplamaInputs): {
