...
import {
  validateVadeFarkiVeErkenOdemeIskontosuOptimizasyonCalculatorInputs,
  type VadeFarkiVeErkenOdemeIskontosuOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/vade-farki-ve-erken-odeme-iskontosu-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: VadeFarkiVeErkenOdemeIskontosuOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof VadeFarkiVeErkenOdemeIskontosuOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: VadeFarkiVeErkenOdemeIskontosuOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateVadeFarkiVeErkenOdemeIskontosuOptimizasyonCalculator(inputs: VadeFarkiVeErkenOdemeIskontosuOptimizasyonCalculatorInputs): {
