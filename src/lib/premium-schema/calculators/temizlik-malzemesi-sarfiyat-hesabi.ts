...
import {
  validateTemizlikMalzemesiSarfiyatHesabiInputs,
  type TemizlikMalzemesiSarfiyatHesabiInputs,
} from "@/lib/premium-schema/calculators/temizlik-malzemesi-sarfiyat-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TemizlikMalzemesiSarfiyatHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TemizlikMalzemesiSarfiyatHesabiInputs] as number;
}

function runFormulaPipeline(inputs: TemizlikMalzemesiSarfiyatHesabiInputs): Record<string, number> {
...
export function calculateTemizlikMalzemesiSarfiyatHesabi(inputs: TemizlikMalzemesiSarfiyatHesabiInputs): {
