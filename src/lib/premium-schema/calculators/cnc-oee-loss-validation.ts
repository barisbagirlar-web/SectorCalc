...
export type CncOeeLossValidationResult =
  | { ok: true; errors: []; warnings: string[] }
  | { ok: false; errors: string[]; warnings: string[] };

export const CNC_OEE_LOSS_INPUT_KEYS: readonly (keyof CncOeeLossInputs)[] = [
  "machineRate",
  "plannedHours",
  "downtimeHours",
  "materialCost",
...
const INPUT_LABELS: Record<keyof CncOeeLossInputs, string> = {
...
function collectInputErrors(inputs: CncOeeLossInputs): string[] {
  const errors: string[] = [];

  for (const key of CNC_OEE_LOSS_INPUT_KEYS) {
...
function collectWarnings(inputs: CncOeeLossInputs): string[] {
...
export function validateCncOeeLossInputs(inputs: CncOeeLossInputs): CncOeeLossValidationResult {
