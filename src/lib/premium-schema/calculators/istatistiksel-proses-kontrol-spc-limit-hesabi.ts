...
import {
  validateIstatistikselProsesKontrolSpcLimitHesabiInputs,
  type IstatistikselProsesKontrolSpcLimitHesabiInputs,
} from "@/lib/premium-schema/calculators/istatistiksel-proses-kontrol-spc-limit-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IstatistikselProsesKontrolSpcLimitHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IstatistikselProsesKontrolSpcLimitHesabiInputs] as number;
}

function runFormulaPipeline(inputs: IstatistikselProsesKontrolSpcLimitHesabiInputs): Record<string, number> {
...
export function calculateIstatistikselProsesKontrolSpcLimitHesabi(inputs: IstatistikselProsesKontrolSpcLimitHesabiInputs): {
