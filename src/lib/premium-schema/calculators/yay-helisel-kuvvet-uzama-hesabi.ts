...
import {
  validateYayHeliselKuvvetUzamaHesabiInputs,
  type YayHeliselKuvvetUzamaHesabiInputs,
} from "@/lib/premium-schema/calculators/yay-helisel-kuvvet-uzama-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: YayHeliselKuvvetUzamaHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof YayHeliselKuvvetUzamaHesabiInputs] as number;
}

function runFormulaPipeline(inputs: YayHeliselKuvvetUzamaHesabiInputs): Record<string, number> {
...
export function calculateYayHeliselKuvvetUzamaHesabi(inputs: YayHeliselKuvvetUzamaHesabiInputs): {
