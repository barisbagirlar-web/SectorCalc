...
import {
  validateScrapRateCalculatorInputs,
  type ScrapRateCalculatorInputs,
} from "@/lib/premium-schema/calculators/scrap-rate-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ScrapRateCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ScrapRateCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ScrapRateCalculatorInputs): Record<string, number> {
...
export function calculateScrapRateCalculator(inputs: ScrapRateCalculatorInputs): {
