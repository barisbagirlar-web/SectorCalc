...
import {
  validateAqlKabulOrneklemesiRiskVeMaliyetCalculatorInputs,
  type AqlKabulOrneklemesiRiskVeMaliyetCalculatorInputs,
} from "@/lib/premium-schema/calculators/aql-kabul-orneklemesi-risk-ve-maliyet-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AqlKabulOrneklemesiRiskVeMaliyetCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AqlKabulOrneklemesiRiskVeMaliyetCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: AqlKabulOrneklemesiRiskVeMaliyetCalculatorInputs): Record<string, number> {
...
export function calculateAqlKabulOrneklemesiRiskVeMaliyetCalculator(inputs: AqlKabulOrneklemesiRiskVeMaliyetCalculatorInputs): {
