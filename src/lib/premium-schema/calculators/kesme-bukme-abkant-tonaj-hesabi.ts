...
import {
  validateKesmeBukmeAbkantTonajHesabiInputs,
  type KesmeBukmeAbkantTonajHesabiInputs,
} from "@/lib/premium-schema/calculators/kesme-bukme-abkant-tonaj-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KesmeBukmeAbkantTonajHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KesmeBukmeAbkantTonajHesabiInputs] as number;
}

function runFormulaPipeline(inputs: KesmeBukmeAbkantTonajHesabiInputs): Record<string, number> {
...
export function calculateKesmeBukmeAbkantTonajHesabi(inputs: KesmeBukmeAbkantTonajHesabiInputs): {
