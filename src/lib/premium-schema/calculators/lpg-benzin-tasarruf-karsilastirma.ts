...
import {
  validateLpgBenzinTasarrufKarsilastirmaInputs,
  type LpgBenzinTasarrufKarsilastirmaInputs,
} from "@/lib/premium-schema/calculators/lpg-benzin-tasarruf-karsilastirma-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: LpgBenzinTasarrufKarsilastirmaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof LpgBenzinTasarrufKarsilastirmaInputs] as number;
}

function runFormulaPipeline(inputs: LpgBenzinTasarrufKarsilastirmaInputs): Record<string, number> {
...
export function calculateLpgBenzinTasarrufKarsilastirma(inputs: LpgBenzinTasarrufKarsilastirmaInputs): {
