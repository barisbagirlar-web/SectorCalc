...
import {
  validateVakumSistemiKacakVeEnerjiKayipCalculatorInputs,
  type VakumSistemiKacakVeEnerjiKayipCalculatorInputs,
} from "@/lib/premium-schema/calculators/vakum-sistemi-kacak-ve-enerji-kayip-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: VakumSistemiKacakVeEnerjiKayipCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof VakumSistemiKacakVeEnerjiKayipCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: VakumSistemiKacakVeEnerjiKayipCalculatorInputs): Record<string, number> {
...
export function calculateVakumSistemiKacakVeEnerjiKayipCalculator(inputs: VakumSistemiKacakVeEnerjiKayipCalculatorInputs): {
