...
import {
  validateNpuNpiProfilAgirlikHesaplamaInputs,
  type NpuNpiProfilAgirlikHesaplamaInputs,
} from "@/lib/premium-schema/calculators/npu-npi-profil-agirlik-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: NpuNpiProfilAgirlikHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof NpuNpiProfilAgirlikHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: NpuNpiProfilAgirlikHesaplamaInputs): Record<string, number> {
...
export function calculateNpuNpiProfilAgirlikHesaplama(inputs: NpuNpiProfilAgirlikHesaplamaInputs): {
