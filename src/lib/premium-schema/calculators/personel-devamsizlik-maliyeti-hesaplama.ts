...
import {
  validatePersonelDevamsizlikMaliyetiHesaplamaInputs,
  type PersonelDevamsizlikMaliyetiHesaplamaInputs,
} from "@/lib/premium-schema/calculators/personel-devamsizlik-maliyeti-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PersonelDevamsizlikMaliyetiHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PersonelDevamsizlikMaliyetiHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: PersonelDevamsizlikMaliyetiHesaplamaInputs): Record<string, number> {
...
export function calculatePersonelDevamsizlikMaliyetiHesaplama(inputs: PersonelDevamsizlikMaliyetiHesaplamaInputs): {
