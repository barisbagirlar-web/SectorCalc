...
import {
  validateIrrigationCostCheckInputs,
  type IrrigationCostCheckInputs,
} from "@/lib/premium-schema/calculators/irrigation-cost-check-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IrrigationCostCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IrrigationCostCheckInputs] as number;
}

function runFormulaPipeline(inputs: IrrigationCostCheckInputs): Record<string, number> {
...
export function calculateIrrigationCostCheck(inputs: IrrigationCostCheckInputs): {
