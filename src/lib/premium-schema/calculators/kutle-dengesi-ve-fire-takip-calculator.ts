...
import {
  validateKutleDengesiVeFireTakipCalculatorInputs,
  type KutleDengesiVeFireTakipCalculatorInputs,
} from "@/lib/premium-schema/calculators/kutle-dengesi-ve-fire-takip-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KutleDengesiVeFireTakipCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KutleDengesiVeFireTakipCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KutleDengesiVeFireTakipCalculatorInputs): Record<string, number> {
...
export function calculateKutleDengesiVeFireTakipCalculator(inputs: KutleDengesiVeFireTakipCalculatorInputs): {
