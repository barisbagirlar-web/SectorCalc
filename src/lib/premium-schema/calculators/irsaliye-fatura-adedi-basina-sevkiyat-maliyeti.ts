...
import {
  validateIrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs,
  type IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs,
} from "@/lib/premium-schema/calculators/irsaliye-fatura-adedi-basina-sevkiyat-maliyeti-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs] as number;
}

function runFormulaPipeline(inputs: IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs): Record<string, number> {
...
export function calculateIrsaliyeFaturaAdediBasinaSevkiyatMaliyeti(inputs: IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs): {
