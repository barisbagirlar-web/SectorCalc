...
import {
  validateCelikCatiMakasYaklasikAgirligiInputs,
  type CelikCatiMakasYaklasikAgirligiInputs,
} from "@/lib/premium-schema/calculators/celik-cati-makas-yaklasik-agirligi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CelikCatiMakasYaklasikAgirligiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CelikCatiMakasYaklasikAgirligiInputs] as number;
}

function runFormulaPipeline(inputs: CelikCatiMakasYaklasikAgirligiInputs): Record<string, number> {
...
export function calculateCelikCatiMakasYaklasikAgirligi(inputs: CelikCatiMakasYaklasikAgirligiInputs): {
