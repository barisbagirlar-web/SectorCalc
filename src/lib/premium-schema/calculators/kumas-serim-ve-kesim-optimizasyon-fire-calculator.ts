...
import {
  validateKumasSerimVeKesimOptimizasyonFireCalculatorInputs,
  type KumasSerimVeKesimOptimizasyonFireCalculatorInputs,
} from "@/lib/premium-schema/calculators/kumas-serim-ve-kesim-optimizasyon-fire-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KumasSerimVeKesimOptimizasyonFireCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KumasSerimVeKesimOptimizasyonFireCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KumasSerimVeKesimOptimizasyonFireCalculatorInputs): Record<string, number> {
...
export function calculateKumasSerimVeKesimOptimizasyonFireCalculator(inputs: KumasSerimVeKesimOptimizasyonFireCalculatorInputs): {
