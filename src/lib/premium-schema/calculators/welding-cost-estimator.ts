...
import {
  validateWeldingCostEstimatorInputs,
  type WeldingCostEstimatorInputs,
} from "@/lib/premium-schema/calculators/welding-cost-estimator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: WeldingCostEstimatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof WeldingCostEstimatorInputs] as number;
}

function runFormulaPipeline(inputs: WeldingCostEstimatorInputs): Record<string, number> {
...
export function calculateWeldingCostEstimator(inputs: WeldingCostEstimatorInputs): {
