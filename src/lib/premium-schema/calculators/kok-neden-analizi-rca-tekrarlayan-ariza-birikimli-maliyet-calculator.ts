...
import {
  validateKokNedenAnaliziRcaTekrarlayanArizaBirikimliMaliyetCalculatorInputs,
  type KokNedenAnaliziRcaTekrarlayanArizaBirikimliMaliyetCalculatorInputs,
} from "@/lib/premium-schema/calculators/kok-neden-analizi-rca-tekrarlayan-ariza-birikimli-maliyet-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KokNedenAnaliziRcaTekrarlayanArizaBirikimliMaliyetCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KokNedenAnaliziRcaTekrarlayanArizaBirikimliMaliyetCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KokNedenAnaliziRcaTekrarlayanArizaBirikimliMaliyetCalculatorInputs): Record<string, number> {
...
export function calculateKokNedenAnaliziRcaTekrarlayanArizaBirikimliMaliyetCalculator(inputs: KokNedenAnaliziRcaTekrarlayanArizaBirikimliMaliyetCalculatorInputs): {
