...
import {
  validateTextileFabricWasteRiskInputs,
  type TextileFabricWasteRiskInputs,
} from "@/lib/premium-schema/calculators/textile-fabric-waste-risk-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TextileFabricWasteRiskInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TextileFabricWasteRiskInputs] as number;
}

function runFormulaPipeline(inputs: TextileFabricWasteRiskInputs): Record<string, number> {
...
export function calculateTextileFabricWasteRisk(inputs: TextileFabricWasteRiskInputs): {
