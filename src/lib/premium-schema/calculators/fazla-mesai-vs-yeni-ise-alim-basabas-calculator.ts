...
import {
  validateFazlaMesaiVsYeniIseAlimBasabasCalculatorInputs,
  type FazlaMesaiVsYeniIseAlimBasabasCalculatorInputs,
} from "@/lib/premium-schema/calculators/fazla-mesai-vs-yeni-ise-alim-basabas-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FazlaMesaiVsYeniIseAlimBasabasCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FazlaMesaiVsYeniIseAlimBasabasCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FazlaMesaiVsYeniIseAlimBasabasCalculatorInputs): Record<string, number> {
...
export function calculateFazlaMesaiVsYeniIseAlimBasabasCalculator(inputs: FazlaMesaiVsYeniIseAlimBasabasCalculatorInputs): {
