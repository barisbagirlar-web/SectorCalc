...
import {
  validateMilAksCapiHesabiEgilmeBurulmaInputs,
  type MilAksCapiHesabiEgilmeBurulmaInputs,
} from "@/lib/premium-schema/calculators/mil-aks-capi-hesabi-egilme-burulma-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: MilAksCapiHesabiEgilmeBurulmaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof MilAksCapiHesabiEgilmeBurulmaInputs] as number;
}

function runFormulaPipeline(inputs: MilAksCapiHesabiEgilmeBurulmaInputs): Record<string, number> {
...
export function calculateMilAksCapiHesabiEgilmeBurulma(inputs: MilAksCapiHesabiEgilmeBurulmaInputs): {
