...
import {
  validateSquareFootageCalculatorInputs,
  type SquareFootageCalculatorInputs,
} from "@/lib/premium-schema/calculators/square-footage-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SquareFootageCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SquareFootageCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: SquareFootageCalculatorInputs): Record<string, number> {
...
export function calculateSquareFootageCalculator(inputs: SquareFootageCalculatorInputs): {
