...
import {
  validateExcavationVolumeCalculatorInputs,
  type ExcavationVolumeCalculatorInputs,
} from "@/lib/premium-schema/calculators/excavation-volume-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ExcavationVolumeCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ExcavationVolumeCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ExcavationVolumeCalculatorInputs): Record<string, number> {
...
export function calculateExcavationVolumeCalculator(inputs: ExcavationVolumeCalculatorInputs): {
