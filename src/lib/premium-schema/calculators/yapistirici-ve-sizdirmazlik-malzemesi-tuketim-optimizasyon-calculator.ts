...
import {
  validateYapistiriciVeSizdirmazlikMalzemesiTuketimOptimizasyonCalculatorInputs,
  type YapistiriciVeSizdirmazlikMalzemesiTuketimOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/yapistirici-ve-sizdirmazlik-malzemesi-tuketim-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: YapistiriciVeSizdirmazlikMalzemesiTuketimOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof YapistiriciVeSizdirmazlikMalzemesiTuketimOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: YapistiriciVeSizdirmazlikMalzemesiTuketimOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateYapistiriciVeSizdirmazlikMalzemesiTuketimOptimizasyonCalculator(inputs: YapistiriciVeSizdirmazlikMalzemesiTuketimOptimizasyonCalculatorInputs): {
