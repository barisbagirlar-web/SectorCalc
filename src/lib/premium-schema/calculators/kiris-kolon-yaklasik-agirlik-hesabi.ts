...
import {
  validateKirisKolonYaklasikAgirlikHesabiInputs,
  type KirisKolonYaklasikAgirlikHesabiInputs,
} from "@/lib/premium-schema/calculators/kiris-kolon-yaklasik-agirlik-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KirisKolonYaklasikAgirlikHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KirisKolonYaklasikAgirlikHesabiInputs] as number;
}

function runFormulaPipeline(inputs: KirisKolonYaklasikAgirlikHesabiInputs): Record<string, number> {
...
export function calculateKirisKolonYaklasikAgirlikHesabi(inputs: KirisKolonYaklasikAgirlikHesabiInputs): {
