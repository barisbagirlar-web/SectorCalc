...
import {
  validateIsitmaSogutmaYukuKcalKwHesabiInputs,
  type IsitmaSogutmaYukuKcalKwHesabiInputs,
} from "@/lib/premium-schema/calculators/isitma-sogutma-yuku-kcal-kw-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IsitmaSogutmaYukuKcalKwHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IsitmaSogutmaYukuKcalKwHesabiInputs] as number;
}

function runFormulaPipeline(inputs: IsitmaSogutmaYukuKcalKwHesabiInputs): Record<string, number> {
...
export function calculateIsitmaSogutmaYukuKcalKwHesabi(inputs: IsitmaSogutmaYukuKcalKwHesabiInputs): {
