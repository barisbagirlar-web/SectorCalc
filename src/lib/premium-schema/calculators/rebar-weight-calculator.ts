...
import {
  validateRebarWeightCalculatorInputs,
  type RebarWeightCalculatorInputs,
} from "@/lib/premium-schema/calculators/rebar-weight-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: RebarWeightCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof RebarWeightCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: RebarWeightCalculatorInputs): Record<string, number> {
...
export function calculateRebarWeightCalculator(inputs: RebarWeightCalculatorInputs): {
