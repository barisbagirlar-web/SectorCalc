...
import {
  validateCitKorkulukMalzemeHesabiInputs,
  type CitKorkulukMalzemeHesabiInputs,
} from "@/lib/premium-schema/calculators/cit-korkuluk-malzeme-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CitKorkulukMalzemeHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CitKorkulukMalzemeHesabiInputs] as number;
}

function runFormulaPipeline(inputs: CitKorkulukMalzemeHesabiInputs): Record<string, number> {
...
export function calculateCitKorkulukMalzemeHesabi(inputs: CitKorkulukMalzemeHesabiInputs): {
