...
import {
  validateYatakRulmanOmurHesabiL10Inputs,
  type YatakRulmanOmurHesabiL10Inputs,
} from "@/lib/premium-schema/calculators/yatak-rulman-omur-hesabi-l10-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: YatakRulmanOmurHesabiL10Inputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof YatakRulmanOmurHesabiL10Inputs] as number;
}

function runFormulaPipeline(inputs: YatakRulmanOmurHesabiL10Inputs): Record<string, number> {
...
export function calculateYatakRulmanOmurHesabiL10(inputs: YatakRulmanOmurHesabiL10Inputs): {
