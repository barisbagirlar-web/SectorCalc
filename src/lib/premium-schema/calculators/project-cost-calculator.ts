...
import {
  validateProjectCostCalculatorInputs,
  type ProjectCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/project-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ProjectCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ProjectCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ProjectCostCalculatorInputs): Record<string, number> {
...
export function calculateProjectCostCalculator(inputs: ProjectCostCalculatorInputs): {
