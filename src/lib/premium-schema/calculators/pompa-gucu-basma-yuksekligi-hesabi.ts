...
import {
  validatePompaGucuBasmaYuksekligiHesabiInputs,
  type PompaGucuBasmaYuksekligiHesabiInputs,
} from "@/lib/premium-schema/calculators/pompa-gucu-basma-yuksekligi-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PompaGucuBasmaYuksekligiHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PompaGucuBasmaYuksekligiHesabiInputs] as number;
}

function runFormulaPipeline(inputs: PompaGucuBasmaYuksekligiHesabiInputs): Record<string, number> {
...
export function calculatePompaGucuBasmaYuksekligiHesabi(inputs: PompaGucuBasmaYuksekligiHesabiInputs): {
