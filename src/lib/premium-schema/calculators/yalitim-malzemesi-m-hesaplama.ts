...
import {
  validateYalitimMalzemesiMHesaplamaInputs,
  type YalitimMalzemesiMHesaplamaInputs,
} from "@/lib/premium-schema/calculators/yalitim-malzemesi-m-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: YalitimMalzemesiMHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof YalitimMalzemesiMHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: YalitimMalzemesiMHesaplamaInputs): Record<string, number> {
...
export function calculateYalitimMalzemesiMHesaplama(inputs: YalitimMalzemesiMHesaplamaInputs): {
