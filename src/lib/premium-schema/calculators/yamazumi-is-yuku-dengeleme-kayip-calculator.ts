...
import {
  validateYamazumiIsYukuDengelemeKayipCalculatorInputs,
  type YamazumiIsYukuDengelemeKayipCalculatorInputs,
} from "@/lib/premium-schema/calculators/yamazumi-is-yuku-dengeleme-kayip-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: YamazumiIsYukuDengelemeKayipCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof YamazumiIsYukuDengelemeKayipCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: YamazumiIsYukuDengelemeKayipCalculatorInputs): Record<string, number> {
...
export function calculateYamazumiIsYukuDengelemeKayipCalculator(inputs: YamazumiIsYukuDengelemeKayipCalculatorInputs): {
