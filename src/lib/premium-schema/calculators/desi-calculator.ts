...
import {
  validateDesiCalculatorInputs,
  type DesiCalculatorInputs,
} from "@/lib/premium-schema/calculators/desi-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DesiCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DesiCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: DesiCalculatorInputs): Record<string, number> {
...
export function calculateDesiCalculator(inputs: DesiCalculatorInputs): {
