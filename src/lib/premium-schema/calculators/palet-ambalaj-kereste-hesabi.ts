...
import {
  validatePaletAmbalajKeresteHesabiInputs,
  type PaletAmbalajKeresteHesabiInputs,
} from "@/lib/premium-schema/calculators/palet-ambalaj-kereste-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PaletAmbalajKeresteHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PaletAmbalajKeresteHesabiInputs] as number;
}

function runFormulaPipeline(inputs: PaletAmbalajKeresteHesabiInputs): Record<string, number> {
...
export function calculatePaletAmbalajKeresteHesabi(inputs: PaletAmbalajKeresteHesabiInputs): {
