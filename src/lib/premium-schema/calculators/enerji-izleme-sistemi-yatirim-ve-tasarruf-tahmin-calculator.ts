...
import {
  validateEnerjiIzlemeSistemiYatirimVeTasarrufTahminCalculatorInputs,
  type EnerjiIzlemeSistemiYatirimVeTasarrufTahminCalculatorInputs,
} from "@/lib/premium-schema/calculators/enerji-izleme-sistemi-yatirim-ve-tasarruf-tahmin-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: EnerjiIzlemeSistemiYatirimVeTasarrufTahminCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof EnerjiIzlemeSistemiYatirimVeTasarrufTahminCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: EnerjiIzlemeSistemiYatirimVeTasarrufTahminCalculatorInputs): Record<string, number> {
...
export function calculateEnerjiIzlemeSistemiYatirimVeTasarrufTahminCalculator(inputs: EnerjiIzlemeSistemiYatirimVeTasarrufTahminCalculatorInputs): {
