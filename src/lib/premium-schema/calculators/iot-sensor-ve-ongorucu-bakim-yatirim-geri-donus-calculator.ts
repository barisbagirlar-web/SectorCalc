...
import {
  validateIotSensorVeOngorucuBakimYatirimGeriDonusCalculatorInputs,
  type IotSensorVeOngorucuBakimYatirimGeriDonusCalculatorInputs,
} from "@/lib/premium-schema/calculators/iot-sensor-ve-ongorucu-bakim-yatirim-geri-donus-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IotSensorVeOngorucuBakimYatirimGeriDonusCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IotSensorVeOngorucuBakimYatirimGeriDonusCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: IotSensorVeOngorucuBakimYatirimGeriDonusCalculatorInputs): Record<string, number> {
...
export function calculateIotSensorVeOngorucuBakimYatirimGeriDonusCalculator(inputs: IotSensorVeOngorucuBakimYatirimGeriDonusCalculatorInputs): {
