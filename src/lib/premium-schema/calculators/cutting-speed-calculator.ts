...
import {
  validateCuttingSpeedCalculatorInputs,
  type CuttingSpeedCalculatorInputs,
} from "@/lib/premium-schema/calculators/cutting-speed-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CuttingSpeedCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CuttingSpeedCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: CuttingSpeedCalculatorInputs): Record<string, number> {
...
export function calculateCuttingSpeedCalculator(inputs: CuttingSpeedCalculatorInputs): {
