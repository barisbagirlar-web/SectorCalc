...
import {
  validateKumlamaRaspaKumSarfiyatiHesabiInputs,
  type KumlamaRaspaKumSarfiyatiHesabiInputs,
} from "@/lib/premium-schema/calculators/kumlama-raspa-kum-sarfiyati-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KumlamaRaspaKumSarfiyatiHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KumlamaRaspaKumSarfiyatiHesabiInputs] as number;
}

function runFormulaPipeline(inputs: KumlamaRaspaKumSarfiyatiHesabiInputs): Record<string, number> {
...
export function calculateKumlamaRaspaKumSarfiyatiHesabi(inputs: KumlamaRaspaKumSarfiyatiHesabiInputs): {
