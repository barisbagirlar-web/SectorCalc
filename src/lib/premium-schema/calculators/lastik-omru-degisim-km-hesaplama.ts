...
import {
  validateLastikOmruDegisimKmHesaplamaInputs,
  type LastikOmruDegisimKmHesaplamaInputs,
} from "@/lib/premium-schema/calculators/lastik-omru-degisim-km-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: LastikOmruDegisimKmHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof LastikOmruDegisimKmHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: LastikOmruDegisimKmHesaplamaInputs): Record<string, number> {
...
export function calculateLastikOmruDegisimKmHesaplama(inputs: LastikOmruDegisimKmHesaplamaInputs): {
