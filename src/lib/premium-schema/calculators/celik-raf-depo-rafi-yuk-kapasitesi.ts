...
import {
  validateCelikRafDepoRafiYukKapasitesiInputs,
  type CelikRafDepoRafiYukKapasitesiInputs,
} from "@/lib/premium-schema/calculators/celik-raf-depo-rafi-yuk-kapasitesi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CelikRafDepoRafiYukKapasitesiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CelikRafDepoRafiYukKapasitesiInputs] as number;
}

function runFormulaPipeline(inputs: CelikRafDepoRafiYukKapasitesiInputs): Record<string, number> {
...
export function calculateCelikRafDepoRafiYukKapasitesi(inputs: CelikRafDepoRafiYukKapasitesiInputs): {
