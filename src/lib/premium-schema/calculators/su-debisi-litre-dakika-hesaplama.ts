...
import {
  validateSuDebisiLitreDakikaHesaplamaInputs,
  type SuDebisiLitreDakikaHesaplamaInputs,
} from "@/lib/premium-schema/calculators/su-debisi-litre-dakika-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SuDebisiLitreDakikaHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SuDebisiLitreDakikaHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: SuDebisiLitreDakikaHesaplamaInputs): Record<string, number> {
...
export function calculateSuDebisiLitreDakikaHesaplama(inputs: SuDebisiLitreDakikaHesaplamaInputs): {
