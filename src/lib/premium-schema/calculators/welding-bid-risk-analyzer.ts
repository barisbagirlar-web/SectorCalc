...
import {
  validateWeldingBidRiskAnalyzerInputs,
  type WeldingBidRiskAnalyzerInputs,
} from "@/lib/premium-schema/calculators/welding-bid-risk-analyzer-validation";
...
const FORMULA_PIPELINE = [
  {
    ...
    "inputMap": {
      "totalCost": "monthlyToolCost",
      "quantity": "partsProduced"
    },
...
const SUMMARY_WARNING_THRESHOLD = 0.5;
const SUMMARY_CRITICAL_THRESHOLD = 1.5;
const summaryDirection: "lower_is_bad" | "higher_is_bad" = "higher_is_bad";

function resolveMappedValue(
  sourceKey: string,
  userInputs: WeldingBidRiskAnalyzerInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof WeldingBidRiskAnalyzerInputs] as number;
}

function runFormulaPipeline(inputs: WeldingBidRiskAnalyzerInputs): Record<string, number> {
...
export function calculateWeldingBidRiskAnalyzer(inputs: WeldingBidRiskAnalyzerInputs): {
  ...
  return {
    ...
    toolCostPerPart: computed.toolCostPerPart,
    toolChangeDowntimeCost: computed.toolChangeDowntimeCost,
    summaryLevel,
    primaryDriver: "totalExposure",
    decisionVerdict: {
      summaryLevel,
