...
import {
  validateFuelCostCalculatorInputs,
  type FuelCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/fuel-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FuelCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FuelCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FuelCostCalculatorInputs): Record<string, number> {
...
export function calculateFuelCostCalculator(inputs: FuelCostCalculatorInputs): {
