...
import {
  validateCncOeeLossInputs,
  type CncOeeLossInputs,
} from "@/lib/premium-schema/calculators/cnc-oee-loss-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CncOeeLossInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CncOeeLossInputs] as number;
}

function runFormulaPipeline(inputs: CncOeeLossInputs): Record<string, number> {
...
export function calculateCncOeeLoss(inputs: CncOeeLossInputs): {
  oeeScore: number;
