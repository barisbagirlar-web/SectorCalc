...
import {
  validateVidaSomunAdimDisUstuCapHesabiInputs,
  type VidaSomunAdimDisUstuCapHesabiInputs,
} from "@/lib/premium-schema/calculators/vida-somun-adim-dis-ustu-cap-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: VidaSomunAdimDisUstuCapHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof VidaSomunAdimDisUstuCapHesabiInputs] as number;
}

function runFormulaPipeline(inputs: VidaSomunAdimDisUstuCapHesabiInputs): Record<string, number> {
...
export function calculateVidaSomunAdimDisUstuCapHesabi(inputs: VidaSomunAdimDisUstuCapHesabiInputs): {
