...
import {
  validateBreakEvenCalculatorInputs,
  type BreakEvenCalculatorInputs,
} from "@/lib/premium-schema/calculators/break-even-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BreakEvenCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BreakEvenCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: BreakEvenCalculatorInputs): Record<string, number> {
...
export function calculateBreakEvenCalculator(inputs: BreakEvenCalculatorInputs): {
