...
import {
  validateTripBudgetOptimizerInputs,
  type TripBudgetOptimizerInputs,
} from "@/lib/premium-schema/calculators/trip-budget-optimizer-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TripBudgetOptimizerInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TripBudgetOptimizerInputs] as number;
}

function runFormulaPipeline(inputs: TripBudgetOptimizerInputs): Record<string, number> {
...
export function calculateTripBudgetOptimizer(inputs: TripBudgetOptimizerInputs): {
