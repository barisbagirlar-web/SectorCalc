...
import {
  validatePokaYokeHataOnlemeYatirimGeriDonusCalculatorInputs,
  type PokaYokeHataOnlemeYatirimGeriDonusCalculatorInputs,
} from "@/lib/premium-schema/calculators/poka-yoke-hata-onleme-yatirim-geri-donus-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PokaYokeHataOnlemeYatirimGeriDonusCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PokaYokeHataOnlemeYatirimGeriDonusCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: PokaYokeHataOnlemeYatirimGeriDonusCalculatorInputs): Record<string, number> {
...
export function calculatePokaYokeHataOnlemeYatirimGeriDonusCalculator(inputs: PokaYokeHataOnlemeYatirimGeriDonusCalculatorInputs): {
