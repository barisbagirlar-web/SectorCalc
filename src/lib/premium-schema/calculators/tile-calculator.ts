...
import {
  validateTileCalculatorInputs,
  type TileCalculatorInputs,
} from "@/lib/premium-schema/calculators/tile-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TileCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TileCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: TileCalculatorInputs): Record<string, number> {
...
export function calculateTileCalculator(inputs: TileCalculatorInputs): {
