...
import {
  validateRouteCostCalculatorInputs,
  type RouteCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/route-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: RouteCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof RouteCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: RouteCostCalculatorInputs): Record<string, number> {
...
export function calculateRouteCostCalculator(inputs: RouteCostCalculatorInputs): {
