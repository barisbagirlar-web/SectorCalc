...
import {
  validateLaboratuvarAnalizMaliyetiVeNumuneAlmaOptimizasyonCalculatorInputs,
  type LaboratuvarAnalizMaliyetiVeNumuneAlmaOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/laboratuvar-analiz-maliyeti-ve-numune-alma-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: LaboratuvarAnalizMaliyetiVeNumuneAlmaOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof LaboratuvarAnalizMaliyetiVeNumuneAlmaOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: LaboratuvarAnalizMaliyetiVeNumuneAlmaOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateLaboratuvarAnalizMaliyetiVeNumuneAlmaOptimizasyonCalculator(inputs: LaboratuvarAnalizMaliyetiVeNumuneAlmaOptimizasyonCalculatorInputs): {
