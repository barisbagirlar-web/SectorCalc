...
import {
  validateFuelTravelCalculatorInputs,
  type FuelTravelCalculatorInputs,
} from "@/lib/premium-schema/calculators/fuel-travel-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FuelTravelCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FuelTravelCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FuelTravelCalculatorInputs): Record<string, number> {
...
export function calculateFuelTravelCalculator(inputs: FuelTravelCalculatorInputs): {
