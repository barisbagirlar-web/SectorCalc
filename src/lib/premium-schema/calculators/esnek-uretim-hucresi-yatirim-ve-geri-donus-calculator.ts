...
import {
  validateEsnekUretimHucresiYatirimVeGeriDonusCalculatorInputs,
  type EsnekUretimHucresiYatirimVeGeriDonusCalculatorInputs,
} from "@/lib/premium-schema/calculators/esnek-uretim-hucresi-yatirim-ve-geri-donus-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: EsnekUretimHucresiYatirimVeGeriDonusCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof EsnekUretimHucresiYatirimVeGeriDonusCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: EsnekUretimHucresiYatirimVeGeriDonusCalculatorInputs): Record<string, number> {
...
export function calculateEsnekUretimHucresiYatirimVeGeriDonusCalculator(inputs: EsnekUretimHucresiYatirimVeGeriDonusCalculatorInputs): {
