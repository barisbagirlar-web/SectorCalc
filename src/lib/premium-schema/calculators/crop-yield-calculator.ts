...
import {
  validateCropYieldCalculatorInputs,
  type CropYieldCalculatorInputs,
} from "@/lib/premium-schema/calculators/crop-yield-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CropYieldCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CropYieldCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: CropYieldCalculatorInputs): Record<string, number> {
...
export function calculateCropYieldCalculator(inputs: CropYieldCalculatorInputs): {
