...
import {
  validateOgrenmeEgrisiVePartiSureTahminCalculatorInputs,
  type OgrenmeEgrisiVePartiSureTahminCalculatorInputs,
} from "@/lib/premium-schema/calculators/ogrenme-egrisi-ve-parti-sure-tahmin-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: OgrenmeEgrisiVePartiSureTahminCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof OgrenmeEgrisiVePartiSureTahminCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: OgrenmeEgrisiVePartiSureTahminCalculatorInputs): Record<string, number> {
...
export function calculateOgrenmeEgrisiVePartiSureTahminCalculator(inputs: OgrenmeEgrisiVePartiSureTahminCalculatorInputs): {
