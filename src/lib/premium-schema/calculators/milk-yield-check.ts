...
import {
  validateMilkYieldCheckInputs,
  type MilkYieldCheckInputs,
} from "@/lib/premium-schema/calculators/milk-yield-check-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: MilkYieldCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof MilkYieldCheckInputs] as number;
}

function runFormulaPipeline(inputs: MilkYieldCheckInputs): Record<string, number> {
...
export function calculateMilkYieldCheck(inputs: MilkYieldCheckInputs): {
