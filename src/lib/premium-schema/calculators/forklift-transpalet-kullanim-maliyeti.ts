...
import {
  validateForkliftTranspaletKullanimMaliyetiInputs,
  type ForkliftTranspaletKullanimMaliyetiInputs,
} from "@/lib/premium-schema/calculators/forklift-transpalet-kullanim-maliyeti-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ForkliftTranspaletKullanimMaliyetiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ForkliftTranspaletKullanimMaliyetiInputs] as number;
}

function runFormulaPipeline(inputs: ForkliftTranspaletKullanimMaliyetiInputs): Record<string, number> {
...
export function calculateForkliftTranspaletKullanimMaliyeti(inputs: ForkliftTranspaletKullanimMaliyetiInputs): {
