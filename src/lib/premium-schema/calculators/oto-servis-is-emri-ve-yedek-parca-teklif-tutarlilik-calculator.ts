...
import {
  validateOtoServisIsEmriVeYedekParcaTeklifTutarlilikCalculatorInputs,
  type OtoServisIsEmriVeYedekParcaTeklifTutarlilikCalculatorInputs,
} from "@/lib/premium-schema/calculators/oto-servis-is-emri-ve-yedek-parca-teklif-tutarlilik-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: OtoServisIsEmriVeYedekParcaTeklifTutarlilikCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof OtoServisIsEmriVeYedekParcaTeklifTutarlilikCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: OtoServisIsEmriVeYedekParcaTeklifTutarlilikCalculatorInputs): Record<string, number> {
...
export function calculateOtoServisIsEmriVeYedekParcaTeklifTutarlilikCalculator(inputs: OtoServisIsEmriVeYedekParcaTeklifTutarlilikCalculatorInputs): {
