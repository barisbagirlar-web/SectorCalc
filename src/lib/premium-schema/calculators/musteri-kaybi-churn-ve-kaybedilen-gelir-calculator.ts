...
import {
  validateMusteriKaybiChurnVeKaybedilenGelirCalculatorInputs,
  type MusteriKaybiChurnVeKaybedilenGelirCalculatorInputs,
} from "@/lib/premium-schema/calculators/musteri-kaybi-churn-ve-kaybedilen-gelir-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: MusteriKaybiChurnVeKaybedilenGelirCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof MusteriKaybiChurnVeKaybedilenGelirCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: MusteriKaybiChurnVeKaybedilenGelirCalculatorInputs): Record<string, number> {
...
export function calculateMusteriKaybiChurnVeKaybedilenGelirCalculator(inputs: MusteriKaybiChurnVeKaybedilenGelirCalculatorInputs): {
