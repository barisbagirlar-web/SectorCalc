...
import {
  validateBoyaKaplamaSarfiyatiMBasinaHesabiInputs,
  type BoyaKaplamaSarfiyatiMBasinaHesabiInputs,
} from "@/lib/premium-schema/calculators/boya-kaplama-sarfiyati-m-basina-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BoyaKaplamaSarfiyatiMBasinaHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BoyaKaplamaSarfiyatiMBasinaHesabiInputs] as number;
}

function runFormulaPipeline(inputs: BoyaKaplamaSarfiyatiMBasinaHesabiInputs): Record<string, number> {
...
export function calculateBoyaKaplamaSarfiyatiMBasinaHesabi(inputs: BoyaKaplamaSarfiyatiMBasinaHesabiInputs): {
