...
import {
  validateTalepTahminHatasiVeStokStokYokluguMaliyetCalculatorInputs,
  type TalepTahminHatasiVeStokStokYokluguMaliyetCalculatorInputs,
} from "@/lib/premium-schema/calculators/talep-tahmin-hatasi-ve-stok-stok-yoklugu-maliyet-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TalepTahminHatasiVeStokStokYokluguMaliyetCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TalepTahminHatasiVeStokStokYokluguMaliyetCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: TalepTahminHatasiVeStokStokYokluguMaliyetCalculatorInputs): Record<string, number> {
...
export function calculateTalepTahminHatasiVeStokStokYokluguMaliyetCalculator(inputs: TalepTahminHatasiVeStokStokYokluguMaliyetCalculatorInputs): {
