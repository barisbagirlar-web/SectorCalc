...
import {
  validateSquareMeterCalculatorInputs,
  type SquareMeterCalculatorInputs,
} from "@/lib/premium-schema/calculators/square-meter-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SquareMeterCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SquareMeterCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: SquareMeterCalculatorInputs): Record<string, number> {
...
export function calculateSquareMeterCalculator(inputs: SquareMeterCalculatorInputs): {
