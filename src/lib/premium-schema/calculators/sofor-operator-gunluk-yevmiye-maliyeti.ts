...
import {
  validateSoforOperatorGunlukYevmiyeMaliyetiInputs,
  type SoforOperatorGunlukYevmiyeMaliyetiInputs,
} from "@/lib/premium-schema/calculators/sofor-operator-gunluk-yevmiye-maliyeti-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SoforOperatorGunlukYevmiyeMaliyetiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SoforOperatorGunlukYevmiyeMaliyetiInputs] as number;
}

function runFormulaPipeline(inputs: SoforOperatorGunlukYevmiyeMaliyetiInputs): Record<string, number> {
...
export function calculateSoforOperatorGunlukYevmiyeMaliyeti(inputs: SoforOperatorGunlukYevmiyeMaliyetiInputs): {
