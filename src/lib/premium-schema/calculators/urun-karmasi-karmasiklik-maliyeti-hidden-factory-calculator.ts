...
import {
  validateUrunKarmasiKarmasiklikMaliyetiHiddenFactoryCalculatorInputs,
  type UrunKarmasiKarmasiklikMaliyetiHiddenFactoryCalculatorInputs,
} from "@/lib/premium-schema/calculators/urun-karmasi-karmasiklik-maliyeti-hidden-factory-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: UrunKarmasiKarmasiklikMaliyetiHiddenFactoryCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof UrunKarmasiKarmasiklikMaliyetiHiddenFactoryCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: UrunKarmasiKarmasiklikMaliyetiHiddenFactoryCalculatorInputs): Record<string, number> {
...
export function calculateUrunKarmasiKarmasiklikMaliyetiHiddenFactoryCalculator(inputs: UrunKarmasiKarmasiklikMaliyetiHiddenFactoryCalculatorInputs): {
