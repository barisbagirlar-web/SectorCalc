...
import {
  validateDepoRafPaletYerlesimOptimizasyonuInputs,
  type DepoRafPaletYerlesimOptimizasyonuInputs,
} from "@/lib/premium-schema/calculators/depo-raf-palet-yerlesim-optimizasyonu-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DepoRafPaletYerlesimOptimizasyonuInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DepoRafPaletYerlesimOptimizasyonuInputs] as number;
}

function runFormulaPipeline(inputs: DepoRafPaletYerlesimOptimizasyonuInputs): Record<string, number> {
...
export function calculateDepoRafPaletYerlesimOptimizasyonu(inputs: DepoRafPaletYerlesimOptimizasyonuInputs): {
