export type IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs = {
  distanceKm: number;
  tripCount: number;
...
export type IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiValidationResult =
  | { ok: true; errors: []; warnings: string[] }
  | { ok: false; errors: string[]; warnings: string[] };

export const IRSALIYE_FATURA_ADEDI_BASINA_SEVKIYAT_MALIYETI_INPUT_KEYS: readonly (keyof IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs)[] = [
  "distanceKm",
  "tripCount",
  "vehicleLoadCapacity",
  "loadFactorPercent",
...
const INPUT_LABELS: Record<keyof IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs, string> = {
...
function collectInputErrors(inputs: IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs): string[] {
  const errors: string[] = [];

  for (const key of IRSALIYE_FATURA_ADEDI_BASINA_SEVKIYAT_MALIYETI_INPUT_KEYS) {
  ...
  if (inputs.vehicleLoadCapacity < 0 || inputs.vehicleLoadCapacity > 50000) {
    errors.push("vehicleLoadCapacity must be between 0 and 50000.");
  }
...
function collectWarnings(inputs: IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs): string[] {
...
export function validateIrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs(inputs: IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiInputs): IrsaliyeFaturaAdediBasinaSevkiyatMaliyetiValidationResult {
