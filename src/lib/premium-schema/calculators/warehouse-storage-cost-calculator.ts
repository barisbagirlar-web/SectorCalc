...
import {
  validateWarehouseStorageCostCalculatorInputs,
  type WarehouseStorageCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/warehouse-storage-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: WarehouseStorageCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof WarehouseStorageCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: WarehouseStorageCostCalculatorInputs): Record<string, number> {
...
export function calculateWarehouseStorageCostCalculator(inputs: WarehouseStorageCostCalculatorInputs): {
