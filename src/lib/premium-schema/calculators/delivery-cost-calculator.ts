...
import {
  validateDeliveryCostCalculatorInputs,
  type DeliveryCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/delivery-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DeliveryCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DeliveryCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: DeliveryCostCalculatorInputs): Record<string, number> {
...
export function calculateDeliveryCostCalculator(inputs: DeliveryCostCalculatorInputs): {
