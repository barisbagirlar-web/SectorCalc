...
import {
  validateOfisKirtasiyeSarfiyatHesabiInputs,
  type OfisKirtasiyeSarfiyatHesabiInputs,
} from "@/lib/premium-schema/calculators/ofis-kirtasiye-sarfiyat-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: OfisKirtasiyeSarfiyatHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof OfisKirtasiyeSarfiyatHesabiInputs] as number;
}

function runFormulaPipeline(inputs: OfisKirtasiyeSarfiyatHesabiInputs): Record<string, number> {
...
export function calculateOfisKirtasiyeSarfiyatHesabi(inputs: OfisKirtasiyeSarfiyatHesabiInputs): {
