...
import {
  validateKesmeParametreleriTakimOmruOptimizasyonCalculatorInputs,
  type KesmeParametreleriTakimOmruOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/kesme-parametreleri-takim-omru-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KesmeParametreleriTakimOmruOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KesmeParametreleriTakimOmruOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KesmeParametreleriTakimOmruOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateKesmeParametreleriTakimOmruOptimizasyonCalculator(inputs: KesmeParametreleriTakimOmruOptimizasyonCalculatorInputs): {
