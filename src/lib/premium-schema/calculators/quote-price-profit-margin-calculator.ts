...
import {
  validateQuotePriceProfitMarginCalculatorInputs,
  type QuotePriceProfitMarginCalculatorInputs,
} from "@/lib/premium-schema/calculators/quote-price-profit-margin-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: QuotePriceProfitMarginCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof QuotePriceProfitMarginCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: QuotePriceProfitMarginCalculatorInputs): Record<string, number> {
...
export function calculateQuotePriceProfitMarginCalculator(inputs: QuotePriceProfitMarginCalculatorInputs): {
