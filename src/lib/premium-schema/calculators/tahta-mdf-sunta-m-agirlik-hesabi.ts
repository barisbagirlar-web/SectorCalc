...
import {
  validateTahtaMdfSuntaMAgirlikHesabiInputs,
  type TahtaMdfSuntaMAgirlikHesabiInputs,
} from "@/lib/premium-schema/calculators/tahta-mdf-sunta-m-agirlik-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TahtaMdfSuntaMAgirlikHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TahtaMdfSuntaMAgirlikHesabiInputs] as number;
}

function runFormulaPipeline(inputs: TahtaMdfSuntaMAgirlikHesabiInputs): Record<string, number> {
...
export function calculateTahtaMdfSuntaMAgirlikHesabi(inputs: TahtaMdfSuntaMAgirlikHesabiInputs): {
