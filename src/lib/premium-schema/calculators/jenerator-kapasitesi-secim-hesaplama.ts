...
import {
  validateJeneratorKapasitesiSecimHesaplamaInputs,
  type JeneratorKapasitesiSecimHesaplamaInputs,
} from "@/lib/premium-schema/calculators/jenerator-kapasitesi-secim-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: JeneratorKapasitesiSecimHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof JeneratorKapasitesiSecimHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: JeneratorKapasitesiSecimHesaplamaInputs): Record<string, number> {
...
export function calculateJeneratorKapasitesiSecimHesaplama(inputs: JeneratorKapasitesiSecimHesaplamaInputs): {
