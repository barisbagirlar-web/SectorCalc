...
import {
  validateHafriyatVeDolguDengesiOptimizasyonCalculatorInputs,
  type HafriyatVeDolguDengesiOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/hafriyat-ve-dolgu-dengesi-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: HafriyatVeDolguDengesiOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof HafriyatVeDolguDengesiOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: HafriyatVeDolguDengesiOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateHafriyatVeDolguDengesiOptimizasyonCalculator(inputs: HafriyatVeDolguDengesiOptimizasyonCalculatorInputs): {
