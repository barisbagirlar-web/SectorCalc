...
import {
  validateSolarPanelOutputCalculatorInputs,
  type SolarPanelOutputCalculatorInputs,
} from "@/lib/premium-schema/calculators/solar-panel-output-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SolarPanelOutputCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SolarPanelOutputCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: SolarPanelOutputCalculatorInputs): Record<string, number> {
...
export function calculateSolarPanelOutputCalculator(inputs: SolarPanelOutputCalculatorInputs): {
