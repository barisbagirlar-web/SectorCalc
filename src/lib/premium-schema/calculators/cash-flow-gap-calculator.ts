...
import {
  validateCashFlowGapCalculatorInputs,
  type CashFlowGapCalculatorInputs,
} from "@/lib/premium-schema/calculators/cash-flow-gap-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CashFlowGapCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CashFlowGapCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: CashFlowGapCalculatorInputs): Record<string, number> {
...
export function calculateCashFlowGapCalculator(inputs: CashFlowGapCalculatorInputs): {
