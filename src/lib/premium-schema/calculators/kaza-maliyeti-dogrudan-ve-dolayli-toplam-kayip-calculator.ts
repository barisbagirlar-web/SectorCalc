...
import {
  validateKazaMaliyetiDogrudanVeDolayliToplamKayipCalculatorInputs,
  type KazaMaliyetiDogrudanVeDolayliToplamKayipCalculatorInputs,
} from "@/lib/premium-schema/calculators/kaza-maliyeti-dogrudan-ve-dolayli-toplam-kayip-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KazaMaliyetiDogrudanVeDolayliToplamKayipCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KazaMaliyetiDogrudanVeDolayliToplamKayipCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KazaMaliyetiDogrudanVeDolayliToplamKayipCalculatorInputs): Record<string, number> {
...
export function calculateKazaMaliyetiDogrudanVeDolayliToplamKayipCalculator(inputs: KazaMaliyetiDogrudanVeDolayliToplamKayipCalculatorInputs): {
