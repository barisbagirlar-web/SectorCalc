...
import {
  validateEnergyEfficiencyReportInputs,
  type EnergyEfficiencyReportInputs,
} from "@/lib/premium-schema/calculators/energy-efficiency-report-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: EnergyEfficiencyReportInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof EnergyEfficiencyReportInputs] as number;
}

function runFormulaPipeline(inputs: EnergyEfficiencyReportInputs): Record<string, number> {
...
export function calculateEnergyEfficiencyReport(inputs: EnergyEfficiencyReportInputs): {
