...
import {
  validatePaketlemeMalzemesiStrecKoliSarfiyatiInputs,
  type PaketlemeMalzemesiStrecKoliSarfiyatiInputs,
} from "@/lib/premium-schema/calculators/paketleme-malzemesi-strec-koli-sarfiyati-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PaketlemeMalzemesiStrecKoliSarfiyatiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PaketlemeMalzemesiStrecKoliSarfiyatiInputs] as number;
}

function runFormulaPipeline(inputs: PaketlemeMalzemesiStrecKoliSarfiyatiInputs): Record<string, number> {
...
export function calculatePaketlemeMalzemesiStrecKoliSarfiyati(inputs: PaketlemeMalzemesiStrecKoliSarfiyatiInputs): {
