...
import {
  validateCabinetCostEstimatorInputs,
  type CabinetCostEstimatorInputs,
} from "@/lib/premium-schema/calculators/cabinet-cost-estimator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CabinetCostEstimatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CabinetCostEstimatorInputs] as number;
}

function runFormulaPipeline(inputs: CabinetCostEstimatorInputs): Record<string, number> {
...
export function calculateCabinetCostEstimator(inputs: CabinetCostEstimatorInputs): {
