...
import {
  validateRenovationBudgetOptimizerInputs,
  type RenovationBudgetOptimizerInputs,
} from "@/lib/premium-schema/calculators/renovation-budget-optimizer-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: RenovationBudgetOptimizerInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof RenovationBudgetOptimizerInputs] as number;
}

function runFormulaPipeline(inputs: RenovationBudgetOptimizerInputs): Record<string, number> {
...
export function calculateRenovationBudgetOptimizer(inputs: RenovationBudgetOptimizerInputs): {
