...
import {
  validateMusteriYasamBoyuDegerClvVeEdinmeMaliyetiCacCalculatorInputs,
  type MusteriYasamBoyuDegerClvVeEdinmeMaliyetiCacCalculatorInputs,
} from "@/lib/premium-schema/calculators/musteri-yasam-boyu-deger-clv-ve-edinme-maliyeti-cac-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: MusteriYasamBoyuDegerClvVeEdinmeMaliyetiCacCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof MusteriYasamBoyuDegerClvVeEdinmeMaliyetiCacCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: MusteriYasamBoyuDegerClvVeEdinmeMaliyetiCacCalculatorInputs): Record<string, number> {
...
export function calculateMusteriYasamBoyuDegerClvVeEdinmeMaliyetiCacCalculator(inputs: MusteriYasamBoyuDegerClvVeEdinmeMaliyetiCacCalculatorInputs): {
