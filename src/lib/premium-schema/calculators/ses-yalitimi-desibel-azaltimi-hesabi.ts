...
import {
  validateSesYalitimiDesibelAzaltimiHesabiInputs,
  type SesYalitimiDesibelAzaltimiHesabiInputs,
} from "@/lib/premium-schema/calculators/ses-yalitimi-desibel-azaltimi-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SesYalitimiDesibelAzaltimiHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SesYalitimiDesibelAzaltimiHesabiInputs] as number;
}

function runFormulaPipeline(inputs: SesYalitimiDesibelAzaltimiHesabiInputs): Record<string, number> {
...
export function calculateSesYalitimiDesibelAzaltimiHesabi(inputs: SesYalitimiDesibelAzaltimiHesabiInputs): {
