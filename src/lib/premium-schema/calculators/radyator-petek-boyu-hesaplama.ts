...
import {
  validateRadyatorPetekBoyuHesaplamaInputs,
  type RadyatorPetekBoyuHesaplamaInputs,
} from "@/lib/premium-schema/calculators/radyator-petek-boyu-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: RadyatorPetekBoyuHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof RadyatorPetekBoyuHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: RadyatorPetekBoyuHesaplamaInputs): Record<string, number> {
...
export function calculateRadyatorPetekBoyuHesaplama(inputs: RadyatorPetekBoyuHesaplamaInputs): {
