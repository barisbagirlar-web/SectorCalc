...
import {
  validateZamanEtuduVeStandartSureCalculatorInputs,
  type ZamanEtuduVeStandartSureCalculatorInputs,
} from "@/lib/premium-schema/calculators/zaman-etudu-ve-standart-sure-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ZamanEtuduVeStandartSureCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ZamanEtuduVeStandartSureCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ZamanEtuduVeStandartSureCalculatorInputs): Record<string, number> {
...
export function calculateZamanEtuduVeStandartSureCalculator(inputs: ZamanEtuduVeStandartSureCalculatorInputs): {
