...
import {
  validateToleranceDriftCalculatorInputs,
  type ToleranceDriftCalculatorInputs,
} from "@/lib/premium-schema/calculators/tolerance-drift-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ToleranceDriftCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ToleranceDriftCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ToleranceDriftCalculatorInputs): Record<string, number> {
...
export function calculateToleranceDriftCalculator(inputs: ToleranceDriftCalculatorInputs): {
