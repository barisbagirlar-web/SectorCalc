...
import {
  validateMtbfMttrVeKullanilabilirlikFinansalCalculatorInputs,
  type MtbfMttrVeKullanilabilirlikFinansalCalculatorInputs,
} from "@/lib/premium-schema/calculators/mtbf-mttr-ve-kullanilabilirlik-finansal-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: MtbfMttrVeKullanilabilirlikFinansalCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof MtbfMttrVeKullanilabilirlikFinansalCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: MtbfMttrVeKullanilabilirlikFinansalCalculatorInputs): Record<string, number> {
...
export function calculateMtbfMttrVeKullanilabilirlikFinansalCalculator(inputs: MtbfMttrVeKullanilabilirlikFinansalCalculatorInputs): {
