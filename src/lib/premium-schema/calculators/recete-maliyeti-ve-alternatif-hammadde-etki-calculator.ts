...
import {
  validateReceteMaliyetiVeAlternatifHammaddeEtkiCalculatorInputs,
  type ReceteMaliyetiVeAlternatifHammaddeEtkiCalculatorInputs,
} from "@/lib/premium-schema/calculators/recete-maliyeti-ve-alternatif-hammadde-etki-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ReceteMaliyetiVeAlternatifHammaddeEtkiCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ReceteMaliyetiVeAlternatifHammaddeEtkiCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ReceteMaliyetiVeAlternatifHammaddeEtkiCalculatorInputs): Record<string, number> {
...
export function calculateReceteMaliyetiVeAlternatifHammaddeEtkiCalculator(inputs: ReceteMaliyetiVeAlternatifHammaddeEtkiCalculatorInputs): {
