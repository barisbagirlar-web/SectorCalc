...
import {
  validateAracKiraSatinAlmaKarsilastirmaInputs,
  type AracKiraSatinAlmaKarsilastirmaInputs,
} from "@/lib/premium-schema/calculators/arac-kira-satin-alma-karsilastirma-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AracKiraSatinAlmaKarsilastirmaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AracKiraSatinAlmaKarsilastirmaInputs] as number;
}

function runFormulaPipeline(inputs: AracKiraSatinAlmaKarsilastirmaInputs): Record<string, number> {
...
export function calculateAracKiraSatinAlmaKarsilastirma(inputs: AracKiraSatinAlmaKarsilastirmaInputs): {
