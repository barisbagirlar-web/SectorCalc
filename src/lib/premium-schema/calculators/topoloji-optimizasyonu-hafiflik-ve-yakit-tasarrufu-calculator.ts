...
import {
  validateTopolojiOptimizasyonuHafiflikVeYakitTasarrufuCalculatorInputs,
  type TopolojiOptimizasyonuHafiflikVeYakitTasarrufuCalculatorInputs,
} from "@/lib/premium-schema/calculators/topoloji-optimizasyonu-hafiflik-ve-yakit-tasarrufu-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TopolojiOptimizasyonuHafiflikVeYakitTasarrufuCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TopolojiOptimizasyonuHafiflikVeYakitTasarrufuCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: TopolojiOptimizasyonuHafiflikVeYakitTasarrufuCalculatorInputs): Record<string, number> {
...
export function calculateTopolojiOptimizasyonuHafiflikVeYakitTasarrufuCalculator(inputs: TopolojiOptimizasyonuHafiflikVeYakitTasarrufuCalculatorInputs): {
