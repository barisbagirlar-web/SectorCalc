...
import {
  validateCleaningCostCalculatorInputs,
  type CleaningCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/cleaning-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CleaningCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CleaningCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: CleaningCostCalculatorInputs): Record<string, number> {
...
export function calculateCleaningCostCalculator(inputs: CleaningCostCalculatorInputs): {
