...
import {
  validateDizelBenzinMaliyetKarsilastirmaInputs,
  type DizelBenzinMaliyetKarsilastirmaInputs,
} from "@/lib/premium-schema/calculators/dizel-benzin-maliyet-karsilastirma-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DizelBenzinMaliyetKarsilastirmaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DizelBenzinMaliyetKarsilastirmaInputs] as number;
}

function runFormulaPipeline(inputs: DizelBenzinMaliyetKarsilastirmaInputs): Record<string, number> {
...
export function calculateDizelBenzinMaliyetKarsilastirma(inputs: DizelBenzinMaliyetKarsilastirmaInputs): {
