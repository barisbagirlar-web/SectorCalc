...
import {
  validateCompressorEnergyCostCalculatorInputs,
  type CompressorEnergyCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/compressor-energy-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CompressorEnergyCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CompressorEnergyCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: CompressorEnergyCostCalculatorInputs): Record<string, number> {
...
export function calculateCompressorEnergyCostCalculator(inputs: CompressorEnergyCostCalculatorInputs): {
