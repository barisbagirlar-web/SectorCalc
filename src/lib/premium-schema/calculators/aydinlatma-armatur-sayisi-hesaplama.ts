...
import {
  validateAydinlatmaArmaturSayisiHesaplamaInputs,
  type AydinlatmaArmaturSayisiHesaplamaInputs,
} from "@/lib/premium-schema/calculators/aydinlatma-armatur-sayisi-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AydinlatmaArmaturSayisiHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AydinlatmaArmaturSayisiHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: AydinlatmaArmaturSayisiHesaplamaInputs): Record<string, number> {
...
export function calculateAydinlatmaArmaturSayisiHesaplama(inputs: AydinlatmaArmaturSayisiHesaplamaInputs): {
