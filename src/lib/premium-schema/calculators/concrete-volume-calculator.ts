...
import {
  validateConcreteVolumeCalculatorInputs,
  type ConcreteVolumeCalculatorInputs,
} from "@/lib/premium-schema/calculators/concrete-volume-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ConcreteVolumeCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ConcreteVolumeCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ConcreteVolumeCalculatorInputs): Record<string, number> {
...
export function calculateConcreteVolumeCalculator(inputs: ConcreteVolumeCalculatorInputs): {
