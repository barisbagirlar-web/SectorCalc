...
import {
  validateKaynakMaliyetiVeDolguMetaliTuketimCalculatorInputs,
  type KaynakMaliyetiVeDolguMetaliTuketimCalculatorInputs,
} from "@/lib/premium-schema/calculators/kaynak-maliyeti-ve-dolgu-metali-tuketim-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KaynakMaliyetiVeDolguMetaliTuketimCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KaynakMaliyetiVeDolguMetaliTuketimCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KaynakMaliyetiVeDolguMetaliTuketimCalculatorInputs): Record<string, number> {
...
export function calculateKaynakMaliyetiVeDolguMetaliTuketimCalculator(inputs: KaynakMaliyetiVeDolguMetaliTuketimCalculatorInputs): {
