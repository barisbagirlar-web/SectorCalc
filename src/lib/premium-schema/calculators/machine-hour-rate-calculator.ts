...
import {
  validateMachineHourRateCalculatorInputs,
  type MachineHourRateCalculatorInputs,
} from "@/lib/premium-schema/calculators/machine-hour-rate-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: MachineHourRateCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof MachineHourRateCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: MachineHourRateCalculatorInputs): Record<string, number> {
...
export function calculateMachineHourRateCalculator(inputs: MachineHourRateCalculatorInputs): {
