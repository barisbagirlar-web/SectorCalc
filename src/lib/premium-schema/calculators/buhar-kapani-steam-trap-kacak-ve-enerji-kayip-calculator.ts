...
import {
  validateBuharKapaniSteamTrapKacakVeEnerjiKayipCalculatorInputs,
  type BuharKapaniSteamTrapKacakVeEnerjiKayipCalculatorInputs,
} from "@/lib/premium-schema/calculators/buhar-kapani-steam-trap-kacak-ve-enerji-kayip-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BuharKapaniSteamTrapKacakVeEnerjiKayipCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BuharKapaniSteamTrapKacakVeEnerjiKayipCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: BuharKapaniSteamTrapKacakVeEnerjiKayipCalculatorInputs): Record<string, number> {
...
export function calculateBuharKapaniSteamTrapKacakVeEnerjiKayipCalculator(inputs: BuharKapaniSteamTrapKacakVeEnerjiKayipCalculatorInputs): {
