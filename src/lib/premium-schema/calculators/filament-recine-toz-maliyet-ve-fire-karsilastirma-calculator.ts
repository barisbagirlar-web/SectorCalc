...
import {
  validateFilamentRecineTozMaliyetVeFireKarsilastirmaCalculatorInputs,
  type FilamentRecineTozMaliyetVeFireKarsilastirmaCalculatorInputs,
} from "@/lib/premium-schema/calculators/filament-recine-toz-maliyet-ve-fire-karsilastirma-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FilamentRecineTozMaliyetVeFireKarsilastirmaCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FilamentRecineTozMaliyetVeFireKarsilastirmaCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FilamentRecineTozMaliyetVeFireKarsilastirmaCalculatorInputs): Record<string, number> {
...
export function calculateFilamentRecineTozMaliyetVeFireKarsilastirmaCalculator(inputs: FilamentRecineTozMaliyetVeFireKarsilastirmaCalculatorInputs): {
