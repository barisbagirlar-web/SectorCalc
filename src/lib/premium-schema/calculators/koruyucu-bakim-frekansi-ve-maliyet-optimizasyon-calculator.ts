...
import {
  validateKoruyucuBakimFrekansiVeMaliyetOptimizasyonCalculatorInputs,
  type KoruyucuBakimFrekansiVeMaliyetOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/koruyucu-bakim-frekansi-ve-maliyet-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KoruyucuBakimFrekansiVeMaliyetOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KoruyucuBakimFrekansiVeMaliyetOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KoruyucuBakimFrekansiVeMaliyetOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateKoruyucuBakimFrekansiVeMaliyetOptimizasyonCalculator(inputs: KoruyucuBakimFrekansiVeMaliyetOptimizasyonCalculatorInputs): {
