...
import {
  validateToplamSahipOlmaMaliyetiTcoEkipmanKarsilastirmaCalculatorInputs,
  type ToplamSahipOlmaMaliyetiTcoEkipmanKarsilastirmaCalculatorInputs,
} from "@/lib/premium-schema/calculators/toplam-sahip-olma-maliyeti-tco-ekipman-karsilastirma-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ToplamSahipOlmaMaliyetiTcoEkipmanKarsilastirmaCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ToplamSahipOlmaMaliyetiTcoEkipmanKarsilastirmaCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ToplamSahipOlmaMaliyetiTcoEkipmanKarsilastirmaCalculatorInputs): Record<string, number> {
...
export function calculateToplamSahipOlmaMaliyetiTcoEkipmanKarsilastirmaCalculator(inputs: ToplamSahipOlmaMaliyetiTcoEkipmanKarsilastirmaCalculatorInputs): {
