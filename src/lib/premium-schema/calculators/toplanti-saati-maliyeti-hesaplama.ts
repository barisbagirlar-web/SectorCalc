...
import {
  validateToplantiSaatiMaliyetiHesaplamaInputs,
  type ToplantiSaatiMaliyetiHesaplamaInputs,
} from "@/lib/premium-schema/calculators/toplanti-saati-maliyeti-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ToplantiSaatiMaliyetiHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ToplantiSaatiMaliyetiHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: ToplantiSaatiMaliyetiHesaplamaInputs): Record<string, number> {
...
export function calculateToplantiSaatiMaliyetiHesaplama(inputs: ToplantiSaatiMaliyetiHesaplamaInputs): {
