...
import {
  validateIsElbisesiKkdKisiselKoruyucuDonanimSarfiyatiInputs,
  type IsElbisesiKkdKisiselKoruyucuDonanimSarfiyatiInputs,
} from "@/lib/premium-schema/calculators/is-elbisesi-kkd-kisisel-koruyucu-donanim-sarfiyati-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IsElbisesiKkdKisiselKoruyucuDonanimSarfiyatiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IsElbisesiKkdKisiselKoruyucuDonanimSarfiyatiInputs] as number;
}

function runFormulaPipeline(inputs: IsElbisesiKkdKisiselKoruyucuDonanimSarfiyatiInputs): Record<string, number> {
...
export function calculateIsElbisesiKkdKisiselKoruyucuDonanimSarfiyati(inputs: IsElbisesiKkdKisiselKoruyucuDonanimSarfiyatiInputs): {
