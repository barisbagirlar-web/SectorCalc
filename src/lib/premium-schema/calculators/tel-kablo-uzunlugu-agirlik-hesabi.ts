...
import {
  validateTelKabloUzunluguAgirlikHesabiInputs,
  type TelKabloUzunluguAgirlikHesabiInputs,
} from "@/lib/premium-schema/calculators/tel-kablo-uzunlugu-agirlik-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TelKabloUzunluguAgirlikHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TelKabloUzunluguAgirlikHesabiInputs] as number;
}

function runFormulaPipeline(inputs: TelKabloUzunluguAgirlikHesabiInputs): Record<string, number> {
...
export function calculateTelKabloUzunluguAgirlikHesabi(inputs: TelKabloUzunluguAgirlikHesabiInputs): {
