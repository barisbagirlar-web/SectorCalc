...
import {
  validateFuelConsumptionCalculatorInputs,
  type FuelConsumptionCalculatorInputs,
} from "@/lib/premium-schema/calculators/fuel-consumption-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FuelConsumptionCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FuelConsumptionCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FuelConsumptionCalculatorInputs): Record<string, number> {
...
export function calculateFuelConsumptionCalculator(inputs: FuelConsumptionCalculatorInputs): {
