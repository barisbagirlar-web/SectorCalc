...
import {
  validateCpkPpkHataMaliyetiPpmCalculatorInputs,
  type CpkPpkHataMaliyetiPpmCalculatorInputs,
} from "@/lib/premium-schema/calculators/cpk-ppk-hata-maliyeti-ppm-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: CpkPpkHataMaliyetiPpmCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof CpkPpkHataMaliyetiPpmCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: CpkPpkHataMaliyetiPpmCalculatorInputs): Record<string, number> {
...
export function calculateCpkPpkHataMaliyetiPpmCalculator(inputs: CpkPpkHataMaliyetiPpmCalculatorInputs): {
