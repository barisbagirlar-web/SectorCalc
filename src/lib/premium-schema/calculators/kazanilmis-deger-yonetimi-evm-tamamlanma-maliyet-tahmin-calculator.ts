...
import {
  validateKazanilmisDegerYonetimiEvmTamamlanmaMaliyetTahminCalculatorInputs,
  type KazanilmisDegerYonetimiEvmTamamlanmaMaliyetTahminCalculatorInputs,
} from "@/lib/premium-schema/calculators/kazanilmis-deger-yonetimi-evm-tamamlanma-maliyet-tahmin-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KazanilmisDegerYonetimiEvmTamamlanmaMaliyetTahminCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KazanilmisDegerYonetimiEvmTamamlanmaMaliyetTahminCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: KazanilmisDegerYonetimiEvmTamamlanmaMaliyetTahminCalculatorInputs): Record<string, number> {
...
export function calculateKazanilmisDegerYonetimiEvmTamamlanmaMaliyetTahminCalculator(inputs: KazanilmisDegerYonetimiEvmTamamlanmaMaliyetTahminCalculatorInputs): {
