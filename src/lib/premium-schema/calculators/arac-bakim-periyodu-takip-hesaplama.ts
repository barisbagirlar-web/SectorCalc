...
import {
  validateAracBakimPeriyoduTakipHesaplamaInputs,
  type AracBakimPeriyoduTakipHesaplamaInputs,
} from "@/lib/premium-schema/calculators/arac-bakim-periyodu-takip-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AracBakimPeriyoduTakipHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AracBakimPeriyoduTakipHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: AracBakimPeriyoduTakipHesaplamaInputs): Record<string, number> {
...
export function calculateAracBakimPeriyoduTakipHesaplama(inputs: AracBakimPeriyoduTakipHesaplamaInputs): {
