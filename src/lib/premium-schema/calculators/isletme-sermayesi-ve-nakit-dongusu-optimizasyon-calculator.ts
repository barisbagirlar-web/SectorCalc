...
import {
  validateIsletmeSermayesiVeNakitDongusuOptimizasyonCalculatorInputs,
  type IsletmeSermayesiVeNakitDongusuOptimizasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/isletme-sermayesi-ve-nakit-dongusu-optimizasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IsletmeSermayesiVeNakitDongusuOptimizasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IsletmeSermayesiVeNakitDongusuOptimizasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: IsletmeSermayesiVeNakitDongusuOptimizasyonCalculatorInputs): Record<string, number> {
...
export function calculateIsletmeSermayesiVeNakitDongusuOptimizasyonCalculator(inputs: IsletmeSermayesiVeNakitDongusuOptimizasyonCalculatorInputs): {
