...
import {
  validateKonteynerYuklemeKapasitesiTeuHesabiInputs,
  type KonteynerYuklemeKapasitesiTeuHesabiInputs,
} from "@/lib/premium-schema/calculators/konteyner-yukleme-kapasitesi-teu-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KonteynerYuklemeKapasitesiTeuHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KonteynerYuklemeKapasitesiTeuHesabiInputs] as number;
}

function runFormulaPipeline(inputs: KonteynerYuklemeKapasitesiTeuHesabiInputs): Record<string, number> {
...
export function calculateKonteynerYuklemeKapasitesiTeuHesabi(inputs: KonteynerYuklemeKapasitesiTeuHesabiInputs): {
