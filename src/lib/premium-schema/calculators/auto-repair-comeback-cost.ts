...
import {
  validateAutoRepairComebackCostInputs,
  type AutoRepairComebackCostInputs,
} from "@/lib/premium-schema/calculators/auto-repair-comeback-cost-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AutoRepairComebackCostInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AutoRepairComebackCostInputs] as number;
}

function runFormulaPipeline(inputs: AutoRepairComebackCostInputs): Record<string, number> {
...
export function calculateAutoRepairComebackCost(inputs: AutoRepairComebackCostInputs): {
