...
import {
  validateBatchYieldCalculatorInputs,
  type BatchYieldCalculatorInputs,
} from "@/lib/premium-schema/calculators/batch-yield-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BatchYieldCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BatchYieldCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: BatchYieldCalculatorInputs): Record<string, number> {
...
export function calculateBatchYieldCalculator(inputs: BatchYieldCalculatorInputs): {
