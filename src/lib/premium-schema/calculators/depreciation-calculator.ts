...
import {
  validateDepreciationCalculatorInputs,
  type DepreciationCalculatorInputs,
} from "@/lib/premium-schema/calculators/depreciation-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DepreciationCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DepreciationCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: DepreciationCalculatorInputs): Record<string, number> {
...
export function calculateDepreciationCalculator(inputs: DepreciationCalculatorInputs): {
