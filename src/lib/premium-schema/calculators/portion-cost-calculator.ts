...
import {
  validatePortionCostCalculatorInputs,
  type PortionCostCalculatorInputs,
} from "@/lib/premium-schema/calculators/portion-cost-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: PortionCostCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof PortionCostCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: PortionCostCalculatorInputs): Record<string, number> {
...
export function calculatePortionCostCalculator(inputs: PortionCostCalculatorInputs): {
