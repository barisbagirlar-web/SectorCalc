...
import {
  validateOeeCalculatorInputs,
  type OeeCalculatorInputs,
} from "@/lib/premium-schema/calculators/oee-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: OeeCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof OeeCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: OeeCalculatorInputs): Record<string, number> {
...
export function calculateOeeCalculator(inputs: OeeCalculatorInputs): {
