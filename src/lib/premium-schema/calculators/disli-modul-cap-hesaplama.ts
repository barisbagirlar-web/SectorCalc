...
import {
  validateDisliModulCapHesaplamaInputs,
  type DisliModulCapHesaplamaInputs,
} from "@/lib/premium-schema/calculators/disli-modul-cap-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: DisliModulCapHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof DisliModulCapHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: DisliModulCapHesaplamaInputs): Record<string, number> {
...
export function calculateDisliModulCapHesaplama(inputs: DisliModulCapHesaplamaInputs): {
