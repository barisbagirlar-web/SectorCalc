...
import {
  validateFiyatArtisiVeTalepEsnekligiKarlilikSimulasyonCalculatorInputs,
  type FiyatArtisiVeTalepEsnekligiKarlilikSimulasyonCalculatorInputs,
} from "@/lib/premium-schema/calculators/fiyat-artisi-ve-talep-esnekligi-karlilik-simulasyon-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: FiyatArtisiVeTalepEsnekligiKarlilikSimulasyonCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof FiyatArtisiVeTalepEsnekligiKarlilikSimulasyonCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: FiyatArtisiVeTalepEsnekligiKarlilikSimulasyonCalculatorInputs): Record<string, number> {
...
export function calculateFiyatArtisiVeTalepEsnekligiKarlilikSimulasyonCalculator(inputs: FiyatArtisiVeTalepEsnekligiKarlilikSimulasyonCalculatorInputs): {
