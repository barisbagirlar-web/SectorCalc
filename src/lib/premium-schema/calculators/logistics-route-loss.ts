...
import {
  validateLogisticsRouteLossInputs,
  type LogisticsRouteLossInputs,
} from "@/lib/premium-schema/calculators/logistics-route-loss-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: LogisticsRouteLossInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof LogisticsRouteLossInputs] as number;
}

function runFormulaPipeline(inputs: LogisticsRouteLossInputs): Record<string, number> {
...
export function calculateLogisticsRouteLoss(inputs: LogisticsRouteLossInputs): {
  deadheadCost: number;
  driverCost: number;
