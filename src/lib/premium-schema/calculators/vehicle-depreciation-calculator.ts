...
import {
  validateVehicleDepreciationCalculatorInputs,
  type VehicleDepreciationCalculatorInputs,
} from "@/lib/premium-schema/calculators/vehicle-depreciation-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: VehicleDepreciationCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof VehicleDepreciationCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: VehicleDepreciationCalculatorInputs): Record<string, number> {
...
export function calculateVehicleDepreciationCalculator(inputs: VehicleDepreciationCalculatorInputs): {
