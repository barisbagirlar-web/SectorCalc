...
import {
  validateElectricityBillCalculatorInputs,
  type ElectricityBillCalculatorInputs,
} from "@/lib/premium-schema/calculators/electricity-bill-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: ElectricityBillCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof ElectricityBillCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: ElectricityBillCalculatorInputs): Record<string, number> {
...
export function calculateElectricityBillCalculator(inputs: ElectricityBillCalculatorInputs): {
