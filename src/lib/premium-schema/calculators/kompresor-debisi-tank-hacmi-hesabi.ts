...
import {
  validateKompresorDebisiTankHacmiHesabiInputs,
  type KompresorDebisiTankHacmiHesabiInputs,
} from "@/lib/premium-schema/calculators/kompresor-debisi-tank-hacmi-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KompresorDebisiTankHacmiHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KompresorDebisiTankHacmiHesabiInputs] as number;
}

function runFormulaPipeline(inputs: KompresorDebisiTankHacmiHesabiInputs): Record<string, number> {
...
export function calculateKompresorDebisiTankHacmiHesabi(inputs: KompresorDebisiTankHacmiHesabiInputs): {
