...
import {
  validateTirKamyonYukKapasitesiHesaplamaInputs,
  type TirKamyonYukKapasitesiHesaplamaInputs,
} from "@/lib/premium-schema/calculators/tir-kamyon-yuk-kapasitesi-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: TirKamyonYukKapasitesiHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof TirKamyonYukKapasitesiHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: TirKamyonYukKapasitesiHesaplamaInputs): Record<string, number> {
...
export function calculateTirKamyonYukKapasitesiHesaplama(inputs: TirKamyonYukKapasitesiHesaplamaInputs): {
