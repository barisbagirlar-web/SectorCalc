...
import {
  validateRoofingSquareCostCheckInputs,
  type RoofingSquareCostCheckInputs,
} from "@/lib/premium-schema/calculators/roofing-square-cost-check-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: RoofingSquareCostCheckInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof RoofingSquareCostCheckInputs] as number;
}

function runFormulaPipeline(inputs: RoofingSquareCostCheckInputs): Record<string, number> {
...
export function calculateRoofingSquareCostCheck(inputs: RoofingSquareCostCheckInputs): {
