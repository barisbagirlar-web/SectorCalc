...
import {
  validateAsgariSiparisMiktariMoqVeStokTasimaMaliyetDengeCalculatorInputs,
  type AsgariSiparisMiktariMoqVeStokTasimaMaliyetDengeCalculatorInputs,
} from "@/lib/premium-schema/calculators/asgari-siparis-miktari-moq-ve-stok-tasima-maliyet-denge-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AsgariSiparisMiktariMoqVeStokTasimaMaliyetDengeCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AsgariSiparisMiktariMoqVeStokTasimaMaliyetDengeCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: AsgariSiparisMiktariMoqVeStokTasimaMaliyetDengeCalculatorInputs): Record<string, number> {
...
export function calculateAsgariSiparisMiktariMoqVeStokTasimaMaliyetDengeCalculator(inputs: AsgariSiparisMiktariMoqVeStokTasimaMaliyetDengeCalculatorInputs): {
