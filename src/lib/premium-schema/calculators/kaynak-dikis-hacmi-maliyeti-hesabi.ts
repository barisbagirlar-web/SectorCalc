...
import {
  validateKaynakDikisHacmiMaliyetiHesabiInputs,
  type KaynakDikisHacmiMaliyetiHesabiInputs,
} from "@/lib/premium-schema/calculators/kaynak-dikis-hacmi-maliyeti-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: KaynakDikisHacmiMaliyetiHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof KaynakDikisHacmiMaliyetiHesabiInputs] as number;
}

function runFormulaPipeline(inputs: KaynakDikisHacmiMaliyetiHesabiInputs): Record<string, number> {
...
export function calculateKaynakDikisHacmiMaliyetiHesabi(inputs: KaynakDikisHacmiMaliyetiHesabiInputs): {
