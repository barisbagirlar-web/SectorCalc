...
import {
  validateAmperKilowattKwCeviriciInputs,
  type AmperKilowattKwCeviriciInputs,
} from "@/lib/premium-schema/calculators/amper-kilowatt-kw-cevirici-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AmperKilowattKwCeviriciInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AmperKilowattKwCeviriciInputs] as number;
}

function runFormulaPipeline(inputs: AmperKilowattKwCeviriciInputs): Record<string, number> {
...
export function calculateAmperKilowattKwCevirici(inputs: AmperKilowattKwCeviriciInputs): {
