...
import {
  validateIsiDegistiriciEsanjorKapasiteHesabiInputs,
  type IsiDegistiriciEsanjorKapasiteHesabiInputs,
} from "@/lib/premium-schema/calculators/isi-degistirici-esanjor-kapasite-hesabi-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: IsiDegistiriciEsanjorKapasiteHesabiInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof IsiDegistiriciEsanjorKapasiteHesabiInputs] as number;
}

function runFormulaPipeline(inputs: IsiDegistiriciEsanjorKapasiteHesabiInputs): Record<string, number> {
...
export function calculateIsiDegistiriciEsanjorKapasiteHesabi(inputs: IsiDegistiriciEsanjorKapasiteHesabiInputs): {
