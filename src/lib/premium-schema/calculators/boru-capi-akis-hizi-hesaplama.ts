...
import {
  validateBoruCapiAkisHiziHesaplamaInputs,
  type BoruCapiAkisHiziHesaplamaInputs,
} from "@/lib/premium-schema/calculators/boru-capi-akis-hizi-hesaplama-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: BoruCapiAkisHiziHesaplamaInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof BoruCapiAkisHiziHesaplamaInputs] as number;
}

function runFormulaPipeline(inputs: BoruCapiAkisHiziHesaplamaInputs): Record<string, number> {
...
export function calculateBoruCapiAkisHiziHesaplama(inputs: BoruCapiAkisHiziHesaplamaInputs): {
