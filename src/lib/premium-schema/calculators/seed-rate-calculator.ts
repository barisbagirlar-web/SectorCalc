...
import {
  validateSeedRateCalculatorInputs,
  type SeedRateCalculatorInputs,
} from "@/lib/premium-schema/calculators/seed-rate-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: SeedRateCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof SeedRateCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: SeedRateCalculatorInputs): Record<string, number> {
...
export function calculateSeedRateCalculator(inputs: SeedRateCalculatorInputs): {
