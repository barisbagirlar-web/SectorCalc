...
import {
  validateAgvAmrOtonomTasimaGeriDonusCalculatorInputs,
  type AgvAmrOtonomTasimaGeriDonusCalculatorInputs,
} from "@/lib/premium-schema/calculators/agv-amr-otonom-tasima-geri-donus-calculator-validation";
...
function resolveMappedValue(
  sourceKey: string,
  userInputs: AgvAmrOtonomTasimaGeriDonusCalculatorInputs,
  computed: Record<string, number>,
): number {
  if (sourceKey in computed) {
    return computed[sourceKey] ?? 0;
  }
  return userInputs[sourceKey as keyof AgvAmrOtonomTasimaGeriDonusCalculatorInputs] as number;
}

function runFormulaPipeline(inputs: AgvAmrOtonomTasimaGeriDonusCalculatorInputs): Record<string, number> {
...
export function calculateAgvAmrOtonomTasimaGeriDonusCalculator(inputs: AgvAmrOtonomTasimaGeriDonusCalculatorInputs): {
