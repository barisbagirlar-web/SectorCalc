...
export const DELIVERY_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "totalDeliveries", label: "Toplam Teslimat Sayısı", label_i18n: {"en":"Toplam Teslimat Sayısı","tr":"Toplam Teslimat Sayısı"}, type: "number", unit: "adet/ay", required: true, smartDefault: 500, validation: { min: 1 }, helper: "", expertMeaning: "Total monthly deliveries", expertMeaning_i18n: {"en":"Total monthly deliveries","tr":"Total monthly deliveries"} },
    { id: "successfulDeliveries", label: "Başarılı Teslimat", label_i18n: {"en":"Başarılı Teslimat","tr":"Başarılı Teslimat"}, type: "number", unit: "adet/ay", required: true, smartDefault: 470, validation: { min: 0 }, helper: "", expertMeaning: "Successful deliveries", expertMeaning_i18n: {"en":"Successful deliveries","tr":"Successful deliveries"} },
    { id: "costPerDelivery", label: "Teslimat Başına Maliyet", label_i18n: {"en":"Teslimat Başına Maliyet","tr":"Teslimat Başına Maliyet"}, type: "number", unit: "USD", required: true, smartDefault: 8, validation: { min: 0.01 }, helper: "", expertMeaning: "Cost per delivery", expertMeaning_i18n: {"en":"Cost per delivery","tr":"Cost per delivery"} },
    { id: "extraFailedDeliveryCost", label: "Başarısız Teslimat Ek Maliyeti", label_i18n: {"en":"Başarısız Teslimat Ek Maliyeti","tr":"Başarısız Teslimat Ek Maliyeti"}, type: "number", unit: "USD/adet", required: true, smartDefault: 15, validation: { min: 0 }, helper: "", expertMeaning: "Additional cost per failed delivery", expertMeaning_i18n: {"en":"Additional cost per failed delivery","tr":"Additional cost per failed delivery"} },
    { id: "returnHandlingCost", label: "İade İşlem Maliyeti", label_i18n: {"en":"İade İşlem Maliyeti","tr":"İade İşlem Maliyeti"}, type: "number", unit: "USD/adet", required: false, smartDefault: 5, validation: { min: 0 }, helper: "", expertMeaning: "Return handling cost per unit", expertMeaning_i18n: {"en":"Return handling cost per unit","tr":"Return handling cost per unit"} },
  ],
  outputs: [
    { id: "deliveryEfficiency", label: "Teslimat Verimliliği", label_i18n: {"en":"Teslimat Verimliliği","tr":"Teslimat Verimliliği"}, unit: "%", format: "percentage" },
    { id: "failedDeliveryCost", label: "Başarısız Teslimat Maliyeti", label_i18n: {"en":"Başarısız Teslimat Maliyeti","tr":"Başarısız Teslimat Maliyeti"}, unit: "USD/ay", format: "currency" },
