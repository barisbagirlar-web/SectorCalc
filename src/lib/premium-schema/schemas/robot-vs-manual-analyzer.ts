...
export const ROBOT_VS_MANUAL_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Robot vs Manuel Operasyon Maliyet", name_i18n: {"en":"Robot vs Manual Operation Cost Analysis","tr":"Robot vs Manuel Operasyon Maliyet"}, sectorSlug: "cnc-manufacturing", category: "cost",
  painStatement: "Otomasyon yatırımının manuel operasyona göre gerçek maliyet avantajı hesaplanmazsa yanlış karar üretim maliyetini katlayabilir.", painStatement_i18n: {"en":"Without calculating the true cost advantage of automation investment over manual operation, wrong decisions can multiply production costs.","tr":"Otomasyon yatırımının manuel operasyona göre gerçek maliyet avantajı hesaplanmazsa yanlış karar üretim maliyetini katlayabilir."},
  inputs: [
  ...
  thresholds: [{ fieldId: "costPerUnitRobot", warning: 0.5, critical: 0.8, direction: "higher_is_bad", warningMessage: "Robot birim maliyet > $0.50 — verimlilik değerlendirilmeli.", warningMessage_i18n: {"en":"Robot unit cost > $0.50 — efficiency should be evaluated.","tr":"Robot birim maliyet > $0.50 — verimlilik değerlendirilmeli."}, criticalMessage: "Robot birim maliyet > $0.80 — fizibilite sorgulanmalı.", criticalMessage_i18n: {"en":"Robot unit cost > $0.80 — feasibility should be questioned.","tr":"Robot birim maliyet > $0.80 — fizibilite sorgulanmalı."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Robot vs Manual Analysis Report", title_i18n: {"en":"Robot vs Manual Analysis Report","tr":"Robot vs Manuel Analiz Raporu"}, sections: ["executive_summary", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.0, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["Manuel maliyet = işçi sayısı × yıllık maliyet.", "Robot maliyeti = bakım + enerji + (yatırım/ömür).", "Birim maliyet = yıllık maliyet / yıllık çıktı.", "ROI = (manuel - robot) / yatırım × 100."],assumptionNotes_i18n:[{"en":"Manual cost = worker count × annual cost.","tr":"Manuel maliyet = işçi sayısı × yıllık maliyet."},{"en":"Robot cost = maintenance + energy + (investment/life).","tr":"Robot maliyeti = bakım + enerji + (yatırım/ömür)."},{"en":"Unit cost = annual cost / annual output.","tr":"Birim maliyet = yıllık maliyet / yıllık çıktı."},{"en":"ROI = (manual - robot) / investment × 100.","tr":"ROI = (manuel - robot) / yatırım × 100."}] },
};
