...
export const BOLT_TORQUE_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Cıvata Tork & Öngerilme Analizi", name_i18n: {"en":"Cıvata Tork & Öngerilme Analizi","tr":"Cıvata Tork & Öngerilme Analizi"}, sectorSlug: "cnc-manufacturing", category: "measurement",
  ...
  inputs: [
    { id: "nominalDiameter", label: "Nominal Çap (d)", label_i18n: {"en":"Nominal Çap (d)","tr":"Nominal Çap (d)"}, type: "number", unit: "mm", required: true, smartDefault: 12, validation: { min: 1 }, helper: "", expertMeaning: "Bolt nominal diameter", expertMeaning_i18n: {"en":"Bolt nominal diameter","tr":"Bolt nominal diameter"} },
    ...
    { id: "torqueCoefficient", label: "Sürtünme Katsayısı (K)", label_i18n: {"en":"Sürtünme Katsayısı (K)","tr":"Sürtünme Katsayısı (K)"}, type: "number", unit: "", required: true, smartDefault: 0.2, validation: { min: 0.05, max: 0.5 }, helper: "", expertMeaning: "Nut factor (typically 0.15-0.25)", expertMeaning_i18n: {"en":"Nut factor (typically 0.15-0.25)","tr":"Nut factor (typically 0.15-0.25)"} },
    { id: "grade", label: "Malzeme Sınıfı", label_i18n: {"en":"Malzeme Sınıfı","tr":"Malzeme Sınıfı"}, type: "select", unit: "", required: true, smartDefault: "8.8", options: [...GRADE_OPTIONS], helper: "", expertMeaning: "Bolt grade per ISO 898", expertMeaning_i18n: {"en":"Bolt grade per ISO 898","tr":"Bolt grade per ISO 898"} },
    { id: "yieldStrength", label: "Akma Dayanımı", label_i18n: {"en":"Akma Dayanımı","tr":"Akma Dayanımı"}, type: "number", unit: "MPa", required: true, smartDefault: 660, validation: { min: 100 }, helper: "", expertMeaning: "Yield strength of bolt material", expertMeaning_i18n: {"en":"Yield strength of bolt material","tr":"Yield strength of bolt material"} },
    { id: "targetPreloadPercent", label: "Hedef Öngerilme (%)", label_i18n: {"en":"Hedef Öngerilme (%)","tr":"Hedef Öngerilme (%)"}, type: "number", unit: "%", required: false, smartDefault: 70, validation: { min: 10, max: 100 }, helper: "", expertMeaning: "Target preload as % of yield", expertMeaning_i18n: {"en":"Target preload as % of yield","tr":"Target preload as % of yield"} },
    { id: "calibrationError", label: "Tork Anahtarı Kalibrasyon Hatası", label_i18n: {"en":"Tork Anahtarı Kalibrasyon Hatası","tr":"Tork Anahtarı Kalibrasyon Hatası"}, type: "number", unit: "%", required: false, smartDefault: 5, validation: { min: 0, max: 50 }, helper: "", expertMeaning: "Wrench calibration uncertainty", expertMeaning_i18n: {"en":"Wrench calibration uncertainty","tr":"Wrench calibration uncertainty"} },
  ],
  outputs: [
