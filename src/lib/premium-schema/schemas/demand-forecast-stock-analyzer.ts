...
export const DEMAND_FORECAST_STOCK_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "actualDemand", label: "Gerçekleşen Talep", label_i18n: {"en":"Gerçekleşen Talep","tr":"Gerçekleşen Talep"}, type: "number", unit: "adet/dönem", required: true, smartDefault: 1000, validation: { min: 1 }, helper: "", expertMeaning: "Actual realized demand", expertMeaning_i18n: {"en":"Actual realized demand","tr":"Actual realized demand"} },
    ...
    { id: "stockoutCost", label: "Stok Dışı Kalma Maliyeti", label_i18n: {"en":"Stok Dışı Kalma Maliyeti","tr":"Stok Dışı Kalma Maliyeti"}, type: "number", unit: "USD/adet", required: true, smartDefault: 12, validation: { min: 0.01 }, helper: "", expertMeaning: "Cost per stockout unit", expertMeaning_i18n: {"en":"Cost per stockout unit","tr":"Cost per stockout unit"} },
    { id: "leadTime", label: "Teslim Süresi", label_i18n: {"en":"Teslim Süresi","tr":"Teslim Süresi"}, type: "number", unit: "gün", required: false, smartDefault: 5, validation: { min: 0 }, helper: "", expertMeaning: "Replenishment lead time", expertMeaning_i18n: {"en":"Replenishment lead time","tr":"Replenishment lead time"} },
  ],
  outputs: [
    { id: "forecastError", label: "Tahmin Hatası", label_i18n: {"en":"Tahmin Hatası","tr":"Tahmin Hatası"}, unit: "adet", format: "number" },
    { id: "safetyStockForecast", label: "Tahmin Bazlı Güvenlik Stoğu", label_i18n: {"en":"Tahmin Bazlı Güvenlik Stoğu","tr":"Tahmin Bazlı Güvenlik Stoğu"}, unit: "adet", format: "number" },
    { id: "forecastCarryingCost", label: "Taşıma Maliyeti (Sapma)", label_i18n: {"en":"Taşıma Maliyeti (Sapma)","tr":"Taşıma Maliyeti (Sapma)"}, unit: "USD/dönem", format: "currency" },
    { id: "stockoutCostForecast", label: "Stok Dışı Maliyeti", label_i18n: {"en":"Stok Dışı Maliyeti","tr":"Stok Dışı Maliyeti"}, unit: "USD/dönem", format: "currency" },
    { id: "totalForecastCost", label: "Toplam Tahmin Kaynaklı Maliyet", label_i18n: {"en":"Toplam Tahmin Kaynaklı Maliyet","tr":"Toplam Tahmin Kaynaklı Maliyet"}, unit: "USD/dönem", format: "currency", isBigNumber: true },
  ],
