...
export const MOQ_STOCK_BALANCE_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "MOQ Stok Denge Analizi", name_i18n: {"en":"MOQ Stock Balance Analysis","tr":"MOQ Stok Denge Analizi"}, sectorSlug: "logistics-transport", category: "cost",
  painStatement: "MOQ ile gerçek talep arasındaki fark hesaplanmazsa ya stok fazlası cezası ya da fırsat maliyeti oluşur.", painStatement_i18n: {"en":"Without calculating the gap between MOQ and actual demand, either excess stock penalties or opportunity costs arise.","tr":"MOQ ile gerçek talep arasındaki fark hesaplanmazsa ya stok fazlası cezası ya da fırsat maliyeti oluşur."},
  inputs: [
    ...
    { id: "unitCost", label: "Birim Maliyet", label_i18n: {"en":"Unit cost","tr":"Birim Maliyet"}, type: "number", unit: "USD", required: true, smartDefault: 12, validation: { min: 0.01 }, helper: "", expertMeaning: "Unit cost", expertMeaning_i18n: {"en":"Unit cost","tr":"Birim Maliyet"} },
  ...
  outputs:  [
    { id: "optimalQty", label: "Optimum Sipariş Miktarı", label_i18n: {"en":"Optimal Order Quantity","tr":"Optimum Sipariş Miktarı"}, unit: "adet", format: "number" },
    { id: "moqPenalty", label: "MOQ Cezası", label_i18n: {"en":"MOQ Penalty","tr":"MOQ Cezası"}, unit: "USD/yıl", format: "currency" },
    { id: "priceBreakSavings", label: "Fiyat Kırılım Tasarrufu", label_i18n: {"en":"Price Break Savings","tr":"Fiyat Kırılım Tasarrufu"}, unit: "USD/yıl", format: "currency" },
    { id: "netBenefit", label: "Net Fayda", label_i18n: {"en":"Net Benefit","tr":"Net Fayda"}, unit: "USD/yıl", format: "currency" },
  ],
  thresholds: [{ fieldId: "moqPenalty", warning: 5000, critical: 15000, direction: "higher_is_bad", warningMessage: "MOQ cezası > $5K — tedarikçi ile MOQ yeniden müzakere edilmeli.", warningMessage_i18n: {"en":"MOQ penalty > $5K — renegotiate MOQ with supplier.","tr":"MOQ cezası > $5K — tedarikçi ile MOQ yeniden müzakere edilmeli."}, criticalMessage: "MOQ cezası > $15K — alternatif tedarikçi değerlendirilmeli.", criticalMessage_i18n: {"en":"MOQ penalty > $15K — alternative supplier should be evaluated.","tr":"MOQ cezası > $15K — alternatif tedarikçi değerlendirilmeli."} }],
  formulaPipeline: [
  ...
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["MOQ cezası = max(0, MOQ − talep) × birim maliyet × sipariş sayısı.", "Optimum miktar = max(talep, MOQ).", "Fiyat kırılım avantajı skontolu fiyat bazında hesaplanır."],assumptionNotes_i18n:[{"en":"MOQ penalty = max(0, MOQ − demand) × unit cost × order count.","tr":"MOQ cezası = max(0, MOQ − talep) × birim maliyet × sipariş sayısı."},{"en":"Optimal quantity = max(demand, MOQ).","tr":"Optimum miktar = max(talep, MOQ)."},{"en":"Price break advantage is calculated on discounted price basis.","tr":"Fiyat kırılım avantajı skontolu fiyat bazında hesaplanır."}] },
};
