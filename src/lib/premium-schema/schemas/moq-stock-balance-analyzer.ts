...
export const MOQ_STOCK_BALANCE_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "moqQty", label: "Minimum Sipariş Miktarı (MOQ)", label_i18n: {"en":"Minimum Sipariş Miktarı (MOQ)","tr":"Minimum Sipariş Miktarı (MOQ)"}, type: "number", unit: "adet", required: true, smartDefault: 500, validation: { min: 1 }, helper: "", expertMeaning: "Minimum order quantity", expertMeaning_i18n: {"en":"Minimum order quantity","tr":"Minimum order quantity"} },
    { id: "actualDemand", label: "Gerçek Talep", label_i18n: {"en":"Gerçek Talep","tr":"Gerçek Talep"}, type: "number", unit: "adet/sipariş", required: true, smartDefault: 300, validation: { min: 1 }, helper: "", expertMeaning: "Actual demand per order", expertMeaning_i18n: {"en":"Actual demand per order","tr":"Actual demand per order"} },
    ...
    { id: "priceBreakQty", label: "Fiyat Kırılım Miktarı", label_i18n: {"en":"Fiyat Kırılım Miktarı","tr":"Fiyat Kırılım Miktarı"}, type: "number", unit: "adet", required: false, smartDefault: 1000, validation: { min: 1 }, helper: "", expertMeaning: "Quantity for price break", expertMeaning_i18n: {"en":"Quantity for price break","tr":"Quantity for price break"} },
    { id: "priceBreakUnitPrice", label: "Fiyat Kırılım Birim Fiyatı", label_i18n: {"en":"Fiyat Kırılım Birim Fiyatı","tr":"Fiyat Kırılım Birim Fiyatı"}, type: "number", unit: "USD", required: false, smartDefault: 10, validation: { min: 0.01 }, helper: "", expertMeaning: "Price break unit price", expertMeaning_i18n: {"en":"Price break unit price","tr":"Price break unit price"} },
    { id: "annualOrders", label: "Yıllık Sipariş Sayısı", label_i18n: {"en":"Yıllık Sipariş Sayısı","tr":"Yıllık Sipariş Sayısı"}, type: "number", unit: "sipariş/yıl", required: true, smartDefault: 12, validation: { min: 1 }, helper: "", expertMeaning: "Orders per year", expertMeaning_i18n: {"en":"Orders per year","tr":"Orders per year"} },
  ],
  outputs: [
    { id: "optimalQty", label: "Optimum Sipariş Miktarı", label_i18n: {"en":"Optimum Sipariş Miktarı","tr":"Optimum Sipariş Miktarı"}, unit: "adet", format: "number" },
    { id: "moqPenalty", label: "MOQ Cezası", label_i18n: {"en":"MOQ Cezası","tr":"MOQ Cezası"}, unit: "USD/yıl", format: "currency" },
    { id: "priceBreakSavings", label: "Fiyat Kırılım Tasarrufu", label_i18n: {"en":"Fiyat Kırılım Tasarrufu","tr":"Fiyat Kırılım Tasarrufu"}, unit: "USD/yıl", format: "currency" },
