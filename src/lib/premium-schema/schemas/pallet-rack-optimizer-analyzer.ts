...
export const PALLET_RACK_OPTIMIZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Palet Rafı Kapasite Optimizasyonu", name_i18n: {"en":"Palet Rafı Kapasite Optimizasyonu","tr":"Palet Rafı Kapasite Optimizasyonu"}, sectorSlug: "logistics-transport", category: "measurement",
  ...
  inputs: [
    { id: "rackWidth", label: "Raf Genişliği", label_i18n: {"en":"Raf Genişliği","tr":"Raf Genişliği"}, type: "number", unit: "mm", required: true, smartDefault: 2700, validation: { min: 100 }, helper: "", expertMeaning: "Rack width in mm", expertMeaning_i18n: {"en":"Rack width in mm","tr":"Rack width in mm"} },
    { id: "rackDepth", label: "Raf Derinliği", label_i18n: {"en":"Raf Derinliği","tr":"Raf Derinliği"}, type: "number", unit: "mm", required: true, smartDefault: 1100, validation: { min: 100 }, helper: "", expertMeaning: "Rack depth in mm", expertMeaning_i18n: {"en":"Rack depth in mm","tr":"Rack depth in mm"} },
    { id: "rackHeight", label: "Raf Yüksekliği", label_i18n: {"en":"Raf Yüksekliği","tr":"Raf Yüksekliği"}, type: "number", unit: "mm", required: true, smartDefault: 5000, validation: { min: 100 }, helper: "", expertMeaning: "Rack height in mm", expertMeaning_i18n: {"en":"Rack height in mm","tr":"Rack height in mm"} },
    { id: "loadCapacity", label: "Birim Yük Kapasitesi", label_i18n: {"en":"Birim Yük Kapasitesi","tr":"Birim Yük Kapasitesi"}, type: "number", unit: "kg", required: true, smartDefault: 1000, validation: { min: 1 }, helper: "", expertMeaning: "Load capacity per pallet position", expertMeaning_i18n: {"en":"Load capacity per pallet position","tr":"Load capacity per pallet position"} },
    { id: "numberOfLevels", label: "Raf Kat Sayısı", label_i18n: {"en":"Raf Kat Sayısı","tr":"Raf Kat Sayısı"}, type: "number", unit: "kat", required: true, smartDefault: 4, validation: { min: 1 }, helper: "", expertMeaning: "Number of rack levels", expertMeaning_i18n: {"en":"Number of rack levels","tr":"Number of rack levels"} },
    { id: "warehouseWidth", label: "Depo Genişliği", label_i18n: {"en":"Depo Genişliği","tr":"Depo Genişliği"}, type: "number", unit: "m", required: false, smartDefault: 40, validation: { min: 1 }, helper: "", expertMeaning: "Warehouse width in meters", expertMeaning_i18n: {"en":"Warehouse width in meters","tr":"Warehouse width in meters"} },
    { id: "warehouseLength", label: "Depo Uzunluğu", label_i18n: {"en":"Depo Uzunluğu","tr":"Depo Uzunluğu"}, type: "number", unit: "m", required: false, smartDefault: 80, validation: { min: 1 }, helper: "", expertMeaning: "Warehouse length in meters", expertMeaning_i18n: {"en":"Warehouse length in meters","tr":"Warehouse length in meters"} },
  ...
  outputs: [
    ...
    { id: "floorUtilization", label: "Alan Kullanım Oranı", label_i18n: {"en":"Alan Kullanım Oranı","tr":"Alan Kullanım Oranı"}, unit: "%", format: "number" },
    { id: "rackThroughput", label: "Raf Çıktı Hızı", label_i18n: {"en":"Raf Çıktı Hızı","tr":"Raf Çıktı Hızı"}, unit: "birim/saat", format: "number" },
    { id: "rackSafetyFactor", label: "Raf Güvenlik Faktörü", label_i18n: {"en":"Raf Güvenlik Faktörü","tr":"Raf Güvenlik Faktörü"}, unit: "", format: "number" },
    { id: "rackCostPerPosition", label: "Pozisyon Başına Maliyet", label_i18n: {"en":"Pozisyon Başına Maliyet","tr":"Pozisyon Başına Maliyet"}, unit: "USD", format: "currency" },
    { id: "retrievalTime", label: "Ortalama Erişim Süresi", label_i18n: {"en":"Ortalama Erişim Süresi","tr":"Ortalama Erişim Süresi"}, unit: "saniye", format: "number" },
  ],
