...
export const INFLATION_ESCALATION_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "baseLabor", label: "Baz İşçilik Maliyeti", label_i18n: {"en":"Baz İşçilik Maliyeti","tr":"Baz İşçilik Maliyeti"}, type: "number", unit: "USD", required: true, smartDefault: 300000, validation: { min: 0 }, helper: "", expertMeaning: "Base labor cost", expertMeaning_i18n: {"en":"Base labor cost","tr":"Base labor cost"} },
    { id: "inflLabor", label: "İşçilik Enflasyonu", label_i18n: {"en":"İşçilik Enflasyonu","tr":"İşçilik Enflasyonu"}, type: "number", unit: "%/yıl", required: true, smartDefault: 6, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Labor inflation rate", expertMeaning_i18n: {"en":"Labor inflation rate","tr":"Labor inflation rate"} },
    { id: "projectYears", label: "Proje Süresi", label_i18n: {"en":"Proje Süresi","tr":"Proje Süresi"}, type: "number", unit: "yıl", required: true, smartDefault: 3, validation: { min: 1 }, helper: "", expertMeaning: "Project duration in years", expertMeaning_i18n: {"en":"Project duration in years","tr":"Project duration in years"} },
    { id: "nominalRate", label: "Nominal İskonto Oranı", label_i18n: {"en":"Nominal İskonto Oranı","tr":"Nominal İskonto Oranı"}, type: "number", unit: "%", required: false, smartDefault: 15, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Nominal discount rate", expertMeaning_i18n: {"en":"Nominal discount rate","tr":"Nominal discount rate"} },
    ...
    { id: "confidenceFactor", label: "Güven Faktörü", label_i18n: {"en":"Güven Faktörü","tr":"Güven Faktörü"}, type: "number", unit: "%", required: false, smartDefault: 15, validation: { min: 0, max: 50 }, helper: "", expertMeaning: "Contingency confidence factor", expertMeaning_i18n: {"en":"Contingency confidence factor","tr":"Contingency confidence factor"} },
  ...
  outputs: [
    ...
    { id: "escalatedLabor", label: "Eskale İşçilik", label_i18n: {"en":"Eskale İşçilik","tr":"Eskale İşçilik"}, unit: "USD", format: "currency" },
    { id: "realDiscount", label: "Reel İskonto Oranı", label_i18n: {"en":"Reel İskonto Oranı","tr":"Reel İskonto Oranı"}, unit: "%", format: "percentage" },
    { id: "contingency", label: "Karşılık (Contingency)", label_i18n: {"en":"Karşılık (Contingency)","tr":"Karşılık (Contingency)"}, unit: "USD", format: "currency", isBigNumber: true },
  ],
