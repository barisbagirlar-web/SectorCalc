...
export const AI_TOKEN_COST_ANALYZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "dailyRequests", label: "Günlük API Request", label_i18n: {"en":"Günlük API Request","tr":"Günlük API Request"}, type: "number", unit: "adet", required: true, smartDefault: 10000, validation: { min: 1, max: 100_000_000 }, helper: "Total LLM API calls per day.", helper_i18n: {"en":"Total LLM API calls per day.","tr":"Total LLM API calls per day."}, expertMeaning: "Daily inference request count", expertMeaning_i18n: {"en":"Daily inference request count","tr":"Daily inference request count"} },
    ...
    { id: "monthlyGrowthRate", label: "Aylık Büyüme Oranı", label_i18n: {"en":"Aylık Büyüme Oranı","tr":"Aylık Büyüme Oranı"}, type: "number", unit: "%", required: true, smartDefault: 10, validation: { min: 0, max: 500 }, helper: "Month-over-month volume growth.", helper_i18n: {"en":"Month-over-month volume growth.","tr":"Month-over-month volume growth."}, expertMeaning: "MoM growth rate", expertMeaning_i18n: {"en":"MoM growth rate","tr":"MoM growth rate"} },
    { id: "confidenceBuffer", label: "Güven Tamponu", label_i18n: {"en":"Güven Tamponu","tr":"Güven Tamponu"}, type: "number", unit: "%", required: false, smartDefault: 15, validation: { min: 0, max: 100 }, helper: "Safety margin on projection.", helper_i18n: {"en":"Safety margin on projection.","tr":"Safety margin on projection."}, expertMeaning: "Contingency buffer", expertMeaning_i18n: {"en":"Contingency buffer","tr":"Contingency buffer"} },
    { id: "infraOverhead", label: "Altyapı Overhead", label_i18n: {"en":"Altyapı Overhead","tr":"Altyapı Overhead"}, type: "number", unit: "USD/ay", required: false, smartDefault: 500, validation: { min: 0, max: 1_000_000 }, helper: "GPU/server/orchestration costs.", helper_i18n: {"en":"GPU/server/orchestration costs.","tr":"GPU/server/orchestration costs."}, expertMeaning: "Non-API monthly infra cost", expertMeaning_i18n: {"en":"Non-API monthly infra cost","tr":"Non-API monthly infra cost"} },
    { id: "fallbackCost", label: "Fallback Model Maliyeti", label_i18n: {"en":"Fallback Model Maliyeti","tr":"Fallback Model Maliyeti"}, type: "number", unit: "USD/ay", required: false, smartDefault: 300, validation: { min: 0, max: 1_000_000 }, helper: "Backup model API spend.", helper_i18n: {"en":"Backup model API spend.","tr":"Backup model API spend."}, expertMeaning: "Secondary model cost", expertMeaning_i18n: {"en":"Secondary model cost","tr":"Secondary model cost"} },
  ],
  outputs: [
