...
export const FIRE_HYDRANT_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Yangın Hidrantı Akış Analizi", name_i18n: {"en":"Yangın Hidrantı Akış Analizi","tr":"Yangın Hidrantı Akış Analizi"}, sectorSlug: "quality", category: "measurement",
  ...
  inputs: [
    { id: "hydrantDiameter", label: "Hidrant Çapı", label_i18n: {"en":"Hidrant Çapı","tr":"Hidrant Çapı"}, type: "number", unit: "mm", required: true, smartDefault: 100, validation: { min: 25 }, helper: "", expertMeaning: "Hydrant outlet diameter", expertMeaning_i18n: {"en":"Hydrant outlet diameter","tr":"Hydrant outlet diameter"} },
    { id: "staticPressure", label: "Statik Basınç", label_i18n: {"en":"Statik Basınç","tr":"Statik Basınç"}, type: "number", unit: "bar", required: true, smartDefault: 4, validation: { min: 0.5 }, helper: "", expertMeaning: "Static pressure at hydrant", expertMeaning_i18n: {"en":"Static pressure at hydrant","tr":"Static pressure at hydrant"} },
    { id: "residualPressure", label: "Rezidüel Basınç", label_i18n: {"en":"Rezidüel Basınç","tr":"Rezidüel Basınç"}, type: "number", unit: "bar", required: true, smartDefault: 2.5, validation: { min: 0.1 }, helper: "", expertMeaning: "Residual pressure during flow", expertMeaning_i18n: {"en":"Residual pressure during flow","tr":"Residual pressure during flow"} },
    { id: "flowTestDuration", label: "Test Süresi", label_i18n: {"en":"Test Süresi","tr":"Test Süresi"}, type: "number", unit: "dk", required: false, smartDefault: 5, validation: { min: 1 }, helper: "", expertMeaning: "Flow test duration", expertMeaning_i18n: {"en":"Flow test duration","tr":"Flow test duration"} },
    ...
    { id: "numHydrants", label: "Hidrant Sayısı", label_i18n: {"en":"Hidrant Sayısı","tr":"Hidrant Sayısı"}, type: "number", unit: "adet", required: false, smartDefault: 10, validation: { min: 1 }, helper: "", expertMeaning: "Total hydrants in facility", expertMeaning_i18n: {"en":"Total hydrants in facility","tr":"Total hydrants in facility"} },
    { id: "complianceFine", label: "Uyumsuzluk Cezası", label_i18n: {"en":"Uyumsuzluk Cezası","tr":"Uyumsuzluk Cezası"}, type: "number", unit: "USD", required: false, smartDefault: 5000, validation: { min: 0 }, helper: "", expertMeaning: "Penalty per non-compliant hydrant", expertMeaning_i18n: {"en":"Penalty per non-compliant hydrant","tr":"Penalty per non-compliant hydrant"} },
  ],
  outputs: [
    ...
    { id: "availableFlow", label: "Kullanılabilir Akış", label_i18n: {"en":"Kullanılabilir Akış","tr":"Kullanılabilir Akış"}, unit: "L/dk", format: "number" },
