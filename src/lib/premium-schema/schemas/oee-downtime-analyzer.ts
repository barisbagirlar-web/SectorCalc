...
export const OEE_DOWNTIME_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "plannedProdTime", label: "Planlı Üretim Süresi", label_i18n: {"en":"Planned Production Time","tr":"Planlı Üretim Süresi", type: "number", unit: "saat/vardiya", required: true, smartDefault: 480, validation: { min: 1 }, helper: "", expertMeaning: "Planned production time per shift", expertMeaning_i18n: {"en":"Planned production time per shift","tr":"Planned production time per shift" },
    { id: "downtimeMinutes", label: "Toplam Durma Süresi", label_i18n: {"en":"Total Downtime","tr":"Toplam Durma Süresi", type: "number", unit: "dakika/vardiya", required: true, smartDefault: 60, validation: { min: 0 }, helper: "", expertMeaning: "Total downtime per shift", expertMeaning_i18n: {"en":"Total downtime per shift","tr":"Total downtime per shift" },
    ...
    { id: "totalParts", label: "Üretilen Toplam Parça", label_i18n: {"en":"Total Parts Produced","tr":"Üretilen Toplam Parça", type: "number", unit: "adet", required: true, smartDefault: 800, validation: { min: 1 }, helper: "", expertMeaning: "Total parts produced", expertMeaning_i18n: {"en":"Total parts produced","tr":"Total parts produced" },
    { id: "goodParts", label: "Sağlam Parça Sayısı", label_i18n: {"en":"Good Parts Count","tr":"Sağlam Parça Sayısı", type: "number", unit: "adet", required: true, smartDefault: 760, validation: { min: 0 }, helper: "", expertMeaning: "Good (defect-free) parts", expertMeaning_i18n: {"en":"Good (defect-free) parts","tr":"Good (defect-free) parts" },
    { id: "machineHourlyCost", label: "Makine Saatlik Maliyeti", label_i18n: {"en":"Machine Hourly Cost","tr":"Makine Saatlik Maliyeti", type: "number", unit: "USD/saat", required: true, smartDefault: 120, validation: { min: 0.01 }, helper: "", expertMeaning: "Machine cost per hour", expertMeaning_i18n: {"en":"Machine cost per hour","tr":"Machine cost per hour" },
    { id: "numShiftsYear", label: "Yıllık Vardiya Sayısı", label_i18n: {"en":"Annual Shift Count","tr":"Yıllık Vardiya Sayısı", type: "number", unit: "vardiya/yıl", required: false, smartDefault: 720, validation: { min: 1 }, helper: "", expertMeaning: "Shifts per year", expertMeaning_i18n: {"en":"Shifts per year","tr":"Shifts per year" },
  ],
  outputs: [
