...
export const MATERIAL_REPLACEMENT_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "currentMaterialWeight", label: "Mevcut Malzeme Ağırlık", label_i18n: {"en":"Mevcut Malzeme Ağırlık","tr":"Mevcut Malzeme Ağırlık"}, type: "number", unit: "kg", required: true, smartDefault: 100, validation: { min: 0.1 }, helper: "", expertMeaning: "Current material weight", expertMeaning_i18n: {"en":"Current material weight","tr":"Current material weight"} },
    ...
    { id: "alternativeWeight", label: "Alternatif Malzeme Ağırlık", label_i18n: {"en":"Alternatif Malzeme Ağırlık","tr":"Alternatif Malzeme Ağırlık"}, type: "number", unit: "kg", required: true, smartDefault: 60, validation: { min: 0.1 }, helper: "", expertMeaning: "Alternative material weight", expertMeaning_i18n: {"en":"Alternative material weight","tr":"Alternative material weight"} },
    { id: "productionVolume", label: "Yıllık Üretim Hacmi", label_i18n: {"en":"Yıllık Üretim Hacmi","tr":"Yıllık Üretim Hacmi"}, type: "number", unit: "adet", required: true, smartDefault: 50000, validation: { min: 1 }, helper: "", expertMeaning: "Annual production volume", expertMeaning_i18n: {"en":"Annual production volume","tr":"Annual production volume"} },
    { id: "toolingCost", label: "Takım/Geçiş Maliyeti", label_i18n: {"en":"Takım/Geçiş Maliyeti","tr":"Takım/Geçiş Maliyeti"}, type: "number", unit: "USD", required: false, smartDefault: 15000, validation: { min: 0 }, helper: "", expertMeaning: "Tooling or transition cost", expertMeaning_i18n: {"en":"Tooling or transition cost","tr":"Tooling or transition cost"} },
  ],
  outputs: [
    ...
    { id: "weightSavings", label: "Ağırlık Tasarrufu", label_i18n: {"en":"Ağırlık Tasarrufu","tr":"Ağırlık Tasarrufu"}, unit: "kg/yıl", format: "number" },
    { id: "netBenefit", label: "Net Yıllık Fayda", label_i18n: {"en":"Net Yıllık Fayda","tr":"Net Yıllık Fayda"}, unit: "USD/yıl", format: "currency" },
    { id: "paybackYears", label: "Geri Ödeme Süresi", label_i18n: {"en":"Geri Ödeme Süresi","tr":"Geri Ödeme Süresi"}, unit: "yıl", format: "number" },
  ],
