...
export const CPK_PPM_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "CPK → PPM & Sigma Seviye Dönüştürücü", name_i18n: {"en":"CPK → PPM & Sigma Seviye Dönüştürücü","tr":"CPK → PPM & Sigma Seviye Dönüştürücü"}, sectorSlug: "cnc-manufacturing", category: "measurement",
  ...
  inputs: [
    { id: "usl", label: "Üst Spesifikasyon Limiti (USL)", label_i18n: {"en":"Üst Spesifikasyon Limiti (USL)","tr":"Üst Spesifikasyon Limiti (USL)"}, type: "number", unit: "", required: true, smartDefault: 10.05, validation: { min: 0 }, helper: "", expertMeaning: "Upper specification limit", expertMeaning_i18n: {"en":"Upper specification limit","tr":"Upper specification limit"} },
    ...
    { id: "mean", label: "Proses Ortalaması", label_i18n: {"en":"Proses Ortalaması","tr":"Proses Ortalaması"}, type: "number", unit: "", required: true, smartDefault: 10.0, validation: { min: 0 }, helper: "", expertMeaning: "Process mean", expertMeaning_i18n: {"en":"Process mean","tr":"Process mean"} },
    ...
    { id: "dailyVolume", label: "Günlük Üretim Hacmi", label_i18n: {"en":"Günlük Üretim Hacmi","tr":"Günlük Üretim Hacmi"}, type: "number", unit: "adet", required: false, smartDefault: 1000, validation: { min: 0 }, helper: "", expertMeaning: "Daily production volume", expertMeaning_i18n: {"en":"Daily production volume","tr":"Daily production volume"} },
  ],
  outputs: [
    ...
    { id: "sigmaShort", label: "Sigma Seviyesi (Kısa Dönem)", label_i18n: {"en":"Sigma Seviyesi (Kısa Dönem)","tr":"Sigma Seviyesi (Kısa Dönem)"}, unit: "", format: "number" },
    { id: "expectedDefectsDaily", label: "Tahmini Günlük Hata", label_i18n: {"en":"Tahmini Günlük Hata","tr":"Tahmini Günlük Hata"}, unit: "adet/gün", format: "number", isBigNumber: true },
  ],
  thresholds: [
