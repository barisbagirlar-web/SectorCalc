...
export const FUEL_ROUTE_DRIFT_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Yakıt Rota Sapma Maliyeti", name_i18n: {"en":"Yakıt Rota Sapma Maliyeti","tr":"Yakıt Rota Sapma Maliyeti"}, sectorSlug: "logistics-transport", category: "cost",
  ...
  inputs: [
    ...
    { id: "actualDistance", label: "Gerçekleşen Rota Mesafesi", label_i18n: {"en":"Gerçekleşen Rota Mesafesi","tr":"Gerçekleşen Rota Mesafesi"}, type: "number", unit: "km", required: true, smartDefault: 250, validation: { min: 1 }, helper: "", expertMeaning: "Actual route distance", expertMeaning_i18n: {"en":"Actual route distance","tr":"Actual route distance"} },
    { id: "fuelCostPerLiter", label: "Yakıt Birim Maliyeti", label_i18n: {"en":"Yakıt Birim Maliyeti","tr":"Yakıt Birim Maliyeti"}, type: "number", unit: "USD/L", required: true, smartDefault: 1.5, validation: { min: 0.01 }, helper: "", expertMeaning: "Fuel cost per liter", expertMeaning_i18n: {"en":"Fuel cost per liter","tr":"Fuel cost per liter"} },
    { id: "fuelEfficiency", label: "Yakıt Tüketimi", label_i18n: {"en":"Yakıt Tüketimi","tr":"Yakıt Tüketimi"}, type: "number", unit: "km/L", required: true, smartDefault: 4, validation: { min: 0.1 }, helper: "", expertMeaning: "Vehicle fuel efficiency", expertMeaning_i18n: {"en":"Vehicle fuel efficiency","tr":"Vehicle fuel efficiency"} },
    { id: "numTrips", label: "Sefer Sayısı / Yıl", label_i18n: {"en":"Sefer Sayısı / Yıl","tr":"Sefer Sayısı / Yıl"}, type: "number", unit: "sefer/yıl", required: true, smartDefault: 250, validation: { min: 1 }, helper: "", expertMeaning: "Annual trip count", expertMeaning_i18n: {"en":"Annual trip count","tr":"Annual trip count"} },
    { id: "idleTimePerTrip", label: "Rötar Süresi / Sefer", label_i18n: {"en":"Rötar Süresi / Sefer","tr":"Rötar Süresi / Sefer"}, type: "number", unit: "dk", required: false, smartDefault: 20, validation: { min: 0 }, helper: "", expertMeaning: "Idle time per trip due to drift", expertMeaning_i18n: {"en":"Idle time per trip due to drift","tr":"Idle time per trip due to drift"} },
    { id: "driverCostPerHour", label: "Sürücü Saat Ücreti", label_i18n: {"en":"Sürücü Saat Ücreti","tr":"Sürücü Saat Ücreti"}, type: "number", unit: "USD/saat", required: false, smartDefault: 25, validation: { min: 1 }, helper: "", expertMeaning: "Driver cost per hour", expertMeaning_i18n: {"en":"Driver cost per hour","tr":"Driver cost per hour"} },
    { id: "fleetSize", label: "Araç Sayısı", label_i18n: {"en":"Araç Sayısı","tr":"Araç Sayısı"}, type: "number", unit: "araç", required: false, smartDefault: 10, validation: { min: 1 }, helper: "", expertMeaning: "Fleet size", expertMeaning_i18n: {"en":"Fleet size","tr":"Fleet size"} },
  ],
  outputs: [
    { id: "fuelWasteDistance", label: "Sapma Mesafesi", label_i18n: {"en":"Sapma Mesafesi","tr":"Sapma Mesafesi"}, unit: "km/sefer", format: "number" },
    { id: "fuelWasteEfficiency", label: "Yakıt İsrafı", label_i18n: {"en":"Yakıt İsrafı","tr":"Yakıt İsrafı"}, unit: "L/sefer", format: "number" },
    { id: "idleFuelCost", label: "Rötar Yakıt Maliyeti", label_i18n: {"en":"Rötar Yakıt Maliyeti","tr":"Rötar Yakıt Maliyeti"}, unit: "USD/yıl", format: "currency" },
