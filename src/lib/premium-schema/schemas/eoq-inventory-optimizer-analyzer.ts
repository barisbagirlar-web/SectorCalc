...
export const EOQ_INVENTORY_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "annualDemand", label: "Yıllık Talep", label_i18n: {"en":"Yıllık Talep","tr":"Yıllık Talep"}, type: "number", unit: "adet", required: true, smartDefault: 10000, validation: { min: 1 }, helper: "", expertMeaning: "Annual demand", expertMeaning_i18n: {"en":"Annual demand","tr":"Annual demand"} },
    { id: "orderCost", label: "Sipariş Maliyeti", label_i18n: {"en":"Sipariş Maliyeti","tr":"Sipariş Maliyeti"}, type: "number", unit: "USD/sipariş", required: true, smartDefault: 50, validation: { min: 1 }, helper: "", expertMeaning: "Cost per order", expertMeaning_i18n: {"en":"Cost per order","tr":"Cost per order"} },
    { id: "holdingCost", label: "Taşıma Maliyeti", label_i18n: {"en":"Taşıma Maliyeti","tr":"Taşıma Maliyeti"}, type: "number", unit: "USD/adet/yıl", required: true, smartDefault: 2, validation: { min: 0.01 }, helper: "", expertMeaning: "Holding cost per unit", expertMeaning_i18n: {"en":"Holding cost per unit","tr":"Holding cost per unit"} },
    { id: "leadTime", label: "Teslim Süresi", label_i18n: {"en":"Teslim Süresi","tr":"Teslim Süresi"}, type: "number", unit: "gün", required: true, smartDefault: 7, validation: { min: 1 }, helper: "", expertMeaning: "Lead time in days", expertMeaning_i18n: {"en":"Lead time in days","tr":"Lead time in days"} },
  ...
  outputs: [
    { id: "eoq", label: "EOQ (Optimum Sipariş)", label_i18n: {"en":"EOQ (Optimum Sipariş)","tr":"EOQ (Optimum Sipariş)"}, unit: "adet", format: "number" },
    { id: "safetyStock", label: "Güvenlik Stoğu", label_i18n: {"en":"Güvenlik Stoğu","tr":"Güvenlik Stoğu"}, unit: "adet", format: "number" },
    { id: "rop", label: "Yeniden Sipariş Noktası (ROP)", label_i18n: {"en":"Yeniden Sipariş Noktası (ROP)","tr":"Yeniden Sipariş Noktası (ROP)"}, unit: "adet", format: "number" },
    ...
    { id: "inventoryTurnover", label: "Stok Devir Hızı", label_i18n: {"en":"Stok Devir Hızı","tr":"Stok Devir Hızı"}, unit: "tur/yıl", format: "number", isBigNumber: true },
  ],
