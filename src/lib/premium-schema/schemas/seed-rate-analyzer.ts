...
export const SEED_RATE_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Tohum Oranı ve Maliyet Analizi", name_i18n: {"en":"Tohum Oranı ve Maliyet Analizi","tr":"Tohum Oranı ve Maliyet Analizi"}, sectorSlug: "food", category: "cost",
  painStatement: "Tohum oranı ve çimlenme kaybı hesaplanmazsa ekim maliyeti gereksiz artar ve verim düşer.", painStatement_i18n: {"en":"Tohum oranı ve çimlenme kaybı hesaplanmazsa ekim maliyeti gereksiz artar ve verim düşer.","tr":"Tohum oranı ve çimlenme kaybı hesaplanmazsa ekim maliyeti gereksiz artar ve verim düşer."},
  inputs: [
  ...
  outputs: [
    { id: "seedRequirement", label: "Toplam Tohum İhtiyacı", label_i18n: {"en":"Toplam Tohum İhtiyacı","tr":"Toplam Tohum İhtiyacı"}, unit: "kg", format: "number" },
    ...
    { id: "seedFinancialLoss", label: "Fire ve Çimlenme Kaybı", label_i18n: {"en":"Fire ve Çimlenme Kaybı","tr":"Fire ve Çimlenme Kaybı"}, unit: "USD", format: "currency" },
  ],
  thresholds: [{ fieldId: "seedFinancialLoss", warning: 1000, critical: 3000, direction: "higher_is_bad", warningMessage: "Tohum kaybı > $1K — ekim hassasiyeti artırılmalı.", warningMessage_i18n: {"en":"Tohum kaybı > $1K — ekim hassasiyeti artırılmalı.","tr":"Tohum kaybı > $1K — ekim hassasiyeti artırılmalı."}, criticalMessage: "Tohum kaybı > $3K — tohum kalitesi ve ekim yöntemi gözden geçirilmeli.", criticalMessage_i18n: {"en":"Tohum kaybı > $3K — tohum kalitesi ve ekim yöntemi gözden geçirilmeli.","tr":"Tohum kaybı > $3K — tohum kalitesi ve ekim yöntemi gözden geçirilmeli."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Seed Rate & Cost Report", title_i18n: {"en":"Seed Rate & Cost Report","tr":"Seed Rate & Cost Report"}, sections: ["executive_summary", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 5, targetMarginPercent: 20, assumptionNotes: ["Seed req. = Area × Rate / (Germ% × (1−Waste%)).", "Total cost = Requirement × UnitCost.", "Loss = Cost × (1 − Germ%) + Waste adjustment."],assumptionNotes_i18n:[{"en":"Seed req. = Area × Rate / (Germ% × (1−Waste%)).","tr":"Seed req. = Area × Rate / (Germ% × (1−Waste%))."},{"en":"Total cost = Requirement × UnitCost.","tr":"Total cost = Requirement × UnitCost."},{"en":"Loss = Cost × (1 − Germ%) + Waste adjustment.","tr":"Loss = Cost × (1 − Germ%) + Waste adjustment."}] },
};
