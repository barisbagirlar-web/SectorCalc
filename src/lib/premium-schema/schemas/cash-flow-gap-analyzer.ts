...
export const CASH_FLOW_GAP_SCHEMA: PremiumCalculatorSchema = {
  ...
  painStatement: "Nakit akışı açığı, DSO/DPO/DIO dengesi ve nakit döngüsü hesaplanmazsa likidite krizi önceden tespit edilemez.", painStatement_i18n: {"en":"Without calculating DSO/DPO/DIO balance and cash conversion cycle, a liquidity crisis cannot be detected early.","tr":"Nakit akışı açığı, DSO/DPO/DIO dengesi ve nakit döngüsü hesaplanmazsa likidite krizi önceden tespit edilemez."},
  inputs: [
    ...
    { id: "monthlyExpenses", label: "Aylık Giderler", label_i18n: {"en":"Monthly Expenses","tr":"Aylık Giderler", type: "number", unit: "USD", required: true, smartDefault: 85000, validation: { min: 0 }, helper: "", expertMeaning: "Monthly expenses", expertMeaning_i18n: {"en":"Monthly expenses","tr":"Monthly expenses" },
    ...
    { id: "accountsPayable", label: "Borç Bakiyesi", label_i18n: {"en":"Accounts Payable","tr":"Borç Bakiyesi", type: "number", unit: "USD", required: true, smartDefault: 80000, validation: { min: 0 }, helper: "", expertMeaning: "Accounts payable", expertMeaning_i18n: {"en":"Accounts payable","tr":"Accounts payable" },
    { id: "inventoryValue", label: "Stok Değeri", label_i18n: {"en":"Inventory Value","tr":"Stok Değeri", type: "number", unit: "USD", required: true, smartDefault: 120000, validation: { min: 0 }, helper: "", expertMeaning: "Inventory value", expertMeaning_i18n: {"en":"Inventory value","tr":"Inventory value" },
    { id: "cogsMonthly", label: "Aylık Satılan Mal Maliyeti", label_i18n: {"en":"Monthly COGS","tr":"Aylık Satılan Mal Maliyeti", type: "number", unit: "USD", required: true, smartDefault: 60000, validation: { min: 0 }, helper: "", expertMeaning: "Monthly COGS", expertMeaning_i18n: {"en":"Monthly COGS","tr":"Monthly COGS" },
    { id: "numMonths", label: "Projeksiyon Ay Sayısı", label_i18n: {"en":"Projection Months","tr":"Projeksiyon Ay Sayısı", type: "number", unit: "ay", required: false, smartDefault: 6, validation: { min: 1, max: 24 }, helper: "", expertMeaning: "Projection months", expertMeaning_i18n: {"en":"Projection months","tr":"Projection months" },
  ],
  outputs: [
  ...
  thresholds: [{ fieldId: "cashGap", warning: 20000, critical: 50000, direction: "higher_is_bad", warningMessage: "Nakit açığı > $20K — kredi limiti veya factoring değerlendirilmeli.", warningMessage_i18n: {"en":"Cash gap > $20K — consider credit line or factoring.","tr":"Nakit açığı > $20K — kredi limiti veya factoring değerlendirilmeli."}, criticalMessage: "Nakit açığı > $50K — acil nakit yönetimi aksiyonu gerekli.", criticalMessage_i18n: {"en":"Cash gap > $50K — urgent cash management action required.","tr":"Nakit açığı > $50K — acil nakit yönetimi aksiyonu gerekli."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Cash Flow Gap Report", title_i18n: {"en":"Cash Flow Gap Report","tr":"Nakit Akışı Açığı Raporu"}, sections: ["executive_summary", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["DSO = (Alacak/Gelir)×30 gün.", "CCC = DSO + DIO − DPO.", "Negatif kümülatif nakit = nakit açığı."],assumptionNotes_i18n:[{"en":"DSO = (AR/Revenue)×30 days.","tr":"DSO = (Alacak/Gelir)×30 gün."},{"en":"CCC = DSO + DIO − DPO.","tr":"CCC = DSO + DIO − DPO."},{"en":"Negative cumulative cash = cash gap.","tr":"Negatif kümülatif nakit = nakit açığı."}] },
};
