...
export const CONTRACT_INCENTIVE_ANALYZER: PremiumCalculatorSchema = {
  ...
  inputs: [
    ...
    { id: "actualCost", label: "Gerçekleşen Maliyet", label_i18n: {"en":"Actual Cost","tr":"Gerçekleşen Maliyet", type: "number", unit: "USD", required: true, smartDefault: 4800000, validation: { min: 1 }, helper: "", expertMeaning: "Actual cost incurred", expertMeaning_i18n: {"en":"Actual cost incurred","tr":"Actual cost incurred" },
    { id: "targetFee", label: "Hedef Ücret", label_i18n: {"en":"Target Fee","tr":"Hedef Ücret", type: "number", unit: "USD", required: true, smartDefault: 500000, validation: { min: 1 }, helper: "", expertMeaning: "Target fee amount", expertMeaning_i18n: {"en":"Target fee amount","tr":"Target fee amount" },
    ...
    { id: "minFee", label: "Minimum Ücret", label_i18n: {"en":"Minimum Fee","tr":"Minimum Ücret", type: "number", unit: "USD", required: false, smartDefault: 100000, validation: { min: 0 }, helper: "", expertMeaning: "Minimum guaranteed fee", expertMeaning_i18n: {"en":"Minimum guaranteed fee","tr":"Minimum guaranteed fee" },
    { id: "maxFee", label: "Maksimum Ücret", label_i18n: {"en":"Maximum Fee","tr":"Maksimum Ücret", type: "number", unit: "USD", required: false, smartDefault: 800000, validation: { min: 0 }, helper: "", expertMeaning: "Maximum fee cap", expertMeaning_i18n: {"en":"Maximum fee cap","tr":"Maximum fee cap" },
  ],
  outputs: [
