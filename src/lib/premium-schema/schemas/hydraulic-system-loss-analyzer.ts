...
export const HYDRAULIC_LOSS_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Hidrolik Sistem Kayıp & Verim Analizi", name_i18n: {"en":"Hydraulic System Loss & Efficiency Analysis","tr":"Hidrolik Sistem Kayıp & Verim Analizi"}, sectorSlug: "sheet-metal", category: "energy",
  painStatement: "Hidrolik sistemlerde kaçak, sürtünme ve vana kayıpları enerji verimini düşürür ve işletme maliyetini artırır.", painStatement_i18n: {"en":"In hydraulic systems, leakage, friction, and valve losses reduce energy efficiency and increase operating costs.","tr":"Hidrolik sistemlerde kaçak, sürtünme ve vana kayıpları enerji verimini düşürür ve işletme maliyetini artırır."},
  inputs: [
    ...
    { id: "flowRate", label: "Pompa Debisi", label_i18n: {"en":"Pump flow rate","tr":"Pompa Debisi"}, type: "number", unit: "L/dak", required: true, smartDefault: 100, validation: { min: 0.1 }, helper: "", expertMeaning: "Pump flow rate", expertMeaning_i18n: {"en":"Pump flow rate","tr":"Pompa Debisi"} },
  ...
  outputs:  [
    { id: "leakLoss", label: "Kaçak Güç Kaybı", label_i18n: {"en":"Leak Power Loss","tr":"Kaçak Güç Kaybı"}, unit: "kW", format: "number" },
    { id: "heatLoss", label: "Toplam Isıl Kayıp", label_i18n: {"en":"Total Thermal Loss","tr":"Toplam Isıl Kayıp"}, unit: "kW", format: "number" },
    { id: "eff", label: "Sistem Verimi", label_i18n: {"en":"System Efficiency","tr":"Sistem Verimi"}, unit: "%", format: "percentage" },
    { id: "annualEnergyCost", label: "Yıllık Enerji Kayıp Maliyeti", label_i18n: {"en":"Annual Energy Loss Cost","tr":"Yıllık Enerji Kayıp Maliyeti"}, unit: "USD/yıl", format: "currency", isBigNumber: true },
  ],
  thresholds: [{ fieldId: "eff", warning: 80, critical: 70, direction: "lower_is_bad", warningMessage: "Verim < %80 — kaçak ve sürtünme kayıpları azaltılmalı.", warningMessage_i18n: {"en":"Efficiency < 80% — leak and friction losses should be reduced.","tr":"Verim < %80 — kaçak ve sürtünme kayıpları azaltılmalı."}, criticalMessage: "Verim < %70 — sistem revizyonu gerekli.", criticalMessage_i18n: {"en":"Efficiency < 70% — system overhaul required.","tr":"Verim < %70 — sistem revizyonu gerekli."} }],
  formulaPipeline: [
  ...
  reportTemplate: { title: "Hydraulic System Loss Report", title_i18n: {"en":"Hydraulic System Loss Report","tr":"Hidrolik Sistem Kayıp Raporu"}, sections: ["executive_summary", "thresholds", "action_plan", "assumptions"], exportFormats: ["pdf", "excel"] },
