...
export const IRR_INVESTMENT_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "İç Verim Oranı (IRR) & Yatırım Analizi", name_i18n: {"en":"ic Verim Orani (IRR) & Yatirim Analizi","tr":"İç Verim Oranı (IRR) & Yatırım Analizi"}, sectorSlug: "financial-planning", category: "cost",
  ...
  inputs: [
    ...
    { id: "discountRate", label: "İskonto Oranı (WACC)", label_i18n: {"en":"Discount rate","tr":"İskonto Oranı (WACC)"}, type: "number", unit: "%", required: true, smartDefault: 10, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Discount rate", expertMeaning_i18n: {"en":"Discount rate","tr":"i̇skonto oranı (wacc)"} },
  ...
  outputs: [
    { id: "npv", label: "NPV (Net Bugünkü Değer)", label_i18n: {"en":"NPV (Net Bugunku Deger)","tr":"NPV (Net Bugünkü Değer)"}, unit: "USD", format: "currency" },
    { id: "irr", label: "IRR (İç Verim Oranı)", label_i18n: {"en":"IRR (Ic Verim Oran)","tr":"IRR (İç Verim Oranı)"}, unit: "%", format: "percentage" },
    { id: "payback", label: "Geri Ödeme Süresi", label_i18n: {"en":"Geri Odeme Suresi","tr":"Geri Ödeme Süresi"}, unit: "yıl", format: "number" },
    { id: "profitabilityIndex", label: "Kârlılık İndeksi (PI)", label_i18n: {"en":"Karllk Indeksi (PI)","tr":"Kârlılık İndeksi (PI)"}, unit: "", format: "number", isBigNumber: true },
  ],
