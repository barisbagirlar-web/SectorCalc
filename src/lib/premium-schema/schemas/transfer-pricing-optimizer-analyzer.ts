...
export const TRANSFER_PRICING_OPTIMIZER_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Transfer Fiyatlandırması Optimizasyonu", name_i18n: {"en":"Transfer Fiyatlandirmasi Optimizasyonu","tr":"Transfer Fiyatlandırması Optimizasyonu"}, sectorSlug: "financial-planning", category: "cost",
  ...
  inputs: [
    { id: "entityALocation", label: "A Şirketi Ülke", label_i18n: {"en":"Entity A country","tr":"A Şirketi Ülke"}, type: "select", unit: "", required: true, smartDefault: "TR", validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Entity A country", expertMeaning_i18n: {"en":"Entity A country","tr":"a şirketi ülke"}, enumValues: ["TR", "DE", "US", "UK", "CN", "AE", "NL", "SG"] },
    { id: "entityBLocation", label: "B Şirketi Ülke", label_i18n: {"en":"Entity B country","tr":"B Şirketi Ülke"}, type: "select", unit: "", required: true, smartDefault: "DE", validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Entity B country", expertMeaning_i18n: {"en":"Entity B country","tr":"b şirketi ülke"}, enumValues: ["TR", "DE", "US", "UK", "CN", "AE", "NL", "SG"] },
    { id: "transferAmount", label: "Transfer Tutarı", label_i18n: {"en":"Intercompany transfer amount","tr":"Transfer Tutarı"}, type: "number", unit: "USD", required: true, smartDefault: 500000, validation: { min: 1 }, helper: "", expertMeaning: "Intercompany transfer amount", expertMeaning_i18n: {"en":"Intercompany transfer amount","tr":"transfer tutarı"} },
    { id: "entityATaxRate", label: "A Şirketi Vergi Oranı", label_i18n: {"en":"Entity A tax rate","tr":"A Şirketi Vergi Oranı"}, type: "number", unit: "%", required: true, smartDefault: 25, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Entity A tax rate", expertMeaning_i18n: {"en":"Entity A tax rate","tr":"a şirketi vergi oranı"} },
    { id: "entityBTaxRate", label: "B Şirketi Vergi Oranı", label_i18n: {"en":"Entity B tax rate","tr":"B Şirketi Vergi Oranı"}, type: "number", unit: "%", required: true, smartDefault: 30, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Entity B tax rate", expertMeaning_i18n: {"en":"Entity B tax rate","tr":"b şirketi vergi oranı"} },
    { id: "transferPrice", label: "Transfer Fiyatı", label_i18n: {"en":"Actual transfer price","tr":"Transfer Fiyatı"}, type: "number", unit: "USD", required: false, smartDefault: 550000, validation: { min: 0 }, helper: "", expertMeaning: "Actual transfer price", expertMeaning_i18n: {"en":"Actual transfer price","tr":"transfer fiyatı"} },
    { id: "armLengthPrice", label: "Piyasa (Arm's Length) Fiyatı", label_i18n: {"en":"Arm's length market price","tr":"Piyasa (Arm's Length) Fiyatı"}, type: "number", unit: "USD", required: false, smartDefault: 520000, validation: { min: 0 }, helper: "", expertMeaning: "Arm's length market price", expertMeaning_i18n: {"en":"Arm's length market price","tr":"piyasa (arm's length) fiyatı"} },
  ],
  outputs: [
