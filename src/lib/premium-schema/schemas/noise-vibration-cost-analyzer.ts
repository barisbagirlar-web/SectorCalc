...
export const NOISE_VIBRATION_COST_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "Noise & Vibration Maliyet Analizi", name_i18n: {"en":"Noise & Vibration Cost Analysis","tr":"Noise & Vibration Maliyet Analizi"}, sectorSlug: "quality", category: "cost",
  painStatement: "Gürültü ve titreşim maruziyetinin sağlık, verimlilik ve kalite maliyeti hesaplanmazsa iyileştirme yatırımı için fizibilite kurulamaz.", painStatement_i18n: {"en":"Without calculating the health, productivity, and quality costs of noise and vibration exposure, improvement investment feasibility cannot be established.","tr":"Gürültü ve titreşim maruziyetinin sağlık, verimlilik ve kalite maliyeti hesaplanmazsa iyileştirme yatırımı için fizibilite kurulamaz."},
  inputs: [
  ...
  outputs:  [
    { id: "noiseExposure", label: "Gürültü Maruziyet Skoru", label_i18n: {"en":"Noise Exposure Score","tr":"Gürültü Maruziyet Skoru"}, unit: "dB(A)×saat", format: "number" },
    { id: "vibrationLevel", label: "Titreşim Seviyesi", label_i18n: {"en":"Vibration Level","tr":"Titreşim Seviyesi"}, unit: "m/s²", format: "number" },
    { id: "healthCost", label: "Sağlık Maliyeti (Gürültü)", label_i18n: {"en":"Health Cost (Noise)","tr":"Sağlık Maliyeti (Gürültü)"}, unit: "USD/yıl", format: "currency" },
    { id: "productivityLoss", label: "Verimlilik Kaybı", label_i18n: {"en":"Productivity Loss","tr":"Verimlilik Kaybı"}, unit: "USD/yıl", format: "currency" },
    { id: "reworkCost", label: "Hata/Revizyon Maliyeti", label_i18n: {"en":"Error/Rework Cost","tr":"Hata/Revizyon Maliyeti"}, unit: "USD/yıl", format: "currency" },
    { id: "mitigationRoi", label: "Azaltma ROI", label_i18n: {"en":"Mitigation ROI","tr":"Azaltma ROI"}, unit: "%", format: "number", isBigNumber: true },
  ],
  thresholds: [{ fieldId: "mitigationRoi", warning: 50, critical: 20, direction: "lower_is_bad", warningMessage: "ROI < %50 — yatırım fizibilitesi riskli.", warningMessage_i18n: {"en":"ROI < 50% — investment feasibility is risky.","tr":"ROI < %50 — yatırım fizibilitesi riskli."}, criticalMessage: "ROI < %20 — azaltma yatırımı önerilmez.", criticalMessage_i18n: {"en":"ROI < 20% — mitigation investment not recommended.","tr":"ROI < %20 — azaltma yatırımı önerilmez."} }],
  formulaPipeline: [
  ...
  assumptions: { hiddenLossMultiplier: 1.1, volatilityPercent: 10, targetMarginPercent: 15, assumptionNotes: ["85 dB(A) üzeri gürültü sağlık riski oluşturur.", "Titreşim RMS > 2.5 m/s² el-kol vibrasyon riski.", "Sağlık maliyeti = işitme kaybı + tazminat riski."],assumptionNotes_i18n:[{"en":"Noise above 85 dB(A) poses health risk.","tr":"85 dB(A) üzeri gürültü sağlık riski oluşturur."},{"en":"Vibration RMS > 2.5 m/s² creates hand-arm vibration risk.","tr":"Titreşim RMS > 2.5 m/s² el-kol vibrasyon riski."},{"en":"Health cost = hearing loss + compensation risk.","tr":"Sağlık maliyeti = işitme kaybı + tazminat riski."}] },
};
