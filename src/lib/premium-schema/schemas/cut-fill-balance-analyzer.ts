...
export const CUT_FILL_BALANCE_SCHEMA: PremiumCalculatorSchema = {
  ...
  inputs: [
    { id: "cutVolume", label: "Kazı Hacmi", label_i18n: {"en":"Kazı Hacmi","tr":"Kazı Hacmi"}, type: "number", unit: "m³", required: true, smartDefault: 5000, validation: { min: 0 }, helper: "", expertMeaning: "Total cut volume", expertMeaning_i18n: {"en":"Total cut volume","tr":"Total cut volume"} },
    ...
    { id: "shrinkageFactor", label: "Sıkışma Faktörü", label_i18n: {"en":"Sıkışma Faktörü","tr":"Sıkışma Faktörü"}, type: "number", unit: "", required: false, smartDefault: 0.85, validation: { min: 0, max: 1 }, helper: "", expertMeaning: "Compaction shrinkage", expertMeaning_i18n: {"en":"Compaction shrinkage","tr":"Compaction shrinkage"} },
    { id: "swellFactor", label: "Kabarma Faktörü", label_i18n: {"en":"Kabarma Faktörü","tr":"Kabarma Faktörü"}, type: "number", unit: "", required: false, smartDefault: 1.25, validation: { min: 1 }, helper: "", expertMeaning: "Material swell factor", expertMeaning_i18n: {"en":"Material swell factor","tr":"Material swell factor"} },
    { id: "haulCost", label: "Nakliye Birim Fiyatı", label_i18n: {"en":"Nakliye Birim Fiyatı","tr":"Nakliye Birim Fiyatı"}, type: "number", unit: "USD/m³-km", required: true, smartDefault: 2.5, validation: { min: 0 }, helper: "", expertMeaning: "Haul cost per m³-km", expertMeaning_i18n: {"en":"Haul cost per m³-km","tr":"Haul cost per m³-km"} },
    { id: "borrowDistance", label: "Ödünç Malzeme Mesafesi", label_i18n: {"en":"Ödünç Malzeme Mesafesi","tr":"Ödünç Malzeme Mesafesi"}, type: "number", unit: "km", required: false, smartDefault: 10, validation: { min: 0 }, helper: "", expertMeaning: "Borrow pit distance", expertMeaning_i18n: {"en":"Borrow pit distance","tr":"Borrow pit distance"} },
    { id: "wasteDistance", label: "Depo Sahası Mesafesi", label_i18n: {"en":"Depo Sahası Mesafesi","tr":"Depo Sahası Mesafesi"}, type: "number", unit: "km", required: false, smartDefault: 5, validation: { min: 0 }, helper: "", expertMeaning: "Waste site distance", expertMeaning_i18n: {"en":"Waste site distance","tr":"Waste site distance"} },
  ],
  outputs: [
    { id: "netBalance", label: "Net Denge", label_i18n: {"en":"Net Denge","tr":"Net Denge"}, unit: "m³", format: "number" },
    { id: "borrowRequired", label: "Ödünç Malzeme", label_i18n: {"en":"Ödünç Malzeme","tr":"Ödünç Malzeme"}, unit: "m³", format: "number" },
