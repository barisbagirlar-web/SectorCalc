...
export const AQL_SAMPLING_SCHEMA: PremiumCalculatorSchema = {
  ...
  name: "AQL Sampling Risk & Maliyet Analizi", name_i18n: {"en":"AQL Sampling Risk & Maliyet Analizi","tr":"AQL Sampling Risk & Maliyet Analizi"}, sectorSlug: "sheet-metal", category: "scrap",
  ...
  inputs: [
    { id: "lotSize", label: "Parti Büyüklüğü (N)", label_i18n: {"en":"Parti Büyüklüğü (N)","tr":"Parti Büyüklüğü (N)"}, type: "number", unit: "adet", required: true, smartDefault: 1000, validation: { min: 1 }, helper: "Toplam parti büyüklüğü.", helper_i18n: {"en":"Toplam parti büyüklüğü.","tr":"Toplam parti büyüklüğü."}, expertMeaning: "Lot size", expertMeaning_i18n: {"en":"Lot size","tr":"Lot size"} },
    ...
    { id: "sampleSize", label: "Örneklem Büyüklüğü (n)", label_i18n: {"en":"Örneklem Büyüklüğü (n)","tr":"Örneklem Büyüklüğü (n)"}, type: "number", unit: "adet", required: true, smartDefault: 80, validation: { min: 1 }, helper: "Muayene edilecek örnek sayısı.", helper_i18n: {"en":"Muayene edilecek örnek sayısı.","tr":"Muayene edilecek örnek sayısı."}, expertMeaning: "Sample size per ANSI/ASQ Z1.4", expertMeaning_i18n: {"en":"Sample size per ANSI/ASQ Z1.4","tr":"Sample size per ANSI/ASQ Z1.4"} },
    { id: "acceptanceNumber", label: "Kabul Sayısı (Ac)", label_i18n: {"en":"Kabul Sayısı (Ac)","tr":"Kabul Sayısı (Ac)"}, type: "number", unit: "adet", required: true, smartDefault: 2, validation: { min: 0 }, helper: "Maksimum kabul edilebilir hatalı sayısı.", helper_i18n: {"en":"Maksimum kabul edilebilir hatalı sayısı.","tr":"Maksimum kabul edilebilir hatalı sayısı."}, expertMeaning: "Accept number per code letter", expertMeaning_i18n: {"en":"Accept number per code letter","tr":"Accept number per code letter"} },
    { id: "unitInspectionCost", label: "Birim Muayene Maliyeti", label_i18n: {"en":"Birim Muayene Maliyeti","tr":"Birim Muayene Maliyeti"}, type: "number", unit: "USD", required: true, smartDefault: 2, validation: { min: 0.01 }, helper: "", expertMeaning: "Cost per unit inspected", expertMeaning_i18n: {"en":"Cost per unit inspected","tr":"Cost per unit inspected"} },
    { id: "costPerDefectEscaped", label: "Kaçan Hata Maliyeti", label_i18n: {"en":"Kaçan Hata Maliyeti","tr":"Kaçan Hata Maliyeti"}, type: "number", unit: "USD", required: true, smartDefault: 100, validation: { min: 0.01 }, helper: "", expertMeaning: "Cost per defect that escapes", expertMeaning_i18n: {"en":"Cost per defect that escapes","tr":"Cost per defect that escapes"} },
    { id: "actualDefectRate", label: "Ortalama Kusur Oranı (p)", label_i18n: {"en":"Ortalama Kusur Oranı (p)","tr":"Ortalama Kusur Oranı (p)"}, type: "number", unit: "%", required: true, smartDefault: 1.5, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Actual process defect rate", expertMeaning_i18n: {"en":"Actual process defect rate","tr":"Actual process defect rate"} },
    { id: "detectionRate", label: "Tespit Oranı", label_i18n: {"en":"Tespit Oranı","tr":"Tespit Oranı"}, type: "number", unit: "%", required: false, smartDefault: 80, validation: { min: 0, max: 100 }, helper: "", expertMeaning: "Inspection detection effectiveness", expertMeaning_i18n: {"en":"Inspection detection effectiveness","tr":"Inspection detection effectiveness"} },
  ],
  outputs: [
