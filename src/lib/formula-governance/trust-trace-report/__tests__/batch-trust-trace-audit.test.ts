...
describe("batch trust trace audit — Phase 5H-I", () => {
  test("batch audit returns 120 contracts", () => {
    ...
    expect(result.reports).toHaveLength(287);
    expect(result.trustTraceReady + result.needsReview + result.blocked).toBe(287);
  });
  ...
  test("formatted report includes summary lines", () => {
    ...
    expect(report).toContain("Total contracts: 287");
