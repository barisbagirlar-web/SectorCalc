...
describe('stack-pdf-builder', () => {
  it('title first', () => {
    expect(buildStackReportLines(input)[0]!.style).toBe('title');
  });
  it('uses the server CAPABLE verdict', () => {
    expect(
      buildStackReportLines({
        ...input,
        reportData: {
          verdict: 'CAPABLE',
          cpk: '2.70',
      ...
      }).some((l) => l.text.includes('CAPABLE'))
    ).toBe(true);
  });
  it('uses the server NOT CAPABLE verdict', () => {
    expect(
      buildStackReportLines({
        ...input,
        reportData: {
      ...
      }).some((l) => l.text.includes('NOT CAPABLE'))
    ).toBe(true);
  });
  ...
  it('includes risk analysis and standards when reportData present', () => {
    const lines = buildStackReportLines({
      ...input,
      reportData: {
        verdict: 'CAPABLE',
        cpk: '1.5',
        ppm: '10',
        riskAnalysis: [
          { level: 'PASS', code: 'CPK_CAPABLE', message: 'Cpk = 1.5', recommendation: 'maintain' }
        ],
        insights: ['top contributor A'],
