...
export const ALL_CALCULATORS: Record<string, (values: Record<string, any>) => any> = {
  ...
  "discounted-payback-period": (values) => {
      ...
      return {
        ...
        explanation: payback >= 0 
          ...
          : "The total present value of cash flows does not cover the initial investment cost.",
  ...
  "rent-vs-buy-decision-ratio": (values) => {
    ...
    const decisionTr = ratio > 20 ? "Rent" : "Buy";
    return {
  ...
  "espp-discounted-stock-options": (values) => {
    ...
    const discount_pct = normalizeNumber(values.discount);
    ...
    let resultValue: any = 0;
    try {
    const purchasePrice = marketPrice * (1 - discount_pct/100);
  ...
  "convertible-note-shares": (values) => {
    ...
    const discount2 = normalizeNumber(values.discount);
    const interestRate = normalizeNumber(values.interestRate);
    ...
    let resultValue: any = 0;
    try {
    const conversionPrice = valuation * (1 - discount2/100);
  ...
  "welding-heat-input": (values) => {
    ...
    const travelSpeed = normalizeNumber(values.ilerlemehiz);
    ...
    let resultValue: any = 0;
    try {
    const heat = (current * voltage * efficiency) / Math.max(0.0001, travelSpeed);
    resultValue = heat;
      if (typeof resultValue === "number" && !Number.isFinite(resultValue)) {
        resultValue = 0;
      }
  ...
  "heat-conduction-fourier": (values) => {
    const area = normalizeNumber(values.alan);
    ...
    let resultValue: any = 0;
    try {
    const flow = (thermalConductivity * area * temperatureDifference) / Math.max(0.0001, thickness);
    resultValue = flow;
      if (typeof resultValue === "number" && !Number.isFinite(resultValue)) {
        resultValue = 0;
      }
