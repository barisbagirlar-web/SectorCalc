...
export const additionalRevenueTools: AdditionalRevenueTool[] = [
 ...
 "Unlock the Route & Freight Loss Analyzer for deadhead, toll and rest-risk verdict.",
  ...
  buildTool({
    ...
    paidSlug: "construction-project-overrun",
    freeTitle: "Project Cost Calculator",
    paidTitle: "Construction Project Overrun Analyzer",
    painStatement:
      "Unbudgeted material, labor and overhead can overrun construction project estimates before breaking ground.",
    freeValue: "Estimate total project cost from material, labor, overhead and contingency.",
    paidValue: "Model change orders, delay penalties and real margin with overrun verdict.",
    freeInputs: [
      currency("materialCost", "Material cost"),
      numberInput("laborHours", "Labor hours", "hr"),
      numberInput("laborRate", "Labor rate", "USD/hr"),
    ...
    paidInputs: [
      ...
      percentInput("overheadPct", "Overhead", 15),
      percentInput("contingencyPct", "Contingency", 10),
      percentInput("changeOrderRisk", "Change order risk", 8),
      numberInput("delayDays", "Delay exposure", "days", 0),
      percentInput("targetMargin", "Target margin", 18),
    ],
    freeResultPromise: "Shows total estimated cost with overhead and contingency included.",
    paidResultPromise: "Returns overrun-adjusted cost, margin risk and project verdict.",
    verdictLabels: ["HIGH OVERRUN RISK", "MODERATE RISK", "ON BUDGET"],
    seoKeywords: ["project cost calculator", "construction cost estimator"],
    freeMissingFactors: ["Change order risk", "Delay penalties", "Real margin risk", "Overrun verdict"],
    premiumCtaLabel: "Unlock Overrun Analyzer",
    premiumTeaserTitle: "Bidding a construction project?",
    premiumTeaserText:
      "Unlock the Construction Project Overrun Analyzer for change orders and delay risk.",
  }),
  ...
  buildTool({
    ...
    painStatement:
      "Underestimated crew time and visit frequency can turn cleaning contracts into a margin loss.",
    freeValue: "Estimate cost per visit and monthly total from crew size and area.",
    paidValue: "Find minimum safe monthly contract price with supply, travel and rework risk.",
    freeInputs: [
      numberInput("areaSize", "Area size", "sq ft"),
      numberInput("staffCount", "Staff per visit"),
      numberInput("visitFrequency", "Visits per month"),
      currency("hourlyRate", "Hourly rate"),
      numberInput("hoursPerVisit", "Hours per visit", "hr"),
    ],
    paidInputs: [
      numberInput("areaSize", "Area size", "sq ft"),
      numberInput("staffCount", "Staff per visit"),
      numberInput("visitFrequency", "Visits per month"),
      currency("hourlyRate", "Hourly rate"),
      currency("supplyCost", "Supply cost", "USD/month"),
      currency("travelCost", "Travel cost", "USD/month"),
      percentInput("reworkRiskPercent", "Rework risk", 8),
      percentInput("targetMargin", "Target margin", 20),
    ],
    freeResultPromise: "Shows cost per visit and monthly cleaning cost before supplies and travel.",
    paidResultPromise: "Returns minimum safe monthly contract price and contract verdict.",
    verdictLabels: ["UNDERPRICED", "LOW MARGIN", "SAFE CONTRACT"],
    seoKeywords: ["cleaning cost calculator", "office cleaning bid optimizer"],
    freeMissingFactors: ["Supply cost", "Travel cost", "Rework risk", "Minimum contract price"],
    premiumCtaLabel: "Unlock Bid Optimizer",
    premiumTeaserTitle: "Pricing a cleaning contract?",
    premiumTeaserText:
      "Unlock the Office Cleaning Bid Optimizer for minimum safe price and rework risk.",
  }),
  ...
  buildTool({
    ...
    paidTitle: "Return Rate Profit Erosion Tool",
    painStatement:
      "Return costs and hidden fees silently erode product margin that looks healthy at first glance.",
    freeValue: "Check gross margin and effective margin after return cost impact.",
    paidValue: "Model ad spend, platform fee tiers and return rate sensitivity with margin verdict.",
    freeInputs: [
      currency("productPrice", "Selling price"),
      currency("productCost", "Product cost"),
      percentInput("returnRate", "Return rate", 8),
      numberInput("unitsPerPeriod", "Units per period", undefined, 100),
    ],
    paidInputs: [
      currency("productPrice", "Selling price"),
      currency("productCost", "Product cost"),
      percentInput("returnRate", "Return rate", 8),
      numberInput("unitsPerPeriod", "Units per period", undefined, 100),
      percentInput("platformFeeRate", "Platform fee", 12),
      percentInput("paymentFeeRate", "Payment fee", 3),
      currency("adCostPerUnit", "Ad cost per unit"),
      percentInput("targetMargin", "Target margin", 25),
    ],
    freeResultPromise: "Shows gross and effective margin with return cost impact.",
    paidResultPromise: "Returns true profit, break-even ad spend and margin verdict.",
    verdictLabels: ["LEAKING PROFIT", "MARGIN PRESSURE", "HEALTHY MARGIN"],
    seoKeywords: ["product margin calculator", "return rate profit erosion"],
    freeMissingFactors: ["Platform fees", "Ad cost", "Payment processing", "Net margin verdict"],
    premiumCtaLabel: "Unlock Profit Erosion Tool",
    premiumTeaserTitle: "Returns eating your margin?",
    premiumTeaserText:
      "Unlock the Return Rate Profit Erosion Tool for ad spend and platform fee impact.",
  }),
];
