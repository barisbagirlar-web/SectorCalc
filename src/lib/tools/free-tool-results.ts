...
const CALCULATOR_REGISTRY: Record<string, CalculatorFn> = {
  ...
  "paint-coverage-cost-check": (values) => {
    ...
    return {
      headline:
        risk === "HIGH"
          ? "Paint cost per m² is high — review coverage spec and waste"
          : risk === "MEDIUM"
            ? "Paint cost per m² is moderate"
            : "Paint cost per m² within typical range",
      summary: formatCurrency(totalCost) + " (" + formatCurrency(costPerM2) + "/m²)",
      riskLevel: risk,
    };
  },
  ...
  "desi-calculator": (values) => {
    ...
    return {
      ...
      summary: formatNumber(totalVolWeight, 1) + " kg (volumetric)",
      riskLevel: risk,
    };
  },
  ...
  "irrigation-cost-check": (values) => {
    ...
    const monthlyCost = pumpingHours * electricityRate;
  ...
  "carbon-footprint-quick": (values) => {
    ...
    return {
      headline:
        risk === "HIGH"
          ? "High estimated CO₂ — review for CBAM exposure"
          : risk === "MEDIUM"
            ? "Moderate CO₂ estimate"
            : "CO₂ estimate within manageable range",
      summary: formatNumber(totalCO2, 0) + " tCO₂",
      riskLevel: risk,
    };
  },
  ...
  "home-renovation-m2": (values) => {
    ...
    return {
      headline:
        risk === "HIGH"
          ? "Total cost per m² is high — review material spec and scope"
          : risk === "MEDIUM"
            ? "Total cost per m² is moderate"
            : "Total cost per m² within typical range",
      summary: formatCurrency(total) + " (" + formatCurrency(costPerM2) + "/m²)",
      riskLevel: risk,
    };
  },
  ...
  "cleaning-cost-calculator": (values) => {
    ...
    return {
      headline:
        risk === "HIGH"
          ? "Cost per m² per visit is high — optimize crew size or frequency"
          : risk === "MEDIUM"
            ? "Cost per m² is moderate"
            : "Cost per m² within efficient range",
