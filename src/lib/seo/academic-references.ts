...
export function getAcademicReferenceByMRId(mrId: number): AcademicReference | undefined {
  return academicReferences.find((ref) => ref.mrId === mrId);
}

export function getAcademicReferenceById(id: string): AcademicReference | undefined {
  return academicReferences.find((ref) => ref.id === id);
}

export function getAcademicReferencesByRole(role: AcademicReferenceRole): AcademicReference[] {
  return academicReferences.filter((ref) => ref.role === role);
}
