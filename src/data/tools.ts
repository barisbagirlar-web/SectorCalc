...
export const FREE_TOOLS: Tool[] = revenueToolRegistry.tools
  .filter(tool => tool.freeSlug !== "not-applicable")
  .map((tool) => ({
    slug: tool.freeSlug,
    name: tool.freeTitle,
...
export const PREMIUM_TOOLS: Tool[] = revenueToolRegistry.tools
  .filter(tool => tool.paidSlug !== "not-applicable")
  .map((tool) => ({
    slug: tool.paidSlug,
    name: tool.paidTitle,
