...
import { INDEXABLE_PAGES, LEGACY_WWW_ORIGIN, SITE_ORIGIN, TOOL_PAGES, canonicalUrls } from './seo-registry.mjs';
...
function read(path) {
  if (!existsSync(path)) { fail(`missing file: ${path.replace(ROOT + '/', '')}`); return ''; }
  return readFileSync(path, 'utf8');
}
function matches(text, pattern) { return [...text.matchAll(pattern)].map((m) => m[1]); }
...
if (sitemap.includes(LEGACY_WWW_ORIGIN)) fail('sitemap contains legacy www host');
...
for (const value of forbiddenDiscovery) if (sitemap.includes(value)) fail(`sitemap contains retired route: ${value}`);
...
if (!robots.includes(`Sitemap: ${SITE_ORIGIN}/sitemap.xml`)) fail('robots.txt missing canonical sitemap declaration');
if (robots.includes(LEGACY_WWW_ORIGIN)) fail('robots.txt contains legacy www discovery URLs');
...
if (llms.includes(LEGACY_WWW_ORIGIN)) fail('llms.txt contains legacy www URLs');
for (const page of TOOL_PAGES) {
  const url = `${SITE_ORIGIN}${page.path}`;
  if (!llms.includes(url)) fail(`llms.txt missing live tool URL: ${url}`);
}
for (const value of forbiddenDiscovery) if (llms.includes(value)) fail(`llms.txt contains retired route: ${value}`);

const firebaseRaw = read(join(ROOT, 'firebase.json'));
try {
  const firebase = JSON.parse(firebaseRaw);
  const rewrites = firebase?.hosting?.rewrites ?? [];
  if (rewrites.some((rule) => rule?.source === '**' && rule?.destination === '/index.html')) fail('firebase catch-all homepage rewrite creates soft-404 risk');
} catch (error) {
...
console.log(`[PASS] Enterprise SEO guard: ${INDEXABLE_PAGES.length} apex canonical pages, ${TOOL_PAGES.length} live tools, zero known first-party index blockers`);
