...
export function cwvThresholdsFromConfig(config: unknown): CwvThresholds {
  const t = (config as { thresholds?: Partial<CwvThresholds> })?.thresholds ?? {};
  const values = { lcpP75Ms: t.lcpP75Ms, inpP75Ms: t.inpP75Ms, clsP75: t.clsP75 };
  if (!Number.isFinite(values.lcpP75Ms) || !Number.isFinite(values.inpP75Ms) || !Number.isFinite(values.clsP75)) {
    throw new Error('CWV_THRESHOLDS_CONFIG_INVALID');
  }
  return values as CwvThresholds;
}
...
export function findOrphans(nodes: LinkNode[], roots: string[]): string[] {
  ...
  while (queue.length) {
    ...
    visited.add(path);
    for (const target of index.get(path)?.links ?? []) {
      if (index.has(target) && !visited.has(target)) queue.push(target);
    }
  }
...
export function assertAnchorQuality(anchor: string): void {
  const text = anchor.replace(/\s+/g, ' ').trim().toLowerCase();
  if (!text || /^(click here|read more|learn more|here|link|more)$/.test(text)) throw new Error(`GENERIC_ANCHOR ${anchor}`);
}
