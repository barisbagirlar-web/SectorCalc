...
export function validateOffpage(plan: OffpagePlan, ledger: string, config: Config): { errors: string[]; warnings: string[]; infos: string[] } {
  const errors: string[] = []; const warnings: string[] = []; const infos: string[] = [];
  if (plan.disavow && !(plan.manualAction || (plan.provenNegativeSeo && plan.approvalRef && ledger.includes(plan.approvalRef)))) errors.push('INV-13.1 unconditional disavow');
  ...
  if (plan.brandSerpOwnershipPct !== null && plan.brandSerpOwnershipPct < config.thresholds.brandSerpOwnershipWarnPct) warnings.push(`INV-13.5 brand SERP ownership below ${config.thresholds.brandSerpOwnershipWarnPct}%`);
  if (!plan.aiCitationMethodologyDeclared) infos.push('INV-13.6 AI citation methodology not declared');
  return { errors, warnings, infos };
}
function main(): void {
  const siteArg = process.argv.indexOf('--site'); const site = siteArg >= 0 ? process.argv[siteArg + 1] : process.env.SITE_ID;
  ...
  const result = validateOffpage(safe, '', config); if (result.errors.length) { console.error(result.errors.join('\n')); process.exit(1); }
