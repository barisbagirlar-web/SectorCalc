...
for (const file of filesToFix) {
  ...
  content = content.replace(/Yönetici oturumu bulunamadı\. Lütfen repeat giriş yapın\./g, "Admin session not found. Please log in again.");
  ...
  content = content.replace(/Bağlantı hatası\. Lütfen repeat deneyin\./g, "Connection error. Please try again.");
  content = content.replace(/Yönetici erişimi control ediliyor…/g, "Checking admin access...");
  ...
  content = content.replace(/Project Süresi/g, "Project Duration");
  content = content.replace(/Örn: Ocak 2026 – Mayıs 2026/g, "Ex: January 2026 – May 2026");
  content = content.replace(/Useılan Araçlar/g, "Tools Used");
  ...
  content = content.replace(/Hikaye \(Difficulty \/ Çözüm\)/g, "Story (Challenge / Solution)");
  content = content.replace(/Müşterinin profitşılaştığı problemi yazın\.\.\./g, "Write the problem faced by the customer...");
  content = content.replace(/Çözüm \(Solution\)/g, "Solution");
  content = content.replace(/Which modüller useıldı, which standartlar uygulandı\?/g, "Which modules were used, which standards were applied?");
