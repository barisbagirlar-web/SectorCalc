...
for (const bot of discoveryBots) {
  for (const path of publicPaths) if (!isRobotsAllowed(robots, bot, path)) fail(`${bot} must be allowed on public path ${path}`);
  for (const path of privatePaths) if (isRobotsAllowed(robots, bot, path)) fail(`${bot} must be blocked on private path ${path}`);
}
for (const bot of ['GPTBot', 'ClaudeBot', 'CCBot', 'FacebookBot']) {
  for (const path of ['/', '/calculator/bearing-life-l10', '/admin/']) {
    if (isRobotsAllowed(robots, bot, path)) fail(`${bot} must be blocked by training-crawler policy on ${path}`);
  }
}
