 ...
 *
 * Reads data/premium-formulas-batch.txt and generates TypeScript formula
...
function guessFamily(varName, expression) {
  ...
  if (/pressure|flow|vessel|hydraulic|tank|pump|pipe|valve|heat|exchanger|steam|kaynak|weld/i.test(vn)) return "fluid";
...
function guessOutputHint(varName, expression) {
  ...
  if (/time|hour|minute|duration|period|leadtime|cycle|hafta|ay|yil|year|month|week|day|sure/i.test(vn)) return "duration";
  if (/^n$|^k$|count|number|volume|area|quantity|size|weight|mass|distance|adet|miktar/i.test(vn)) return "number";
...
function generateDefinition(toolPrefix, varName, expression) {
  ...
  // Make sure we have reasonable wrapping
