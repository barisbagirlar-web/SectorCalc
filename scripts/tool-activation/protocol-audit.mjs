...
function main() {
  ...
  // Tur 2 hedefleri: Tur 1'deki 5 araç. Onların yeni durumu gösterilecek.
  const targetSlugs = ["age-calculator", "agirlikli-ortalama-hesaplama", "akaryakit-fiyati-ile-yol-butcesi", "amper-kilowatt-kw-cevirici", "arac-amortisman-hesaplama"];
  const targetTools = targetSlugs.map(slug => toolsArray.find(t => t.slug === slug)).filter(Boolean);

  console.log("SECTORCALC TOOL AUDIT PROTOCOL - TUR 2 RAPORU");
  console.log("===============================================\n");
  ...
  for (const tool of targetTools) {
    ...
    console.log(`TUR 2 — ADIM 1: ${step1Pass ? 'PASS' : 'FAIL'}`);
    ...
    console.log(`TUR 2 — ADIM 2: ${step2Pass ? 'PASS' : 'FAIL'}`);
    ...
    console.log(`TUR 2 — ADIM 3: ${step2Pass ? 'PASS' : 'WARN'}`);
    ...
    console.log(`TUR 2 — ADIM 4: ${step4Pass ? 'PASS' : 'FAIL'}`);
    ...
    console.log(`TUR 2 — ADIM 5: ${step5Pass ? 'PASS' : 'FAIL'}`);
