#!/usr/bin/env npx tsx
/**
 * Generates src/lib/i18n/taxonomy-display-labels.generated.ts from taxonomy.ts
 * with 6-locale sector/category labels (en, tr, de, fr, es, ar).
 */
...
type SupportedLocale = "en" | "tr" | "de" | "fr" | "es" | "ar";
type LocaleLabelMap = Readonly<Record<SupportedLocale, string>>;

/** Professional 6-locale labels — extend when adding taxonomy sectors. */
const SECTOR_LOCALE_OVERRIDES: Readonly<Record<string, LocaleLabelMap>> = {
  otomotiv: {
...
const CATEGORY_LOCALE_OVERRIDES: Readonly<Record<string, LocaleLabelMap>> = {
  cost: {
    en: "Cost & Budgeting",
...
function assertSectorCoverage(): void {
  const missing: string[] = [];
  for (const sector of [...SECTORS, OTHER_SECTOR, ALL_TOOLS_SECTOR]) {
    if (!SECTOR_LOCALE_OVERRIDES[sector.id]) {
      missing.push(sector.id);
    }
  }
  if (missing.length > 0) {
    throw new Error(`Missing SECTOR_LOCALE_OVERRIDES for: ${missing.join(", ")}`);
  }
}

function assertCategoryCoverage(): void {
  const missing: string[] = [];
  for (const category of CATEGORIES) {
    if (!CATEGORY_LOCALE_OVERRIDES[category.id]) {
      missing.push(category.id);
    }
  }
  if (!CATEGORY_LOCALE_OVERRIDES.diger) {
    missing.push("diger");
  }
  if (missing.length > 0) {
    throw new Error(`Missing CATEGORY_LOCALE_OVERRIDES for: ${missing.join(", ")}`);
  }
}

function serializeMap(name: string, map: Readonly<Record<string, LocaleLabelMap>>): string {
  const entries = Object.entries(map)
    .map(([key, labels]) => {
      const inner = (Object.entries(labels) as [SupportedLocale, string][])
        .map(([locale, label]) => `    ${locale}: ${JSON.stringify(label)},`)
        .join("\n");
      return `  ${JSON.stringify(key)}: {\n${inner}\n  },`;
    })
    .join("\n");
  return `export const ${name}: Readonly<Record<string, LocaleLabelMap>> = {\n${entries}\n} as const;`;
}

function main(): void {
  assertSectorCoverage();
  assertCategoryCoverage();

  const outPath = path.join(process.cwd(), "src/lib/i18n/taxonomy-display-labels.generated.ts");
  const content = `// AUTO-GENERATED by scripts/generate-taxonomy-display-labels.ts — do not edit.
import type { SupportedLocale } from "@/lib/i18n/locale-config";

export type LocaleLabelMap = Readonly<Record<SupportedLocale, string>>;

${serializeMap("TAXONOMY_SECTOR_DISPLAY_LABELS", SECTOR_LOCALE_OVERRIDES)}

${serializeMap("TAXONOMY_CATEGORY_DISPLAY_LABELS", CATEGORY_LOCALE_OVERRIDES)}
`;
