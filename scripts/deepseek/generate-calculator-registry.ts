...
function isTrustGateActive(slug: string): boolean {
  ...
  // QUARANTINE tools have stub formulas but still compile and run — register them with warning
  return trust.status === "PASS" || trust.status === "WARN" || trust.status === "QUARANTINE";
}

function getTrustStatus(slug: string): string {
  const schemaPath = path.join(SCHEMAS_DIR, `${slug}-schema.json`);
  if (!fs.existsSync(schemaPath)) {
    return "PASS";
  }
...
function main(): void {
  ...
  const loaderEntries = toolFiles
    .map((slug) => {
      const trustStatus = getTrustStatus(slug);
      return `  ${JSON.stringify(slug)}: createLoader(${JSON.stringify(slug)}, ${JSON.stringify(trustStatus)}),`;
    })
    .join("\n");
