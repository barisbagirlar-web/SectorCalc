/* sc-hero-cell.js — SectorCalc Live Cell v24
   Cinema-grade CNC turning cell. Fail-soft forever.
   - Clear 360° orbit (no oval ground shadow, no rear metal bay plate)
   - 4K-feel metals + ACES controlled exposure
   - Spectacular chip/spark trails on probe contact
*/
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float d = length(uv);
        // Hard hot core + thin rim — bright trail, no soft white fog.
        float core = smoothstep(0.12, 0.0, d);
        float rim = smoothstep(0.48, 0.1, d) * (1.0 - core);
        float a = (core * 1.0 + rim * 0.42) * vLife;
        if (a < 0.035) discard;
        vec3 col = mix(vColor, vec3(1.0, 0.97, 0.9), core * 0.62);
        gl_FragColor = vec4(col * (0.85 + core * 1.15), a);
      }
    `
  });
}
...
async function boot() {
  ...
  try {
    ...
    // Invisible shadow catcher only (no visible oval disc / mirror plate).
    const catchMat = THREE.ShadowMaterial
      ? new THREE.ShadowMaterial({ opacity: 0.16 })
      : new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false });
    const shadowCatch = M(new THREE.PlaneGeometry(10, 10), catchMat, 0, -1.48, 0, false, true, -Math.PI / 2, 0, 0);
    shadowCatch.name = 'shadowCatch';
    root.add(shadowCatch);

    // Faint infinite-feel grid — square, not oval; stays under machine for depth.
    const grid = new THREE.GridHelper(9.5, 19, 0x7aa8c8, 0xb4c0cc);
    grid.position.y = -1.475;
    grid.name = 'grid';
    const gm = Array.isArray(grid.material) ? grid.material : [grid.material];
    gm.forEach((m) => { m.transparent = true; m.opacity = 0.16; });
    root.add(grid);
    ...
    // —— BED / WAYS —— (no rear metal bay — full 360° orbit visibility)
    ...
    // —— SPARKS / CHIP TRAILS (spectacular, still no white fog) ——
    const SN = 180;
    ...
    // Dual streak layers: amber chip trails + hot white cores
    ...
    const streakHotGeo = new THREE.BufferGeometry();
    const streakHotPos = new Float32Array(SN * 2 * 3);
    streakHotGeo.setAttribute('position', new THREE.BufferAttribute(streakHotPos, 3));
    const streaksHot = new THREE.LineSegments(streakHotGeo, new THREE.LineBasicMaterial({
      color: 0xffe6a8,
      transparent: true,
      opacity: 0,
      blending: THREE.AdditiveBlending,
    ...
    streaksHot.frustumCulled = false;
    streaksHot.visible = false;
    root.add(streaksHot);
    ...
    function burst(x, y, z) {
      sparkV = [];
      // Tangential chip spray off spinning workpiece — longer, faster trails.
      const spinDir = spin.rotation.x;
      for (let i = 0; i < SN; i++) {
        sparkPos[i * 3] = x + (Math.random() - 0.5) * 0.022;
        sparkPos[i * 3 + 1] = y + (Math.random() - 0.5) * 0.014;
        sparkPos[i * 3 + 2] = z + (Math.random() - 0.5) * 0.022;

        const speed = 0.08 + Math.random() * 0.18;
        const elev = 0.2 + Math.random() * 0.95;
        const az = spinDir + (Math.random() - 0.5) * 1.8 + Math.PI * 0.5;
        sparkV.push([
          Math.cos(az) * Math.sin(elev) * speed * 0.7,
          Math.cos(elev) * speed * 1.15 + 0.03,
          Math.sin(az) * Math.sin(elev) * speed
        ]);

        const hot = Math.random();
        if (hot > 0.82) {
          sparkCol[i * 3] = 1.0; sparkCol[i * 3 + 1] = 0.95; sparkCol[i * 3 + 2] = 0.82;
          sparkSize[i] = 2.6 + Math.random() * 1.6;
        } else if (hot > 0.35) {
          sparkCol[i * 3] = 1.0; sparkCol[i * 3 + 1] = 0.58; sparkCol[i * 3 + 2] = 0.12;
          sparkSize[i] = 1.7 + Math.random() * 1.5;
        } else {
          sparkCol[i * 3] = 0.98; sparkCol[i * 3 + 1] = 0.32; sparkCol[i * 3 + 2] = 0.04;
          sparkSize[i] = 1.15 + Math.random() * 1.1;
        }
      }
      ...
      streaksHot.visible = true;
      sparkT = performance.now();
      sparkLife = preferCalm ? 780 : 980;

      flash.position.set(x, y, z);
      flash.intensity = 2.1;
      flashOrb.position.set(x, y, z);
      flashOrb.material.opacity = 0.5;
    ...
    function loop(now) {
      ...
      if (sparks.visible) {
        ...
        for (let i = 0; i < SN; i++) {
          ...
          sparkPos[i * 3 + 1] += sparkV[i][1] - dt * 1.35;
          sparkPos[i * 3 + 2] += sparkV[i][2];
          sparkV[i][0] *= 0.978;
          sparkV[i][1] *= 0.978;
          sparkV[i][2] *= 0.978;
          // Long amber chip trail
          const tx = sparkV[i][0] * 4.2;
          const ty = sparkV[i][1] * 4.2;
          const tz = sparkV[i][2] * 4.2;
          streakPos[i * 6] = sparkPos[i * 3] - tx;
          streakPos[i * 6 + 1] = sparkPos[i * 3 + 1] - ty;
          streakPos[i * 6 + 2] = sparkPos[i * 3 + 2] - tz;
          streakPos[i * 6 + 3] = sparkPos[i * 3];
          streakPos[i * 6 + 4] = sparkPos[i * 3 + 1];
          streakPos[i * 6 + 5] = sparkPos[i * 3 + 2];
          // Shorter hot core streak
          streakHotPos[i * 6] = ox;
          streakHotPos[i * 6 + 1] = oy;
          streakHotPos[i * 6 + 2] = oz;
          streakHotPos[i * 6 + 3] = sparkPos[i * 3];
          streakHotPos[i * 6 + 4] = sparkPos[i * 3 + 1];
          streakHotPos[i * 6 + 5] = sparkPos[i * 3 + 2];
        }
        sparkGeo.attributes.position.needsUpdate = true;
        streakGeo.attributes.position.needsUpdate = true;
        streakHotGeo.attributes.position.needsUpdate = true;
        streaks.material.opacity = life * 0.95;
        streaksHot.material.opacity = life * 0.7;
        flash.intensity = Math.max(0, 2.1 * life * life);
        flashOrb.material.opacity = life * 0.4;
        flashOrb.scale.setScalar(1 + (1 - life) * 1.35);
        if (el > sparkLife) {
          sparks.visible = false;
          streaks.visible = false;
          streaksHot.visible = false;
