...
for(let i=0;i<DN;i++){dPos[i*3]=(Math.random()-0.5)*9;dPos[i*3+1]=(Math.random()-0.5)*4;dPos[i*3+2]=(Math.random()-0.5)*4;}
...
function loop(now){
  ...
  if(phase==='move'){
    ...
    if(py<st.r+0.09){phase='touch';phaseT=0;
  ...
  if(sparks.visible){
    ...
    for(let i=0;i<SN;i++){pos[i*3]+=sparkV[i][0];pos[i*3+1]+=sparkV[i][1]-el*0.00004;pos[i*3+2]+=sparkV[i][2];}
    sparkGeo.attributes.position.needsUpdate=true;
