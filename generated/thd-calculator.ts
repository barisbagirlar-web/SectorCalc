...
function evaluateAllFormulas(input: Thd_calculatorInput): Record<string, number> {
  ...
  try { const v = input.V2**2 + input.V3**2 + input.V4**2 + input.V5**2 + input.V6**2 + input.V7**2 + input.V8**2; results["sumOfSquares"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["sumOfSquares"] = Number.NaN; }
  try { const v = input.V2**2 + input.V3**2 + input.V4**2 + input.V5**2 + input.V6**2 + input.V7**2 + input.V8**2; results["sumOfSquares_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["sumOfSquares_aux"] = Number.NaN; }
  return results;
}
