...
function evaluateAllFormulas(input: Lhopitals_rule_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numerator_coeff * input.numerator_power * (input.limit_point ** (input.numerator_power - 1)); results["numerator_derivative"] = Number.isFinite(v) ? v : 0; } catch { results["numerator_derivative"] = 0; }
  try { const v = input.denominator_coeff * input.denominator_power * (input.limit_point ** (input.denominator_power - 1)); results["denominator_derivative"] = Number.isFinite(v) ? v : 0; } catch { results["denominator_derivative"] = 0; }
  try { const v = (results["numerator_derivative"] ?? 0) / (results["denominator_derivative"] ?? 0); results["limit_value"] = Number.isFinite(v) ? v : 0; } catch { results["limit_value"] = 0; }
  return results;
}
...
export function calculateLhopitals_rule_calculator(input: Lhopitals_rule_calculatorInput): Lhopitals_rule_calculatorOutput {
  ...
  const totalWasteCost = values["limit_value"] ?? 0;
  const breakdown = {
