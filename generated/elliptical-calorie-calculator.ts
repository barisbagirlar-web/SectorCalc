...
function evaluateAllFormulas(input: Elliptical_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * input.met * (input.duration / 60) * input.correctionFactor; results["totalCalories"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalCalories"] = 0; }
  try { const v = input.weight * input.met * input.correctionFactor / 60; results["caloriesPerMinute"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["caloriesPerMinute"] = 0; }
  try { const v = input.met; results["metUsed"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["metUsed"] = 0; }
  return results;
}
