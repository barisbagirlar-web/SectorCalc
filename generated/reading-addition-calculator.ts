...
function evaluateAllFormulas(input: Reading_addition_calculatorInput): Record<string, number> {
  ...
  try { const v = input.reading1 + input.reading2 + input.reading3 + input.reading4; results["total"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["total"] = Number.NaN; }
  try { const v = (input.reading1 + input.reading2 + input.reading3 + input.reading4) / 4; results["average"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["average"] = Number.NaN; }
  return results;
}
