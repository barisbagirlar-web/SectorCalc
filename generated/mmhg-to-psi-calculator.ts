...
function evaluateAllFormulas(input: Mmhg_to_psi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.pressureMMHG * input.conversionFactor + input.offset; results["psiRaw"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["psiRaw"] = 0; }
  try { const v = input.pressureMMHG * input.conversionFactor + input.offset; results["psiRaw_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["psiRaw_aux"] = 0; }
  return results;
}
...
export function calculateMmhg_to_psi_calculator(input: Mmhg_to_psi_calculatorInput): Mmhg_to_psi_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["psiRaw_aux"]));
  const breakdown = {
