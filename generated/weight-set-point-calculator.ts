...
function evaluateAllFormulas(input: Weight_set_point_calculatorInput): Record<string, number> {
  ...
  try { const v = input.minimumLegalNetWeight + input.safetyFactorZ * input.processStandardDeviation; results["minFillWeight"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["minFillWeight"] = Number.NaN; }
  try { const v = input.minimumLegalNetWeight + input.safetyFactorZ * input.processStandardDeviation; results["minFillWeight_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["minFillWeight_aux"] = Number.NaN; }
  return results;
}
