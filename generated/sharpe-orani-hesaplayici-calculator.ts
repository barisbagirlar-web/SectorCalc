...
function evaluateAllFormulas(input: Sharpe_orani_hesaplayici_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.expectedReturn / 100) * (input.riskFreeRate / 100) * (input.standardDeviation / 100) * input.periodsPerYear; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = (input.expectedReturn / 100) * (input.riskFreeRate / 100) * (input.standardDeviation / 100) * input.periodsPerYear; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
