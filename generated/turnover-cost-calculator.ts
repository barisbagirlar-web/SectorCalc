...
function evaluateAllFormulas(input: Turnover_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = input.num_employees * (input.annual_turnover_rate / 100) * 1 * input.avg_salary; results["direct_labor_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["direct_labor_cost"] = Number.NaN; }
  try { const v = input.num_employees * (input.annual_turnover_rate / 100) * 1 * input.avg_salary * input.productivity_factor; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
...
export const Turnover_cost_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
