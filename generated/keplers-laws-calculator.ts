...
function evaluateAllFormulas(input: Keplers_laws_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.orbitalPeriod**2 * input.massCentralBody)**(1/3); results["semiMajorAxisFromPeriod"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["semiMajorAxisFromPeriod"] = Number.NaN; }
  try { const v = (input.distanceAtAphelion - input.distanceAtPerihelion) / (input.distanceAtAphelion + input.distanceAtPerihelion); results["eccentricityFromDistances"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["eccentricityFromDistances"] = Number.NaN; }
  return results;
}
