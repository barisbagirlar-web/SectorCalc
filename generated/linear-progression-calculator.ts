...
function evaluateAllFormulas(input: Linear_progression_calculatorInput): Record<string, number> {
  ...
  try { const v = input.firstTerm * input.commonDifference * input.numberOfTerms * input.termPosition; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.firstTerm * input.commonDifference * input.numberOfTerms * input.termPosition * (input.decimals); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
