...
function evaluateAllFormulas(input: Linear_progression_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.firstTerm * input.commonDifference * input.numberOfTerms * input.termPosition; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.firstTerm * input.commonDifference * input.numberOfTerms * input.termPosition * (input.decimals); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
