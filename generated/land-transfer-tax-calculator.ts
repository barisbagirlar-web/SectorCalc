...
function evaluateAllFormulas(input: Land_transfer_tax_calculatorInput): Record<string, number> {
  ...
  try { const v = input.propertyValue * (input.taxRate / 100); results["baseTax"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["baseTax"] = 0; }
  try { const v = input.isFirstTimeBuyer * input.exemptionAmount; results["exemptionApplied"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["exemptionApplied"] = 0; }
  try { const v = (asFormulaNumber(results["baseTax"])) - (asFormulaNumber(results["exemptionApplied"])) > 0 ? (asFormulaNumber(results["baseTax"])) - (asFormulaNumber(results["exemptionApplied"])) : 0; results["netTax"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["netTax"] = 0; }
  try { const v = (asFormulaNumber(results["netTax"])) + input.fixedFee; results["totalTax"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalTax"] = 0; }
  return results;
}
