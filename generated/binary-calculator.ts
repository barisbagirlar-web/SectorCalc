...
function evaluateAllFormulas(input: Binary_calculatorInput): Record<string, number> {
  ...
  try { const v = input.binary1; results["breakdown"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["breakdown"] = 0; }
  try { const v = input.binary1; results["breakdown_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["breakdown_aux"] = 0; }
  return results;
}
