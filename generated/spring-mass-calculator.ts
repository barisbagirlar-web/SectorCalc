...
function evaluateAllFormulas(input: Spring_mass_calculatorInput): Record<string, number> {
  ...
  try { const v = input.springConstant * input.mass * input.initialDisplacement * input.initialVelocity; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.springConstant * input.mass * input.initialDisplacement * input.initialVelocity; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
