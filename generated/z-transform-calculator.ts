...
function evaluateAllFormulas(input: Z_transform_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a0 + input.a1 / input.z + input.a2 / (input.z**2) + input.a3 / (input.z**3); results["X_z"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["X_z"] = 0; }
  try { const v = input.a0; results["term0"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["term0"] = 0; }
  try { const v = input.a1 / input.z; results["term1"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["term1"] = 0; }
  try { const v = input.a2 / (input.z**2); results["term2"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["term2"] = 0; }
  try { const v = input.a3 / (input.z**3); results["term3"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["term3"] = 0; }
  return results;
}
...
export function calculateZ_transform_calculator(input: Z_transform_calculatorInput): Z_transform_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["X_z"]));
  const breakdown = {
