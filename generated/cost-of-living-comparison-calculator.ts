...
function evaluateAllFormulas(input: Cost_of_living_comparison_calculatorInput): Record<string, number> {
  ...
  try { const v = input.baseRent + input.baseUtilities + input.baseGroceries + input.baseTransport + input.baseMisc; results["totalBaseCost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalBaseCost"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["totalBaseCost"])) * input.targetIndex / 100; results["totalTargetCost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalTargetCost"] = Number.NaN; }
  return results;
}
