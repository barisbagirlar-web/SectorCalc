...
function evaluateAllFormulas(input: Cost_of_goods_sold_calculatorInput): Record<string, number> {
  ...
  try { const v = input.materialPurchases + input.directLabor + input.overhead; results["totalProductionCost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalProductionCost"] = Number.NaN; }
  try { const v = input.beginningInventory + (toNumericFormulaValue(results["totalProductionCost"])); results["costOfGoodsAvailableForSale"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["costOfGoodsAvailableForSale"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["costOfGoodsAvailableForSale"])) - input.endingInventory; results["cogs"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["cogs"] = Number.NaN; }
  return results;
}
