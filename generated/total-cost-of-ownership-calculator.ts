...
function evaluateAllFormulas(input: Total_cost_of_ownership_calculatorInput): Record<string, number> {
  ...
  try { const v = input.purchasePrice + (input.annualMaintenance + input.annualEnergy) * input.ownershipYears - input.resaleValue; results["totalUndiscounted"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalUndiscounted"] = 0; }
  try { const v = input.discountRate / 100; results["r"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["r"] = 0; }
  try { const v = input.purchasePrice + (input.annualMaintenance + input.annualEnergy) * ((1 - (1 + input.discountRate/100) ** (-input.ownershipYears)) / (input.discountRate/100)) - input.resaleValue * (1 + input.discountRate/100) ** (-input.ownershipYears); results["netPresentValue"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["netPresentValue"] = 0; }
  return results;
}
