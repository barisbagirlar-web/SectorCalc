...
function evaluateAllFormulas(input: Apa_citation_generator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentYear - input.publicationYear; results["age"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["age"] = 0; }
  try { const v = input.authorCount; results["authorImpact"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["authorImpact"] = 0; }
  return results;
}
...
export function calculateApa_citation_generator_calculator(input: Apa_citation_generator_calculatorInput): Apa_citation_generator_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["authorImpact"]));
  const breakdown = {
