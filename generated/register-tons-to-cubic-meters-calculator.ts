...
function evaluateAllFormulas(input: Register_tons_to_cubic_meters_calculatorInput): Record<string, number> {
  ...
  try { const v = input.registerTons * input.cubicMeters * input.direction * input.decimalPlaces; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.registerTons * input.cubicMeters * input.direction * input.decimalPlaces * (input.useStandardConversion * input.customFactor); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.useStandardConversion * input.customFactor; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
