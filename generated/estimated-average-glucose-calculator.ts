...
function evaluateAllFormulas(input: Estimated_average_glucose_calculatorInput): Record<string, number> {
  ...
  try { const v = input.factor_mgdl * input.hba1c + input.offset_mgdl; results["eag_mgdl"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["eag_mgdl"] = Number.NaN; }
  try { const v = input.factor_mmol * input.hba1c + input.offset_mmol; results["eag_mmol"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["eag_mmol"] = Number.NaN; }
  return results;
}
