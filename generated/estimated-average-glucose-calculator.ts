...
function evaluateAllFormulas(input: Estimated_average_glucose_calculatorInput): Record<string, number> {
  ...
  try { const v = input.factor_mgdl * input.hba1c + input.offset_mgdl; results["eag_mgdl"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["eag_mgdl"] = 0; }
  try { const v = input.factor_mmol * input.hba1c + input.offset_mmol; results["eag_mmol"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["eag_mmol"] = 0; }
  return results;
}
...
export function calculateEstimated_average_glucose_calculator(input: Estimated_average_glucose_calculatorInput): Estimated_average_glucose_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["eag_mmol"]));
  const breakdown = {
