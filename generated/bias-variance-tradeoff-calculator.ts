...
function evaluateAllFormulas(input: Bias_variance_tradeoff_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.averagePrediction * input.trueValue * input.variance * input.irreducibleError; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.averagePrediction * input.trueValue * input.variance * input.irreducibleError; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
