...
function evaluateAllFormulas(input: Bias_variance_tradeoff_calculatorInput): Record<string, number> {
  ...
  try { const v = input.averagePrediction * input.trueValue * input.variance * input.irreducibleError; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.averagePrediction * input.trueValue * input.variance * input.irreducibleError; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
