...
function evaluateAllFormulas(input: Exponential_calculatorInput): Record<string, number> {
  ...
  try { const v = input.rateConstant * input.time; results["exponent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["exponent"] = Number.NaN; }
  try { const v = input.rateConstant * input.time; results["exponent_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["exponent_aux"] = Number.NaN; }
  return results;
}
