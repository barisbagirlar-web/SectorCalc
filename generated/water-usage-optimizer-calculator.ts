...
function evaluateAllFormulas(input: Water_usage_optimizer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalWaterIn * input.productionOutput * input.processWater * input.coolingWater; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.totalWaterIn * input.productionOutput * input.processWater * input.coolingWater * (input.wastewaterDischarge * input.recycledWater * input.leakageLosses); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.wastewaterDischarge * input.recycledWater * input.leakageLosses; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
