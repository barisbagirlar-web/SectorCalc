...
function evaluateAllFormulas(input: Wells_score_calculatorInput): Record<string, number> {
  ...
  try { const v = input.activeCancer * input.paralysis * input.surgeryOrBedridden * input.tendernessAlongVeins; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.activeCancer * input.paralysis * input.surgeryOrBedridden * input.tendernessAlongVeins * (input.entireLegSwollen * input.calfSwellingOver3cm * input.pittingEdema * input.alternativeDiagnosis); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.entireLegSwollen * input.calfSwellingOver3cm * input.pittingEdema * input.alternativeDiagnosis; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
