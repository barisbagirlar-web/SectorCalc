...
function evaluateAllFormulas(input: Lawn_fertilizer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.lawnArea * input.nitrogenRatePerSqM * 0.1 / input.nitrogenPercent * input.applicationsPerYear; results["totalFertilizerWeight_kg"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalFertilizerWeight_kg"] = Number.NaN; }
  try { const v = input.lawnArea * input.nitrogenRatePerSqM * 0.1 / input.nitrogenPercent * input.applicationsPerYear; results["totalFertilizerWeight_kg_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalFertilizerWeight_kg_aux"] = Number.NaN; }
  return results;
}
