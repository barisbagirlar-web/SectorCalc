...
function evaluateAllFormulas(input: Centuries_to_millennia_calculatorInput): Record<string, number> {
  ...
  try { const v = input.centuries * input.centuryYears * input.millenniumYears * input.precision; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.centuries * input.centuryYears * input.millenniumYears * input.precision; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
