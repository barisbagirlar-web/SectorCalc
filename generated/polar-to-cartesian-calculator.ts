...
function evaluateAllFormulas(input: Polar_to_cartesian_calculatorInput): Record<string, number> {
  ...
  try { const v = input.r * input.theta * input.cx * input.cy; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.r * input.theta * input.cx * input.cy; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
