...
function evaluateAllFormulas(input: Weed_tolerance_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.baseLength / 1000) * input.numWelds * input.weldPasses * input.materialFactor; results["totalShrinkage"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalShrinkage"] = Number.NaN; }
  try { const v = input.expansionCoeff * input.baseLength * (input.temperature - 20) / 1000000; results["thermalExpansion"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["thermalExpansion"] = Number.NaN; }
  return results;
}
