...
function evaluateAllFormulas(input: Pin_generator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.pinLength * input.seed * input.multiplier * input.shiftFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.pinLength * input.seed * input.multiplier * input.shiftFactor; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
