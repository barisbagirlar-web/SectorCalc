...
function evaluateAllFormulas(input: Cpm_delay_penalty_optimizer_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.planned_duration_days * input.actual_duration_days * (input.daily_penalty_rate / 100) * input.critical_path_float_total; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.planned_duration_days * input.actual_duration_days * (input.daily_penalty_rate / 100) * input.critical_path_float_total * (input.productivity_factor * (input.rework_percentage / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.productivity_factor * (input.rework_percentage / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
