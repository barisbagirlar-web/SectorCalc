...
function evaluateAllFormulas(input: Cpm_delay_penalty_optimizer_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.planned_duration_days + input.actual_duration_days + input.daily_penalty_rate; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.planned_duration_days + input.actual_duration_days + input.daily_penalty_rate; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
