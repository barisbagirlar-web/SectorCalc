...
function evaluateAllFormulas(input: Cronometer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.observedTime * input.cycles; results["totalObservedTime"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalObservedTime"] = Number.NaN; }
  ...
  try { const v = input.observedTime * (input.ratingFactor / 100) * (1 + input.allowanceFactor / 100); results["standardTime"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["standardTime"] = Number.NaN; }
  return results;
}
