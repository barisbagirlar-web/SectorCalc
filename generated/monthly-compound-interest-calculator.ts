...
function evaluateAllFormulas(input: Monthly_compound_interest_calculatorInput): Record<string, number> {
  ...
  try { const v = input.principal + input.monthlyContribution * input.years * 12; results["totalContributions"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalContributions"] = 0; }
  try { const v = input.principal + input.monthlyContribution * input.years * 12; results["totalContributions_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalContributions_aux"] = 0; }
  return results;
}
