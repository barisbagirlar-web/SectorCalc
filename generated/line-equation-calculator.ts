...
function evaluateAllFormulas(input: Line_equation_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.y1 - (asFormulaNumber(results["slope"])) * input.x1; results["intercept"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["intercept"] = 0; }
  try { const v = (asFormulaNumber(results["slope"])) * input.x_val + (asFormulaNumber(results["intercept"])); results["y_at_x"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["y_at_x"] = 0; }
  return results;
}
