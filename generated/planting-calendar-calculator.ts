...
function evaluateAllFormulas(input: Planting_calendar_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.lastFrostDay * input.plantingDateOffset * input.baseTemperature * input.targetGDD; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.lastFrostDay * input.plantingDateOffset * input.baseTemperature * input.targetGDD * (input.averageDailyTemp * input.soilTemp * input.minSoilTemp * input.safetyMargin); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.averageDailyTemp * input.soilTemp * input.minSoilTemp * input.safetyMargin; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
