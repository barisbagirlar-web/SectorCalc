...
function evaluateAllFormulas(input: Planting_calendar_calculatorInput): Record<string, number> {
  ...
  try { const v = input.lastFrostDay * input.plantingDateOffset * input.baseTemperature * input.targetGDD; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.lastFrostDay * input.plantingDateOffset * input.baseTemperature * input.targetGDD * (input.averageDailyTemp * input.soilTemp * input.minSoilTemp * input.safetyMargin); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.averageDailyTemp * input.soilTemp * input.minSoilTemp * input.safetyMargin; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
