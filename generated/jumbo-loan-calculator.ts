...
function evaluateAllFormulas(input: Jumbo_loan_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.loanAmount / (1 - input.downPaymentPercent / 100); results["purchasePrice"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["purchasePrice"] = 0; }
  ...
  try { const v = (asFormulaNumber(results["purchasePrice"])) * (input.propertyTaxRate / 100) / 12; results["monthlyPropertyTax"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["monthlyPropertyTax"] = 0; }
  ...
  try { const v = (input.loanAmount / (asFormulaNumber(results["purchasePrice"]))) * 100; results["loanToValue"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["loanToValue"] = 0; }
  return results;
}
