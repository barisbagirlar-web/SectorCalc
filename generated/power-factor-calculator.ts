...
function evaluateAllFormulas(input: Power_factor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.realPower / input.apparentPower; results["powerFactor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["powerFactor"] = Number.NaN; }
  try { const v = (input.voltage * input.current) / 1000; results["apparentPowerFromVI"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["apparentPowerFromVI"] = Number.NaN; }
  return results;
}
