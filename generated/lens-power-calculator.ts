...
function evaluateAllFormulas(input: Lens_power_calculatorInput): Record<string, number> {
  ...
  try { const v = 1000 * (input.n - 1) * (1/input.R1 - 1/input.R2 + (input.n - 1) * input.d / (input.n * input.R1 * input.R2)); results["powerD"] = Number.isFinite(v) ? v : 0; } catch { results["powerD"] = 0; }
  try { const v = 1 / ((input.n - 1) * (1/input.R1 - 1/input.R2 + (input.n - 1) * input.d / (input.n * input.R1 * input.R2))); results["focalLengthMM"] = Number.isFinite(v) ? v : 0; } catch { results["focalLengthMM"] = 0; }
  return results;
}
...
export function calculateLens_power_calculator(input: Lens_power_calculatorInput): Lens_power_calculatorOutput {
  ...
  const totalWasteCost = values["powerD"] ?? 0;
  const breakdown = {
