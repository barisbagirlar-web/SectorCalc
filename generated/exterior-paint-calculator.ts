...
function evaluateAllFormulas(input: Exterior_paint_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalWallArea - (input.doorArea + input.windowArea + input.otherOpeningsArea); results["netArea"] = Number.isFinite(v) ? v : 0; } catch { results["netArea"] = 0; }
  try { const v = (results["netArea"] ?? 0) * (1 + input.wasteFactor / 100); results["effectiveArea"] = Number.isFinite(v) ? v : 0; } catch { results["effectiveArea"] = 0; }
  try { const v = (results["effectiveArea"] ?? 0) * input.numberOfCoats / input.paintCoverage; results["paintNeeded"] = Number.isFinite(v) ? v : 0; } catch { results["paintNeeded"] = 0; }
  return results;
}
