...
function evaluateAllFormulas(input: Indefinite_integralInput): Record<string, number | string> {
  ...
  try { const v = input.functionType * input.coefficient * input.exponent * input.constant; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.functionType * input.coefficient * input.exponent * input.constant; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
