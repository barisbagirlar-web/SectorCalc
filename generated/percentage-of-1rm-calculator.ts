...
function evaluateAllFormulas(input: Percentage_of_1rm_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.oneRepMax * (input.targetPercentage / 100) * input.roundingIncrement * input.bodyWeight; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.oneRepMax * (input.targetPercentage / 100) * input.roundingIncrement * input.bodyWeight; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
