...
function evaluateAllFormulas(input: Percentage_of_1rm_calculatorInput): Record<string, number> {
  ...
  try { const v = input.oneRepMax * (input.targetPercentage / 100) * input.roundingIncrement * input.bodyWeight; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.oneRepMax * (input.targetPercentage / 100) * input.roundingIncrement * input.bodyWeight; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
