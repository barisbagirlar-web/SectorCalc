...
function evaluateAllFormulas(input: Scatter_plot_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.n * input.sumXY - input.sumX * input.sumY) / (input.n * input.sumX2 - input.sumX * input.sumX); results["slope"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["slope"] = Number.NaN; }
