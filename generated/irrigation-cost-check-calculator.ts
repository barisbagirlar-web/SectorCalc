...
function evaluateAllFormulas(input: Irrigation_cost_check_calculatorInput): Record<string, number> {
  ...
  try { const v = input.water_volume * input.water_cost_per_m3 * input.energy_cost_per_kwh * (input.pump_efficiency / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.water_volume * input.water_cost_per_m3 * input.energy_cost_per_kwh * (input.pump_efficiency / 100) * ((input.distribution_losses / 100) * input.labor_hours_per_year * (input.labor_rate / 100) * input.maintenance_cost); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.distribution_losses / 100) * input.labor_hours_per_year * (input.labor_rate / 100) * input.maintenance_cost; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
