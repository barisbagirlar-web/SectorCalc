...
function evaluateAllFormulas(input: Quadratic_formula_calculatorInput): Record<string, number> {
  ...
  try { results["vertex_y"] = input.coefficient_a * (results["vertex_x"] ?? 0) * (results["vertex_x"] ?? 0) + input.coefficient_b * (results["vertex_x"] ?? 0) + input.coefficient_c; } catch { results["vertex_y"] = 0; }
  try { results["sum_of_roots"] = -input.coefficient_b / input.coefficient_a; } catch { results["sum_of_roots"] = 0; }
  return results;
}
