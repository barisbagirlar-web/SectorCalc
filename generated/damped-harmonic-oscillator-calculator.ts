...
function evaluateAllFormulas(input: Damped_harmonic_oscillator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mass * input.dampingCoeff * input.springConst * input.initDisp; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.mass * input.dampingCoeff * input.springConst * input.initDisp * (input.initVeloc * input.time); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.initVeloc * input.time; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
