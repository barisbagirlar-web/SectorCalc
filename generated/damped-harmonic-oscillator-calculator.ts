...
function evaluateAllFormulas(input: Damped_harmonic_oscillator_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.mass * input.dampingCoeff * input.springConst * input.initDisp; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mass * input.dampingCoeff * input.springConst * input.initDisp * (input.initVeloc * input.time); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.initVeloc * input.time; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
