...
function evaluateAllFormulas(input: Handshaking_lemma_calculatorInput): Record<string, number> {
  ...
  try { const v = input.v1 + input.v2 + input.v3 + input.v4 + input.v5 + input.v6; results["sum"] = Number.isFinite(v) ? v : 0; } catch { results["sum"] = 0; }
  try { const v = (results["sum"] ?? 0) / 2; results["edges"] = Number.isFinite(v) ? v : 0; } catch { results["edges"] = 0; }
  try { const v = (results["sum"] ?? 0) % 2 === 0 ? 1 : 0; results["isValid"] = Number.isFinite(v) ? v : 0; } catch { results["isValid"] = 0; }
  return results;
}
