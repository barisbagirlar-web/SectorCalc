...
function evaluateAllFormulas(input: Deload_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentIntensity - input.deloadIntensityReduction; results["deloadIntensity"] = Number.isFinite(v) ? v : 0; } catch { results["deloadIntensity"] = 0; }
  try { const v = input.currentVolume * (1 - input.deloadVolumeReduction / 100); results["deloadVolume"] = Number.isFinite(v) ? v : 0; } catch { results["deloadVolume"] = 0; }
  try { const v = input.currentTrainingMax * ((results["deloadIntensity"] ?? 0) / 100); results["deloadWorkingWeight"] = Number.isFinite(v) ? v : 0; } catch { results["deloadWorkingWeight"] = 0; }
  return results;
}
...
export function calculateDeload_calculator(input: Deload_calculatorInput): Deload_calculatorOutput {
  ...
  const totalWasteCost = values["deloadWorkingWeight"] ?? 0;
  const breakdown = {
