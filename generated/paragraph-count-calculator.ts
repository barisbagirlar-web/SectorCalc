...
function evaluateAllFormulas(input: Paragraph_count_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.totalWords * input.wordsPerParagraph * input.totalCharacters * input.charactersPerWord; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.totalWords * input.wordsPerParagraph * input.totalCharacters * input.charactersPerWord * (input.totalPages * input.linesPerPage * input.wordsPerLine * input.paragraphsPerPage); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.totalPages * input.linesPerPage * input.wordsPerLine * input.paragraphsPerPage; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
