...
function evaluateAllFormulas(input: Stones_to_pounds_calculatorInput): Record<string, number> {
  ...
  try { const v = input.stones * input.adjustmentFactor + input.additionalWeight - input.subtractWeight + input.ounces / 16; results["rawPounds"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawPounds"] = Number.NaN; }
  try { const v = input.stones * input.adjustmentFactor; results["stonesToPounds"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["stonesToPounds"] = Number.NaN; }
  return results;
}
