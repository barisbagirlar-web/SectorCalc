...
function evaluateAllFormulas(input: Half_life_nuclear_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialQuantity * (0.5 ** (input.elapsedTime / input.halfLife)); results["remainingQuantity"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["remainingQuantity"] = 0; }
  try { const v = input.initialQuantity - (asFormulaNumber(results["remainingQuantity"])); results["decayedAmount"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["decayedAmount"] = 0; }
  try { const v = (asFormulaNumber(results["remainingQuantity"])) * input.safetyFactor; results["safeRemaining"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["safeRemaining"] = 0; }
  return results;
}
...
export function calculateHalf_life_nuclear_calculator(input: Half_life_nuclear_calculatorInput): Half_life_nuclear_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["remainingQuantity"]));
  const breakdown = {
