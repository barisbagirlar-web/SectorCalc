...
function evaluateAllFormulas(input: Polynomial_calculatorInput): Record<string, number | string> {
  ...
  try { const v = (asFormulaNumber(results["term4"])) + (asFormulaNumber(results["term3"])) + (asFormulaNumber(results["term2"])) + (asFormulaNumber(results["term1"])) + (asFormulaNumber(results["term0"])); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
