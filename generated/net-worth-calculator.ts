...
function evaluateAllFormulas(input: Net_worth_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cash + input.investments + input.realEstate + input.personalProperty + input.otherAssets; results["totalAssets"] = Number.isFinite(v) ? v : 0; } catch { results["totalAssets"] = 0; }
  try { const v = input.currentDebt + input.longTermDebt + input.otherDebt; results["totalDebt"] = Number.isFinite(v) ? v : 0; } catch { results["totalDebt"] = 0; }
  try { const v = (results["totalAssets"] ?? 0) - (results["totalDebt"] ?? 0); results["netWorth"] = Number.isFinite(v) ? v : 0; } catch { results["netWorth"] = 0; }
  return results;
}
...
export function calculateNet_worth_calculator(input: Net_worth_calculatorInput): Net_worth_calculatorOutput {
  ...
  const totalWasteCost = values["netWorth"] ?? 0;
  const breakdown = {
