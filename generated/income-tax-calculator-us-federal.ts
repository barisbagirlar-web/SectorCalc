...
function evaluateAllFormulas(input: Income_tax_calculator_us_federalInput): Record<string, number> {
  ...
  try { const v = input.annual_income - input.pre_tax_deductions + input.additional_income; results["adjusted_gross_income"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_gross_income"] = Number.NaN; }
  try { const v = input.annual_income - input.pre_tax_deductions + input.additional_income; results["adjusted_gross_income_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_gross_income_aux"] = Number.NaN; }
  return results;
}
