...
function evaluateAllFormulas(input: Tile_layout_calculatorInput): Record<string, number> {
  ...
  try { const v = input.roomLength * input.roomWidth * input.tileLength * input.tileWidth; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.roomLength * input.roomWidth * input.tileLength * input.tileWidth * (input.groutWidth * (input.wastePercent / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.groutWidth * (input.wastePercent / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
