...
function evaluateAllFormulas(input: Lcm_calculatorInput): Record<string, number> {
  ...
  try { const v = input.expected_life_years * input.acquisition_cost; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.expected_life_years * input.acquisition_cost * (1 + (input.labor_rate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.expected_life_years * input.acquisition_cost * (1 + (input.labor_rate / 100)) * (input.annual_operating_hours); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.annual_operating_hours; results["factor_annual_operating_hours"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_annual_operating_hours"] = Number.NaN; }
  return results;
}
