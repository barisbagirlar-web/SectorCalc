...
function evaluateAllFormulas(input: Fractal_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numberOfCopies * input.scalingFactor * input.iterationCount * input.initialLength; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.numberOfCopies * input.scalingFactor * input.iterationCount * input.initialLength; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
