...
function evaluateAllFormulas(input: Free_cash_flow_calculatorInput): Record<string, number> {
  ...
  try { const v = input.ebit * (1 - input.taxRate / 100) + input.depreciation - input.changeNWC - input.capex; results["freeCashFlow"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["freeCashFlow"] = Number.NaN; }
  try { const v = input.ebit * (1 - input.taxRate / 100) + input.depreciation; results["operatingCashFlow"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["operatingCashFlow"] = Number.NaN; }
  try { const v = input.capex + input.changeNWC; results["capitalInvestments"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["capitalInvestments"] = Number.NaN; }
  return results;
}
