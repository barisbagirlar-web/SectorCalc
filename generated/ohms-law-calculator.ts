...
function evaluateAllFormulas(input: Ohms_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.voltage * input.current * input.resistance * input.powerFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.voltage * input.current * input.resistance * input.powerFactor * (input.temperature); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.temperature; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
