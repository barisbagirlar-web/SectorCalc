...
function evaluateAllFormulas(input: Microseconds_to_nanoseconds_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.microseconds * input.conversionFactor * input.batchSize + input.calibrationOffset; results["rawNanoseconds"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["rawNanoseconds"] = 0; }
  try { const v = input.microseconds * input.conversionFactor * input.batchSize + input.calibrationOffset; results["rawNanoseconds_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["rawNanoseconds_aux"] = 0; }
  return results;
}
