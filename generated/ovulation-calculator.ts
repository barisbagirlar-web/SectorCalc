...
function evaluateAllFormulas(input: Ovulation_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.cycle_length * input.luteal_phase_length * input.last_period_start * input.cycle_variability; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.cycle_length * input.luteal_phase_length * input.last_period_start * input.cycle_variability; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
