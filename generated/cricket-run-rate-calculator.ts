...
function evaluateAllFormulas(input: Cricket_run_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.runsScored * input.oversFaced * input.targetRuns * input.totalOvers; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.runsScored * input.oversFaced * input.targetRuns * input.totalOvers; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
