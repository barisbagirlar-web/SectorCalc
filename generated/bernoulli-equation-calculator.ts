...
function evaluateAllFormulas(input: Bernoulli_equation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.density * input.g * input.P1 * input.v1; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.density * input.g * input.P1 * input.v1 * (input.z1 * input.P2 * input.z2 * input.head_loss); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.z1 * input.P2 * input.z2 * input.head_loss; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
