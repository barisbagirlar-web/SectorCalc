...
function evaluateAllFormulas(input: Bernoulli_equation_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.density * input.g * input.P1 * input.v1; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.density * input.g * input.P1 * input.v1 * (input.z1 * input.P2 * input.z2 * input.head_loss); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.z1 * input.P2 * input.z2 * input.head_loss; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
