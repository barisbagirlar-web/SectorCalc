...
function evaluateAllFormulas(input: Stairmaster_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.weight * 9.81 * input.stepHeight * input.stepsPerMinute * input.duration) / 4184; results["mechanicalWorkCalories"] = Number.isFinite(v) ? v : 0; } catch { results["mechanicalWorkCalories"] = 0; }
  return results;
}
