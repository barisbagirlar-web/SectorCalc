...
function evaluateAllFormulas(input: Rigging_calculatorInput): Record<string, number> {
  ...
  try { const v = input.loadWeight * input.slingAngle * input.numberOfLegs * input.safetyFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.loadWeight * input.slingAngle * input.numberOfLegs * input.safetyFactor; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
