...
function evaluateAllFormulas(input: Social_security_benefits_calculatorInput): Record<string, number> {
  ...
  try { const v = input.employeeRate * (input.employeeRate / 100) * 1 * input.monthlySalary; results["direct_labor_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["direct_labor_cost"] = 0; }
  try { const v = input.employeeRate * (input.employeeRate / 100) * 1 * input.monthlySalary; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
