...
function evaluateAllFormulas(input: Miller_formula_calculatorInput): Record<string, number> {
  ...
  try { const v = input.dbhCm / 100; results["diameterM"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["diameterM"] = 0; }
  try { const v = input.formFactor * ((asFormulaNumber(results["diameterM"])) ** 2) * input.heightM; results["baseVolume"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["baseVolume"] = 0; }
  try { const v = (asFormulaNumber(results["baseVolume"])) * input.adjustmentFactor; results["totalVolume"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalVolume"] = 0; }
  return results;
}
...
export function calculateMiller_formula_calculator(input: Miller_formula_calculatorInput): Miller_formula_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["diameterM"]));
  const breakdown = {
