...
function evaluateAllFormulas(input: Thermal_conductivity_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.temperature_k * input.thickness_m * input.cross_section_area_m2 * input.heat_flow_w; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.temperature_k * input.thickness_m * input.cross_section_area_m2 * input.heat_flow_w * (input.temperature_delta_k * (input.moisture_content_pct / 100) * input.aging_factor); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.temperature_delta_k * (input.moisture_content_pct / 100) * input.aging_factor; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
