...
function evaluateAllFormulas(input: Wind_load_calculatorInput): Record<string, number> {
  ...
  try { const v = input.basic_wind_speed * input.building_height * input.exposure_factor * input.gust_factor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.basic_wind_speed * input.building_height * input.exposure_factor * input.gust_factor * (input.pressure_coefficient * input.air_density); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.pressure_coefficient * input.air_density; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
