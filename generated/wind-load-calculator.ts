...
function evaluateAllFormulas(input: Wind_load_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.basic_wind_speed + input.terrain_category + input.building_height; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.basic_wind_speed + input.terrain_category + input.building_height; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
