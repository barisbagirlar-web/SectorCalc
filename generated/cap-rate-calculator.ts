...
function evaluateAllFormulas(input: Cap_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualGrossIncome * (1 - input.vacancyRate / 100) + input.otherAnnualIncome; results["effectiveGrossIncome"] = Number.isFinite(v) ? v : 0; } catch { results["effectiveGrossIncome"] = 0; }
  try { const v = (results["effectiveGrossIncome"] ?? 0) - input.annualOperatingExpenses; results["netOperatingIncome"] = Number.isFinite(v) ? v : 0; } catch { results["netOperatingIncome"] = 0; }
  try { const v = (results["netOperatingIncome"] ?? 0) / input.propertyValue * 100; results["capRate"] = Number.isFinite(v) ? v : 0; } catch { results["capRate"] = 0; }
  return results;
}
...
export function calculateCap_rate_calculator(input: Cap_rate_calculatorInput): Cap_rate_calculatorOutput {
  ...
  const totalWasteCost = values["capRate"] ?? 0;
  const breakdown = {
