...
function evaluateAllFormulas(input: Battery_backup_calculatorInput): Record<string, number> {
  ...
  try { const v = input.batteryCapacityAh * input.systemVoltage * input.safetyFactor * input.temperatureDerating; results["totalBatteryEnergyWh"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalBatteryEnergyWh"] = 0; }
  try { const v = (asFormulaNumber(results["totalBatteryEnergyWh"])) * (input.depthOfDischarge / 100) * (input.inverterEfficiency / 100); results["usableEnergyWh"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["usableEnergyWh"] = 0; }
  try { const v = (asFormulaNumber(results["usableEnergyWh"])) / input.loadPower; results["estimatedBackupTimeHours"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["estimatedBackupTimeHours"] = 0; }
  return results;
}
...
export function calculateBattery_backup_calculator(input: Battery_backup_calculatorInput): Battery_backup_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["estimatedBackupTimeHours"]));
  const breakdown = {
