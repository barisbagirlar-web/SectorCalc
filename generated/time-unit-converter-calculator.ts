...
function evaluateAllFormulas(input: Time_unit_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.days * 86400 + input.hours * 3600 + input.minutes * 60 + input.seconds) / 60; results["totalMinutes"] = Number.isFinite(v) ? v : 0; } catch { results["totalMinutes"] = 0; }
  try { const v = (input.days * 86400 + input.hours * 3600 + input.minutes * 60 + input.seconds) / 3600; results["totalHours"] = Number.isFinite(v) ? v : 0; } catch { results["totalHours"] = 0; }
  try { const v = (input.days * 86400 + input.hours * 3600 + input.minutes * 60 + input.seconds) / 86400; results["totalDays"] = Number.isFinite(v) ? v : 0; } catch { results["totalDays"] = 0; }
  return results;
}
