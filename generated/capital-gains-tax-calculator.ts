...
function evaluateAllFormulas(input: Capital_gains_tax_calculatorInput): Record<string, number> {
  ...
  try { const v = input.salePrice - (input.purchasePrice + input.acquisitionCost + input.improvementCost); results["capitalGain"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["capitalGain"] = Number.NaN; }
  try { const v = input.salePrice - (input.purchasePrice + input.acquisitionCost + input.improvementCost); results["capitalGain_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["capitalGain_aux"] = Number.NaN; }
  return results;
}
