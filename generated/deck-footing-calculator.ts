...
function evaluateAllFormulas(input: Deck_footing_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.liveLoad + input.deadLoad) * input.span * input.beamLength * 0.25; results["loadPerFooting"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["loadPerFooting"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["loadPerFooting"])) / input.soilBearing; results["requiredArea"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["requiredArea"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["requiredArea"])) * (input.footingThickness / 12); results["volume"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["volume"] = Number.NaN; }
  return results;
}
