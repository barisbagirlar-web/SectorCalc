...
function evaluateAllFormulas(input: Points_calculatorInput): Record<string, number> {
  ...
  try { const v = input.purchaseAmount * input.basePointsRate; results["basePointsEarned"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["basePointsEarned"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["basePointsEarned"])) * input.tierMultiplier + input.bonusPoints; results["totalBeforeCap"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalBeforeCap"] = Number.NaN; }
  return results;
}
