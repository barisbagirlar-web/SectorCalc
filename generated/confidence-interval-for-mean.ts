...
function evaluateAllFormulas(input: Confidence_interval_for_meanInput): Record<string, number | string> {
  ...
  try { const v = input.sampleMean * input.sampleStdDev * input.sampleSize * input.confidenceLevel; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.sampleMean * input.sampleStdDev * input.sampleSize * input.confidenceLevel; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
