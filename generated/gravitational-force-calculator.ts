...
function evaluateAllFormulas(input: Gravitational_force_calculatorInput): Record<string, number> {
  ...
  try { const v = input.gravitationalConstant * input.mass1 * input.mass2 / (input.distance ** 2); results["force"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["force"] = 0; }
  try { const v = input.gravitationalConstant * input.mass1 * input.mass2 / (input.distance ** 2); results["force_copy"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["force_copy"] = 0; }
  return results;
}
