...
function evaluateAllFormulas(input: Rc_time_constant_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.R * input.C; results["tau_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["tau_aux"] = 0; }
  return results;
}
