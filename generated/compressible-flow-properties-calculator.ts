...
function evaluateAllFormulas(input: Compressible_flow_properties_calculatorInput): Record<string, number> {
  ...
  try { const v = input.staticTemperature * (1 + ((input.gamma - 1) / 2) * input.machNumber ** 2); results["stagnationTemperature"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["stagnationTemperature"] = Number.NaN; }
  try { const v = input.staticPressure * (1 + ((input.gamma - 1) / 2) * input.machNumber ** 2) ** (input.gamma / (input.gamma - 1)); results["stagnationPressure"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["stagnationPressure"] = Number.NaN; }
  try { const v = input.staticPressure / (input.gasConstant * input.staticTemperature); results["density"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["density"] = Number.NaN; }
  return results;
}
