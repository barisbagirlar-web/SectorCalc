...
function evaluateAllFormulas(input: Clv_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.averageOrderValue * input.orderFrequency * (input.profitMargin / 100)); results["annualProfit"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annualProfit"] = Number.NaN; }
  try { const v = ((toNumericFormulaValue(results["annualProfit"])) * input.customerLifespan); results["totalProfitUndiscounted"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalProfitUndiscounted"] = Number.NaN; }
  return results;
}
