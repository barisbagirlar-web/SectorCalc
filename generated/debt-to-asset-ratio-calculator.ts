...
function evaluateAllFormulas(input: Debt_to_asset_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.shortTermLiabilities * input.longTermLiabilities * input.currentAssets * input.nonCurrentAssets; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.shortTermLiabilities * input.longTermLiabilities * input.currentAssets * input.nonCurrentAssets; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
