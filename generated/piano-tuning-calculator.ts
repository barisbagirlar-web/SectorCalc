...
function evaluateAllFormulas(input: Piano_tuning_calculatorInput): Record<string, number> {
  ...
  try { const v = input.referenceMidi * input.referenceFreq * input.targetMidi * input.detuneCents; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.referenceMidi * input.referenceFreq * input.targetMidi * input.detuneCents * (input.inharmonicityCoeff); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.inharmonicityCoeff; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
