...
function evaluateAllFormulas(input: Credit_card_payoff_calculatorInput): Record<string, number> {
  ...
  try { const v = input.balance * (input.annualInterestRate / 100) * input.monthlyPayment; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.balance * (input.annualInterestRate / 100) * input.monthlyPayment; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
