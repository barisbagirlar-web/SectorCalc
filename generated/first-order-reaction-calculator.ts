...
function evaluateAllFormulas(input: First_order_reaction_calculatorInput): Record<string, number> {
  ...
  try { const v = input.c0 * input.k * input.t * input.order; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.c0 * input.k * input.t * input.order; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
