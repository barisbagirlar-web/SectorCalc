...
function evaluateAllFormulas(input: Text_to_ascii_calculatorInput): Record<string, number> {
  ...
  try { const v = input.ascii1 + input.ascii2 + input.ascii3 + input.ascii4 + input.ascii5 + input.ascii6 + input.ascii7 + input.ascii8; results["outputPrimary"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["outputPrimary"] = Number.NaN; }
  try { const v = input.ascii1; results["outputBreakdown"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["outputBreakdown"] = Number.NaN; }
  return results;
}
