...
function evaluateAllFormulas(input: Torr_to_psi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.torrValue * input.conversionFactor; results["intermediatePsi"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["intermediatePsi"] = 0; }
  try { const v = input.torrValue * input.conversionFactor + input.offset; results["adjustedPsi"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustedPsi"] = 0; }
  return results;
}
...
export function calculateTorr_to_psi_calculator(input: Torr_to_psi_calculatorInput): Torr_to_psi_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["adjustedPsi"]));
  const breakdown = {
