...
function evaluateAllFormulas(input: Tooth_eruption_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.currentWear * input.load * input.rpm * input.materialHardness; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.currentWear * input.load * input.rpm * input.materialHardness * (input.initialWearLimit); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.initialWearLimit; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
