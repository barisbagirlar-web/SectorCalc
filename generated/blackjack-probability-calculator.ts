...
function evaluateAllFormulas(input: Blackjack_probability_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.acesRemaining / (input.acesRemaining + input.tensRemaining + input.otherCardsRemaining)) * (input.tensRemaining / (input.acesRemaining + input.tensRemaining + input.otherCardsRemaining - 1)) + (input.tensRemaining / (input.acesRemaining + input.tensRemaining + input.otherCardsRemaining)) * (input.acesRemaining / (input.acesRemaining + input.tensRemaining + input.otherCardsRemaining - 1))) * 100; results["blackjackProbability"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["blackjackProbability"] = Number.NaN; }
  try { const v = (input.acesRemaining / (input.acesRemaining + input.tensRemaining + input.otherCardsRemaining)) * (input.tensRemaining / (input.acesRemaining + input.tensRemaining + input.otherCardsRemaining - 1)) * 100; results["aceThenTenProbability"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["aceThenTenProbability"] = Number.NaN; }
  try { const v = (input.tensRemaining / (input.acesRemaining + input.tensRemaining + input.otherCardsRemaining)) * (input.acesRemaining / (input.acesRemaining + input.tensRemaining + input.otherCardsRemaining - 1)) * 100; results["tenThenAceProbability"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["tenThenAceProbability"] = Number.NaN; }
  return results;
}
