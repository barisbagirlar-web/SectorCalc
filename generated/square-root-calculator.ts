...
function evaluateAllFormulas(input: Square_root_calculatorInput): Record<string, number> {
  ...
  try { const v = input.radicand * input.precision * input.initialGuess * input.tolerance; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.radicand * input.precision * input.initialGuess * input.tolerance * (input.maxIterations); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.maxIterations; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
