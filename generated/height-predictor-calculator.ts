...
function evaluateAllFormulas(input: Height_predictor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.gender * (input.motherHeight + 13 + input.fatherHeight) / 2 + (1 - input.gender) * (input.fatherHeight - 13 + input.motherHeight) / 2; results["midParentalHeight"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["midParentalHeight"] = 0; }
  try { const v = ((asFormulaNumber(results["midParentalHeight"])) + input.currentHeight) / 2; results["predictedHeight"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["predictedHeight"] = 0; }
  return results;
}
