...
function evaluateAllFormulas(input: Horse_weight_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.heartGirthCm ** 2 * input.bodyLengthCm) / 11877; results["weightKg"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["weightKg"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["weightKg"])) * 2.20462; results["weightLbs"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["weightLbs"] = Number.NaN; }
  return results;
}
