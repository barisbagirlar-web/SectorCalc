...
function evaluateAllFormulas(input: Kwh_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.powerRating * input.numberOfDevices * input.usageHoursPerDay) / 1000; results["dailyEnergyKWh"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["dailyEnergyKWh"] = Number.NaN; }
  ...
  try { const v = (toNumericFormulaValue(results["dailyEnergyKWh"])) * 365; results["yearlyEnergyKWh"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["yearlyEnergyKWh"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["monthlyEnergyKWh"])) * input.electricityRate; results["monthlyCost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["monthlyCost"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["yearlyEnergyKWh"])) * input.electricityRate; results["yearlyCost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["yearlyCost"] = Number.NaN; }
  return results;
}
