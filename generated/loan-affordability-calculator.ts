...
function evaluateAllFormulas(input: Loan_affordability_calculatorInput): Record<string, number> {
  ...
  try { const v = input.monthlyIncome - input.monthlyExpenses; results["netIncome"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["netIncome"] = Number.NaN; }
  return results;
}
