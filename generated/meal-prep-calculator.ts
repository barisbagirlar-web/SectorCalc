...
function evaluateAllFormulas(input: Meal_prep_calculatorInput): Record<string, number> {
  ...
  try { const v = input.recipeServings * (1 - input.wastePercentage / 100); results["effectiveServingsPerBatch"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["effectiveServingsPerBatch"] = Number.NaN; }
  try { const v = input.desiredServings * input.packagingCostPerServing; results["totalPackagingCost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalPackagingCost"] = Number.NaN; }
  return results;
}
