...
function evaluateAllFormulas(input: System_of_equations_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.machineHoursPerUnitProd1 * input.materialPerUnitProd2) - (input.machineHoursPerUnitProd2 * input.materialPerUnitProd1); results["determinant"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["determinant"] = Number.NaN; }
  try { const v = ((input.totalMachineHours * input.materialPerUnitProd2) - (input.totalMaterial * input.machineHoursPerUnitProd2)) / (toNumericFormulaValue(results["determinant"])); results["product1Quantity"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["product1Quantity"] = Number.NaN; }
  try { const v = ((input.machineHoursPerUnitProd1 * input.totalMaterial) - (input.materialPerUnitProd1 * input.totalMachineHours)) / (toNumericFormulaValue(results["determinant"])); results["product2Quantity"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["product2Quantity"] = Number.NaN; }
  return results;
}
