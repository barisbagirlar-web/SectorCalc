...
function evaluateAllFormulas(input: Cm_to_inchInput): Record<string, number> {
  ...
  try { const v = input.cm / 2.54; results["inch"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["inch"] = Number.NaN; }
  try { const v = input.cm / 2.54; results["inch_copy"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["inch_copy"] = Number.NaN; }
  return results;
}
