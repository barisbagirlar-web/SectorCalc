...
function evaluateAllFormulas(input: Work_study_calculatorInput): Record<string, number> {
  ...
  try { const v = input.observedTime * (input.ratingFactor / 100); results["normalTime"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalTime"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["normalTime"])) * (1 + input.allowanceFactor / 100); results["standardTime"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["standardTime"] = Number.NaN; }
  try { const v = (input.zScore * input.standardDeviation / (input.precision / 100 * input.observedTime)) ** 2; results["requiredSamples"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["requiredSamples"] = Number.NaN; }
  return results;
}
