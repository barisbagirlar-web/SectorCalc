...
function evaluateAllFormulas(input: Mad_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.data_points * input.auto_input_2 * input.auto_input_3; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
