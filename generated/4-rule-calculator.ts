...
function evaluateAllFormulas(input: _4_rule_calculatorInput): Record<string, number> {
  ...
  try { const v = input.portfolioValue * (input.withdrawalRate / 100); results["annualWithdrawal"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annualWithdrawal"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["annualWithdrawal"])) / 12; results["monthlyWithdrawal"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["monthlyWithdrawal"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["annualWithdrawal"])) * (1 + input.inflationRate / 100); results["adjustedAnnualWithdrawal"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustedAnnualWithdrawal"] = Number.NaN; }
  return results;
}
