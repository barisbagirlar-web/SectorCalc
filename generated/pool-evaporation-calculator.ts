...
function evaluateAllFormulas(input: Pool_evaporation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.surfaceArea * input.waterTemp * input.airTemp * (input.relativeHumidity / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.surfaceArea * input.waterTemp * input.airTemp * (input.relativeHumidity / 100) * (input.windSpeed); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.windSpeed; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
