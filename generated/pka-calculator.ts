...
function evaluateAllFormulas(input: Pka_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.acidConcentration * input.measuredpH * input.knownKa * input.temperature; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.acidConcentration * input.measuredpH * input.knownKa * input.temperature; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
