...
function evaluateAllFormulas(input: Billionaire_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.currentNetWorth) * (input.annualIncome) * (input.savingsRate) * (input.annualReturn) * (input.desiredNetWorth); results["annualSavings"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annualSavings"] = Number.NaN; }
  try { const v = (input.currentNetWorth) * (input.annualIncome) * (input.savingsRate); results["annualSavings_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annualSavings_aux"] = Number.NaN; }
  return results;
}
