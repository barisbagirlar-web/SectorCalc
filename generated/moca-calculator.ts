...
function evaluateAllFormulas(input: Moca_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.usl - input.mean) / (3 * input.stddev); results["CPU"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["CPU"] = Number.NaN; }
  try { const v = (input.mean - input.lsl) / (3 * input.stddev); results["CPL"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["CPL"] = Number.NaN; }
  return results;
}
