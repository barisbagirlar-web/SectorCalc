...
function evaluateAllFormulas(input: Probability_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.defect_count + input.sample_size + input.sigma_shift; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
