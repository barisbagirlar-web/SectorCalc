...
function evaluateAllFormulas(input: Probability_calculatorInput): Record<string, number> {
  ...
  try { const v = input.defect_count * input.sample_size * input.sigma_shift; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.defect_count * input.sample_size * input.sigma_shift; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
