...
function evaluateAllFormulas(input: Lipid_panel_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalCholesterol; results["Total Cholesterol"] = Number.isFinite(v) ? v : 0; } catch { results["Total Cholesterol"] = 0; }
  try { const v = input.hdlCholesterol; results["HDL"] = Number.isFinite(v) ? v : 0; } catch { results["HDL"] = 0; }
  try { const v = input.triglycerides; results["Triglycerides"] = Number.isFinite(v) ? v : 0; } catch { results["Triglycerides"] = 0; }
  try { const v = input.triglycerides / 5; results["VLDL"] = Number.isFinite(v) ? v : 0; } catch { results["VLDL"] = 0; }
  try { const v = input.totalCholesterol - input.hdlCholesterol - (input.triglycerides / 5); results["LDL"] = Number.isFinite(v) ? v : 0; } catch { results["LDL"] = 0; }
  return results;
}
