...
function evaluateAllFormulas(input: Font_size_calculatorInput): Record<string, number> {
  ...
  try { const v = input.viewingDistance * input.visualAcuity * input.safetyFactor * input.pointConversion; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.viewingDistance * input.visualAcuity * input.safetyFactor * input.pointConversion * (input.pixelDensity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.pixelDensity; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
