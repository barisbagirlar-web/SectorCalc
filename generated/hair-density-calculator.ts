...
function evaluateAllFormulas(input: Hair_density_calculatorInput): Record<string, number> {
  ...
  try { const v = input.counted_hairs * input.magnification**2 * input.calibration_factor / input.sample_area_cm2; results["hair_density"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["hair_density"] = Number.NaN; }
  try { const v = input.counted_hairs / input.sample_area_cm2; results["raw_density"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["raw_density"] = Number.NaN; }
  try { const v = input.sample_area_cm2 / (input.magnification**2 * input.calibration_factor); results["effective_area"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["effective_area"] = Number.NaN; }
  return results;
}
