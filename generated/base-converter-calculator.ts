...
function evaluateAllFormulas(input: Base_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.inputNumber * input.fromBase * input.toBase * input.roundDigits; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.inputNumber * input.fromBase * input.toBase * input.roundDigits; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
