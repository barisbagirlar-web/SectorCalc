...
function evaluateAllFormulas(input: Poka_yoke_roi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annual_units * input.defect_cost_per_unit; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.annual_units * input.defect_cost_per_unit * (1 + (input.current_defect_rate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.annual_units * input.defect_cost_per_unit * (1 + (input.current_defect_rate / 100)) * (input.implementation_cost); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.implementation_cost; results["factor_implementation_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["factor_implementation_cost"] = 0; }
  return results;
}
