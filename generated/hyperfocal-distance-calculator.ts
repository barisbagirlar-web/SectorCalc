...
function evaluateAllFormulas(input: Hyperfocal_distance_calculatorInput): Record<string, number> {
  ...
  try { const v = input.focalLength**2 / (input.aperture * input.circleOfConfusion) + input.focalLength; results["H_mm"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["H_mm"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["H_mm"])) * input.outputUnitMultiplier; results["hyperfocal"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["hyperfocal"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["H_mm"])) * input.outputUnitMultiplier / 2; results["nearLimit"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["nearLimit"] = Number.NaN; }
  return results;
}
