...
function evaluateAllFormulas(input: Youngs_modulus_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.elongation / input.originalLength; results["strain"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["strain"] = 0; }
  try { const v = input.elongation / input.originalLength; results["strain_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["strain_aux"] = 0; }
  return results;
}
