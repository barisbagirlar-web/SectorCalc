...
function evaluateAllFormulas(input: Octal_calculatorInput): Record<string, number> {
  ...
  try { const v = input.octalValue1 * input.octalValue2 * input.octalValue3 * input.octalValue4; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.octalValue1 * input.octalValue2 * input.octalValue3 * input.octalValue4; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
