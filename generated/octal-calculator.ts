...
function evaluateAllFormulas(input: Octal_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.octalValue1 * input.octalValue2 * input.octalValue3 * input.octalValue4; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.octalValue1 * input.octalValue2 * input.octalValue3 * input.octalValue4; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
