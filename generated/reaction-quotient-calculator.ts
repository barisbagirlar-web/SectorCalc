...
function evaluateAllFormulas(input: Reaction_quotient_calculatorInput): Record<string, number> {
  ...
  try { const v = input.concA * input.coeffA * input.concB * input.coeffB; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.concA * input.coeffA * input.concB * input.coeffB * (input.concC * input.coeffC * input.concD * input.coeffD); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.concC * input.coeffC * input.concD * input.coeffD; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
