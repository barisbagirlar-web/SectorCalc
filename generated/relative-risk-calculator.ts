...
function evaluateAllFormulas(input: Relative_risk_calculatorInput): Record<string, number> {
  ...
  try { const v = input.exposed_events / input.exposed_total; results["risk_exposed"] = Number.isFinite(v) ? v : 0; } catch { results["risk_exposed"] = 0; }
  try { const v = input.control_events / input.control_total; results["risk_unexposed"] = Number.isFinite(v) ? v : 0; } catch { results["risk_unexposed"] = 0; }
  try { const v = (input.exposed_events / input.exposed_total) / (input.control_events / input.control_total); results["relative_risk"] = Number.isFinite(v) ? v : 0; } catch { results["relative_risk"] = 0; }
  return results;
}
...
export function calculateRelative_risk_calculator(input: Relative_risk_calculatorInput): Relative_risk_calculatorOutput {
  ...
  const totalWasteCost = values["relative_risk"] ?? 0;
  const breakdown = {
