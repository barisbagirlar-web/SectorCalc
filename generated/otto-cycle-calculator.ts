...
function evaluateAllFormulas(input: Otto_cycle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.compressionRatio * input.specificHeatRatio * input.initialTemperature * input.initialPressure; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.compressionRatio * input.specificHeatRatio * input.initialTemperature * input.initialPressure * (input.maxTemperature * input.specificHeatCv); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.maxTemperature * input.specificHeatCv; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
