...
function evaluateAllFormulas(input: Dpi_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.widthPixels / input.widthInches; results["horizontalDpi"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["horizontalDpi"] = 0; }
  try { const v = input.heightPixels / input.heightInches; results["verticalDpi"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["verticalDpi"] = 0; }
  return results;
}
