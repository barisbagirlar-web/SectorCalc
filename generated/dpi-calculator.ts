...
function evaluateAllFormulas(input: Dpi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.widthPixels / input.widthInches; results["horizontalDpi"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["horizontalDpi"] = 0; }
  try { const v = input.heightPixels / input.heightInches; results["verticalDpi"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["verticalDpi"] = 0; }
  return results;
}
...
export function calculateDpi_calculator(input: Dpi_calculatorInput): Dpi_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["verticalDpi"]));
  const breakdown = {
