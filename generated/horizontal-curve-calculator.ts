...
function evaluateAllFormulas(input: Horizontal_curve_calculatorInput): Record<string, number> {
  ...
  try { const v = input.designSpeed * (input.superelevation / 100) * input.frictionFactor * input.centralAngle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.designSpeed * (input.superelevation / 100) * input.frictionFactor * input.centralAngle * (input.stationPI); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.stationPI; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
