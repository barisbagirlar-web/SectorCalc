...
function evaluateAllFormulas(input: Horizontal_curve_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.designSpeed * (input.superelevation / 100) * input.frictionFactor * input.centralAngle; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.designSpeed * (input.superelevation / 100) * input.frictionFactor * input.centralAngle * (input.stationPI); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.stationPI; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
