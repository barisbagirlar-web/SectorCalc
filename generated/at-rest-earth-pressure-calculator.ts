...
function evaluateAllFormulas(input: At_rest_earth_pressure_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sigma_v * input.phi * input.OCR * input.SF; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.sigma_v * input.phi * input.OCR * input.SF; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
