...
function evaluateAllFormulas(input: Week_number_calculatorInput): Record<string, number> {
  ...
  try { const v = input.year * input.month * input.day * input.weekStartDay; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.year * input.month * input.day * input.weekStartDay; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
