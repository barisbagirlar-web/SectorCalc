...
function evaluateAllFormulas(input: Roth_vs_traditional_ira_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.yearsUntilRetirement; results["n"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["n"] = 0; }
  try { const v = input.yearsUntilRetirement; results["n_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["n_aux"] = 0; }
  return results;
}
