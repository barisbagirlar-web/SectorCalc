...
function evaluateAllFormulas(input: Mach_to_kmh_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mach * input.temperature * input.gamma * input.gasConstant; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.mach * input.temperature * input.gamma * input.gasConstant; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
