...
function evaluateAllFormulas(input: Julian_date_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.hour - 12) / 24 + input.minute / 1440 + input.second / 86400; results["fractionOfDay"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["fractionOfDay"] = Number.NaN; }
  try { const v = (input.hour - 12) / 24 + input.minute / 1440 + input.second / 86400; results["fractionOfDay_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["fractionOfDay_aux"] = Number.NaN; }
  return results;
}
