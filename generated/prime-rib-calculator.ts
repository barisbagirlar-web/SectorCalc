...
function evaluateAllFormulas(input: Prime_rib_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * input.minutesPerPound + input.baseTime; results["cookingTime"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["cookingTime"] = 0; }
  try { const v = (asFormulaNumber(results["cookingTime"])) + input.restingTime; results["totalTime"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalTime"] = 0; }
  return results;
}
