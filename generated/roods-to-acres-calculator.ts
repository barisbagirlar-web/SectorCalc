...
function evaluateAllFormulas(input: Roods_to_acres_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.roods + input.perches / 40; results["totalRoods"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalRoods"] = 0; }
  try { const v = (asFormulaNumber(results["totalRoods"])) * 0.25; results["acres"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["acres"] = 0; }
  return results;
}
