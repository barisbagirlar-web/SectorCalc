...
function evaluateAllFormulas(input: Thou_to_mm_calculatorInput): Record<string, number> {
  ...
  try { const v = input.nominalThou * input.upperToleranceThou * input.lowerToleranceThou * input.conversionFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.nominalThou * input.upperToleranceThou * input.lowerToleranceThou * input.conversionFactor * (input.precision); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
