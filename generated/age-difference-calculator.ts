...
function evaluateAllFormulas(input: Age_difference_calculatorInput): Record<string, number> {
  ...
  try { const v = input.person1Years + input.person1Months / 12; results["totalAge1"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalAge1"] = 0; }
  try { const v = input.person2Years + input.person2Months / 12; results["totalAge2"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalAge2"] = 0; }
  return results;
}
...
export function calculateAge_difference_calculator(input: Age_difference_calculatorInput): Age_difference_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["totalAge2"]));
  const breakdown = {
