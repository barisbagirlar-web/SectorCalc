...
function evaluateAllFormulas(input: Stock_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualDemand / input.workingDaysPerYear; results["dailyDemand"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["dailyDemand"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["dailyDemand"])) * input.leadTimeDays; results["reorderPoint"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["reorderPoint"] = Number.NaN; }
  return results;
}
