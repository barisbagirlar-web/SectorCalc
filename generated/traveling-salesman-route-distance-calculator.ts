...
function evaluateAllFormulas(input: Traveling_salesman_route_distance_calculatorInput): Record<string, number> {
  ...
  try { const v = input.ax * input.ay * input.bx * input.by; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.ax * input.ay * input.bx * input.by * (input.cx * input.cy * input.dx * input.dy); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.cx * input.cy * input.dx * input.dy; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
