...
function evaluateAllFormulas(input: Reaumur_to_celsius_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.reaumur + input.calibrationOffset) * 1.25; results["celsiusRaw"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["celsiusRaw"] = 0; }
  try { const v = input.measurementUncertainty * 1.25; results["uncertaintyCelsius"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["uncertaintyCelsius"] = 0; }
  return results;
}
...
export function calculateReaumur_to_celsius_calculator(input: Reaumur_to_celsius_calculatorInput): Reaumur_to_celsius_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["uncertaintyCelsius"]));
  const breakdown = {
