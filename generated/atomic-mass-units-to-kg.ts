...
function evaluateAllFormulas(input: Atomic_mass_units_to_kgInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.amu * 1.66053906660e-27; results["kg"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["kg"] = 0; }
  try { const v = input.amu * 1.66053906660e-27; results["kg_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["kg_copy"] = 0; }
  return results;
}
