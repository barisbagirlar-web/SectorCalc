...
function evaluateAllFormulas(input: Combined_gas_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.P1 * input.V1 / input.T1; results["constantK"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["constantK"] = Number.NaN; }
  try { const v = (input.P1 * input.V1 * input.T2) / (input.T1 * input.V2); results["finalPressure"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["finalPressure"] = Number.NaN; }
  return results;
}
