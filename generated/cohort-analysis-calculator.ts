...
function evaluateAllFormulas(input: Cohort_analysis_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.cohortASize * input.cohortBSize * input.retentionRateA * input.retentionRateB; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.cohortASize * input.cohortBSize * input.retentionRateA * input.retentionRateB * (input.periods); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.periods; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
