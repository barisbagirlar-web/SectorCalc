...
function evaluateAllFormulas(input: Induktor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.diameter * input.length * input.turns * input.frequency; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.diameter * input.length * input.turns * input.frequency; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
