...
function evaluateAllFormulas(input: Shingle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.roofFootprintArea * input.roofPitchAngle * (input.wasteFactor / 100) * input.shingleCoveragePerBundle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.roofFootprintArea * input.roofPitchAngle * (input.wasteFactor / 100) * input.shingleCoveragePerBundle * (input.ridgeLength * input.capCoveragePerBundle); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.ridgeLength * input.capCoveragePerBundle; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
