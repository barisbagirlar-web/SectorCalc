...
function evaluateAllFormulas(input: Sections_to_acres_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.fullSections + 0.5 * input.halfSections + 0.25 * input.quarterSections + 0.0625 * input.quarterQuarterSections; results["totalSections"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalSections"] = 0; }
  try { const v = (asFormulaNumber(results["totalSections"])) * 640; results["acres"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["acres"] = 0; }
  return results;
}
