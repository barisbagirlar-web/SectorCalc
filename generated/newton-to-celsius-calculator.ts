...
function evaluateAllFormulas(input: Newton_to_celsius_calculatorInput): Record<string, number> {
  ...
  try { const v = input.newton * 100/33; results["rawCelsius"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawCelsius"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["rawCelsius"])) + input.offset; results["celsiusWithOffset"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["celsiusWithOffset"] = Number.NaN; }
  return results;
}
