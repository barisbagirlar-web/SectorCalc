...
function evaluateAllFormulas(input: Cricket_net_run_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.runsScored * input.oversFaced * input.runsConceded * input.oversBowled; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.runsScored * input.oversFaced * input.runsConceded * input.oversBowled; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
