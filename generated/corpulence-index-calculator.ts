...
function evaluateAllFormulas(input: Corpulence_index_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.unit_system === 0 ? input.weight_kg : (input.weight_stones * 6.35029 + input.weight_pounds * 0.453592)) ? 1 : 0); results["weight_kg_total"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["weight_kg_total"] = Number.NaN; }
  try { const v = input.unit_system === 0 ? (input.height_cm / 100) : (input.height_feet * 0.3048 + input.height_inches * 0.0254); results["height_m"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["height_m"] = Number.NaN; }
  return results;
}
