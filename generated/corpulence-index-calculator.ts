...
function evaluateAllFormulas(input: Corpulence_index_calculatorInput): Record<string, number> {
  ...
  try { const v = input.unit_system === 0 ? input.weight_kg : (input.weight_stones * 6.35029 + input.weight_pounds * 0.453592); results["weight_kg_total"] = Number.isFinite(v) ? v : 0; } catch { results["weight_kg_total"] = 0; }
  try { const v = input.unit_system === 0 ? (input.height_cm / 100) : (input.height_feet * 0.3048 + input.height_inches * 0.0254); results["height_m"] = Number.isFinite(v) ? v : 0; } catch { results["height_m"] = 0; }
  try { const v = (results["weight_kg_total"] ?? 0) / Math.pow((results["height_m"] ?? 0), 3); results["corpulence_index"] = Number.isFinite(v) ? v : 0; } catch { results["corpulence_index"] = 0; }
  return results;
}
...
export function calculateCorpulence_index_calculator(input: Corpulence_index_calculatorInput): Corpulence_index_calculatorOutput {
  ...
  const totalWasteCost = values["corpulence_index"] ?? 0;
  const breakdown = {
