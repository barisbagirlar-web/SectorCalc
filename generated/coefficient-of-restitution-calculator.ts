...
function evaluateAllFormulas(input: Coefficient_of_restitution_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.v1i * input.v2i * input.v1f * input.v2f; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.v1i * input.v2i * input.v1f * input.v2f; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
