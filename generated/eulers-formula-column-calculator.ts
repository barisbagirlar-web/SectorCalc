...
function evaluateAllFormulas(input: Eulers_formula_column_calculatorInput): Record<string, number> {
  ...
  try { const v = input.E * input.I * input.K * input.L; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.E * input.I * input.K * input.L * (input.A); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.A; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
