...
function evaluateAllFormulas(input: Circadian_rhythm_diet_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mealCount * input.costPerMeal; results["dailyCost"] = Number.isFinite(v) ? v : 0; } catch { results["dailyCost"] = 0; }
  try { const v = (results["dailyCost"] ?? 0) * input.days; results["totalCost"] = Number.isFinite(v) ? v : 0; } catch { results["totalCost"] = 0; }
  try { const v = input.mealCount > 1 ? (24 - input.fastingWindow) / (input.mealCount - 1) : 0; results["mealInterval"] = Number.isFinite(v) ? v : 0; } catch { results["mealInterval"] = 0; }
  try { const v = ((input.shiftStart - input.chronotype * 0.5) % 24 + 24) % 24; results["firstMealTime"] = Number.isFinite(v) ? v : 0; } catch { results["firstMealTime"] = 0; }
  return results;
}
