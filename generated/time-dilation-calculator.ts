...
function evaluateAllFormulas(input: Time_dilation_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.velocity_km_s * 1000; results["v_m_s"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["v_m_s"] = 0; }
  try { const v = input.velocity_km_s * 1000; results["v_m_s_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["v_m_s_aux"] = 0; }
  return results;
}
