...
function evaluateAllFormulas(input: Inverting_amplifier_calculatorInput): Record<string, number> {
  ...
  try { const v = -(input.rf/input.rin)*input.vin; results["vout_ideal"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["vout_ideal"] = 0; }
  try { const v = -(input.rf/input.rin)*input.vin; results["vout_ideal_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["vout_ideal_aux"] = 0; }
  return results;
}
...
export function calculateInverting_amplifier_calculator(input: Inverting_amplifier_calculatorInput): Inverting_amplifier_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["vout_ideal_aux"]));
  const breakdown = {
