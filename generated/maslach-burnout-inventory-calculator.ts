...
function evaluateAllFormulas(input: Maslach_burnout_inventory_calculatorInput): Record<string, number> {
  ...
  try { const v = input.eeScore / input.eeItems; results["avgEE"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["avgEE"] = Number.NaN; }
  try { const v = input.dpScore / input.dpItems; results["avgDP"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["avgDP"] = Number.NaN; }
  try { const v = input.paScore / input.paItems; results["avgPA"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["avgPA"] = Number.NaN; }
  return results;
}
