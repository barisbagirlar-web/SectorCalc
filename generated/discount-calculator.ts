...
function evaluateAllFormulas(input: Discount_calculatorInput): Record<string, number> {
  ...
  try { const v = (results["total_variable_cost"] ?? 0) + input.fixed_cost_allocation; results["total_cost"] = Number.isFinite(v) ? v : 0; } catch { results["total_cost"] = 0; }
  try { const v = (results["total_revenue"] ?? 0) - (results["total_cost"] ?? 0); results["net_margin"] = Number.isFinite(v) ? v : 0; } catch { results["net_margin"] = 0; }
  try { const v = (((results["total_revenue"] ?? 0) > 0) ? (((results["net_margin"] ?? 0) / (results["total_revenue"] ?? 0)) * 100) : (0)); results["net_margin_percent"] = Number.isFinite(v) ? v : 0; } catch { results["net_margin_percent"] = 0; }
  return results;
}
