...
function evaluateAllFormulas(input: Skiing_calculatorInput): Record<string, number> {
  ...
  try { const v = input.verticalDrop * input.horizontalDistance * input.frictionCoeff * input.initialSpeed; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.verticalDrop * input.horizontalDistance * input.frictionCoeff * input.initialSpeed; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
