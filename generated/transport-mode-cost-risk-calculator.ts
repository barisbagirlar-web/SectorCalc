...
function evaluateAllFormulas(input: Transport_mode_cost_risk_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.annual_volume + input.avg_shipment_weight + input.distance_km; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.annual_volume + input.avg_shipment_weight + input.distance_km; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
