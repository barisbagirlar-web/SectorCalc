...
function evaluateAllFormulas(input: Transport_mode_cost_risk_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annual_volume * input.fuel_price_per_liter; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.annual_volume * input.fuel_price_per_liter * (1 + (input.labor_rate_per_hour / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.annual_volume * input.fuel_price_per_liter * (1 + (input.labor_rate_per_hour / 100)) * (input.avg_shipment_weight); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.avg_shipment_weight; results["factor_avg_shipment_weight"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["factor_avg_shipment_weight"] = 0; }
  return results;
}
