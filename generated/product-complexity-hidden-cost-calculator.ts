...
function evaluateAllFormulas(input: Product_complexity_hidden_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = input.num_sku * input.avg_volume_per_sku * input.num_components * input.avg_bom_levels; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.num_sku * input.avg_volume_per_sku * input.num_components * input.avg_bom_levels * (input.setup_time_minutes * (input.labor_rate / 100) * (input.overhead_rate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.setup_time_minutes * (input.labor_rate / 100) * (input.overhead_rate / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
