...
function evaluateAllFormulas(input: Container_load_calculatorInput): Record<string, number> {
  ...
  try { results["units_per_layer"] = FLOOR(((results["container_internal_dimensions"] ?? 0).length / input.cargo_length_cm) * ((results["container_internal_dimensions"] ?? 0).width / input.cargo_width_cm) * 0.85); } catch { results["units_per_layer"] = 0; }
  try { results["total_layers"] = Math.min(input.stacking_factor, FLOOR((results["container_internal_dimensions"] ?? 0).height / (results["total_unit_height"] ?? 0))); } catch { results["total_layers"] = 0; }
