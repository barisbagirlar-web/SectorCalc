...
function evaluateAllFormulas(input: Container_load_calculatorInput): Record<string, number> {
  ...
  try { results["container_internal_dimensions"] = (input.container_type === '20ft_standard' ? {length: 589.8, width: 235.2, height: 239.2, max_gross_kg: 24000} : (input.container_type === '40ft_standard' ? {length: 1203.2, width: 235.2, height: 239.2, max_gross_kg: 30480} : (input.container_type === '40ft_highcube' ? {length: 1203.2, width: 235.2, height: 269.2, max_gross_kg: 30480} : (input.container_type === '20ft_open_top' ? {length: 589.8, width: 235.2, height: 233.0, max_gross_kg: 24000} : 0)))); } catch { results["container_internal_dimensions"] = 0; }
  try { results["total_unit_height"] = input.cargo_height_cm + (input.use_pallet ? input.pallet_height_cm : 0); } catch { results["total_unit_height"] = 0; }
  try { results["units_per_layer"] = Math.floor(((results["container_internal_dimensions"] ?? 0).length / input.cargo_length_cm) * ((results["container_internal_dimensions"] ?? 0).width / input.cargo_width_cm) * 0.85); } catch { results["units_per_layer"] = 0; }
  try { results["total_layers"] = Math.min(input.stacking_factor, Math.floor((results["container_internal_dimensions"] ?? 0).height / (results["total_unit_height"] ?? 0))); } catch { results["total_layers"] = 0; }
  try { results["total_units"] = (results["units_per_layer"] ?? 0) * (results["total_layers"] ?? 0); } catch { results["total_units"] = 0; }
  try { results["volume_utilization"] = ((results["total_units"] ?? 0) * input.cargo_length_cm * input.cargo_width_cm * (results["total_unit_height"] ?? 0)) / ((results["container_internal_dimensions"] ?? 0).length * (results["container_internal_dimensions"] ?? 0).width * (results["container_internal_dimensions"] ?? 0).height); } catch { results["volume_utilization"] = 0; }
  try { results["weight_utilization"] = ((results["total_units"] ?? 0) * input.cargo_weight_kg) / (results["container_internal_dimensions"] ?? 0).max_gross_kg; } catch { results["weight_utilization"] = 0; }
  return results;
}
