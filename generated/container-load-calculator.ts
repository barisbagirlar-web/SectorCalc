...
function evaluateAllFormulas(input: Container_load_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cargo_length_cm * input.cargo_width_cm * input.cargo_height_cm * input.cargo_weight_kg; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.cargo_length_cm * input.cargo_width_cm * input.cargo_height_cm * input.cargo_weight_kg * (input.stacking_factor); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.stacking_factor; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
...
export const Container_load_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
