...
function evaluateAllFormulas(input: Caffeine_sleep_calculatorInput): Record<string, number> {
  ...
  try { const v = input.caffeineDose * input.halfLife * input.hoursBeforeSleep * input.toleranceFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.caffeineDose * input.halfLife * input.hoursBeforeSleep * input.toleranceFactor; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
