...
function evaluateAllFormulas(input: Mean_median_mode_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.value1 + input.value2 + input.value3 + input.value4 + input.value5 + input.value6 + input.value7 + input.value8) / 8; results["mean"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["mean"] = Number.NaN; }
  try { const v = (input.value1 + input.value2 + input.value3 + input.value4 + input.value5 + input.value6 + input.value7 + input.value8) / 8; results["mean_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["mean_aux"] = Number.NaN; }
  return results;
}
