...
function evaluateAllFormulas(input: Drain_size_calculatorInput): Record<string, number> {
  ...
  try { const v = input.flowRate * input.slope * input.manningN * input.safetyFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.flowRate * input.slope * input.manningN * input.safetyFactor; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
