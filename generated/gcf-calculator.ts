...
function evaluateAllFormulas(input: Gcf_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.number1 * input.number2 * input.number3 * input.number4; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.number1 * input.number2 * input.number3 * input.number4; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
