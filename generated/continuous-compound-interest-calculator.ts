...
function evaluateAllFormulas(input: Continuous_compound_interest_calculatorInput): Record<string, number> {
  ...
  try { const v = input.principal * (input.annualRate / 100) * input.time * input.decimalPlaces; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.principal * (input.annualRate / 100) * input.time * input.decimalPlaces; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
