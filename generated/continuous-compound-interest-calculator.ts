...
function evaluateAllFormulas(input: Continuous_compound_interest_calculatorInput): Record<string, number> {
  ...
  try { const v = input.principal * (input.annualRate / 100) * input.time * input.decimalPlaces; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.principal * (input.annualRate / 100) * input.time * input.decimalPlaces; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
