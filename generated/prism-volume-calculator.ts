...
function evaluateAllFormulas(input: Prism_volume_calculatorInput): Record<string, number> {
  ...
  try { const v = input.baseArea * input.sides * input.sideLength * input.height; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.baseArea * input.sides * input.sideLength * input.height; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
