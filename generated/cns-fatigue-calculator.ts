...
function evaluateAllFormulas(input: Cns_fatigue_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weightLifted * input.reps * input.sets; results["totalLoad"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalLoad"] = Number.NaN; }
  try { const v = input.weightLifted / input.bodyWeight; results["intensity"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["intensity"] = Number.NaN; }
  return results;
}
