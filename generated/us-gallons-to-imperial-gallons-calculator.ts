...
function evaluateAllFormulas(input: Us_gallons_to_imperial_gallons_calculatorInput): Record<string, number> {
  ...
  try { const v = input.usGallons * input.conversionFactor * input.batchQuantity; results["imperialUnrounded"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["imperialUnrounded"] = Number.NaN; }
  try { const v = input.usGallons * input.conversionFactor * input.batchQuantity; results["imperialUnrounded_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["imperialUnrounded_aux"] = Number.NaN; }
  return results;
}
