...
function evaluateAllFormulas(input: Kelly_criterion_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.b * input.p - (1 - input.p)) / input.b; results["f_star"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["f_star"] = Number.NaN; }
  try { const v = input.fraction * (input.b * input.p - (1 - input.p)) / input.b; results["f"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["f"] = Number.NaN; }
  try { const v = input.bankroll * input.fraction * (input.b * input.p - (1 - input.p)) / input.b; results["bet_amount"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["bet_amount"] = Number.NaN; }
  return results;
}
