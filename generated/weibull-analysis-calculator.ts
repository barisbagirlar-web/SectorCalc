...
function evaluateAllFormulas(input: Weibull_analysis_calculatorInput): Record<string, number> {
  ...
  try { const v = input.shape * input.scale * input.time * input.location; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.shape * input.scale * input.time * input.location; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
