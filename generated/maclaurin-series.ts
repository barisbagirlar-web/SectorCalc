...
function evaluateAllFormulas(input: Maclaurin_seriesInput): Record<string, number> {
  ...
  try { const v = input.function * input.center * input.x_value * input.terms; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.function * input.center * input.x_value * input.terms; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
