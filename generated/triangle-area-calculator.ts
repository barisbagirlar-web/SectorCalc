...
function evaluateAllFormulas(input: Triangle_area_calculatorInput): Record<string, number> {
  ...
  try { const v = input.side1 * input.side2 * input.side3 * input.base; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.side1 * input.side2 * input.side3 * input.base * (input.height * input.angle); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.height * input.angle; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
