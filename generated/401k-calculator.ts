...
function evaluateAllFormulas(input: _401k_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentAge * input.currentSalary; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.currentAge * input.currentSalary * (1 + (input.contributionRate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.currentAge * input.currentSalary * (1 + (input.contributionRate / 100)) * (input.retirementAge); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
