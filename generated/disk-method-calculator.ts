...
function evaluateAllFormulas(input: Disk_method_calculatorInput): Record<string, number> {
  ...
  try { const v = input.coeffA**2*(input.upperBound**5 - input.lowerBound**5)/5 + 2*input.coeffA*input.coeffB*(input.upperBound**4 - input.lowerBound**4)/4 + (2*input.coeffA*input.coeffC + input.coeffB**2)*(input.upperBound**3 - input.lowerBound**3)/3 + 2*input.coeffB*input.coeffC*(input.upperBound**2 - input.lowerBound**2)/2 + input.coeffC**2*(input.upperBound - input.lowerBound); results["integral"] = Number.isFinite(v) ? v : 0; } catch { results["integral"] = 0; }
  try { const v = Math.PI * (results["integral"] ?? 0); results["volume"] = Number.isFinite(v) ? v : 0; } catch { results["volume"] = 0; }
  try { const v = input.coeffA*((input.lowerBound+input.upperBound)/2)**2 + input.coeffB*(input.lowerBound+input.upperBound)/2 + input.coeffC; results["midRadius"] = Number.isFinite(v) ? v : 0; } catch { results["midRadius"] = 0; }
  return results;
}
