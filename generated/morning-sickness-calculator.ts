...
function evaluateAllFormulas(input: Morning_sickness_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.machine_age_years * 0.1; results["age_contribution"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["age_contribution"] = 0; }
  ...
  try { const v = (input.ambient_temperature_celsius - 20) * 0.05; results["temp_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["temp_factor"] = 0; }
  try { const v = input.cold_starts_per_day * 0.2; results["cold_start_penalty"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["cold_start_penalty"] = 0; }
  try { const v = ((asFormulaNumber(results["age_contribution"])) + (asFormulaNumber(results["downtime_factor"])) + (asFormulaNumber(results["temp_factor"])) + (asFormulaNumber(results["cold_start_penalty"]))) * input.severity_factor; results["msi"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["msi"] = 0; }
  return results;
}
