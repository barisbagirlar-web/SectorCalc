...
function evaluateAllFormulas(input: Rhombus_area_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sideLength ? 4 * input.sideLength : 0; results["perimeter"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["perimeter"] = 0; }
  try { const v = input.diagonal1 && input.diagonal2 ? input.diagonal1 * input.diagonal2 : 0; results["diagonalProduct"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["diagonalProduct"] = 0; }
  return results;
}
...
export function calculateRhombus_area_calculator(input: Rhombus_area_calculatorInput): Rhombus_area_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["diagonalProduct"]));
  const breakdown = {
