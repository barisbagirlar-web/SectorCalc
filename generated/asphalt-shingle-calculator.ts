...
function evaluateAllFormulas(input: Asphalt_shingle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.roofLength * input.roofWidth * input.pitch * input.numberOfPlanes; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.roofLength * input.roofWidth * input.pitch * input.numberOfPlanes * ((input.wasteFactor / 100) * input.bundleCoverage); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.wasteFactor / 100) * input.bundleCoverage; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
