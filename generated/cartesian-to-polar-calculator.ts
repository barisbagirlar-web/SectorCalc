...
function evaluateAllFormulas(input: Cartesian_to_polar_calculatorInput): Record<string, number> {
  ...
  try { const v = input.pointX * input.pointY * input.originX * input.originY; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.pointX * input.pointY * input.originX * input.originY; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
