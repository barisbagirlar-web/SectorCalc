...
function evaluateAllFormulas(input: Schrodinger_equationInput): Record<string, number> {
  ...
  try { const v = input.mass * input.potential * input.energy * input.hbar; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mass * input.potential * input.energy * input.hbar * (input.position); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.position; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
