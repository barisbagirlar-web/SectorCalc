...
function evaluateAllFormulas(input: Electrical_panel_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numberOfCircuits * input.loadPerCircuit * input.diversityFactor; results["totalCurrent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalCurrent"] = Number.NaN; }
  try { const v = input.numberOfCircuits * input.loadPerCircuit * input.diversityFactor; results["totalCurrent_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalCurrent_aux"] = Number.NaN; }
  return results;
}
