...
function evaluateAllFormulas(input: E_notation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.coeff1 * input.exp1 * input.coeff2 * input.exp2; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.coeff1 * input.exp1 * input.coeff2 * input.exp2; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
