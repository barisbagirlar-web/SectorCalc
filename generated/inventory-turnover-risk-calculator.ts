...
function evaluateAllFormulas(input: Inventory_turnover_risk_calculatorInput): Record<string, number> {
  ...
  try { const v = input.avg_inventory_value * input.cogs_annual * input.demand_variability_coefficient * (input.obsolescence_rate / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.avg_inventory_value * input.cogs_annual * input.demand_variability_coefficient * (input.obsolescence_rate / 100) * ((input.carrying_cost_rate / 100) * input.lead_time_days * (input.service_level_target / 100) * (input.inventory_accuracy_pct / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.carrying_cost_rate / 100) * input.lead_time_days * (input.service_level_target / 100) * (input.inventory_accuracy_pct / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
