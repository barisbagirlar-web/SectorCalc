...
function evaluateAllFormulas(input: Buffer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.service_level_z * input.average_daily_demand * input.lead_time_standard_deviation; results["leadTimeVariabilityComponent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["leadTimeVariabilityComponent"] = Number.NaN; }
  try { const v = input.service_level_z * input.average_daily_demand * input.lead_time_standard_deviation; results["leadTimeVariabilityComponent_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["leadTimeVariabilityComponent_aux"] = Number.NaN; }
  return results;
}
