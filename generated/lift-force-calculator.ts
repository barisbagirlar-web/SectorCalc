...
function evaluateAllFormulas(input: Lift_force_calculatorInput): Record<string, number> {
  ...
  try { const v = input.density * input.velocity * input.area * input.liftCoefficient; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.density * input.velocity * input.area * input.liftCoefficient; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
