...
function evaluateAllFormulas(input: Lift_force_calculatorInput): Record<string, number> {
  ...
  try { const v = input.density * input.velocity * input.area * input.liftCoefficient; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.density * input.velocity * input.area * input.liftCoefficient; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
