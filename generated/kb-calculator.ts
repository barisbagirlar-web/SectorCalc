...
function evaluateAllFormulas(input: Kb_calculatorInput): Record<string, number> {
  ...
  try { const v = input.wireDia * input.coilDia * input.activeCoils * input.shearModulus; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.wireDia * input.coilDia * input.activeCoils * input.shearModulus; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
