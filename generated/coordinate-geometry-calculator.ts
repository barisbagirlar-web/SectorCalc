...
function evaluateAllFormulas(input: Coordinate_geometry_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.x1 + input.x2) / 2; results["midpointX"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["midpointX"] = 0; }
  try { const v = (input.y1 + input.y2) / 2; results["midpointY"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["midpointY"] = 0; }
  ...
  try { const v = input.y1 - input.y2; results["lineA"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["lineA"] = 0; }
  try { const v = input.x2 - input.x1; results["lineB"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["lineB"] = 0; }
  try { const v = input.x1 * input.y2 - input.x2 * input.y1; results["lineC"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["lineC"] = 0; }
  return results;
}
