...
function evaluateAllFormulas(input: Load_factor_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.totalEnergy * input.peakDemand * input.numberOfDays * input.dailyOperatingHours; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.totalEnergy * input.peakDemand * input.numberOfDays * input.dailyOperatingHours; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
