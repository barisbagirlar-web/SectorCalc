...
function evaluateAllFormulas(input: Color_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.red * input.green * input.blue * input.alpha; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.red * input.green * input.blue * input.alpha; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
