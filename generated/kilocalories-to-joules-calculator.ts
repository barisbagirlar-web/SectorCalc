...
function evaluateAllFormulas(input: Kilocalories_to_joules_calculatorInput): Record<string, number> {
  ...
  try { const v = input.kcal * input.joulePerKcal * input.scale; results["rawJoules"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["rawJoules"] = 0; }
  try { const v = input.joulePerKcal; results["conversionFactorUsed"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["conversionFactorUsed"] = 0; }
  return results;
}
