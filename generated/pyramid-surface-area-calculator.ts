...
function evaluateAllFormulas(input: Pyramid_surface_area_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.baseSide * input.height * input.slantHeight * (input.wasteFactor / 100); results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.baseSide * input.height * input.slantHeight * (input.wasteFactor / 100); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
