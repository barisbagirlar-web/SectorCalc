...
function evaluateAllFormulas(input: Distance_formula_calculatorInput): Record<string, number> {
  const results: Record<string, number> = {};
  try { const v = Math.sqrt(Math.pow(input.x2 - input.x1, 2) + Math.pow(input.y2 - input.y1, 2)); results["distance"] = Number.isFinite(v) ? v : 0; } catch { results["distance"] = 0; }
  ...
  try { const v = Math.pow(input.x2 - input.x1, 2) + Math.pow(input.y2 - input.y1, 2); results["distanceSquared"] = Number.isFinite(v) ? v : 0; } catch { results["distanceSquared"] = 0; }
  return results;
}
