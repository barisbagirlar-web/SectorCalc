...
function evaluateAllFormulas(input: Winsorized_mean_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.x1 * input.x2 * input.x3 * input.x4; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.x1 * input.x2 * input.x3 * input.x4 * (input.x5 * input.x6 * (input.winPercent / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.x5 * input.x6 * (input.winPercent / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
