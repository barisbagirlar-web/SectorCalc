...
function evaluateAllFormulas(input: Winsorized_mean_calculatorInput): Record<string, number> {
  ...
  try { const v = input.x1 * input.x2 * input.x3 * input.x4; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.x1 * input.x2 * input.x3 * input.x4 * (input.x5 * input.x6 * (input.winPercent / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.x5 * input.x6 * (input.winPercent / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
