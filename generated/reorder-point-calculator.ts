...
function evaluateAllFormulas(input: Reorder_point_calculatorInput): Record<string, number> {
  ...
  try { const v = input.averageDailyUsage * input.leadTime; results["averageDemandDuringLeadTime"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["averageDemandDuringLeadTime"] = 0; }
  try { const v = input.averageDailyUsage * input.leadTime; results["averageDemandDuringLeadTime_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["averageDemandDuringLeadTime_aux"] = 0; }
  return results;
}
...
export function calculateReorder_point_calculator(input: Reorder_point_calculatorInput): Reorder_point_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["averageDemandDuringLeadTime_aux"]));
  const breakdown = {
