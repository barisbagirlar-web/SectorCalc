...
function evaluateAllFormulas(input: Baby_growth_percentile_calculatorInput): Record<string, number> {
  ...
  try { const v = input.measurement * input.L * input.M * input.S; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.measurement * input.L * input.M * input.S * (input.measurementType); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.measurementType; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
