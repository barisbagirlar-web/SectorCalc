...
function evaluateAllFormulas(input: Brayton_cycle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.T1 * input.T3 * input.rp * input.eta_c; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.T1 * input.T3 * input.rp * input.eta_c * (input.eta_t * input.gamma * input.cp); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.eta_t * input.gamma * input.cp; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
