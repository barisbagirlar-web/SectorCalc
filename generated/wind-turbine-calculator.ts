...
function evaluateAllFormulas(input: Wind_turbine_calculatorInput): Record<string, number> {
  ...
  try { const v = input.diameter * input.airDensity * input.windSpeed * input.cp; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.diameter * input.airDensity * input.windSpeed * input.cp * ((input.efficiency / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
