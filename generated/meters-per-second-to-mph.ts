...
function evaluateAllFormulas(input: Meters_per_second_to_mphInput): Record<string, number> {
  ...
  try { const v = input.speed_mps * 2.23694; results["speed_mph"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["speed_mph"] = Number.NaN; }
  try { const v = input.speed_mps * 2.23694; results["speed_mph_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["speed_mph_aux"] = Number.NaN; }
  return results;
}
