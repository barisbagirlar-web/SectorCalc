...
function evaluateAllFormulas(input: Fault_current_calculatorInput): Record<string, number> {
  ...
  try { const v = input.primaryVoltage * input.secondaryVoltage * input.sourceSCMVA * input.transformerKVA; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.primaryVoltage * input.secondaryVoltage * input.sourceSCMVA * input.transformerKVA * ((input.transformerPercentZ / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.transformerPercentZ / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
