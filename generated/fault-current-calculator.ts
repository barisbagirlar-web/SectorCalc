...
function evaluateAllFormulas(input: Fault_current_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.primaryVoltage * input.secondaryVoltage * input.sourceSCMVA * input.transformerKVA; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.primaryVoltage * input.secondaryVoltage * input.sourceSCMVA * input.transformerKVA * ((input.transformerPercentZ / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.transformerPercentZ / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
