...
function evaluateAllFormulas(input: Life_expectancy_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentAge * input.gender * input.smokingStatus * input.alcoholConsumption; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.currentAge * input.gender * input.smokingStatus * input.alcoholConsumption * (input.bmi * input.exerciseHours); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.bmi * input.exerciseHours; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
