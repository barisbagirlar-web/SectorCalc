...
function evaluateAllFormulas(input: Angstroms_to_nmInput): Record<string, number> {
  ...
  try { const v = input.angstroms * 0.1; results["nanometers"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["nanometers"] = 0; }
  try { const v = input.angstroms * 0.1; results["nanometers___angstroms___0_1"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["nanometers___angstroms___0_1"] = 0; }
  return results;
}
