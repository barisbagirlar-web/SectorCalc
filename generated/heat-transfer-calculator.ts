...
function evaluateAllFormulas(input: Heat_transfer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.area * (input.tempInside - input.tempOutside) / (1/input.hInside + input.thickness/input.thermalConductivity + 1/input.hOutside); results["heatTransferRate"] = Number.isFinite(v) ? v : 0; } catch { results["heatTransferRate"] = 0; }
  try { const v = 1 / (1/input.hInside + input.thickness/input.thermalConductivity + 1/input.hOutside); results["overallHeatTransferCoeff"] = Number.isFinite(v) ? v : 0; } catch { results["overallHeatTransferCoeff"] = 0; }
  try { const v = input.tempInside - input.tempOutside; results["deltaTemp"] = Number.isFinite(v) ? v : 0; } catch { results["deltaTemp"] = 0; }
  return results;
}
