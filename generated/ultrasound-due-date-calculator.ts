...
function evaluateAllFormulas(input: Ultrasound_due_date_calculatorInput): Record<string, number> {
  ...
  try { const v = input.corrosionRateMmPerYear * input.measurementConfidenceFactor; results["effectiveCorrosionRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["effectiveCorrosionRate"] = Number.NaN; }
  try { const v = input.corrosionRateMmPerYear * input.measurementConfidenceFactor; results["effectiveCorrosionRate_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["effectiveCorrosionRate_aux"] = Number.NaN; }
  return results;
}
