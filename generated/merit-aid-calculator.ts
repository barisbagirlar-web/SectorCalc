...
function evaluateAllFormulas(input: Merit_aid_calculatorInput): Record<string, number> {
  ...
  try { const v = input.coa * (input.meritCapPercent / 100); results["maxMerit"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["maxMerit"] = Number.NaN; }
  try { const v = input.coa * (input.meritCapPercent / 100); results["maxMerit_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["maxMerit_aux"] = Number.NaN; }
  return results;
}
