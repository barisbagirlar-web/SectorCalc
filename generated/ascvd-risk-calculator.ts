...
function evaluateAllFormulas(input: Ascvd_risk_calculatorInput): Record<string, number> {
  ...
  try { const v = (-5) + 0.05*input.age + 0.01*input.totalCholesterol - 0.02*input.hdlCholesterol + 0.01*input.systolicBP + 0.5*input.treatedHypertension + 0.5*input.diabetes + 0.5*input.smoker + 0.3*input.male; results["calcLogOdds"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["calcLogOdds"] = Number.NaN; }
  try { const v = (-5) + 0.05*input.age + 0.01*input.totalCholesterol - 0.02*input.hdlCholesterol + 0.01*input.systolicBP + 0.5*input.treatedHypertension + 0.5*input.diabetes + 0.5*input.smoker + 0.3*input.male; results["calcLogOdds_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["calcLogOdds_aux"] = Number.NaN; }
  return results;
}
