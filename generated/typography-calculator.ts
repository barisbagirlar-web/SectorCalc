...
function evaluateAllFormulas(input: Typography_calculatorInput): Record<string, number> {
  ...
  try { const v = input.viewingDistance * 2.5 * input.ageFactor * input.lightingFactor * input.safetyFactor; results["recommendedFontHeight"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["recommendedFontHeight"] = 0; }
  try { const v = input.viewingDistance * 2.5 * input.ageFactor * input.lightingFactor * input.safetyFactor; results["recommendedFontHeight_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["recommendedFontHeight_aux"] = 0; }
  return results;
}
...
export function calculateTypography_calculator(input: Typography_calculatorInput): Typography_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["recommendedFontHeight"]));
  const breakdown = {
