...
function evaluateAllFormulas(input: Life_insurance_needs_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annual_income * input.years_needed; results["income_replacement"] = Number.isFinite(v) ? v : 0; } catch { results["income_replacement"] = 0; }
  try { const v = (results["income_replacement"] ?? 0) + input.outstanding_debts + input.education_expenses + input.final_expenses; results["total_needs"] = Number.isFinite(v) ? v : 0; } catch { results["total_needs"] = 0; }
  try { const v = (results["total_needs"] ?? 0) - input.current_savings; results["recommended_coverage"] = Number.isFinite(v) ? v : 0; } catch { results["recommended_coverage"] = 0; }
  return results;
}
...
export function calculateLife_insurance_needs_calculator(input: Life_insurance_needs_calculatorInput): Life_insurance_needs_calculatorOutput {
  ...
  const totalWasteCost = values["recommended_coverage"] ?? 0;
  const breakdown = {
