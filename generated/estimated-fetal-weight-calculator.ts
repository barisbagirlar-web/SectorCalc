...
function evaluateAllFormulas(input: Estimated_fetal_weight_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bpd * input.hc * input.ac * input.fl; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.bpd * input.hc * input.ac * input.fl; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
