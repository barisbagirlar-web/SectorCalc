...
function evaluateAllFormulas(input: Waveguide_calculatorInput): Record<string, number> {
  ...
  try { const v = input.width * input.height * input.frequency * input.permittivity; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.width * input.height * input.frequency * input.permittivity; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
