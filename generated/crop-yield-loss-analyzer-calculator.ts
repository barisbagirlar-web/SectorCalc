...
function evaluateAllFormulas(input: Crop_yield_loss_analyzer_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.expected_yield * input.actual_yield * (input.harvest_loss_pct / 100) * (input.post_harvest_loss_pct / 100); results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.expected_yield * input.actual_yield * (input.harvest_loss_pct / 100) * (input.post_harvest_loss_pct / 100) * (input.pest_pressure * input.disease_index); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.pest_pressure * input.disease_index; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
