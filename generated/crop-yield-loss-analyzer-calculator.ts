...
function evaluateAllFormulas(input: Crop_yield_loss_analyzer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.expected_yield * input.actual_yield * (input.harvest_loss_pct / 100) * (input.post_harvest_loss_pct / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.expected_yield * input.actual_yield * (input.harvest_loss_pct / 100) * (input.post_harvest_loss_pct / 100) * (input.pest_pressure * input.disease_index); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.pest_pressure * input.disease_index; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
