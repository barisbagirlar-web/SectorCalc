...
function evaluateAllFormulas(input: Crop_yield_loss_analyzer_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.expected_yield + input.actual_yield + input.harvest_loss_pct; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.expected_yield + input.actual_yield + input.harvest_loss_pct; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
