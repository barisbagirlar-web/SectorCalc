...
function evaluateAllFormulas(input: Orbit_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.perigeeAltitude + input.apogeeAltitude + 2 * input.earthRadius) / 2; results["semiMajorAxis"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["semiMajorAxis"] = Number.NaN; }
  try { const v = (input.apogeeAltitude - input.perigeeAltitude) / (input.perigeeAltitude + input.apogeeAltitude + 2 * input.earthRadius); results["eccentricity"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["eccentricity"] = Number.NaN; }
  return results;
}
