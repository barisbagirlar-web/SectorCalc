...
function evaluateAllFormulas(input: Kg_to_tons_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.gross_kg - input.tare_kg) * input.batch_count; results["net_kg"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["net_kg"] = 0; }
  try { const v = (input.gross_kg * input.batch_count) / 1000; results["gross_tons"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["gross_tons"] = 0; }
  return results;
}
