...
function evaluateAllFormulas(input: Ode_solver_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.t0 + input.n * input.h; results["t_final"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["t_final"] = 0; }
  try { const v = input.y0 + input.b / input.a; results["constant_C"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["constant_C"] = 0; }
  return results;
}
