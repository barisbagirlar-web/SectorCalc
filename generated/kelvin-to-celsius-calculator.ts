...
function evaluateAllFormulas(input: Kelvin_to_celsius_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.kelvin * input.offset * input.precision * input.timestamp; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.kelvin * input.offset * input.precision * input.timestamp * (input.ambient_c * (input.humidity / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.ambient_c * (input.humidity / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
