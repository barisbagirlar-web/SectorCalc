...
function evaluateAllFormulas(input: Euclidean_algorithm_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.num1 * input.num2 * input.num3 * input.num4; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
