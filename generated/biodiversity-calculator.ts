...
function evaluateAllFormulas(input: Biodiversity_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.sp1>0?1:0)+(input.sp2>0?1:0)+(input.sp3>0?1:0)+(input.sp4>0?1:0)+(input.sp5>0?1:0)); results["speciesRichness"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["speciesRichness"] = 0; }
  try { const v = ((input.sp1>0?1:0)+(input.sp2>0?1:0)+(input.sp3>0?1:0)+(input.sp4>0?1:0)+(input.sp5>0?1:0)); results["speciesRichness_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["speciesRichness_aux"] = 0; }
  return results;
}
...
export function calculateBiodiversity_calculator(input: Biodiversity_calculatorInput): Biodiversity_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["speciesRichness_aux"]));
  const breakdown = {
