...
function evaluateAllFormulas(input: Basketball_shooting_percentage_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fieldGoalsAttempted > 0 ? (input.fieldGoalsMade / input.fieldGoalsAttempted) * 100 : 0; results["shootingPercentage"] = Number.isFinite(v) ? v : 0; } catch { results["shootingPercentage"] = 0; }
  try { const v = input.threePointersAttempted > 0 ? (input.threePointersMade / input.threePointersAttempted) * 100 : 0; results["threePointPercentage"] = Number.isFinite(v) ? v : 0; } catch { results["threePointPercentage"] = 0; }
  try { const v = input.freeThrowsAttempted > 0 ? (input.freeThrowsMade / input.freeThrowsAttempted) * 100 : 0; results["freeThrowPercentage"] = Number.isFinite(v) ? v : 0; } catch { results["freeThrowPercentage"] = 0; }
  return results;
}
...
export function calculateBasketball_shooting_percentage_calculator(input: Basketball_shooting_percentage_calculatorInput): Basketball_shooting_percentage_calculatorOutput {
  ...
  const totalWasteCost = values["shootingPercentage"] ?? 0;
  const breakdown = {
