...
function evaluateAllFormulas(input: Apgar_score_calculatorInput): Record<string, number> {
  ...
  try { const v = input.appearance * input.pulse * input.grimace * input.activity; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.appearance * input.pulse * input.grimace * input.activity * (input.respiration); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.respiration; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
