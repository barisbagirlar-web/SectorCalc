...
function evaluateAllFormulas(input: Meat_cooking_temperature_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.ovenTemp - input.startTemp) / (input.ovenTemp - input.targetTemp); results["tempRatio"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["tempRatio"] = Number.NaN; }
  try { const v = (input.ovenTemp - input.startTemp) / (input.ovenTemp - input.targetTemp); results["tempRatio_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["tempRatio_aux"] = Number.NaN; }
  return results;
}
