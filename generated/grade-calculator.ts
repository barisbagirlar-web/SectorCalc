...
function evaluateAllFormulas(input: Grade_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.finalElevation - input.initialElevation) / input.horizontalDistance) * 100; results["gradePercent"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["gradePercent"] = 0; }
  try { const v = input.horizontalDistance / (input.finalElevation - input.initialElevation); results["slopeRatio"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["slopeRatio"] = 0; }
  return results;
}
