...
function evaluateAllFormulas(input: Lenzs_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.n * input.dB * input.A * input.dt; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.n * input.dB * input.A * input.dt * (input.theta); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
