...
function evaluateAllFormulas(input: Payback_period_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialInvestment; results["totalInvestment"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalInvestment"] = Number.NaN; }
  try { const v = input.annualNetCashFlow; results["annualCashFlowDisplay"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annualCashFlowDisplay"] = Number.NaN; }
  return results;
}
