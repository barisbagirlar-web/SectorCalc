...
function evaluateAllFormulas(input: Margin_of_error_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.sample_size * (input.confidence_level / 100) * input.z_score * input.sample_proportion; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.sample_size * (input.confidence_level / 100) * input.z_score * input.sample_proportion; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
