...
function evaluateAllFormulas(input: Margin_of_error_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sample_size * (input.confidence_level / 100) * input.z_score * input.sample_proportion; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.sample_size * (input.confidence_level / 100) * input.z_score * input.sample_proportion; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
