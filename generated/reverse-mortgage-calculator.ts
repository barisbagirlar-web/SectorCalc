...
function evaluateAllFormulas(input: Reverse_mortgage_calculatorInput): Record<string, number> {
  ...
  try { const v = input.age * input.homeValue; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.age * input.homeValue * (1 + (input.interestRate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.age * input.homeValue * (1 + (input.interestRate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
