...
function evaluateAllFormulas(input: Madrs_calculatorInput): Record<string, number> {
  ...
  try { const v = input.axleDiameter * input.axleLength * input.load * input.elasticModulus; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.axleDiameter * input.axleLength * input.load * input.elasticModulus * (input.safetyFactor); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
