...
function evaluateAllFormulas(input: Log_base_calculatorInput): Record<string, number> {
  ...
  try { const v = input.argument * input.base * input.multiplier * input.offset; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.argument * input.base * input.multiplier * input.offset; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
