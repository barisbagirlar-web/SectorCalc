...
function evaluateAllFormulas(input: Risk_probability_calculatorInput): Record<string, number> {
  ...
  try { const v = input.failureRate * input.stressFactor / input.maintenanceFactor; results["effectiveFailureRate"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectiveFailureRate"] = 0; }
  try { const v = input.failureRate * input.stressFactor / input.maintenanceFactor; results["effectiveFailureRate_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectiveFailureRate_aux"] = 0; }
  return results;
}
