...
function evaluateAllFormulas(input: Amperes_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.permeability * input.current * input.wireCount / (2 * Math.PI * input.distance); results["magneticField"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["magneticField"] = 0; }
  try { const v = input.permeability * input.current * input.wireCount / (2 * Math.PI * input.distance); results["magneticField_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["magneticField_aux"] = 0; }
  return results;
}
...
export function calculateAmperes_law_calculator(input: Amperes_law_calculatorInput): Amperes_law_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["magneticField"]));
  const breakdown = {
