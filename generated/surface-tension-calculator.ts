...
function evaluateAllFormulas(input: Surface_tension_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.density * input.gravity * input.height * input.radius; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.density * input.gravity * input.height * input.radius * (input.contactAngle); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
