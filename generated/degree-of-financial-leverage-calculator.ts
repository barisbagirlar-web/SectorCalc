...
function evaluateAllFormulas(input: Degree_of_financial_leverage_calculatorInput): Record<string, number> {
  ...
  try { const v = input.ebit * input.interest * input.taxRate * input.preferredDividends; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.ebit * input.interest * input.taxRate * input.preferredDividends; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
