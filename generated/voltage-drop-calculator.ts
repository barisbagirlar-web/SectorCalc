...
function evaluateAllFormulas(input: Voltage_drop_calculatorInput): Record<string, number> {
  ...
  try { const v = input.circuit_length * input.load_current * input.system_voltage * input.power_factor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.circuit_length * input.load_current * input.system_voltage * input.power_factor * (input.ambient_temperature); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.ambient_temperature; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
