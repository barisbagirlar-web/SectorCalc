...
function evaluateAllFormulas(input: Ideal_weight_calculatorInput): Record<string, number> {
  ...
  try { const v = input.height_ft * input.height_in * input.gender * input.body_frame; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.height_ft * input.height_in * input.gender * input.body_frame; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
