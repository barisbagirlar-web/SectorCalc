...
function evaluateAllFormulas(input: Growing_annuity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.payment * input.growthRate * input.discountRate * input.periods; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.payment * input.growthRate * input.discountRate * input.periods; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
