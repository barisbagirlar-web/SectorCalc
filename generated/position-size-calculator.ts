...
function evaluateAllFormulas(input: Position_size_calculatorInput): Record<string, number> {
  ...
  try { const v = input.accountBalance * (input.riskPercent / 100); results["riskAmount"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["riskAmount"] = Number.NaN; }
  try { const v = input.accountBalance * (input.riskPercent / 100); results["riskAmount_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["riskAmount_aux"] = Number.NaN; }
  return results;
}
