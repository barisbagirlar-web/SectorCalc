...
function evaluateAllFormulas(input: Climbing_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.climberMass * input.fallFactor * input.ratedImpactForce * input.ratedMass; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.climberMass * input.fallFactor * input.ratedImpactForce * input.ratedMass * (input.ratedFallFactor * input.safetyFactor); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.ratedFallFactor * input.safetyFactor; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
