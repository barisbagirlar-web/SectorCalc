...
function evaluateAllFormulas(input: Climbing_calculatorInput): Record<string, number> {
  ...
  try { const v = input.climberMass * input.fallFactor * input.ratedImpactForce * input.ratedMass; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.climberMass * input.fallFactor * input.ratedImpactForce * input.ratedMass * (input.ratedFallFactor * input.safetyFactor); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.ratedFallFactor * input.safetyFactor; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
