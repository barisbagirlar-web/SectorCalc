...
function evaluateAllFormulas(input: Triple_integral_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.xUpper - input.xLower) * (input.yUpper - input.yLower) * (input.zUpper - input.zLower); results["volume"] = Number.isFinite(v) ? v : 0; } catch { results["volume"] = 0; }
  try { const v = input.density * (results["volume"] ?? 0); results["mass"] = Number.isFinite(v) ? v : 0; } catch { results["mass"] = 0; }
  return results;
}
