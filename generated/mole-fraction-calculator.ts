...
function evaluateAllFormulas(input: Mole_fraction_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.moleA * input.moleB * input.moleC * input.moleD; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.moleA * input.moleB * input.moleC * input.moleD * (input.selectedIndex); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.selectedIndex; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
