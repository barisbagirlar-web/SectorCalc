...
function evaluateAllFormulas(input: Mole_fraction_calculatorInput): Record<string, number> {
  ...
  try { const v = input.moleA * input.moleB * input.moleC * input.moleD; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.moleA * input.moleB * input.moleC * input.moleD * (input.selectedIndex); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.selectedIndex; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
