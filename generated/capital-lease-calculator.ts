...
function evaluateAllFormulas(input: Capital_lease_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.interestRate / 100) / input.paymentFrequency; results["r"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["r"] = Number.NaN; }
  try { const v = input.leaseTerm * input.paymentFrequency; results["n"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["n"] = Number.NaN; }
  return results;
}
