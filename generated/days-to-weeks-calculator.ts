...
function evaluateAllFormulas(input: Days_to_weeks_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalDays * input.daysPerWeek * input.precision * input.showRemainder; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.totalDays * input.daysPerWeek * input.precision * input.showRemainder; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
