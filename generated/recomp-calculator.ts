...
function evaluateAllFormulas(input: Recomp_calculatorInput): Record<string, number> {
  ...
  try { const v = input.baseAmount * (input.rate / 100) * (input.time / 12); results["interestAmount"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["interestAmount"] = 0; }
  try { const v = input.baseAmount + (asFormulaNumber(results["interestAmount"])) + input.expenses; results["totalBeforeAdjustment"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalBeforeAdjustment"] = 0; }
  try { const v = (asFormulaNumber(results["totalBeforeAdjustment"])) * input.adjustmentFactor; results["totalCompensation"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalCompensation"] = 0; }
  return results;
}
