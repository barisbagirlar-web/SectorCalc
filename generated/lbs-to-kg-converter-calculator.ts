...
function evaluateAllFormulas(input: Lbs_to_kg_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mass_lbs * (input.measurement_uncertainty_pct / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mass_lbs * (input.measurement_uncertainty_pct / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
