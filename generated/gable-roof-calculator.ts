...
function evaluateAllFormulas(input: Gable_roof_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.buildingLength; results["ridgeLength"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["ridgeLength"] = 0; }
  try { const v = input.buildingLength; results["ridgeLength_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["ridgeLength_aux"] = 0; }
  return results;
}
