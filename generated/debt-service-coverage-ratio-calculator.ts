...
function evaluateAllFormulas(input: Debt_service_coverage_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.netOperatingIncome * input.loanAmount * (input.interestRate / 100) * input.loanTerm; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.netOperatingIncome * input.loanAmount * (input.interestRate / 100) * input.loanTerm; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
