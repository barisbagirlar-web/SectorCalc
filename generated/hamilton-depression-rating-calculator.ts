...
function evaluateAllFormulas(input: Hamilton_depression_rating_calculatorInput): Record<string, number> {
  ...
  try { const v = input.depth * input.area * (1 + (input.count - 1) * 0.5) * input.sharpness / input.materialFactor; results["totalScore"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalScore"] = 0; }
  try { const v = input.depth * input.area * input.sharpness / input.materialFactor; results["depthEffect"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["depthEffect"] = 0; }
  try { const v = 1 + (input.count - 1) * 0.5; results["countMultiplier"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["countMultiplier"] = 0; }
  return results;
}
...
export function calculateHamilton_depression_rating_calculator(input: Hamilton_depression_rating_calculatorInput): Hamilton_depression_rating_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["countMultiplier"]));
  const breakdown = {
