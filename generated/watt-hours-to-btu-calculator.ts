...
function evaluateAllFormulas(input: Watt_hours_to_btu_calculatorInput): Record<string, number> {
  ...
  try { const v = input.power * input.time * input.efficiency / 100; results["wattHours"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["wattHours"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["wattHours"])) * (input.btuType == 1 ? 3.412141633 : input.btuType == 2 ? 3.4142 : 3.608249); results["btu"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["btu"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["wattHours"])) / 1000 * input.electricityCost; results["cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["cost"] = Number.NaN; }
  return results;
}
