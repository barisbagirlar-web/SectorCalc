...
function evaluateAllFormulas(input: Steak_temperature_calculatorInput): Record<string, number> {
  ...
  try { const v = input.thickness * input.initialTemp * input.targetTemp * input.grillTemp; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.thickness * input.initialTemp * input.targetTemp * input.grillTemp; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
