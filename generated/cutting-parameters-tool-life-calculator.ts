...
function evaluateAllFormulas(input: Cutting_parameters_tool_life_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cutting_speed * (input.feed_rate / 100) * input.depth_of_cut * input.machine_stability_factor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.cutting_speed * (input.feed_rate / 100) * input.depth_of_cut * input.machine_stability_factor; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
...
export const Cutting_parameters_tool_life_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
