...
function evaluateAllFormulas(input: Lh_surge_calculatorInput): Record<string, number> {
  ...
  try { const v = input.flowVelocity * input.pipeDiameter * input.pipeLength * input.valveCloseTime; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.flowVelocity * input.pipeDiameter * input.pipeLength * input.valveCloseTime * (input.waveSpeed * input.fluidDensity * input.wallThickness); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.waveSpeed * input.fluidDensity * input.wallThickness; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
