...
function evaluateAllFormulas(input: Interest_coverage_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.ebit / input.interestExpense; results["interestCoverageRatio"] = Number.isFinite(v) ? v : 0; } catch { results["interestCoverageRatio"] = 0; }
  try { const v = input.ebitda / input.interestExpense; results["ebitdaCoverageRatio"] = Number.isFinite(v) ? v : 0; } catch { results["ebitdaCoverageRatio"] = 0; }
  return results;
}
...
export function calculateInterest_coverage_ratio_calculator(input: Interest_coverage_ratio_calculatorInput): Interest_coverage_ratio_calculatorOutput {
  ...
  const totalWasteCost = values["interestCoverageRatio"] ?? 0;
  const breakdown = {
