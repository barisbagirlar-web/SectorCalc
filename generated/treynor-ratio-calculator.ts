...
function evaluateAllFormulas(input: Treynor_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.portfolioReturn / 100) * (input.riskFreeRate / 100) * input.portfolioBeta * input.periodsPerYear; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = (input.portfolioReturn / 100) * (input.riskFreeRate / 100) * input.portfolioBeta * input.periodsPerYear; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
