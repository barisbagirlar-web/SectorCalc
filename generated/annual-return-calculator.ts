...
function evaluateAllFormulas(input: Annual_return_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.finalValue / input.initialInvestment) ** (1 / input.years) - 1; results["nominalAnnualReturn"] = Number.isFinite(v) ? v : 0; } catch { results["nominalAnnualReturn"] = 0; }
  try { const v = (1 + ((input.finalValue / input.initialInvestment) ** (1 / input.years) - 1)) / (1 + input.inflationRate / 100) - 1; results["realAnnualReturn"] = Number.isFinite(v) ? v : 0; } catch { results["realAnnualReturn"] = 0; }
  try { const v = input.finalValue / input.initialInvestment; results["totalReturnMultiple"] = Number.isFinite(v) ? v : 0; } catch { results["totalReturnMultiple"] = 0; }
  return results;
}
...
export function calculateAnnual_return_calculator(input: Annual_return_calculatorInput): Annual_return_calculatorOutput {
  ...
  const totalWasteCost = values["nominalAnnualReturn"] ?? 0;
  const breakdown = {
