...
function evaluateAllFormulas(input: Annual_return_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.finalValue / input.initialInvestment) ** (1 / input.years) - 1; results["nominalAnnualReturn"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["nominalAnnualReturn"] = Number.NaN; }
  try { const v = (1 + ((input.finalValue / input.initialInvestment) ** (1 / input.years) - 1)) / (1 + input.inflationRate / 100) - 1; results["realAnnualReturn"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["realAnnualReturn"] = Number.NaN; }
  try { const v = input.finalValue / input.initialInvestment; results["totalReturnMultiple"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalReturnMultiple"] = Number.NaN; }
  return results;
}
