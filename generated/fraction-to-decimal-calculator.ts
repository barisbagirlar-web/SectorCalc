...
function evaluateAllFormulas(input: Fraction_to_decimal_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numerator * input.denominator * input.wholeNumber * input.decimalPlaces; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.numerator * input.denominator * input.wholeNumber * input.decimalPlaces * (input.uncertaintyInput); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.uncertaintyInput; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
