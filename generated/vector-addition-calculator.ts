...
function evaluateAllFormulas(input: Vector_addition_calculatorInput): Record<string, number> {
  ...
  try { const v = input.vector1_magnitude * input.vector1_angle * input.vector2_magnitude * input.vector2_angle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.vector1_magnitude * input.vector1_angle * input.vector2_magnitude * input.vector2_angle * ((input.confidence_level / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.confidence_level / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
