...
function evaluateAllFormulas(input: Prime_number_calculatorInput): Record<string, number> {
  ...
  try { const v = input.lowerBound * input.upperBound * input.sampleSize * (input.tolerance / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.lowerBound * input.upperBound * input.sampleSize * (input.tolerance / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
