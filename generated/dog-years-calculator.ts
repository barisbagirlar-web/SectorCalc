...
function evaluateAllFormulas(input: Dog_years_calculatorInput): Record<string, number> {
  ...
  try { const v = input.humanAge * input.breedSize * input.weight * input.activityLevel; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.humanAge * input.breedSize * input.weight * input.activityLevel; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
