...
function evaluateAllFormulas(input: Dog_years_calculatorInput): Record<string, number> {
  ...
  try { const v = input.humanAge * input.breedSize * input.weight * input.activityLevel; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.humanAge * input.breedSize * input.weight * input.activityLevel; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
