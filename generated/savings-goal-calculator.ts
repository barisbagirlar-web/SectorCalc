...
function evaluateAllFormulas(input: Savings_goal_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.currentSavings * (1 + (asFormulaNumber(results["monthlyRate"]))) ** (input.numberOfYears * 12) + input.monthlyContribution * (((1 + (asFormulaNumber(results["monthlyRate"]))) ** (input.numberOfYears * 12) - 1) / (asFormulaNumber(results["monthlyRate"]))); results["totalFutureValue"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalFutureValue"] = 0; }
  try { const v = input.currentSavings + input.monthlyContribution * input.numberOfYears * 12; results["totalContributions"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalContributions"] = 0; }
  try { const v = (asFormulaNumber(results["totalFutureValue"])) - (asFormulaNumber(results["totalContributions"])); results["interestEarned"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["interestEarned"] = 0; }
  try { const v = (asFormulaNumber(results["totalFutureValue"])) - input.goalAmount; results["goalDifference"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["goalDifference"] = 0; }
  return results;
}
