...
function evaluateAllFormulas(input: Shakes_to_nanoseconds_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.shakes * input.conversionFactor; results["exactNanoseconds"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["exactNanoseconds"] = 0; }
  try { const v = input.shakes * input.conversionFactor; results["exactNanoseconds_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["exactNanoseconds_aux"] = 0; }
  return results;
}
