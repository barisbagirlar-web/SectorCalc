...
function evaluateAllFormulas(input: Race_equivalent_calculatorInput): Record<string, number> {
  ...
  try { const v = input.knownDistance * input.knownTimeHours * input.knownTimeMinutes * input.knownTimeSeconds; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.knownDistance * input.knownTimeHours * input.knownTimeMinutes * input.knownTimeSeconds * (input.targetDistance); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.targetDistance; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
