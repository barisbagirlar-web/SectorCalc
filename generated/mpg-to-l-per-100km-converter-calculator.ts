...
function evaluateAllFormulas(input: Mpg_to_l_per_100km_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mpg_value * input.annual_mileage * input.fuel_price_per_gallon * input.co2_per_gallon; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.mpg_value * input.annual_mileage * input.fuel_price_per_gallon * input.co2_per_gallon; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
