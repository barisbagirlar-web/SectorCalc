...
function evaluateAllFormulas(input: Kc_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.d30 ** 2) / (input.d10 * input.d60); results["primary"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["primary"] = Number.NaN; }
  try { const v = input.d30 ** 2; results["numerator"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["numerator"] = Number.NaN; }
  try { const v = input.d10 * input.d60; results["denominator"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["denominator"] = Number.NaN; }
  return results;
}
