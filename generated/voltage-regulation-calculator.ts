...
function evaluateAllFormulas(input: Voltage_regulation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.receivingVoltage * input.loadCurrent * input.lineResistance * input.lineReactance; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.receivingVoltage * input.loadCurrent * input.lineResistance * input.lineReactance * (input.powerFactor); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.powerFactor; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
