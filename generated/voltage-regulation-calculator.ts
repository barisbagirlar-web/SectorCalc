...
function evaluateAllFormulas(input: Voltage_regulation_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.receivingVoltage * input.loadCurrent * input.lineResistance * input.lineReactance; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.receivingVoltage * input.loadCurrent * input.lineResistance * input.lineReactance * (input.powerFactor); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.powerFactor; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
