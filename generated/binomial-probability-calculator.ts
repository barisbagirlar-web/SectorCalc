...
function evaluateAllFormulas(input: Binomial_probability_calculatorInput): Record<string, number> {
  ...
  try { const v = input.n * input.k * input.p * input.precision; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.n * input.k * input.p * input.precision; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
