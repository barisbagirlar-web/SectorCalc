...
function evaluateAllFormulas(input: Tool_wear_cost_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.tool_cost + input.tool_life_parts + input.replacement_time_min; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.tool_cost + input.tool_life_parts + input.replacement_time_min; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
