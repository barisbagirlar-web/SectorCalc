...
function evaluateAllFormulas(input: Rectangular_prism_volume_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.length * input.width * input.height * input.quantity) / 1000000000; results["volume_m3"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["volume_m3"] = 0; }
  try { const v = input.length * input.width * input.height * input.quantity; results["volume_mm3"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["volume_mm3"] = 0; }
  return results;
}
...
export function calculateRectangular_prism_volume_calculator(input: Rectangular_prism_volume_calculatorInput): Rectangular_prism_volume_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["volume_m3"]));
  const breakdown = {
