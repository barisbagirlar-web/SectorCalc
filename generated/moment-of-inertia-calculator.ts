...
function evaluateAllFormulas(input: Moment_of_inertia_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.shape; results["1"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["1"] = 0; }
  try { const v = input.shape; results["2"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["2"] = 0; }
  try { const v = input.shape; results["3"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["3"] = 0; }
  try { const v = input.shape; results["4"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["4"] = 0; }
  return results;
}
