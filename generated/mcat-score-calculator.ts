...
function evaluateAllFormulas(input: Mcat_score_calculatorInput): Record<string, number> {
  ...
  try { const v = input.raw_cpbs * input.raw_cars * input.raw_bbls * input.raw_psbb; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.raw_cpbs * input.raw_cars * input.raw_bbls * input.raw_psbb; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
