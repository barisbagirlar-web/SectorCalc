...
function evaluateAllFormulas(input: Overtime_vs_hiring_breakevenInput): Record<string, number> {
  ...
  try { results["effective_hours_per_fte_regular"] = 40 * (1 - input.quality_defect_rate_normal / 100); } catch { results["effective_hours_per_fte_regular"] = 0; }
  try { results["effective_overtime_hours_per_fte"] = input.overtime_hours_per_week * (input.productivity_factor_overtime / 100) * (1 - (input.quality_defect_rate_normal + input.quality_defect_rate_overtime) / 100); } catch { results["effective_overtime_hours_per_fte"] = 0; }
  try { results["total_effective_hours_overtime_scenario"] = input.current_headcount * ((results["effective_hours_per_fte_regular"] ?? 0) + (results["effective_overtime_hours_per_fte"] ?? 0)); } catch { results["total_effective_hours_overtime_scenario"] = 0; }
  try { results["fte_shortfall"] = Math.max(0, (input.weekly_productive_hours_needed - input.current_headcount * (results["effective_hours_per_fte_regular"] ?? 0)) / (40 * (input.productivity_factor_new_hire / 100) * (1 - input.quality_defect_rate_normal / 100))); } catch { results["fte_shortfall"] = 0; }
  try { results["annual_cost_overtime"] = input.current_headcount * (40 * input.avg_hourly_wage * input.weeks_per_year) + input.current_headcount * (input.overtime_hours_per_week * input.avg_hourly_wage * input.overtime_premium * input.weeks_per_year) + (input.current_headcount * input.overtime_hours_per_week * (input.quality_defect_rate_normal + input.quality_defect_rate_overtime) / 100 * input.cost_per_defect * input.weeks_per_year) * (input.include_risk_adjustment ? input.risk_multiplier : 1); } catch { results["annual_cost_overtime"] = 0; }
  try { results["annual_cost_hiring"] = (input.current_headcount + (results["fte_shortfall"] ?? 0)) * (40 * input.avg_hourly_wage * input.weeks_per_year + input.annual_benefits_cost_per_fte) + (results["fte_shortfall"] ?? 0) * input.hiring_cost_per_fte + (input.current_headcount + (results["fte_shortfall"] ?? 0)) * 40 * (input.quality_defect_rate_normal / 100) * input.cost_per_defect * input.weeks_per_year; } catch { results["annual_cost_hiring"] = 0; }
  try { results["breakeven_hours"] = ((results["annual_cost_hiring"] ?? 0) - input.current_headcount * 40 * input.avg_hourly_wage * input.weeks_per_year) / (input.current_headcount * (input.avg_hourly_wage * input.overtime_premium * input.weeks_per_year + (input.quality_defect_rate_normal + input.quality_defect_rate_overtime) / 100 * input.cost_per_defect * input.weeks_per_year * (input.include_risk_adjustment ? input.risk_multiplier : 1))); } catch { results["breakeven_hours"] = 0; }
  return results;
}
