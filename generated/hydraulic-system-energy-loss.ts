...
function evaluateAllFormulas(input: Hydraulic_system_energy_lossInput): Record<string, number> {
  ...
  try { results["reynolds_number"] = (v * D) / ν; } catch { results["reynolds_number"] = 0; }
  results["darcy_friction_factor"] = 0;
  results["pipe_friction_loss"] = 0;
  results["minor_losses"] = 0;
  try { results["total_pressure_loss"] = ΔP_total = ΔP_pipe + ΔP_minor; } catch { results["total_pressure_loss"] = 0; }
  try { results["primary_energy_loss"] = ΔP_total * Q; } catch { results["primary_energy_loss"] = 0; }
  return results;
}
