...
function evaluateAllFormulas(input: Drum_tuning_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.drumDiameter * input.headThickness * input.headDensity * input.tension; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.drumDiameter * input.headThickness * input.headDensity * input.tension * (input.densityAdjustment); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.densityAdjustment; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
