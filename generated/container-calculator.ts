...
function evaluateAllFormulas(input: Container_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalQuantity * input.itemsPerPallet * input.palletsPerContainer * (input.reservePercent / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.totalQuantity * input.itemsPerPallet * input.palletsPerContainer * (input.reservePercent / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
