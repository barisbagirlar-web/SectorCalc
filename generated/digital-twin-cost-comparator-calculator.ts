...
function evaluateAllFormulas(input: Digital_twin_cost_comparator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.asset_count * input.sensor_density * input.data_quality_index * (input.labor_rate / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.asset_count * input.sensor_density * input.data_quality_index * (input.labor_rate / 100) * (input.expected_lifespan); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.expected_lifespan; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
