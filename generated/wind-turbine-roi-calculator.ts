...
function evaluateAllFormulas(input: Wind_turbine_roi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.project_lifetime * input.capital_cost_per_mw; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.project_lifetime * input.capital_cost_per_mw * (1 + (input.capacity_factor / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.project_lifetime * input.capital_cost_per_mw * (1 + (input.capacity_factor / 100)) * (input.turbine_capacity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.turbine_capacity; results["factor_turbine_capacity"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_turbine_capacity"] = Number.NaN; }
  return results;
}
...
export function calculateWind_turbine_roi_calculator(input: Wind_turbine_roi_calculatorInput): Wind_turbine_roi_calculatorOutput {
  ...
  const breakdown = {
    ...
    factor_turbine_capacity: toNumericFormulaValue(values["factor_turbine_capacity"])
  };
...
export interface Wind_turbine_roi_calculatorOutput {
  ...
  unit: string;
  breakdown: { base_cost: number; adjusted_cost: number; factor_turbine_capacity: number };
...
export const Wind_turbine_roi_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
  breakdownKeys: ["base_cost","adjusted_cost","factor_turbine_capacity"],
} as const;
