...
function evaluateAllFormulas(input: Race_time_predictor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.d1 * input.t1 * input.d2 * input.fatigue; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.d1 * input.t1 * input.d2 * input.fatigue; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
