...
function evaluateAllFormulas(input: Race_time_predictor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.d1 * input.t1 * input.d2 * input.fatigue; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.d1 * input.t1 * input.d2 * input.fatigue; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
