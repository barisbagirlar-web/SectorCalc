...
function evaluateAllFormulas(input: Treadmill_incline_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.distance * 5280 * (input.inclinePercent / 100); results["elevationGain"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["elevationGain"] = 0; }
  try { const v = ( (0.2 * (input.speed * 26.8224) + 0.9 * (input.speed * 26.8224) * (input.inclinePercent / 100) + 3.5) * (input.weight / 2.20462) * input.time * 5 ) / 1000; results["caloriesBurned"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["caloriesBurned"] = 0; }
  try { const v = (( (0.2 * (input.speed * 26.8224) + 0.9 * (input.speed * 26.8224) * (input.inclinePercent / 100) + 3.5) * (input.weight / 2.20462) * input.time * 5 ) / 1000) / (((0.2 * (input.speed * 26.8224) + 3.5) * (input.weight / 2.20462) * 5 ) / 1000 * (60 / input.speed)); results["equivalentFlatDistance"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["equivalentFlatDistance"] = 0; }
  return results;
}
