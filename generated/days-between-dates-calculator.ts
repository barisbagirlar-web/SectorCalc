...
function evaluateAllFormulas(input: Days_between_dates_calculatorInput): Record<string, number> {
  ...
  try { const v = input.startYear * input.startMonth * input.startDay * input.endYear; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.startYear * input.startMonth * input.startDay * input.endYear * (input.endMonth * input.endDay); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.endMonth * input.endDay; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
