...
function evaluateAllFormulas(input: Safe_note_calculatorInput): Record<string, number> {
  ...
  try { const v = input.preMoneyValuation * (1 - input.discountRate/100); results["discountedVal"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["discountedVal"] = 0; }
  try { const v = input.preMoneyValuation * (1 - input.discountRate/100); results["discountedVal_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["discountedVal_aux"] = 0; }
  return results;
}
...
export function calculateSafe_note_calculator(input: Safe_note_calculatorInput): Safe_note_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["discountedVal_aux"]));
  const breakdown = {
