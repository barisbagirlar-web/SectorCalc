...
function evaluateAllFormulas(input: Cramers_rule_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a11 * input.a12 * input.a13 * input.b1; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.a11 * input.a12 * input.a13 * input.b1 * (input.a21 * input.a22 * input.a23 * input.b2); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.a21 * input.a22 * input.a23 * input.b2; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
