...
function evaluateAllFormulas(input: Cramers_rule_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.a11 * input.a12 * input.a13 * input.b1; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.a11 * input.a12 * input.a13 * input.b1 * (input.a21 * input.a22 * input.a23 * input.b2); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.a21 * input.a22 * input.a23 * input.b2; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
