...
function evaluateAllFormulas(input: Universal_life_calculatorInput): Record<string, number> {
  ...
  try { const v = input.interestRate / 100; results["interestRateDecimal"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["interestRateDecimal"] = Number.NaN; }
  try { const v = input.expenseCharge / 100; results["expenseChargeDecimal"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["expenseChargeDecimal"] = Number.NaN; }
  try { const v = input.annualPremium * input.policyDuration; results["totalPremiumsPaid"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalPremiumsPaid"] = Number.NaN; }
  return results;
}
