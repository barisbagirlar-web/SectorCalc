...
function evaluateAllFormulas(input: Reliability_calculatorInput): Record<string, number> {
  ...
  try { const v = input.failureRate * input.missionTime * input.seriesCount * input.parallelCount; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.failureRate * input.missionTime * input.seriesCount * input.parallelCount; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
