...
function evaluateAllFormulas(input: Reliability_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.failureRate + input.missionTime + input.seriesCount; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.failureRate + input.missionTime + input.seriesCount; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
