...
function evaluateAllFormulas(input: Building_envelope_size_calculatorInput): Record<string, number> {
  ...
  try { const v = 2 * (input.length + input.width) * input.height; results["wallArea"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["wallArea"] = 0; }
  try { const v = 2 * (input.length + input.width) * input.height; results["wallArea_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["wallArea_aux"] = 0; }
  return results;
}
...
export function calculateBuilding_envelope_size_calculator(input: Building_envelope_size_calculatorInput): Building_envelope_size_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["wallArea_aux"]));
  const breakdown = {
