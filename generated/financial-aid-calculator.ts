...
function evaluateAllFormulas(input: Financial_aid_calculatorInput): Record<string, number> {
  ...
  try { const v = input.dependents * input.annualIncome; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.dependents * input.annualIncome; results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.dependents * input.annualIncome; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
