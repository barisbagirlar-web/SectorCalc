...
function evaluateAllFormulas(input: Far_point_calculatorInput): Record<string, number> {
  ...
  try { const v = input.earthRadius / (1 - input.refractionCoefficient); results["effectiveRadius"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectiveRadius"] = 0; }
  try { const v = input.earthRadius / (1 - input.refractionCoefficient); results["effectiveRadius_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectiveRadius_aux"] = 0; }
  return results;
}
...
export function calculateFar_point_calculator(input: Far_point_calculatorInput): Far_point_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["effectiveRadius"]));
  const breakdown = {
