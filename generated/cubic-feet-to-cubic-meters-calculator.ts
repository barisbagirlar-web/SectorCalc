...
function evaluateAllFormulas(input: Cubic_feet_to_cubic_meters_calculatorInput): Record<string, number> {
  ...
  try { const v = input.lengthFeet * input.widthFeet * input.heightFeet; results["volumeCubicFeet"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["volumeCubicFeet"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["volumeCubicFeet"])) * 0.028316846592; results["rawCubicMeters"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawCubicMeters"] = Number.NaN; }
  return results;
}
