...
function evaluateAllFormulas(input: Fitness_age_calculatorInput): Record<string, number> {
  ...
  try { const v = input.age * input.restingHeartRate * input.gender * input.maxHeartRate; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.age * input.restingHeartRate * input.gender * input.maxHeartRate; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
