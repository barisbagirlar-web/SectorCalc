...
function evaluateAllFormulas(input: Parts_per_million_calculatorInput): Record<string, number> {
  ...
  try { const v = input.solutionMass !== 0 ? (input.soluteMass / input.solutionMass) * 1000000 : 0; results["ppmMass"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["ppmMass"] = 0; }
  try { const v = input.solutionVolume !== 0 ? (input.soluteVolume / input.solutionVolume) * 1000000 : 0; results["ppmVolume"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["ppmVolume"] = 0; }
  return results;
}
...
export function calculateParts_per_million_calculator(input: Parts_per_million_calculatorInput): Parts_per_million_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["ppmMass"]));
  const breakdown = {
