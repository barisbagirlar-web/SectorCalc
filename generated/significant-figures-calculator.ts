...
function evaluateAllFormulas(input: Significant_figures_calculatorInput): Record<string, number> {
  ...
  try { const v = input.value * input.sigFigs * input.auto_input_3; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.value * input.sigFigs * input.auto_input_3; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
