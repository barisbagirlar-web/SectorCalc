...
function evaluateAllFormulas(input: Sqft_to_sqmInput): Record<string, number> {
  ...
  try { const v = input.sqft * 0.09290304; results["sqm"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["sqm"] = 0; }
  try { const v = input.sqft * 0.09290304; results["sqm_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["sqm_aux"] = 0; }
  return results;
}
...
export function calculateSqft_to_sqm(input: Sqft_to_sqmInput): Sqft_to_sqmOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["sqm"]));
  const breakdown = {
