...
function evaluateAllFormulas(input: Chains_to_links_calculatorInput): Record<string, number> {
  ...
  try { const v = input.chains * input.linksPerChain; results["rawLinks"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["rawLinks"] = 0; }
  try { const v = input.chains * input.linksPerChain; results["rawLinks_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["rawLinks_aux"] = 0; }
  return results;
}
...
export function calculateChains_to_links_calculator(input: Chains_to_links_calculatorInput): Chains_to_links_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["rawLinks_aux"]));
  const breakdown = {
