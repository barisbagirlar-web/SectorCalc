...
function evaluateAllFormulas(input: Chains_to_links_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.chains * input.linksPerChain; results["rawLinks"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["rawLinks"] = 0; }
  try { const v = input.chains * input.linksPerChain; results["rawLinks_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["rawLinks_aux"] = 0; }
  return results;
}
