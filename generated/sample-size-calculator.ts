...
function evaluateAllFormulas(input: Sample_size_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.zScore**2 * input.proportion * (1 - input.proportion)) / (input.marginOfError**2); results["sampleSizeInfinite"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["sampleSizeInfinite"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["sampleSizeInfinite"])) / (1 + ((toNumericFormulaValue(results["sampleSizeInfinite"])) - 1) / input.populationSize); results["sampleSizeFinite"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["sampleSizeFinite"] = Number.NaN; }
  return results;
}
