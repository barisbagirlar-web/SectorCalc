...
function evaluateAllFormulas(input: Medicare_part_d_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.annualDrugCost * input.deductible * (input.initialCoinsurance / 100) * input.catastrophicThreshold; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.annualDrugCost * input.deductible * (input.initialCoinsurance / 100) * input.catastrophicThreshold * ((input.catastrophicCoinsurance / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.catastrophicCoinsurance / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
