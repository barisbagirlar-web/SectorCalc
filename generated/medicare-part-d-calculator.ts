...
function evaluateAllFormulas(input: Medicare_part_d_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualDrugCost * input.deductible * (input.initialCoinsurance / 100) * input.catastrophicThreshold; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.annualDrugCost * input.deductible * (input.initialCoinsurance / 100) * input.catastrophicThreshold * ((input.catastrophicCoinsurance / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.catastrophicCoinsurance / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
