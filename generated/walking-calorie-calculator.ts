...
function evaluateAllFormulas(input: Walking_calorie_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = 3.5 + ((input.speed * 1000 / 60) * 0.1) + ((input.grade / 100) * (input.speed * 1000 / 60) * 1.8); results["VO2"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["VO2"] = 0; }
  try { const v = (asFormulaNumber(results["VO2"])) / 3.5; results["MET"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["MET"] = 0; }
