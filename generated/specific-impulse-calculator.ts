...
function evaluateAllFormulas(input: Specific_impulse_calculatorInput): Record<string, number> {
  ...
  try { const v = input.thrust / (input.massFlowRate * input.standardGravity); results["primary"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["primary"] = 0; }
  try { const v = input.thrust / (input.massFlowRate * input.standardGravity); results["primary_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["primary_aux"] = 0; }
  return results;
}
