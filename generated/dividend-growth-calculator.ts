...
function evaluateAllFormulas(input: Dividend_growth_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentDividend * (input.growthRate / 100) * (input.requiredRate / 100) * input.years; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.currentDividend * (input.growthRate / 100) * (input.requiredRate / 100) * input.years; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
