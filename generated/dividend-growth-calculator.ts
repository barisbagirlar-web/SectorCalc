...
function evaluateAllFormulas(input: Dividend_growth_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.currentDividend * (input.growthRate / 100) * (input.requiredRate / 100) * input.years; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.currentDividend * (input.growthRate / 100) * (input.requiredRate / 100) * input.years; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
