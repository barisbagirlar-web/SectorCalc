...
function evaluateAllFormulas(input: Number_needed_to_treat_calculatorInput): Record<string, number> {
  ...
  try { const v = input.controlEvents / input.controlTotal * 100; results["controlRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["controlRate"] = Number.NaN; }
  try { const v = input.treatmentEvents / input.treatmentTotal * 100; results["treatmentRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["treatmentRate"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["controlRate"])) - (toNumericFormulaValue(results["treatmentRate"])); results["arr"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["arr"] = Number.NaN; }
  return results;
}
