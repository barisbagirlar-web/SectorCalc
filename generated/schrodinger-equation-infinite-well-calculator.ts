...
function evaluateAllFormulas(input: Schrodinger_equation_infinite_well_calculatorInput): Record<string, number> {
  ...
  try { const v = input.n * input.m * input.L * input.x; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.n * input.m * input.L * input.x * (input.hbar); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.hbar; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
