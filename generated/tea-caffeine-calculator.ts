...
function evaluateAllFormulas(input: Tea_caffeine_calculatorInput): Record<string, number> {
  ...
  try { const v = input.teaCaffeinePerGram * input.teaAmount * input.steepingTime * input.waterTemperature; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.teaCaffeinePerGram * input.teaAmount * input.steepingTime * input.waterTemperature; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
