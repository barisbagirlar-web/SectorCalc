...
function evaluateAllFormulas(input: Conic_section_calculatorInput): Record<string, number> {
  ...
  try { const v = input.B**2 - 4*input.A*input.C; results["discriminant"] = Number.isFinite(v) ? v : 0; } catch { results["discriminant"] = 0; }
  try { const v = (input.B === 0 && input.A === input.C) ? 0 : (input.B**2 - 4*input.A*input.C < 0 ? 1 : (input.B**2 - 4*input.A*input.C === 0 ? 2 : 3)); results["conicType"] = Number.isFinite(v) ? v : 0; } catch { results["conicType"] = 0; }
  return results;
}
...
export function calculateConic_section_calculator(input: Conic_section_calculatorInput): Conic_section_calculatorOutput {
  ...
  const totalWasteCost = values["conicType"] ?? 0;
  const breakdown = {
