...
function evaluateAllFormulas(input: Auto_repair_quote_consistency_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.labor_rate + input.labor_hours + input.parts_cost; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.labor_rate + input.labor_hours + input.parts_cost; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
