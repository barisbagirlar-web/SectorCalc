...
function evaluateAllFormulas(input: Divide_decimals_calculatorInput): Record<string, number> {
  ...
  try { const v = input.divisor === 0 ? 0 : input.dividend / input.divisor; results["rawQuotient"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawQuotient"] = Number.NaN; }
  try { const v = input.divisor === 0 ? 0 : input.dividend / input.divisor; results["rawQuotient_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawQuotient_aux"] = Number.NaN; }
  return results;
}
