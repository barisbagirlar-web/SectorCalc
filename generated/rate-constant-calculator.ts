...
function evaluateAllFormulas(input: Rate_constant_calculatorInput): Record<string, number> {
  ...
  try { const v = -input.activationEnergy / (input.gasConstant * input.temperature); results["exponent"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["exponent"] = 0; }
  try { const v = -input.activationEnergy / (input.gasConstant * input.temperature); results["exponent_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["exponent_aux"] = 0; }
  return results;
}
...
export function calculateRate_constant_calculator(input: Rate_constant_calculatorInput): Rate_constant_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["exponent_aux"]));
  const breakdown = {
