...
function evaluateAllFormulas(input: Pascal_to_psi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.pressurePascals / 6894.76 * input.calibrationFactor; results["rawPsi"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["rawPsi"] = 0; }
  try { const v = (input.sensorMaxPa * input.sensorAccuracyClass / 100) / 6894.76; results["uncertaintyPsi"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["uncertaintyPsi"] = 0; }
  try { const v = (asFormulaNumber(results["rawPsi"])) !== 0 ? ((asFormulaNumber(results["uncertaintyPsi"])) / (asFormulaNumber(results["rawPsi"]))) * 100 : 0; results["relativeUncertaintyPercent"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["relativeUncertaintyPercent"] = 0; }
  return results;
}
...
export function calculatePascal_to_psi_calculator(input: Pascal_to_psi_calculatorInput): Pascal_to_psi_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["relativeUncertaintyPercent"]));
  const breakdown = {
