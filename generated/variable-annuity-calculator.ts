...
function evaluateAllFormulas(input: Variable_annuity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialBalance + input.annualContribution * input.years; results["totalContributions"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalContributions"] = Number.NaN; }
  try { const v = input.initialBalance + input.annualContribution * input.years; results["totalContributions_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalContributions_aux"] = Number.NaN; }
  return results;
}
