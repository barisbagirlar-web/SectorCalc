...
function evaluateAllFormulas(input: Salary_to_hourly_calculatorInput): Record<string, number | string> {
  ...
  try { const v = 1 * input.paid_weeks_per_year * input.work_hours_per_week * (input.benefits_percentage / 100); results["annual_kwh"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["annual_kwh"] = 0; }
  try { const v = 1 * input.paid_weeks_per_year * input.work_hours_per_week * (input.benefits_percentage / 100) * input.annual_salary; results["annual_energy_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["annual_energy_cost"] = 0; }
  try { const v = 1 * input.paid_weeks_per_year * input.work_hours_per_week * (input.benefits_percentage / 100) * input.annual_salary; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
