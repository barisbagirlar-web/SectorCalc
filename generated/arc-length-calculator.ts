...
function evaluateAllFormulas(input: Arc_length_calculatorInput): Record<string, number> {
  ...
  try { const v = input.radius * input.startAngle * input.endAngle * input.angleUnit; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.radius * input.startAngle * input.endAngle * input.angleUnit; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
