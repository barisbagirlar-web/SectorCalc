...
function evaluateAllFormulas(input: Rule_of_114_calculatorInput): Record<string, number> {
  ...
  try { const v = 114 / input.annualInterestRate; results["approximateYears"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["approximateYears"] = Number.NaN; }
  try { const v = input.initialInvestment * input.targetMultiplier; results["futureValue"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["futureValue"] = Number.NaN; }
  return results;
}
