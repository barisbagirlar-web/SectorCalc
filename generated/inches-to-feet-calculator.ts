...
function evaluateAllFormulas(input: Inches_to_feet_calculatorInput): Record<string, number> {
  ...
  try { const v = input.inch1 * input.inch2 * input.inch3 * input.inch4; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.inch1 * input.inch2 * input.inch3 * input.inch4 * (input.precision); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
