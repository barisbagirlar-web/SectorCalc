...
function evaluateAllFormulas(input: Qrisk_calculatorInput): Record<string, number> {
  ...
  try { const v = -5.5 + 0.05 * input.age + 0.01 * input.systolicBP + 0.005 * input.totalCholesterol - 0.008 * input.hdlCholesterol + 0.5 * input.smoker + 0.3 * input.diabetes; results["beta"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["beta"] = Number.NaN; }
  try { const v = -5.5 + 0.05 * input.age + 0.01 * input.systolicBP + 0.005 * input.totalCholesterol - 0.008 * input.hdlCholesterol + 0.5 * input.smoker + 0.3 * input.diabetes; results["beta_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["beta_aux"] = Number.NaN; }
  return results;
}
