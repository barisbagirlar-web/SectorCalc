...
function evaluateAllFormulas(input: Calibration_drift_risk_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.calibration_interval_days * input.days_since_last_calibration * (input.measurement_tolerance_pct / 100) * (input.observed_drift_pct / 100); results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.calibration_interval_days * input.days_since_last_calibration * (input.measurement_tolerance_pct / 100) * (input.observed_drift_pct / 100) * ((input.historical_failure_rate / 100) * input.data_confidence_score); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.historical_failure_rate / 100) * input.data_confidence_score; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
