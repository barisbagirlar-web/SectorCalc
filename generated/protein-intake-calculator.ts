...
function evaluateAllFormulas(input: Protein_intake_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bodyWeightKg * input.baseProtein * input.activityFactor * input.goalFactor; results["dailyProteinGrams"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["dailyProteinGrams"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["dailyProteinGrams"])) * 4; results["proteinCalories"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["proteinCalories"] = Number.NaN; }
  return results;
}
