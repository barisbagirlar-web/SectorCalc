...
function evaluateAllFormulas(input: Future_value_calculatorInput): Record<string, number> {
  ...
  try { const v = input.presentValue+input.periodicPayment*input.periods*input.compoundingFrequency; results["totalContributions"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalContributions"] = 0; }
  try { const v = input.presentValue+input.periodicPayment*input.periods*input.compoundingFrequency; results["totalContributions_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalContributions_aux"] = 0; }
  return results;
}
