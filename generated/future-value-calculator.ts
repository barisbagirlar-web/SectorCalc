...
function evaluateAllFormulas(input: Future_value_calculatorInput): Record<string, number> {
  ...
  try { const v = input.presentValue+input.periodicPayment*input.periods*input.compoundingFrequency; results["totalContributions"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalContributions"] = Number.NaN; }
  try { const v = input.presentValue+input.periodicPayment*input.periods*input.compoundingFrequency; results["totalContributions_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalContributions_aux"] = Number.NaN; }
  return results;
}
