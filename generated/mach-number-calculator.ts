...
function evaluateAllFormulas(input: Mach_number_calculatorInput): Record<string, number> {
  ...
  try { const v = input.airspeed * input.temperature * input.gamma * input.R; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.airspeed * input.temperature * input.gamma * input.R; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
