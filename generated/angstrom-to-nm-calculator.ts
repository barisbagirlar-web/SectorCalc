...
function evaluateAllFormulas(input: Angstrom_to_nm_calculatorInput): Record<string, number> {
  ...
  try { const v = input.angstromValue * 0.1; results["nanoMeters"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["nanoMeters"] = Number.NaN; }
  try { const v = input.angstromValue * 0.1; results["nanoMeters_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["nanoMeters_aux"] = Number.NaN; }
  return results;
}
