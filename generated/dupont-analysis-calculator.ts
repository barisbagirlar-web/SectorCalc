...
function evaluateAllFormulas(input: Dupont_analysis_calculatorInput): Record<string, number> {
  ...
  try { const v = input.netIncome / input.revenue; results["netProfitMargin"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["netProfitMargin"] = Number.NaN; }
  ...
  try { const v = input.totalAssets / input.equity; results["equityMultiplier"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["equityMultiplier"] = Number.NaN; }
  try { const v = input.netIncome / input.equity; results["roe"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["roe"] = Number.NaN; }
  return results;
}
