...
function evaluateAllFormulas(input: Absolute_value_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.value1 * input.value2 * input.value3 * input.value4; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
