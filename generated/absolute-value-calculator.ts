...
function evaluateAllFormulas(input: Absolute_value_calculatorInput): Record<string, number> {
  ...
  try { const v = input.value1 * input.value2 * input.value3 * input.value4; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
