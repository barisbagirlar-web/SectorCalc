...
function evaluateAllFormulas(input: Water_intake_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.numEmployees * input.workDaysPerYear * input.avgWaterUsePerPersonPerDay * input.processWaterIntensity; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.numEmployees * input.workDaysPerYear * input.avgWaterUsePerPersonPerDay * input.processWaterIntensity * (input.annualProductionUnits * (input.leakageFactor / 100) * (input.recyclingRate / 100) * input.seasonalFactor); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.annualProductionUnits * (input.leakageFactor / 100) * (input.recyclingRate / 100) * input.seasonalFactor; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
