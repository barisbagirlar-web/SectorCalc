...
function evaluateAllFormulas(input: Eigenvalue_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a + input.d; results["trace"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["trace"] = 0; }
  try { const v = input.a * input.d - input.b * input.c; results["determinant"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["determinant"] = 0; }
  return results;
}
