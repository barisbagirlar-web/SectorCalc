...
function evaluateAllFormulas(input: Eigenvalue_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a + input.d; results["trace"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["trace"] = Number.NaN; }
  try { const v = input.a * input.d - input.b * input.c; results["determinant"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["determinant"] = Number.NaN; }
  return results;
}
