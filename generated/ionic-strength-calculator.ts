...
function evaluateAllFormulas(input: Ionic_strength_calculatorInput): Record<string, number> {
  ...
  try { const v = 0.5 * (input.concentration1 * input.charge1**2 + input.concentration2 * input.charge2**2 + input.concentration3 * input.charge3**2 + input.concentration4 * input.charge4**2); results["ionicStrength"] = Number.isFinite(v) ? v : 0; } catch { results["ionicStrength"] = 0; }
  try { const v = 0.5 * input.concentration1 * input.charge1**2; results["term1"] = Number.isFinite(v) ? v : 0; } catch { results["term1"] = 0; }
  try { const v = 0.5 * input.concentration2 * input.charge2**2; results["term2"] = Number.isFinite(v) ? v : 0; } catch { results["term2"] = 0; }
  try { const v = 0.5 * input.concentration3 * input.charge3**2; results["term3"] = Number.isFinite(v) ? v : 0; } catch { results["term3"] = 0; }
  try { const v = 0.5 * input.concentration4 * input.charge4**2; results["term4"] = Number.isFinite(v) ? v : 0; } catch { results["term4"] = 0; }
  return results;
}
...
export function calculateIonic_strength_calculator(input: Ionic_strength_calculatorInput): Ionic_strength_calculatorOutput {
  ...
  const totalWasteCost = values["ionicStrength"] ?? 0;
  const breakdown = {
