...
function evaluateAllFormulas(input: Gamma_function_calculatorInput): Record<string, number> {
  ...
  try { const v = input.x * input.tolerance * input.maxIterations * input.useStirling; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.x * input.tolerance * input.maxIterations * input.useStirling; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
