...
function evaluateAllFormulas(input: Cash_flow_gap_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.avg_days_receivable * input.avg_days_payable * input.inventory_days * input.operating_expenses_daily; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.avg_days_receivable * input.avg_days_payable * input.inventory_days * input.operating_expenses_daily * (input.revenue_daily); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.revenue_daily; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
