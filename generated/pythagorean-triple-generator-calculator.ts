...
function evaluateAllFormulas(input: Pythagorean_triple_generator_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.m*input.m - input.n*input.n; results["a"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["a"] = 0; }
  try { const v = 2*input.m*input.n; results["b"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["b"] = 0; }
  try { const v = input.m*input.m + input.n*input.n; results["c"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["c"] = 0; }
  return results;
}
