...
function evaluateAllFormulas(input: Sortino_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.averageReturn * input.riskFreeRate * input.targetReturn * input.downsideDeviation; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.averageReturn * input.riskFreeRate * input.targetReturn * input.downsideDeviation; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
