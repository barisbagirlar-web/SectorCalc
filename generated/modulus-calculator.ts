...
function evaluateAllFormulas(input: Modulus_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.totalQuantity + input.offset) % input.batchSize; results["remainder"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["remainder"] = 0; }
  try { const v = ((input.totalQuantity + input.offset) % input.batchSize) === input.targetRemainder ? 1 : 0; results["match"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["match"] = 0; }
  return results;
}
