...
function evaluateAllFormulas(input: Modulus_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.totalQuantity + input.offset) % input.batchSize; results["remainder"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["remainder"] = Number.NaN; }
  try { const v = ((input.totalQuantity + input.offset) % input.batchSize) === input.targetRemainder ? 1 : 0; results["match"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["match"] = Number.NaN; }
  return results;
}
