...
function evaluateAllFormulas(input: Qr_decomposition_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.a11 * input.a12 * input.a21 * input.a22; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.a11 * input.a12 * input.a21 * input.a22; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
