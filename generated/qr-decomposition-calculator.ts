...
function evaluateAllFormulas(input: Qr_decomposition_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a11 * input.a12 * input.a21 * input.a22; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.a11 * input.a12 * input.a21 * input.a22; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
