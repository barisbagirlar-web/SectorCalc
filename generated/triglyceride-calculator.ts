...
function evaluateAllFormulas(input: Triglyceride_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.fa_moles + input.fa_mw + input.glycerol_moles; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.fa_moles + input.fa_mw + input.glycerol_moles; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
