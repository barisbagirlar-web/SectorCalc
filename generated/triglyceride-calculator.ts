...
function evaluateAllFormulas(input: Triglyceride_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fa_moles * input.fa_mw * input.glycerol_moles * input.glycerol_mw; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.fa_moles * input.fa_mw * input.glycerol_moles * input.glycerol_mw * ((input.yield_percent / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.yield_percent / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
