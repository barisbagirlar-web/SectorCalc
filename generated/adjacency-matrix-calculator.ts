...
function evaluateAllFormulas(input: Adjacency_matrix_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.a11 + input.a22) * (input.a11 + input.a22) - 4 * (input.a11 * input.a22 - input.a12 * input.a21); results["discriminant"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["discriminant"] = 0; }
  return results;
}
...
export function calculateAdjacency_matrix_calculator(input: Adjacency_matrix_calculatorInput): Adjacency_matrix_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["discriminant"]));
  const breakdown = {
