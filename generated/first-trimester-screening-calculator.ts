...
function evaluateAllFormulas(input: First_trimester_screening_calculatorInput): Record<string, number> {
  ...
  try { const v = input.maternalAge * input.nuchalTranslucency * input.crownRumpLength * input.pappA; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.maternalAge * input.nuchalTranslucency * input.crownRumpLength * input.pappA * (input.freeBetaHCG); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.freeBetaHCG; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
