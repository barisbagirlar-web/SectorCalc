...
function evaluateAllFormulas(input: First_trimester_screening_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.maternalAge * input.nuchalTranslucency * input.crownRumpLength * input.pappA; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.maternalAge * input.nuchalTranslucency * input.crownRumpLength * input.pappA * (input.freeBetaHCG); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.freeBetaHCG; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
