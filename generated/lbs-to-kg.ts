...
function evaluateAllFormulas(input: Lbs_to_kgInput): Record<string, number> {
  ...
  try { const v = input.lbs * 0.45359237; results["kg"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["kg"] = 0; }
  try { const v = input.lbs * 0.45359237; results["kg_copy"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["kg_copy"] = 0; }
  return results;
}
