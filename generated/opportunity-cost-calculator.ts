...
function evaluateAllFormulas(input: Opportunity_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = input.investmentAmount * input.currentReturnRate / 100 * input.timePeriod; results["currentReturn"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["currentReturn"] = 0; }
  try { const v = input.investmentAmount * input.alternativeReturnRate / 100 * input.timePeriod; results["alternativeReturn"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["alternativeReturn"] = 0; }
  return results;
}
...
export function calculateOpportunity_cost_calculator(input: Opportunity_cost_calculatorInput): Opportunity_cost_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["alternativeReturn"]));
  const breakdown = {
