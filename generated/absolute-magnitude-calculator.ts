...
function evaluateAllFormulas(input: Absolute_magnitude_calculatorInput): Record<string, number> {
  ...
  try { const v = input.apparentMagnitude * input.distanceParsecs * input.extinction * input.bolometricCorrection; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.apparentMagnitude * input.distanceParsecs * input.extinction * input.bolometricCorrection; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
