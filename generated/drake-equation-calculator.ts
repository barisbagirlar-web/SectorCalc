...
function evaluateAllFormulas(input: Drake_equation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.R * input.fp * input.ne * input.fl * input.fi * input.fc * input.L; results["N"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["N"] = Number.NaN; }
  try { const v = input.R * input.fp * input.ne * input.fl * input.fi * input.fc * input.L; results["N___R___fp___ne___fl___fi___fc___L"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["N___R___fp___ne___fl___fi___fc___L"] = Number.NaN; }
  return results;
}
