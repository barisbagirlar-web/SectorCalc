...
function evaluateAllFormulas(input: Extreme_value_theorem_calculatorInput): Record<string, number> {
  ...
  try { const v = 1 / input.returnPeriod; results["exceedanceProbability"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["exceedanceProbability"] = 0; }
  try { const v = 1 / input.returnPeriod; results["exceedanceProbability_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["exceedanceProbability_aux"] = 0; }
  return results;
}
