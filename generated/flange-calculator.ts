...
function evaluateAllFormulas(input: Flange_calculatorInput): Record<string, number> {
  ...
  try { const v = input.outerDiameter * input.thickness * input.boltCircleDiameter * input.numberOfBolts; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.outerDiameter * input.thickness * input.boltCircleDiameter * input.numberOfBolts * (input.boltHoleDiameter * input.density); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.boltHoleDiameter * input.density; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
