...
function evaluateAllFormulas(input: Law_of_cosines_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sideA * input.sideB * input.angleC * input.decimalPlaces; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.sideA * input.sideB * input.angleC * input.decimalPlaces; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
