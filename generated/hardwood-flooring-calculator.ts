...
function evaluateAllFormulas(input: Hardwood_flooring_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.plankLength * input.plankWidth) / 144; results["plankAreaSqFt"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["plankAreaSqFt"] = Number.NaN; }
  return results;
}
