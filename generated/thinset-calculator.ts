...
function evaluateAllFormulas(input: Thinset_calculatorInput): Record<string, number> {
  ...
  try { const v = input.notchSize * 0.4 + 0.8; results["coverageRate"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["coverageRate"] = 0; }
  try { const v = input.area * (input.notchSize * 0.4 + 0.8) * (1 + input.wasteFactor/100); results["totalKgs"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalKgs"] = 0; }
  return results;
}
...
export function calculateThinset_calculator(input: Thinset_calculatorInput): Thinset_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["totalKgs"]));
  const breakdown = {
