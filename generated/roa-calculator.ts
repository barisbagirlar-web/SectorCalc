...
function evaluateAllFormulas(input: Roa_calculatorInput): Record<string, number> {
  ...
  try { const v = input.netIncome * input.beginningAssets * input.endingAssets * input.averageAssetsOverride; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.netIncome * input.beginningAssets * input.endingAssets * input.averageAssetsOverride; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
