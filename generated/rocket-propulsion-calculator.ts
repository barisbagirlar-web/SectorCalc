...
function evaluateAllFormulas(input: Rocket_propulsion_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.specificImpulse * input.gravitationalAcceleration; results["exhaustVelocity"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["exhaustVelocity"] = 0; }
  try { const v = input.massFlowRate * (asFormulaNumber(results["exhaustVelocity"])); results["thrust"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["thrust"] = 0; }
  try { const v = (asFormulaNumber(results["propellantMass"])) / input.massFlowRate; results["burnTime"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["burnTime"] = 0; }
  return results;
}
