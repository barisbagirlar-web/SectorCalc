...
function evaluateAllFormulas(input: Seed_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.seed_weight * input.seed_cost_per_unit; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.seed_weight * input.seed_cost_per_unit * (1 + (input.germination_rate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.seed_weight * input.seed_cost_per_unit * (1 + (input.germination_rate / 100)) * (input.target_population); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.target_population; results["factor_target_population"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["factor_target_population"] = 0; }
  return results;
}
