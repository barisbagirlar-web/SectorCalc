...
function evaluateAllFormulas(input: Pallet_rack_optimizer_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.rack_height_mm * input.pallet_depth_mm * input.pallet_width_mm * input.pallet_height_mm; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.rack_height_mm * input.pallet_depth_mm * input.pallet_width_mm * input.pallet_height_mm * (input.beam_thickness_mm * input.clearance_vertical_mm * input.aisle_width_mm * input.rack_length_mm); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.beam_thickness_mm * input.clearance_vertical_mm * input.aisle_width_mm * input.rack_length_mm; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
