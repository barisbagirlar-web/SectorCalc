...
function evaluateAllFormulas(input: Portion_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = input.number_of_portions * input.recipe_cost; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.number_of_portions * input.recipe_cost * (1 + (input.desired_margin_percent / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.number_of_portions * input.recipe_cost * (1 + (input.desired_margin_percent / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
