...
function evaluateAllFormulas(input: Bmi_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.weight_kg * input.height_cm * input.age_years * input.waist_circumference_cm; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.weight_kg * input.height_cm * input.age_years * input.waist_circumference_cm; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
