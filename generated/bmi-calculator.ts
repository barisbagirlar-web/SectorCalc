...
function evaluateAllFormulas(input: Bmi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight_kg * input.height_cm * input.age_years * input.waist_circumference_cm; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.weight_kg * input.height_cm * input.age_years * input.waist_circumference_cm; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
