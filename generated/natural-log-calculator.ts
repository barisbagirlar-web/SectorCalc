...
function evaluateAllFormulas(input: Natural_log_calculatorInput): Record<string, number> {
  ...
  try { const v = input.inputValue * input.precision * input.scaleFactor * input.offset; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.inputValue * input.precision * input.scaleFactor * input.offset * (input.expDisplay); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.expDisplay; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
