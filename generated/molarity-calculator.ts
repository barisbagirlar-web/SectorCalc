...
function evaluateAllFormulas(input: Molarity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mass / (input.molarMass * input.volume); results["molarity"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["molarity"] = 0; }
...
export function calculateMolarity_calculator(input: Molarity_calculatorInput): Molarity_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["molarity"]));
  const breakdown = {
