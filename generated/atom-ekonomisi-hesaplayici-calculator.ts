...
function evaluateAllFormulas(input: Atom_ekonomisi_hesaplayici_calculatorInput): Record<string, number> {
  ...
  try { const v = input.reactantMW1 + input.reactantMW2 + input.reactantMW3 + input.reactantMW4 + input.reactantMW5; results["toplamReaktanAgirlik"] = Number.isFinite(v) ? v : 0; } catch { results["toplamReaktanAgirlik"] = 0; }
  try { const v = input.productMW; results["urunAgirlik"] = Number.isFinite(v) ? v : 0; } catch { results["urunAgirlik"] = 0; }
  try { const v = input.productMW / (input.reactantMW1 + input.reactantMW2 + input.reactantMW3 + input.reactantMW4 + input.reactantMW5) * 100; results["atomEkonomisi"] = Number.isFinite(v) ? v : 0; } catch { results["atomEkonomisi"] = 0; }
  return results;
}
...
export function calculateAtom_ekonomisi_hesaplayici_calculator(input: Atom_ekonomisi_hesaplayici_calculatorInput): Atom_ekonomisi_hesaplayici_calculatorOutput {
  ...
  const totalWasteCost = values["atomEkonomisi"] ?? 0;
  const breakdown = {
