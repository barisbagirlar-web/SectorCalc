...
function evaluateAllFormulas(input: Intermediate_value_theorem_checker_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a * input.b * input.f_a * input.f_b; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.a * input.b * input.f_a * input.f_b * (input.k); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.k; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
