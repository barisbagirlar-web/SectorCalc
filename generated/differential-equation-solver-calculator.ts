...
function evaluateAllFormulas(input: Differential_equation_solver_calculatorInput): Record<string, number> {
  ...
  try { const v = input.y0 * input.a * input.b * input.t0; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.y0 * input.a * input.b * input.t0 * (input.t); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.t; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
