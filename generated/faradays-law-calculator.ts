...
function evaluateAllFormulas(input: Faradays_law_calculatorInput): Record<string, number> {
  ...
  try { const v = -input.number_of_turns * (input.magnetic_flux_change / input.time_interval); results["induced_emf"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["induced_emf"] = 0; }
  try { const v = -input.number_of_turns * (input.magnetic_flux_change / input.time_interval); results["induced_emf_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["induced_emf_aux"] = 0; }
  return results;
}
