...
function evaluateAllFormulas(input: Hydro_power_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.turbineEfficiency * input.generatorEfficiency) / 100; results["overallEfficiencyPercent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["overallEfficiencyPercent"] = Number.NaN; }
  try { const v = ((toNumericFormulaValue(results["overallEfficiencyPercent"])) / 100) * input.waterDensity * input.gravity * input.flowRate * input.head; results["powerWatts"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["powerWatts"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["powerWatts"])) / 1000; results["powerKW"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["powerKW"] = Number.NaN; }
  return results;
}
