...
function evaluateAllFormulas(input: Sunrise_sunset_calculatorInput): Record<string, number> {
  ...
  try { const v = input.latitude * input.longitude * input.dayOfYear * input.timezoneOffset; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.latitude * input.longitude * input.dayOfYear * input.timezoneOffset; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
