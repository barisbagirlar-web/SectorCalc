...
function evaluateAllFormulas(input: Spring_force_calculatorInput): Record<string, number> {
  ...
  try { const v = input.springConstant * input.displacement * input.wireDiameter * input.coilDiameter; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.springConstant * input.displacement * input.wireDiameter * input.coilDiameter * (input.activeCoils * input.shearModulus); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.activeCoils * input.shearModulus; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
