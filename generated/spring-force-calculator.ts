...
function evaluateAllFormulas(input: Spring_force_calculatorInput): Record<string, number> {
  ...
  try { const v = input.displacement; results["displacement"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["displacement"] = 0; }
  try { const v = input.displacement; results["displacement_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["displacement_aux"] = 0; }
  return results;
}
