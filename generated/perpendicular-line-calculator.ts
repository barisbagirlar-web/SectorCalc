...
function evaluateAllFormulas(input: Perpendicular_line_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.x1 * input.y1 * input.x2 * input.y2 * (input.x3 * input.y3); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.x3 * input.y3; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
