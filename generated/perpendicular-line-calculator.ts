...
function evaluateAllFormulas(input: Perpendicular_line_calculatorInput): Record<string, number> {
  ...
  try { const v = input.x1 * input.y1 * input.x2 * input.y2 * (input.x3 * input.y3); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.x3 * input.y3; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
