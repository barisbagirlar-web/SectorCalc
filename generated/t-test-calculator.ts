...
function evaluateAllFormulas(input: T_test_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sampleMean - input.hypothesizedMean; results["meanDifference_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["meanDifference_aux"] = Number.NaN; }
  return results;
}
