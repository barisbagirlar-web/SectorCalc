...
function evaluateAllFormulas(input: Compression_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bore * input.stroke * input.combustionChamberVolume * input.gasketThickness; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.bore * input.stroke * input.combustionChamberVolume * input.gasketThickness * (input.gasketBore * input.deckClearance * input.pistonDishVolume); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.gasketBore * input.deckClearance * input.pistonDishVolume; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
