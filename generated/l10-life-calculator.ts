...
function evaluateAllFormulas(input: L10_life_calculatorInput): Record<string, number> {
  ...
  try { const v = input.dynamicLoadRating * input.equivalentLoad * input.exponent * input.reliabilityFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.dynamicLoadRating * input.equivalentLoad * input.exponent * input.reliabilityFactor * (input.speed); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.speed; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
