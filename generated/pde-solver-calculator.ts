...
function evaluateAllFormulas(input: Pde_solver_calculatorInput): Record<string, number> {
  ...
  try { const v = input.alpha * input.length * input.x * input.time; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.alpha * input.length * input.x * input.time * (input.A1 * input.A2 * input.A3); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.A1 * input.A2 * input.A3; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
