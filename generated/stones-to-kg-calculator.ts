...
function evaluateAllFormulas(input: Stones_to_kg_calculatorInput): Record<string, number> {
  ...
  try { const v = input.stones + input.pounds/14; results["total_stones"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["total_stones"] = Number.NaN; }
  try { const v = (input.stones + input.pounds/14) * input.conversionFactor; results["raw_kg"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["raw_kg"] = Number.NaN; }
  return results;
}
