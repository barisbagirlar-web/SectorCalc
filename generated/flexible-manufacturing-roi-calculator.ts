...
function evaluateAllFormulas(input: Flexible_manufacturing_roi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annual_production_volume * input.avg_batch_size * input.current_setup_time * input.target_setup_time; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.annual_production_volume * input.avg_batch_size * input.current_setup_time * input.target_setup_time * (input.hourly_operating_cost * input.changeover_cost_per_hour * (input.inventory_holding_cost_rate / 100) * input.avg_inventory_value); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.hourly_operating_cost * input.changeover_cost_per_hour * (input.inventory_holding_cost_rate / 100) * input.avg_inventory_value; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
