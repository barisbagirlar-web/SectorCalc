...
function evaluateAllFormulas(input: Adiabatic_lapse_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.finalAltitude - input.initialAltitude; results["heightDifference"] = Number.isFinite(v) ? v : 0; } catch { results["heightDifference"] = 0; }
  try { const v = -input.gravityAcceleration / input.specificHeatCapacity; results["lapseRate"] = Number.isFinite(v) ? v : 0; } catch { results["lapseRate"] = 0; }
  try { const v = (results["lapseRate"] ?? 0) * (results["heightDifference"] ?? 0); results["temperatureChange"] = Number.isFinite(v) ? v : 0; } catch { results["temperatureChange"] = 0; }
  try { const v = input.initialTemperature + (results["temperatureChange"] ?? 0); results["finalTemperature"] = Number.isFinite(v) ? v : 0; } catch { results["finalTemperature"] = 0; }
  return results;
}
...
export function calculateAdiabatic_lapse_rate_calculator(input: Adiabatic_lapse_rate_calculatorInput): Adiabatic_lapse_rate_calculatorOutput {
  ...
  const totalWasteCost = values["finalTemperature"] ?? 0;
  const breakdown = {
