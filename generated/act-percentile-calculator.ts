...
function evaluateAllFormulas(input: Act_percentile_calculatorInput): Record<string, number> {
  ...
  try { const v = input.english * input.math * input.reading * input.science; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.english * input.math * input.reading * input.science * (input.distributionMean * input.distributionStd); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.distributionMean * input.distributionStd; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
