...
function evaluateAllFormulas(input: Signage_safe_price_tool_calculatorInput): Record<string, number> {
  ...
  try { const v = input.labor_hours; results["annual_exposure_hours"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annual_exposure_hours"] = Number.NaN; }
  try { const v = input.quantity * (input.labor_rate / 100) * input.labor_hours * input.material_cost; results["direct_labor_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["direct_labor_cost"] = Number.NaN; }
  try { const v = input.quantity * (input.labor_rate / 100) * input.labor_hours * input.material_cost; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
