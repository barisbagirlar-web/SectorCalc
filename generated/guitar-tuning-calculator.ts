...
function evaluateAllFormulas(input: Guitar_tuning_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fretCount * input.scaleLength * input.stringTension * input.linearDensity; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.fretCount * input.scaleLength * input.stringTension * input.linearDensity * (input.openNoteFrequency * input.fretNumber); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.openNoteFrequency * input.fretNumber; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
