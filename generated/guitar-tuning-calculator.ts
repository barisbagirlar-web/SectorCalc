...
function evaluateAllFormulas(input: Guitar_tuning_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.fretCount * input.scaleLength * input.stringTension * input.linearDensity; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.fretCount * input.scaleLength * input.stringTension * input.linearDensity * (input.openNoteFrequency * input.fretNumber); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.openNoteFrequency * input.fretNumber; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
