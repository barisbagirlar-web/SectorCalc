...
function evaluateAllFormulas(input: Stefan_boltzmann_calculatorInput): Record<string, number> {
  ...
  try { const v = input.emissivity * input.area * input.temperature * input.stefanConstant; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.emissivity * input.area * input.temperature * input.stefanConstant; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
