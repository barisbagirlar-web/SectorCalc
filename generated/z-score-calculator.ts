...
function evaluateAllFormulas(input: Z_score_calculatorInput): Record<string, number> {
  ...
  try { const v = input.data_points * input.population_mean * input.population_stddev; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.data_points * input.population_mean * input.population_stddev; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
