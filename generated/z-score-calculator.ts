...
function evaluateAllFormulas(input: Z_score_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.data_points + input.population_mean + input.population_stddev; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
