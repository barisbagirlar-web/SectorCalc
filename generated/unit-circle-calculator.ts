...
function evaluateAllFormulas(input: Unit_circle_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.initialAngle * Math.PI / 180; results["angleRad"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["angleRad"] = 0; }
  try { const v = (asFormulaNumber(results["angleRad"])) + input.angularVelocity * input.time; results["totalAngleRad"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalAngleRad"] = 0; }
  return results;
}
