...
function evaluateAllFormulas(input: Act_score_calculatorInput): Record<string, number> {
  ...
  try { const v = input.englishScore * input.mathScore * input.readingScore * input.scienceScore; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.englishScore * input.mathScore * input.readingScore * input.scienceScore; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
