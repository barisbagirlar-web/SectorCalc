...
function evaluateAllFormulas(input: Kgf_per_cm2_to_psiInput): Record<string, number> {
  ...
  try { const v = input.pressure_kgfcm2 * 14.2233433071; results["primary"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["primary"] = Number.NaN; }
  try { const v = input.pressure_kgfcm2; results["breakdown"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["breakdown"] = Number.NaN; }
  return results;
}
