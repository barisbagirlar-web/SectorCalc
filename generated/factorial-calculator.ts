...
function evaluateAllFormulas(input: Factorial_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.n * input.reserved1 * input.reserved2 * input.reserved3; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.n * input.reserved1 * input.reserved2 * input.reserved3; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
