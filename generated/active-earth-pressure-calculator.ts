...
function evaluateAllFormulas(input: Active_earth_pressure_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.height * input.unitWeight * input.frictionAngle * input.cohesion; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.height * input.unitWeight * input.frictionAngle * input.cohesion * (input.surcharge); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
