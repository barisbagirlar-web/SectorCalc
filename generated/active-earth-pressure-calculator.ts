...
function evaluateAllFormulas(input: Active_earth_pressure_calculatorInput): Record<string, number> {
  ...
  try { const v = input.height * input.unitWeight * input.frictionAngle * input.cohesion; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.height * input.unitWeight * input.frictionAngle * input.cohesion * (input.surcharge); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
