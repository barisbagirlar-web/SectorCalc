...
function evaluateAllFormulas(input: Golf_handicap_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.grossScore - input.courseRating) * 113 / input.slopeRating; results["differential"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["differential"] = 0; }
...
export function calculateGolf_handicap_calculator(input: Golf_handicap_calculatorInput): Golf_handicap_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["differential"]));
  const breakdown = {
