...
function evaluateAllFormulas(input: Titration_calculatorInput): Record<string, number> {
  ...
  try { const v = input.titrantConc * (input.titrantVol / 1000); results["titrantMoles"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["titrantMoles"] = 0; }
  try { const v = (asFormulaNumber(results["titrantMoles"])) * (input.stoichCoeffAnalyte / input.stoichCoeffTitrant); results["analyteMoles"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["analyteMoles"] = 0; }
  try { const v = (asFormulaNumber(results["analyteMoles"])) / (input.analyteVol / 1000); results["analyteConcentration"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["analyteConcentration"] = 0; }
  return results;
}
...
export function calculateTitration_calculator(input: Titration_calculatorInput): Titration_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["analyteConcentration"]));
  const breakdown = {
