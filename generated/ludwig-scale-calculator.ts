...
function evaluateAllFormulas(input: Ludwig_scale_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cuttingSpeed / (input.feedRate * input.depthOfCut); results["cuttingComponent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["cuttingComponent"] = Number.NaN; }
  try { const v = input.cuttingSpeed / (input.feedRate * input.depthOfCut); results["cuttingComponent_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["cuttingComponent_aux"] = Number.NaN; }
  return results;
}
