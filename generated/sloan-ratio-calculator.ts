...
function evaluateAllFormulas(input: Sloan_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.netIncome - input.cashFlowOperations - input.cashFlowInvesting; results["netAccruals"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["netAccruals"] = 0; }
  try { const v = ((asFormulaNumber(results["netAccruals"])) / input.totalAssets) * 100; results["sloanRatio"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["sloanRatio"] = 0; }
  return results;
}
...
export function calculateSloan_ratio_calculator(input: Sloan_ratio_calculatorInput): Sloan_ratio_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["sloanRatio"]));
  const breakdown = {
