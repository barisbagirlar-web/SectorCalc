...
function evaluateAllFormulas(input: Cbam_exposure_check_calculatorInput): Record<string, number> {
  ...
  try { const v = input.carbon_intensity * input.cbam_carbon_price; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.carbon_intensity * input.cbam_carbon_price * (1 + (input.free_allocation_percentage / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.carbon_intensity * input.cbam_carbon_price * (1 + (input.free_allocation_percentage / 100)) * (input.annual_production_volume); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.annual_production_volume; results["factor_annual_production_volume"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_annual_production_volume"] = Number.NaN; }
  return results;
}
...
export function calculateCbam_exposure_check_calculator(input: Cbam_exposure_check_calculatorInput): Cbam_exposure_check_calculatorOutput {
  ...
  const breakdown = {
    ...
    factor_annual_production_volume: toNumericFormulaValue(values["factor_annual_production_volume"])
  };
...
export interface Cbam_exposure_check_calculatorOutput {
  ...
  unit: string;
  breakdown: { base_cost: number; adjusted_cost: number; factor_annual_production_volume: number };
...
export const Cbam_exposure_check_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
  breakdownKeys: ["base_cost","adjusted_cost","factor_annual_production_volume"],
} as const;
