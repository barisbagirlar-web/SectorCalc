...
function evaluateAllFormulas(input: Body_surface_area_pediatric_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * input.height * input.formula * input.adjustment; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.weight * input.height * input.formula * input.adjustment; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
