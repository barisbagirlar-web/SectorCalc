...
function evaluateAllFormulas(input: Sinking_fund_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualInterestRate / 100 / input.compoundsPerYear; results["periodicRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["periodicRate"] = Number.NaN; }
  try { const v = input.years * input.compoundsPerYear; results["totalPeriods"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalPeriods"] = Number.NaN; }
  return results;
}
