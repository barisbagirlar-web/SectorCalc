...
function evaluateAllFormulas(input: Christmas_club_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualInterestRate / 100 / input.compoundingFrequency; results["periodicRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["periodicRate"] = Number.NaN; }
  try { const v = input.weeklyDeposit * input.numberOfWeeks; results["totalDeposits"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalDeposits"] = Number.NaN; }
  return results;
}
