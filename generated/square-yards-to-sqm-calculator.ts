...
function evaluateAllFormulas(input: Square_yards_to_sqm_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.lengthYards * input.widthYards) * input.conversionFactor; results["areaSqm"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["areaSqm"] = 0; }
  try { const v = input.lengthYards * input.widthYards; results["areaSqYards"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["areaSqYards"] = 0; }
