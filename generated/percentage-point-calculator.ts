...
function evaluateAllFormulas(input: Percentage_point_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.percentageA / 100) * (input.percentageB / 100) * input.absolute * input.decimalPlaces; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = (input.percentageA / 100) * (input.percentageB / 100) * input.absolute * input.decimalPlaces; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
