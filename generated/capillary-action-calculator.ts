...
function evaluateAllFormulas(input: Capillary_action_calculatorInput): Record<string, number> {
  ...
  try { const v = input.surfaceTension * input.contactAngle * input.density * input.radius; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.surfaceTension * input.contactAngle * input.density * input.radius * (input.gravity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
