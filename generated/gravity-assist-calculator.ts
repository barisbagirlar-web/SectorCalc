...
function evaluateAllFormulas(input: Gravity_assist_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.v_planet * input.v_inf * input.r_p * input.mu; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.v_planet * input.v_inf * input.r_p * input.mu; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
