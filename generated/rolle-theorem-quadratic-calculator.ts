...
function evaluateAllFormulas(input: Rolle_theorem_quadratic_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.a*input.x1*input.x1 + input.bCoef*input.x1 + input.cConst; results["f_x1"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["f_x1"] = 0; }
  try { const v = input.a*input.x2*input.x2 + input.bCoef*input.x2 + input.cConst; results["f_x2"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["f_x2"] = 0; }
  return results;
}
