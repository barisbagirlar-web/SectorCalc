...
function evaluateAllFormulas(input: Permanent_teeth_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.wisdomOverride == 1) ? 32 : ( (input.age < 6) ? 0 : ((input.age <= 12) ? (input.age - 6) / 6 * 28 : ((input.age <= 21) ? 28 + (input.age - 12) / 9 * 4 : 32) ) ); results["expectedTeeth"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["expectedTeeth"] = Number.NaN; }
  try { const v = (input.wisdomOverride == 1) ? 32 : ( (input.age < 6) ? 0 : ((input.age <= 12) ? (input.age - 6) / 6 * 28 : ((input.age <= 21) ? 28 + (input.age - 12) / 9 * 4 : 32) ) ); results["expectedTeeth_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["expectedTeeth_aux"] = Number.NaN; }
  return results;
}
