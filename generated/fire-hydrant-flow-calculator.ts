...
function evaluateAllFormulas(input: Fire_hydrant_flow_calculatorInput): Record<string, number> {
  ...
  try { results["flow_velocity"] = 0.4085 * (results["available_flow"] ?? 0) / (input.pipe_diameter ^ 2); } catch { results["flow_velocity"] = 0; }
  try { results["pressure_loss_percent"] = (((results["friction_loss"] ?? 0) + Math.abs((results["elevation_pressure"] ?? 0))) / input.static_pressure) * 100; } catch { results["pressure_loss_percent"] = 0; }
  return results;
}
