...
function evaluateAllFormulas(input: Fire_hydrant_flow_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.static_pressure + input.residual_pressure + input.flow_rate_test; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.static_pressure + input.residual_pressure + input.flow_rate_test; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
