...
function evaluateAllFormulas(input: Fortnights_to_days_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fortnights * input.daysPerFortnight; results["rawDays"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawDays"] = Number.NaN; }
  try { const v = input.fortnights * input.daysPerFortnight * input.batchSize; results["batchTotal"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["batchTotal"] = Number.NaN; }
  return results;
}
