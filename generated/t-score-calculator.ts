...
function evaluateAllFormulas(input: T_score_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.sampleSize - 1; results["degreesOfFreedom_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["degreesOfFreedom_aux"] = 0; }
  return results;
}
