...
function evaluateAllFormulas(input: Weld_volume_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = input.plate_thickness * input.root_face * input.root_gap * input.groove_angle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.plate_thickness * input.root_face * input.root_gap * input.groove_angle * (input.weld_length * input.leg_length); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.weld_length * input.leg_length; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
