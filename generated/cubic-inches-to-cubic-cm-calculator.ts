...
function evaluateAllFormulas(input: Cubic_inches_to_cubic_cm_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cubicInches * input.conversionFactor; results["exactCubicCm"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["exactCubicCm"] = Number.NaN; }
  try { const v = input.decimalPlaces; results["appliedPrecision"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["appliedPrecision"] = Number.NaN; }
  return results;
}
