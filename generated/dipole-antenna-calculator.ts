...
function evaluateAllFormulas(input: Dipole_antenna_calculatorInput): Record<string, number> {
  ...
  try { const v = (300 / input.frequency); results["wavelength_m"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["wavelength_m"] = Number.NaN; }
  try { const v = (300 / input.frequency) * 0.5 * input.velocityFactor; results["dipoleLength_m"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["dipoleLength_m"] = Number.NaN; }
  try { const v = (300 / input.frequency) * 0.5 * input.velocityFactor * 3.28084; results["dipoleLength_ft"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["dipoleLength_ft"] = Number.NaN; }
  return results;
}
