...
function evaluateAllFormulas(input: Orbital_velocity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.centralBodyMass * input.centralBodyRadius * input.altitude * input.gravitationalConstant; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.centralBodyMass * input.centralBodyRadius * input.altitude * input.gravitationalConstant; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
