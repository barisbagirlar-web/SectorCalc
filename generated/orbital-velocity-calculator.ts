...
function evaluateAllFormulas(input: Orbital_velocity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.centralBodyMass * input.centralBodyRadius * input.altitude * input.gravitationalConstant; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.centralBodyMass * input.centralBodyRadius * input.altitude * input.gravitationalConstant; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
