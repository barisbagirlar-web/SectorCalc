...
function evaluateAllFormulas(input: Fertilizer_dosage_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.target_yield * input.soil_nitrogen * input.soil_phosphorus * input.soil_potassium; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.target_yield * input.soil_nitrogen * input.soil_phosphorus * input.soil_potassium * ((input.application_efficiency / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.application_efficiency / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
