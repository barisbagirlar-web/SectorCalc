...
function evaluateAllFormulas(input: Fertilizer_dosage_calculatorInput): Record<string, number> {
  ...
  try { const v = input.target_yield * input.soil_nitrogen * input.soil_phosphorus * input.soil_potassium; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.target_yield * input.soil_nitrogen * input.soil_phosphorus * input.soil_potassium * ((input.application_efficiency / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.application_efficiency / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
...
export const Fertilizer_dosage_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "%",
