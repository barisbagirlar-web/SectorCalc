...
function evaluateAllFormulas(input: Clausius_clapeyron_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.T1_C * input.P1_kPa * input.T2_C * input.P2_kPa; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.T1_C * input.P1_kPa * input.T2_C * input.P2_kPa * (input.R_JmolK * input.M_gmol); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.R_JmolK * input.M_gmol; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
