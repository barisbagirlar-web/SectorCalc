...
function evaluateAllFormulas(input: Cpa_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.marketingCost + input.salesCost + input.overheadCost + input.otherCost; results["totalCost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalCost"] = 0; }
  try { const v = input.acquiredCustomers === 0 ? 0 : ((asFormulaNumber(results["totalCost"])) / input.acquiredCustomers); results["cpa"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["cpa"] = 0; }
  return results;
}
