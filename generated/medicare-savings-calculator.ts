...
function evaluateAllFormulas(input: Medicare_savings_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.currentAge * input.currentAnnualMedicalCost; results["base_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.currentAge * input.currentAnnualMedicalCost * (1 + (input.inflationRate / 100)); results["adjusted_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.currentAge * input.currentAnnualMedicalCost * (1 + (input.inflationRate / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
