...
function evaluateAllFormulas(input: Crop_factor_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.fullFrameWidth * input.fullFrameHeight) / (input.sensorWidth * input.sensorHeight); results["areaRatio"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["areaRatio"] = 0; }
  try { const v = (input.fullFrameWidth * input.fullFrameHeight) / (input.sensorWidth * input.sensorHeight); results["areaRatio_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["areaRatio_aux"] = 0; }
  return results;
}
