...
function evaluateAllFormulas(input: Fetal_weight_calculatorInput): Record<string, number> {
  ...
  try { const v = 1.3596 - 0.00386 * input.ac * input.fl + 0.0064 * input.hc + 0.00061 * input.bpd * input.ac + 0.0424 * input.ac + 0.174 * input.fl; results["log10EFW"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["log10EFW"] = Number.NaN; }
  try { const v = 1.3596 - 0.00386 * input.ac * input.fl + 0.0064 * input.hc + 0.00061 * input.bpd * input.ac + 0.0424 * input.ac + 0.174 * input.fl; results["log10EFW_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["log10EFW_aux"] = Number.NaN; }
  return results;
}
