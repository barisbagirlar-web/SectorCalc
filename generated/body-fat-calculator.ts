...
function evaluateAllFormulas(input: Body_fat_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.age * input.height * input.weight * input.neckCircumference; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.age * input.height * input.weight * input.neckCircumference * (input.waistCircumference * input.hipCircumference); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.waistCircumference * input.hipCircumference; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
