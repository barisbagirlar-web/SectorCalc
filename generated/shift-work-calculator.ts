...
function evaluateAllFormulas(input: Shift_work_calculatorInput): Record<string, number> {
  ...
  try { const v = input.hoursWorked * input.hourlyRate * input.shiftMultiplier; results["basePay"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["basePay"] = Number.NaN; }
  ...
  try { const v = (toNumericFormulaValue(results["basePay"])) + (toNumericFormulaValue(results["overtimePay"])); results["totalPay"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalPay"] = Number.NaN; }
  return results;
}
