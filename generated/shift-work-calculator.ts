...
function evaluateAllFormulas(input: Shift_work_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.hoursWorked * input.hourlyRate * input.shiftMultiplier; results["basePay"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["basePay"] = 0; }
  ...
  try { const v = (asFormulaNumber(results["basePay"])) + (asFormulaNumber(results["overtimePay"])); results["totalPay"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalPay"] = 0; }
  return results;
}
...
export function calculateShift_work_calculator(input: Shift_work_calculatorInput): Shift_work_calculatorOutput {
  ...
  const totalWasteCost = toNumericFormulaValue(values["totalPay"]);
  const breakdown = {
