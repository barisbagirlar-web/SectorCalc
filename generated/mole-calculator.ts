...
function evaluateAllFormulas(input: Mole_calculatorInput): Record<string, number | string> {
  ...
  try { const v = (asFormulaNumber(results["moles"])) * input.avogadro; results["particles"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["particles"] = 0; }
  try { const v = (asFormulaNumber(results["moles"])) * input.molarVolume; results["volumeSTP"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["volumeSTP"] = 0; }
  return results;
}
