...
function evaluateAllFormulas(input: Orbital_mechanics_calculatorInput): Record<string, number> {
  ...
  try { const v = input.altitude * input.planetRadius * input.eccentricity * input.mu; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.altitude * input.planetRadius * input.eccentricity * input.mu; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
