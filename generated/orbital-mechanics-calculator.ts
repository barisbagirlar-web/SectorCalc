...
function evaluateAllFormulas(input: Orbital_mechanics_calculatorInput): Record<string, number> {
  ...
  try { const v = input.altitude * input.planetRadius * input.eccentricity * input.mu; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.altitude * input.planetRadius * input.eccentricity * input.mu; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
