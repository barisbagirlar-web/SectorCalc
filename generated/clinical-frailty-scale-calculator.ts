...
function evaluateAllFormulas(input: Clinical_frailty_scale_calculatorInput): Record<string, number> {
  ...
  try { const v = input.comorbidities * 0.3; results["comorbidityEffect"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["comorbidityEffect"] = 0; }
  try { const v = input.comorbidities * 0.3; results["comorbidityEffect_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["comorbidityEffect_aux"] = 0; }
  return results;
}
...
export function calculateClinical_frailty_scale_calculator(input: Clinical_frailty_scale_calculatorInput): Clinical_frailty_scale_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["comorbidityEffect_aux"]));
  const breakdown = {
