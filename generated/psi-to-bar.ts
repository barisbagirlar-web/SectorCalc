...
function evaluateAllFormulas(input: Psi_to_barInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.psi_value / 14.5038; results["bar"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["bar"] = 0; }
  try { const v = input.psi_value / 14.5038; results["bar_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["bar_copy"] = 0; }
  return results;
}
