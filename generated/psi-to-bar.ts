...
function evaluateAllFormulas(input: Psi_to_barInput): Record<string, number> {
  ...
  try { const v = input.psi_value / 14.5038; results["bar"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["bar"] = Number.NaN; }
  try { const v = input.psi_value / 14.5038; results["bar_copy"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["bar_copy"] = Number.NaN; }
  return results;
}
