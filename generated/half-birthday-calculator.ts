...
function evaluateAllFormulas(input: Half_birthday_calculatorInput): Record<string, number> {
  ...
  try { const v = input.birthMonth + input.monthOffset - 1; results["totalMonths"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalMonths"] = 0; }
  try { const v = input.birthMonth + input.monthOffset - 1; results["totalMonths_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalMonths_aux"] = 0; }
  return results;
}
...
export function calculateHalf_birthday_calculator(input: Half_birthday_calculatorInput): Half_birthday_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["totalMonths_aux"]));
  const breakdown = {
