...
function evaluateAllFormulas(input: Series_i_savings_bond_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fixedRate/100 + 2*(input.inflationRate/100) + (input.fixedRate/100)*(input.inflationRate/100); results["compositeRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["compositeRate"] = Number.NaN; }
  try { const v = input.fixedRate/100 + 2*(input.inflationRate/100) + (input.fixedRate/100)*(input.inflationRate/100); results["compositeRate_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["compositeRate_aux"] = Number.NaN; }
  return results;
}
