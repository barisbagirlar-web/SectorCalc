...
function evaluateAllFormulas(input: Factory_layout_distance_optimizer_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.num_departments + input.flow_intensity_matrix + input.distance_matrix; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.num_departments + input.flow_intensity_matrix + input.distance_matrix; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
