...
function evaluateAllFormulas(input: Factory_layout_distance_optimizer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.num_departments * input.flow_intensity_matrix * input.distance_matrix * (input.path_efficiency_factor / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.num_departments * input.flow_intensity_matrix * input.distance_matrix * (input.path_efficiency_factor / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
