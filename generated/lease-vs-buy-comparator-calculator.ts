...
function evaluateAllFormulas(input: Lease_vs_buy_comparator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.leaseMaintenanceIncluded * input.equipmentCost; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.leaseMaintenanceIncluded * input.equipmentCost * (1 + (input.discountRate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.leaseMaintenanceIncluded * input.equipmentCost * (1 + (input.discountRate / 100)) * (input.leaseTermMonths); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.leaseTermMonths; results["factor_leaseTermMonths"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_leaseTermMonths"] = Number.NaN; }
  return results;
}
