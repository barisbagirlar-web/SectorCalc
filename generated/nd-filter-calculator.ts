...
function evaluateAllFormulas(input: Nd_filter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numerator / input.denominator; results["baseShutter"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["baseShutter"] = 0; }
  try { const v = input.ndStops > 0 ? input.ndStops : (input.od > 0 ? input.od / 0.3 : 0); results["effectiveStops"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectiveStops"] = 0; }
  return results;
}
...
export function calculateNd_filter_calculator(input: Nd_filter_calculatorInput): Nd_filter_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["effectiveStops"]));
  const breakdown = {
