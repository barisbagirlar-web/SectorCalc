...
function evaluateAllFormulas(input: Pool_volume_calculatorInput): Record<string, number> {
  ...
  try { const v = input.length * input.width * (input.shallowDepth + input.deepDepth) / 2; results["volume"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["volume"] = Number.NaN; }
  return results;
}
