...
function evaluateAllFormulas(input: Thrust_calculatorInput): Record<string, number> {
  ...
  try { const v = input.massFlowRate * input.exhaustVelocity; results["momentumThrust"] = Number.isFinite(v) ? v : 0; } catch { results["momentumThrust"] = 0; }
  try { const v = (input.exitPressure - input.ambientPressure) * input.exitArea; results["pressureThrust"] = Number.isFinite(v) ? v : 0; } catch { results["pressureThrust"] = 0; }
  try { const v = input.massFlowRate * input.exhaustVelocity + (input.exitPressure - input.ambientPressure) * input.exitArea; results["totalThrust"] = Number.isFinite(v) ? v : 0; } catch { results["totalThrust"] = 0; }
  return results;
}
...
export function calculateThrust_calculator(input: Thrust_calculatorInput): Thrust_calculatorOutput {
  ...
  const totalWasteCost = values["totalThrust"] ?? 0;
  const breakdown = {
