...
function evaluateAllFormulas(input: Effective_dose_calculatorInput): Record<string, number> {
  ...
  try { const v = input.absorbedDose * input.numExposures * input.wR * input.convFactor; results["equivalentDose_mSv"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["equivalentDose_mSv"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["equivalentDose_mSv"])) * input.wT; results["effectiveDose_mSv"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["effectiveDose_mSv"] = Number.NaN; }
  return results;
}
