...
function evaluateAllFormulas(input: Conservation_of_momentum_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mass1 * input.velocity1 + input.mass2 * input.velocity2; results["totalMomentum"] = Number.isFinite(v) ? v : 0; } catch { results["totalMomentum"] = 0; }
  try { const v = input.mass1 * input.velocity1; results["momentum1"] = Number.isFinite(v) ? v : 0; } catch { results["momentum1"] = 0; }
  try { const v = input.mass2 * input.velocity2; results["momentum2"] = Number.isFinite(v) ? v : 0; } catch { results["momentum2"] = 0; }
  return results;
}
...
export function calculateConservation_of_momentum_calculator(input: Conservation_of_momentum_calculatorInput): Conservation_of_momentum_calculatorOutput {
  ...
  const totalWasteCost = values["totalMomentum"] ?? 0;
  const breakdown = {
