...
function evaluateAllFormulas(input: Standard_deviation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.data_points * input.sample_size * input.unit_of_measure * input.specification_limit_lower; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.data_points * input.sample_size * input.unit_of_measure * input.specification_limit_lower * (input.specification_limit_upper * input.target_value); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.specification_limit_upper * input.target_value; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
