...
function evaluateAllFormulas(input: Standard_deviation_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.data_points + input.sample_size + input.data_type; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.data_points + input.sample_size + input.data_type; results["result_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_aux"] = 0; }
  return results;
}
