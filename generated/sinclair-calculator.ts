...
function evaluateAllFormulas(input: Sinclair_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bodyweight / input.coefficientB; results["bodyweightRatio"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["bodyweightRatio"] = Number.NaN; }
  try { const v = input.bodyweight / input.coefficientB; results["bodyweightRatio_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["bodyweightRatio_aux"] = Number.NaN; }
  return results;
}
