...
function evaluateAllFormulas(input: Churn_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.lostCustomers / input.startingCustomers) * 100; results["customerChurnRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["customerChurnRate"] = Number.NaN; }
  try { const v = (input.lostMRR / input.startingMRR) * 100; results["revenueChurnRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["revenueChurnRate"] = Number.NaN; }
  return results;
}
