...
function evaluateAllFormulas(input: Power_factor_correction_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.realPower / input.existingPowerFactor) - (input.realPower / input.targetPowerFactor); results["apparentPowerReduction"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["apparentPowerReduction"] = Number.NaN; }
  try { const v = (input.realPower / input.existingPowerFactor) - (input.realPower / input.targetPowerFactor); results["apparentPowerReduction_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["apparentPowerReduction_aux"] = Number.NaN; }
  return results;
}
