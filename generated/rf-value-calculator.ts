...
function evaluateAllFormulas(input: Rf_value_calculatorInput): Record<string, number> {
  ...
  try { const v = input.spotDistance1 / input.solventDistance; results["rf1"] = Number.isFinite(v) ? v : 0; } catch { results["rf1"] = 0; }
  try { const v = input.spotDistance2 / input.solventDistance; results["rf2"] = Number.isFinite(v) ? v : 0; } catch { results["rf2"] = 0; }
  try { const v = input.spotDistance3 / input.solventDistance; results["rf3"] = Number.isFinite(v) ? v : 0; } catch { results["rf3"] = 0; }
  try { const v = (input.spotDistance1 / input.solventDistance + input.spotDistance2 / input.solventDistance + input.spotDistance3 / input.solventDistance) / 3; results["averageRf"] = Number.isFinite(v) ? v : 0; } catch { results["averageRf"] = 0; }
  return results;
}
...
export function calculateRf_value_calculator(input: Rf_value_calculatorInput): Rf_value_calculatorOutput {
  ...
  const totalWasteCost = values["averageRf"] ?? 0;
  const breakdown = {
