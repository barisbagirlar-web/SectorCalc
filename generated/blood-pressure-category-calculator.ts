...
function evaluateAllFormulas(input: Blood_pressure_category_calculatorInput): Record<string, number> {
  ...
  try { const v = input.systolic * input.diastolic * (input.heart_rate / 100) * input.rest_duration; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.systolic * input.diastolic * (input.heart_rate / 100) * input.rest_duration; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
