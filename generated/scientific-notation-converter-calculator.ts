...
function evaluateAllFormulas(input: Scientific_notation_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.decimalNumber * input.scientificCoefficient * input.scientificExponent * input.conversionMode; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.decimalNumber * input.scientificCoefficient * input.scientificExponent * input.conversionMode * (input.precision); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
