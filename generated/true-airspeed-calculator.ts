...
function evaluateAllFormulas(input: True_airspeed_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.outsideAirTemperature + 273.15) / 288.15; results["theta"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["theta"] = Number.NaN; }
  try { const v = (input.outsideAirTemperature + 273.15) / 288.15; results["theta_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["theta_aux"] = Number.NaN; }
  return results;
}
