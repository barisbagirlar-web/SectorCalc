...
function evaluateAllFormulas(input: Real_return_calculatorInput): Record<string, number | string> {
  ...
  try { const v = (input.nominalReturn / 100) * (input.inflationRate / 100) * (input.taxRate / 100) * input.investmentAmount; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = (input.nominalReturn / 100) * (input.inflationRate / 100) * (input.taxRate / 100) * input.investmentAmount; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
