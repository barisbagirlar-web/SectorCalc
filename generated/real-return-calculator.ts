...
function evaluateAllFormulas(input: Real_return_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.nominalReturn / 100) * (input.inflationRate / 100) * (input.taxRate / 100) * input.investmentAmount; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = (input.nominalReturn / 100) * (input.inflationRate / 100) * (input.taxRate / 100) * input.investmentAmount; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
