...
function evaluateAllFormulas(input: Complex_number_multiplication_calculatorInput): Record<string, number> {
  ...
  try { const v = input.real1 * input.real2 - input.imag1 * input.imag2; results["realPart"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["realPart"] = Number.NaN; }
  try { const v = input.real1 * input.imag2 + input.imag1 * input.real2; results["imaginaryPart"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["imaginaryPart"] = Number.NaN; }
  return results;
}
