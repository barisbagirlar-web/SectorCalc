...
function evaluateAllFormulas(input: Complex_number_multiplication_calculatorInput): Record<string, number> {
  ...
  try { const v = input.real1 * input.real2 - input.imag1 * input.imag2; results["realPart"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["realPart"] = 0; }
  try { const v = input.real1 * input.imag2 + input.imag1 * input.real2; results["imaginaryPart"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["imaginaryPart"] = 0; }
  return results;
}
...
export function calculateComplex_number_multiplication_calculator(input: Complex_number_multiplication_calculatorInput): Complex_number_multiplication_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["realPart"]));
  const breakdown = {
