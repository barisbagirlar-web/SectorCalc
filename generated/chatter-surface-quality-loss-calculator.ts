...
function evaluateAllFormulas(input: Chatter_surface_quality_loss_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cutting_speed * (input.feed_rate / 100) * input.depth_of_cut * input.tool_overhang; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.cutting_speed * (input.feed_rate / 100) * input.depth_of_cut * input.tool_overhang * (input.tool_diameter * input.machine_spindle_power * input.machine_damping_ratio); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.tool_diameter * input.machine_spindle_power * input.machine_damping_ratio; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
...
export const Chatter_surface_quality_loss_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
