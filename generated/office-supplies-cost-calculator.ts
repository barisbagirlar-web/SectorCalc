...
function evaluateAllFormulas(input: Office_supplies_cost_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.annual_usage_units * input.unit_cost; results["base_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.annual_usage_units * input.unit_cost * (1 + (input.holding_cost_rate / 100)); results["adjusted_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.annual_usage_units * input.unit_cost * (1 + (input.holding_cost_rate / 100)) * (input.order_cost); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.order_cost; results["factor_order_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["factor_order_cost"] = 0; }
  return results;
}
