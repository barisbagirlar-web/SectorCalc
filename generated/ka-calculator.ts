...
function evaluateAllFormulas(input: Ka_calculatorInput): Record<string, number> {
  ...
  try { const v = input.concentrationA / input.concentrationHA; results["ratio"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["ratio"] = Number.NaN; }
  try { const v = input.concentrationA / input.concentrationHA; results["ratio_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["ratio_aux"] = Number.NaN; }
  return results;
}
