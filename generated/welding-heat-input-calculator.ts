...
function evaluateAllFormulas(input: Welding_heat_input_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.voltage * input.current * 60 * input.efficiency) / (input.travelSpeed * 1000); results["heatInput"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["heatInput"] = 0; }
  try { const v = (input.voltage * input.current * input.efficiency * 60) / 1000; results["effectiveHeatRate"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectiveHeatRate"] = 0; }
  try { const v = input.voltage * input.current; results["powerInput"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["powerInput"] = 0; }
  return results;
}
...
export function calculateWelding_heat_input_calculator(input: Welding_heat_input_calculatorInput): Welding_heat_input_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["heatInput"]));
  const breakdown = {
