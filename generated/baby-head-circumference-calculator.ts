...
function evaluateAllFormulas(input: Baby_head_circumference_calculatorInput): Record<string, number> {
  ...
  try { const v = input.head_length / 2; results["a"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["a"] = Number.NaN; }
  try { const v = input.head_width / 2; results["b"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["b"] = Number.NaN; }
  try { const v = input.head_height / 2; results["c"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["c"] = Number.NaN; }
  return results;
}
