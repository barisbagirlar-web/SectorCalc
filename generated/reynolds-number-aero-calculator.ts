...
function evaluateAllFormulas(input: Reynolds_number_aero_calculatorInput): Record<string, number> {
  ...
  try { const v = input.velocity * input.length; results["velocityLengthProduct"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["velocityLengthProduct"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["velocityLengthProduct"])) / input.kinematicViscosity; results["reynoldsNumber"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["reynoldsNumber"] = Number.NaN; }
  return results;
}
