...
function evaluateAllFormulas(input: Humidity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.temperature * (input.relativeHumidity / 100) * input.pressure * input.altitude; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.temperature * (input.relativeHumidity / 100) * input.pressure * input.altitude; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
