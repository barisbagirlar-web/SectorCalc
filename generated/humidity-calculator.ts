...
function evaluateAllFormulas(input: Humidity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.temperature * (input.relativeHumidity / 100) * input.pressure * input.altitude; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.temperature * (input.relativeHumidity / 100) * input.pressure * input.altitude; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
