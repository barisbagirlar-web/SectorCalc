...
function evaluateAllFormulas(input: Margin_of_safety_calculatorInput): Record<string, number> {
  ...
  try { const v = input.appliedForce / input.crossSectionArea; results["actualStress"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["actualStress"] = 0; }
  try { const v = input.materialUltimateStrength / input.safetyFactor; results["allowableStress"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["allowableStress"] = 0; }
  try { const v = ((asFormulaNumber(results["allowableStress"])) / (asFormulaNumber(results["actualStress"]))) - 1; results["marginOfSafety"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["marginOfSafety"] = 0; }
  try { const v = (asFormulaNumber(results["allowableStress"])) / (asFormulaNumber(results["actualStress"])); results["stressRatio"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["stressRatio"] = 0; }
  return results;
}
