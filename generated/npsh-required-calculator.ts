...
function evaluateAllFormulas(input: Npsh_required_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.speed + input.flowRate + input.suctionSpecificSpeed; results["result_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_aux"] = 0; }
  return results;
}
