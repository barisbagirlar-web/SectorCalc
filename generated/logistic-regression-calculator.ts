...
function evaluateAllFormulas(input: Logistic_regression_calculatorInput): Record<string, number> {
  ...
  try { const v = input.intercept + input.coef1 * input.value1 + input.coef2 * input.value2 + input.coef3 * input.value3; results["linearCombination"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["linearCombination"] = Number.NaN; }
  try { const v = input.intercept + input.coef1 * input.value1 + input.coef2 * input.value2 + input.coef3 * input.value3; results["linearCombination_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["linearCombination_aux"] = Number.NaN; }
  return results;
}
