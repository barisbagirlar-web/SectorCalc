...
function evaluateAllFormulas(input: Clustering_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.numPoints) * (input.numClusters) * (input.dimensions) * (input.maxIterations) * (input.tolerance) * (input.seed); results["numClusters"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["numClusters"] = Number.NaN; }
  try { const v = (input.numPoints) * (input.numClusters) * (input.dimensions); results["numClusters_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["numClusters_aux"] = Number.NaN; }
  return results;
}
