...
function evaluateAllFormulas(input: De_broglie_wavelength_calculatorInput): Record<string, number> {
  ...
  try { const v = input.electronMass * input.elementaryCharge * input.voltage * input.planckConstant; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.electronMass * input.elementaryCharge * input.voltage * input.planckConstant * (input.unitMultiplier); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.unitMultiplier; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
