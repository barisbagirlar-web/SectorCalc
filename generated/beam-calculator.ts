...
function evaluateAllFormulas(input: Beam_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.load * input.span) / 4; results["maxBendingMoment"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["maxBendingMoment"] = 0; }
  try { const v = (input.load * input.span) / 4; results["maxBendingMoment_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["maxBendingMoment_aux"] = 0; }
  return results;
}
...
export function calculateBeam_calculator(input: Beam_calculatorInput): Beam_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["maxBendingMoment"]));
  const breakdown = {
