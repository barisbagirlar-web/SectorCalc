...
function evaluateAllFormulas(input: Pound_to_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weightPounds * input.caloriesPerPound * input.includeKilojoules * input.roundingPrecision; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.weightPounds * input.caloriesPerPound * input.includeKilojoules * input.roundingPrecision; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
