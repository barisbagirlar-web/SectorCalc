...
function evaluateAllFormulas(input: Time_value_of_money_calculatorInput): Record<string, number> {
  ...
  try { const v = input.presentValue * (input.annualInterestRate / 100) * input.numberOfYears * input.compoundingFrequency; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.presentValue * (input.annualInterestRate / 100) * input.numberOfYears * input.compoundingFrequency; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
