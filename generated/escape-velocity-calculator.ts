...
function evaluateAllFormulas(input: Escape_velocity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mass * input.radius * input.altitude * input.G; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.mass * input.radius * input.altitude * input.G; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
