...
function evaluateAllFormulas(input: Refraction_calculatorInput): Record<string, number> {
  ...
  try { const v = input.incidentAngle * input.refractiveIndex1 * input.refractiveIndex2 * input.thickness; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.incidentAngle * input.refractiveIndex1 * input.refractiveIndex2 * input.thickness; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
