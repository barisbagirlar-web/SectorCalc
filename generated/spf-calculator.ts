...
function evaluateAllFormulas(input: Spf_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.fuelMassFlow * 1000 / input.powerOutput; results["sfc"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["sfc"] = 0; }
  try { const v = input.fuelMassFlow * input.carbonContent * input.oxidationFactor * (44/12); results["co2Emissions"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["co2Emissions"] = 0; }
  return results;
}
