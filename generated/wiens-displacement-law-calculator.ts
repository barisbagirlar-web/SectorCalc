...
function evaluateAllFormulas(input: Wiens_displacement_law_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.constant_b / input.temperature; results["peakWavelength_m"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["peakWavelength_m"] = 0; }
  try { const v = (asFormulaNumber(results["peakWavelength_m"])) * input.wavelength_scale; results["peakWavelength_scaled"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["peakWavelength_scaled"] = 0; }
  try { const v = input.constant_b_freq * input.temperature; results["peakFrequency_Hz"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["peakFrequency_Hz"] = 0; }
  return results;
}
