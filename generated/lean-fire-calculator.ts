...
function evaluateAllFormulas(input: Lean_fire_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualExpenses / (input.safeWithdrawalRate / 100); results["fireNumber"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["fireNumber"] = Number.NaN; }
  try { const v = input.monthlyContribution * 12; results["annualContribution"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annualContribution"] = Number.NaN; }
  ...
  try { const v = ((toNumericFormulaValue(results["fireNumber"])) * (input.safeWithdrawalRate / 100)) / 12; results["monthlySafeWithdrawal"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["monthlySafeWithdrawal"] = Number.NaN; }
  return results;
}
