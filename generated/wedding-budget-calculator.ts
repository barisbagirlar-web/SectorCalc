...
function evaluateAllFormulas(input: Wedding_budget_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numberOfGuests * input.costPerGuest; results["foodCost"] = Number.isFinite(v) ? v : 0; } catch { results["foodCost"] = 0; }
  try { const v = (results["foodCost"] ?? 0) + input.venueCost + input.decorationCost + input.photographyCost + input.miscellaneousCost; results["totalBudget"] = Number.isFinite(v) ? v : 0; } catch { results["totalBudget"] = 0; }
  try { const v = (results["totalBudget"] ?? 0) / input.numberOfGuests; results["costPerGuestOverall"] = Number.isFinite(v) ? v : 0; } catch { results["costPerGuestOverall"] = 0; }
  return results;
}
