...
function evaluateAllFormulas(input: Wedding_budget_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numberOfGuests * input.costPerGuest; results["foodCost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["foodCost"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["foodCost"])) + input.venueCost + input.decorationCost + input.photographyCost + input.miscellaneousCost; results["totalBudget"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalBudget"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["totalBudget"])) / input.numberOfGuests; results["costPerGuestOverall"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["costPerGuestOverall"] = Number.NaN; }
  return results;
}
