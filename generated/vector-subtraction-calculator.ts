...
function evaluateAllFormulas(input: Vector_subtraction_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.a_x - input.b_x; results["result_x"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_x"] = 0; }
  try { const v = input.a_y - input.b_y; results["result_y"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_y"] = 0; }
  try { const v = input.a_z - input.b_z; results["result_z"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_z"] = 0; }
  return results;
}
