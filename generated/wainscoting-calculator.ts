...
function evaluateAllFormulas(input: Wainscoting_calculatorInput): Record<string, number> {
  ...
  try { const v = input.wallLength * input.wainscotingHeight * input.panelWidth * input.stileWidth; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.wallLength * input.wainscotingHeight * input.panelWidth * input.stileWidth * (input.sheetLength * input.sheetWidth); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.sheetLength * input.sheetWidth; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
