...
function evaluateAllFormulas(input: Small_business_grants_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.numEmployees * 1 * 1 * input.projectCost; results["direct_labor_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["direct_labor_cost"] = 0; }
  try { const v = input.numEmployees * 1 * 1 * input.projectCost * input.avgAnnualRevenue; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
