...
function evaluateAllFormulas(input: Two_sample_t_test_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.sample1StdDev ** 2 / input.sample1Size + input.sample2StdDev ** 2 / input.sample2Size) ** 2) / (((input.sample1StdDev ** 2 / input.sample1Size) ** 2) / (input.sample1Size - 1) + ((input.sample2StdDev ** 2 / input.sample2Size) ** 2) / (input.sample2Size - 1)); results["degreesOfFreedom"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["degreesOfFreedom"] = Number.NaN; }
  try { const v = ((input.sample1StdDev ** 2 / input.sample1Size + input.sample2StdDev ** 2 / input.sample2Size) ** 2) / (((input.sample1StdDev ** 2 / input.sample1Size) ** 2) / (input.sample1Size - 1) + ((input.sample2StdDev ** 2 / input.sample2Size) ** 2) / (input.sample2Size - 1)); results["degreesOfFreedom_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["degreesOfFreedom_aux"] = Number.NaN; }
  return results;
}
