...
function evaluateAllFormulas(input: Cagr_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.endingValue / input.beginningValue) ** (1 / (input.endYear - input.startYear)) - 1; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
...
export function calculateCagr_calculator(input: Cagr_calculatorInput): Cagr_calculatorOutput {
  ...
  const hiddenLossDrivers: string[] = ["Inflation not accounted for","Dividends or cash flows excluded"];
  const suggestedActions: string[] = ["Verify beginning and ending values for accuracy","Consider reinvestment of dividends for total return"];
