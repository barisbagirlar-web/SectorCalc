...
function evaluateAllFormulas(input: Footing_design_calculatorInput): Record<string, number> {
  ...
  try { const v = input.columnLoad * input.safetyFactor / input.soilBearingCapacity; results["requiredArea"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["requiredArea"] = Number.NaN; }
  try { const v = input.columnLoad * input.safetyFactor / input.soilBearingCapacity; results["requiredArea_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["requiredArea_aux"] = Number.NaN; }
  return results;
}
