...
function evaluateAllFormulas(input: Boundary_layer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.velocity * input.distance / input.kinematicViscosity; results["re"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["re"] = Number.NaN; }
  try { const v = input.velocity * input.distance / input.kinematicViscosity; results["re_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["re_aux"] = Number.NaN; }
  return results;
}
