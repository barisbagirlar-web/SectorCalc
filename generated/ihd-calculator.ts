...
function evaluateAllFormulas(input: Ihd_calculatorInput): Record<string, number> {
  ...
  try { const v = input.plannedProduction * input.actualProduction * input.goodParts * input.totalParts; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.plannedProduction * input.actualProduction * input.goodParts * input.totalParts; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
