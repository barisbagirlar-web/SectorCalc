...
function evaluateAllFormulas(input: Ihd_calculatorInput): Record<string, number> {
  ...
  try { const v = input.plannedProduction * input.actualProduction * input.goodParts * input.totalParts; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.plannedProduction * input.actualProduction * input.goodParts * input.totalParts; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
