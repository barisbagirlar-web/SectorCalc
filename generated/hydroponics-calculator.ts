...
function evaluateAllFormulas(input: Hydroponics_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.fertilizerMass * 10 * input.nutrientPercentage) / input.waterVolumeForStock; results["stockPpm"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["stockPpm"] = 0; }
  try { const v = (input.targetPpm * input.reservoirWaterVolume) / (asFormulaNumber(results["stockPpm"])); results["stockVolumeL"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["stockVolumeL"] = 0; }
  try { const v = (asFormulaNumber(results["stockVolumeL"])) * 1000; results["stockVolumeML"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["stockVolumeML"] = 0; }
  return results;
}
