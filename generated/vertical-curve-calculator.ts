...
function evaluateAllFormulas(input: Vertical_curve_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.g1 / 100) * (input.g2 / 100) * input.L * input.station_PVC; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = (input.g1 / 100) * (input.g2 / 100) * input.L * input.station_PVC * (input.elevation_PVC * input.target_station); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.elevation_PVC * input.target_station; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
