...
function evaluateAllFormulas(input: Equilibrium_constant_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.c_A * input.c_B * input.c_C * input.c_D; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.c_A * input.c_B * input.c_C * input.c_D * (input.a * input.b * input.c * input.d); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.a * input.b * input.c * input.d; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
