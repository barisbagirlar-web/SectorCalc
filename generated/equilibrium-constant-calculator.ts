...
function evaluateAllFormulas(input: Equilibrium_constant_calculatorInput): Record<string, number> {
  ...
  try { const v = input.c_A * input.c_B * input.c_C * input.c_D; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.c_A * input.c_B * input.c_C * input.c_D * (input.a * input.b * input.c * input.d); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.a * input.b * input.c * input.d; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
