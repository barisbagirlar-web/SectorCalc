...
function evaluateAllFormulas(input: Heat_exchanger_area_calculatorInput): Record<string, number> {
  ...
  try { const v = input.heatDuty * 1000; results["heatDutyW"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["heatDutyW"] = Number.NaN; }
  try { const v = input.hotInletTemp - input.coldOutletTemp; results["deltaT1"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["deltaT1"] = Number.NaN; }
  try { const v = input.hotOutletTemp - input.coldInletTemp; results["deltaT2"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["deltaT2"] = Number.NaN; }
  return results;
}
