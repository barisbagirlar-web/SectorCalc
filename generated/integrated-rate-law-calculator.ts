...
function evaluateAllFormulas(input: Integrated_rate_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialConcentration * input.rateConstant * input.time * input.reactionOrder; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.initialConcentration * input.rateConstant * input.time * input.reactionOrder; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
