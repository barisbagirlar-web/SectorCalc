...
function evaluateAllFormulas(input: Semi_annual_compound_interest_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.principal * (1 + (input.annualRate/100) / input.compoundingPeriods) ** (input.compoundingPeriods * input.years); results["futureValue"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["futureValue"] = 0; }
  ...
  try { const v = ((1 + (input.annualRate/100) / input.compoundingPeriods) ** input.compoundingPeriods - 1) * 100; results["effectiveAnnualRate"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["effectiveAnnualRate"] = 0; }
  return results;
}
