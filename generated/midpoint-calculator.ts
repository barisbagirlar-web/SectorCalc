...
function evaluateAllFormulas(input: Midpoint_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.x1 + input.x2) / 2; results["midX"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["midX"] = Number.NaN; }
  try { const v = (input.y1 + input.y2) / 2; results["midY"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["midY"] = Number.NaN; }
  return results;
}
