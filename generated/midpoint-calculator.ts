...
function evaluateAllFormulas(input: Midpoint_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.x1 + input.x2) / 2; results["midX"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["midX"] = 0; }
  try { const v = (input.y1 + input.y2) / 2; results["midY"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["midY"] = 0; }
  return results;
}
