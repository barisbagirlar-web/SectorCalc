...
function evaluateAllFormulas(input: Freight_cost_calculatorInput): Record<string, number> {
  ...
  try { results["dimensional_weight"] = (Math.Math.round((input.shipment_volume_cbm * 167) * 10**(1)) / 10**(1)); } catch { results["dimensional_weight"] = 0; }
  ...
  try { results["fuel_surcharge_amount"] = (Math.Math.round((base_freight_cost * (input.fuel_surcharge_percent / 100)) * 10**(2)) / 10**(2)); } catch { results["fuel_surcharge_amount"] = 0; }
  try { results["hazardous_premium"] = IF(input.is_hazardous == true, (results["chargeable_weight"] ?? 0) * 0.10, 0); } catch { results["hazardous_premium"] = 0; }
  try { results["expedite_multiplier"] = IF(input.is_expedited == true, 1.5, 1.0); } catch { results["expedite_multiplier"] = 0; }
  try { results["total_freight_cost"] = (Math.Math.round(((base_freight_cost + (results["fuel_surcharge_amount"] ?? 0) + (results["hazardous_premium"] ?? 0) + input.accessorial_charges_usd) * (results["expedite_multiplier"] ?? 0)) * 10**(2)) / 10**(2)); } catch { results["total_freight_cost"] = 0; }
  return results;
}
