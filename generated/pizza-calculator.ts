...
function evaluateAllFormulas(input: Pizza_calculatorInput): Record<string, number> {
  ...
  try { const v = input.pizzaPrice * input.quantity * (1 - input.discountRate / 100); results["totalCostAfterDiscount"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalCostAfterDiscount"] = 0; }
  try { const v = input.pizzaPrice * input.quantity * (1 - input.discountRate / 100); results["totalCostAfterDiscount_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalCostAfterDiscount_aux"] = 0; }
  return results;
}
...
export function calculatePizza_calculator(input: Pizza_calculatorInput): Pizza_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["totalCostAfterDiscount"]));
  const breakdown = {
