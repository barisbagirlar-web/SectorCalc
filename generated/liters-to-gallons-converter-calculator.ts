...
function evaluateAllFormulas(input: Liters_to_gallons_converter_calculatorInput): Record<string, number> {
  const results: Record<string, number> = {};
  try { const v = input.liters * input.temperatureAdjustment * (input.batchLossFactor / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.liters * input.temperatureAdjustment * (input.batchLossFactor / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
