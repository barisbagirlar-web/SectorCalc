...
function evaluateAllFormulas(input: Liters_to_gallons_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.liters * input.temperatureAdjustment * (input.batchLossFactor / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.liters * input.temperatureAdjustment * (input.batchLossFactor / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
