...
function evaluateAllFormulas(input: Arcsine_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sineValue * input.outputUnit * input.decimalPlaces * input.angleSelection; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.sineValue * input.outputUnit * input.decimalPlaces * input.angleSelection; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
