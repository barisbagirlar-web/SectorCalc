...
function evaluateAllFormulas(input: Mass_percent_calculatorInput): Record<string, number> {
  ...
  try { const v = input.componentMass * input.otherComponentMass * input.totalMass * input.precision; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.componentMass * input.otherComponentMass * input.totalMass * input.precision; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
