...
function evaluateAllFormulas(input: Mass_percent_calculatorInput): Record<string, number> {
  ...
  try { const v = input.componentMass * input.otherComponentMass * input.totalMass * input.precision; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.componentMass * input.otherComponentMass * input.totalMass * input.precision; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
