...
function evaluateAllFormulas(input: Polarization_calculatorInput): Record<string, number> {
  ...
  try { const v = input.incident_intensity * input.incident_angle * input.polarizer1_angle * input.polarizer2_angle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.incident_intensity * input.incident_angle * input.polarizer1_angle * input.polarizer2_angle; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
