...
function evaluateAllFormulas(input: Polarization_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.incident_intensity * input.incident_angle * input.polarizer1_angle * input.polarizer2_angle; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.incident_intensity * input.incident_angle * input.polarizer1_angle * input.polarizer2_angle; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
