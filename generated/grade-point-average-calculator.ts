...
function evaluateAllFormulas(input: Grade_point_average_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.credit1*input.grade1 + input.credit2*input.grade2 + input.credit3*input.grade3 + input.credit4*input.grade4) / (input.credit1+input.credit2+input.credit3+input.credit4); results["gpa"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["gpa"] = Number.NaN; }
  try { const v = input.credit1*input.grade1 + input.credit2*input.grade2 + input.credit3*input.grade3 + input.credit4*input.grade4; results["totalGradePoints"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalGradePoints"] = Number.NaN; }
