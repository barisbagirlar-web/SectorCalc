...
function evaluateAllFormulas(input: Gibbs_free_energy_reaction_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.S_products - input.S_reactants) / 1000; results["deltaS_kJ_per_molK"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["deltaS_kJ_per_molK"] = 0; }
  try { const v = input.temperature * ((input.S_products - input.S_reactants) / 1000); results["T_deltaS_kJ_per_mol"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["T_deltaS_kJ_per_mol"] = 0; }
  try { const v = input.deltaH - (input.temperature * ((input.S_products - input.S_reactants) / 1000)); results["deltaG_kJ_per_mol"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["deltaG_kJ_per_mol"] = 0; }
  return results;
}
...
export function calculateGibbs_free_energy_reaction_calculator(input: Gibbs_free_energy_reaction_calculatorInput): Gibbs_free_energy_reaction_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["deltaG_kJ_per_mol"]));
  const breakdown = {
