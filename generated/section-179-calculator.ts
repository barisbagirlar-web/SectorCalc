...
function evaluateAllFormulas(input: Section_179_calculatorInput): Record<string, number> {
  ...
  try { const v = input.assetCost * (input.businessUsePercent / 100) * input.section179Limit * input.phaseOutThreshold; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.assetCost * (input.businessUsePercent / 100) * input.section179Limit * input.phaseOutThreshold * (input.totalAssetAdditions); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.totalAssetAdditions; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
