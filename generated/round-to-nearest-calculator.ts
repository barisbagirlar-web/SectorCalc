...
function evaluateAllFormulas(input: Round_to_nearest_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.value - input.offset) / input.nearest; results["adjustedValue"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustedValue"] = Number.NaN; }
  try { const v = (input.value - input.offset) / input.nearest; results["adjustedValue_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustedValue_aux"] = Number.NaN; }
  return results;
}
