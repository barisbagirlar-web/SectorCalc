...
function evaluateAllFormulas(input: Wire_mesh_calculatorInput): Record<string, number> {
  ...
  try { const v = input.wireDiameter * input.meshOpening * input.sheetWidth * input.sheetLength; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.wireDiameter * input.meshOpening * input.sheetWidth * input.sheetLength * (input.quantity * input.materialDensity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.quantity * input.materialDensity; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
