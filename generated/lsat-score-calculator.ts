...
function evaluateAllFormulas(input: Lsat_score_calculatorInput): Record<string, number> {
  ...
  try { const v = input.lr1 * input.lr2 * input.ar * input.rc; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.lr1 * input.lr2 * input.ar * input.rc; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
