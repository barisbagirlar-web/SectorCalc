...
function evaluateAllFormulas(input: Loan_amortization_schedule_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.annualInterestRate / 100) / input.paymentsPerYear; results["periodicInterestRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["periodicInterestRate"] = Number.NaN; }
  try { const v = input.loanTermYears * input.paymentsPerYear; results["totalPayments"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalPayments"] = Number.NaN; }
  return results;
}
