...
function evaluateAllFormulas(input: Egitim_birikim_hesaplayiciInput): Record<string, number | string> {
  ...
  try { const v = input.years * 12; results["months"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["months"] = 0; }
  return results;
}
