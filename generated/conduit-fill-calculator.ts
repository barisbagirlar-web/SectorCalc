...
function evaluateAllFormulas(input: Conduit_fill_calculatorInput): Record<string, number> {
  ...
  try { const v = input.conduitFillLimitPercent > 0 ? input.conduitFillLimitPercent - input.safetyMarginPercent : (input.numberOfConductors === 1 ? 53 : input.numberOfConductors === 2 ? 31 : 40) - input.safetyMarginPercent; results["allowedFillPercent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["allowedFillPercent"] = Number.NaN; }
  try { const v = input.conduitFillLimitPercent > 0 ? input.conduitFillLimitPercent - input.safetyMarginPercent : (input.numberOfConductors === 1 ? 53 : input.numberOfConductors === 2 ? 31 : 40) - input.safetyMarginPercent; results["allowedFillPercent_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["allowedFillPercent_aux"] = Number.NaN; }
  return results;
}
