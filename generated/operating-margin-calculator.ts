...
function evaluateAllFormulas(input: Operating_margin_calculatorInput): Record<string, number> {
  ...
  try { const v = input.revenue - input.cogs - input.opex + input.otherIncome; results["operatingIncome"] = Number.isFinite(v) ? v : 0; } catch { results["operatingIncome"] = 0; }
  try { const v = ((results["operatingIncome"] ?? 0) / input.revenue) * 100; results["operatingMargin"] = Number.isFinite(v) ? v : 0; } catch { results["operatingMargin"] = 0; }
  return results;
}
...
export function calculateOperating_margin_calculator(input: Operating_margin_calculatorInput): Operating_margin_calculatorOutput {
  ...
  const totalWasteCost = values["operatingMargin"] ?? 0;
  const breakdown = {
