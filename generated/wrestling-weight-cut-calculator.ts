...
function evaluateAllFormulas(input: Wrestling_weight_cut_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentWeight - input.targetWeight; results["requiredWeightLoss"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["requiredWeightLoss"] = 0; }
  try { const v = input.currentWeight * input.maxDehydrationPercentage / 100; results["maxSafeWaterLoss"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["maxSafeWaterLoss"] = 0; }
  try { const v = (asFormulaNumber(results["requiredWeightLoss"])) / input.hoursUntilWeighIn; results["cutRatePerHour"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["cutRatePerHour"] = 0; }
  return results;
}
...
export function calculateWrestling_weight_cut_calculator(input: Wrestling_weight_cut_calculatorInput): Wrestling_weight_cut_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["requiredWeightLoss"]));
  const breakdown = {
