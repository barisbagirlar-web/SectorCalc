...
function evaluateAllFormulas(input: Integral_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a * input.b * input.c * input.d; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.a * input.b * input.c * input.d * (input.x1 * input.x2); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.x1 * input.x2; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
