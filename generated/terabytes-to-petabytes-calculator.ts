...
function evaluateAllFormulas(input: Terabytes_to_petabytes_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.totalRawTerabytes * input.raidUsableMultiplier * (1 - input.formatOverheadPercent / 100); results["usableTB"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["usableTB"] = 0; }
  try { const v = (input.totalRawTerabytes * input.raidUsableMultiplier * (1 - input.formatOverheadPercent / 100)) / input.conversionFactor; results["petabytes"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["petabytes"] = 0; }
  try { const v = input.totalRawTerabytes / input.conversionFactor; results["rawPB"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["rawPB"] = 0; }
  return results;
}
