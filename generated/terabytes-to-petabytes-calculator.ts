...
function evaluateAllFormulas(input: Terabytes_to_petabytes_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalRawTerabytes * input.raidUsableMultiplier * (1 - input.formatOverheadPercent / 100); results["usableTB"] = Number.isFinite(v) ? v : 0; } catch { results["usableTB"] = 0; }
  try { const v = (input.totalRawTerabytes * input.raidUsableMultiplier * (1 - input.formatOverheadPercent / 100)) / input.conversionFactor; results["petabytes"] = Number.isFinite(v) ? v : 0; } catch { results["petabytes"] = 0; }
  try { const v = input.totalRawTerabytes / input.conversionFactor; results["rawPB"] = Number.isFinite(v) ? v : 0; } catch { results["rawPB"] = 0; }
  return results;
}
...
export function calculateTerabytes_to_petabytes_calculator(input: Terabytes_to_petabytes_calculatorInput): Terabytes_to_petabytes_calculatorOutput {
  ...
  const totalWasteCost = values["petabytes"] ?? 0;
  const breakdown = {
