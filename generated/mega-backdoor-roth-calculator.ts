...
function evaluateAllFormulas(input: Mega_backdoor_roth_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.overallLimit - input.preTaxDeferral - input.employerContribution; results["afterTaxSpace"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["afterTaxSpace"] = 0; }
  try { const v = (asFormulaNumber(results["afterTaxSpace"])) + input.preTaxDeferral + input.employerContribution; results["totalPotentialContributions"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalPotentialContributions"] = 0; }
  return results;
}
