...
function evaluateAllFormulas(input: Cycling_atl_calculatorInput): Record<string, number> {
  ...
  try { const v = input.atlPrevious + (input.tssToday - input.atlPrevious) / input.timeConstant; results["atlCurrent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["atlCurrent"] = Number.NaN; }
  try { const v = (input.tssToday - input.atlPrevious) / input.timeConstant; results["atlChange"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["atlChange"] = Number.NaN; }
  return results;
}
