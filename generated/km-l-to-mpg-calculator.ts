...
function evaluateAllFormulas(input: Km_l_to_mpg_calculatorInput): Record<string, number> {
  ...
  try { const v = input.value * input.sourceUnit * input.decimalPlaces * input.gallonType; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.value * input.sourceUnit * input.decimalPlaces * input.gallonType; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
