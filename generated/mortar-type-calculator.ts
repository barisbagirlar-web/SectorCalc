...
function evaluateAllFormulas(input: Mortar_type_calculatorInput): Record<string, number> {
  ...
  try { const v = input.design_strength * input.exposure_factor * (input.reinforced ? 1.2 : 1) * input.safety_factor; results["required_strength"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["required_strength"] = Number.NaN; }
  try { const v = input.design_strength * input.exposure_factor * (input.reinforced ? 1.2 : 1) * input.safety_factor; results["required_strength_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["required_strength_aux"] = Number.NaN; }
  return results;
}
