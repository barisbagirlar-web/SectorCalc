...
function evaluateAllFormulas(input: C_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.current / input.capacity; results["C-Rate (C)"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["C-Rate (C)"] = 0; }
  try { const v = input.capacity / input.current; results["Discharge Time (h)"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["Discharge Time (h)"] = 0; }
  try { const v = input.current * input.voltage; results["Power (W)"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["Power (W)"] = 0; }
  return results;
}
