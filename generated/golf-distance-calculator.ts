...
function evaluateAllFormulas(input: Golf_distance_calculatorInput): Record<string, number> {
  ...
  try { const v = input.clubSpeed * input.smashFactor; results["ballSpeed"] = Number.isFinite(v) ? v : 0; } catch { results["ballSpeed"] = 0; }
  try { const v = ((results["ballSpeed"] ?? 0) * 1.75) - 40 + (input.launchAngle - 12) * 3 - (input.spinRate - 2500) * 0.03 + (input.windSpeed * 2) + (input.altitude * 0.001); results["carryDistance"] = Number.isFinite(v) ? v : 0; } catch { results["carryDistance"] = 0; }
  try { const v = (results["carryDistance"] ?? 0) * 1.08; results["totalDistance"] = Number.isFinite(v) ? v : 0; } catch { results["totalDistance"] = 0; }
  return results;
}
