...
function evaluateAllFormulas(input: Golf_distance_calculatorInput): Record<string, number> {
  ...
  try { const v = input.clubSpeed * input.smashFactor; results["ballSpeed"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["ballSpeed"] = Number.NaN; }
  try { const v = ((toNumericFormulaValue(results["ballSpeed"])) * 1.75) - 40 + (input.launchAngle - 12) * 3 - (input.spinRate - 2500) * 0.03 + (input.windSpeed * 2) + (input.altitude * 0.001); results["carryDistance"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["carryDistance"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["carryDistance"])) * 1.08; results["totalDistance"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalDistance"] = Number.NaN; }
  return results;
}
