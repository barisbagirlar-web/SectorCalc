...
function evaluateAllFormulas(input: Iso_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.availability / 100) * (input.performance / 100) * (input.quality / 100) * 100; results["primary"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["primary"] = Number.NaN; }
  try { const v = input.availability; results["breakdown"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["breakdown"] = Number.NaN; }
  return results;
}
