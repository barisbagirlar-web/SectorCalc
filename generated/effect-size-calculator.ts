...
function evaluateAllFormulas(input: Effect_size_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mean1 - input.mean2; results["meanDiff"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["meanDiff"] = Number.NaN; }
  try { const v = input.mean1 - input.mean2; results["meanDiff_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["meanDiff_aux"] = Number.NaN; }
  return results;
}
