...
function evaluateAllFormulas(input: Sleep_quality_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.sleepEfficiency; results["efficiencyScore"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["efficiencyScore"] = 0; }
  try { const v = input.sleepEfficiency; results["efficiencyScore_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["efficiencyScore_aux"] = 0; }
  return results;
}
