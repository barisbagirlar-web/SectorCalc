...
function evaluateAllFormulas(input: Months_to_years_converter_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.months * input.monthsPerYear * input.decimalPlaces * input.outputMode; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.months * input.monthsPerYear * input.decimalPlaces * input.outputMode; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
