...
function evaluateAllFormulas(input: Polar_koordinat_donusturucu_calculatorInput): Record<string, number> {
  ...
  try { const v = input.conversionType * input.x * input.y * input.r; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.conversionType * input.x * input.y * input.r * (input.theta); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
