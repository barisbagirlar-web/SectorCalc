...
function evaluateAllFormulas(input: Wrinkle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.thickness * input.blankDiameter * input.punchDiameter * input.drawDepth; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.thickness * input.blankDiameter * input.punchDiameter * input.drawDepth * (input.yieldStrength * input.elasticModulus); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.yieldStrength * input.elasticModulus; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
