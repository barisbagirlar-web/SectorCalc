...
function evaluateAllFormulas(input: Kpa_to_psi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.pressureKpa * input.conversionFactor + input.offset; results["psi"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["psi"] = Number.NaN; }
  try { const v = input.pressureKpa * input.conversionFactor + input.offset; results["psi_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["psi_aux"] = Number.NaN; }
  return results;
}
