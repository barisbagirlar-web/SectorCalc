...
function evaluateAllFormulas(input: Triangular_prism_volume_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.sideA + input.sideB + input.sideC) / 2; results["s"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["s"] = 0; }
  try { const v = (input.sideA + input.sideB + input.sideC) / 2; results["s_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["s_aux"] = 0; }
  return results;
}
...
export function calculateTriangular_prism_volume_calculator(input: Triangular_prism_volume_calculatorInput): Triangular_prism_volume_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["s"]));
  const breakdown = {
