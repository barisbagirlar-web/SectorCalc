...
function evaluateAllFormulas(input: Light_years_to_km_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.lightYears) * (input.significantDigits) * (input.outputFormat) * (input.roundingMode); results["kmRaw"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["kmRaw"] = Number.NaN; }
  try { const v = (input.lightYears) * (input.significantDigits) * (input.outputFormat); results["kmRaw_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["kmRaw_aux"] = Number.NaN; }
  return results;
}
