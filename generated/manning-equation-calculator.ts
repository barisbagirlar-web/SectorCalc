...
function evaluateAllFormulas(input: Manning_equation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.roughness * input.hydraulicRadius * input.area * input.slope; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.roughness * input.hydraulicRadius * input.area * input.slope; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
