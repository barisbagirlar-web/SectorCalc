...
function evaluateAllFormulas(input: Fiber_optic_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fiberLength * input.attenuationCoeff + input.numSplices * input.spliceLoss + input.numConnectors * input.connectorLoss; results["totalLoss"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalLoss"] = 0; }
  try { const v = input.inputPower - (asFormulaNumber(results["totalLoss"])); results["receivedPower"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["receivedPower"] = 0; }
  return results;
}
...
export function calculateFiber_optic_calculator(input: Fiber_optic_calculatorInput): Fiber_optic_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["totalLoss"]));
  const breakdown = {
