...
function evaluateAllFormulas(input: Body_recomposition_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * (input.bodyFatPercentage / 100) * input.height * input.age; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.weight * (input.bodyFatPercentage / 100) * input.height * input.age * (input.activityLevel); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.activityLevel; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
