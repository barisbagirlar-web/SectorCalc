...
function evaluateAllFormulas(input: Ielts_score_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.listening + input.reading + input.writing + input.speaking) / 4; results["averageScore"] = Number.isFinite(v) ? v : 0; } catch { results["averageScore"] = 0; }
  try { const v = Math.round(((input.listening + input.reading + input.writing + input.speaking) / 4) * 2) / 2; results["overallBand"] = Number.isFinite(v) ? v : 0; } catch { results["overallBand"] = 0; }
  return results;
}
...
export function calculateIelts_score_calculator(input: Ielts_score_calculatorInput): Ielts_score_calculatorOutput {
  ...
  const totalWasteCost = values["overallBand"] ?? 0;
  const breakdown = {
