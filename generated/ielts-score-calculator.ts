...
function evaluateAllFormulas(input: Ielts_score_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.listening + input.reading + input.writing + input.speaking) / 4; results["averageScore"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["averageScore"] = Number.NaN; }
  try { const v = (input.listening + input.reading + input.writing + input.speaking) / 4; results["averageScore_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["averageScore_aux"] = Number.NaN; }
  return results;
}
