...
function evaluateAllFormulas(input: Wet_bulb_temperatureInput): Record<string, number | string> {
  ...
  try { const v = input.dryBulbTemp * (input.relativeHumidity / 100) * input.atmosphericPressure * input.altitude; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.dryBulbTemp * (input.relativeHumidity / 100) * input.atmosphericPressure * input.altitude; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
