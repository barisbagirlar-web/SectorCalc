...
function evaluateAllFormulas(input: Pounds_to_ouncesInput): Record<string, number> {
  ...
  try { const v = input.pounds * 16; results["ounces"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["ounces"] = 0; }
  try { const v = input.pounds * 16; results["ounces___pounds___16"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["ounces___pounds___16"] = 0; }
  return results;
}
...
export function calculatePounds_to_ounces(input: Pounds_to_ouncesInput): Pounds_to_ouncesOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["ounces___pounds___16"]));
  const breakdown = {
