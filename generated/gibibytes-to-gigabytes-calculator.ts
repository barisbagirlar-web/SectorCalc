...
function evaluateAllFormulas(input: Gibibytes_to_gigabytes_calculatorInput): Record<string, number> {
  ...
  try { const v = input.gibibytes * input.conversionFactor; results["rawGigabytes"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["rawGigabytes"] = 0; }
  try { const v = (asFormulaNumber(results["rawGigabytes"])) * (1 + input.safetyFactorPercent / 100); results["safetyAppliedGigabytes"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["safetyAppliedGigabytes"] = 0; }
  return results;
}
...
export function calculateGibibytes_to_gigabytes_calculator(input: Gibibytes_to_gigabytes_calculatorInput): Gibibytes_to_gigabytes_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["safetyAppliedGigabytes"]));
  const breakdown = {
