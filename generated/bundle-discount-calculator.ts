...
function evaluateAllFormulas(input: Bundle_discount_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numberOfBundles * input.itemsPerBundle; results["totalItems"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalItems"] = 0; }
  try { const v = (asFormulaNumber(results["totalItems"])) * input.pricePerItem; results["undiscountedCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["undiscountedCost"] = 0; }
  try { const v = input.pricePerItem * (1 - input.bundleDiscountPercent / 100); results["bundleDiscountedPricePerItem"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["bundleDiscountedPricePerItem"] = 0; }
  try { const v = input.itemsPerBundle * (asFormulaNumber(results["bundleDiscountedPricePerItem"])); results["bundlePrice"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["bundlePrice"] = 0; }
  try { const v = input.numberOfBundles * (asFormulaNumber(results["bundlePrice"])); results["totalBundleCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalBundleCost"] = 0; }
  try { const v = (asFormulaNumber(results["undiscountedCost"])) - (asFormulaNumber(results["totalBundleCost"])); results["bundleDiscountAmount"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["bundleDiscountAmount"] = 0; }
...
export function calculateBundle_discount_calculator(input: Bundle_discount_calculatorInput): Bundle_discount_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["totalItems"]));
  const breakdown = {
