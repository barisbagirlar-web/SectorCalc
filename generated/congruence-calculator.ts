...
function evaluateAllFormulas(input: Congruence_calculatorInput): Record<string, number> {
  ...
  try { const v = 'Remainder of ('+input.a+' + '+input.shift+') mod '+input.modulus+' = ' + (((input.a + input.shift) % input.modulus + input.modulus) % input.modulus); results["breakdown[0]"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["breakdown[0]"] = 0; }
  try { const v = 'Remainder of '+input.b+' mod '+input.modulus+' = ' + ((input.b % input.modulus + input.modulus) % input.modulus); results["breakdown[1]"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["breakdown[1]"] = 0; }
  return results;
}
