...
function evaluateAllFormulas(input: Skewness_calculatorInput): Record<string, number> {
  ...
  try { const v = 8; results["n"] = Number.isFinite(v) ? v : 0; } catch { results["n"] = 0; }
  ...
  try { const v = (results["ss"] ?? 0) / ((results["n"] ?? 0) - 1); results["variance"] = Number.isFinite(v) ? v : 0; } catch { results["variance"] = 0; }
  ...
  try { const v = ((results["n"] ?? 0) / (((results["n"] ?? 0) - 1) * ((results["n"] ?? 0) - 2))) * (results["sumCubedDev"] ?? 0); results["skewness"] = Number.isFinite(v) ? v : 0; } catch { results["skewness"] = 0; }
  return results;
}
