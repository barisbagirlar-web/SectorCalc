...
function evaluateAllFormulas(input: Skewness_calculatorInput): Record<string, number> {
  ...
  try { const v = input.dataPoint1 * input.dataPoint2 * input.dataPoint3 * input.dataPoint4; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.dataPoint1 * input.dataPoint2 * input.dataPoint3 * input.dataPoint4 * (input.dataPoint5 * input.dataPoint6 * input.dataPoint7 * input.dataPoint8); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.dataPoint5 * input.dataPoint6 * input.dataPoint7 * input.dataPoint8; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
