...
function evaluateAllFormulas(input: Pythagorean_triples_calculatorInput): Record<string, number> {
  ...
  try { const v = input.m * input.n * input.scaleFactor * input.maxHypotenuse; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.m * input.n * input.scaleFactor * input.maxHypotenuse; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
