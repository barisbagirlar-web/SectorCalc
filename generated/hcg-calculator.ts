...
function evaluateAllFormulas(input: Hcg_calculatorInput): Record<string, number> {
  ...
  try { const v = input.hcg_level / input.initial_hcg; results["hcg_ratio"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["hcg_ratio"] = Number.NaN; }
  try { const v = (input.hcg_level - input.initial_hcg) / input.initial_hcg * 100; results["percent_increase"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["percent_increase"] = Number.NaN; }
  return results;
}
