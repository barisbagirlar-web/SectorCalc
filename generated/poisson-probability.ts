...
function evaluateAllFormulas(input: Poisson_probabilityInput): Record<string, number> {
  ...
  try { const v = input.lambda * input.k * input.kumulatif; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.lambda * input.k * input.kumulatif; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
