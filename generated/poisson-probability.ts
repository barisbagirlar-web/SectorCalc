...
function evaluateAllFormulas(input: Poisson_probabilityInput): Record<string, number> {
  ...
  try { const v = input.lambda * input.k * input.kumulatif; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.lambda * input.k * input.kumulatif; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
