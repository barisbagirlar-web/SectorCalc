...
function evaluateAllFormulas(input: Volumetric_flow_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.velocity * (input.crossSectionArea > 0 ? input.crossSectionArea : (input.innerDiameter > 0 ? Math.PI * (input.innerDiameter / 2000) ** 2 : 0)); results["flowRateFromAreaVelocity"] = Number.isFinite(v) ? v : 0; } catch { results["flowRateFromAreaVelocity"] = 0; }
  try { const v = input.fluidDensity > 0 ? input.massFlowRate / input.fluidDensity : 0; results["flowRateFromMass"] = Number.isFinite(v) ? v : 0; } catch { results["flowRateFromMass"] = 0; }
  return results;
}
...
export function calculateVolumetric_flow_rate_calculator(input: Volumetric_flow_rate_calculatorInput): Volumetric_flow_rate_calculatorOutput {
  ...
  const totalWasteCost = values["flowRateFromAreaVelocity"] ?? 0;
  const breakdown = {
