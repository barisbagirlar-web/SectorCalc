...
function evaluateAllFormulas(input: Dew_point_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.temperature * (input.humidity / 100) * input.a * input.b; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.temperature * (input.humidity / 100) * input.a * input.b; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
