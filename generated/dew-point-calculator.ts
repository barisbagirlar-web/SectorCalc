...
function evaluateAllFormulas(input: Dew_point_calculatorInput): Record<string, number> {
  ...
  try { const v = input.temperature * (input.humidity / 100) * input.a * input.b; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.temperature * (input.humidity / 100) * input.a * input.b; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
