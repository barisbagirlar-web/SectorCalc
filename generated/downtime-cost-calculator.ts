...
function evaluateAllFormulas(input: Downtime_cost_calculatorInput): Record<string, number | string> {
  ...
  try { const v = (input.planned_production_rate / 100) * input.downtime_duration * input.revenue_per_unit * input.direct_labor_cost_per_hour; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = (input.planned_production_rate / 100) * input.downtime_duration * input.revenue_per_unit * input.direct_labor_cost_per_hour * (input.energy_cost_per_hour * (input.scrap_rate_during_downtime / 100) * input.recovery_time_factor); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.energy_cost_per_hour * (input.scrap_rate_during_downtime / 100) * input.recovery_time_factor; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
