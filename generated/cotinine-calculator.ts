...
function evaluateAllFormulas(input: Cotinine_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cotininePlasma * input.slope * input.intercept * input.bodyWeight; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.cotininePlasma * input.slope * input.intercept * input.bodyWeight * (input.nicotinePerCig); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.nicotinePerCig; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
