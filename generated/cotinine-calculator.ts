...
function evaluateAllFormulas(input: Cotinine_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.cotininePlasma * input.slope * input.intercept * input.bodyWeight; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.cotininePlasma * input.slope * input.intercept * input.bodyWeight * (input.nicotinePerCig); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.nicotinePerCig; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
