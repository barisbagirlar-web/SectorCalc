...
function evaluateAllFormulas(input: Stem_and_leaf_plotInput): Record<string, number | string> {
  ...
  try { const v = input.data * input.leafUnit * input.stemUnit * input.sortOrder; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.data * input.leafUnit * input.stemUnit * input.sortOrder; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
