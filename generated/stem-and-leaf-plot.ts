...
function evaluateAllFormulas(input: Stem_and_leaf_plotInput): Record<string, number> {
  ...
  try { const v = input.data * input.leafUnit * input.stemUnit * input.sortOrder; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.data * input.leafUnit * input.stemUnit * input.sortOrder; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
