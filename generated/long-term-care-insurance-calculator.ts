...
function evaluateAllFormulas(input: Long_term_care_insurance_calculatorInput): Record<string, number> {
  ...
  try { const v = input.dailyBenefit * input.benefitPeriodYears * input.eliminationDays * (input.inflationProtection / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.dailyBenefit * input.benefitPeriodYears * input.eliminationDays * (input.inflationProtection / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
