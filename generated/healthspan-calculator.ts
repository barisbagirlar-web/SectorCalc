...
function evaluateAllFormulas(input: Healthspan_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = 80 - input.currentAge; results["baseRemainingYears"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["baseRemainingYears"] = 0; }
  try { const v = (input.dietQualityScore - 50) / 10; results["dietGain"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["dietGain"] = 0; }
  try { const v = input.alcoholUnitsPerWeek * 0.5; results["alcoholLoss"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["alcoholLoss"] = 0; }
  return results;
}
