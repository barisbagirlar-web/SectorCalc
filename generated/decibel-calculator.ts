...
function evaluateAllFormulas(input: Decibel_calculatorInput): Record<string, number> {
  ...
  try { const v = input.powerMeas / input.powerRef; results["powerRatio"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["powerRatio"] = 0; }
  try { const v = input.powerMeas / input.powerRef; results["powerRatio_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["powerRatio_aux"] = 0; }
  return results;
}
