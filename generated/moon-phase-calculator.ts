...
function evaluateAllFormulas(input: Moon_phase_calculatorInput): Record<string, number> {
  ...
  try { const v = input.year * input.month * input.day * input.hour; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.year * input.month * input.day * input.hour; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
