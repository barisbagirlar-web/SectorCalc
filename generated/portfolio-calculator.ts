...
function evaluateAllFormulas(input: Portfolio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialInvestment + input.monthlyContribution * input.years * 12; results["totalContributions"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalContributions"] = Number.NaN; }
  try { const v = input.initialInvestment + input.monthlyContribution * input.years * 12; results["totalContributions_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalContributions_aux"] = Number.NaN; }
  return results;
}
