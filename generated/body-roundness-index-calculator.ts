...
function evaluateAllFormulas(input: Body_roundness_index_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.waist_cm * input.height_cm * input.waist_in * input.height_in; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.waist_cm * input.height_cm * input.waist_in * input.height_in; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
