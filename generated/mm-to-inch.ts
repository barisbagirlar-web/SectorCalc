...
function evaluateAllFormulas(input: Mm_to_inchInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.mm / 25.4; results["inch"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["inch"] = 0; }
  try { const v = input.mm / 25.4; results["inch_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["inch_aux"] = 0; }
  return results;
}
