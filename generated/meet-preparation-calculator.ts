...
function evaluateAllFormulas(input: Meet_preparation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.openerWeight * (input.secondAttemptFactor / 100) * (input.thirdAttemptFactor / 100) * (input.warmUpStartPercent / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.openerWeight * (input.secondAttemptFactor / 100) * (input.thirdAttemptFactor / 100) * (input.warmUpStartPercent / 100) * ((input.warmUpEndPercent / 100) * (input.warmUpStepPercent / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.warmUpEndPercent / 100) * (input.warmUpStepPercent / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
