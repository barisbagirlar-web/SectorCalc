...
function evaluateAllFormulas(input: Decay_constant_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.halfLife) + (input.meanLifetime) + (input.initialQuantity) + (input.finalQuantity) + (input.timeElapsed) + (input.timeUnitConversion)) / 6; results["meanLifetimeBased"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["meanLifetimeBased"] = Number.NaN; }
  try { const v = ((input.halfLife) + (input.meanLifetime) + (input.initialQuantity)) / 3; results["meanLifetimeBased_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["meanLifetimeBased_aux"] = Number.NaN; }
  return results;
}
