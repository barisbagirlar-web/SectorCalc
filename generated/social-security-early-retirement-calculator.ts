...
function evaluateAllFormulas(input: Social_security_early_retirement_calculatorInput): Record<string, number> {
  ...
  try { const v = input.birthYear * input.monthlyBenefitAtFRA; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.birthYear * input.monthlyBenefitAtFRA; results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.birthYear * input.monthlyBenefitAtFRA; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
