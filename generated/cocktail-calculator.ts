...
function evaluateAllFormulas(input: Cocktail_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.numberOfCocktails * input.drinkVolume * input.ingredient1Part * input.ingredient2Part; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.numberOfCocktails * input.drinkVolume * input.ingredient1Part * input.ingredient2Part * (input.ingredient3Part * input.ingredient4Part * (input.wastagePercentage / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.ingredient3Part * input.ingredient4Part * (input.wastagePercentage / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
