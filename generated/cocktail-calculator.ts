...
function evaluateAllFormulas(input: Cocktail_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numberOfCocktails * input.drinkVolume * input.ingredient1Part * input.ingredient2Part; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.numberOfCocktails * input.drinkVolume * input.ingredient1Part * input.ingredient2Part * (input.ingredient3Part * input.ingredient4Part * (input.wastagePercentage / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.ingredient3Part * input.ingredient4Part * (input.wastagePercentage / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
