...
function evaluateAllFormulas(input: Crossfit_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = input.genderFactor * input.metValue * input.weight * (input.duration / 60); results["totalCalories"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalCalories"] = Number.NaN; }
  try { const v = input.genderFactor * input.metValue * input.weight / 60; results["caloriesPerMinute"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["caloriesPerMinute"] = Number.NaN; }
  try { const v = input.genderFactor * input.metValue * input.weight; results["caloriesPerHour"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["caloriesPerHour"] = Number.NaN; }
  return results;
}
