...
function evaluateAllFormulas(input: Transmission_line_calculatorInput): Record<string, number> {
  ...
  try { const v = input.frequency * input.inductance * input.capacitance * input.resistance; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.frequency * input.inductance * input.capacitance * input.resistance * (input.conductance * input.length); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.conductance * input.length; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
