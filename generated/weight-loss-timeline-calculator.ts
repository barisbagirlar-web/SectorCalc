...
function evaluateAllFormulas(input: Weight_loss_timeline_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.initialWeight) * (input.dryWeight) * (input.targetWeight) * (input.dryingRateConstant) * (input.processEfficiency) * (input.setupTime); results["weightLossAmount"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["weightLossAmount"] = Number.NaN; }
  try { const v = (input.initialWeight) * (input.dryWeight) * (input.targetWeight); results["weightLossAmount_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["weightLossAmount_aux"] = Number.NaN; }
  return results;
}
...
export function calculateWeight_loss_timeline_calculator(input: Weight_loss_timeline_calculatorInput): Weight_loss_timeline_calculatorOutput {
  ...
  const totalWasteCost = toNumericFormulaValue(values["weightLossAmount"]);
  const breakdown = {
