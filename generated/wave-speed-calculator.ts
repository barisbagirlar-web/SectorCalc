...
function evaluateAllFormulas(input: Wave_speed_calculatorInput): Record<string, number> {
  ...
  try { const v = input.tension * input.linearDensity * input.length * input.harmonicNumber; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.tension * input.linearDensity * input.length * input.harmonicNumber; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
