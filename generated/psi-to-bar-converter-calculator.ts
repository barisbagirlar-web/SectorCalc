...
function evaluateAllFormulas(input: Psi_to_bar_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.pressure_psi * input.temperature_celsius * input.altitude_meters; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.pressure_psi * input.temperature_celsius * input.altitude_meters; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
