...
function evaluateAllFormulas(input: Loan_payoff_calculatorInput): Record<string, number> {
  ...
  try { const v = input.monthlyPayment + input.extraMonthlyPayment; results["totalMonthly"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalMonthly"] = 0; }
  return results;
}
...
export function calculateLoan_payoff_calculator(input: Loan_payoff_calculatorInput): Loan_payoff_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["totalMonthly"]));
  const breakdown = {
