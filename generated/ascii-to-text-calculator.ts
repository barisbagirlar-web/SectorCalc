...
function evaluateAllFormulas(input: Ascii_to_text_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.ascii1 * input.ascii2 * input.ascii3 * input.ascii4; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.ascii1 * input.ascii2 * input.ascii3 * input.ascii4 * (input.ascii5 * input.ascii6 * input.ascii7 * input.ascii8); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.ascii5 * input.ascii6 * input.ascii7 * input.ascii8; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
