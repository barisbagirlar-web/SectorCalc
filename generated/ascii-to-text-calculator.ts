...
function evaluateAllFormulas(input: Ascii_to_text_calculatorInput): Record<string, number> {
  ...
  try { const v = input.ascii1 * input.ascii2 * input.ascii3 * input.ascii4; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.ascii1 * input.ascii2 * input.ascii3 * input.ascii4 * (input.ascii5 * input.ascii6 * input.ascii7 * input.ascii8); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.ascii5 * input.ascii6 * input.ascii7 * input.ascii8; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
