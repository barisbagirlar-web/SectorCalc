...
function evaluateAllFormulas(input: Mirror_equation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.objectDistance * input.radiusOfCurvature * input.mirrorTypeSign * input.objectHeight; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.objectDistance * input.radiusOfCurvature * input.mirrorTypeSign * input.objectHeight; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
