...
function evaluateAllFormulas(input: Profitability_index_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialInvestment * (input.discountRate / 100) * input.cashFlowYear1 * input.cashFlowYear2; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.initialInvestment * (input.discountRate / 100) * input.cashFlowYear1 * input.cashFlowYear2 * (input.cashFlowYear3 * input.cashFlowYear4 * input.cashFlowYear5); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.cashFlowYear3 * input.cashFlowYear4 * input.cashFlowYear5; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
