...
function evaluateAllFormulas(input: Pkb_calculatorInput): Record<string, number> {
  ...
  try { const v = input.kb * input.ka * input.pka * input.kw; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.kb * input.ka * input.pka * input.kw; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
