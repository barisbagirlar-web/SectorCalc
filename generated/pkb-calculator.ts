...
function evaluateAllFormulas(input: Pkb_calculatorInput): Record<string, number> {
  ...
  try { const v = input.kb * input.ka * input.pka * input.kw; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.kb * input.ka * input.pka * input.kw; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
