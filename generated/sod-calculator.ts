...
function evaluateAllFormulas(input: Sod_calculatorInput): Record<string, number | string> {
  ...
  try { const v = (asFormulaNumber(results["area"])) * input.wasteFactor / 100; results["wasteArea"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["wasteArea"] = 0; }
  try { const v = (asFormulaNumber(results["area"])) + (asFormulaNumber(results["wasteArea"])); results["netArea"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["netArea"] = 0; }
  return results;
}
