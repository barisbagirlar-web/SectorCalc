...
function evaluateAllFormulas(input: Normality_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * input.molecularWeight * input.valency * input.volume; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.weight * input.molecularWeight * input.valency * input.volume; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
