...
function evaluateAllFormulas(input: Implantation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.refX * input.refY * input.distance * input.azimuth; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.refX * input.refY * input.distance * input.azimuth; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
