...
function evaluateAllFormulas(input: Plant_spacing_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bedLength * input.bedWidth * input.plantSpacing * input.rowSpacing; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.bedLength * input.bedWidth * input.plantSpacing * input.rowSpacing * (input.edgeSpacing); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.edgeSpacing; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
