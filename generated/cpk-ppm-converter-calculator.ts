...
function evaluateAllFormulas(input: Cpk_ppm_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cpk * input.sigmaShift * input.sampleSize; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.cpk * input.sigmaShift * input.sampleSize; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
