...
function evaluateAllFormulas(input: Concrete_strength_calculatorInput): Record<string, number> {
  ...
  try { const v = input.waterContent / input.cementContent; results["waterCementRatio"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["waterCementRatio"] = Number.NaN; }
  try { const v = input.constantA / (input.constantB ** (input.waterContent / input.cementContent)); results["compressiveStrength"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["compressiveStrength"] = Number.NaN; }
  return results;
}
