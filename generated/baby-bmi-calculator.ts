...
function evaluateAllFormulas(input: Baby_bmi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight_kg * input.length_cm * input.weight_lb * input.length_in; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.weight_kg * input.length_cm * input.weight_lb * input.length_in; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
