...
function evaluateAllFormulas(input: Melanin_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.eumelaninPercent * input.melanocyteDensity * input.avgMelaninPerCell) / (input.pheomelaninPercent + 1); results["melaninIndex"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["melaninIndex"] = 0; }
  try { const v = (input.eumelaninPercent * input.melanocyteDensity * input.avgMelaninPerCell) / (input.pheomelaninPercent + 1); results["melaninIndex_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["melaninIndex_aux"] = 0; }
  return results;
}
