...
function evaluateAllFormulas(input: Wood_siding_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.wallWidth * input.wallHeight; results["wallArea_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["wallArea_aux"] = 0; }
  return results;
}
