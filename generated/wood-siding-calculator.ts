...
function evaluateAllFormulas(input: Wood_siding_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.wallWidth) * (input.wallHeight) * (input.exposedFace) * (input.boardLength) * (input.wasteFactor) * (input.pricePerBoard); results["wallArea"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["wallArea"] = Number.NaN; }
  try { const v = (input.wallWidth) * (input.wallHeight) * (input.exposedFace); results["wallArea_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["wallArea_aux"] = Number.NaN; }
  return results;
}
