...
function evaluateAllFormulas(input: Sling_angle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.loadWeight * input.numberOfSlingLegs * input.slingAngle * input.safetyFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.loadWeight * input.numberOfSlingLegs * input.slingAngle * input.safetyFactor * (input.dynamicFactor * (input.efficiencyFactor / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.dynamicFactor * (input.efficiencyFactor / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
