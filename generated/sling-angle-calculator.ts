...
function evaluateAllFormulas(input: Sling_angle_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.loadWeight * input.numberOfSlingLegs * input.slingAngle * input.safetyFactor; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.loadWeight * input.numberOfSlingLegs * input.slingAngle * input.safetyFactor * (input.dynamicFactor * (input.efficiencyFactor / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.dynamicFactor * (input.efficiencyFactor / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
