...
function evaluateAllFormulas(input: Gpa_calculatorInput): Record<string, number> {
  ...
  try { const v = input.credit1 * input.grade1 * input.credit2 * input.grade2; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.credit1 * input.grade1 * input.credit2 * input.grade2 * (input.credit3 * input.grade3 * input.credit4 * input.grade4); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.credit3 * input.grade3 * input.credit4 * input.grade4; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
