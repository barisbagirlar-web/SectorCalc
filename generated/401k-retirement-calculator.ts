...
function evaluateAllFormulas(input: _401k_retirement_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentAge * input.currentBalance; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.currentAge * input.currentBalance * (1 + (input.employerMatchRate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.currentAge * input.currentBalance * (1 + (input.employerMatchRate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
