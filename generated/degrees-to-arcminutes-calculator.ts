...
function evaluateAllFormulas(input: Degrees_to_arcminutes_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.deg * 60) + input.min + (input.sec / 60); results["exactTotal"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["exactTotal"] = 0; }
  try { const v = (asFormulaNumber(results["exactTotal"])) * input.coeff + input.offset; results["calibratedTotal"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["calibratedTotal"] = 0; }
  return results;
}
...
export function calculateDegrees_to_arcminutes_calculator(input: Degrees_to_arcminutes_calculatorInput): Degrees_to_arcminutes_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["calibratedTotal"]));
  const breakdown = {
