...
function evaluateAllFormulas(input: Sat_percentile_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.mathScore * input.readingWritingScore * input.meanTotal * input.stdDev; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mathScore * input.readingWritingScore * input.meanTotal * input.stdDev; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
