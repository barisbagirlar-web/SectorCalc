...
function evaluateAllFormulas(input: Sat_percentile_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mathScore * input.readingWritingScore * input.meanTotal * input.stdDev; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.mathScore * input.readingWritingScore * input.meanTotal * input.stdDev; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
