...
function evaluateAllFormulas(input: Baby_height_percentile_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sex * input.ageMonths * input.heightCm * input.L; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.sex * input.ageMonths * input.heightCm * input.L * (input.M * input.S); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.M * input.S; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
