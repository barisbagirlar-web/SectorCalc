...
function evaluateAllFormulas(input: Argand_diagram_calculatorInput): Record<string, number> {
  ...
  try { const v = input.re1 * input.im1 * input.re2 * input.im2; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.re1 * input.im1 * input.re2 * input.im2; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
