...
function evaluateAllFormulas(input: Adc_calculatorInput): Record<string, number> {
  ...
  try { const v = input.resolutionBits * input.referenceVoltage * input.inputVoltage * input.gain; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.resolutionBits * input.referenceVoltage * input.inputVoltage * input.gain; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
