...
function evaluateAllFormulas(input: Adc_calculatorInput): Record<string, number> {
  ...
  try { const v = input.resolutionBits * input.referenceVoltage * input.inputVoltage * input.gain; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.resolutionBits * input.referenceVoltage * input.inputVoltage * input.gain; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
