...
function evaluateAllFormulas(input: Number_base_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.decimalValue * input.targetBase1 * input.targetBase2 * input.targetBase3; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.decimalValue * input.targetBase1 * input.targetBase2 * input.targetBase3 * (input.targetBase4); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.targetBase4; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
