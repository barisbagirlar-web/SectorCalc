...
function evaluateAllFormulas(input: Cure_calculatorInput): Record<string, number> {
  ...
  try { const v = input.curingTemperature * input.curingTimeHours * input.datumTemperature * input.strengthCoefficientA; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.curingTemperature * input.curingTimeHours * input.datumTemperature * input.strengthCoefficientA * (input.strengthCoefficientB); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.strengthCoefficientB; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
