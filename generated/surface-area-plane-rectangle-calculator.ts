...
function evaluateAllFormulas(input: Surface_area_plane_rectangle_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.xMax - input.xMin) * (input.yMax - input.yMin); results["rectangleArea"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["rectangleArea"] = 0; }
  try { const v = (input.xMax - input.xMin) * (input.yMax - input.yMin); results["rectangleArea_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["rectangleArea_aux"] = 0; }
  return results;
}
...
export function calculateSurface_area_plane_rectangle_calculator(input: Surface_area_plane_rectangle_calculatorInput): Surface_area_plane_rectangle_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["rectangleArea_aux"]));
  const breakdown = {
