...
function evaluateAllFormulas(input: Static_pressure_calculatorInput): Record<string, number> {
  ...
  try { const v = input.density * input.gravity * input.height; results["pressure"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["pressure"] = Number.NaN; }
  try { const v = input.atmPressure + (toNumericFormulaValue(results["pressure"])); results["absolutePressure"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["absolutePressure"] = Number.NaN; }
  return results;
}
