...
function evaluateAllFormulas(input: Regular_polygon_area_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numberOfSides * input.sideLength * input.unitScale * input.decimals; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.numberOfSides * input.sideLength * input.unitScale * input.decimals; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
