...
function evaluateAllFormulas(input: Pull_up_calculatorInput): Record<string, number> {
  ...
  try { const v = input.loadWeight * input.inclineAngle * input.frictionCoefficient * input.mechanicalAdvantage; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.loadWeight * input.inclineAngle * input.frictionCoefficient * input.mechanicalAdvantage; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
