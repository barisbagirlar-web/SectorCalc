...
function evaluateAllFormulas(input: Enerji_icecegi_hesaplayici_calculatorInput): Record<string, number> {
  ...
  try { const v = input.servings * input.caffeinePerServing; results["totalCaffeine"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalCaffeine"] = Number.NaN; }
  try { const v = input.servings * input.sugarPerServing; results["totalSugar"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalSugar"] = Number.NaN; }
  try { const v = input.costPerCan / input.servingsPerCan; results["costPerServing"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["costPerServing"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["totalCaffeine"])) / input.costPerCan; results["caffeinePerCost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["caffeinePerCost"] = Number.NaN; }
  return results;
}
