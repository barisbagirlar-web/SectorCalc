...
function evaluateAllFormulas(input: Sample_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.zScore**2 * input.estimatedProportion * (1 - input.estimatedProportion) / ((input.marginOfError / 100) ** 2); results["n0"] = Number.isFinite(v) ? v : 0; } catch { results["n0"] = 0; }
  try { const v = (results["n0"] ?? 0) / (1 + ((results["n0"] ?? 0) - 1) / input.populationSize); results["sampleSize"] = Number.isFinite(v) ? v : 0; } catch { results["sampleSize"] = 0; }
  return results;
}
