...
function evaluateAllFormulas(input: Data_transfer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.dataSize * input.transferSpeed * (input.overheadPercent / 100) * input.numberOfFiles; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.dataSize * input.transferSpeed * (input.overheadPercent / 100) * input.numberOfFiles * (input.latencyPerFile); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.latencyPerFile; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
