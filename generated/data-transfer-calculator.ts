...
function evaluateAllFormulas(input: Data_transfer_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.dataSize * input.transferSpeed * (input.overheadPercent / 100) * input.numberOfFiles; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.dataSize * input.transferSpeed * (input.overheadPercent / 100) * input.numberOfFiles * (input.latencyPerFile); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.latencyPerFile; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
