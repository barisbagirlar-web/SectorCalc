...
function evaluateAllFormulas(input: Half_life_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.initialAmount * (0.5 ** (input.elapsedTime / input.halfLife)); results["remainingAmount"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["remainingAmount"] = 0; }
  ...
  try { const v = 0.5 ** (input.elapsedTime / input.halfLife); results["fractionRemaining"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["fractionRemaining"] = 0; }
  return results;
}
...
export function calculateHalf_life_calculator(input: Half_life_calculatorInput): Half_life_calculatorOutput {
  ...
  const totalWasteCost = toNumericFormulaValue(values["remainingAmount"]);
  const breakdown = {
