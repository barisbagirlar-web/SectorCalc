...
function evaluateAllFormulas(input: Half_life_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialAmount * (0.5 ** (input.elapsedTime / input.halfLife)); results["remainingAmount"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["remainingAmount"] = Number.NaN; }
  ...
  try { const v = 0.5 ** (input.elapsedTime / input.halfLife); results["fractionRemaining"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["fractionRemaining"] = Number.NaN; }
  return results;
}
