...
function evaluateAllFormulas(input: Rca_recurring_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.labor_rate / 100) * input.labor_hours_per_cycle * input.material_cost_per_unit * input.energy_cost_per_cycle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = (input.labor_rate / 100) * input.labor_hours_per_cycle * input.material_cost_per_unit * input.energy_cost_per_cycle * (input.maintenance_cost_per_cycle * (input.overhead_rate / 100) * input.cycle_time_minutes * (input.defect_rate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.maintenance_cost_per_cycle * (input.overhead_rate / 100) * input.cycle_time_minutes * (input.defect_rate / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
