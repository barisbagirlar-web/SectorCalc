...
function evaluateAllFormulas(input: Matrix_rank_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a11*input.a22 - input.a12*input.a21; results["determinant_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["determinant_aux"] = 0; }
  return results;
}
...
export function calculateMatrix_rank_calculator(input: Matrix_rank_calculatorInput): Matrix_rank_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["determinant_aux"]));
  const breakdown = {
