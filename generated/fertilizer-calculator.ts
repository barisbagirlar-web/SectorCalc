...
function evaluateAllFormulas(input: Fertilizer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.area * input.nitrogenRate * input.phosphorusRate * input.potassiumRate; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.area * input.nitrogenRate * input.phosphorusRate * input.potassiumRate * ((input.fertilizerN / 100) * (input.fertilizerP / 100) * (input.fertilizerK / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.fertilizerN / 100) * (input.fertilizerP / 100) * (input.fertilizerK / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
