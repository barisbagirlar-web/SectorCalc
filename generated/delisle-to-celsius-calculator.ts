...
function evaluateAllFormulas(input: Delisle_to_celsius_calculatorInput): Record<string, number> {
  ...
  try { const v = input.delisleTemp * input.freezingDe * input.boilingDe * input.decimals; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.delisleTemp * input.freezingDe * input.boilingDe * input.decimals; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
