...
function evaluateAllFormulas(input: Estate_tax_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.grossEstateValue * input.standardExemption * input.otherDeductions * (input.taxRate / 100); results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.grossEstateValue * input.standardExemption * input.otherDeductions * (input.taxRate / 100) * (input.surchargeThreshold * (input.surchargeRate / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.surchargeThreshold * (input.surchargeRate / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
