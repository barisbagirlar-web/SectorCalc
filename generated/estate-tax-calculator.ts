...
function evaluateAllFormulas(input: Estate_tax_calculatorInput): Record<string, number> {
  ...
  try { const v = input.grossEstateValue * input.standardExemption * input.otherDeductions * (input.taxRate / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.grossEstateValue * input.standardExemption * input.otherDeductions * (input.taxRate / 100) * (input.surchargeThreshold * (input.surchargeRate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.surchargeThreshold * (input.surchargeRate / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
