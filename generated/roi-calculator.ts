...
function evaluateAllFormulas(input: Roi_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.annual_revenue * (input.operating_margin / 100) * input.total_inventory_value * (input.inventory_carrying_cost_percent / 100); results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.annual_revenue * (input.operating_margin / 100) * input.total_inventory_value * (input.inventory_carrying_cost_percent / 100) * (input.annual_labor_cost * (input.defect_rate_percent / 100) * input.cost_per_defect * input.annual_energy_cost); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.annual_labor_cost * (input.defect_rate_percent / 100) * input.cost_per_defect * input.annual_energy_cost; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
