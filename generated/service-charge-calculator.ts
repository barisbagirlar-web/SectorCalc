...
function evaluateAllFormulas(input: Service_charge_calculatorInput): Record<string, number> {
  ...
  try { const v = input.baseAmount * input.serviceChargeRate * input.minServiceCharge * input.partySize; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.baseAmount * input.serviceChargeRate * input.minServiceCharge * input.partySize * (input.partyThreshold); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.partyThreshold; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
