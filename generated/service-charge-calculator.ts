...
function evaluateAllFormulas(input: Service_charge_calculatorInput): Record<string, number> {
  ...
  try { const v = input.baseAmount * input.serviceChargeRate * input.minServiceCharge * input.partySize; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.baseAmount * input.serviceChargeRate * input.minServiceCharge * input.partySize * (input.partyThreshold); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.partyThreshold; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
