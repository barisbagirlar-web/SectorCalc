...
function evaluateAllFormulas(input: Hypothesis_testing_calculatorInput): Record<string, number> {
  ...
  results["____zScore_toFixed_4_"] = 0;
  results["____pValue_toFixed_4_____pValue___alpha_"] = 0;
  try { const v = ((results["pValue"] ?? 0) < input.alpha) ? 'Reject H₀' : 'Fail to reject H₀'; results["result"] = Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
