...
function evaluateAllFormulas(input: Hypothesis_testing_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sampleMean * input.nullMean * input.stdDev * input.sampleSize; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.sampleMean * input.nullMean * input.stdDev * input.sampleSize * (input.alpha * input.tails); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
