...
function evaluateAllFormulas(input: Hypothesis_testing_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.sampleMean * input.nullMean * input.stdDev * input.sampleSize; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.sampleMean * input.nullMean * input.stdDev * input.sampleSize * (input.alpha * input.tails); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
