...
function evaluateAllFormulas(input: Paint_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = input.surfaceArea / input.paintCoverage * input.coats * (1 + input.wasteFactor / 100); results["totalPaintNeeded"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalPaintNeeded"] = 0; }
  try { const v = (asFormulaNumber(results["totalPaintNeeded"])) * input.paintPrice; results["totalPaintCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalPaintCost"] = 0; }
  try { const v = input.laborHours * input.laborRate; results["totalLaborCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalLaborCost"] = 0; }
  try { const v = (asFormulaNumber(results["totalPaintCost"])) + (asFormulaNumber(results["totalLaborCost"])); results["totalCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalCost"] = 0; }
  return results;
}
