...
function evaluateAllFormulas(input: Filament_recycling_cost_comparator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.labor_hours_per_kg; results["annual_exposure_hours"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["annual_exposure_hours"] = 0; }
  try { const v = input.machine_life_years * (input.labor_rate / 100) * input.labor_hours_per_kg * input.recycling_machine_cost; results["direct_labor_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["direct_labor_cost"] = 0; }
  try { const v = (input.machine_life_years * (input.labor_rate / 100) * input.labor_hours_per_kg * input.recycling_machine_cost) + (input.machine_life_years * input.recycling_machine_cost); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.machine_life_years * input.recycling_machine_cost; results["machine_maintenance_annual"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["machine_maintenance_annual"] = 0; }
  try { const v = input.machine_life_years * input.labor_hours_per_kg; results["machine_runtime_hours"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["machine_runtime_hours"] = 0; }
  return results;
}
