...
function evaluateAllFormulas(input: Cloud_waste_elimination_calculatorInput): Record<string, number> {
  ...
  try { const v = input.idle_resource_count * input.total_cloud_spend; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.idle_resource_count * input.total_cloud_spend * (1 + (input.compute_utilization_rate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.idle_resource_count * input.total_cloud_spend * (1 + (input.compute_utilization_rate / 100)) * ((input.storage_waste_percentage / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.storage_waste_percentage / 100); results["factor_storage_waste_percentage"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_storage_waste_percentage"] = Number.NaN; }
  return results;
}
...
export function calculateCloud_waste_elimination_calculator(input: Cloud_waste_elimination_calculatorInput): Cloud_waste_elimination_calculatorOutput {
  ...
  const breakdown = {
    ...
    factor_storage_waste_percentage: toNumericFormulaValue(values["factor_storage_waste_percentage"])
  };
...
export interface Cloud_waste_elimination_calculatorOutput {
  ...
  unit: string;
  breakdown: { base_cost: number; adjusted_cost: number; factor_storage_waste_percentage: number };
...
export const Cloud_waste_elimination_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
  breakdownKeys: ["base_cost","adjusted_cost","factor_storage_waste_percentage"],
} as const;
