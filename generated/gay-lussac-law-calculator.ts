...
function evaluateAllFormulas(input: Gay_lussac_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.p1 === 0 ? (input.p2 * input.t1 / input.t2) : (input.t1 === 0 ? (input.p1 * input.t2 / input.p2) : (input.p2 === 0 ? (input.p1 * input.t2 / input.t1) : (input.p2 * input.t1 / input.p1))); results["calculated_value"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["calculated_value"] = Number.NaN; }
  try { const v = input.p1 === 0 ? (input.p2 * input.t1 / input.t2) : (input.t1 === 0 ? (input.p1 * input.t2 / input.p2) : (input.p2 === 0 ? (input.p1 * input.t2 / input.t1) : (input.p2 * input.t1 / input.p1))); results["calculated_value_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["calculated_value_aux"] = Number.NaN; }
  return results;
}
