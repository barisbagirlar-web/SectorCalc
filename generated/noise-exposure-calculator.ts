...
function evaluateAllFormulas(input: Noise_exposure_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.noiseLevel * input.exposureTime * input.exchangeRate * input.criterionLevel; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.noiseLevel * input.exposureTime * input.exchangeRate * input.criterionLevel * (input.thresholdLevel); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.thresholdLevel; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
