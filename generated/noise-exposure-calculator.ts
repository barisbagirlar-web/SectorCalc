...
function evaluateAllFormulas(input: Noise_exposure_calculatorInput): Record<string, number> {
  ...
  try { const v = input.noiseLevel * input.exposureTime * input.exchangeRate * input.criterionLevel; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.noiseLevel * input.exposureTime * input.exchangeRate * input.criterionLevel * (input.thresholdLevel); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.thresholdLevel; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
