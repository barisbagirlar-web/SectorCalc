...
function evaluateAllFormulas(input: Fluidization_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.particleDensity - input.fluidDensity) * input.gravity; results["c"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["c"] = Number.NaN; }
  try { const v = (input.particleDensity - input.fluidDensity) * (1 - input.voidFraction) * input.gravity * input.bedHeight; results["DeltaP_mf"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["DeltaP_mf"] = Number.NaN; }
  return results;
}
