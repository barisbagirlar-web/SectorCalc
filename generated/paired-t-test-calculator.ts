...
function evaluateAllFormulas(input: Paired_t_test_calculatorInput): Record<string, number> {
  ...
  try { const v = input.meanDifference * input.standardDeviationDifference * input.sampleSize * input.hypothesizedDifference; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.meanDifference * input.standardDeviationDifference * input.sampleSize * input.hypothesizedDifference; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
