...
function evaluateAllFormulas(input: Paired_t_test_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.meanDifference * input.standardDeviationDifference * input.sampleSize * input.hypothesizedDifference; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.meanDifference * input.standardDeviationDifference * input.sampleSize * input.hypothesizedDifference; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
