...
function evaluateAllFormulas(input: Triathlon_calculatorInput): Record<string, number> {
  ...
  try { const v = input.swimDistance * input.bikeDistance * input.runDistance * input.swimPace; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.swimDistance * input.bikeDistance * input.runDistance * input.swimPace * (input.bikeSpeed * input.runPace); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.bikeSpeed * input.runPace; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
