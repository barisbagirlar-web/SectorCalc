...
function evaluateAllFormulas(input: Spray_paint_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.coverage * (1 - input.wasteFactor / 100); results["effectiveCoverage"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["effectiveCoverage"] = 0; }
  try { const v = (input.area * input.coats) / (asFormulaNumber(results["effectiveCoverage"])); results["totalPaintVolume"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalPaintVolume"] = 0; }
  return results;
}
