...
function evaluateAllFormulas(input: Sampling_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bandwidth * 2; results["nyquistRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["nyquistRate"] = Number.NaN; }
  try { const v = input.bandwidth * input.samplingFactor; results["samplingRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["samplingRate"] = Number.NaN; }
  try { const v = input.bandwidth * input.samplingFactor * input.bitDepth * input.channels; results["dataRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["dataRate"] = Number.NaN; }
  return results;
}
