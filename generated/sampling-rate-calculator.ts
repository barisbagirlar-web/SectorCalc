...
function evaluateAllFormulas(input: Sampling_rate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bandwidth * 2; results["nyquistRate"] = Number.isFinite(v) ? v : 0; } catch { results["nyquistRate"] = 0; }
  try { const v = input.bandwidth * input.samplingFactor; results["samplingRate"] = Number.isFinite(v) ? v : 0; } catch { results["samplingRate"] = 0; }
  try { const v = input.bandwidth * input.samplingFactor * input.bitDepth * input.channels; results["dataRate"] = Number.isFinite(v) ? v : 0; } catch { results["dataRate"] = 0; }
  return results;
}
...
export function calculateSampling_rate_calculator(input: Sampling_rate_calculatorInput): Sampling_rate_calculatorOutput {
  ...
  const totalWasteCost = values["samplingRate"] ?? 0;
  const breakdown = {
