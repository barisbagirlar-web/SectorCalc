...
function evaluateAllFormulas(input: Cows_calculatorInput): Record<string, number> {
  ...
  try { const v = input.downtimeHours * input.affectedMachines * input.hourlyCostPerMachine; results["directCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["directCost"] = 0; }
  try { const v = input.downtimeHours * input.affectedMachines * input.lostProductionPerHour * input.profitPerUnit; results["lostProfit"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["lostProfit"] = 0; }
  try { const v = ((asFormulaNumber(results["directCost"])) + (asFormulaNumber(results["lostProfit"]))) * input.overheadMultiplier; results["totalCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalCost"] = 0; }
  return results;
}
