...
function evaluateAllFormulas(input: Euler_formula_calculatorInput): Record<string, number> {
  ...
  try { const v = input.realPart * input.imagPart * input.angleDeg * input.modulus; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.realPart * input.imagPart * input.angleDeg * input.modulus; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
