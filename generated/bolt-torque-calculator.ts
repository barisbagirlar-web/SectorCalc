...
function evaluateAllFormulas(input: Bolt_torque_calculatorInput): Record<string, number> {
  ...
  try { const v = input.diameter * input.pitch * input.yieldStrength * input.safetyFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.diameter * input.pitch * input.yieldStrength * input.safetyFactor * (input.nutFactor); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.nutFactor; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
