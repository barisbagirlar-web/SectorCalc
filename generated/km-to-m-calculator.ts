...
function evaluateAllFormulas(input: Km_to_m_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.inputKm) * (input.conversionFactor) * (input.decimalPrecision) * (input.roundingMethod) * (input.minimumValue) * (input.maximumValue) * (input.scaleFactor); results["rawMeters"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawMeters"] = Number.NaN; }
  try { const v = (input.inputKm) * (input.conversionFactor) * (input.decimalPrecision); results["rawMeters_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawMeters_aux"] = Number.NaN; }
  return results;
}
