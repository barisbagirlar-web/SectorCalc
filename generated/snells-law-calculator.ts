...
function evaluateAllFormulas(input: Snells_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.n1 * input.theta1 * input.n2 * input.theta2; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.n1 * input.theta1 * input.n2 * input.theta2; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
