...
function evaluateAllFormulas(input: Body_surface_area_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight_kg * input.height_cm * input.weight_lb * input.height_in; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.weight_kg * input.height_cm * input.weight_lb * input.height_in; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
