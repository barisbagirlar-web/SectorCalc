...
function evaluateAllFormulas(input: Trapezoidal_rule_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.upperLimit - input.lowerLimit) / input.intervals) / 2 * (input.fa + 2 * input.sumMid + input.fb); results["integralValue"] = Number.isFinite(v) ? v : 0; } catch { results["integralValue"] = 0; }
  try { const v = (input.upperLimit - input.lowerLimit) / input.intervals; results["stepSize"] = Number.isFinite(v) ? v : 0; } catch { results["stepSize"] = 0; }
  try { const v = input.fa + input.fb; results["sumOfEnds"] = Number.isFinite(v) ? v : 0; } catch { results["sumOfEnds"] = 0; }
  try { const v = 2 * input.sumMid; results["sumOfInterior"] = Number.isFinite(v) ? v : 0; } catch { results["sumOfInterior"] = 0; }
  return results;
}
...
export function calculateTrapezoidal_rule_calculator(input: Trapezoidal_rule_calculatorInput): Trapezoidal_rule_calculatorOutput {
  ...
  const totalWasteCost = values["integralValue"] ?? 0;
  const breakdown = {
