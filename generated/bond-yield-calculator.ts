...
function evaluateAllFormulas(input: Bond_yield_calculatorInput): Record<string, number> {
  ...
  try { const v = (asFormulaNumber(results["annualCoupon"])) / input.currentPrice * 100; results["currentYield"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["currentYield"] = 0; }
  try { const v = ((asFormulaNumber(results["annualCoupon"])) + (input.faceValue - input.currentPrice) / input.yearsToMaturity) / ((input.faceValue + input.currentPrice) / 2) * 100; results["approxYTM"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["approxYTM"] = 0; }
  return results;
}
...
export function calculateBond_yield_calculator(input: Bond_yield_calculatorInput): Bond_yield_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["annualCoupon"]));
  const breakdown = {
