...
function evaluateAllFormulas(input: Supply_chain_disruption_risk_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.supplier_reliability_score * input.inventory_buffer_days * input.lead_time_variability * (input.single_source_dependency / 100); results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.supplier_reliability_score * input.inventory_buffer_days * input.lead_time_variability * (input.single_source_dependency / 100) * (input.geopolitical_risk_index * input.demand_volatility * input.transportation_disruption_probability * (input.quality_defect_rate / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.geopolitical_risk_index * input.demand_volatility * input.transportation_disruption_probability * (input.quality_defect_rate / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
