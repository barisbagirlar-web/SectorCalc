...
function evaluateAllFormulas(input: Day_of_year_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.month * input.day * input.year * input.fiscalStartMonth; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.month * input.day * input.year * input.fiscalStartMonth * (input.fiscalStartDay); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.fiscalStartDay; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
