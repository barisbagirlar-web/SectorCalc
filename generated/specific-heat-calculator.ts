...
function evaluateAllFormulas(input: Specific_heat_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.temp_final - input.temp_initial; results["deltaT"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["deltaT"] = 0; }
  try { const v = input.heat / (input.mass * (asFormulaNumber(results["deltaT"]))); results["specificHeat"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["specificHeat"] = 0; }
  return results;
}
