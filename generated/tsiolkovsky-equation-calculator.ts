...
function evaluateAllFormulas(input: Tsiolkovsky_equation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialMass / input.finalMass; results["massRatio_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["massRatio_aux"] = 0; }
  return results;
}
...
export function calculateTsiolkovsky_equation_calculator(input: Tsiolkovsky_equation_calculatorInput): Tsiolkovsky_equation_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["massRatio_aux"]));
  const breakdown = {
