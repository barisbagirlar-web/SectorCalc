...
function evaluateAllFormulas(input: Project_overrun_risk_calculatorInput): Record<string, number> {
  ...
  try { const v = input.qualityDefectRate * input.plannedCost; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.qualityDefectRate * input.plannedCost * (1 + (input.percentComplete / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.qualityDefectRate * input.plannedCost * (1 + (input.percentComplete / 100)) * (input.plannedDuration); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.plannedDuration; results["factor_plannedDuration"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["factor_plannedDuration"] = 0; }
  return results;
}
