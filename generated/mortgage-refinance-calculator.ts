...
function evaluateAllFormulas(input: Mortgage_refinance_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentBalance * input.closingCosts; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.currentBalance * input.closingCosts * (1 + (input.currentRate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.currentBalance * input.closingCosts * (1 + (input.currentRate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
