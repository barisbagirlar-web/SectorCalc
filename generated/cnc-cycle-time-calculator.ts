...
function evaluateAllFormulas(input: Cnc_cycle_time_calculatorInput): Record<string, number> {
  ...
  try { results["cutting_time"] = (input.cutting_length / input.feed_rate) * input.number_of_passes * input.material_hardness_factor * input.tool_wear_factor; } catch { results["cutting_time"] = 0; }
  try { results["rapid_traverse_time"] = input.rapid_traverse_distance / input.rapid_traverse_rate; } catch { results["rapid_traverse_time"] = 0; }
  try { results["tool_change_total_time"] = input.tool_change_time * input.number_of_tool_changes / 60; } catch { results["tool_change_total_time"] = 0; }
  try { results["auxiliary_time"] = (input.part_loading_time + input.coolant_on_time + input.measurement_time) / 60; } catch { results["auxiliary_time"] = 0; }
  try { results["raw_cycle_time"] = (results["cutting_time"] ?? 0) + (results["rapid_traverse_time"] ?? 0) + (results["tool_change_total_time"] ?? 0) + (results["auxiliary_time"] ?? 0); } catch { results["raw_cycle_time"] = 0; }
  try { results["efficiency_adjustment"] = 100 / input.machine_efficiency; } catch { results["efficiency_adjustment"] = 0; }
  try { results["primary_result"] = (results["raw_cycle_time"] ?? 0) * (results["efficiency_adjustment"] ?? 0); } catch { results["primary_result"] = 0; }
  return results;
}
...
export function calculateCnc_cycle_time_calculator(input: Cnc_cycle_time_calculatorInput): Cnc_cycle_time_calculatorOutput {
  ...
  const totalWasteCost = values["total_cycle_time"] ?? 0;
  const breakdown = {
