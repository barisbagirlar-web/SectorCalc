...
function evaluateAllFormulas(input: Kilobytes_to_megabytes_calculatorInput): Record<string, number> {
  ...
  try { const v = input.kilobytes / input.divisor; results["rawMB"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawMB"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["rawMB"])) + input.offset; results["rawWithOffset"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawWithOffset"] = Number.NaN; }
  return results;
}
