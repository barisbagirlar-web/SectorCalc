...
function evaluateAllFormulas(input: Lightweight_cost_savings_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.annual_volume_units * input.material_cost_per_kg; results["base_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.annual_volume_units * input.material_cost_per_kg * (1 + (input.waste_rate_percent / 100)); results["adjusted_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.annual_volume_units * input.material_cost_per_kg * (1 + (input.waste_rate_percent / 100)) * (input.current_weight_kg); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.current_weight_kg; results["factor_current_weight_kg"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["factor_current_weight_kg"] = 0; }
  return results;
}
