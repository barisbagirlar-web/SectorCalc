...
function evaluateAllFormulas(input: Law_of_tangents_verification_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.a - input.b) / (input.a + input.b); results["sideRatio"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["sideRatio"] = 0; }
  try { const v = (input.a - input.b) / (input.a + input.b); results["sideRatio_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["sideRatio_aux"] = 0; }
  return results;
}
...
export function calculateLaw_of_tangents_verification_calculator(input: Law_of_tangents_verification_calculatorInput): Law_of_tangents_verification_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["sideRatio_aux"]));
  const breakdown = {
