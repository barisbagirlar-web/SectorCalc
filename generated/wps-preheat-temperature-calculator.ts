...
function evaluateAllFormulas(input: Wps_preheat_temperature_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.material_thickness + input.carbon_equivalent + input.hydrogen_level; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.material_thickness + input.carbon_equivalent + input.hydrogen_level; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
