...
function evaluateAllFormulas(input: Pounds_to_stones_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.pounds + (input.ounces / 16); results["totalPounds"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalPounds"] = 0; }
  try { const v = (asFormulaNumber(results["totalPounds"])) / input.stoneWeight; results["decimalStones"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["decimalStones"] = 0; }
  return results;
}
