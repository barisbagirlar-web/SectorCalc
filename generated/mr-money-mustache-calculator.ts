...
function evaluateAllFormulas(input: Mr_money_mustache_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualIncome - input.annualExpenses; results["annualSavings"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annualSavings"] = Number.NaN; }
  try { const v = (input.annualIncome - input.annualExpenses) / input.annualIncome; results["savingsRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["savingsRate"] = Number.NaN; }
  try { const v = input.annualExpenses / (input.safeWithdrawalRate / 100); results["futurePortfolioNeeded"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["futurePortfolioNeeded"] = Number.NaN; }
  return results;
}
