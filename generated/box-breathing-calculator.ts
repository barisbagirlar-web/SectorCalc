...
function evaluateAllFormulas(input: Box_breathing_calculatorInput): Record<string, number> {
  ...
  try { const v = input.inhaleTime + input.holdTime1 + input.exhaleTime + input.holdTime2; results["cycleDuration"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["cycleDuration"] = 0; }
  try { const v = (asFormulaNumber(results["cycleDuration"])) * input.numCycles; results["totalDuration"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalDuration"] = 0; }
  try { const v = 60 / (asFormulaNumber(results["cycleDuration"])); results["breathsPerMinute"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["breathsPerMinute"] = 0; }
  return results;
}
