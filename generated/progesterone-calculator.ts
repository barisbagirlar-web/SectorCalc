...
function evaluateAllFormulas(input: Progesterone_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.progesteroneNgPerMl * input.dilutionFactor * 1000) / input.molecularWeight; results["progesterone_nmol_per_L"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["progesterone_nmol_per_L"] = Number.NaN; }
  try { const v = (input.progesteroneNgPerMl * input.dilutionFactor * input.volumeSample) / input.molecularWeight; results["total_progesterone_nmol"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["total_progesterone_nmol"] = Number.NaN; }
  return results;
}
