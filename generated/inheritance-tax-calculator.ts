...
function evaluateAllFormulas(input: Inheritance_tax_calculatorInput): Record<string, number> {
  ...
  try { const v = input.estateValue * input.funeralExpenses * input.debts * input.taxAllowance; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.estateValue * input.funeralExpenses * input.debts * input.taxAllowance * ((input.taxRate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
