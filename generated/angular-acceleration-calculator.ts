...
function evaluateAllFormulas(input: Angular_acceleration_calculatorInput): Record<string, number> {
  ...
  try { const v = input.torque / input.momentOfInertia; results["angularAcceleration"] = Number.isFinite(v) ? v : 0; } catch { results["angularAcceleration"] = 0; }
  try { const v = input.initialAngularVelocity + (input.torque / input.momentOfInertia) * input.time; results["finalAngularVelocity"] = Number.isFinite(v) ? v : 0; } catch { results["finalAngularVelocity"] = 0; }
  return results;
}
...
export function calculateAngular_acceleration_calculator(input: Angular_acceleration_calculatorInput): Angular_acceleration_calculatorOutput {
  ...
  const totalWasteCost = values["angularAcceleration"] ?? 0;
  const breakdown = {
