...
function evaluateAllFormulas(input: Calendar_converterInput): Record<string, number> {
  ...
  try { const v = input.gregorianYear * input.gregorianMonth * input.gregorianDay * input.targetCalendar; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.gregorianYear * input.gregorianMonth * input.gregorianDay * input.targetCalendar; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
