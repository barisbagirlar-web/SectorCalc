...
function evaluateAllFormulas(input: Calendar_converterInput): Record<string, number> {
  ...
  try { const v = input.gregorianYear * input.gregorianMonth * input.gregorianDay * input.targetCalendar; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.gregorianYear * input.gregorianMonth * input.gregorianDay * input.targetCalendar; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
