...
function evaluateAllFormulas(input: Kilograms_to_grams_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.kg * input.items + input.packagingKg) * input.factor; results["totalGrams"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalGrams"] = Number.NaN; }
  try { const v = input.kg * input.items * input.factor; results["itemGrams"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["itemGrams"] = Number.NaN; }
  try { const v = input.packagingKg * input.factor; results["packagingGrams"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["packagingGrams"] = Number.NaN; }
  return results;
}
