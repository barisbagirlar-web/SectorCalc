...
function evaluateAllFormulas(input: Us_mpg_to_uk_mpg_calculatorInput): Record<string, number> {
  ...
  try { const v = input.usMpg * input.conversionFactor; results["rawUkMpg"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawUkMpg"] = Number.NaN; }
  try { const v = input.usMpg * input.conversionFactor; results["rawUkMpg_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawUkMpg_aux"] = Number.NaN; }
  return results;
}
