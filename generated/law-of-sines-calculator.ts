...
function evaluateAllFormulas(input: Law_of_sines_calculatorInput): Record<string, number> {
  ...
  try { const v = 180 - input.angleA - input.angleB; results["angleC"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["angleC"] = 0; }
  try { const v = 180 - input.angleA - input.angleB; results["angleC_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["angleC_aux"] = 0; }
  return results;
}
...
export function calculateLaw_of_sines_calculator(input: Law_of_sines_calculatorInput): Law_of_sines_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["angleC_aux"]));
  const breakdown = {
