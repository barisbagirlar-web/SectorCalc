...
function evaluateAllFormulas(input: Multiply_fractions_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numerator1 * input.denominator1 * input.numerator2 * input.denominator2; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.numerator1 * input.denominator1 * input.numerator2 * input.denominator2; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
