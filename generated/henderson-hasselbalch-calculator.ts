...
function evaluateAllFormulas(input: Henderson_hasselbalch_calculatorInput): Record<string, number> {
  ...
  try { const v = input.conc_base / input.conc_acid; results["ratio"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["ratio"] = 0; }
  try { const v = input.conc_base / input.conc_acid; results["ratio_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["ratio_aux"] = 0; }
  return results;
}
