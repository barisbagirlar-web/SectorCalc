...
function evaluateAllFormulas(input: Retaining_wall_design_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.H + input.gamma + input.phi; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.H + input.gamma + input.phi; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
