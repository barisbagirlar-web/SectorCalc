...
function evaluateAllFormulas(input: Retaining_wall_design_calculatorInput): Record<string, number> {
  ...
  try { const v = input.H * input.gamma * input.phi * input.q; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.H * input.gamma * input.phi * input.q; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
