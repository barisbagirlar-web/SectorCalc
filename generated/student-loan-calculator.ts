...
function evaluateAllFormulas(input: Student_loan_calculatorInput): Record<string, number> {
  ...
  try { const v = input.loanAmount * (1 + (asFormulaNumber(results["monthlyRate"]))) ** input.defermentMonths; results["effectivePrincipal"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectivePrincipal"] = 0; }
  try { const v = input.loanTermYears * 12; results["n"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["n"] = 0; }
  try { const v = (asFormulaNumber(results["effectivePrincipal"])) * (asFormulaNumber(results["monthlyRate"])) * (1 + (asFormulaNumber(results["monthlyRate"]))) ** (asFormulaNumber(results["n"])) / ((1 + (asFormulaNumber(results["monthlyRate"]))) ** (asFormulaNumber(results["n"])) - 1); results["monthlyPayment"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["monthlyPayment"] = 0; }
  try { const v = (asFormulaNumber(results["monthlyPayment"])) * (asFormulaNumber(results["n"])); results["totalPayment"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalPayment"] = 0; }
  try { const v = (asFormulaNumber(results["totalPayment"])) - (asFormulaNumber(results["effectivePrincipal"])); results["totalInterest"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalInterest"] = 0; }
  return results;
}
