...
function evaluateAllFormulas(input: Social_security_calculatorInput): Record<string, number> {
  ...
  try { const v = input.employeeRate * (input.employeeRate / 100) * 1 * input.monthlyGrossSalary; results["direct_labor_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["direct_labor_cost"] = Number.NaN; }
  try { const v = input.employeeRate * (input.employeeRate / 100) * 1 * input.monthlyGrossSalary * input.capAmount; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
