...
function evaluateAllFormulas(input: Day_length_calculatorInput): Record<string, number> {
  ...
  try { const v = input.latitude * input.year * input.month * input.day; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.latitude * input.year * input.month * input.day * (input.utcOffset); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.utcOffset; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
