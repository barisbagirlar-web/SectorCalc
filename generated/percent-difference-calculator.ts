...
function evaluateAllFormulas(input: Percent_difference_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.value1 + input.value2) / 2; results["avg"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["avg"] = 0; }
  try { const v = (input.value1 + input.value2) / 2; results["avg_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["avg_aux"] = 0; }
  return results;
}
...
export function calculatePercent_difference_calculator(input: Percent_difference_calculatorInput): Percent_difference_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["avg_aux"]));
  const breakdown = {
