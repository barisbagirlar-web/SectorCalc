...
function evaluateAllFormulas(input: Deferred_annuity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.deferralYears * input.presentValue; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.deferralYears * input.presentValue * (1 + (input.annualInterestRate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.deferralYears * input.presentValue * (1 + (input.annualInterestRate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
