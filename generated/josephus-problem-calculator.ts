...
function evaluateAllFormulas(input: Josephus_problem_calculatorInput): Record<string, number> {
  ...
  try { const v = input.n * input.auto_input_2 * input.auto_input_3; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.n * input.auto_input_2 * input.auto_input_3; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
