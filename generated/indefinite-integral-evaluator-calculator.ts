...
function evaluateAllFormulas(input: Indefinite_integral_evaluator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a5 * input.a4 * input.a3 * input.a2; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.a5 * input.a4 * input.a3 * input.a2 * (input.a1 * input.a0 * input.x * input.C); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.a1 * input.a0 * input.x * input.C; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
