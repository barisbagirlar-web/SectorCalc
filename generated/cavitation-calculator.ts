...
function evaluateAllFormulas(input: Cavitation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fluidPressure - input.vaporPressure; results["pressureDiff"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["pressureDiff"] = Number.NaN; }
  try { const v = input.fluidPressure - input.vaporPressure; results["pressureDiff_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["pressureDiff_aux"] = Number.NaN; }
  return results;
}
