...
function evaluateAllFormulas(input: Parsecs_to_light_years_calculatorInput): Record<string, number> {
  ...
  try { const v = input.parsecs * input.conversionFactor; results["lightYears"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["lightYears"] = Number.NaN; }
  try { const v = input.parsecs * input.conversionFactor; results["lightYears_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["lightYears_aux"] = Number.NaN; }
  return results;
}
