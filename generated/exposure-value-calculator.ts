...
function evaluateAllFormulas(input: Exposure_value_calculatorInput): Record<string, number> {
  ...
  try { const v = input.aperture * input.shutterSpeed * input.iso * input.measuredEV; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.aperture * input.shutterSpeed * input.iso * input.measuredEV; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
