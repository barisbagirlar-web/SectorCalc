...
function evaluateAllFormulas(input: Snellen_chart_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.testDistance * input.optotypeHeight * input.auto_input_3; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
