...
function evaluateAllFormulas(input: Debye_huckel_calculatorInput): Record<string, number> {
  ...
  try { const v = input.aConstant * input.bConstant * input.ionicStrength * input.chargeCation; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.aConstant * input.bConstant * input.ionicStrength * input.chargeCation * (input.chargeAnion * input.ionSize); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.chargeAnion * input.ionSize; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
