...
function evaluateAllFormulas(input: Business_debt_service_coverage_ratio_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.netOperatingIncome * input.principalRepayments * input.interestPayments * input.leasePayments; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.netOperatingIncome * input.principalRepayments * input.interestPayments * input.leasePayments; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
