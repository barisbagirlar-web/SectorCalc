...
function evaluateAllFormulas(input: Business_debt_service_coverage_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.netOperatingIncome * input.principalRepayments * input.interestPayments * input.leasePayments; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.netOperatingIncome * input.principalRepayments * input.interestPayments * input.leasePayments; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
