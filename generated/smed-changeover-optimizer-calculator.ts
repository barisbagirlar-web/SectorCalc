...
function evaluateAllFormulas(input: Smed_changeover_optimizer_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.current_changeover_time * (input.internal_operations_percentage / 100) * input.setup_team_size * input.waste_motion_score; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.current_changeover_time * (input.internal_operations_percentage / 100) * input.setup_team_size * input.waste_motion_score; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
