...
function evaluateAllFormulas(input: Smed_changeover_optimizer_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.current_changeover_time + input.internal_operations_percentage + input.setup_team_size; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.current_changeover_time + input.internal_operations_percentage + input.setup_team_size; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
