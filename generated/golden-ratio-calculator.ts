...
function evaluateAllFormulas(input: Golden_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalLength * input.longSegment * input.shortSegment * input.tolerance; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.totalLength * input.longSegment * input.shortSegment * input.tolerance; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
