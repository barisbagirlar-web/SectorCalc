...
function evaluateAllFormulas(input: Cycling_ctl_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.dailyTSS; results["steadyStateCTL"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["steadyStateCTL"] = 0; }
  try { const v = input.dailyTSS; results["steadyStateCTL_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["steadyStateCTL_aux"] = 0; }
  return results;
}
