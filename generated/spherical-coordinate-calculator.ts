...
function evaluateAllFormulas(input: Spherical_coordinate_calculatorInput): Record<string, number> {
  ...
  try { const v = input.r * input.theta * input.phi * input.x; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.r * input.theta * input.phi * input.x * (input.y * input.z); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.y * input.z; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
