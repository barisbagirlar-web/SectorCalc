...
function evaluateAllFormulas(input: Business_loan_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.loanAmount * (input.annualRate / 100) * input.termYears * (input.originationFee / 100); results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.loanAmount * (input.annualRate / 100) * input.termYears * (input.originationFee / 100); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
