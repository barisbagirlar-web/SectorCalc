...
function evaluateAllFormulas(input: Business_loan_calculatorInput): Record<string, number> {
  ...
  try { const v = input.loanAmount * (input.annualRate / 100) * input.termYears * (input.originationFee / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.loanAmount * (input.annualRate / 100) * input.termYears * (input.originationFee / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
