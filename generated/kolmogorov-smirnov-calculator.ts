...
function evaluateAllFormulas(input: Kolmogorov_smirnov_calculatorInput): Record<string, number> {
  ...
  try { const v = input.dStat * input.n1 * input.n2 * input.alpha; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.dStat * input.n1 * input.n2 * input.alpha; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
