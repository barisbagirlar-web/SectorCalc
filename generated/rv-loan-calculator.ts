...
function evaluateAllFormulas(input: Rv_loan_calculatorInput): Record<string, number> {
  ...
  try { const v = input.rvPrice * (1 + input.salesTaxRate/100) - input.downPayment - input.tradeInValue; results["amountFinanced"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["amountFinanced"] = Number.NaN; }
  try { const v = input.annualInterestRate / 1200; results["monthlyInterestRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["monthlyInterestRate"] = Number.NaN; }
  return results;
}
