...
function evaluateAllFormulas(input: Inverse_laplace_exponential_sum_calculatorInput): Record<string, number> {
  ...
  try { const v = input.A1 * input.p1 * input.A2 * input.p2; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.A1 * input.p1 * input.A2 * input.p2 * (input.A3 * input.p3 * input.t); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.A3 * input.p3 * input.t; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
