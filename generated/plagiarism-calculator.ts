...
function evaluateAllFormulas(input: Plagiarism_calculatorInput): Record<string, number> {
  ...
  try { const v = input.matchingWords * input.totalWordsDocA * input.totalWordsDocB * input.matchingCitations; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.matchingWords * input.totalWordsDocA * input.totalWordsDocB * input.matchingCitations * (input.totalCitationsA * input.totalCitationsB); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.totalCitationsA * input.totalCitationsB; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
