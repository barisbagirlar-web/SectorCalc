...
function evaluateAllFormulas(input: Normal_distribution_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.x - input.mean) / input.stdDev; results["zScore"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["zScore"] = 0; }
  try { const v = (input.x - input.mean) / input.stdDev; results["zScore_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["zScore_aux"] = 0; }
  return results;
}
