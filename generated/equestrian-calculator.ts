...
function evaluateAllFormulas(input: Equestrian_calculatorInput): Record<string, number> {
  ...
  try { const v = input.load_mass * (input.slope_percent / 100) * input.rolling_resistance * input.desired_speed; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.load_mass * (input.slope_percent / 100) * input.rolling_resistance * input.desired_speed * (input.horse_pull_force * (input.efficiency / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.horse_pull_force * (input.efficiency / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
