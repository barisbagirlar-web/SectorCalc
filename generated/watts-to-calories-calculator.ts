...
function evaluateAllFormulas(input: Watts_to_calories_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.watts * (input.efficiency / 100) * input.time) / input.calFactor; results["totalCalories"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalCalories"] = 0; }
  try { const v = (input.watts * (input.efficiency / 100)) / input.calFactor; results["calPerSec"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["calPerSec"] = 0; }
  try { const v = ((input.watts * (input.efficiency / 100)) * 60) / input.calFactor; results["calPerMin"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["calPerMin"] = 0; }
  try { const v = ((input.watts * (input.efficiency / 100)) * 3600) / input.calFactor; results["calPerHour"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["calPerHour"] = 0; }
  return results;
}
