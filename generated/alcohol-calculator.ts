...
function evaluateAllFormulas(input: Alcohol_calculatorInput): Record<string, number> {
  ...
  try { const v = input.volume * (input.startABV / input.targetABV - 1); results["waterToAdd"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["waterToAdd"] = 0; }
  try { const v = input.volume + (asFormulaNumber(results["waterToAdd"])); results["finalVolume"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["finalVolume"] = 0; }
  return results;
}
...
export function calculateAlcohol_calculator(input: Alcohol_calculatorInput): Alcohol_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["waterToAdd"]));
  const breakdown = {
