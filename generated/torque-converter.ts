...
function evaluateAllFormulas(input: Torque_converterInput): Record<string, number> {
  ...
  try { const v = input.torqueNm * 0.7375621492772655; results["torqueFtLb"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["torqueFtLb"] = Number.NaN; }
  try { const v = input.torqueNm * 0.7375621492772655; results["torqueFtLb_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["torqueFtLb_aux"] = Number.NaN; }
  return results;
}
