...
function evaluateAllFormulas(input: Simple_harmonic_motion_calculatorInput): Record<string, number> {
  ...
  try { const v = input.A * input.f * input.phi * input.t; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.A * input.f * input.phi * input.t * (input.m); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.m; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
