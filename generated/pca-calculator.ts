...
function evaluateAllFormulas(input: Pca_calculatorInput): Record<string, number> {
  ...
  try { const v = input.varianceX * input.varianceY * input.covarianceXY * input.scaling; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.varianceX * input.varianceY * input.covarianceXY * input.scaling; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
