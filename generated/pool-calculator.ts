...
function evaluateAllFormulas(input: Pool_calculatorInput): Record<string, number> {
  ...
  try { const v = input.length * input.width; results["surfaceArea"] = Number.isFinite(v) ? v : 0; } catch { results["surfaceArea"] = 0; }
  try { const v = (results["surfaceArea"] ?? 0) * (results["averageDepth"] ?? 0); results["volume"] = Number.isFinite(v) ? v : 0; } catch { results["volume"] = 0; }
  try { const v = (results["volume"] ?? 0) * 1000; results["volumeLiters"] = Number.isFinite(v) ? v : 0; } catch { results["volumeLiters"] = 0; }
  try { const v = (results["volume"] ?? 0) * 264.172; results["volumeGallons"] = Number.isFinite(v) ? v : 0; } catch { results["volumeGallons"] = 0; }
  return results;
}
