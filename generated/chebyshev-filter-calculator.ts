...
function evaluateAllFormulas(input: Chebyshev_filter_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.stopbandFrequency / input.cutoffFrequency; results["selectivity"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["selectivity"] = 0; }
  try { const v = input.stopbandFrequency / input.cutoffFrequency; results["selectivity_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["selectivity_aux"] = 0; }
  return results;
}
