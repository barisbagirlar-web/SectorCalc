...
function evaluateAllFormulas(input: Material_replacement_cost_comparator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annual_volume * input.current_material_cost; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.annual_volume * input.current_material_cost * (1 + (input.scrap_rate_current / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.annual_volume * input.current_material_cost * (1 + (input.scrap_rate_current / 100)) * (input.alternative_material_cost); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.alternative_material_cost; results["factor_alternative_material_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_alternative_material_cost"] = Number.NaN; }
  return results;
}
