...
function evaluateAllFormulas(input: Cone_surface_area_calculatorInput): Record<string, number> {
  ...
  try { const v = Math.PI * input.radius**2; results["baseArea"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["baseArea"] = 0; }
  try { const v = Math.PI * input.radius**2; results["baseArea_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["baseArea_aux"] = 0; }
  return results;
}
...
export function calculateCone_surface_area_calculator(input: Cone_surface_area_calculatorInput): Cone_surface_area_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["baseArea_aux"]));
  const breakdown = {
