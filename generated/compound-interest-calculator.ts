...
function evaluateAllFormulas(input: Compound_interest_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.principal * input.annualInterestRate * input.timePeriod * input.additionalContribution; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.principal * input.annualInterestRate * input.timePeriod * input.additionalContribution * (input.inflationRate * input.taxRate); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.inflationRate * input.taxRate; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
