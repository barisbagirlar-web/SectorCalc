...
function evaluateAllFormulas(input: Pallet_calculatorInput): Record<string, number> {
  ...
  try { const v = input.itemsPerLayer * input.layersPerPallet; results["itemsPerPalletActual"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["itemsPerPalletActual"] = Number.NaN; }
  try { const v = input.totalItems * input.itemWeight; results["totalWeight"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalWeight"] = Number.NaN; }
  return results;
}
