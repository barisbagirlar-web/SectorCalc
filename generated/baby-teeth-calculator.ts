...
function evaluateAllFormulas(input: Baby_teeth_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.geneticFactor - 1; results["geneticModifier"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["geneticModifier"] = 0; }
  try { const v = (input.calciumIntake - 500) / 500; results["calciumModifier"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["calciumModifier"] = 0; }
  return results;
}
