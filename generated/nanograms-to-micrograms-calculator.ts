...
function evaluateAllFormulas(input: Nanograms_to_micrograms_calculatorInput): Record<string, number> {
  ...
  try { const v = input.nanograms * input.batchSize; results["totalNanograms"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalNanograms"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["totalNanograms"])) / input.conversionFactor; results["rawMicrograms"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawMicrograms"] = Number.NaN; }
  return results;
}
