...
function evaluateAllFormulas(input: Atl_ctl_tsb_calculatorInput): Record<string, number> {
  ...
  try { const v = input.t1 * input.t2 * input.t3 * input.t4; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.t1 * input.t2 * input.t3 * input.t4 * (input.nominal); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.nominal; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
