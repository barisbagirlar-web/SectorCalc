...
function evaluateAllFormulas(input: Plateau_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialOutput * input.outputAfterPeriod * input.timePeriod * input.learningRate; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.initialOutput * input.outputAfterPeriod * input.timePeriod * input.learningRate; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
