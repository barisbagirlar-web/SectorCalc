...
function evaluateAllFormulas(input: Vector_calculatorInput): Record<string, number> {
  ...
  try { const v = input.v1x*input.v2x + input.v1y*input.v2y + input.v1z*input.v2z; results["dotProduct"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dotProduct"] = 0; }
  try { const v = input.v1x*input.v2x + input.v1y*input.v2y + input.v1z*input.v2z; results["dotProduct_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dotProduct_aux"] = 0; }
  return results;
}
...
export function calculateVector_calculator(input: Vector_calculatorInput): Vector_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["dotProduct_aux"]));
  const breakdown = {
