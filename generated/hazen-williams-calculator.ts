...
function evaluateAllFormulas(input: Hazen_williams_calculatorInput): Record<string, number> {
  ...
  try { const v = input.flowRate / (Math.PI * (input.pipeDiameter / 2) ** 2); results["velocity"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["velocity"] = Number.NaN; }
  try { const v = (10.67 * input.pipeLength * input.flowRate ** 1.852) / (input.cFactor ** 1.852 * input.pipeDiameter ** 4.87); results["headLoss"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["headLoss"] = Number.NaN; }
  try { const v = (10.67 * input.flowRate ** 1.852) / (input.cFactor ** 1.852 * input.pipeDiameter ** 4.87); results["headLossPerUnitLength"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["headLossPerUnitLength"] = Number.NaN; }
  return results;
}
