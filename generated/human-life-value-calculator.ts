...
function evaluateAllFormulas(input: Human_life_value_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualIncome * (1 - input.expenseRatio); results["netAnnualContribution"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["netAnnualContribution"] = Number.NaN; }
  try { const v = (1 + input.rateOfReturn) / (1 + input.inflationRate) - 1; results["realRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["realRate"] = Number.NaN; }
  return results;
}
