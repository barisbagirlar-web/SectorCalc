...
function evaluateAllFormulas(input: Length_contraction_calculatorInput): Record<string, number> {
  ...
  try { const v = input.relativeVelocity / input.speedOfLight; results["speedFraction"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["speedFraction"] = Number.NaN; }
  try { const v = input.relativeVelocity / input.speedOfLight; results["speedFraction_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["speedFraction_aux"] = Number.NaN; }
  return results;
}
