...
function evaluateAllFormulas(input: Absolute_humidity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.temperature * (input.relativeHumidity / 100) * input.molarMassWater * input.gasConstant; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.temperature * (input.relativeHumidity / 100) * input.molarMassWater * input.gasConstant; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
