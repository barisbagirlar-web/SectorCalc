...
function evaluateAllFormulas(input: Miles_per_gallon_to_km_per_literInput): Record<string, number> {
  ...
  try { const v = input.mpg * 0.425143707 * input.fuelType * input.temperatureCorrection * input.altitudeCorrection; results["kmPerLiter"] = Number.isFinite(v) ? v : 0; } catch { results["kmPerLiter"] = 0; }
  try { const v = 235.214583 / (input.mpg * input.fuelType * input.temperatureCorrection * input.altitudeCorrection); results["litersPer100km"] = Number.isFinite(v) ? v : 0; } catch { results["litersPer100km"] = 0; }
  try { const v = 100 / input.mpg; results["gallonsPer100Miles"] = Number.isFinite(v) ? v : 0; } catch { results["gallonsPer100Miles"] = 0; }
  return results;
}
...
export function calculateMiles_per_gallon_to_km_per_liter(input: Miles_per_gallon_to_km_per_literInput): Miles_per_gallon_to_km_per_literOutput {
  ...
  const totalWasteCost = values["kmPerLiter"] ?? 0;
  const breakdown = {
