...
function evaluateAllFormulas(input: Miles_per_gallon_to_km_per_literInput): Record<string, number> {
  ...
  try { const v = input.mpg * 0.425143707 * input.fuelType * input.temperatureCorrection * input.altitudeCorrection; results["kmPerLiter"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["kmPerLiter"] = Number.NaN; }
  try { const v = 235.214583 / (input.mpg * input.fuelType * input.temperatureCorrection * input.altitudeCorrection); results["litersPer100km"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["litersPer100km"] = Number.NaN; }
  try { const v = 100 / input.mpg; results["gallonsPer100Miles"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["gallonsPer100Miles"] = Number.NaN; }
  return results;
}
