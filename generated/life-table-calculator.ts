...
function evaluateAllFormulas(input: Life_table_calculatorInput): Record<string, number> {
  ...
  try { const v = input.l0 * input.B * input.c * input.age; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.l0 * input.B * input.c * input.age; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
