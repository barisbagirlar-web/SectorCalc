...
function evaluateAllFormulas(input: Marathon_time_predictor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.recentDistance * input.recentTime * input.marathonDistance * input.exponent; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.recentDistance * input.recentTime * input.marathonDistance * input.exponent; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
