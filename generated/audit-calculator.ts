...
function evaluateAllFormulas(input: Audit_calculatorInput): Record<string, number> {
  ...
  try { const v = input.populationSize * input.zScore * (input.marginOfError / 100) * input.proportion; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.populationSize * input.zScore * (input.marginOfError / 100) * input.proportion * ((input.responseRate / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.responseRate / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
