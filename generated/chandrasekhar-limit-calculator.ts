...
function evaluateAllFormulas(input: Chandrasekhar_limit_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.mu_e * input.m_H * input.hbar * input.c; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mu_e * input.m_H * input.hbar * input.c * (input.G * input.solar_mass); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.G * input.solar_mass; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
