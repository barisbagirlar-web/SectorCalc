...
function evaluateAllFormulas(input: Weld_strength_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.base_material_yield_strength * input.weld_material_tensile_strength * input.throat_thickness * input.weld_length; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.base_material_yield_strength * input.weld_material_tensile_strength * input.throat_thickness * input.weld_length * (input.applied_load * input.load_angle); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.applied_load * input.load_angle; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
