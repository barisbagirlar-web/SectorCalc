...
function evaluateAllFormulas(input: Sign_test_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.sampleSize) * (input.posCount) * (input.p0) * (input.alpha) * (input.testType); results["expected"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["expected"] = Number.NaN; }
  try { const v = (input.sampleSize) * (input.posCount) * (input.p0); results["expected_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["expected_aux"] = Number.NaN; }
  return results;
}
