...
function evaluateAllFormulas(input: Horizontal_center_of_gravity_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.weight1 + input.weight2 + input.weight3); results["totalWeight"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalWeight"] = 0; }
  try { const v = (input.weight1 * input.distance1 + input.weight2 * input.distance2 + input.weight3 * input.distance3); results["totalMoment"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalMoment"] = 0; }
  try { const v = ((input.weight1 * input.distance1 + input.weight2 * input.distance2 + input.weight3 * input.distance3) / (input.weight1 + input.weight2 + input.weight3)); results["cgHorizontal"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["cgHorizontal"] = 0; }
  return results;
}
...
export function calculateHorizontal_center_of_gravity_calculator(input: Horizontal_center_of_gravity_calculatorInput): Horizontal_center_of_gravity_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["cgHorizontal"]));
  const breakdown = {
