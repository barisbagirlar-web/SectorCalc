...
function evaluateAllFormulas(input: Insulation_r_value_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.thickness1 + input.conductivity1 + input.thickness2; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.thickness1 + input.conductivity1 + input.thickness2; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
