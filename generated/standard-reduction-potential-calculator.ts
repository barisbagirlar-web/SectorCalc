...
function evaluateAllFormulas(input: Standard_reduction_potential_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.E0) * (input.T) * (input.n) * (input.Ox) * (input.Red) * (input.a) * (input.b); results["RT_nF"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["RT_nF"] = Number.NaN; }
  try { const v = (input.E0) * (input.T) * (input.n); results["RT_nF_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["RT_nF_aux"] = Number.NaN; }
  return results;
}
