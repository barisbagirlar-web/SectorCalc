...
function evaluateAllFormulas(input: Smith_chart_calculatorInput): Record<string, number> {
  ...
  try { const v = input.loadResistance / input.characteristicImpedance; results["r"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["r"] = 0; }
  try { const v = input.loadReactance / input.characteristicImpedance; results["x"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["x"] = 0; }
  return results;
}
