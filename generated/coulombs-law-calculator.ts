...
function evaluateAllFormulas(input: Coulombs_law_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.charge1 * input.charge2) / (4 * Math.PI * input.permittivity * input.distance * input.distance)); results["force"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["force"] = Number.NaN; }
  try { const v = ((input.charge1 * input.charge2) / (4 * Math.PI * input.permittivity * input.distance * input.distance)); results["force_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["force_aux"] = Number.NaN; }
  return results;
}
