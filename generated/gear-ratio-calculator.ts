...
function evaluateAllFormulas(input: Gear_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = input.drivenTeeth / input.drivingTeeth; results["ratio"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["ratio"] = Number.NaN; }
  try { const v = input.inputSpeed / (input.drivenTeeth / input.drivingTeeth); results["outputSpeed"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["outputSpeed"] = Number.NaN; }
  return results;
}
