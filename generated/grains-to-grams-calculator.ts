...
function evaluateAllFormulas(input: Grains_to_grams_calculatorInput): Record<string, number> {
  ...
  try { const v = input.conversionFactor; results["gramsPerGrain"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["gramsPerGrain"] = Number.NaN; }
  try { const v = input.grains * input.batchCount; results["totalGrains"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalGrains"] = Number.NaN; }
  try { const v = input.grains * input.conversionFactor * input.batchCount; results["unroundedGrams"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["unroundedGrams"] = Number.NaN; }
  return results;
}
