...
function evaluateAllFormulas(input: Training_load_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.avgHeartRate - input.restingHeartRate) / (input.maxHeartRate - input.restingHeartRate); results["hrRatio"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["hrRatio"] = Number.NaN; }
  try { const v = (input.avgHeartRate - input.restingHeartRate) / (input.maxHeartRate - input.restingHeartRate); results["hrRatio_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["hrRatio_aux"] = Number.NaN; }
  return results;
}
