...
function evaluateAllFormulas(input: Torus_volume_calculatorInput): Record<string, number> {
  ...
  try { const v = input.majorRadius * input.minorRadius * input.density * input.precision; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.majorRadius * input.minorRadius * input.density * input.precision; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
