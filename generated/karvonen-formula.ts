...
function evaluateAllFormulas(input: Karvonen_formulaInput): Record<string, number | string> {
  ...
  try { const v = input.max_hr - input.resting_hr; results["heartRateReserve_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["heartRateReserve_aux"] = 0; }
  return results;
}
