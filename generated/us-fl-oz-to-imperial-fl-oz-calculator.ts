...
function evaluateAllFormulas(input: Us_fl_oz_to_imperial_fl_oz_calculatorInput): Record<string, number> {
  ...
  try { const v = input.usFlOz * input.batchSize * input.packageCount * (input.tolerance / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.usFlOz * input.batchSize * input.packageCount * (input.tolerance / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
