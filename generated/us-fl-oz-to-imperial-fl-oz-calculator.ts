...
function evaluateAllFormulas(input: Us_fl_oz_to_imperial_fl_oz_calculatorInput): Record<string, number> {
  ...
  try { const v = input.usFlOz * input.batchSize * input.packageCount * (input.tolerance / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.usFlOz * input.batchSize * input.packageCount * (input.tolerance / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
