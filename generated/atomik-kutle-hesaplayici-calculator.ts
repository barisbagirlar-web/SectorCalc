...
function evaluateAllFormulas(input: Atomik_kutle_hesaplayici_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.mass1 * input.abundance1) + (input.mass2 * input.abundance2) + (input.mass3 * input.abundance3) + (input.mass4 * input.abundance4); results["weightedSum"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["weightedSum"] = 0; }
  try { const v = input.abundance1 + input.abundance2 + input.abundance3 + input.abundance4; results["totalAbundance"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalAbundance"] = 0; }
  try { const v = ((input.mass1 * input.abundance1) + (input.mass2 * input.abundance2) + (input.mass3 * input.abundance3) + (input.mass4 * input.abundance4)) / (input.abundance1 + input.abundance2 + input.abundance3 + input.abundance4); results["atomicMass"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["atomicMass"] = 0; }
  return results;
}
