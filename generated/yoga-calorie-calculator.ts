...
function evaluateAllFormulas(input: Yoga_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = (asFormulaNumber(results["bmr"])) * (input.duration / 1440); results["restingDuring"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["restingDuring"] = 0; }
  try { const v = (asFormulaNumber(results["totalCalories"])) - (asFormulaNumber(results["restingDuring"])); results["netCalories"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["netCalories"] = 0; }
  return results;
}
...
export function calculateYoga_calorie_calculator(input: Yoga_calorie_calculatorInput): Yoga_calorie_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["netCalories"]));
  const breakdown = {
