...
function evaluateAllFormulas(input: Flash_guide_number_calculatorInput): Record<string, number> {
  ...
  try { const v = input.distance * input.aperture; results["effectiveGN"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["effectiveGN"] = Number.NaN; }
  try { const v = input.distance * input.aperture; results["effectiveGN_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["effectiveGN_aux"] = Number.NaN; }
  return results;
}
