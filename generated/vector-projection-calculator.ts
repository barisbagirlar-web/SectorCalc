...
function evaluateAllFormulas(input: Vector_projection_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a_x * input.b_x + input.a_y * input.b_y + input.a_z * input.b_z; results["dotProduct_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dotProduct_aux"] = 0; }
  return results;
}
