...
function evaluateAllFormulas(input: Modular_arithmetic_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.a + input.b) % input.modulus + input.modulus) % input.modulus; results["sum"] = Number.isFinite(v) ? v : 0; } catch { results["sum"] = 0; }
  try { const v = ((input.a - input.b) % input.modulus + input.modulus) % input.modulus; results["difference"] = Number.isFinite(v) ? v : 0; } catch { results["difference"] = 0; }
  try { const v = ((input.a * input.b) % input.modulus + input.modulus) % input.modulus; results["product"] = Number.isFinite(v) ? v : 0; } catch { results["product"] = 0; }
  try { const v = ((input.a ** input.exponent) % input.modulus + input.modulus) % input.modulus; results["power"] = Number.isFinite(v) ? v : 0; } catch { results["power"] = 0; }
  return results;
}
