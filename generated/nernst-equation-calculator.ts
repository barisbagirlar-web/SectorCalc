...
function evaluateAllFormulas(input: Nernst_equation_calculatorInput): Record<string, number> {
  ...
  try { const v = 8.314462618 * input.temp / (input.n * 96485.33212); results["RT_over_nF"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["RT_over_nF"] = Number.NaN; }
  try { const v = input.C_ox / input.C_red; results["Q"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["Q"] = Number.NaN; }
  return results;
}
