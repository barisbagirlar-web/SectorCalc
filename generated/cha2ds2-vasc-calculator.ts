...
function evaluateAllFormulas(input: Cha2ds2_vasc_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.chf + input.hypertension + input.age75 * 2 + input.diabetes + input.stroke * 2 + input.vascular + input.age65to74 + input.sexFemale; results["score"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["score"] = 0; }
  try { const v = input.chf + input.hypertension + input.age75 * 2 + input.diabetes + input.stroke * 2 + input.vascular + input.age65to74 + input.sexFemale; results["score_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["score_aux"] = 0; }
  return results;
}
