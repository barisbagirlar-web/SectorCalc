...
function evaluateAllFormulas(input: Turkey_installment_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.interestRate / 100 / 12) * (1 + input.bsmv/100 + input.kkdf/100); results["effectiveMonthlyRate"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectiveMonthlyRate"] = 0; }
  try { const v = (input.interestRate / 100 / 12) * (1 + input.bsmv/100 + input.kkdf/100); results["effectiveMonthlyRate_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["effectiveMonthlyRate_aux"] = 0; }
  return results;
}
...
export function calculateTurkey_installment_calculator(input: Turkey_installment_calculatorInput): Turkey_installment_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["effectiveMonthlyRate_aux"]));
  const breakdown = {
