...
function evaluateAllFormulas(input: Hess_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.deltaH1 * input.deltaH2 * input.deltaH3 * input.deltaH4; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.deltaH1 * input.deltaH2 * input.deltaH3 * input.deltaH4 * (input.deltaH5 * input.deltaH6); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.deltaH5 * input.deltaH6; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
