...
function evaluateAllFormulas(input: Determinant_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a * input.d - input.b * input.c; results["det"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["det"] = 0; }
  try { const v = input.a * input.d - input.b * input.c; results["det_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["det_aux"] = 0; }
  return results;
}
