...
function evaluateAllFormulas(input: Travel_insurance_calculatorInput): Record<string, number> {
  ...
  try { const v = input.tripDurationDays * baseDailyRate * (results["riskFactor"] ?? 0) * (results["ageFactor"] ?? 0) * input.numberOfTravelers; results["premiumBeforeDeductible"] = Number.isFinite(v) ? v : 0; } catch { results["premiumBeforeDeductible"] = 0; }
  try { const v = input.deductible * 0.01; results["deductibleDiscount"] = Number.isFinite(v) ? v : 0; } catch { results["deductibleDiscount"] = 0; }
  try { const v = baseDailyRate * (results["riskFactor"] ?? 0) * (results["ageFactor"] ?? 0) * input.numberOfTravelers; results["dailyCost"] = Number.isFinite(v) ? v : 0; } catch { results["dailyCost"] = 0; }
