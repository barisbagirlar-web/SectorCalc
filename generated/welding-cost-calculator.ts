...
function evaluateAllFormulas(input: Welding_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weld_length_mm * input.weld_throat_mm * input.material_density_g_per_cm3 * (input.deposition_efficiency / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.weld_length_mm * input.weld_throat_mm * input.material_density_g_per_cm3 * (input.deposition_efficiency / 100) * ((input.labor_rate_per_hour / 100) * input.welding_speed_mm_per_min * input.filler_wire_cost_per_kg * input.gas_cost_per_liter); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.labor_rate_per_hour / 100) * input.welding_speed_mm_per_min * input.filler_wire_cost_per_kg * input.gas_cost_per_liter; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
...
export const Welding_cost_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
