...
function evaluateAllFormulas(input: Calibrated_airspeed_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.totalPressure * input.staticPressure * input.referencePressure * input.airDensity; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.totalPressure * input.staticPressure * input.referencePressure * input.airDensity * (input.gamma); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.gamma; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
