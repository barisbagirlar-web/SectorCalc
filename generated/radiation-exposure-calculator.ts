...
function evaluateAllFormulas(input: Radiation_exposure_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.sourceActivity * input.gammaConstant) / (input.distance ** 2); results["exposureRate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["exposureRate"] = Number.NaN; }
  try { const v = (input.sourceActivity * input.gammaConstant) / (input.distance ** 2); results["exposureRate_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["exposureRate_aux"] = Number.NaN; }
  return results;
}
