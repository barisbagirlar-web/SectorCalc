...
function evaluateAllFormulas(input: Lorentz_factor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.vx * input.vy * input.vz * input.c; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.vx * input.vy * input.vz * input.c; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
