...
function evaluateAllFormulas(input: Lorentz_factor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.vx * input.vy * input.vz * input.c; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.vx * input.vy * input.vz * input.c; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
