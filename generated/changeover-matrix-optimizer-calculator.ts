...
function evaluateAllFormulas(input: Changeover_matrix_optimizer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.changeover_time_matrix * input.number_of_changeovers_per_month * input.setup_external_time * input.setup_internal_time; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.changeover_time_matrix * input.number_of_changeovers_per_month * input.setup_external_time * input.setup_internal_time * (input.product_family_count * input.standard_deviation_changeover_time); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.product_family_count * input.standard_deviation_changeover_time; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
