...
function evaluateAllFormulas(input: Overhead_press_calculatorInput): Record<string, number> {
  ...
  try { const v = input.boreDiameter * input.pressure * (input.efficiency / 100) * input.safetyFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.boreDiameter * input.pressure * (input.efficiency / 100) * input.safetyFactor; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
