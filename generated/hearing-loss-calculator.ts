...
function evaluateAllFormulas(input: Hearing_loss_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.noise_level + input.exposure_hours + input.years_exposed; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.noise_level + input.exposure_hours + input.years_exposed; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
