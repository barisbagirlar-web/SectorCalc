...
function evaluateAllFormulas(input: Experience_calculatorInput): Record<string, number> {
  ...
  try { const v = input.firstUnitTime * (input.learningRate / 100) * input.cumulativeUnits; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.firstUnitTime * (input.learningRate / 100) * input.cumulativeUnits; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
