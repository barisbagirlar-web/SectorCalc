...
function evaluateAllFormulas(input: Fahrenheit_to_kelvin_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fahrenheit - input.subtractOffset; results["celsiusDifference"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["celsiusDifference"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["celsiusDifference"])) * input.multiplyFactor; results["kelvinDifference"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["kelvinDifference"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["kelvinDifference"])) + input.addOffset; results["kelvin"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["kelvin"] = Number.NaN; }
  return results;
}
