...
function evaluateAllFormulas(input: Circumference_calculatorInput): Record<string, number> {
  ...
  try { const v = input.radius * input.diameter * input.area * input.conversionFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.radius * input.diameter * input.area * input.conversionFactor * (input.decimalPlaces); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
