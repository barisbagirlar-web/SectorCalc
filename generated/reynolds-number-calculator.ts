...
function evaluateAllFormulas(input: Reynolds_number_calculatorInput): Record<string, number> {
  ...
  try { const v = input.density * input.dynamic_viscosity * input.velocity * input.pipe_diameter; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.density * input.dynamic_viscosity * input.velocity * input.pipe_diameter * (input.pipe_roughness * input.temperature); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.pipe_roughness * input.temperature; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
