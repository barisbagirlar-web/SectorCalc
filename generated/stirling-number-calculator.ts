...
function evaluateAllFormulas(input: Stirling_number_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.n * input.k * input.type; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
