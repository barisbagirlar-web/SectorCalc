...
function evaluateAllFormulas(input: Stirling_number_calculatorInput): Record<string, number> {
  ...
  try { const v = input.n * input.k * input.type; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.n * input.k * input.type; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
