...
function evaluateAllFormulas(input: Slope_intercept_form_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.slope * input.xValue + input.yIntercept; results["y_calc"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["y_calc"] = 0; }
  try { const v = (input.yValue - input.yIntercept) / input.slope; results["x_calc"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["x_calc"] = 0; }
  return results;
}
