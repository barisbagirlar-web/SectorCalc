...
function evaluateAllFormulas(input: Milliseconds_to_microseconds_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.milliseconds * 1000 + input.calibrationOffset) * input.safetyFactor; results["rawMicroseconds"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawMicroseconds"] = Number.NaN; }
  try { const v = (input.milliseconds * 1000 + input.calibrationOffset) * input.safetyFactor; results["rawMicroseconds_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawMicroseconds_aux"] = Number.NaN; }
  return results;
}
