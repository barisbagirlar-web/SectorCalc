...
function evaluateAllFormulas(input: Butt_weld_calculatorInput): Record<string, number> {
  ...
  try { const v = input.plateThickness * input.rootGap * input.includedAngle * input.weldLength; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.plateThickness * input.rootGap * input.includedAngle * input.weldLength * (input.metalDensity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.metalDensity; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
