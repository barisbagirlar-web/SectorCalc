...
function evaluateAllFormulas(input: Butt_weld_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.plateThickness * input.rootGap * input.includedAngle * input.weldLength; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.plateThickness * input.rootGap * input.includedAngle * input.weldLength * (input.metalDensity); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.metalDensity; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
