...
function evaluateAllFormulas(input: Coefficient_of_variation_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.measurement1 + input.measurement2 + input.measurement3 + input.measurement4) / 4; results["mean"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["mean"] = Number.NaN; }
  try { const v = (input.measurement1 + input.measurement2 + input.measurement3 + input.measurement4) / 4; results["mean_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["mean_aux"] = Number.NaN; }
  return results;
}
