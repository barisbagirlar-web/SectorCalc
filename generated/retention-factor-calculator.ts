...
function evaluateAllFormulas(input: Retention_factor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.feedConc * input.permeateConc * input.feedFlow * input.permeateFlow; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.feedConc * input.permeateConc * input.feedFlow * input.permeateFlow; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
