...
function evaluateAllFormulas(input: Mtbf_mttr_financial_calculatorInput): Record<string, number> {
  ...
  try { const v = input.operating_hours_per_year; results["annual_exposure_hours"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annual_exposure_hours"] = Number.NaN; }
  try { const v = input.number_of_units * (input.maintenance_labor_rate / 100) * input.operating_hours_per_year * input.cost_per_failure_event; results["direct_labor_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["direct_labor_cost"] = Number.NaN; }
  try { const v = input.number_of_units * (input.maintenance_labor_rate / 100) * input.operating_hours_per_year * input.cost_per_failure_event * input.reliability_improvement_investment; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
