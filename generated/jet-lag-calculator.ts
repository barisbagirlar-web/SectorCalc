...
function evaluateAllFormulas(input: Jet_lag_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.timeZonesCrossed * input.direction * input.flightDuration * input.preTravelRest; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.timeZonesCrossed * input.direction * input.flightDuration * input.preTravelRest * (input.age); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.age; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
