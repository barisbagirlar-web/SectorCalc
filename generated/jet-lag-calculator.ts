...
function evaluateAllFormulas(input: Jet_lag_calculatorInput): Record<string, number> {
  ...
  try { const v = input.timeZonesCrossed * input.direction * input.flightDuration * input.preTravelRest; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.timeZonesCrossed * input.direction * input.flightDuration * input.preTravelRest * (input.age); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.age; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
