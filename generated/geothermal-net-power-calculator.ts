...
function evaluateAllFormulas(input: Geothermal_net_power_calculatorInput): Record<string, number> {
  ...
  try { const v = input.massFlowRate * input.specificHeat * (input.inletTemp - input.outletTemp) * (input.conversionEfficiency / 100) / 1000; results["grossPower"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["grossPower"] = 0; }
  try { const v = (asFormulaNumber(results["grossPower"])) - input.parasiticLoad; results["netPower"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["netPower"] = 0; }
  return results;
}
...
export function calculateGeothermal_net_power_calculator(input: Geothermal_net_power_calculatorInput): Geothermal_net_power_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["netPower"]));
  const breakdown = {
