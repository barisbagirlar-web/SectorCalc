...
function evaluateAllFormulas(input: Shipping_calculatorInput): Record<string, number> {
  ...
  try { const v = input.distance * input.weight * input.volume * input.fuelPrice; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.distance * input.weight * input.volume * input.fuelPrice; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
