...
function evaluateAllFormulas(input: Quote_risk_analyzer_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.order_volume + input.unit_price + input.production_lead_time_days; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.order_volume + input.unit_price + input.production_lead_time_days; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
