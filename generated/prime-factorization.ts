...
function evaluateAllFormulas(input: Prime_factorizationInput): Record<string, number | string> {
  ...
  try { const v = input.number * input.maxIterations * input.auto_input_3; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
