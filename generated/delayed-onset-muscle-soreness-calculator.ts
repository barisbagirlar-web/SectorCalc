...
function evaluateAllFormulas(input: Delayed_onset_muscle_soreness_calculatorInput): Record<string, number> {
  ...
  try { const v = input.age * input.fitnessLevel * input.exerciseIntensity * input.exerciseDuration; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.age * input.fitnessLevel * input.exerciseIntensity * input.exerciseDuration * (input.recoveryHours); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.recoveryHours; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
