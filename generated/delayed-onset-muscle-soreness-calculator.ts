...
function evaluateAllFormulas(input: Delayed_onset_muscle_soreness_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.age * input.fitnessLevel * input.exerciseIntensity * input.exerciseDuration; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.age * input.fitnessLevel * input.exerciseIntensity * input.exerciseDuration * (input.recoveryHours); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.recoveryHours; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
