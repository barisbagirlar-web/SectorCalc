...
function evaluateAllFormulas(input: Violin_string_calculatorInput): Record<string, number> {
  ...
  try { const v = input.scaleLength * input.frequency * input.stringDiameter * input.materialDensity; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.scaleLength * input.frequency * input.stringDiameter * input.materialDensity; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
