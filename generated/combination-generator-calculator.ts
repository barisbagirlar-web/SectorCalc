...
function evaluateAllFormulas(input: Combination_generator_calculatorInput): Record<string, number> {
  ...
  try { const v = input.n * input.k * input.decimalPlaces * input.outputStyle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.n * input.k * input.decimalPlaces * input.outputStyle; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
