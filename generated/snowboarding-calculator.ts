...
function evaluateAllFormulas(input: Snowboarding_calculatorInput): Record<string, number> {
  ...
  try { const v = input.slopeAngle * input.slopeLength * input.frictionCoeff * input.mass; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.slopeAngle * input.slopeLength * input.frictionCoeff * input.mass; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
