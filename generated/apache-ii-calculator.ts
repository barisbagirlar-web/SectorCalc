...
function evaluateAllFormulas(input: Apache_ii_calculatorInput): Record<string, number> {
  ...
  try { const v = input.operatorSkill * 5; results["operatorSkillComponent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["operatorSkillComponent"] = Number.NaN; }
  try { const v = 100 - input.machineAge * 2; results["machineAgeComponent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["machineAgeComponent"] = Number.NaN; }
  try { const v = input.maintenanceFrequency * 10; results["maintenanceComponent"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["maintenanceComponent"] = Number.NaN; }
  return results;
}
