...
function evaluateAllFormulas(input: Shutter_speed_calculatorInput): Record<string, number> {
  ...
  try { const v = input.objectSpeed; results["shutterSpeed"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["shutterSpeed"] = 0; }
  try { const v = input.objectSpeed; results["shutterSpeed_ms"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["shutterSpeed_ms"] = 0; }
  try { const v = input.objectSpeed; results["shutterSpeed_us"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["shutterSpeed_us"] = 0; }
  return results;
}
