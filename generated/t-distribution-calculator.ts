...
function evaluateAllFormulas(input: T_distribution_calculatorInput): Record<string, number> {
  ...
  try { const v = input.t_value * input.degrees_of_freedom * input.tails; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.t_value * input.degrees_of_freedom * input.tails; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
