...
function evaluateAllFormulas(input: T_distribution_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.t_value * input.degrees_of_freedom * input.tails; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
