...
function evaluateAllFormulas(input: Gram_to_cup_calculatorInput): Record<string, number> {
  ...
  try { const v = input.grams / (input.density * input.packingFactor * input.cupVolume); results["cups"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["cups"] = Number.NaN; }
  try { const v = input.grams / (input.density * input.packingFactor); results["volumeML"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["volumeML"] = Number.NaN; }
  try { const v = input.density * input.packingFactor * input.cupVolume; results["gramsPerCup"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["gramsPerCup"] = Number.NaN; }
  return results;
}
