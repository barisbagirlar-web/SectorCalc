...
function evaluateAllFormulas(input: Grace_score_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = 1 - (input.utilization / 24) * 0.4; results["util_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["util_factor"] = 0; }
  try { const v = 1 + (1 - (input.maintenance_interval / 365)) * 0.3; results["maint_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["maint_factor"] = 0; }
  try { const v = 1 - (input.environment_severity - 1) * 0.1; results["env_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["env_factor"] = 0; }
  try { const v = input.last_inspection_score / 100; results["insp_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["insp_factor"] = 0; }
  return results;
}
