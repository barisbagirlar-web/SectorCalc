...
function evaluateAllFormulas(input: Hydrograph_calculatorInput): Record<string, number> {
  ...
  try { const v = input.L * input.Lca * input.A * input.Ct; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.L * input.Lca * input.A * input.Ct * (input.Cp); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.Cp; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
