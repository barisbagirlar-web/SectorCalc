...
function evaluateAllFormulas(input: Cmyk_to_rgb_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.cyan / 100) * (input.magenta / 100) * (input.yellow / 100) * (input.black / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = (input.cyan / 100) * (input.magenta / 100) * (input.yellow / 100) * (input.black / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
