...
function evaluateAllFormulas(input: Wavelength_calculatorInput): Record<string, number> {
  ...
  try { const v = input.waveSpeed / input.frequency; results["primary"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["primary"] = Number.NaN; }
  try { const v = 1 / input.frequency; results["period"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["period"] = Number.NaN; }
  try { const v = (input.planckConstant * input.waveSpeed) / input.energy; results["wavelength_energy"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["wavelength_energy"] = Number.NaN; }
  try { const v = (input.waveSpeed / input.frequency) / input.refractiveIndex; results["wavelength_medium"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["wavelength_medium"] = Number.NaN; }
  return results;
}
