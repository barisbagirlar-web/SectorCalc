...
function evaluateAllFormulas(input: Coin_flip_calculatorInput): Record<string, number> {
  ...
  try { const v = input.numberOfFlips * input.numberOfHeads * input.probabilityOfHead * input.decimalPlaces; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.numberOfFlips * input.numberOfHeads * input.probabilityOfHead * input.decimalPlaces; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
