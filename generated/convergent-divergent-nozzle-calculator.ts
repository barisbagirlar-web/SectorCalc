...
function evaluateAllFormulas(input: Convergent_divergent_nozzle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalPressure * input.totalTemperature * input.throatArea * input.exitArea; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.totalPressure * input.totalTemperature * input.throatArea * input.exitArea * (input.gasConstant * input.specificHeatRatio * input.ambientPressure); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.gasConstant * input.specificHeatRatio * input.ambientPressure; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
