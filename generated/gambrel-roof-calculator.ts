...
function evaluateAllFormulas(input: Gambrel_roof_calculatorInput): Record<string, number> {
  ...
  try { const v = input.lowerAngle * Math.PI / 180; results["radLower"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["radLower"] = Number.NaN; }
  try { const v = input.upperAngle * Math.PI / 180; results["radUpper"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["radUpper"] = Number.NaN; }
  try { const v = input.width / 2 - input.lowerRun; results["upperRun"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["upperRun"] = Number.NaN; }
  try { const v = input.length; results["ridgeLength"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["ridgeLength"] = Number.NaN; }
  return results;
}
