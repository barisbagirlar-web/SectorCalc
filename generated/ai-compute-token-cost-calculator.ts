...
function evaluateAllFormulas(input: Ai_compute_token_cost_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.model_type + input.input_tokens_per_month + input.output_tokens_per_month; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.model_type + input.input_tokens_per_month + input.output_tokens_per_month; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
