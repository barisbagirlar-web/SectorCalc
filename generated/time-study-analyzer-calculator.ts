...
function evaluateAllFormulas(input: Time_study_analyzer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.observed_time * (input.performance_rating / 100) * (input.allowance_percentage / 100) * input.frequency_per_cycle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.observed_time * (input.performance_rating / 100) * (input.allowance_percentage / 100) * input.frequency_per_cycle; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
