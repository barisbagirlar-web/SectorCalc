...
function evaluateAllFormulas(input: Strength_standard_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weightLifted * (1 + input.reps / 30); results["estimatedOneRM"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["estimatedOneRM"] = 0; }
  try { const v = (asFormulaNumber(results["estimatedOneRM"])) / input.bodyWeight; results["strengthRatio"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["strengthRatio"] = 0; }
  return results;
}
