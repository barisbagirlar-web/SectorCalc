...
function evaluateAllFormulas(input: Subcontractor_margin_leak_detector_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.contract_value * input.actual_labor_cost * input.actual_material_cost * input.actual_equipment_cost; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.contract_value * input.actual_labor_cost * input.actual_material_cost * input.actual_equipment_cost * ((input.overhead_percentage / 100) * input.quality_rework_cost * input.schedule_delay_penalty * (input.waste_factor / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.overhead_percentage / 100) * input.quality_rework_cost * input.schedule_delay_penalty * (input.waste_factor / 100); results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
