...
function evaluateAllFormulas(input: Exterior_angle_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.sides * input.interiorAngle * input.sumInteriorAngles * input.exteriorAngleGiven; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.sides * input.interiorAngle * input.sumInteriorAngles * input.exteriorAngleGiven * (input.precision); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
