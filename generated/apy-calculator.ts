...
function evaluateAllFormulas(input: Apy_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.time_period_years * input.initial_principal; results["base_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.time_period_years * input.initial_principal * (1 + (input.nominal_rate / 100)); results["adjusted_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.time_period_years * input.initial_principal * (1 + (input.nominal_rate / 100)) * ((input.fee_percentage / 100)); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.fee_percentage / 100); results["factor_fee_percentage"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["factor_fee_percentage"] = 0; }
  return results;
}
