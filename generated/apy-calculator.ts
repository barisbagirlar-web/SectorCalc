...
function evaluateAllFormulas(input: Apy_calculatorInput): Record<string, number> {
  ...
  try { const v = input.time_period_years * input.initial_principal; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.time_period_years * input.initial_principal * (1 + (input.nominal_rate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.time_period_years * input.initial_principal * (1 + (input.nominal_rate / 100)) * ((input.fee_percentage / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.fee_percentage / 100); results["factor_fee_percentage"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_fee_percentage"] = Number.NaN; }
  return results;
}
