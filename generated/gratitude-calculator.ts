...
function evaluateAllFormulas(input: Gratitude_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.daily_acts * input.days_practiced * (input.intensity_factor / 10); results["gratitude_score"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["gratitude_score"] = 0; }
  try { const v = input.social_shares * input.days_practiced * 0.5; results["social_impact"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["social_impact"] = 0; }
  return results;
}
