...
function evaluateAllFormulas(input: Foundation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.length * input.width * input.depth * input.concreteDensity; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.length * input.width * input.depth * input.concreteDensity * (input.rebarRatio * input.steelDensity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.rebarRatio * input.steelDensity; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
