...
function evaluateAllFormulas(input: Foundation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.length * input.width * input.depth * input.concreteDensity; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.length * input.width * input.depth * input.concreteDensity * (input.rebarRatio * input.steelDensity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.rebarRatio * input.steelDensity; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
