...
function evaluateAllFormulas(input: Sleep_cycle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.wake_time * input.sleep_debt_hours * input.cycle_length_minutes * input.fall_asleep_minutes; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.wake_time * input.sleep_debt_hours * input.cycle_length_minutes * input.fall_asleep_minutes * ((input.recovery_efficiency / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.recovery_efficiency / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
