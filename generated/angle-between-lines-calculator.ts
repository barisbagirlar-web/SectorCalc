...
function evaluateAllFormulas(input: Angle_between_lines_calculatorInput): Record<string, number> {
  ...
  try { const v = input.x2 - input.x1; results["dx1"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dx1"] = 0; }
  try { const v = input.y2 - input.y1; results["dy1"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dy1"] = 0; }
  try { const v = input.x4 - input.x3; results["dx2"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dx2"] = 0; }
  try { const v = input.y4 - input.y3; results["dy2"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dy2"] = 0; }
  return results;
}
