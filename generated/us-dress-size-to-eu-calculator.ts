...
function evaluateAllFormulas(input: Us_dress_size_to_eu_calculatorInput): Record<string, number> {
  ...
  try { const v = input.ageGroup == 1 ? 30 : (input.ageGroup == 2 ? 34 : 32); results["baseOffset"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["baseOffset"] = Number.NaN; }
  try { const v = input.usDressSize + (toNumericFormulaValue(results["baseOffset"])) + input.brandAdjustment; results["rawEuSize"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawEuSize"] = Number.NaN; }
  return results;
}
