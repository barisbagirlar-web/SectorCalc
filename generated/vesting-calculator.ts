...
function evaluateAllFormulas(input: Vesting_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalGrant * input.vestingStartMonth * input.currentMonth * input.vestingPeriodMonths; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.totalGrant * input.vestingStartMonth * input.currentMonth * input.vestingPeriodMonths * (input.cliffPeriodMonths); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.cliffPeriodMonths; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
