...
function evaluateAllFormulas(input: Stopping_sight_distance_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.speed * input.reactionTime) / 3.6; results["reactionDist"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["reactionDist"] = 0; }
  try { const v = (input.speed * input.reactionTime) / 3.6; results["reactionDist_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["reactionDist_aux"] = 0; }
  return results;
}
...
export function calculateStopping_sight_distance_calculator(input: Stopping_sight_distance_calculatorInput): Stopping_sight_distance_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["reactionDist"]));
  const breakdown = {
