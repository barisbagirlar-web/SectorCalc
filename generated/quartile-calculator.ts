...
function evaluateAllFormulas(input: Quartile_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.num1 * input.num2 * input.num3 * input.num4; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.num1 * input.num2 * input.num3 * input.num4 * (input.num5 * input.num6 * input.num7 * input.num8); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.num5 * input.num6 * input.num7 * input.num8; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
