...
function evaluateAllFormulas(input: Zumba_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * input.MET * (input.duration / 60) * input.intensityFactor; results["caloriesBurned"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["caloriesBurned"] = Number.NaN; }
  try { const v = input.duration / 60; results["hours"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["hours"] = Number.NaN; }
  return results;
}
