...
function evaluateAllFormulas(input: Vector_angle_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.x1 * input.x2 + input.y1 * input.y2; results["dotProduct"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["dotProduct"] = 0; }
  try { const v = input.x1 * input.x2 + input.y1 * input.y2; results["dotProduct_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["dotProduct_aux"] = 0; }
  return results;
}
