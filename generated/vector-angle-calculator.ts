...
function evaluateAllFormulas(input: Vector_angle_calculatorInput): Record<string, number> {
  ...
  try { const v = input.x1 * input.x2 + input.y1 * input.y2; results["dotProduct"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["dotProduct"] = Number.NaN; }
  try { const v = input.x1 * input.x2 + input.y1 * input.y2; results["dotProduct_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["dotProduct_aux"] = Number.NaN; }
  return results;
}
