...
function evaluateAllFormulas(input: Stair_rise_run_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalRise * input.totalRun * input.maxRiserHeight * input.minTreadDepth; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.totalRise * input.totalRun * input.maxRiserHeight * input.minTreadDepth * (input.desiredRise * input.nosing); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.desiredRise * input.nosing; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
