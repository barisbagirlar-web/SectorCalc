...
function evaluateAllFormulas(input: Customer_lifetime_value_calculatorInput): Record<string, number> {
  ...
  try { const v = input.avgPurchaseValue * input.purchaseFrequency * input.lifespanYears; results["totalRevenue"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["totalRevenue"] = Number.NaN; }
  try { const v = input.avgPurchaseValue * input.purchaseFrequency * input.grossMarginPercent / 100; results["annualValue"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annualValue"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["totalRevenue"])) * input.grossMarginPercent / 100; results["clv"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["clv"] = Number.NaN; }
  return results;
}
