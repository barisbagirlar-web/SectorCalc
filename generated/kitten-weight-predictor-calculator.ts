...
function evaluateAllFormulas(input: Kitten_weight_predictor_calculatorInput): Record<string, number> {
  ...
  try { const v = input.ageInWeeks * input.currentWeight * input.birthWeight * input.breedSizeFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.ageInWeeks * input.currentWeight * input.birthWeight * input.breedSizeFactor * (input.sexFactor * input.dailyCalories); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.sexFactor * input.dailyCalories; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
