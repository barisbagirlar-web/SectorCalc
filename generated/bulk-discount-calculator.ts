...
function evaluateAllFormulas(input: Bulk_discount_calculatorInput): Record<string, number> {
  ...
  try { const v = input.quantity; results["annual_exposure_hours"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["annual_exposure_hours"] = Number.NaN; }
  ...
  try { const v = input.quantity * (input.maxDiscount / 100) * input.basePricePerUnit * input.scale; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
