...
function evaluateAllFormulas(input: Bell_number_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.power1 / input.power2; results["powerRatio"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["powerRatio"] = 0; }
  try { const v = input.power1 / input.power2; results["powerRatio_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["powerRatio_aux"] = 0; }
  return results;
}
