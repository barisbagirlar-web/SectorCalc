...
function evaluateAllFormulas(input: Wedding_cake_serving_calculatorInput): Record<string, number> {
  ...
  try { const v = input.shape * input.tier1Diameter * input.tier2Diameter * input.tier3Diameter; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.shape * input.tier1Diameter * input.tier2Diameter * input.tier3Diameter * (input.servingSize * input.guestCount); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.servingSize * input.guestCount; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
