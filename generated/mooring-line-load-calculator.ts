...
function evaluateAllFormulas(input: Mooring_line_load_calculatorInput): Record<string, number> {
  ...
  try { const v = results["kopmaYuku"] / Math.max(0.0001, input["guvenlikFaktoru"]); results["sonuc"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["sonuc"] = Number.NaN; }
  return results;
}
