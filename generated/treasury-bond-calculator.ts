...
function evaluateAllFormulas(input: Treasury_bond_calculatorInput): Record<string, number> {
  ...
  try { const v = input.faceValue * (input.couponRate / 100) * input.yearsToMaturity * (input.yieldToMaturity / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.faceValue * (input.couponRate / 100) * input.yearsToMaturity * (input.yieldToMaturity / 100) * (input.frequency); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.frequency; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
