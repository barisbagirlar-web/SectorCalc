...
function evaluateAllFormulas(input: Blackjack_calculatorInput): Record<string, number> {
  ...
  try { const v = input.lambdaA * input.costPerBlackjack; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.lambdaA * input.costPerBlackjack; results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.lambdaA * input.costPerBlackjack; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
