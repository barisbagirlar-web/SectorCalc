...
function evaluateAllFormulas(input: Beer_pairing_calculatorInput): Record<string, number> {
  ...
  try { const v = input.beerIBU * (input.beerABV / 100) * input.foodRichness * input.foodSpiciness; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.beerIBU * (input.beerABV / 100) * input.foodRichness * input.foodSpiciness * (input.foodAcidity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.foodAcidity; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
