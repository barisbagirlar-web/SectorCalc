...
function evaluateAllFormulas(input: Beer_pairing_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.beerIBU * (input.beerABV / 100) * input.foodRichness * input.foodSpiciness; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.beerIBU * (input.beerABV / 100) * input.foodRichness * input.foodSpiciness * (input.foodAcidity); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.foodAcidity; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
