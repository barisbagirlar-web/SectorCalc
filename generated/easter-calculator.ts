...
function evaluateAllFormulas(input: Easter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.year * input.daysToAdd * input.algorithmVersion * input.moonPhaseOffset; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.year * input.daysToAdd * input.algorithmVersion * input.moonPhaseOffset; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
