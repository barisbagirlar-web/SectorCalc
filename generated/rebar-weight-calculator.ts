...
function evaluateAllFormulas(input: Rebar_weight_calculatorInput): Record<string, number> {
  ...
  try { const v = input.diameter * input.length * input.quantity * input.density; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.diameter * input.length * input.quantity * input.density; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
