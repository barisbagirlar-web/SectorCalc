...
function evaluateAllFormulas(input: Viscosity_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.diameter / 2; results["radius_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["radius_aux"] = 0; }
  return results;
}
