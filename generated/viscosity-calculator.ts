...
function evaluateAllFormulas(input: Viscosity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.flowRate * input.pressureDrop * input.diameter * input.length; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.flowRate * input.pressureDrop * input.diameter * input.length * (input.density); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.density; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
