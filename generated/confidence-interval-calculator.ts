...
function evaluateAllFormulas(input: Confidence_interval_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sample_mean * input.standard_deviation * input.sample_size * input.critical_value; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.sample_mean * input.standard_deviation * input.sample_size * input.critical_value; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
