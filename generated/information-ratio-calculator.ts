...
function evaluateAllFormulas(input: Information_ratio_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.portfolioReturn / 100) * (input.benchmarkReturn / 100) * (input.trackingError / 100) * input.periodsPerYear; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = (input.portfolioReturn / 100) * (input.benchmarkReturn / 100) * (input.trackingError / 100) * input.periodsPerYear; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
