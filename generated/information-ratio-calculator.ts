...
function evaluateAllFormulas(input: Information_ratio_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.portfolioReturn + input.benchmarkReturn + input.trackingError; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.portfolioReturn + input.benchmarkReturn + input.trackingError; results["result_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_aux"] = 0; }
  return results;
}
