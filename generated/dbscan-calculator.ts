...
function evaluateAllFormulas(input: Dbscan_calculatorInput): Record<string, number> {
  ...
  try { const v = input.neighborCount / (Math.PI * input.epsilon ** 2); results["density"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["density"] = 0; }
  try { const v = Math.PI * input.epsilon ** 2; results["area"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["area"] = 0; }
  try { const v = input.neighborCount; results["neighborCountOut"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["neighborCountOut"] = 0; }
  return results;
}
...
export function calculateDbscan_calculator(input: Dbscan_calculatorInput): Dbscan_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["density"]));
  const breakdown = {
