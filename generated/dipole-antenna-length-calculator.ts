...
function evaluateAllFormulas(input: Dipole_antenna_length_calculatorInput): Record<string, number> {
  ...
  try { const v = 492 * input.velocityFactor * input.wavelengthFraction * input.endEffectCorrection / input.frequency; results["lengthFeet"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["lengthFeet"] = Number.NaN; }
  try { const v = 300 * input.velocityFactor * input.wavelengthFraction * input.endEffectCorrection / input.frequency; results["lengthMeters"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["lengthMeters"] = Number.NaN; }
  try { const v = input.unitSelection === 0 ? 492 * input.velocityFactor * input.wavelengthFraction * input.endEffectCorrection / input.frequency : 300 * input.velocityFactor * input.wavelengthFraction * input.endEffectCorrection / input.frequency; results["lengthInSelectedUnit"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["lengthInSelectedUnit"] = Number.NaN; }
  return results;
}
