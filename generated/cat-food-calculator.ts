...
function evaluateAllFormulas(input: Cat_food_calculatorInput): Record<string, number> {
  ...
  try { const v = input.catWeight * input.dailyFoodPerKg * input.numCats; results["dailyFoodAmount"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dailyFoodAmount"] = 0; }
  try { const v = (input.catWeight * input.dailyFoodPerKg * input.numCats / 1000) / input.foodPackageWeight * input.foodPackagePrice; results["dailyCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dailyCost"] = 0; }
  try { const v = (input.catWeight * input.dailyFoodPerKg * input.numCats / 1000) / input.foodPackageWeight * input.foodPackagePrice * 30; results["monthlyCost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["monthlyCost"] = 0; }
  return results;
}
