...
function evaluateAllFormulas(input: Flow_state_calculatorInput): Record<string, number> {
  ...
  try { const v = input.density * input.velocity * input.diameter / input.dynamicViscosity; results["reynoldsNumber"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["reynoldsNumber"] = Number.NaN; }
  try { const v = input.density * input.velocity * input.diameter / input.dynamicViscosity; results["reynoldsNumber_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["reynoldsNumber_aux"] = Number.NaN; }
  return results;
}
