...
function evaluateAllFormulas(input: Log_base_10_calculatorInput): Record<string, number> {
  ...
  try { const v = input.x * input.y * input.n * input.base; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.x * input.y * input.n * input.base * (input.precision); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
