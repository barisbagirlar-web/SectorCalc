...
function evaluateAllFormulas(input: Log_base_10_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.x * input.y * input.n * input.base; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.x * input.y * input.n * input.base * (input.precision); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
