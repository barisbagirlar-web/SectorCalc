...
function evaluateAllFormulas(input: Surface_area_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bottomLength * input.bottomWidth; results["bottomArea"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["bottomArea"] = Number.NaN; }
  try { const v = input.topLength * input.topWidth; results["topArea"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["topArea"] = Number.NaN; }
  return results;
}
...
export function calculateSurface_area_calculator(input: Surface_area_calculatorInput): Surface_area_calculatorOutput {
  ...
  const totalWasteCost = toNumericFormulaValue(values["topArea"]);
  const breakdown = {
