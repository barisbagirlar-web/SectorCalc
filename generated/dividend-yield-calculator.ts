...
function evaluateAllFormulas(input: Dividend_yield_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualDividendPerShare / input.marketPricePerShare * 100; results["dividendYield"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["dividendYield"] = 0; }
  try { const v = input.annualDividendPerShare * input.numberOfShares; results["totalDividend"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalDividend"] = 0; }
  try { const v = (asFormulaNumber(results["dividendYield"])) * (1 - input.taxRate / 100); results["afterTaxYield"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["afterTaxYield"] = 0; }
  return results;
}
