...
function evaluateAllFormulas(input: Tons_of_refrigeration_to_kwInput): Record<string, number> {
  ...
  try { const v = input.tons * 3.5168525; results["primary"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["primary"] = Number.NaN; }
  try { const v = input.tons; results["breakdown"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["breakdown"] = Number.NaN; }
  return results;
}
