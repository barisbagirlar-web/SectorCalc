...
function evaluateAllFormulas(input: Foundation_shade_calculatorInput): Record<string, number> {
  ...
  try { const v = Math.round(input.skinDepth * 10 + input.skinUndertone + ((input.coveragePreference - 5) / 10) * 2 + ((input.lightingCondition - 5) / 10) * 2); results["recommendedShade"] = Number.isFinite(v) ? v : 0; } catch { results["recommendedShade"] = 0; }
  try { const v = input.skinDepth * 10 + input.skinUndertone; results["baseShade"] = Number.isFinite(v) ? v : 0; } catch { results["baseShade"] = 0; }
  try { const v = ((input.coveragePreference - 5) / 10) * 2; results["coverageAdjustment"] = Number.isFinite(v) ? v : 0; } catch { results["coverageAdjustment"] = 0; }
  try { const v = ((input.lightingCondition - 5) / 10) * 2; results["lightingAdjustment"] = Number.isFinite(v) ? v : 0; } catch { results["lightingAdjustment"] = 0; }
  return results;
}
...
export function calculateFoundation_shade_calculator(input: Foundation_shade_calculatorInput): Foundation_shade_calculatorOutput {
  ...
  const totalWasteCost = values["recommendedShade"] ?? 0;
  const breakdown = {
