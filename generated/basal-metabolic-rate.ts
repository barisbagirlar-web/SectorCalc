...
function evaluateAllFormulas(input: Basal_metabolic_rateInput): Record<string, number> {
  ...
  try { const v = 10 * input.weight + 6.25 * input.height - 5 * input.age; results["base"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base"] = 0; }
  try { const v = input.gender == 1 ? 5 : -161; results["gender_adjustment"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["gender_adjustment"] = 0; }
  try { const v = (asFormulaNumber(results["base"])) + (asFormulaNumber(results["gender_adjustment"])); results["bmr"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["bmr"] = 0; }
  return results;
}
