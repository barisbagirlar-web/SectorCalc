...
function evaluateAllFormulas(input: Cholesky_decomposition_calculatorInput): Record<string, number> {
  ...
  try { const v = input.a11 * input.a12 * input.a13 * input.a22; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.a11 * input.a12 * input.a13 * input.a22 * (input.a23 * input.a33); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.a23 * input.a33; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
