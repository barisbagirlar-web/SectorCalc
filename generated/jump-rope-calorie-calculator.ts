...
function evaluateAllFormulas(input: Jump_rope_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = input.jumpsPerMinute / 13.636; results["met"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["met"] = 0; }
  try { const v = (asFormulaNumber(results["met"])) * input.weight * 3.5 / 200 * input.efficiencyFactor; results["caloriesPerMinute"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["caloriesPerMinute"] = 0; }
  try { const v = (asFormulaNumber(results["caloriesPerMinute"])) * input.duration; results["caloriesBurned"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["caloriesBurned"] = 0; }
  return results;
}
