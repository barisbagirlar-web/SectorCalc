...
function evaluateAllFormulas(input: Pressure_vessel_thickness_calculatorInput): Record<string, number> {
  ...
  try { const v = input.design_pressure * input.vessel_diameter * input.allowable_stress * input.corrosion_allowance; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.design_pressure * input.vessel_diameter * input.allowable_stress * input.corrosion_allowance * ((input.material_utilization / 100) * input.custom_safety_factor); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.material_utilization / 100) * input.custom_safety_factor; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
