...
function evaluateAllFormulas(input: Radioactive_decay_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialMass * (0.5 ** ((input.endTime - input.startTime) / input.halfLife)); results["remainingMass"] = Number.isFinite(v) ? v : 0; } catch { results["remainingMass"] = 0; }
  try { const v = 0.5 ** ((input.endTime - input.startTime) / input.halfLife); results["decayFraction"] = Number.isFinite(v) ? v : 0; } catch { results["decayFraction"] = 0; }
  try { const v = (input.endTime - input.startTime) / input.halfLife; results["numberOfHalfLives"] = Number.isFinite(v) ? v : 0; } catch { results["numberOfHalfLives"] = 0; }
  return results;
}
...
export function calculateRadioactive_decay_calculator(input: Radioactive_decay_calculatorInput): Radioactive_decay_calculatorOutput {
  ...
  const totalWasteCost = values["remainingMass"] ?? 0;
  const breakdown = {
