...
function evaluateAllFormulas(input: Hrv_recovery_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.supplyTemp - input.outdoorTemp) / (input.exhaustTemp - input.outdoorTemp)) * 100; results["efficiency"] = Number.isFinite(v) ? v : 0; } catch { results["efficiency"] = 0; }
  try { const v = input.supplyTemp - input.outdoorTemp; results["temperatureGain"] = Number.isFinite(v) ? v : 0; } catch { results["temperatureGain"] = 0; }
  try { const v = (input.airflow / 3600) * input.airDensity * input.specificHeat * (input.supplyTemp - input.outdoorTemp); results["heatRecoveryRate"] = Number.isFinite(v) ? v : 0; } catch { results["heatRecoveryRate"] = 0; }
  return results;
}
