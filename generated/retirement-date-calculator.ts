...
function evaluateAllFormulas(input: Retirement_date_calculatorInput): Record<string, number> {
  ...
  try { const v = input.currentAge * input.currentSavings * (1 + (input.annualReturnPercent / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.currentAge * input.currentSavings * (1 + (input.annualReturnPercent / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
