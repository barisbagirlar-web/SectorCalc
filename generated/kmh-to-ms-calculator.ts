...
function evaluateAllFormulas(input: Kmh_to_ms_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.speed_kmh / input.conversion_factor; results["speed_ms_raw"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["speed_ms_raw"] = 0; }
  try { const v = input.speed_kmh / input.conversion_factor; results["speed_ms_raw_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["speed_ms_raw_aux"] = 0; }
  return results;
}
