...
function evaluateAllFormulas(input: Kmh_to_ms_calculatorInput): Record<string, number> {
  ...
  try { const v = input.speed_kmh / input.conversion_factor; results["speed_ms_raw"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["speed_ms_raw"] = Number.NaN; }
  try { const v = input.speed_kmh / input.conversion_factor; results["speed_ms_raw_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["speed_ms_raw_aux"] = Number.NaN; }
  return results;
}
