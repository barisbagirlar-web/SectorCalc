...
function evaluateAllFormulas(input: _529_plan_calculatorInput): Record<string, number> {
  ...
  try { const v = input.childAge * input.annualContribution; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.childAge * input.annualContribution * (1 + (input.annualReturn / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.childAge * input.annualContribution * (1 + (input.annualReturn / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
