...
function evaluateAllFormulas(input: Network_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.ip1 << 24) >>> 0) + (input.ip2 << 16) + (input.ip3 << 8) + input.ip4; results["ipInt"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["ipInt"] = Number.NaN; }
  try { const v = input.cidr === 0 ? 0 : (4294967295 << (32 - input.cidr)) >>> 0; results["maskInt"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["maskInt"] = Number.NaN; }
  try { const v = input.cidr === 32 ? 1 : (1 << (32 - input.cidr)) - 2; results["hostCount"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["hostCount"] = Number.NaN; }
  return results;
}
