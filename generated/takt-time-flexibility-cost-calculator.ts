...
function evaluateAllFormulas(input: Takt_time_flexibility_cost_calculatorInput): Record<string, number> {
  ...
  try { const v = input.available_work_time_seconds * input.customer_demand_per_shift * input.cycle_time_seconds * input.changeover_time_minutes; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.available_work_time_seconds * input.customer_demand_per_shift * input.cycle_time_seconds * input.changeover_time_minutes * (input.batch_size * input.labor_cost_per_hour * (input.overhead_rate_percent / 100) * input.demand_variability_coefficient); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.batch_size * input.labor_cost_per_hour * (input.overhead_rate_percent / 100) * input.demand_variability_coefficient; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
...
export const Takt_time_flexibility_cost_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
