...
function evaluateAllFormulas(input: Cut_fill_balance_optimizer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.swellFactor * input.unitHaulCost; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.swellFactor * input.unitHaulCost; results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.swellFactor * input.unitHaulCost * 1 * (input.cutVolume * input.fillVolume); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.cutVolume; results["factor_cutVolume"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["factor_cutVolume"] = 0; }
  try { const v = input.fillVolume; results["factor_fillVolume"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["factor_fillVolume"] = 0; }
  return results;
}
