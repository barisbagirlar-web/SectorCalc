...
function evaluateAllFormulas(input: Cut_fill_balance_optimizer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.swellFactor * input.unitHaulCost; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.swellFactor * input.unitHaulCost; results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.swellFactor * input.unitHaulCost * 1 * (input.cutVolume * input.fillVolume); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.cutVolume; results["factor_cutVolume"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_cutVolume"] = Number.NaN; }
  try { const v = input.fillVolume; results["factor_fillVolume"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_fillVolume"] = Number.NaN; }
  return results;
}
