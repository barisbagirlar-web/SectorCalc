...
function evaluateAllFormulas(input: Chord_calculatorInput): Record<string, number> {
  ...
  try { const v = input.radius * input.angleDeg * input.arcLength * input.chordOffset; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.radius * input.angleDeg * input.arcLength * input.chordOffset * (input.precision); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
