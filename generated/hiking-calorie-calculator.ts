...
function evaluateAllFormulas(input: Hiking_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.bodyWeight + input.packWeight) * (input.distance * 0.8 + input.elevationGain * 0.0094) * input.terrainFactor; results["totalCalories"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalCalories"] = 0; }
  try { const v = (input.bodyWeight + input.packWeight) * input.distance * 0.8 * input.terrainFactor; results["horizontalCalories"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["horizontalCalories"] = 0; }
  try { const v = (input.bodyWeight + input.packWeight) * input.elevationGain * 0.0094 * input.terrainFactor; results["verticalCalories"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["verticalCalories"] = 0; }
  return results;
}
