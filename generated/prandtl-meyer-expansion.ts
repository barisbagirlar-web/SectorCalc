...
function evaluateAllFormulas(input: Prandtl_meyer_expansionInput): Record<string, number> {
  ...
  try { const v = input.mach1 * input.gamma * input.deflectionAngle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mach1 * input.gamma * input.deflectionAngle; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
