...
function evaluateAllFormulas(input: Prandtl_meyer_expansionInput): Record<string, number> {
  ...
  try { const v = input.mach1 * input.gamma * input.deflectionAngle; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.mach1 * input.gamma * input.deflectionAngle; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
