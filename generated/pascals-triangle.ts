...
function evaluateAllFormulas(input: Pascals_triangleInput): Record<string, number> {
  ...
  try { const v = input.rowIndex * input.colIndex * input.numRows * input.maxValue; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.rowIndex * input.colIndex * input.numRows * input.maxValue; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
