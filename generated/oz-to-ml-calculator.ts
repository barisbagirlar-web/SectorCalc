...
function evaluateAllFormulas(input: Oz_to_ml_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.ounceType === 0 ? 29.5735 : 28.4131); results["conversionFactor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["conversionFactor"] = 0; }
  try { const v = input.fluidOunces * (asFormulaNumber(results["conversionFactor"])) * input.batchSize; results["baseMilliliters"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["baseMilliliters"] = 0; }
  return results;
}
