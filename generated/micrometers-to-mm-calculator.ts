...
function evaluateAllFormulas(input: Micrometers_to_mm_calculatorInput): Record<string, number> {
  ...
  try { const v = input.micrometerValue + (input.calibrationFactor/100 * input.micrometerValue) + input.temperatureCoeff * (input.ambientTemperature - 20); results["correctedMicrometers"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["correctedMicrometers"] = Number.NaN; }
  try { const v = (toNumericFormulaValue(results["correctedMicrometers"])) / 1000; results["mm"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["mm"] = Number.NaN; }
  return results;
}
