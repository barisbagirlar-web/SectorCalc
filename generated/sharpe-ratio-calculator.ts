...
function evaluateAllFormulas(input: Sharpe_ratio_calculatorInput): Record<string, number | string> {
  ...
  try { const v = (input.portfolioReturn / 100) * (input.riskFreeRate / 100) * (input.dailyStdDev / 100) * input.tradingDays; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = (input.portfolioReturn / 100) * (input.riskFreeRate / 100) * (input.dailyStdDev / 100) * input.tradingDays; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
