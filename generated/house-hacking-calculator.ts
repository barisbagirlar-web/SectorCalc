...
function evaluateAllFormulas(input: House_hacking_calculatorInput): Record<string, number> {
  ...
  try { const v = input.propertyPrice - input.downPayment; results["loanAmount"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["loanAmount"] = Number.NaN; }
  ...
  try { const v = input.monthlyRentIncome * (input.vacancyRate / 100); results["vacancyLoss"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["vacancyLoss"] = Number.NaN; }
  try { const v = input.monthlyRentIncome - (toNumericFormulaValue(results["vacancyLoss"])); results["effectiveRentIncome"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["effectiveRentIncome"] = Number.NaN; }
  return results;
}
