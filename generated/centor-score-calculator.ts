...
function evaluateAllFormulas(input: Centor_score_calculatorInput): Record<string, number> {
  ...
  try { const v = input.fever * input.exudates * input.lymphadenopathy * input.cough_absence; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.fever * input.exudates * input.lymphadenopathy * input.cough_absence; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
