...
function evaluateAllFormulas(input: Percentile_to_z_score_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.percentile / 100) * input.mean * input.stddev; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = (input.percentile / 100) * input.mean * input.stddev; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
