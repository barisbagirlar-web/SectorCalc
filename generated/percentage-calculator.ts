...
function evaluateAllFormulas(input: Percentage_calculatorInput): Record<string, number> {
  ...
  try { const v = input.total_units * input.defective_units * input.rework_units * input.scrap_units; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.total_units * input.defective_units * input.rework_units * input.scrap_units * ((input.target_yield / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = (input.target_yield / 100); results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
