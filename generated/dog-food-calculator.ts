...
function evaluateAllFormulas(input: Dog_food_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * input.activityFactor * input.ageMultiplier * input.foodEnergyDensity; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.weight * input.activityFactor * input.ageMultiplier * input.foodEnergyDensity; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
