...
function evaluateAllFormulas(input: Sheetrock_calculatorInput): Record<string, number> {
  ...
  try { const v = input.roomLength * input.roomWidth * input.roomHeight * input.sheetLength; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.roomLength * input.roomWidth * input.roomHeight * input.sheetLength * (input.sheetWidth * (input.wasteFactor / 100) * input.deductionArea); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.sheetWidth * (input.wasteFactor / 100) * input.deductionArea; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
