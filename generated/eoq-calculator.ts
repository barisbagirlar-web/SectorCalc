...
function evaluateAllFormulas(input: Eoq_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.annualDemand / input.workingDaysPerYear) * input.leadTimeDays; results["reorderPoint"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["reorderPoint"] = Number.NaN; }
  try { const v = (input.annualDemand / input.workingDaysPerYear) * input.leadTimeDays; results["reorderPoint_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["reorderPoint_aux"] = Number.NaN; }
  return results;
}
