...
function evaluateAllFormulas(input: Cat_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = 1.2 + (input.activityLevel - 1) * 0.1 + input.neutered * 0.2 + (input.lifeStage == 1 ? 0.2 : (input.lifeStage == 3 ? -0.1 : 0)); results["factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["factor"] = 0; }
  try { const v = 1.2 + (input.activityLevel - 1) * 0.1 + input.neutered * 0.2 + (input.lifeStage == 1 ? 0.2 : (input.lifeStage == 3 ? -0.1 : 0)); results["factor_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["factor_aux"] = 0; }
  return results;
}
...
export function calculateCat_calorie_calculator(input: Cat_calorie_calculatorInput): Cat_calorie_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["factor_aux"]));
  const breakdown = {
