...
function evaluateAllFormulas(input: Waist_to_hip_ratio_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.waist / input.hip; results["waistToHipRatio"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["waistToHipRatio"] = 0; }
  try { const v = input.waist / input.height; results["waistToHeightRatio"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["waistToHeightRatio"] = 0; }
  ...
  try { const v = input.gender === 0 ? ( (input.waist/input.hip) > 0.85 ? 1 : 0 ) : ( (input.waist/input.hip) > 0.90 ? 1 : 0 ); results["highRiskWHR"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["highRiskWHR"] = 0; }
  return results;
}
