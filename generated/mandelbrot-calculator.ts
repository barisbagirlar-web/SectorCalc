...
function evaluateAllFormulas(input: Mandelbrot_calculatorInput): Record<string, number> {
  ...
  try { const v = input.real * input.imag * input.maxIter * input.escapeRadius; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.real * input.imag * input.maxIter * input.escapeRadius; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
