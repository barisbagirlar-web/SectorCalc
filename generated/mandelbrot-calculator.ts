...
function evaluateAllFormulas(input: Mandelbrot_calculatorInput): Record<string, number> {
  ...
  try { const v = input.real * input.imag * input.maxIter * input.escapeRadius; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.real * input.imag * input.maxIter * input.escapeRadius; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
