...
function evaluateAllFormulas(input: Cost_benefit_analysis_calculatorInput): Record<string, number> {
  ...
  try { const v = input.discountRate / 100; results["rate"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rate"] = Number.NaN; }
  try { const v = input.annualBenefits - input.annualCosts; results["netAnnualCash"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["netAnnualCash"] = Number.NaN; }
  return results;
}
