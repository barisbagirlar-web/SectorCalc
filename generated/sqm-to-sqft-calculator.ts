...
function evaluateAllFormulas(input: Sqm_to_sqft_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.areaSqm * input.conversionFactor; results["areaSqft"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["areaSqft"] = 0; }
  try { const v = input.areaSqm * input.conversionFactor; results["areaSqft_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["areaSqft_aux"] = 0; }
  return results;
}
