...
function evaluateAllFormulas(input: Sqm_to_sqft_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.areaSqm) * (input.conversionFactor) * (input.roundingPrecision) * (input.areaUnitPrice) * (input.wasteFactor) * (input.measurementTolerance); results["areaSqft"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["areaSqft"] = Number.NaN; }
  try { const v = (input.areaSqm) * (input.conversionFactor) * (input.roundingPrecision); results["areaSqft_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["areaSqft_aux"] = Number.NaN; }
  return results;
}
...
export function calculateSqm_to_sqft_calculator(input: Sqm_to_sqft_calculatorInput): Sqm_to_sqft_calculatorOutput {
  ...
  const totalWasteCost = toNumericFormulaValue(values["areaSqft_aux"]);
  const breakdown = {
