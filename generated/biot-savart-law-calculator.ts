...
function evaluateAllFormulas(input: Biot_savart_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.I * input.r * input.zp * input.z1; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.I * input.r * input.zp * input.z1 * (input.z2); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.z2; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
