...
function evaluateAllFormulas(input: Binary_base_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.decimalValue * input.binaryBits * input.octalDigits * input.hexDigits; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.decimalValue * input.binaryBits * input.octalDigits * input.hexDigits; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
