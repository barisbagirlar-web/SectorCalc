...
function evaluateAllFormulas(input: Binary_base_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.decimalValue * input.binaryBits * input.octalDigits * input.hexDigits; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.decimalValue * input.binaryBits * input.octalDigits * input.hexDigits; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
