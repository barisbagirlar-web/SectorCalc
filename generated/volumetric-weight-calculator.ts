...
function evaluateAllFormulas(input: Volumetric_weight_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.length_cm + input.width_cm + input.height_cm; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.length_cm + input.width_cm + input.height_cm; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
