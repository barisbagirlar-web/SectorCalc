...
function evaluateAllFormulas(input: Volumetric_weight_calculatorInput): Record<string, number> {
  ...
  try { const v = input.length_cm * input.width_cm * input.height_cm * input.actual_weight_kg; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.length_cm * input.width_cm * input.height_cm * input.actual_weight_kg; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
