...
function evaluateAllFormulas(input: Von_mises_stress_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sigma_x * input.sigma_y * input.tau_xy * input.yield_strength; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.sigma_x * input.sigma_y * input.tau_xy * input.yield_strength; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
