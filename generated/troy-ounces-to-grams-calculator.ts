...
function evaluateAllFormulas(input: Troy_ounces_to_grams_calculatorInput): Record<string, number> {
  ...
  try { const v = input.troyOunces * input.conversionFactor * input.quantity; results["rawGrams"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawGrams"] = Number.NaN; }
  try { const v = input.troyOunces * input.conversionFactor * input.quantity; results["rawGrams_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["rawGrams_aux"] = Number.NaN; }
  return results;
}
