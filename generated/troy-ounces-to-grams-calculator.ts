...
function evaluateAllFormulas(input: Troy_ounces_to_grams_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.troyOunces * input.conversionFactor * input.quantity; results["rawGrams"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["rawGrams"] = 0; }
  try { const v = input.troyOunces * input.conversionFactor * input.quantity; results["rawGrams_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["rawGrams_aux"] = 0; }
  return results;
}
