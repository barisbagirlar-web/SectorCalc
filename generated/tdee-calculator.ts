...
function evaluateAllFormulas(input: Tdee_calculatorInput): Record<string, number> {
  ...
  try { const v = input.age * input.weight * input.height * (input.bodyFatPercent / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.age * input.weight * input.height * (input.bodyFatPercent / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
