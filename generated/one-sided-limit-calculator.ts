...
function evaluateAllFormulas(input: One_sided_limit_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.mean * input.sd * input.n * input.zp; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mean * input.sd * input.n * input.zp * (input.zgamma * input.direction); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.zgamma * input.direction; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
