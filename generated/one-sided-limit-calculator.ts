...
function evaluateAllFormulas(input: One_sided_limit_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mean * input.sd * input.n * input.zp; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.mean * input.sd * input.n * input.zp * (input.zgamma * input.direction); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.zgamma * input.direction; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
