...
function evaluateAllFormulas(input: Mead_calculatorInput): Record<string, number> {
  ...
  try { const v = ((input.targetABV/100 * input.batchVolumeL) * 0.789 / 0.484) / (input.honeySugarContent/100 * input.yieldFactor/100); results["primary"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["primary"] = Number.NaN; }
  try { const v = input.batchVolumeL; results["breakdown"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["breakdown"] = Number.NaN; }
  return results;
}
