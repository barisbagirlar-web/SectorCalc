...
function evaluateAllFormulas(input: Sugar_cup_to_grams_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cups * 236.588 + input.tablespoons * 14.7868 + input.teaspoons * 4.92892; results["volumeML"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["volumeML"] = Number.NaN; }
  try { const v = input.cups * 236.588 + input.tablespoons * 14.7868 + input.teaspoons * 4.92892; results["volumeML_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["volumeML_aux"] = Number.NaN; }
  return results;
}
