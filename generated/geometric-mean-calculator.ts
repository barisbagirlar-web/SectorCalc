...
function evaluateAllFormulas(input: Geometric_mean_calculatorInput): Record<string, number> {
  ...
  try { const v = input.num1 * input.num2 * input.num3 * input.num4; results["product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["product"] = 0; }
  try { const v = (asFormulaNumber(results["product"])) ** (1/4); results["geometricMean"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["geometricMean"] = 0; }
  return results;
}
...
export function calculateGeometric_mean_calculator(input: Geometric_mean_calculatorInput): Geometric_mean_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["geometricMean"]));
  const breakdown = {
