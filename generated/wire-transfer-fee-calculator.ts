...
function evaluateAllFormulas(input: Wire_transfer_fee_calculatorInput): Record<string, number> {
  ...
  try { const v = input.flatFeeSender + (input.transferAmount * input.percentageFeeSender / 100) + (input.isInternational ? (input.flatFeeIntermediary + input.flatFeeReceiver) : 0); results["totalFee"] = Number.isFinite(v) ? v : 0; } catch { results["totalFee"] = 0; }
  try { const v = input.transferAmount + (results["totalFee"] ?? 0); results["totalCost"] = Number.isFinite(v) ? v : 0; } catch { results["totalCost"] = 0; }
  return results;
}
