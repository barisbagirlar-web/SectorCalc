...
function evaluateAllFormulas(input: Kaplan_meier_calculatorInput): Record<string, number> {
  ...
  try { const v = 1 - (input.events1 / input.risk1); results["survival1"] = Number.isFinite(v) ? v : 0; } catch { results["survival1"] = 0; }
  try { const v = (1 - (input.events1 / input.risk1)) * (1 - (input.events2 / input.risk2)); results["survival2"] = Number.isFinite(v) ? v : 0; } catch { results["survival2"] = 0; }
  return results;
}
...
export function calculateKaplan_meier_calculator(input: Kaplan_meier_calculatorInput): Kaplan_meier_calculatorOutput {
  ...
  const totalWasteCost = values["survival2"] ?? 0;
  const breakdown = {
