...
function evaluateAllFormulas(input: Leagues_to_miles_calculatorInput): Record<string, number> {
  ...
  try { const v = input.leagues * input.leagueType * input.decimals * input.conversionFactorOverride; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.leagues * input.leagueType * input.decimals * input.conversionFactorOverride; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
