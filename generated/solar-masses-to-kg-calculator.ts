...
function evaluateAllFormulas(input: Solar_masses_to_kg_calculatorInput): Record<string, number> {
  ...
  try { const v = input.solarMassValue * input.conversionFactor * input.decimalPlaces * (input.uncertaintyPercent / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.solarMassValue * input.conversionFactor * input.decimalPlaces * (input.uncertaintyPercent / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
