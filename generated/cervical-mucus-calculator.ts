...
function evaluateAllFormulas(input: Cervical_mucus_calculatorInput): Record<string, number> {
  ...
  try { const v = input.temperature * input.shearRate * input.consistencyIndex * input.flowBehaviorIndex; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.temperature * input.shearRate * input.consistencyIndex * input.flowBehaviorIndex * (input.density * input.flowRate * input.pipeDiameter); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.density * input.flowRate * input.pipeDiameter; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
