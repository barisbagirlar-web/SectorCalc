...
function evaluateAllFormulas(input: Property_tax_calculatorInput): Record<string, number> {
  ...
  try { const v = input.marketValue * (input.assessmentRatio / 100); results["assessedValue"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["assessedValue"] = 0; }
  try { const v = ((asFormulaNumber(results["assessedValue"])) * input.millRate) / 1000; results["taxBeforeExemptions"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["taxBeforeExemptions"] = 0; }
  try { const v = (asFormulaNumber(results["taxBeforeExemptions"])) - input.exemptionAmount + input.additionalFees; results["totalTax"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["totalTax"] = 0; }
  return results;
}
