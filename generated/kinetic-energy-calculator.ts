...
function evaluateAllFormulas(input: Kinetic_energy_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mass * input.massUnit * input.velocity * input.velocityUnit; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mass * input.massUnit * input.velocity * input.velocityUnit * (input.quantity); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.quantity; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
