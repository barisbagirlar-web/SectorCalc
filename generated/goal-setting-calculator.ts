...
function evaluateAllFormulas(input: Goal_setting_calculatorInput): Record<string, number> {
  ...
  try { const v = input.targetOEE - input.currentOEE; results["deltaOEE"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["deltaOEE"] = 0; }
  try { const v = (asFormulaNumber(results["deltaOEE"])) / input.timeframe; results["monthlyOEEincrease"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["monthlyOEEincrease"] = 0; }
  return results;
}
...
export function calculateGoal_setting_calculator(input: Goal_setting_calculatorInput): Goal_setting_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["monthlyOEEincrease"]));
  const breakdown = {
