...
function evaluateAllFormulas(input: Km_to_miles_calculatorInput): Record<string, number> {
  ...
  try { const v = input.kilometers * input.conversionFactor * input.roundingDecimals * input.scientificNotation; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.kilometers * input.conversionFactor * input.roundingDecimals * input.scientificNotation; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
