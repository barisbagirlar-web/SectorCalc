...
function evaluateAllFormulas(input: Deck_railing_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalLength - input.numberOfPosts * input.postWidth; results["availableLength"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["availableLength"] = 0; }
  try { const v = input.totalLength - input.numberOfPosts * input.postWidth; results["availableLength_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["availableLength_aux"] = 0; }
  return results;
}
...
export function calculateDeck_railing_calculator(input: Deck_railing_calculatorInput): Deck_railing_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["availableLength_aux"]));
  const breakdown = {
