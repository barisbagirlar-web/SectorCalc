...
function evaluateAllFormulas(input: Password_entropy_calculatorInput): Record<string, number> {
  ...
  try { const v = input.length * input.uppercasePool * input.lowercasePool * input.digitsPool; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.length * input.uppercasePool * input.lowercasePool * input.digitsPool * (input.symbolsPool * input.customChars); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.symbolsPool * input.customChars; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
