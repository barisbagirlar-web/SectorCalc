...
function evaluateAllFormulas(input: Process_economics_npv_calculatorInput): Record<string, number> {
  ...
  try { const v = input.annualCostSavings * ((1 - (1 + input.discountRate/100) ** (-input.projectLife)) / (input.discountRate/100)) - input.initialInvestment; results["netPresentValue"] = Number.isFinite(v) ? v : 0; } catch { results["netPresentValue"] = 0; }
  try { const v = input.initialInvestment / input.annualCostSavings; results["paybackPeriod"] = Number.isFinite(v) ? v : 0; } catch { results["paybackPeriod"] = 0; }
  try { const v = (input.annualCostSavings / input.initialInvestment) * 100; results["annualROI"] = Number.isFinite(v) ? v : 0; } catch { results["annualROI"] = 0; }
  return results;
}
