...
function evaluateAllFormulas(input: Anova_f_statistic_calculatorInput): Record<string, number> {
  ...
  try { const v = input["grup1N"]*(input["grup1"]-results["grandMean"])^2+input["grup2N"]*(input["grup2"]-results["grandMean"])^2; results["ssBetween"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["ssBetween"] = Number.NaN; }
  ...
  try { const v = (input["grup1N"]*(input["grup1"]-results["grandMean"])^2+input["grup2N"]*(input["grup2"]-results["grandMean"])^2)/(2-1); results["msBetween"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["msBetween"] = Number.NaN; }
  ...
  try { const v = results["msBetween"]/results["msWithin"]; results["sonuc"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["sonuc"] = Number.NaN; }
  return results;
}
...
export const Anova_f_statistic_calculatorOutputMeta = {
  primaryKey: "sonuc",
  unit: "F-value",
  breakdownKeys: ["grandMean","ssBetween","ssWithin","msBetween","msWithin"],
} as const;
