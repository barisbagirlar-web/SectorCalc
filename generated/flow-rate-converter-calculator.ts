...
function evaluateAllFormulas(input: Flow_rate_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.massFlow * 8.314 * (input.temperature + 273.15)) / (input.pressure * input.molarMass * 100); results["volumetricFlow"] = Number.isFinite(v) ? v : 0; } catch { results["volumetricFlow"] = 0; }
  try { const v = (input.pressure * input.molarMass * 100) / (8.314 * (input.temperature + 273.15)); results["density"] = Number.isFinite(v) ? v : 0; } catch { results["density"] = 0; }
  try { const v = (input.massFlow * 8.314 * (input.temperature + 273.15)) / (input.pressure * input.molarMass * 6); results["volumetricFlowLmin"] = Number.isFinite(v) ? v : 0; } catch { results["volumetricFlowLmin"] = 0; }
  return results;
}
...
export function calculateFlow_rate_converter_calculator(input: Flow_rate_converter_calculatorInput): Flow_rate_converter_calculatorOutput {
  ...
  const totalWasteCost = values["volumetricFlow"] ?? 0;
  const breakdown = {
