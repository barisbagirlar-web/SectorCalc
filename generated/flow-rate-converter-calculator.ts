...
function evaluateAllFormulas(input: Flow_rate_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.massFlow * 8.314 * (input.temperature + 273.15)) / (input.pressure * input.molarMass * 100); results["volumetricFlow"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["volumetricFlow"] = Number.NaN; }
  try { const v = (input.pressure * input.molarMass * 100) / (8.314 * (input.temperature + 273.15)); results["density"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["density"] = Number.NaN; }
  try { const v = (input.massFlow * 8.314 * (input.temperature + 273.15)) / (input.pressure * input.molarMass * 6); results["volumetricFlowLmin"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["volumetricFlowLmin"] = Number.NaN; }
  return results;
}
