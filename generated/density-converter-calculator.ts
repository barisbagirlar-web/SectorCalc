...
function evaluateAllFormulas(input: Density_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.inputValue * input.fromUnit * input.toUnit * input.precision; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.inputValue * input.fromUnit * input.toUnit * input.precision; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
