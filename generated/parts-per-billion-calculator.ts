...
function evaluateAllFormulas(input: Parts_per_billion_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.solute_mass_mg * input.solution_volume_L * input.solution_density_kg_per_L * input.gas_solute_volume_mL; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.solute_mass_mg * input.solution_volume_L * input.solution_density_kg_per_L * input.gas_solute_volume_mL * (input.total_gas_volume_L); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.total_gas_volume_L; results["adjustment_factor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
