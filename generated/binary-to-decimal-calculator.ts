...
function evaluateAllFormulas(input: Binary_to_decimal_calculatorInput): Record<string, number> {
  ...
  try { const v = '' + input.bit7 + input.bit6 + input.bit5 + input.bit4 + input.bit3 + input.bit2 + input.bit1 + input.bit0; results["binaryString"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["binaryString"] = 0; }
  try { const v = '' + input.bit7 + input.bit6 + input.bit5 + input.bit4 + input.bit3 + input.bit2 + input.bit1 + input.bit0; results["binaryString_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["binaryString_aux"] = 0; }
  return results;
}
...
export function calculateBinary_to_decimal_calculator(input: Binary_to_decimal_calculatorInput): Binary_to_decimal_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["binaryString_aux"]));
  const breakdown = {
