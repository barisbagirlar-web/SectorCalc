...
function evaluateAllFormulas(input: Heat_of_fusion_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.mass * input.specificHeat * (input.meltingTemperature - input.initialTemperature); results["sensibleHeat"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["sensibleHeat"] = 0; }
  ...
  try { const v = (asFormulaNumber(results["sensibleHeat"])) + (asFormulaNumber(results["latentHeat"])); results["totalHeat"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalHeat"] = 0; }
  return results;
}
