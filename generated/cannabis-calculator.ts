...
function evaluateAllFormulas(input: Cannabis_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initial_cannabinoid_mg * input.solvent_volume_ml * input.extraction_time_min * input.temperature_c; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.initial_cannabinoid_mg * input.solvent_volume_ml * input.extraction_time_min * input.temperature_c * (input.k0 * input.Ea); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.k0 * input.Ea; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
