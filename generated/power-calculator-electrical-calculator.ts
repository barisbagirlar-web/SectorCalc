...
function evaluateAllFormulas(input: Power_calculator_electrical_calculatorInput): Record<string, number> {
  ...
  try { const v = input.voltage * input.current * input.powerFactor * input.phaseCount; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.voltage * input.current * input.powerFactor * input.phaseCount * ((input.efficiency / 100) * input.isAC); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.efficiency / 100) * input.isAC; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
