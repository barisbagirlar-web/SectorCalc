...
function evaluateAllFormulas(input: Fret_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.scaleLength + input.compensation) / 2; results["distanceTo12thFret"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["distanceTo12thFret"] = 0; }
  try { const v = (input.scaleLength + input.compensation) / 2; results["distanceTo12thFret_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["distanceTo12thFret_aux"] = 0; }
  return results;
}
...
export function calculateFret_calculator(input: Fret_calculatorInput): Fret_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["distanceTo12thFret_aux"]));
  const breakdown = {
