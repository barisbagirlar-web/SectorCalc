...
function evaluateAllFormulas(input: Preterm_birth_risk_calculatorInput): Record<string, number> {
  ...
  try { const v = -3 + 0.1*input.age - 0.1*input.cervicalLength + 0.01*input.fetalFibronectin + 1.5*input.previousPreterm; results["linearCombo"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["linearCombo"] = Number.NaN; }
  try { const v = -3 + 0.1*input.age - 0.1*input.cervicalLength + 0.01*input.fetalFibronectin + 1.5*input.previousPreterm; results["linearCombo_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["linearCombo_aux"] = Number.NaN; }
  return results;
}
