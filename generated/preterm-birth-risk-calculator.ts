...
function evaluateAllFormulas(input: Preterm_birth_risk_calculatorInput): Record<string, number> {
  ...
  try { const v = -3 + 0.1*input.age - 0.1*input.cervicalLength + 0.01*input.fetalFibronectin + 1.5*input.previousPreterm; results["linearCombo"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["linearCombo"] = 0; }
  try { const v = -3 + 0.1*input.age - 0.1*input.cervicalLength + 0.01*input.fetalFibronectin + 1.5*input.previousPreterm; results["linearCombo_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["linearCombo_aux"] = 0; }
  return results;
}
...
export function calculatePreterm_birth_risk_calculator(input: Preterm_birth_risk_calculatorInput): Preterm_birth_risk_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["linearCombo_aux"]));
  const breakdown = {
