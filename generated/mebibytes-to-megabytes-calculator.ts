...
function evaluateAllFormulas(input: Mebibytes_to_megabytes_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mebibytes * input.binaryBase * input.metricBase * input.precision; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.mebibytes * input.binaryBase * input.metricBase * input.precision; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
