...
function evaluateAllFormulas(input: Cash_out_refinance_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.existingMortgageBalance + input.cashOutAmount + input.closingCosts; results["newLoanAmount"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["newLoanAmount"] = 0; }
  ...
  try { const v = (asFormulaNumber(results["monthlyPayment"])) * (asFormulaNumber(results["numberOfPayments"])); results["totalPayment"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalPayment"] = 0; }
  try { const v = (asFormulaNumber(results["totalPayment"])) - (asFormulaNumber(results["newLoanAmount"])); results["totalInterest"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["totalInterest"] = 0; }
  try { const v = ((asFormulaNumber(results["newLoanAmount"])) / input.propertyValue) * 100; results["loanToValue"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["loanToValue"] = 0; }
  return results;
}
