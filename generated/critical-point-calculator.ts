...
function evaluateAllFormulas(input: Critical_point_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = 4*input.b*input.b - 12*input.a*input.c; results["discriminant"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["discriminant"] = 0; }
  try { const v = 4*input.b*input.b - 12*input.a*input.c; results["discriminant_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["discriminant_aux"] = 0; }
  return results;
}
