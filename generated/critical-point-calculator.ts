...
function evaluateAllFormulas(input: Critical_point_calculatorInput): Record<string, number> {
  ...
  try { const v = 4*input.b*input.b - 12*input.a*input.c; results["discriminant"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["discriminant"] = 0; }
  try { const v = 4*input.b*input.b - 12*input.a*input.c; results["discriminant_aux"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["discriminant_aux"] = 0; }
  return results;
}
...
export function calculateCritical_point_calculator(input: Critical_point_calculatorInput): Critical_point_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["discriminant_aux"]));
  const breakdown = {
