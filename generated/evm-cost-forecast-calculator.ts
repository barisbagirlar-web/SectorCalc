...
function evaluateAllFormulas(input: Evm_cost_forecast_calculatorInput): Record<string, number> {
  ...
  try { const v = input.bac * input.ev * input.ac * input.pv; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.bac * input.ev * input.ac * input.pv * ((input.percentComplete / 100) * input.cpi * input.spi); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.percentComplete / 100) * input.cpi * input.spi; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
...
export const Evm_cost_forecast_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
