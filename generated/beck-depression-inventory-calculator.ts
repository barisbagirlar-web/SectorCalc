...
function evaluateAllFormulas(input: Beck_depression_inventory_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cognitive_affective * input.somatic * input.performance * input.guilt; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.cognitive_affective * input.somatic * input.performance * input.guilt; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
