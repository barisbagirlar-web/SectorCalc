...
function evaluateAllFormulas(input: Beck_depression_inventory_calculatorInput): Record<string, number> {
  ...
  try { const v = input.cognitive_affective * input.somatic * input.performance * input.guilt; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.cognitive_affective * input.somatic * input.performance * input.guilt; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
