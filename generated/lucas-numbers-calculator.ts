...
function evaluateAllFormulas(input: Lucas_numbers_calculatorInput): Record<string, number> {
  ...
  try { const v = input.startIndex * input.endIndex * input.L0 * input.L1; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.startIndex * input.endIndex * input.L0 * input.L1 * (input.showSequence); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.showSequence; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
