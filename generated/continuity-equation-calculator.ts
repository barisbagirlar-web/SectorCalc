...
function evaluateAllFormulas(input: Continuity_equation_calculatorInput): Record<string, number> {
  ...
  try { const v = input.diameter * input.velocity * input.density * input.safetyFactor; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.diameter * input.velocity * input.density * input.safetyFactor * (input.outputUnitMultiplier); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.outputUnitMultiplier; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
