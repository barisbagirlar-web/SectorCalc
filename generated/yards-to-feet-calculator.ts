...
function evaluateAllFormulas(input: Yards_to_feet_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.yards * input.conversionFactor; results["yards___conversionFactor"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["yards___conversionFactor"] = 0; }
