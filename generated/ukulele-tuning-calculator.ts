...
function evaluateAllFormulas(input: Ukulele_tuning_calculatorInput): Record<string, number> {
  ...
  try { const v = input.scaleLength * input.stringDiameter * input.density * input.frequency; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.scaleLength * input.stringDiameter * input.density * input.frequency; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
