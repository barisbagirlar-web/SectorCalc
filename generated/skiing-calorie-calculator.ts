...
function evaluateAllFormulas(input: Skiing_calorie_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * (input.duration / 60) * input.met; results["totalCalories"] = Number.isFinite(v) ? v : 0; } catch { results["totalCalories"] = 0; }
  try { const v = input.weight * input.met / 60; results["caloriesPerMinute"] = Number.isFinite(v) ? v : 0; } catch { results["caloriesPerMinute"] = 0; }
  try { const v = input.weight * input.met; results["caloriesPerHour"] = Number.isFinite(v) ? v : 0; } catch { results["caloriesPerHour"] = 0; }
  return results;
}
