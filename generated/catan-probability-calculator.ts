...
function evaluateAllFormulas(input: Catan_probability_calculatorInput): Record<string, number> {
  ...
  try { const v = input.diceCount * input.diceSides * input.targetSum * input.settlementCount; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.diceCount * input.diceSides * input.targetSum * input.settlementCount * (input.rolls); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.rolls; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
