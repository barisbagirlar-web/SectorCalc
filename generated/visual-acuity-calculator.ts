...
function evaluateAllFormulas(input: Visual_acuity_calculatorInput): Record<string, number> {
  ...
  try { const v = (input.distance) * (input.letterHeight) * (input.snellenNumerator) * (input.overrideDenom); results["H_ft"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["H_ft"] = Number.NaN; }
  try { const v = (input.distance) * (input.letterHeight) * (input.snellenNumerator); results["H_ft_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["H_ft_aux"] = Number.NaN; }
  return results;
}
