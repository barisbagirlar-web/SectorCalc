...
function evaluateAllFormulas(input: Circadian_rhythm_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = ((input.currentTime - input.wakeTime + 24) % 24); results["hoursSinceWake"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["hoursSinceWake"] = 0; }
  try { const v = ((input.currentTime - input.wakeTime + 24) % 24); results["hoursSinceWake_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["hoursSinceWake_aux"] = 0; }
  return results;
}
