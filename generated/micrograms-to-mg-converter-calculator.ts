...
function evaluateAllFormulas(input: Micrograms_to_mg_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.micrograms / 1000; results["milligramsPerUnit"] = Number.isFinite(v) ? v : 0; } catch { results["milligramsPerUnit"] = 0; }
  try { const v = (input.micrograms / 1000) * input.numberOfUnits * (input.purity / 100) * input.safetyFactor; results["totalMilligrams"] = Number.isFinite(v) ? v : 0; } catch { results["totalMilligrams"] = 0; }
  try { const v = input.micrograms * input.numberOfUnits * (input.purity / 100) * input.safetyFactor; results["totalMicrograms"] = Number.isFinite(v) ? v : 0; } catch { results["totalMicrograms"] = 0; }
  return results;
}
...
export function calculateMicrograms_to_mg_converter_calculator(input: Micrograms_to_mg_converter_calculatorInput): Micrograms_to_mg_converter_calculatorOutput {
  ...
  const totalWasteCost = values["totalMilligrams"] ?? 0;
  const breakdown = {
