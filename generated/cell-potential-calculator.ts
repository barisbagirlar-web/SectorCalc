...
function evaluateAllFormulas(input: Cell_potential_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.cathodeStandardPotential - input.anodeStandardPotential; results["standardCellPotential"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["standardCellPotential"] = 0; }
  try { const v = input.cathodeStandardPotential - input.anodeStandardPotential; results["standardCellPotential_aux"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["standardCellPotential_aux"] = 0; }
  return results;
}
