...
function evaluateAllFormulas(input: Baby_percentile_calculatorInput): Record<string, number> {
  ...
  try { const v = input.age * input.weight * input.length * input.head; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.age * input.weight * input.length * input.head * (input.gender); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.gender; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["adjustment_factor"] = 0; }
  return results;
}
