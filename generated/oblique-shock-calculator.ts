...
function evaluateAllFormulas(input: Oblique_shock_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.p01 * (1 + (input.gamma - 1) / 2 * input.mach ** 2) ** (-input.gamma / (input.gamma - 1)); results["p1"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["p1"] = 0; }
  try { const v = input.T01 * (1 + (input.gamma - 1) / 2 * input.mach ** 2) ** (-1); results["T1"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["T1"] = 0; }
  return results;
}
