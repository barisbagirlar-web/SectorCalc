...
function evaluateAllFormulas(input: Kg_to_lb_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.mass_kg * (input.measurement_uncertainty / 100); results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.mass_kg * (input.measurement_uncertainty / 100); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
