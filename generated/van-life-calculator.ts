...
function evaluateAllFormulas(input: Van_life_calculatorInput): Record<string, number> {
  ...
  try { const v = input.dailyEnergyWh / (input.sunPeakHours * input.systemEfficiency / 100); results["requiredTotalWattage"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["requiredTotalWattage"] = 0; }
  try { const v = (input.dailyEnergyWh * input.daysAutonomy) / (input.batteryVoltage * (input.depthOfDischarge / 100)); results["requiredBatteryCapacityAh"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["requiredBatteryCapacityAh"] = 0; }
  return results;
}
...
export function calculateVan_life_calculator(input: Van_life_calculatorInput): Van_life_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["requiredTotalWattage"]));
  const breakdown = {
