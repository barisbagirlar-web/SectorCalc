...
function evaluateAllFormulas(input: Days_sales_outstanding_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = (input.beginningAR + input.endingAR) / 2; results["averageAR"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["averageAR"] = 0; }
  try { const v = ((input.netCreditSales !== 0 ? ((asFormulaNumber(results["averageAR"])) / input.netCreditSales * input.periodDays) : null) ? 1 : 0); results["dso"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["dso"] = 0; }
  return results;
}
