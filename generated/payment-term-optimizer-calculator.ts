...
function evaluateAllFormulas(input: Payment_term_optimizer_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.annual_invoice_count * input.avg_invoice_value; results["base_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["base_cost"] = 0; }
  try { const v = input.annual_invoice_count * input.avg_invoice_value * (1 + (input.early_payment_discount_pct / 100)); results["adjusted_cost"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["adjusted_cost"] = 0; }
  try { const v = input.annual_invoice_count * input.avg_invoice_value * (1 + (input.early_payment_discount_pct / 100)) * (input.current_terms_days); results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  try { const v = input.current_terms_days; results["factor_current_terms_days"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["factor_current_terms_days"] = 0; }
  return results;
}
