...
function evaluateAllFormulas(input: Fha_loan_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.propertyTaxAnnual / 12; results["propertyTaxMonthly"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["propertyTaxMonthly"] = 0; }
  try { const v = input.hazardInsuranceAnnual / 12; results["hazardInsuranceMonthly"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["hazardInsuranceMonthly"] = 0; }
  try { const v = (input.loanAmount * input.mipAnnualRate / 100) / 12; results["mipMonthly"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["mipMonthly"] = 0; }
  return results;
}
