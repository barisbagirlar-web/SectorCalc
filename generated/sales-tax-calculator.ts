...
function evaluateAllFormulas(input: Sales_tax_calculatorInput): Record<string, number> {
  ...
  try { const v = input.include_shipping * input.net_sales; results["base_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["base_cost"] = Number.NaN; }
  try { const v = input.include_shipping * input.net_sales * (1 + (input.tax_rate / 100)); results["adjusted_cost"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjusted_cost"] = Number.NaN; }
  try { const v = input.include_shipping * input.net_sales * (1 + (input.tax_rate / 100)) * ((input.exempt_ratio / 100)); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = (input.exempt_ratio / 100); results["factor_exempt_ratio"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["factor_exempt_ratio"] = Number.NaN; }
  return results;
}
