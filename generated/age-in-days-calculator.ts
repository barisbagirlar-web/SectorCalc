...
function evaluateAllFormulas(input: Age_in_days_calculatorInput): Record<string, number> {
  ...
  try { const v = input.birthYear * input.birthMonth * input.birthDay * input.refYear; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.birthYear * input.birthMonth * input.birthDay * input.refYear * (input.refMonth * input.refDay); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.refMonth * input.refDay; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
