...
function evaluateAllFormulas(input: _2d_inverse_kinematics_calculatorInput): Record<string, number> {
  ...
  try { const v = Math.atan2(input.hedefY, input.hedefX) - Math.atan2(input.kol2Uzunluk*Math.sin((toNumericFormulaValue(results["aci2"]))), input.kol1Uzunluk + input.kol2Uzunluk*Math.cos((toNumericFormulaValue(results["aci2"])))); results["sonuc"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["sonuc"] = Number.NaN; }
  return results;
}
