...
function evaluateAllFormulas(input: Confidence_interval_mean_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.sampleMean * input.standardDeviation * input.sampleSize * input.criticalValue; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.sampleMean * input.standardDeviation * input.sampleSize * input.criticalValue; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
