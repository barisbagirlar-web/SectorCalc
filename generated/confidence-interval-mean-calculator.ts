...
function evaluateAllFormulas(input: Confidence_interval_mean_calculatorInput): Record<string, number> {
  ...
  try { const v = input.sampleMean * input.standardDeviation * input.sampleSize * input.criticalValue; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.sampleMean * input.standardDeviation * input.sampleSize * input.criticalValue; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
