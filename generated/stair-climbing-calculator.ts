...
function evaluateAllFormulas(input: Stair_climbing_calculatorInput): Record<string, number> {
  ...
  try { const v = input.totalRise * input.treadDepth * input.riserHeightTarget * input.stairWidth; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.totalRise * input.treadDepth * input.riserHeightTarget * input.stairWidth; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
