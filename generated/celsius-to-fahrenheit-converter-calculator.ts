...
function evaluateAllFormulas(input: Celsius_to_fahrenheit_converter_calculatorInput): Record<string, number> {
  ...
  try { const v = input.celsius_input * input.sensor_accuracy * input.calibration_offset; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.celsius_input * input.sensor_accuracy * input.calibration_offset; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
