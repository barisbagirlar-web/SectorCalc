...
function evaluateAllFormulas(input: Celsius_to_fahrenheit_converter_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.celsius_input + input.sensor_accuracy + input.calibration_offset; results["result_copy"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result_copy"] = 0; }
  return results;
}
