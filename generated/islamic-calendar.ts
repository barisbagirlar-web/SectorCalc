...
function evaluateAllFormulas(input: Islamic_calendarInput): Record<string, number | string> {
  ...
  try { const v = input.gregorianYear * input.gregorianMonth * input.gregorianDay * input.adjustmentDays; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.gregorianYear * input.gregorianMonth * input.gregorianDay * input.adjustmentDays; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
