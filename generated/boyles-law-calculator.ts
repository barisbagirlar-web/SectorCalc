...
function evaluateAllFormulas(input: Boyles_law_calculatorInput): Record<string, number> {
  ...
  try { const v = input.initialPressure * input.initialVolume * input.finalPressure * input.finalVolume; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.initialPressure * input.initialVolume * input.finalPressure * input.finalVolume; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
