...
function evaluateAllFormulas(input: Energy_consumption_cost_report_calculatorInput): Record<string, number> {
  ...
  try { const v = input.total_energy_kwh * input.peak_demand_kw * (input.tariff_rate_per_kwh / 100) * input.demand_charge_per_kw; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.total_energy_kwh * input.peak_demand_kw * (input.tariff_rate_per_kwh / 100) * input.demand_charge_per_kw * (input.production_units * input.renewable_kwh); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.production_units * input.renewable_kwh; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
...
export const Energy_consumption_cost_report_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "USD",
