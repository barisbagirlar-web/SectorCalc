...
function evaluateAllFormulas(input: Wilcoxon_signed_rankInput): Record<string, number> {
  ...
  try { const v = input.after - input.before; results["differences"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["differences"] = Number.NaN; }
  try { const v = input.after - input.before; results["differences_aux"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["differences_aux"] = Number.NaN; }
  return results;
}
