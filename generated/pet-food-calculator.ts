...
function evaluateAllFormulas(input: Pet_food_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * input.activityFactor * input.caloricDensity * input.mealsPerDay; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.weight * input.activityFactor * input.caloricDensity * input.mealsPerDay; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
