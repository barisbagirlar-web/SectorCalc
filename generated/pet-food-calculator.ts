...
function evaluateAllFormulas(input: Pet_food_calculatorInput): Record<string, number> {
  ...
  try { const v = input.weight * input.activityFactor * input.caloricDensity * input.mealsPerDay; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.weight * input.activityFactor * input.caloricDensity * input.mealsPerDay; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
