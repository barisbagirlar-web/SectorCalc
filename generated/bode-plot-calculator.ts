...
function evaluateAllFormulas(input: Bode_plot_calculatorInput): Record<string, number> {
  ...
  try { const v = input.gain * input.poleFreq * input.zeroFreq * input.freq; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.gain * input.poleFreq * input.zeroFreq * input.freq; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
