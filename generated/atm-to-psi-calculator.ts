...
function evaluateAllFormulas(input: Atm_to_psi_calculatorInput): Record<string, number> {
  ...
  try { const v = input.atm * input.conversionFactor; results["rawPsi"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["rawPsi"] = 0; }
  try { const v = (asFormulaNumber(results["rawPsi"])) + input.offset; results["psiWithOffset"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["psiWithOffset"] = 0; }
  try { const v = input.atm; results["atm"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["atm"] = 0; }
  ...
  try { const v = input.offset; results["offset"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["offset"] = 0; }
  return results;
}
...
export function calculateAtm_to_psi_calculator(input: Atm_to_psi_calculatorInput): Atm_to_psi_calculatorOutput {
  ...
  const totalWasteCost = Math.max(0, toNumericFormulaValue(values["offset"]));
  const breakdown = {
