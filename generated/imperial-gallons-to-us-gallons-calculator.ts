...
function evaluateAllFormulas(input: Imperial_gallons_to_us_gallons_calculatorInput): Record<string, number> {
  ...
  try { const v = input.imperialGallons * input.batchSize * (input.tolerance / 100) * input.temperature; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.imperialGallons * input.batchSize * (input.tolerance / 100) * input.temperature; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
