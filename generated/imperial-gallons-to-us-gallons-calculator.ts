...
function evaluateAllFormulas(input: Imperial_gallons_to_us_gallons_calculatorInput): Record<string, number | string> {
  ...
  try { const v = input.imperialGallons * input.batchSize * (input.tolerance / 100) * input.temperature; results["normalized_product"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.imperialGallons * input.batchSize * (input.tolerance / 100) * input.temperature; results["result"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["result"] = 0; }
  return results;
}
