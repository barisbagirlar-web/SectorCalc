...
function evaluateAllFormulas(input: Scs_method_calculatorInput): Record<string, number> {
  ...
  try { const v = input.rainfall * input.curveNumber * input.amc * input.iaRatio; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.rainfall * input.curveNumber * input.amc * input.iaRatio; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
