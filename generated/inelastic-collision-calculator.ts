...
function evaluateAllFormulas(input: Inelastic_collision_calculatorInput): Record<string, number> {
  ...
  try { const v = ( (input.mass1 - input.restitutionCoefficient * input.mass2) * input.velocity1Initial + input.mass2 * (1 + input.restitutionCoefficient) * input.velocity2Initial ) / (input.mass1 + input.mass2); results["finalVelocity1"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["finalVelocity1"] = Number.NaN; }
  try { const v = ( input.mass1 * (1 + input.restitutionCoefficient) * input.velocity1Initial + (input.mass2 - input.restitutionCoefficient * input.mass1) * input.velocity2Initial ) / (input.mass1 + input.mass2); results["finalVelocity2"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["finalVelocity2"] = Number.NaN; }
  return results;
}
