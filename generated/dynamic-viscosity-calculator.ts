...
function evaluateAllFormulas(input: Dynamic_viscosity_calculatorInput): Record<string, number> {
  ...
  try { const v = input.tRef * input.muRef * input.activationEnergy * input.temp; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["normalized_product"] = 0; }
  try { const v = input.tRef * input.muRef * input.activationEnergy * input.temp; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : 0; } catch { results["result"] = 0; }
  return results;
}
