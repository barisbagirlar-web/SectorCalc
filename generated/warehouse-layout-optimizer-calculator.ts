...
function evaluateAllFormulas(input: Warehouse_layout_optimizer_calculatorInput): Record<string, number> {
  ...
  try { const v = input.total_floor_area * (input.storage_area_percent / 100) * input.aisle_width * input.avg_pallet_size; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.total_floor_area * (input.storage_area_percent / 100) * input.aisle_width * input.avg_pallet_size * (input.avg_inventory_turns * input.labor_cost_per_hour * input.equipment_cost_per_hour); results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  try { const v = input.avg_inventory_turns * input.labor_cost_per_hour * input.equipment_cost_per_hour; results["adjustment_factor"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["adjustment_factor"] = Number.NaN; }
  return results;
}
...
export const Warehouse_layout_optimizer_calculatorOutputMeta = {
  primaryKey: "result",
  unit: "%",
