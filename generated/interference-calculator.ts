...
function evaluateAllFormulas(input: Interference_calculatorInput): Record<string, number> {
  ...
  try { const v = input.amplitude1 * input.amplitude2 * input.wavelength * input.pathDifference; results["normalized_product"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["normalized_product"] = Number.NaN; }
  try { const v = input.amplitude1 * input.amplitude2 * input.wavelength * input.pathDifference; results["result"] = typeof v === "number" && Number.isFinite(v) ? v : Number.NaN; } catch { results["result"] = Number.NaN; }
  return results;
}
