...
function evaluateAllFormulas(input: Barns_to_sqm_calculatorInput): Record<string, number | string> {
  const results: Record<string, number | string> = {};
  try { const v = input.barns * 1e-28; results["rawConversion"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["rawConversion"] = 0; }
  try { const v = (input.barns * 1e-28) * input.multiplier + input.constant; results["scaled"] = typeof v === "number" ? (Number.isFinite(v) ? v : 0) : typeof v === "string" ? v : 0; } catch { results["scaled"] = 0; }
  return results;
}
