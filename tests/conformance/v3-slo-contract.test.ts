...
describe('phase 12 SRE/SLO code contract', () => {
  it('accepts empty code-only evidence', () => expect(validateSlo({ rows: [], assets: [] }, config).errors).toEqual([]));
  ...
  it('reports measured rows without an evidence reference', () => {
    const r = validateSlo({ rows: [{ id: 'inp', measured: 150, thresholdRef: 'thresholds.inpP75Ms', status: 'PASS', issueOpened: false, consecutiveViolations: 0, freezeEscalated: false, evidenceRef: null }], assets: [] }, config);
    expect(r.infos.join(' ')).toContain('INV-12.4');
  });
