...
describe('SEO registry invariant seal', () => {
  it('keeps the production registry internally valid', () => {
    expect(validateRegistryInvariants()).toEqual([]);
  });

  it('negative fixture proves duplicate canonical ownership is rejected', () => {
    const fixture = [
      { canonicalPath: '/calculator/example' },
      { canonicalPath: '/calculator/example' },
    ];
    expect(duplicateCanonicalErrors(fixture)).toEqual([
      'duplicate canonical: /calculator/example',
    ]);
  });
